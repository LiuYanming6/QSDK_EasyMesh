 ...
 *
 * $Revision: 1.1 $
 * $Id: CPEWrapper.c,v 1.1 2012/05/10 17:38:00 dmounday Exp $
 *----------------------------------------------------------------------*/
...
const char *cpeReadLANStats(int field){
	char *r = cpeReadFileField("/proc/net/dev", cpeNetIPState.name, field );
	if ( r == NULL )
		return "";
	return r;
}
...
void cpeRefreshCPEData(CPEState *cpe) {
	...
	const char *format;
	NetIPState *lanState;
	if ((lanState = cpeGetNetIPInfo(0)) != NULL) {
		if (!eqInIPAddr(&cpe->ipAddress, &lanState->ip)) {
		...
		cpe->ipAddress = lanState->ip;
		if ( cpe->ipAddress.inFamily != 0){
			memcpy(cpe->MACAddress, lanState->mac, 6);
			format = (lanState->ip.inFamily == AF_INET6) ? "http://[%s]:%d%s"
					: "http://%s:%d%s";
			snprintf(buf, sizeof(buf), format, writeInIPAddr(&lanState->ip),
