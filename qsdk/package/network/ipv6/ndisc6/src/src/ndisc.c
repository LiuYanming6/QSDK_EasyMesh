...
typedef struct _ra_info_t
{
	unsigned int m_flag;
	unsigned int o_flag;
	unsigned int default_router_lifetime;
	char rdnss[150];	// max 3 dns servers.
	char dnssl[150];
...
parseprefix (const struct nd_opt_prefix_info *pi, size_t optlen, bool verbose)
{
	...
	if ((pi->nd_opt_pi_flags_reserved & ND_OPT_PI_FLAG_AUTO) && (pi->nd_opt_pi_valid_time))
	{
		strncpy(ra_info.prefix, str, INET6_ADDRSTRLEN-1);
		ra_info.prefix_length = pi->nd_opt_pi_prefix_len;
	}
	return 0;
}
...
parserdnss (const uint8_t *opt)
{
	...
	char dns_list[INET6_ADDRSTRLEN*5+4] = {0};	// hardcode: max 5 dns servers.
	...
	{
		strncpy(ra_info.rdnss, dns_list, sizeof(ra_info.rdnss)-1);
	}
...
static int
parsednssl (const uint8_t *opt, bool verbose)
{
	const uint8_t *base;
	uint8_t optlen = opt[1];
	if (optlen < 2)
...
//				printf ("%s%s", str, base[i] ? "." : "");
				strcat (search_domain, str);
				strcat (search_domain, base[i] ? "." : "");
		...
		strncpy(ra_info.dnssl, search_domain, sizeof(ra_info.dnssl)-1);
	}
	return 0;
}
...
#endif
	{
		unsigned v = ra->nd_ra_flags_reserved;;

		ra_info.default_router_lifetime = ntohs (ra->nd_ra_router_lifetime);
		ra_info.m_flag = (v & ND_RA_FLAG_MANAGED) ? 1:0;
		ra_info.o_flag = (v & ND_RA_FLAG_OTHER) ? 1:0;
	}
         ...
         unsigned flags)
{
	...
	for (;;)
	{
...
#endif
		val = poll (&(struct pollfd){ .fd = fd, .events = POLLIN }, 1, 1*1000);
...

		{
			...
			if (strlen(ra_info.rdnss) == 0) strncpy(ra_info.rdnss, "NULL", sizeof(ra_info.rdnss)-1);
			if (strlen(ra_info.dnssl) == 0) strncpy(ra_info.dnssl, "NULL", sizeof(ra_info.dnssl)-1);
			if (strlen(ra_info.prefix) == 0) strncpy(ra_info.prefix, "NULL", sizeof(ra_info.prefix)-1);

			if(ra_info.default_router_lifetime == 0)
            {
                char cmd_result[128]={0};
                cmd_popen("killall -SIGUSR2 cwmpc" , cmd_result);
            }
#if 0
			snprintf (cmdline_buffer, sizeof (cmdline_buffer)-1
				, SYSIFD_CLI_CMD " " SYSIFD_IPV6_RA_OBTAINED " %d %d %u '%s' '%s' '%s'"
				, ra_info.m_flag, ra_info.o_flag, ra_info.default_router_lifetime
				, ra_info.rdnss, ra_info.dnssl
				, ra_info.prefix
				);

#endif
#ifdef DEBUG
			printf ("[%s:%d] buf=<%s>.\n", __func__, __LINE__, cmdline_buffer);
#endif
