...
typedef adf_nbuf_t (*byp_filter_fn_t) (void *ctx, 
                                struct atd_host_vap *vap, adf_nbuf_t skb);

/* Disabling all if checks for optimization */
#define byp_dbg_print(x...)         
/* #define byp_dbg_print(x...)     atd_trace(ATD_DEBUG_FUNCTRACE, (x))       */
/* #define byp_dbg_print(x...)         adf_os_print(x)   */
...
typedef struct byp_sc_s {
    struct atd_host_dev      *atd_handle;    /**< Parent ATD Handle */
...
/** 
 * @brief Check if LAN Bypass Mode is enabled
 * 
 * @param ctx
 * 
 * @return 
 ...
 * @return 
 */
#define         atd_byp_is_mcast(nbuf)         (((nbuf)->data[0]) & 0x01)

#else /** OFFLOAD_LAN_BYPASS */

#define         atd_byp_init(x)             (x)

#define         atd_byp_cb(x, y)            (A_STATUS_OK)

#define         atd_byp_cleanup(x)          do {} while(0) 

#define         atd_byp_from_host(x, y, z)  (1)

#define         atd_byp_en(x)               (0)

#define         atd_byp_is_mcast(x)         (0)
