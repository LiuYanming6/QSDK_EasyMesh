...
void pbcHyfi20GetPipeMsgCB(s32 fd, void *eloop_ctx, void *sock_ctx) {
    ...
    dprintf(MSG_DEBUG, "%s - Got event: %s\n", __func__, buf);
    ...
    } else if (strncmp(buf, BACKHAUL_STA_MAC, strlen(BACKHAUL_STA_MAC)) == 0) {
        apac_mib_backhaul_sta_callback(pData, buf);
    } else {
        dprintf(MSG_ERROR, "%s - Event not found: %s\n", __func__, buf);
    }
...
int pbcGetBssid(char *ifName, u8 *bssid) {
    ...
    if (wlanIfWd->ops->getBSSID(wlanIfWd->ctx, ifName, (struct ether_addr *)bssid) < 0 ) {
...
int pbcGetName(char *ifName, size_t size, char *name) {
    ...
    if (wlanIfWd->ops->getName(wlanIfWd->ctx, ifName, name) < 0) {
        dprintf(MSG_ERROR, "%s: ioctl() failed, ifName: %s.\n", __func__, ifName);
        goto err;
    }

    dprintf(MSG_DEBUG, "%s, Interface %s, name: %s(%s) \n", __func__, ifName, name, name);
...
int pbcReceiveEventNotificationMsg(apacHyfi20Data_t *pData, u8 *frame, size_t frameLen){
    ...
    u8 isfrag = msg->ieee1905Header.flags & IEEE1905_HEADER_FLAG_LAST_FRAGMENT;
    const int num_tlv = 2;
