...
int apacHyfi20CheckMid(u8 *alid, u16 mid, u8 isLastfrag) {
    ...
    if( !delta ||
       ( ( mid + APAC_MID_DELTA > ptrEntry->src_mid ) &&
         ( delta > ( 1 << 15 ) ) ) ) {
        dprintf(MSG_DEBUG, "%s: mid Order Mismatch!! mid %05d, saved %05d", __func__, mid, ptrEntry->src_mid );
        return -1;
    } else {
        ptrEntry->src_mid = mid;
        return 0;
...
apacBool_e apacHyfi20ValidateTlvList(p1905TlvCheck_t *list, const int num, apacBool_e isMap) {
    int i;

    for (i = 0; i < num; i++) {
        if (list[i].valid == APAC_FALSE) {
             if (!isMap && (list[i].tlvType == IEEE1905_TLV_TYPE_SEARCHED_SERVICE ||
                            list[i].tlvType == IEEE1905_TLV_TYPE_SUPPORTED_SERVICE)) {
                 continue;
             }
...
int apacHyfi20ReceiveRenewalE(apacHyfi20Data_t *pData, u8 *msg, u32 msgLen) {
    ...
    if (!apacHyfi20ValidateTlvList(tlvList, num_tlv, apacHyfiMapIsEnabled(HYFI20ToMAP(pData)))) {
...
int apacHyfi20ReceiveSearchR(apacHyfi20Data_t *ptrData, u8 *msg, u32 msgLen) {
    ...
    /* Check if all required TLVs have been received */
    if (!apacHyfi20ValidateTlvList(tlvList, num_tlv, apacHyfiMapIsEnabled(HYFI20ToMAP(ptrData)))) {
        dprintf(MSG_ERROR, "required TLV type missing in the message!\n");
        return -1;
    }
    if (apacHyfiMapIsEnabled(HYFI20ToMAP(ptrData))) {
    ...
    for (freq = 0; freq < APAC_NUM_WIFI_FREQ; freq++) {
        if ((ptrData->ap[freq].valid || apacHyfiMapIsEnabled(HYFI20ToMAP(ptrData))) &&
            isFreqRequested[freq]) {
...
int apacHyfi20SendResponseR(apacHyfi20Data_t *pData, u8 *dest_mac, u8 freq, u16
        mid, apacBool_e isMapCapable)
{
    u8 tlvValue;
    u32 frameLen = IEEE1905_FRAME_MIN_LEN;
    u8 *frame = apacHyfi20GetXmitBuf();
    ...
    while (curWpsMsgLen > 0) {
        ...
        if (apacHyfiMapIsEnabled(HYFI20ToMAP(pData))) {
            if(flags & IEEE1905_HEADER_FLAG_LAST_FRAGMENT) {
                if (pData->config.role == APAC_ENROLLEE &&
                        sess->state == APAC_WPS_E_M1_SENT) {
                    TLV = ieee1905MapAddBasicRadioTLV(TLV, &frameLen,
                            apacS.bandInRegistration,
                            pData);
                }
            }
        }
