...
int interfaceAddVlan(const char* Interface, const int VlanID)
{
    ...
    if (strlcpy(if_request.device1, Interface,
        sizeof(if_request.device1)) >= sizeof(if_request.device1)) {
        return -1;
    }
        ...
        struct NetDeviceStats* NDStats)
{
    ...
    while(fgets(buf, sizeof(buf), f)) {
        ...
        if(n == 2) {
            ...
            // trim left space
            int i, j=0;
            for(i=0; i<strlen(col) && j<(BUFSIZ-1); i++) {
                if(col[i] == ' ') continue;
                name[j] = col[i];
                j++;
            }
...
int interfaceGetDefaultGateway(char *DefaultGateway)
{
	...
	/* TODO - Use the destination size to check before copying. */
	if (strlcpy(DefaultGateway, route[2],
		sizeof(route[2])) >= sizeof(route[2])) {  /* get defaultGateway ip */
		return -1;
	}
				 ...
				 struct NetDeviceStats* NDStats)
{
	...
	if (strlcat(cmd, Interface, sizeof(cmd)) >= sizeof(cmd)) {
