...
void print_usage(void)
{
    ...
    printf("o : enable/disable optional secondary 40 MHz occupancy check\n"
           "    while bringing up 160/80+80 MHz (1:enable 0:disable)\n");
    printf(LINESTR);
...
int icm_init(ICM_DEV_INFO_T* pdev)
{
    ...
    for (i = 0; i < conf->num_radios; i++) {
        ...
        spectral_is_supported = icm_is_spectral_supported(picm);

        if (spectral_is_supported) {
        ...
        /* Do not register for JET since spectral is not supported*/
        if (spectral_is_supported)
        ...
        picm->enable_sec40_occ_check = conf->enable_sec40_occ_check;
...
static void icm_config_defaults(ICM_CONFIG_T* pconfig)
{
    ...
    pconfig->enable_sec40_occ_check = ICM_ENABLE_SEC40_OCC_CHECK_DEFAULT;
}
...
int main(int argc, char* argv[])
{
    ...
    while((optc = getopt(argc, argv, "efhis:tvq:u:o:")) != -1) {
        switch (optc) {
            case 'e':
                pconf->daemon = TRUE;
                break;
            ...
            case 'v':
                ...
                break;
            case 'o':
                if (!icm_is_binarynum_valid(optarg)) {
                    fprintf(stderr, "Invalid value %s for enable secondary "
                            "40 MHz occupancy check.\n",
                            optarg);
                    print_usage();
                    exit(EXIT_FAILURE);
                }
                pconf->enable_sec40_occ_check = atoi(optarg) ? true : false;
                break;
            case '?':
                if ((optopt == 'n') ||
                    (optopt == 'c') ||
