...
void icm_print_dev_info(ICM_DEV_INFO_T* pdev)
{
    ...
    for (i = 0; i < MAX_DEV_NUM; i++) {
        if (IS_DEV_ACTIVE(pdev, i)) {
            ...
            ICM_DPRINTF(pdev, ICM_PRCTRL_FLAG_NONE, ICM_DEBUG_LEVEL_MAJOR, ICM_MODULE_ID_UTIL,  "sec40 occ check     : %s\n", (picm->enable_sec40_occ_check == true) ? "enabled" : "disabled");
        }
    }
...
/*
 * Function     : icm_is_binarynum_valid
 * Description  : Determine if string expected to provide a binary value (0 or
 *                1) is valid. It is the caller's responsibility to ensure that
 *                the string is NULL terminated.
 * Input params : String giving expected binary value.
 * Return       : true if valid, false if invalid (errors are considered to
 *                invalidate the string)
 */
bool icm_is_binarynum_valid(const char* binaryvalstr)
{
    long val = 0;
    char* stopptr = NULL;

    if (binaryvalstr == NULL) {
        return false;
    }

    val = strtol(binaryvalstr, &stopptr, 0);

    if (errno != 0) {
        return false;
    }

    if ((stopptr == NULL) || ((*stopptr) != '\0')) {
        /* Either stopptr is unexpectedly NULL, or if it is non-NULL then strtol
         * has determined that binaryvalstr doesn't consist purely of digits.
