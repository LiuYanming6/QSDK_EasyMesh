...
get_wmac_irq(u_int16_t wmac_num)
{
	...
	qal_vbus_get_resource((struct qdf_pfm_hndl *)spdev, &vmres, IORESOURCE_IRQ, 0);
	res = (struct resource *)vmres;
	if (res == NULL) {
...
get_wmac_base(u_int16_t wmac_num)
{
	...
	unsigned long ret;
	struct qdf_vbus_resource *vmres = NULL;
	qal_vbus_get_resource((struct qdf_pfm_hndl *)spdev, &vmres, IORESOURCE_MEM, 0);
	res = (struct resource *)vmres;
	if (res == NULL) {
...
init_ath_wmac(u_int16_t devid, u_int16_t wlanNum)
{
    ...
    if (dev == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s: no memory for device state\n", __func__);
        goto bad1;
    }
    ...
    if(dev_node) {
        ...
        qal_vbus_get_irq((struct qdf_pfm_hndl *)spdev, "legacy", &dev->irq);
    ...
    printk(" JET_PRINT: %s[%d]: Checking devid: %x\n",__func__,__LINE__,devid);
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO "%s: %s: mem_start: =0x%lx, mem_end: =0x%lx, irq=%d\n",
        dev->name, athname ? athname : "Atheros ???",
        dev->mem_start, dev->mem_end, dev->irq);
...
exit_ath_wmac(u_int16_t wlanNum)
{
    ...
    if(dev == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: No device\n", __func__);
        return -ENODEV;
    }
    __ath_detach(dev);
    free_netdev(dev);
...
int init_ahb(void)
{
	...
	if(strcmp(sysType, "Atheros AR9330 (Hornet)") == 0) {
		...
		devid = AR9300_DEVID_AR956X;
    } else if(strcmp(sysType, "QCN5500") == 0) {
		devid = AR9300_DEVID_AR5500;
	} else {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Unknow sysType: %s\n", sysType);
		return -ENODEV;
	}
...
init_ath_ahb(void)
{
	...
	if (init_ahb() != 0) {
		QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ath_ahb: No devices found, driver not installed.\n");
		return (-ENODEV);
	}
...
static const struct platform_device_id ath9k_platform_id_table[] = {
    ...
    {
        .name = "qcn550x_wmac",
        .driver_data = AR9300_DEVID_AR5500,
    },
    {
        .name = "qca953x_wmac",
...
const struct of_device_id ath_wifi_of_match[] = {
            ...
            {.compatible = "qca,wifi-ar550x", .data = (void *) &ath9k_platform_id_table[5] },
            {.compatible = "qca,wifi-ar953x", .data = (void *) &ath9k_platform_id_table[6] },
                        { /*sentinel*/},
};
#endif
