...
__pktlog_reset_buffer(ath_generic_softc_handle scn, int32_t reset)
{
    ...
    if (pl_info->log_state) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Logging should be disabled before reseting bufer size\n");
        return -EINVAL;
    }

    qdf_spin_lock(&pl_info->log_lock);
    if (pl_info->buf != NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Reseting pktlog buffer!\n");
