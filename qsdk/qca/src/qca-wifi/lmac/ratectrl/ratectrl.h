 ...
 */
#define MAX_TX_RATE_TBL         90
#define MAX_TX_RATE_PHY         64
#define MAX_SCHED_TBL           64
...
struct atheros_node {
    ...
    A_UINT8 singleStream    :1,   /* When TRUE, only single stream Tx possible */
            ...
            quadStream      :1;   /* When TRUE, quad stream Tx possible */
...
#ifdef ATH_SUPPORT_TxBF
#define ATH_MAX_RATES 44 /* CCK(4) + OFDM(8) + HTSS(8) + HTDS(8) + HTTS(8) + HTQS(8) */
...
#ifdef  ATH_SUPPORT_TxBF
#define TX_STREAM_USED_NUM(_pSib) (4*(_pSib->quadStream) + 3*(_pSib->tripleStream) + 2*(_pSib->dualStream) + (_pSib->singleStream))
