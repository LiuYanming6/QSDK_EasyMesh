...
#if 0 //UMCADFS:def ATH_SUPPORT_DFS
#include "dfs.h"
#endif
...
//    ATH_RXBUF_RESET(bf);
    ...
    */
    ...
    if (rxedma->rxfifodepth > rxedma->rxfifohwsize)
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "PANIC depth more than size %d sz %d\n",
...
ath_rx_addbuffer(struct ath_softc *sc, HAL_RX_QUEUE qtype, int size)
{
    ...
    TAILQ_FOREACH_SAFE(bf, &sc->sc_rxbuf, bf_list, tbf) {
        ...
        if (rxedma->rxfifodepth >= rxedma->rxfifohwsize) {
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: size = %d rxfifodepth=%d\n", __func__, size, rxedma->rxfifodepth);
            break;
        }
...
#if !ATH_RXBUF_RECYCLE
static void ath_rx_edma_refill_queue_intsafe(struct ath_rx_edma_requeue_request *requeue)
{
...
void
ath_rx_edma_refill_queue(struct ath_softc *sc, struct ath_buf *bf)
{
    ...
    requeue.sc = sc;
    requeue.bf = bf;
    OS_EXEC_INTSAFE(sc->sc_osdev, ath_rx_edma_refill_queue_intsafe, &requeue);
    ATH_RXBUF_UNLOCK(sc);
}
EXPORT_SYMBOL(ath_rx_edma_refill_queue);
#endif
...
static inline void ath_rx_wifipos_isresp(struct ath_rx_status *rxs, struct ath_softc *sc, wbuf_t wbuf, unsigned char pkt_type)
{
    ...
    wifiposdesc.rssi3 = rxs->rs_rssi_ctl3;
...
ath_rx_handler(ath_dev_t dev, int flush, HAL_RX_QUEUE qtype)
{
    ...
    do {
        ...
        if (wbuf == NULL) {     /* XXX ??? can this happen */
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "no mpdu (%s)\n", __func__);
            continue;
        }
        ++rx_processed;
...
#if 0 //UMACDFS:: def ATH_SUPPORT_DFS
    if (sc->sc_dfs != NULL) {
