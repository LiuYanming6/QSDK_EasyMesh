...
#define ATH_FRAG_PER_QOSNULL_MSDU                   1
    ...
    if (error != 0) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "failed to allocate UAPSD QoS NULL tx descriptors: %d\n", error);
        return error;
    }
    ...
    TAILQ_FOREACH(bf, &sc->sc_uapsdqnulbf, bf_list)
    {
        ...
        if (wbuf == NULL) {
            error = -ENOMEM;
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "failed to allocate UAPSD QoS NULL wbuf\n");
            break;
        }
        ...
        if (sc->sc_enhanceddmasupport) {
            ...
            if (bf == NULL) {
                if (txq->axq_headindex != txq->axq_tailindex)
                    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ath_tx_uapsd_draintxq: ERR head %d tail %d\n",
...
#endif
{
    ...
    while (bf) {
        ...
        if (bf->bf_qosnulleosp) {
            ...
            /* complete this frame */
#ifdef  ATH_SUPPORT_TxBF
            ath_tx_complete_buf(sc, bf, &bf_head, NULL, txok,  txbf_status, tstamp);
#else
            ath_tx_complete_buf(sc, bf, &bf_head, NULL, txok);
#endif
...
ath_tx_stage_queue_uapsd(struct ath_softc *sc, struct ath_node *an, ath_bufhead *bf_head)
{
    ...
    while (bf) {
        ...
        if (status == HAL_EINPROGRESS) {
            ...
            /* complete this frame */
#ifdef  ATH_SUPPORT_TxBF
            ath_tx_complete_buf(sc, bf, &bf_tmp_head, NULL, txok, ds->ds_txstat.ts_status, ds->ds_txstat.ts_tstamp);
#else
            ath_tx_complete_buf(sc, bf, &bf_tmp_head, NULL, txok);
#endif
            sc->sc_stats.ast_uapsddatacomp++;
            ++completed;
        }
