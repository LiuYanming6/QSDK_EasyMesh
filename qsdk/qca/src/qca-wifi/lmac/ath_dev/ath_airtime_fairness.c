...
void ath_atf_resume_all_tid(struct ath_node *an)
{
    ...
    while (an->an_atf_nodepaused && (tidno < WME_NUM_TID)) {
        if (an->an_atf_nodepaused & 0x1) {
            ...
            IEEE80211_ATF_RESET_TID_BITMAP(an, tidno);
...
int32_t ath_atf_debug_node_state(struct ath_node *an, struct atf_nodestate *atfnodestate)
{
    ...
    atfnodestate->nodestate = nodestate;
    atfnodestate->atf_tidbitmap = an->an_atf_nodepaused;
...
void ath_atf_sc_node_unblock(struct ath_node *an, struct atf_stats *stats)
{
    ...
    for (i = 0; i < WME_NUM_AC; i++) {
        ...
        if (subgroup) {
            subgroup->sg_atftidstate = an->an_atf_nodepaused;
        }
    }
 ...
 * Description  : Check if the frame should be excluded out of ATF scheduling logic
                  ...
                  & packets from STA should be excluded.
...
ath_atf_check_txtokens(struct ath_softc *sc, struct ath_buf *bf, ath_atx_tid_t *tid)
{
    ...
    */
    if ((tid->tidno != WME_MGMT_TID)  && (ni != ni->ni_bss_node) &&
        ...
        if (!ath_atf_account_tokens(tid, pkt_duration)) {
            ...
            if (!IEEE80211_ATF_ISSET_TID_BITMAP(node, tid->tidno)) {
                IEEE80211_ATF_SET_TID_BITMAP(node, tid->tidno);
...
{
    u_int32_t actual_pkt_duration=0, duration_per_try = 0;
    u_int32_t ack_duration = 0, pkt_overhead = 0;
    u_int32_t rateindex=0, tries = 0, totalretries = 0;
    u_int32_t backoff = 0, backoff_duration = 0;
    u_int8_t rix = 0, cix = 0;
    int32_t i=0;
    struct ieee80211_node *ni;
