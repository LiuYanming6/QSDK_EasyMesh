...
#undef INST_STATS_VALID_RSSI_MAX
            ...
            if (bf->bf_isampdu) {
                ...
                {
#ifdef  ATH_SUPPORT_TxBF
                    ath_tx_complete_buf(sc, bf, &bf_head, &ts, txok, ts.ts_txbfstatus, ts.ts_tstamp);
#else
                    ath_tx_complete_buf(sc, bf, &bf_head, &ts, txok);
#endif
                }
            }
#ifndef REMOVE_PKT_LOG
        ...
        if (!ath_vap_pause_in_progress(sc)) {
            ...
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:vap_pause is in progress\n",__func__);
        }
        ATH_TXQ_UNLOCK(txq);
