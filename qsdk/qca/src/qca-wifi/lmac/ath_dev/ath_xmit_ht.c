        ...
        ath_bufhead *bf_head, ieee80211_tx_control_t *txctl)
{
    ...
    if (sc->sc_atf_enable) {
        ...
        if (!ath_atf_bypass_frame(tid, bf) &&
            IEEE80211_ATF_ISSET_TID_BITMAP(tid->an, tid->tidno)) {
    ...
    if (!TAILQ_EMPTY(&tid->buf_q) || qdf_atomic_read(&tid->paused) || txq->axq_depth || txctl->istxfrag
        || atf_force_sendto_swq || !(sc->sc_tx_on))
    {
#ifdef ATH_SWRETRY
...
#endif		        
       /*As the packet has been queued in s/w tid queue retrun from here */
        return 0;
...
#if ATH_SUPPORT_WRAP
                if (sc->sc_qwrap_rx_war && sc->sc_enableproxysta){
                    extradelim = 1;
                }
#endif
                if (extradelim)
...
#define MS(_v, _f)  (((_v) & _f) >> _f##_S)
        ...
        if (!rl) {
             ...
             */
            is_ms_rate = (bf->bf_rcs[0].flags & (ATH_RC_DS_FLAG | ATH_RC_TS_FLAG | ATH_RC_QS_FLAG))? 1 : 0;
...
ath_tx_sched_rifs(struct ath_softc *sc, struct ath_txq *txq, ath_atx_tid_t *tid)
{
    ...
    /* If TID queue is empty, break */
        ...
        if (bf->bf_nrifsubframes == 1) {
...
#ifdef ATH_SUPPORT_TxBF
                ath_tx_complete_buf(sc, bf, &bf_q, NULL, 0, 0, 0);
#else
                ath_tx_complete_buf(sc, bf, &bf_q, NULL, 0);
#endif
            }
        } else {
             ...
             */
            if (ath_tx_txqaddbuf(sc, txq, &bf_q) != 0) {
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "--- %s[%d] --- HW Q is full ---\n", __func__, __LINE__);
            }
        }
...
#define    OFDM_SIFS_TIME    16
    ...
    if (ath_tx_txqaddbuf(sc, txq, bf_qfirst) != 0) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "--- %s[%d] --- HW Q Full, Could not add Aggr to HW -- Not being Freed !! LEAK !! \n", __func__, __LINE__);
    }
...
ath_tx_sched_normal(struct ath_softc *sc, struct ath_txq *txq, ath_atx_tid_t *tid)
{
    ...
    if (sc->sc_atf_enable) {
        ...
        */
        ...
        if (ath_tx_txqaddbuf(sc, txq, &bf_q) != 0) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ath_tx_sched_normal: txqadd failed for single\n");
#ifdef ATH_SUPPORT_TxBF
                ath_tx_complete_buf(sc, bf, &bf_q, NULL, 0, 0, 0);
#else
                ath_tx_complete_buf(sc, bf, &bf_q, NULL, 0);
    ...
    do {
        ...
        if (!TAILQ_EMPTY(&tid->buf_q)) {
            ...
            if ((status == ATH_AGGR_BAW_CLOSED) && (TAILQ_EMPTY(&bf_q)) &&
                ...
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Recovering from error condition: Flushing TID queue until head \
                       is within the window\n", __func__);
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: TID[%d] seq_start:%d seq_next:%d Head:%d Tail:%d BAW size:%d seqno:%d\n",
...
#ifdef ATH_SUPPORT_TxBF
                ath_tx_complete_buf(sc, bft, &bf_headt, NULL, 0, 0, 0);
#else
                ath_tx_complete_buf(sc, bft, &bf_headt, NULL, 0);
#endif
            }
        }
        ...
        if (sc->sc_atf_enable) {
            ...
            if (ath_tx_txqaddbuf(sc, txq, &bf_q) != 0) {
                ...
                ath_tx_complete_buf(sc, bf, &bf_q, NULL, 0);
#endif
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ath_tx_sched_aggr: txqadd failed for single\n");
            }
        } else {
            /*
             * setup first desc with rate and aggr info
            ...
            if (status == ATH_AGGR_8K_LIMITED) {
                ...
                if (ath_tx_txqaddbuf(sc, txq, &bf_q) != 0) {
                    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ath_tx_sched_aggr: txqadd failed for aggregate\n");
                }
            }
        }
...
ath_txq_schedule(struct ath_softc *sc, struct ath_txq *txq)
{
    ...
    if (!(sc->sc_tx_on) &&
            ((sc->sc_haltype2q[HAL_WME_AC_BK] == txq->axq_qnum) ||
             ...
             (sc->sc_haltype2q[HAL_WME_AC_VI] == txq->axq_qnum) ||
        ...
        while ( !TAILQ_EMPTY(&ac->tid_q) ){
            ...
            if (sc->sc_txrifs)
                ...
                if (!(tid->an->an_smmode == ATH_SM_PWRSAV_DYNAMIC) ||
                    ...
                    if (ath_aggr_query(tid)&& !tid->an->an_pspoll) {
#ifdef VOW_DEBUG
                        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "RRSA: %d\n", tid->ac->qnum);
#endif
                        ath_tx_sched_aggr(sc, txq, tid);
                    } else {
                        if (tid->baw_head == tid->baw_tail ||
                            txq->axq_depth == 0) {
#ifdef VOW_DEBUG
                            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "RRSN: %d\n", tid->ac->qnum);
#endif
                            ath_tx_sched_normal(sc, txq, tid);
                        } else {
                            TAILQ_INSERT_TAIL(&paused_tid_q, tid, tid_qelem);
        ...
        if (!txpending) {
            ...
            if( defer_completion.bf ) {
                ...
                ath_tx_complete_buf(sc, defer_completion.bf, &defer_completion.bf_head, 
                    ts, !defer_completion.txfail, ts->ts_txbfstatus, ts->ts_tstamp);
#else
