...
ath_slow_ant_div(struct ath_antdiv *antdiv, struct ieee80211_frame *wh, struct ath_rx_status *rx_stats)
{
    ...
    if (antdiv->antdiv_start &&
        ...
        antdiv->antdiv_lastbrssictl[3][curcfg] = rx_stats->rs_rssi_ctl3;

	DPRINTF(sc, sc->sc_reg_parm.showAntDivDbgInfo, "CurrentRSSI: %d%d%d%d\n",
			antdiv->antdiv_lastbrssictl[0][curcfg],
			antdiv->antdiv_lastbrssictl[1][curcfg],
			antdiv->antdiv_lastbrssictl[2][curcfg],
			antdiv->antdiv_lastbrssictl[3][curcfg]);
...
/* not yet
        ...
        antdiv->antdiv_lastbrssiext[3][curcfg] = rx_stats->rs_rssi_ext3;
*/
...
ath_ant_div_comb_scan(struct ath_antcomb *antcomb, struct ath_rx_status *rx_stats)
{
    ...
    rx_ant_conf = (rx_stats->rs_rssi_ctl3 >> 4) & 0x3;
    main_ant_conf= (rx_stats->rs_rssi_ctl3 >> 2) & 0x3;
#endif
...
#ifdef AR5500_EMULATION
    rx_ant_conf   = (rx_stats->rs_rssi_ctl3 >> SMART_ANT_CUR_CONF_S)  & SMART_ANT_CONF_MASK;
    main_ant_conf = (rx_stats->rs_rssi_ctl3 >> SMART_ANT_MAIN_CONF_S) & SMART_ANT_CONF_MASK;
    alt_ant_conf  = (rx_stats->rs_rssi_ctl3 >> SMART_ANT_ALT_CONF_S)  & SMART_ANT_CONF_MASK;
    rx_switch_com = rx_stats->rs_rssi_ext3 & SMART_ANT_SWITCH_MASK;
#endif
