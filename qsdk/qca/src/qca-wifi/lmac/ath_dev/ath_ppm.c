                     ...
                     struct ath_rx_status *rx_stats)
{
    ...
    default:
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "PPM STATE ILLEGAL %x %x\n", forcePpmStateCur, afp->forceState);
        ASSERT(0);
        ath_force_ppm_restart(afp);
        break;
    }
    ...
    {
        DPRINTF(afp->sc, ATH_DEBUG_PPM, "%s: St=%s->%s Act=%d,%d,%d type=%02X Trig=%d Tsf=%d Aggr=%d Rssi=%d BssOk=%d Tsf2=%d rssi=%d,%d,%d,%d addr3=%02X:%02X:%02X:%02X:%02X:%02X\n",
            __func__, 
...
/* TEMP DEBUG ONLY */
    if (dumpForcePpmFlag)
    {
        ...
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "FORCE_PPM %4d %6.6x %8.8x %8.8x %8.8x %3.3x %4.4x\n",
            pktlog_misc[5],
            pktlog_misc[4],
            pktlog_misc[6],
            pktlog_misc[0],
