...
end0 :   
    if (sendbar && !ath_vap_pause_in_progress(sc)) {
        ...
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:txq pause is in progress\n",__func__);
    }
...
static INLINE void
_ath_tx_free_buf(struct ath_softc *sc, struct ath_buf *bf,struct ath_node *an, struct ath_atx_tid *tid, struct ath_txq *txq, struct ath_tx_status *ts, int txok)
{
    ...
    wbuf_unmap_sg(sc->sc_osdev, bf->bf_mpdu,
        ...
        if(an !=NULL){
            ...
            sc->sc_ieee_ops->tx_complete_compact((ieee80211_node_t)((an)->an_node), bf->bf_mpdu, &tx_status);
        ...
        if (bf->bf_atf_accounting == 1) {
            if (an) {
                ...
                subgroup = (struct subgroup_list *)bf->bf_atf_sg;
                if (subgroup) {
                    qdf_spin_lock_bh(&subgroup->atf_sg_lock);
                    if (subgroup->sg_num_buf_held == 0) {
                        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: an with buffer marked for atf accounting is holding no buffers\n", __func__);
                    } else {
                        subgroup->sg_num_buf_held--;
                        if (subgroup->sg_num_buf_held < subgroup->sg_min_num_buf_held) {
                            subgroup->sg_min_num_buf_held = subgroup->sg_num_buf_held;
                        }
                    }
                    subgroup->sg_num_bufs_sent++;
                    subgroup->sg_num_bytes_sent += bf->bf_atf_accounting_size;
                    qdf_spin_unlock_bh(&subgroup->atf_sg_lock);
                }
                ATH_NODE_ATF_TOKEN_UNLOCK(an);
            } else {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: buffer marked for atf accounting has no an reference\n", __func__);
            }
        }
...
void  ath_edma_free_complete_buf( struct ath_softc *sc , struct ath_txq *txq, ath_bufhead * bf_headfree, struct ath_tx_status *ts, int txok )
{
    struct ath_buf *bf;
    struct ath_node *an ;
    struct ath_atx_tid *tid;
    ...
    while(bf != NULL){
        ...
        _ath_tx_free_buf(sc, bf, an, tid, txq, ts, txok);
#if ATH_SUPPORT_HYFI_ENHANCEMENTS
		if(txok)
		{
...
void
ath_tx_complete_buf(struct ath_softc *sc, struct ath_buf *bf, ath_bufhead *bf_q, struct ath_tx_status *ts, int txok, u_int8_t txbf_status, u_int32_t tstamp)
#else
void
ath_tx_complete_buf(struct ath_softc *sc, struct ath_buf *bf, ath_bufhead *bf_q, struct ath_tx_status *ts, int txok)
#endif
{
    if(bf == NULL)
        return;
...
#if ATH_SUPPORT_TxBF
        _ath_tx_complete_buf(sc, bf, bf_q, ts, txok, txbf_status, tstamp);
#else
        _ath_tx_complete_buf(sc, bf, bf_q, ts, txok);
#endif
    }
}
...
#endif
#if ATH_SUPPORT_VOWEXT
OS_EXPORT_SYMBOL(ath_tx_get_vibuf);
#endif
#ifdef AH_NEED_DESC_SWAP
OS_EXPORT_SYMBOL(ath_desc_swap);
#endif
