...
int ath_paprd_cal(struct ath_softc *sc)
{
    ...
    {
        /* Intentional print */
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s sc %pK chan_mask %d PAPRD excessive failure disabling PAPRD now\n", __func__, sc, sc->sc_tx_chainmask);
    ...
    if (ath_hal_Dragonfly(sc->sc_ah) || ath_hal_Honeybee20(sc->sc_ah) || ath_hal_Jet(sc->sc_ah)) {
...
void ath_paprd_send(struct ath_softc *sc,  int count) 
{
    if ( count == 0)
    {
        ...
        } else if (ath_hal_Dragonfly(sc->sc_ah) || ath_hal_Honeybee20(sc->sc_ah) || ath_hal_Jet(sc->sc_ah)) {
...
int ath_tx_paprd_init(struct ath_softc *sc)
{
...
#if 0
    {
        ...
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s sc %pK ---Enable PAPRD DEBUG ---- fail count %d\n", __func__, sc, sc->sc_paprd_failed_count);
    }
#endif
