...
#define TIME_IN_MS 1000
	OS_SET_TIMER(&dfs->ath_dfs_no_chan_vap_restart, VAP_RESTART_WAIT_TIME * TIME_IN_MS);
	ic->no_chans_available = 0;
        ic->ic_pause_stavap_scan = 1;
#undef TIME_IN_MS
#else
        target_channel = ic->ic_random_channel(ic, IEEE80211_SELECT_NONDFS_AND_DFS, IEEE80211_SELECT_NXT_CH_POST_RADAR_DETECT, FALSE);
        if ( target_channel != -1) {
...
void
dfs_nol_addchan(struct ath_dfs *dfs, struct ieee80211_ath_channel *chan,
...
#define TIME_IN_US (TIME_IN_MS * 1000)
    ...
    dfs_deliver_event(dfs->ic, chan->ic_ieee, IEEE80211_EV_NOL_STARTED);
    DFS_DPRINTK(dfs, ATH_DEBUG_DFS_NOL,
    ...
    while (nol != NULL) {
        if (nol->nol_freq == delfreq && nol->nol_chwidth == delchwidth) {
            *prev_next = nol->nol_next;
            dfs_deliver_event(dfs->ic, delchan, IEEE80211_EV_NOL_FINISHED);
            DFS_DPRINTK(dfs, ATH_DEBUG_DFS_NOL,
