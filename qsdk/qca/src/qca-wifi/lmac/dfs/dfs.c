 ...
 */
static void
dfs_channel_mark_radar(struct ath_dfs *dfs, struct ieee80211_ath_channel *chan)
{
...
void dfs_action_on_radar_found(struct ieee80211com *ic)
{
 ...
 */
        ...
        if(dfs->is_radar_found_on_secondary_seg && ic->ic_dfs_is_precac_timer_running(ic)) {
            ...
            struct ieee80211_ath_channel *ichan = ic->ic_dfs_find_precac_secondary_vht80_chan(ic);
...
#endif
        {
            ...
            if (ic->ic_get_dfsdomain(ic) == DFS_ETSI_DOMAIN) {
                /* Remove chan from EPCCL*/
                DFS_PRINTK(" %s : %d remove channel from EPCCL\n", __func__, __LINE__);
                ic->ic_dfs_notify_etsi_precac_radar(ic, dfs->is_radar_found_on_secondary_seg);
            }
#endif
    }
...
*/
    ...
    if (dfs->dfs_rinfo.rn_use_nol == 1 && ic->ic_precac_enable) {
    ...
    dfs_deliver_event(ic, ic->ic_curchan->ic_ieee, IEEE80211_EV_RADAR_DETECTED);
/*
 ...
 */
    if (dfs->dfs_rinfo.rn_use_nol == 1)  {
        ...
        if (ic->ic_dfs_info_notify_channel_available) {
            ic->ic_channel_notify_to_app(ic);
        }
...
static
OS_TIMER_FUNC(dfs_no_chan_vap_restart)
{
    struct ieee80211com *ic;
    OS_GET_TIMER_ARG(ic, struct ieee80211com *);

    ic->ic_bringup_ap_vaps_after_nol(ic);
}
#endif
...
void dfs_extract_radar_found_params(struct ath_dfs *dfs,
        struct dfs_radar_found_params *params)
{
    OS_MEMZERO(params, sizeof (struct dfs_radar_found_params));
    params->pri_min = dfs->ath_dfs_average_pri;
    params->pri_max = dfs->ath_dfs_average_pri;
    params->duration_min = dfs->ath_dfs_average_duration;
    params->duration_max = dfs->ath_dfs_average_duration;
    params->sidx_min = dfs->ath_dfs_average_sidx;
    params->sidx_max = dfs->ath_dfs_average_sidx;

    qdf_print("Avg.PRI =%u, Avg.duration =%u, Avg.sidx =%u\n",dfs->ath_dfs_average_pri, dfs->ath_dfs_average_duration, dfs->ath_dfs_average_sidx);
     ...
     */
    dfs->ath_dfs_average_sidx = 0;
    dfs->ath_dfs_average_duration = 0;
    dfs->ath_dfs_average_pri =0;
...
void dfs_deliver_event(struct ieee80211com *ic, u_int8_t chan, u_int16_t event)
{
    ...
    if(!(ic->ic_osdev)) {
        printk(KERN_ERR "%s: ic->ic_osdev is NULL. No dfs %s event.\n", __func__, dfs_event(event));
        return;
    }
...
void dfs_deliver_events(struct ieee80211com *ic, struct ieee80211_ath_channel *chan, u_int16_t event)
{
    struct ieee80211_channel_list chan_info;
    int i;
...
static OS_TIMER_FUNC(dfs_task)
{
     ...
     * XXX no locking?!
     */
    /* Why should this feature function only if usenol =1?
     * In DFS-HOST-CONFIRMATION feature, on radar found, an indication is sent
     * to FW and further action is taken after the response from FW. But host
     * does not wait for the response from FW indefinitely. If after a
     * "wait timeout", if host does not receive a response from FW, Host
     * proceeds with DFS action (vacating from radar hit channel and adding to
     * NOL). Say, response is received from FW after the timeout, action would
     * have already been taken. But what is the metric to confirm that action was
     * taken? It is the channel change, i.e. comparing the current channel and
     * the channel on which radar was found. If these 2 channels do not match,
     * it is presumed that dfs_action was taken and no further dfs_action is
     * required on receiving the status from FW (for status 0 and 2). This
     * metric would no longer prove fail-proof if usenol is set as 0 (By design,
     * usenol:0 does not do any dfs_action). Hence on receiving the status from
     * from FW, if we find curchan is same as radar_found chan, it could be
     * because either timer did not expire or timer expired,dfs_action was taken
     * but as usenol:0, chan change did not happen which could be misleading.
     * Hence allow indication to FW only if usenol :1.
     */
    if (dfs_process_radarevent(dfs, ic->ic_curchan)) {
        if (ic->ic_is_host_dfs_check_enabled &&
                (dfs->dfs_rinfo.rn_use_nol == 1 ||
                ic->ic_tst_disable_chanchange_post_radar_detect == 1) &&
                DFS_FCC_DOMAIN == ic->ic_get_dfsdomain(ic)) {
            /* Save the cur_chan as radar found chan
             * and send radar found indication to FW.
             */
            ic->ic_radar_found_chan = ic->ic_curchan;
            dfs_extract_radar_found_params(dfs,&radar_params);
            ic->ic_wmi_radar_found_indication_to_fw(ic,&radar_params);
            ic->ic_host_dfs_status_wait_timer_running = 1;
            OS_SET_TIMER(&ic->ic_dfs_state.ath_dfs_fw_status_wait_timer,
                    ((dfs->ath_dfs_host_wait_timeout_override != -1) ?
                     dfs->ath_dfs_host_wait_timeout_override :
                     HOST_DFS_STATUS_WAIT_TIMER_MS));
        } else {
            dfs_action_on_radar_found(ic);
        }
    }
...
static
OS_TIMER_FUNC(dfs_status_wait_timeout)
{
    struct ieee80211com *ic;
    struct ath_dfs *dfs;
    ...
    qdf_print("%s: Host DFS status wait timer expired\n",
            __func__);
    ic->ic_host_dfs_status_wait_timer_running = 0;
    /* The following check is to test the effect of disabling
     * channel change after real radar detection.
     * spoof_test_done = 1 indicates that radar that was
     * detected was a HW radar and not a spoof pulse.
     */
    if (ic->ic_tst_disable_chanchange_post_radar_detect == 1 &&
            ic->ic_spoof_test_done == 1) {
        qdf_print("%s: Disabling channel change \n",
                __func__);
        dfs->dfs_rinfo.rn_use_nol = 0;
    }
    dfs_action_on_radar_found(ic);
}
...
#define	N(a)	(sizeof(a)/sizeof(a[0]))
    ...
    ic->ic_dfs_action_on_radar_found = dfs_action_on_radar_found;
#ifndef ATH_DFS_RADAR_DETECTION_ONLY
    dfs->dfs_nol = NULL;
#endif
...
#endif
    OS_INIT_TIMER(ic->ic_osdev, &(ic->ic_dfs_state.ath_dfs_fw_status_wait_timer),
            dfs_status_wait_timeout, (void *) (ic), QDF_TIMER_TYPE_WAKE_APPS);
    dfs->ath_dfs_host_wait_timeout_override = -1;
#if QCA_DFS_NOL_VAP_RESTART
    OS_INIT_TIMER(ic->ic_osdev, &(dfs->ath_dfs_no_chan_vap_restart), dfs_no_chan_vap_restart, (void *) (ic), QDF_TIMER_TYPE_WAKE_APPS);
#endif
...
#endif
#if QCA_DFS_NOL_VAP_RESTART
    OS_SYNC_CANCEL_TIMER(&dfs->ath_dfs_no_chan_vap_restart);
#endif
}
    ...
    struct ath_dfs_radar_tab_info *radar_info, int no_cac)
{
    ...
    //u_int32_t                        rfilt;
    struct ath_dfs                 *dfs=(struct ath_dfs *)ic->ic_dfs;
    struct ieee80211_ath_channel *chan=ic->ic_curchan, *ext_ch = NULL;
...
void dfs_remove_spoof_chan_from_nol(struct ieee80211com *ic)
{
    ...
    if (ic->ic_radar_found_chan == NULL) {
        qdf_print("%s:Radar found channel is NULL\n",__func__);
        return;
    }

    DFS_DPRINTK(dfs, ATH_DEBUG_DFS1,
            "%s: Spoof Test Channel ic_ieee =%d\n", __func__,
            ic->ic_radar_found_chan->ic_ieee);
    /* Gets all the HT/VHT 20 MHz chan list of the spoof_radar_found_chan
     */
    dfs->ic->ic_get_ext_chan_info(ic, ic->ic_radar_found_chan, &chan_info);
    ATH_DFSNOL_LOCK(dfs);
    for (i = 0; i < chan_info.cl_nchans; i++) {
        if (chan_info.cl_channels[i] == NULL) {
            DFS_PRINTK("%s: No channels found\n", __func__);
        } else {
            nol = dfs->dfs_nol;
            while (nol != NULL) {
                if (nol->nol_freq == chan_info.cl_channels[i]->ic_freq) {
                    /* Sets a 0 second NOL timer, which calls the timer expiry
                     * func dfs_remove_from_nol that removes the element from
                     * NOL list and updates the NOL list to higher layers
                     */
...
/*
 * This is a wrapper function for dfs_is_channel_in_nol to check whether the
 * given freq is present in NOL list or not
 */
u_int8_t dfs_chan_in_nol(struct ieee80211com *ic, u_int32_t chan_freq)
{
        return dfs_is_channel_in_nol(ic->ic_dfs, chan_freq);
}
                ...
                void *outdata, u_int32_t *outsize)
{
	...
	case DFS_RADARDETECTS:
		...
		*((u_int32_t *)outdata) = dfs->ath_dfs_stats.num_radar_detects;
		...
		DFS_PRINTK("%s debug level now = 0x%x \n",
				__func__,
				dfs->dfs_debug_mask);
		/* dfs_event_log_on must be set for radar_logs to be collected.
		 * These radar logs has pulse parameters which are verified by
		 * FW to confirm radar detection if "DFS-HOST-CONFIRMATION" feature
		 * is enabled. Hence event_log must always set to 1 if this feature
		 * is enabled.
		 */
    ...
    case DFS_DISABLE:
        ...
        data = (u_int32_t *) indata;
        if (DFS_FCC_DOMAIN == ic->ic_get_dfsdomain(ic)) {
            dfs->dfs_proc_phyerr &= ~DFS_RADAR_EN;
            dfs->ic->ic_dfs_state.dfs_disable = *data;
        }
        break;
	case DFS_GET_USENOL:
		if (!outdata || !outsize || *outsize < sizeof(u_int32_t)) {
...
#endif
EXPORT_SYMBOL(dfs_remove_spoof_chan_from_nol);
