 ...
 */
#define	PERE_IS_OVERSAMPLING(_dfs)	(_dfs->dfs_caps.ath_chip_is_over_sampled? 1 : 0)
...
//	DFS_DPRINTK(dfs, ATH_DEBUG_DFS_PHYERR, "%s (p=%pK):\n", __func__, buf);
	...
	rsu->agc_mb_gain =
	    MS(rs[RADAR_REPORT_PULSE_REG_2], RADAR_REPORT_PULSE_AGC_MB_GAIN);
}
    ...
    struct rx_search_fft_report *rsfr)
{
	...
	OS_MEMCPY(rs, buf, sizeof(rs));
        ...
        if (dfs->dfs_caps.ath_chip_is_ht160) {
            rsfr->seg_id          = MS(rs[SEARCH_FFT_REPORT_REG_3], SEARCH_FFT_REPORT_SEG_ID);
            DFS_DPRINTK(dfs, ATH_DEBUG_DFS_PHYERR,"%s: rsfr->seg_id = %d\n", __func__, rsfr->seg_id);
        }
}
...
tlv_parse_frame(struct ath_dfs *dfs, struct rx_radar_status *rs,
    struct rx_search_fft_report *rsfr, const char *buf, size_t len, u_int8_t rssi, int *first_short_fft_peak_mag)
{
	int i = 0;
	uint32_t tlv_hdr[1];
        bool first_tlv = true;
	...
	while ((i < len ) && (false_detect == false)) {
		...
		if (MS(tlv_hdr[TLV_REG], TLV_LEN) + i >= len) {
			DFS_DPRINTK(dfs, ATH_DEBUG_DFS_PHYERR,
			    "%s: TLV oversize: TLV LEN=%d, available=%zd, "
			    "i=%d\n",
			    __func__,
			    MS(tlv_hdr[TLV_REG], TLV_LEN),
			    len,
			...
			case TAG_ID_RADAR_PULSE_SUMMARY:		/* Radar pulse summary */
				...
				radar_summary_parse(dfs, buf + i,
				    MS(tlv_hdr[TLV_REG], TLV_LEN), rs);
                                ...
                                /*
                                    check for possible false detection on beeliner
                                    this may also work for Cascade but parameters (e.g. AGC_MB_GAIN_THRESH1)
                                    may be different for Cascade

                                */
                if ((dfs->dfs_caps.ath_chip_is_over_sampled == 0) && (dfs->dfs_caps.ath_chip_is_ht160 == 0 && (dfs->dfs_caps.ath_chip_is_false_detect))) {
                                    if ((rs->agc_mb_gain > AGC_MB_GAIN_THRESH1) &&
                                        ((rs->agc_total_gain - rs->agc_mb_gain) < AGC_OTHER_GAIN_THRESH1)) {
                                        false_detect = true;
    	                            DFS_DPRINTK(dfs,
                                                ATH_DEBUG_DFS_PHYERR,
                                                "%s: setting false_detect to TRUE because of mb/total_gain, agc_mb_gain=%d, agc_total_gain=%d, rssi=%d\n",
                                                __func__, rs->agc_mb_gain, rs->agc_total_gain, rssi);
                                    }


                                    if ((rs->agc_mb_gain > AGC_MB_GAIN_THRESH2) &&
    				    ((rs->agc_total_gain - rs->agc_mb_gain) > AGC_OTHER_GAIN_THRESH2) &&
				...
				break;
				    ...
				    MS(tlv_hdr[TLV_REG], TLV_LEN), rsfr);
                                /* we are interested in the first short FFT report's peak_mag */
                                /* for this value to be reliable, we must ensure that         */
                                /* BB_srch_fft_ctrl_4.radar_fft_short_rpt_scl is set to 0     */
                                ...
                                */
                                ...
                                if (PERE_IS_OVERSAMPLING(dfs) &&
                                    ...
                                    false_detect = true;
	                            DFS_DPRINTK(dfs, ATH_DEBUG_DFS_PHYERR, "%s: setting false_detect to TRUE because of false_rssi_thres\n", __func__);
                                }
...
tlv_calc_freq_info(struct ath_dfs *dfs, struct rx_radar_status *rs)
{
	 ...
	 */
		...
		chan_centre = dfs->ic->ic_ieee2mhz(dfs->ic,
    ...
    struct dfs_phy_err *e)
{
	 ...
	 */
	if (! tlv_parse_frame(dfs, &rs, &rsfr, buf, datalen, rssi, &first_short_fft_peak_mag))
		return (0);
	...
	DFS_DPRINTK(dfs, ATH_DEBUG_DFS_FALSE_DET,
