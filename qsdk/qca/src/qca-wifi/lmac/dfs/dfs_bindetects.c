...
{
        u_int32_t searchpri, searchdur, searchrssi, deltapri = 0,deltapri1, deltapri2, deltadur, averagerefpri=0,MatchCount = 0;
   ...
   /* Count the other delay elements that have  pri and dur with in the
        ...
        for (i=0; i<dl->dl_numelems; i++) {
			...
			if ( (numpulses == 1)  ) {
                                dl->dl_seq_num_second =  dl->dl_elems[delayindex].de_seq_num;
                        	numpulses++;
                                ...
                                if ((rf->rf_check_delta_peak) && (dl->dl_elems[delayindex].de_delta_peak != 0)) {
				...
				delta_time_stamps = dl->dl_elems[delayindex].de_ts - prev_good_timestamp;
                        ...
                        for (j = 0; j < numpulsetochk; j++){
                            ...
                            if ( delta_ts_variance < (2*(j+1)*primargin) ) {
                                dl->dl_seq_num_stop =  dl->dl_elems[delayindex].de_seq_num;
                                numpulses++;
                                /* update sidx min/max for false detection check later */
                                if (sidx_min > dl->dl_elems[delayindex].de_sidx) {
                                    sidx_min = dl->dl_elems[delayindex].de_sidx;
