...
dump_phyerr_contents(const char *d, int len)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (i % 16 == 15) {
			QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: %s\n", __func__, buf);
			n = 0;
			buf[0] = '\0';
		}
	}
...
#if ATH_SUPPORT_ZERO_CAC_DFS
			if((e.seg_id == SEG_ID_SECONDARY) && !(dfs->dfs_proc_phyerr & DFS_SECOND_SEGMENT_RADAR_EN)){
				DFS_DPRINTK(dfs, ATH_DEBUG_DFS3,
				    "%s: Do not process PHY error data from Second segment, "
				    "DFS_SECOND_SEGMENT_RADAR_EN is not enabled\n", __func__);
				return;
			}
#endif
