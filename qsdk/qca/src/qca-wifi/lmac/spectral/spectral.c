...
void spectral_detach(struct ieee80211com* ic)
{
    ...
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO"SPECTRAL : Module removed (spectral = %pK)\n", spectral);
     ...
     case SPECTRAL_IS_SUPPORTED:
        {
           ...
           *outsize = sizeof(u_int32_t);
           *((u_int32_t *)outdata) = (u_int32_t)p_sops->is_spectral_supported(spectral);
        }
        break;
...
int spectral_get_noise_power(struct ath_spectral *spectral,
                             ...
                             CHAIN_NOISE_PWR_INFO* ctl_c3,  /* reports for chain 3 control */
                             ...
                             CHAIN_NOISE_PWR_INFO* ext_c3)  /* reports for chain 3 ext */
{
    ...
    */
    ...
    if (!error) {
        ...
        for(i = ATH_MAX_ANTENNA; i < ATH_MAX_ANTENNA*2; i++) {
            if (ah->ah_max_chainmask & (1 << (i % ATH_MAX_ANTENNA))){
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,   "%3d.%02d ",
                    (rssi_total[i] < 0) ? ((rssi_total[i]+3)>>2) : rssi_total[i]>>2,
                    (rssi_total[i]&3)*25);
            }
...
void spectral_register_funcs(void* arg, SPECTRAL_OPS* p)
{
    ...
    p_sops->is_spectral_supported   = p->is_spectral_supported;
