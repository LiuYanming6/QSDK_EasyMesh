...
{
    SPECTRAL_DPRINTK(sc, ATH_DEBUG_SPECTRAL2,"Turning off SWBA and BMISS interrupts before imask = 0x%x\n", ath_hal_intrget(sc->sc_ah));
...
void print_max_mag_index_data(MAX_MAG_INDEX_DATA *imag, struct ath_softc *sc)
{
    ...
    SPECTRAL_DPRINTK(sc,ATH_DEBUG_SPECTRAL2, "MAX_EXP print_ptr=%pK\n",print_ptr);
...
void print_hex_fft_ht20_packet(HT20_FFT_PACKET *fft_20, struct ath_softc *sc)
{
    ...
    SPECTRAL_DPRINTK(sc,ATH_DEBUG_SPECTRAL2, "BINS print_ptr=%pK\n",print_ptr);
    for(i=0; i<SPECTRAL_HT20_NUM_BINS; i++, total_bytes++) {
        SPECTRAL_DPRINTK(sc,ATH_DEBUG_SPECTRAL2, "[%d]=%x\n",i, *print_ptr++);
    }

    SPECTRAL_DPRINTK(sc,ATH_DEBUG_SPECTRAL2, "MAGNITUDE print_ptr=%pK\n",print_ptr);
    for(i=0; i<sizeof(MAX_MAG_INDEX_DATA); i++, total_bytes++) {
        SPECTRAL_DPRINTK(sc,ATH_DEBUG_SPECTRAL2, "[%d]=%x\n",i,*print_ptr++);
    }
    ...
    SPECTRAL_DPRINTK(sc,ATH_DEBUG_SPECTRAL2, "DONE print_ptr=%pK, sizeof(HT20_FFT_PACKET)=%d total_bytes=%d\n",print_ptr, sizeof(HT20_FFT_PACKET), total_bytes);
}
...
#define AR_OBS_BUS_1 0x806c
...
#ifdef DBG
        SPECTRAL_DPRINTK(sc, ATH_DEBUG_SPECTRAL, "==============SPECTRAL DEBUG TIMEOUT============== channel %d\n", sc->sc_curchan.channel);
        ...
        if (sc->sc_spectral->num_spectral_data < spectral->params.ss_count || spectral->params.ss_count == 128) {
            ...
            SPECTRAL_DPRINTK(sc, ATH_DEBUG_SPECTRAL2,"Start scan at %llu\n", (long long unsigned int) spectral->scan_start_tstamp); 
...
OS_TIMER_FUNC(spectral_classify_scan)
{
        ...
        if(prev_recv == spectral->num_spectral_data) {
            SPECTRAL_DPRINTK(sc, ATH_DEBUG_SPECTRAL2,"this_scan_recv=%d prev_recv=%d\n", spectral->num_spectral_data, prev_recv);
            send_fake_ht40_data(sc);
        } else {
            print_detection(sc);
            SPECTRAL_DPRINTK(sc, ATH_DEBUG_SPECTRAL2,"this_scan_recv=%d total_recv=%d\n", spectral->num_spectral_data, spectral->total_spectral_data);
        }
