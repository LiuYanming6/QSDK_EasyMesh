...
#define DEFAULT_CHAN_NOISE_FLOOR    -110
    ...
    if (((rxs->rs_phyerr != HAL_PHYERR_RADAR)) &&
        ...
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : Unknown PHY error (0x%x)\n", __func__, rxs->rs_phyerr);
        return;
    }

    if (spectral == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Spectral Module not attached\n");
        return;
    }
    ...
    switch((datalen & 0x3)) {
        ...
        case 3:
            ...
            break;
        default:
             QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,   "datalen mod4=%d spectral_data_len=%d\n", (datalen%4), spectral->spectral_data_len);
    }
    ...
    if (pulse_bw_info & SPECTRAL_SCAN_DATA) {
      ...
      if ((datalen == (spectral->spectral_data_len - 1)) ||
          ...
          QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,   "%s %d missing 1 byte datalen=%d expected=%d\n", __func__, __LINE__, datalen, spectral->spectral_data_len);
      ...
      if (spectral->sc_spectral_20_40_mode) {
        ...
        if (spectral->sc_spectral_noise_pwr_cal) {
            ...
            params.chain_ctl_rssi[3] = fix_rssi_inv_only(rxs->rs_rssi_ctl3);
            ...
            params.chain_ext_rssi[3] = fix_rssi_inv_only(rxs->rs_rssi_ext3);
        }
