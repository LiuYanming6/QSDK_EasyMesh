...
int qvit_check_message(ol_scn_t scn, u_int8_t *data)
{
    ...
    if (swba_event->hdr.msgCode == QVIT_MSG_ASYNC_SWBA_EVENT)
    {
        ...
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO "QVIT: %s() msgCode [0x%x]\n", __FUNCTION__, swba_event->hdr.msgCode);
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO "QVIT: %s: Start\n", __FUNCTION__);
        qvit_hexdump((unsigned char *)cptr, swba_event->hdr.msgLen);
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO "QVIT: %s: End\n", __FUNCTION__);
#endif
                             ...
                             u_int32_t datalen)
{
    ...
    else
    {
        ...
        {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "QVIT: Mismatch in expecting seq expected Seq %d got seq %d\n",scn->utf_event_info.expectedSeq,currentSeq);
        }
    }
...
#ifdef QVIT_DEBUG_EVENT
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO "QVIT: %s: scn->utf_event_info.expectedSeq [%d]\n", __FUNCTION__, scn->utf_event_info.expectedSeq);
#endif
    if ( scn->utf_event_info.expectedSeq == totalNumOfSegments )
    {
        if( scn->utf_event_info.offset != segHdrInfo.len )
        {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "QVIT: All segs received total len mismatch .. len %d total len %d\n",scn->utf_event_info.offset,segHdrInfo.len);
        }
#ifdef QVIT_DEBUG_EVENT
...
#ifdef QVIT_debug
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO "QVIT: %s: called\n", __FUNCTION__);
#endif
