...
ol_wow_enable(struct ieee80211com *ic, int clearbssid)
{
    ...
    return wmi_unified_wow_enable_send(scn->wmi_handle);
}
...
ol_wow_wakeup(struct ieee80211com *ic)
{
    ...
    return wmi_unified_wow_wakeup_event_send(scn->wmi_handle);
}
...
ol_wow_add_wakeup_event(struct ieee80211com *ic, u_int32_t type)
{
    ...
    return wmi_unified_wow_add_wakeup_cmd_send(scn->wmi_handle, &param);
}
    ...
    u_int8_t *pattern_bytes, u_int8_t *mask_bytes, u_int32_t pattern_len)
{
    ...
    for (i = 0; i < MAX_NUM_USER_PATTERN; i++) {
        ...
        if (pattern->valid) {
            ...
            {
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Pattern added already \n");
                return 0;
            }
        }
    }
    ...
    if (i == MAX_NUM_USER_PATTERN) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Error : All the %d pattern are in use. Cannot add a new pattern \n", MAX_NUM_PATTERN);
        return (-1);
    }

    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Pattern added to entry %d \n",i);
    ...
    return wmi_unified_wow_add_wakeup_pattern_send(scn->wmi_handle, &param);
}
    ...
    u_int8_t *mask_bytes)
{
    ...
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Remove wake up pattern \n", __func__);
    ...
    for (i = 0; i < MAX_NUM_USER_PATTERN; i++) {
        ...
        if (pattern->valid) {
                ...
                !qdf_mem_cmp(mask_bytes, pattern->maskBytes, MAX_PATTERN_SIZE))
            {
                ...
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Pattern Removed from entry %d \n",i);

                buf = wmi_buf_alloc(scn->wmi_handle, len);
                if(!buf)
                {
                    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "buf alloc failed\n");
                    return -ENOMEM;
                }
                param.pattern_id = i;
                return wmi_unified_wow_remove_wakeup_pattern_send(scn->wmi_handle,
                    &param);
            }
        }
    }
    ...
    // pattern not found
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : Error : Pattern not found \n", __func__);
    return (-1);
}
...
static int
wow_wakeup_host_event(ol_scn_t scn, u_int8_t *datap, u_int16_t len)
{
    struct ol_wow_info *wowInfo;
    wowInfo = scn->scn_wowInfo;
...
ol_ath_wow_attach(struct ieee80211com *ic)
{
    ...
    if (scn->scn_wowInfo == NULL) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: wowInfo allocation failed\n", __func__);
        return -1;
    }

    qdf_mem_zero(scn->scn_wowInfo, sizeof(struct ol_wow_info));

    wmi_unified_register_event_handler(scn->wmi_handle, wmi_wow_wakeup_host_event_id,
...
ol_ath_wow_detach(struct ieee80211com *ic)
{
    ...
    wmi_unified_unregister_event_handler(scn->wmi_handle, wmi_wow_wakeup_host_event_id);
