    ...
    else if ( param & OL_SPECIAL_PARAM_SHIFT )
    {
        ...
        switch (param) {
            case OL_SPECIAL_PARAM_COUNTRY_ID:
                ...
                /* If a valid country has been set,
                 * restart all the vaps as the channel list is updated.
                 */
                if(retval == 0){
                    wlan_if_t tmpvap;
                    TAILQ_FOREACH(tmpvap, &scn->sc_ic.ic_vaps, iv_next) {
                        struct net_device *tmpdev;
                        tmpvap->is_country_changed = 1;
                        tmpdev = ((osif_dev *)tmpvap->iv_ifp)->netdev;

                        /* Bring down the vaps gracefully so that, the flags
                         * are reset properly.
                         */
                        osif_vap_down(tmpdev);
                        if(IS_UP(tmpdev))
                            osif_vap_init(tmpdev, RESCAN);
                        tmpvap->is_country_changed = 0;
                    }
                }else {
            ...
            case OL_SPECIAL_PARAM_REGDOMAIN:
                {
                    ...
                    scn->sc_ic.ic_curchan_ieee = chan->ic_ieee;
                    scn->sc_ic.ic_curchan_cfreq2 = chan->ic_vhtop_ch_freq_seg2;
...
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
                osif_nss_ol_enable_ol_statsv2(scn, value);
#endif
                break;
...
int ol_ath_ucfg_set_country(struct ol_ath_softc_net80211 *scn, char *cntry)
{
    ...
    if (&scn->sc_ic) {
        ...
        /* If a valid country has been set,
         * reset all the vaps as the country has changed
         */
        ...
        if(retval == 0) {
            TAILQ_FOREACH(tmpvap, &scn->sc_ic.ic_vaps, iv_next) {
                ...
                /* Bring down the vaps gracefully so that the flags are reset properly */
                osif_vap_down(tmpdev);
                if(IS_UP(tmpdev))
                    osif_vap_init(tmpdev, RESCAN);
                tmpvap->is_country_changed = 0;
