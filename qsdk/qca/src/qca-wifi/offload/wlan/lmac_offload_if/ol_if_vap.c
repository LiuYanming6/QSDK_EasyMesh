...
ol_ath_vdev_create_start_cmd(struct ol_ath_softc_net80211 *scn,
     u_int8_t if_id, struct ieee80211_ath_channel *chan, u_int32_t freq,
     struct vdev_start_params *param)
{
    u_int32_t chan_mode;
#if ATH_SUPPORT_ZERO_CAC_DFS
    struct ieee80211_ath_channel *prefered_chan = NULL;
    u_int32_t prefered_freq;
#endif
        ...
        if((chan_mode == IEEE80211_MODE_11AC_VHT80) ||
           (chan_mode == IEEE80211_MODE_11AC_VHT160)) {
            ...
            if(ic->ic_precac_enable) {
                /* find ieee80211_channel for 160MHz for prefered channel
                 * precac_prefered_chan will be 0 when curchan==precac_pref_chan
                 * use curchan if prefered_chan chan is 0
                 */
                if (ic->ic_precac_prefered_chan)
                    prefered_freq = ieee80211_ieee2mhz(ic, ic->ic_precac_prefered_chan, 0);
                else
                    prefered_freq = ieee80211_ieee2mhz(ic, ic->ic_curchan->ic_ieee, 0);
                    prefered_chan = ieee80211_find_channel(ic, prefered_freq, 0, IEEE80211_CHAN_11AC_VHT160);

               /* For 160MHz,do preCAC only for prefered chan.
                * we won't continue precac for other channels.
                */
                if ((chan_mode == IEEE80211_MODE_11AC_VHT80) ||
                    ((chan_mode == IEEE80211_MODE_11AC_VHT160) && prefered_chan &&
                      !ieee80211_dfs_is_precac_done(ic, prefered_chan))) {
...
ol_ath_vdev_start_send(struct ol_ath_softc_net80211 *scn, u_int8_t if_id,
        struct ieee80211_ath_channel *chan, u_int32_t freq, bool disable_hw_ack,
        void *nl_handle)
{
    struct vdev_start_params param;
...
int
ol_ath_vdev_set_nac_rssi(struct ol_ath_softc_net80211 *scn, struct ieee80211vap *vap, u_int8_t if_id,
                           u_int32_t action, u_int8_t bssid_macaddr[IEEE80211_ADDR_LEN],
                           u_int8_t client_macaddr[IEEE80211_ADDR_LEN], u_int8_t chan_num)
{
    ...
    if (IEEE80211_NAC_RSSI_PARAM_LIST == action)
    {
        struct stats_request_params list_param = {0};
        u_int8_t macaddr[IEEE80211_ADDR_LEN] = {0};

        list_param.vdev_id = (OL_ATH_VAP_NET80211(vap))->av_if_id;
        list_param.stats_id = WMI_HOST_REQUEST_NAC_RSSI;

        return wmi_unified_stats_request_send(scn->wmi_handle, macaddr, &list_param);
    }

    param.action = action;
    param.chan_num = chan_num;

    qdf_mem_copy(&param.bssid_addr, bssid_macaddr,IEEE80211_ADDR_LEN);
    qdf_mem_copy(&param.client_addr, client_macaddr,IEEE80211_ADDR_LEN);
    IEEE80211_DPRINTF(vap, IEEE80211_MSG_NAC, "%s :vdev =%x, action=%x, bssid_macaddr[0][5]=%2x%2x, client_macaddr[0][5]=%2x%2x",
                      __func__, if_id, action, bssid_macaddr[0],bssid_macaddr[5], client_macaddr[0],client_macaddr[5]);
    return wmi_unified_vdev_set_nac_rssi_send(scn->wmi_handle, &param);
}
...
int
ol_ath_vdev_config_fils(struct ol_ath_softc_net80211 *scn,
                        struct ieee80211vap *vap, u_int32_t fd_period)
{
    struct config_fils_params param;
    struct ol_ath_vap_net80211 *avn = OL_ATH_VAP_NET80211(vap);

    if (!scn->bcn_mode) {
        qdf_print("Beacon mode set to staggered. Cannot enable FD\n");
        return -1;
    }

    avn->av_fd_period = fd_period;
    if (!vap->iv_vap_is_down) {
        qdf_print("%s Configuring FD frame with period %d \n",__func__, fd_period);
        qdf_mem_set(&param, sizeof(param), 0);
        param.vdev_id = avn->av_if_id;
        param.fd_period = fd_period;
        return wmi_unified_vdev_config_fils_cmd_send(scn->wmi_handle, &param);
    } else {
        return 0;
    }
}
              ...
              ieee80211_param param, u_int32_t val)
{
    ...
    switch (param) {
        ...
        break;
        case IEEE80211_ATF_GRP_AC:
        retval = ol_ath_set_atf_grp_ac(ic);
        break;
    ...
    case IEEE80211_TXRX_FW_STATS:
        {
            ...
            else if (val == TXRX_FW_COEX_STATS) { /*for fetch coex stats*/
   ...
   case IEEE80211_VAP_TXRX_FW_STATS_RESET:
    ...
    {
        ol_ath_wmi_send_vdev_param(scn,avn->av_if_id,wmi_vdev_param_tx_power, val);
    }
    break;
    ...
    case IEEE80211_FEATURE_FILS:
        retval = ol_ath_vdev_config_fils(scn, vap, (val & 0xFFFF) );
    break;
#endif
...
ol_ath_vap_up(struct ieee80211vap *vap)
{
        ...
        case IEEE80211_M_IBSS:
            ...
            if (ic->ic_precac_enable &&
                (vap->iv_des_mode == IEEE80211_MODE_11AC_VHT160) &&
                 !vap->iv_pre_cac_timeout_channel_change) {

                /* find ieee80211_channel for 160MHz for prefered channel
                 * precac_prefered_chan wil be 0 when curchan==precac_pref_chan
                 * use curchan if prefered_chan chan is 0
                 */
                if (ic->ic_precac_prefered_chan)
                    prefered_freq = ieee80211_ieee2mhz(ic, ic->ic_precac_prefered_chan, 0);
               ...
               if (prefered_chan && !ieee80211_dfs_is_precac_done(ic, prefered_chan)) {

                    wlan_set_param(vap, IEEE80211_CHWIDTH, IEEE80211_CWM_WIDTH80);
                } else {
                    wlan_set_param(vap, IEEE80211_CHWIDTH, IEEE80211_CWM_WIDTH160);
                }
            }
#endif
    ...
    ol_ath_fils_discovery_alloc(ic, avn->av_if_id);
    if (avn->av_fd_period &&
        ((avn->av_fd_period >= vap->iv_bss->ni_intval) ||
        (vap->iv_bss->ni_intval % avn->av_fd_period != 0))) {
        qdf_print("%sInvalid FD Interval : %d. Disabling FD\n",
                   __func__, avn->av_fd_period);
         avn->av_fd_period = 0;
    }
    ol_ath_vdev_config_fils(scn, vap, avn->av_fd_period);
#endif
                                       ...
                                       u_int8_t *data, u_int16_t datalen)
{
    ...
    if (!ic->ic_nl_handle) {
        qdf_print("STOPPED EVENT for vap %d (%pK)\n",vdev_id, scn->wmi_handle);
    }
...
#if ATH_SUPPORT_NAC_RSSI
static int
ol_ath_scan_nac_rssi(struct ieee80211vap *vap,
                   ...
                   u_int8_t client_macaddr[IEEE80211_ADDR_LEN], u_int8_t chan_num)
{
   ...
   /* send the key to wmi layer  */
   if (ol_ath_vdev_set_nac_rssi(scn, vap, avn->av_if_id, action, bssid_macaddr, client_macaddr, chan_num)) {
                  ...
                  void                *osifp_handle)
{
    ...
    if (opmode == IEEE80211_M_MONITOR) {
        ...
        if (scn->mon_vdev_count > CFG_TGT_MAX_MONITOR_VDEV) {
            ...
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: devhandle (%pK) the ap_monitor vdev count exceeds the supported number %d\n",
        ...
        if (scn->special_ap_vap && !(scn->smart_ap_monitor)) {
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s() devhandle (%pK) Radio is configured in Special ap monitor mode we can't create any other vap except 1 stavap or ap smart monitor\n",
                 __func__, ic);
            ...
            if (flags & IEEE80211_SPECIAL_VAP) {
                ...
                if ((flags & IEEE80211_SMART_MONITOR_VAP) && !(scn->smart_ap_monitor)) {
                    ...
                    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s() devhandle (%pK) Already radio having  Normal AP or STA vaps we can't create ap monitor vap\n",
                         __func__, ic);
    ...
    if (vlimit_exceeded) {
        ...
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: No more VAP's allowed. devhandle (%pK) scn (%pK) vdev_count: %d, mon_vdev_count: %d, num_vdevs: %d\n",
    ...
    if ((opmode == IEEE80211_M_STA) && (scn->sc_nstavaps >0)) {
       if (!(flags & IEEE80211_WRAP_WIRED_STA) && !(flags & IEEE80211_CLONE_MATADDR )) {
           ...
           QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "\n\n %s() devhandle (%pK) A STA is already created on this radio. Not creating this STA\n\n",
               __func__, ic);
           goto err_vap_create;
       }
    }
...
#if UMAC_SUPPORT_FILS
    /* Initialize FILS FD deferred list */
    TAILQ_INIT(&avn->deferred_fd_list);
#endif
    vap = &avn->av_vap;
...
#define ATH_NSCAN_PSTA_VAPS 0
    ...
    if ( nss_if == -1) {
        ...
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s() devhandle (%pK) NSS WiFi Offload unable to allocate VAP %d\n",
             __func__, ic, id);
        goto err_nss_vdev;
    }
#endif
    ...
    if (opmode == IEEE80211_M_STA) {
         ...
         */
        if(qdf_atomic_read(&scn->peer_count) <= 0) {
            goto err_peer_count;
        }
        if (qdf_atomic_dec_and_test(&scn->peer_count)) {
            qdf_print("Unable to create STAVAP, peer count exceeds\n");
...
#endif
#if ATH_SUPPORT_NAC_RSSI
    vap->iv_scan_nac_rssi = ol_ath_scan_nac_rssi;
#endif
#if 0
...
#define WMI_BEACON_RATE_6M   0x03
    ...
    if (ieee80211_vap_attach(vap)) {
        qdf_print("ieee80211_vap_attach failed\n");
        goto fail2;
	}
...
fail2:
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
    osif_nss_ol_vap_delete(osifp_handle);
#endif
    ol_txrx_vdev_detach(avn->av_txrx_handle, NULL, NULL);
err_txrx_attach:
    ...
    */
    ...
    if (tgt_vdev_created
        ...
        vdev_delete_failed = 1;
    }
err_vdev_create:
    if (!vdev_delete_failed && (opmode == IEEE80211_M_STA)) {
        /* For STAVAP, self peer will get deleted on FW
         * for accountability, increment peer count here
         */
...
ol_ath_vap_delete(struct ieee80211vap *vap)
{
    ...
    qdf_print("%s: wmi_unified_vdev_delete_send done ID = %d vap (%pK) VAP Addr = %02x:%02x:%02x:%02x:%02x:%02x:\n",
