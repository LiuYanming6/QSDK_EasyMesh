...
static int
wmi_lowi_oem_cap_resp_event_handler(ol_scn_t scn, u_int8_t *data,
                    u_int16_t datalen)
{
...
static int
wmi_lowi_oem_meas_report_event_handler(ol_scn_t scn, u_int8_t *data,
                    u_int16_t datalen)
{
    struct ieee80211com *ic = &scn->sc_ic;
...
static int
wmi_lowi_oem_err_report_event_handler(ol_scn_t scn, u_int8_t *data,
                    u_int16_t datalen)
{
    struct ieee80211com *ic = &scn->sc_ic;
 ...
 */
void
ol_ath_lowi_wmi_event_attach(struct ieee80211com *ic)
{
                                  ...
                                  int msg_len, void *req, int msgsubType)
{
	...
	if (wmi_unified_lcr_set_cmd_send(scn->wmi_handle, &lcr_param)) {
        	qdf_print("%s: ERROR: Host unable to send LCR LOWI request to FW \n", __func__);
        	return -1;
    	}
	return 0;
    }
	...
	if (wmi_unified_lci_set_cmd_send(scn->wmi_handle, &lci_param)) {
        	qdf_print("%s: ERROR: Host unable to send LCI LOWI request to FW \n", __func__);
        	return -1;
    	}
    ...
    if (wmi_unified_start_oem_data_cmd(scn->wmi_handle, msg_len, req)) {
	    qdf_print("%s: ERROR: Host unable to send OEM request to FW \n", __func__);
	    return -1;
    }
...
static void ol_ath_cache_lcr(void *req)
{
    ...
    num_ap_lcr = 1;
    qdf_print("Copied LCR data to driver cache (%d octets)\n", loc_civic_len);
}
