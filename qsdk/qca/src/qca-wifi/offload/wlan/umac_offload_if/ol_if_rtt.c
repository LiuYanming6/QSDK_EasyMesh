                           ...
                           ieee80211_ol_wifiposdesc_t * wifiposdesc)
{
    ...
    qdf_print("RTTREPORT Request ID=%u status=0x%x MeasurementType=%s ReportType=%d \n\
            ...
            header->num_ap);
    qdf_print("MAC=%.2x:%.2x:%.2x:%.2x:%.2x:%.2x\n", dest_mac[0], dest_mac[1], dest_mac[2], dest_mac[3], dest_mac[4], dest_mac[5]);
...
static int
ol_ath_rtt_meas_report_event_handler(ol_scn_t scn, u_int8_t *data,
                    u_int16_t datalen)
{
#if !ATH_SUPPORT_LOWI
...
#define HDUMP_LEN 324
    ...
    if(wmi_extract_rtt_hdr(scn->wmi_handle, data, &hdr)) {
    ...
    if(wmi_extract_rtt_ev(scn->wmi_handle, data, &body,
 ...
 */
static int
ol_ath_tsf_meas_report_event_handle(ol_scn_t scn, u_int8_t *data,
                    u_int16_t datalen)
 ...
 */
static int
ol_ath_error_report_event_handle(ol_scn_t scn, u_int8_t *data,
                    u_int16_t datalen)
{
    ...
    qdf_print("%s: data=%pK, datalen=%u\n", __func__, data, datalen);
    if (!data) {
        qdf_print("Get NULL point message from FW\n");
        return -1;
    }

    if(wmi_extract_rtt_error_report_ev(scn->wmi_handle, data, &ev)) {
        qdf_print("%s:\n Unable to extract RTT header \n", __func__);
        return -1;
    }
    ...
    if (IS_VALID_INDEX_OF_ARRAY(error_indicator, ev.reject_reason)) {
        qdf_print("%s\n",error_indicator[ev.reject_reason] );
    }
    else {
        qdf_print("Unknown error index (%d)\n", ev.reject_reason );
    }
    ...
    /* Only 3 type of frames are used for Measurement type ,
       even though  RTT measurement type is 3 bit header */
    qdf_print("Measurement Type is %s \n", (ev.hdr.meas_type < 3)? measurement_type[ev.hdr.meas_type]:"Invalid frame type");
    qdf_print("Report Type is %d\n", ev.hdr.report_type);
 ...
 */
static int
ol_ath_rtt_keepalive_event_handle(ol_scn_t scn, u_int8_t *data,
                    u_int16_t datalen)
{
    ...
    if(wmi_extract_rtt_hdr(scn->wmi_handle, data, &header)) {
        qdf_print("%s:Unable to extract rtt header\n", __func__);
        return -1;
    }
    qdf_print("\n\n==================Keepalive Event====================\n");
    qdf_print("Request ID is:0x%x\n", header.req_id);
    qdf_print("Request result is:%d\n", header.result);

    dest_mac = header.dest_mac;
    qdf_print("Request dest_mac is:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x\n", dest_mac[0],dest_mac[1],
...
ol_if_rtt_detach(struct ieee80211com *ic)
{
    ...
    for (i = 0; i < ol_num_global_scn; i++) {
        ...
        if (ic_tmp->rtt_sock == NULL) {
            ...
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Socket already released %pK\n", __func__, ic->rtt_sock);
            return;
        }
    }
...
int rtt_find_channel_info (void *arg, wlan_scan_entry_t scan_entry)
{
    ...
    wmi_chan->phy_mode = ol_get_phymode_info(chan_mode);
...
void ol_ath_rtt_meas_req_test(struct ieee80211com *ic, u_int8_t *mac_addr, int extra)
{
	...
	*/
    ...
    ret = wmi_unified_rtt_meas_req_test_cmd_send(scn->wmi_handle, &param);
    req_id++;
}
#endif
/*
                    ...
                    struct ieee80211vap *vap, ieee80211_wifipos_reqdata_t *reqdata)
{
    ...
    ret = wmi_unified_rtt_meas_req_cmd_send(scn->wmi_handle, &param);
}
                         ...
                         struct ieee80211_node *ni, bool stop)
{
    ...
    ret = wmi_unified_rtt_keepalive_req_cmd_send(scn->wmi_handle, &param);
    req_id = param.req_id;
    return;
}
                    ...
                    void *lci_data)
{
    ...
    if(wmi_unified_lci_set_cmd_send(scn->wmi_handle, &param)) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:Unable to set LCI data\n", __func__);
        return -1;
    }
	...
	/* Max size of LCI is 16 octets */
        ...
        num_ap_lci = 1;
        qdf_print("Copied LCI num %d to driver cache\n", num_ap_lci);
    }
    return 0;
}
                    ...
                    void *lcr_data)
{
    ...
    if(wmi_unified_lcr_set_cmd_send(scn->wmi_handle, &param)) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:Unable to set LCR data\n", __func__);
        return -1;
    }
    ...
    num_ap_lcr = 1;
    qdf_print("Copy LCR num %d to driver cache\n", num_ap_lcr);
