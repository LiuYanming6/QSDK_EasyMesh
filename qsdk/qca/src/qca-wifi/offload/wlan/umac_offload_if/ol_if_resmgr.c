...
#endif
    /* Consider a case where CSA is triggered on a channel.
     * Within the tbtt expiry, channel on which CSA was triggered
     * is removed from ic channel list and AP's curchan is changed.
     * In the next SWBA, CSA is tried on the old channel which is non
     * existant. Hence during vap down/restart, clear the chanswitch
     * flags if set to avoid undesirable CSA on non-existant channel.
     */
...
ol_vdev_wmi_event_handler(ol_scn_t sc, u_int8_t *data, u_int16_t datalen)
{
    ...
    if (!ic->ic_nl_handle) {
        QDF_PRINT_INFO(vaphandle->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ol_vdev_start_resp_ev for vap %d (%pK)\n", vdev_start_resp.vdev_id, scn->wmi_handle);
    }
    ...
    switch (vaphandle->iv_opmode) {
        ...
        case IEEE80211_M_HOSTAP:
             ...
             */
            if (vdev_start_resp.status == WMI_HOST_VDEV_START_CHAN_BLOCKED) {
                IEEE80211_DPRINTF(vaphandle, IEEE80211_MSG_ANY,
                        "Error %s : failed vdev start vap %d status %d\n",
                        __func__, vdev_start_resp.vdev_id, vdev_start_resp.status);
                /* init_in_progress is set to 1 in _ieee80211_resmgr_vap_start.
                 * If not cleared, further operations on vap are not done
                 * and start_send is not sent to FW.
                 */
                qdf_atomic_set(&(vaphandle->init_in_progress), 0);
                /* init_lock is acquired in _ieee80211_resmgr_vap_start.
                 * if not unlocked here, it ends up in a soft-lockup.
                 */
                spin_unlock_dpc(&vaphandle->init_lock);
                if (!(IEEE80211_IS_PRIMARY_OR_SECONDARY_CHAN_DFS (ic))) {
                    IEEE80211_DPRINTF(vaphandle, IEEE80211_MSG_ANY,
                            "Not honoring start response failure as action is"
                            "already taken for vap: %d\n",vdev_start_resp.vdev_id);
                } else {
		 ...
		 */
            ...
            /* This is just a precautionary measure to ensure that even if
             * usenol is set to 0 mistakenly by user before spoof test completion,
             * it resets back to default value 1
             */

            if (ic->ic_dfs_usenol(ic) == 0 &&
                    ic->ic_is_host_dfs_check_enabled &&
                    ic->ic_spoof_test_done == 0 &&
                    DFS_FCC_DOMAIN == ic->ic_get_dfsdomain(ic)) {
#define DFS_SET_USENOL		5
                IEEE80211_DPRINTF(vaphandle, IEEE80211_MSG_DFS,
                "%s: usenol cant be set to 0 before spoof test completion\n"
                        "Setting to 1 again\n",__func__);
                usenol = 1;
...
#endif
            {
                ...
                if (ic->ic_dfs_state.dfs_disable &&
                        ic->ic_get_dfsdomain(ic) != DFS_FCC_DOMAIN) {
                    ic->ic_dfs_state.dfs_disable = 0;
                }
                if (!ic->ic_dfs_state.dfs_disable) {
                     ol_if_dfs_configure(ic);
                } else {
                    qdf_print("%s:DFS is disabled\n",__func__);
                }
            }
    ...
    if(vaphandle->iv_opmode == IEEE80211_M_MONITOR ||
       vaphandle->iv_opmode == IEEE80211_M_HOSTAP) {
        if (ieee80211_vap_join(vaphandle) != 0) {
