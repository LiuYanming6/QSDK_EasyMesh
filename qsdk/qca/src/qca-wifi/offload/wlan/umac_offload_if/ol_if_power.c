        ...
        A_UINT32 value)
{
    ...
    return wmi_unified_ap_ps_cmd_send(svn->wmi_handle, anode->an_node.ni_macaddr,
        &ps_param);
}
        ...
        A_UINT32 value)
{
    ...
    return wmi_unified_sta_ps_cmd_send(svn->wmi_handle, &ps_param);
}
        ...
        ieee80211_pwrsave_mode psmode)
{
    ...
    return wmi_unified_set_psmode_cmd_send(svn->wmi_handle, &param);
}
        ...
        ieee80211_pwrsave_mode mode)
{
    ...
    if (!sta_handle->force_sleep) {
        ...
        err = ol_ath_wmi_send_vdev_param(scn, avn->av_if_id,
                         wmi_vdev_param_listen_interval, lintval);
        if (err) {
            goto error;
        }
