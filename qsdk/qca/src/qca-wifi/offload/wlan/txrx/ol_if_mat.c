...
#ifdef ATH_MAT_TEST
	QDF_PRINT_INFO(out_vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s: src %s type 0x%x",__func__,ether_sprintf(src_mac),ether_type);
	QDF_PRINT_INFO(out_vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "des %s\n",ether_sprintf(des_mac));
#endif
...
#ifdef ATH_DEBUG_MAT
			QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s:%d sport %d dport %d\n",__func__,__LINE__,p_udp->source,p_udp->dest);
#endif
			/*
...
#ifdef ATH_DEBUG_MAT
				QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s:%d sport %d dport %d len %d chaddr %s\n",__func__,__LINE__,p_udp->source,p_udp->dest,udplen,ether_sprintf(p_dhcp->chaddr));
#endif
				if(p_dhcp->magic_cookie[0] == 0x63 && p_dhcp->magic_cookie[1] == 0x82 && p_dhcp->magic_cookie[2] == 0x53 && p_dhcp->magic_cookie[3] == 0x63) {
#ifdef ATH_DEBUG_MAT
					QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " Found DHCP cookie .. \n");
#endif
					if (p_dhcp->op == 1) {
...
#ifdef ATH_DEBUG_MAT
							QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " Make the dhcp flag to broadcast ... \n");
#endif
...
#ifdef ATH_MAT_TEST
	QDF_PRINT_INFO(in_vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s: src %s type 0x%x",__func__,ether_sprintf(src_mac),ether_type);
	QDF_PRINT_INFO(in_vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "des %s\n",ether_sprintf(des_mac));
#endif
...
#ifdef ATH_DEBUG_MAT
				QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s:%d sport %d dport %d chaddr %s\n",__func__,__LINE__,p_udp->source,p_udp->dest,ether_sprintf(p_dhcp->chaddr));
#endif
				...
				if(ismcast){
					...
					if(IEEE80211_ADDR_EQ(p_dhcp->chaddr,in_vap->iv_myaddr)) {
						if(avn->av_use_mat == 1) {
							...
							IEEE80211_ADDR_COPY(p_dhcp->chaddr,avn->av_mat_addr);
#ifdef ATH_DEBUG_MAT
							QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s:%d chaddr %s -> ",__func__,__LINE__,ether_sprintf(orig_chaddr));
							QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s\n",ether_sprintf(p_dhcp->chaddr));
#endif
							p_udp->check = update_checksum_addr(p_udp->check,orig_chaddr,avn->av_mat_addr);
						}
					}else{
