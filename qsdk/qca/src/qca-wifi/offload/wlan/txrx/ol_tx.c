 ...
 */
inline void
ol_txrx_classify(struct ol_txrx_vdev_t *vdev, qdf_nbuf_t nbuf,
        enum ol_txrx_pkt_direction dir, struct ol_txrx_nbuf_classify *nbuf_class)
{
...
#if QCA_PARTNER_DIRECTLINK
    /*Directlink peermap table sync will be done by DL TX*/
    if (ath_is_directlink(pdev)) {
        printk("ERROR: %s called in Directpath - Investigate\n",__func__);
        qdf_assert(0);
    }
#endif /* QCA_PARTNER_DIRECTLINK */
...
ol_pflow_update_pdev_params(struct ol_txrx_pdev_t *pdev, ol_ath_param_t param, u_int32_t value, void *buff)
{
    switch(param)
    {
        ...
        case OL_ATH_PARAM_VIDEO_DELAY_STATS_FC:
            {
                if (value == 1) {
                    printk("===> Video Delay Stats Enabled <=== \n");
...
#if QCA_PARTNER_DIRECTLINK
                    ol_tx_pflow_ctrl_update_partner(pdev);
#endif
                }
                else
...
{
    qdf_print("\nhtt_tx_desc bad for tx_desc:%pK, id (%d), htt_tx_status (%d), allocated (%d)\n",tx_desc, tx_desc->id, tx_desc->status, tx_desc->allocated);
}
        ...
        qdf_nbuf_t netbuf, uint32_t peerid, uint8_t tid, struct ol_tx_desc_t *tx_desc )
{
    ...
    if(QDF_STATUS_E_FAILURE == qdf_nbuf_map_nbytes_single(vdev->osdev, netbuf, QDF_DMA_TO_DEVICE, netlen)){
	    qdf_print("Map error \n");
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Map Error\n");
        if (OL_CFG_RAW_TX_LIKELINESS(vdev->tx_encap_type == htt_pkt_type_raw)) {
...
uint8_t
ol_tx_frame_is_drop(ol_txrx_vdev_handle vdev, qdf_nbuf_t nbuf)
{
    u_int8_t ftype = 0;
    struct ol_tx_me_buf_t *mcbuf;
...
#else
        {
            ...
            if ( ( ftype == CB_FTYPE_MCAST2UCAST)
                ...
                if (pdev->carrier_vow_config && status == OL_TXRX_SUCCESS) {
                    ...
                    peer = ol_txrx_peer_find_by_id(pdev, nbuf_class.peer_id);
                    if (peer)
    ...
    while (tx_pkts <= max_pkts) {
        ...
        if (netbuf) {
            ...
            if (tx_desc) {
                 ...
                 * Delay Counters
                 */
                if (pdev->delay_counters_enabled && netbuf->tstamp.tv64)
...
#if PEER_FLOW_CONTROL_DEBUG
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s dequeued %d tx_pkts and %d tx_bytes \n", num_tx_pkts, num_tx_bytes);
#endif
...
#if QCA_PARTNER_DIRECTLINK
    if (ath_is_directlink(pdev)) {
        ol_tx_partner(netbuf, OSIF_TO_NETDEV(vdev->osif_vdev), peer_id);
        return OL_TXRX_SUCCESS;
    }
#endif /* QCA_PARTNER_DIRECTLINK */
    ...
    if (scn->sc_arp_dbg_conf) {
        ...
        if (iph->protocol == IPPROTO_ICMP ) {
            ...
            if ((iph->saddr == ntohl(scn->sc_arp_dbg_srcaddr)) && (iph->daddr == ntohl(scn->sc_arp_dbg_dstaddr))) {
                QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: ICMP type: %d: Sequence: %u From Source Address: 0x%08x To Destination Address: 0x%08x \n", __func__, icmp->type, icmp->un.echo.sequence
                        , iph->saddr , iph->daddr);
            }
        }
...
ol_tx_flow_ctrl_tidq_stats(struct ol_txrx_pdev_t *pdev, u_int32_t peer_id, u_int8_t tid)
{
    ...
    */
    ...
    qdf_print("Pkts stay time| %llu\n", peer->tstampstat.sum[tid]);
    qdf_print("Msdus stayed  | %llu\n", peer->tstampstat.nummsdu[tid]);
    qdf_print("EWMA time diff| %llu\n", peer->tstampstat.avg[tid]);
...
void ol_txrx_per_peer_stats(struct ol_txrx_pdev_t *pdev, char *addr)
{
    ...
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Found peer_id=%d\n", peer_id);

    if (peer_id == HTT_INVALID_PEER) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "No peer found\n");
        return;
    }

    if (peer_id >= OL_TXRX_MAX_PEER_IDS) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Invalid Peer Id=%d\n", peer_id);
        return;
    }
...
#if !PEER_FLOW_CONTROL
    {
        ...
        while (scn_qcnt) {
            ...
            if (netbuf) {
                ...
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Error: Broken acnbufq, remaining qlen: %d\n",
                    __func__, scn_qcnt);
                break;
            }
            scn_qcnt--;
