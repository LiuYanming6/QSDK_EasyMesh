...
typedef void
    (*ol_txrx_cfr_rx_ind_cb)(void *ctxt,struct htt_cfr_info *pcfr,uint32_t cfrdatalen);
...
struct ol_txrx_pdev_t {
    ...
    struct {
            ol_txrx_cfr_rx_ind_cb cfr_rx_htt_ind;
            void *ctxt;
    } cfr_callback;
    /* tx descriptor pool */
    struct {
...
struct ol_txrx_interframe{
    qdf_ktime_t start_tick;   /* This gets updated with current tick value when first frame is received for a particular group */
    qdf_ktime_t max_interframe_delay; /* This stores the maximum relative value of interframe delay [ we can covert this to sorted array if need more]*/
    qdf_ktime_t max_delay;         /* The ticks value in absolute terms when the max_interframe delay is observed */
    qdf_ktime_t last_frame_tick;   /* The ticks value in absolute terms when the max_interframe delay is observed */
};
...
struct ol_txrx_vdev_t {
    ...
    ol_txrx_tx_fp tx;
    /* classify function used by this vdev */
    ol_txrx_classify_fp txrx_classify;
...
struct txrx_tidq_stats_t {
    u_int32_t stats[TIDQ_STATS_MAX];
};
...
struct ol_txrx_peer_t {
    ...
    struct txrx_tidq_stats_t tidq_stats[OL_TX_PFLOW_CTRL_HOST_MAX_TIDS];
#endif
