...
{
    struct ol_tx_tso_desc_t *sw_tso_sg_desc = (struct ol_tx_tso_desc_t *) qdf_nbuf_get_fctx(netbuf);
...
ol_tx_free_buf_generic(struct ol_txrx_pdev_t *pdev, qdf_nbuf_t netbuf)
{
    ...
    if (qdf_nbuf_get_ftype(netbuf) == CB_FTYPE_MCAST2UCAST) {
        ol_tx_me_free_buf(pdev, (struct ol_tx_me_buf_t *)qdf_nbuf_get_fctx(netbuf));
    }
#endif

#if HOST_SW_TSO_SG_ENABLE
        if (qdf_nbuf_get_ftype(netbuf) == CB_FTYPE_TSO_SG) {
            ol_tx_tso_sg_desc_free(pdev,(struct ol_tx_tso_desc_t *) qdf_nbuf_get_fctx(netbuf));
        }
#endif

#if HOST_SW_SG_ENABLE
        if (qdf_nbuf_get_ftype(netbuf) == CB_FTYPE_SG) {
            ol_tx_sg_desc_free(pdev, (struct ol_tx_sg_desc_t *) qdf_nbuf_get_fctx(netbuf));
        }
#endif
...
extern void wlan_tx_stats_collect_ar98xx(struct ieee80211com *ic,
