...
ol_tx_tso_desc_display(void *tx_desc)
{
    ...
    qdf_print("tx_tso_desc %x \n",(A_UINT32 )tx_tso_desc);
    qdf_print("HOST_TSO::tso_final_segment %x\n",(A_UINT32 )tx_tso_desc->tso_final_segment);
    qdf_print("HOST_TSO::ip_hdr_type %x\n",(A_UINT32 )tx_tso_desc->ip_hdr_type);
    if(tx_tso_desc->ip_hdr_type == TSO_IPV4)
        qdf_print("HOST_TSO::iphdr %x\n",(A_UINT32 )(tx_tso_desc->iphdr));
    else
        qdf_print("HOST_TSO::ip6hdr %x\n",(A_UINT32 )(tx_tso_desc->iphdr));

    qdf_print("HOST_TSO::tcphdr %x\n",(A_UINT32 )tx_tso_desc->tcphdr);
    qdf_print("HOST_TSO::l2_length %x\n",tx_tso_desc->l2_length);
    qdf_print("HOST_TSO::l3_hdr_length %x\n",tx_tso_desc->l3_hdr_size);
    qdf_print("HOST_TSO::l2_l3_l4_hdr_size %x\n",tx_tso_desc->l2_l3_l4_hdr_size);
    qdf_print("HOST_TSO::ref_cnt %x\n",tx_tso_desc->ref_cnt);
}
...
ol_tx_msdu_desc_display(void *msdu_desc)
{
    ...
    qdf_print("HOST_TSO::tso_enable %x\n",msdu_link_ext_p->tso_enable);
    qdf_print("HOST_TSO::tcp_flag %x\n",msdu_link_ext_p->tcp_flag);
    qdf_print("HOST_TSO::tcp_flag_mask %x\n",msdu_link_ext_p->tcp_flag_mask);
    qdf_print("HOST_TSO::l2_length %x\n",msdu_link_ext_p->l2_length);
    qdf_print("HOST_TSO::ip_length %x\n",msdu_link_ext_p->ip_length);
    qdf_print("HOST_TSO::tcp_seq_number %x\n",msdu_link_ext_p->tcp_seq_number);
    qdf_print("HOST_TSO::ip_identification %x\n",msdu_link_ext_p->ip_identification);
    qdf_print("HOST_TSO::ipv4_checksum_en %x\n",msdu_link_ext_p->ipv4_checksum_en);
    qdf_print("HOST_TSO::udp_over_ipv4_checksum_en %x\n",msdu_link_ext_p->udp_over_ipv4_checksum_en);
    qdf_print("HOST_TSO::udp_over_ipv6_checksum_en %x\n",msdu_link_ext_p->udp_over_ipv6_checksum_en);
    qdf_print("HOST_TSO::tcp_over_ipv4_checksum_en %x\n",msdu_link_ext_p->tcp_over_ipv4_checksum_en);
    qdf_print("HOST_TSO::tcp_over_ipv6_checksum_en %x\n",msdu_link_ext_p->tcp_over_ipv6_checksum_en);
    qdf_print("HOST_TSO::buf0_ptr_31_0 %x\n",msdu_link_ext_p->buf0_ptr_31_0);
    qdf_print("HOST_TSO::buf0_len %x\n",msdu_link_ext_p->buf0_len);
    qdf_print("HOST_TSO::buf1_ptr_31_0 %x\n",msdu_link_ext_p->buf1_ptr_31_0);
    qdf_print("HOST_TSO::buf1_len %x\n",msdu_link_ext_p->buf1_len);
                             ...
                             struct ol_txrx_pdev_t *pdev)
{
    ...
    if (qdf_unlikely((typeorlength != ADF_ETH_TYPE_IPV4) &&
        ...
        qdf_print("HOST_TSO::Dropping Pkt->Not an IPV4/V6 packet %d \n",typeorlength);
        return NULL;
    }
...
#if TSO_DBG
    qdf_print("MSS Size %d Pkt Len %d \n",mss_size,msdu_len);
#endif
    ...
    ol_txrx_vdev_handle vdev)
{
    ...
    qdf_print("TSO TX Pkts       : %lu\n", stats->tx.tso.tso_pkts.pkts);
    qdf_print("TSO TX Bytes      : %lu\n", stats->tx.tso.tso_pkts.bytes);
    qdf_print("Non-TSO TX Pkts   : %lu\n", stats->tx.tso.non_tso_pkts.pkts);
    qdf_print("Non-TSO TX Bytes  : %lu\n", stats->tx.tso.non_tso_pkts.bytes);
    qdf_print("TSO Dropped Pkts  : %lu\n", stats->tx.tso.tso_dropped.pkts);
    qdf_print("TSO Dropped Bytes : %lu\n", stats->tx.tso.tso_dropped.bytes);
    qdf_print("TSO Desc Total %d In Use : %d\n", pdev->tx_tso_desc.pool_size, \
                                             stats->tx.tso.tso_desc_cnt);
}
