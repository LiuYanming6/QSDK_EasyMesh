    ...
    ol_txrx_vdev_handle vdev)
{
    ...
    if(stats) {
        ...
        qdf_print("TSO SG TX Pkts       : %llu\n", stats->tx.tso.tso_pkts.pkts);
        qdf_print("TSO SG TX Bytes      : %llu\n", stats->tx.tso.tso_pkts.bytes);
        qdf_print("Non-TSO SG TX Pkts   : %llu\n", stats->tx.tso.non_tso_pkts.pkts);
        qdf_print("Non-TSO SG TX Bytes  : %llu\n", stats->tx.tso.non_tso_pkts.bytes);
        qdf_print("TSO SG Dropped Pkts  : %llu\n", stats->tx.tso.tso_dropped.pkts);
        qdf_print("TSO SG Dropped Bytes : %llu\n", stats->tx.tso.tso_dropped.bytes);
...
ol_tx_rst_tso_stats(ol_txrx_vdev_handle vdev)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
        "tso_stats %llu %llu %llu %llu \n", stats->tx.tso.tso_pkts.pkts,
        stats->tx.tso.tso_pkts.bytes, stats->tx.tso.non_tso_pkts.pkts,
        stats->tx.tso.non_tso_pkts.bytes);
}
