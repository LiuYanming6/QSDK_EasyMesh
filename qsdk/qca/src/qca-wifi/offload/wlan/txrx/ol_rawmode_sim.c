...
static int
decap_dot11_to_8023(qdf_nbuf_t msdu, struct ol_txrx_peer_t *peer);


/* Tx side helper functions */

static int encap_eth_to_dot11(
    qdf_nbuf_t nbuf, enum ieee80211_opmode mode,u_int8_t *bssid,struct ieee80211vap *vap,struct ol_txrx_vdev_t *vdev);
                      ...
                      u_int32_t preconsumed_bytes)
{
    ...
    if (preconsumed_bytes == head_frag_size) {
        ...
        if (qdf_nbuf_is_chfrag_end(sctx->currnbuf)) {
            sctx->nextnbuf = NULL;
        } else {
                      ...
                      u_int8_t *dst)
{
    ...
    if (soffset > OL_RX_FRAGSTREAM_PEEK_OFFSET_MAX) {
        qdf_print("Invalid value %u passed for offset\n", soffset);
        return -1;
    }
                             ...
                             struct ol_txrx_peer_t *peer)
{
    ...
    is_frag = qdf_nbuf_is_chfrag_start(*deliver_list_head);
    ...
    if (is_frag) {
        ...
        qdf_nbuf_set_chfrag_start(headsrcnbuf, 0);
 ...
 */
static int
decap_dot11_to_8023(qdf_nbuf_t mpdu, struct ol_txrx_peer_t *peer)
{
...
int add_mesh_meta_hdr(qdf_nbuf_t nbuf, struct ieee80211vap *vap)
{
    ...
    mhdr->power = dbg_mhdr & 0xff;
    mhdr->rates[0] = (dbg_mhdr >> 8) & 0xff;
    mhdr->retries = (dbg_mhdr >> 16)  & 0xf;
    mhdr->max_tries[0] = (dbg_mhdr >> 16) & 0xf;
    mhdr->keyix = (dbg_mhdr >> 20) & 0xf;
    mhdr->flags = (dbg_mhdr >> 24) & 0xff;
    ...
    qdf_nbuf_t nbuf, enum ieee80211_opmode mode, u_int8_t *bssid,struct ieee80211vap *vap,struct ol_txrx_vdev_t *vdev)
{
...
#if MESH_MODE_SUPPORT
#if PEER_FLOW_CONTROL
    ast_entry = ol_tx_rawsim_getastentry(vdev,nbuf);
    ...
    if (mesh_vap_mode) {
        if (ast_entry) {
            ast_peerid = ast_entry->peer_id;
            ast_peer = ol_txrx_peer_find_by_id(pdev, ast_peerid);
            if (!(ast_peer)) {
                qdf_print("peer not found \n");
                return -1;
            }
            if (vap->iv_rawmodesim_debug) {
                QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ast %s\n",ether_sprintf(ast_peer->mac_addr.raw));
            }
            if (!(IEEE80211_ADDR_EQ(eth_hdr.ether_dhost,ast_peer->mac_addr.raw))) {
                addr4_frame = 1;
            }
        } else {
        ...
        if (vap->iv_rawmodesim_debug) {
            QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "dst %s ",ether_sprintf(eth_hdr.ether_dhost));
            QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "src %s ",ether_sprintf(eth_hdr.ether_shost));
            QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "bssid %s \n",ether_sprintf(bssid));
            QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "is 4addr %d\n",addr4_frame);
        }
    }
#endif
#endif
        ...
        if (!addr4_frame) {
            ...
            if (!IEEE80211_IS_MULTICAST(eth_hdr.ether_dhost)) {
                if (ast_entry){
                    ...
                    ast_peer = ol_txrx_peer_find_by_id(pdev, ast_peerid);
                   if (ast_peer) {
                       IEEE80211_ADDR_COPY(wh->i_addr1, ast_peer->mac_addr.raw);
                   } else {
                        IEEE80211_ADDR_COPY(wh->i_addr1, eth_hdr.ether_dhost);
                  }
                } else {
        ...
        qdf_nbuf_t *pdeliver_list_tail)
{
   ...
   struct ieee80211vap *vap = ol_ath_getvap(vdev);
   ...
   while (nbuf) {
       ...
       is_chfrag_start = qdf_nbuf_is_chfrag_start(nbuf);
       ...
       if (is_amsdu < 0) {
           ...
           if (is_chfrag_start && nbuf) {
               while(nbuf) {
                    next = qdf_nbuf_next(nbuf);
                    is_chfrag_end = qdf_nbuf_is_chfrag_end(nbuf);
                    RAWSIM_TXRX_NODE_DELETE(*pdeliver_list_head,
                           *pdeliver_list_tail,
                           prev,
                           nbuf);
           ...
           while(nbuf && !qdf_nbuf_is_chfrag_end(nbuf)) {
           ...
           if (ret < 0) {
               ...
               if (vap->iv_rawmodesim_debug) {
                   ...
                   for (index = 0, tmpskb = deliver_sublist_head;
                           tmpskb != deliver_sublist_tail;
                           tmpskb = qdf_nbuf_next(tmpskb)) {
                       QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Fragment No : %d\n", ++index);
                       RAWSIM_PKT_HEXDUMP(qdf_nbuf_data(tmpskb),
                                          qdf_nbuf_len(tmpskb));
                   }
               }
...
ol_rsim_tx_encap(ol_txrx_vdev_handle vdev, qdf_nbuf_t *pnbuf)
{
    ...
    if(vap->iv_rawmodesim_debug) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Tx Ethernet II type packet hexdump before encap\n");
    ...
    if (vap->iv_rawmodesim_txaggr && is_ip_pkt && !check_multicast(*pnbuf)) {
        ...
        if (!more_frag && (nbuf_list_head == NULL)) {
            /* SW encapsulation */
            if(encap_eth_to_dot11(*pnbuf, wlan_vap_get_opmode(vap),bssid,vap,vdev)) {
                return -1;
            }
            ...
            if (!more_frag || (frag_count == NUM_RAWSIM_80211_MSDUS_IN_AMSDU)) {
