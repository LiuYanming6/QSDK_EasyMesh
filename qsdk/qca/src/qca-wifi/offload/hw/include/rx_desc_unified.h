...
#if USE_LITEPATH
    #define LITE_PATH_HEADROOM 128
    extern bool is_skb_mixed ( qdf_nbuf_t );
#endif
        ...
        u_int32_t* npackets)
{
    ...
    while (1) {
        ...
        /* Directlink Specific - unmap already done by partner */
        if (is_ar_directlink((void*)(ar->htt_handle))) {
            rx_buf_size = msdu->len;
        } else
#endif
{
        /*
         ...
         */
#if USE_LITEPATH
       if(!is_skb_mixed(msdu))
       {
               qdf_nbuf_set_pktlen(msdu, (rx_buf_size-LITE_PATH_HEADROOM));
       } else
       {
...
#if QCA_PARTNER_DIRECTLINK
        /* Directlink Specific - assert if msdu done bit is not set, recovery is not possible */
        if (is_ar_directlink((void*)(ar->htt_handle))) {
            if(!(((*(u_int32_t *) &rx_desc->attention) & RX_ATTENTION_0_MSDU_DONE_MASK) >> RX_ATTENTION_0_MSDU_DONE_LSB)) {
                printk("<%s> MSDU Done BIT is not set for msdu=%pK \n",__func__,msdu);
                qdf_assert(0);
            }
        }
        else
#endif
{
        ...
        rx_desc_size = ar->rx_callbacks->rx_desc_reservation(ar->htt_handle);
#if USE_LITEPATH
           if ( (!is_skb_mixed(msdu)) && (rx_desc_size >= 128) ) {
               rx_desc_size = rx_desc_size - 128;
           }
#endif

        qdf_nbuf_pull_head(msdu, (rx_desc_size + pad_bytes) );
        (*npackets)++;
        ...
        if (OL_CFG_RAW_RX_LIKELINESS(use_raw_decap) && msdu_chained) {
            ...
            if (qdf_likely(!fcs_error)) {
                is_validfragchain = 1;
                qdf_nbuf_set_chfrag_start(msdu, 1);
            }
        }
        ...
        while (msdu_chained--) {
            ...
            if (msdu_chained == 0) {
                ...
                if (OL_CFG_RAW_RX_LIKELINESS(is_validfragchain)) {
                    qdf_nbuf_set_chfrag_end(next, 1);
                }
            }
        }
                                                  ...
                                                  uint32_t permsdu_flag)
{
...
#else
    struct per_msdu_mesh_data_recv_status *rx_status = NULL;
#endif
    ...
    if (!permsdu_flag || (is_head)) {
        frag = qdf_nbuf_is_chfrag_start(head_msdu);
        first_mpdu =
                 ((*((u_int32_t *) &rx_desc->attention)) &
                  RX_ATTENTION_0_FIRST_MPDU_MASK) >>
                  RX_ATTENTION_0_FIRST_MPDU_LSB;
        if (is_head || first_mpdu) {
#if ATH_DATA_RX_INFO_EN
            if(!(rx_status = qdf_mem_malloc(sizeof(struct per_msdu_data_recv_status)))) {
                return -1;
            }
            qdf_nbuf_set_fctx_type(head_msdu, (void *)rx_status, CB_FTYPE_RX_INFO);
#else
            if (!(rx_status = qdf_mem_malloc(sizeof(struct per_msdu_mesh_data_recv_status)))) {
                return -1;
            }
            qdf_nbuf_set_fctx_type(head_msdu, (void *)rx_status, CB_FTYPE_MESH_RX_INFO);
#endif
            if (is_head) {
        ...
        qdf_nbuf_set_chfrag_start(head_msdu,frag);
    }
    ...
    if (!permsdu_flag || (is_tail)) {
        ...
        if (last_mpdu) {
            if (!rx_status) {
                ...
                qdf_nbuf_set_fctx_type(tail_msdu, (void *)rx_status, CB_FTYPE_RX_INFO);
#else
                if (!(rx_status = qdf_mem_malloc(sizeof(struct per_msdu_mesh_data_recv_status)))) {
                    return -1;
                }
                qdf_nbuf_set_fctx_type(tail_msdu, (void *)rx_status, CB_FTYPE_MESH_RX_INFO);
#endif
            }
