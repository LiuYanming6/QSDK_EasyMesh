 ...
 */
int32_t smart_log_fw_pktlog_stop(struct ieee80211com *ic,
        smart_log_fw_pktlog_stop_reason_t reason)
{
    ...
    if (!ic->smart_logging) {
        qdf_print("%s: Smart logging not enabled. Ignoring FW packetlog stop "
                  "request.\n", __func__);
        ret = QDF_STATUS_E_INVAL;
        goto done;
    }
    ...
    if (!ic->smart_log_fw_pktlog_enabled) {
        qdf_print("%s: Smart log FW initiated packetlog not enabled. Ignoring "
                  "FW packetlog stop request. Investigate!\n", __func__);
        ret = QDF_STATUS_E_INVAL;
        goto done;
    }

    if (ic->smart_log_fw_pktlog_blocked) {
        qdf_print("%s: Smart log FW initiated packetlog activity is blocked "
                  "due to host initiated packetlog. Ignoring FW packetlog stop "
                  "request.\n",  __func__);
        ret = QDF_STATUS_E_INVAL;
        goto done;
    }
    ...
    if (!ic->smart_log_fw_pktlog_started) {
        qdf_print("%s: FW initiated packet logging not in progress. Ignoring "
                  "FW packetlog stop request.\n", __func__);
        ret = QDF_STATUS_E_INVAL;
        goto done;
    }
        ...
        if (acfg_event == NULL) {
            qdf_print("%s: Error - Could not allocate memory for acfg event.\n",
                    __func__);
    ...
    if (scn->pl_dev->pl_funcs->pktlog_enable(scn, 0) == 0) {
        ...
        switch(reason) {
            ...
            case SMART_LOG_FW_PKTLOG_STOP_DISABLE:
                ...
                break;
            default:
                qdf_print("%s: Unknown smart log FW initiated packetlog stop "
                          "reason %u. Asserting!\n", __func__, reason);
                qdf_assert_always(0);
        }
        ...
        qdf_print("%s: Error - Could not successfully stop FW initiated "
                  "packetlog. Smart logging or rest of system may no longer "
                  "function as expected. Investigate!\n"
                  "Marking FW initiated packetlog as stopped within host. This "
                  "may not be consistent with FW state.\n", __func__);
...
/**
 * @brief Work queue handler to enable smart log FW initiated packetlog
 *
 * @param arg - void* pointer to struct ieee80211com for the physical device
 */
void smart_log_fw_pktlog_enable_work_handler(void *arg)
{
            ...
            case WMI_FATAL_CONDITION_PACKET_LOG_CONFIG:
                ...
                switch(events.event[ev].subtype) {
                    case WMI_FATAL_SUBTYPE_PACKET_LOG_CONFIG_START:
                        qdf_print("SmartLogEvent: PACKET LOG CONFIG START "
                                  "SUBEVENT\n");
                        store_smart_debug_log(smart_log_file,
                                SMART_LOG_TYPE_EVENT, data,
                                sizeof(struct wmi_fatal_condition_event));
#if DBGLOG_WQ_BASED
                        qdf_assert_always(ic->ic_smart_log_fw_pktlog_start
                                != NULL);

                        if (ic->ic_smart_log_fw_pktlog_start(ic,
                        ...
                        /* TODO: Add facility to schedule a separate work queue
                         * if required in the future.
                         */
                        qdf_print("%s: Cannot start/stop packetlog since this "
                                  "event handler is not work queue based.\n");
                        ret = -1;
#endif /* DBGLOG_WQ_BASED */
                        break;

                    case WMI_FATAL_SUBTYPE_PACKET_LOG_CONFIG_STOP:
                        qdf_print("SmartLogEvent: PACKET LOG CONFIG STOP "
                                  "SUBEVENT\n");
                        store_smart_debug_log(smart_log_file,
                                SMART_LOG_TYPE_EVENT, data,
                                sizeof(struct wmi_fatal_condition_event));
#if DBGLOG_WQ_BASED
                        qdf_assert_always(ic->ic_smart_log_fw_pktlog_stop
                                != NULL);

                        if (ic->ic_smart_log_fw_pktlog_stop(ic) !=
                                QDF_STATUS_SUCCESS) {
                            ret = -1;
                        }
#else /* DBGLOG_WQ_BASED */
                    ...
                    default:
                        qdf_print("SmartLogEvent: Unknown/unhandled subtype %u "
...
#else /* REMOVE_PKT_LOG */
                qdf_print("SmartLogEvent: FATAL CONDITION PACKET LOG CONFIG- "
...
smart_log_init(void *sc)
{
    ...
    smart_log_file->dbgfs.dfs = qdf_debugfs_create_blob(smart_log_file->dbgfs.name, S_IRUSR, NULL,
    ...
    qdf_print("%s: Smart logging Enabled buf=%pK (size=%lu)\n", __func__, smart_log_file->start, SMART_LOG_MEM);
...
smart_log_deinit(void *sc)
{
    ...
    qdf_debugfs_remove_file(smart_log_file->dbgfs.dfs);
