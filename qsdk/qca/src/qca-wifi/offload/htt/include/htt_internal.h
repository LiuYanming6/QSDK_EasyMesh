...
#if (QCA_PARTNER_DIRECTLINK || QCA_PARTNER_CBM_DIRECTPATH)
...
#elif USE_LITEPATH
	uint32_t lite_path_hdroom=0;
    if ( !is_skb_mixed(msdu) )
    {
		lite_path_hdroom = LITE_PATH_HEADROOM;
    }
    return
        (void *)
            ((((size_t) (qdf_nbuf_head(msdu) + HTT_RX_DESC_ALIGN_MASK)) &
            ~HTT_RX_DESC_ALIGN_MASK) + HTT_RX_DESC_ALIGN_RESERVED - lite_path_hdroom );
#endif
    return
        (void *)
            ((((size_t) (qdf_nbuf_head(msdu) + HTT_RX_DESC_ALIGN_MASK)) &
 ...
 */
#if USE_LITEPATH
#define HTT_LOG2_MAX_CACHE_LINE_SIZE 3 /* 2^3 = 8 */
#else
