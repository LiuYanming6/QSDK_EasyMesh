...
htt_rx_ring_fill_n(struct htt_pdev_t *pdev, int num)
{
    ...
    while (num > 0) {
        ...
        rx_netbuf = ath_dpath_rxbuf_alloc( dev, HTT_RX_BUF_SIZE, 0, 4 );
#else
         ...
         */
#if USE_LITEPATH
       if ( !is_skb_mixed(rx_netbuf) )
       {
               rx_netbuf->head = (uint8_t *)rx_netbuf->head - LITE_PATH_HEADROOM;
               rx_netbuf->data = rx_netbuf->head;
                is_ppa = 1;
       } else {
       ...
       printk ("%s:%d> warning: !!!!! mapping get failed for rx desc !!!!! \n", __func__, __LINE__ );
#endif
...
htt_rx_netbuf_pop(rx_handle_t rxh)
{
     ...
     */
    if (ath_is_directlink((struct ol_txrx_pdev_t*)(pdev->txrx_pdev))) {
        return htt_rx_netbuf_pop_partner(pdev);
    } else
#endif /* QCA_PARTNER_DIRECTLINK */
{
    int idx;
...
htt_rx_netbuf_peek(rx_handle_t rxh)
{
     ...
     */
    if (ath_is_directlink((struct ol_txrx_pdev_t*)(pdev->txrx_pdev))) {
        return htt_rx_netbuf_peek_partner(pdev);
    } else
#endif /* QCA_PARTNER_DIRECTLINK */
{
    int idx;
...
dump_pkt(qdf_nbuf_t msdu, int len)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Dumping %d bytes of data\n", len);

    for (i = 0; i < len; i++) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "0x%x ", *buf++);
        if (i && (i % 8 == 0))
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "\n");
    }
...
dump_htt_rx_desc(htt_pdev_handle pdev, void *rx_desc)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "++++++++++++++++++++++++++++++++++++++++++++\n");
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Dumping %d words of rx_desc of the msdu \n", len);

    for (i = 0; i < len; i++) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "0x%08x ", *buf);
        buf++;
        if (i && (i % 4 == 0))
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "\n");
    }
 ...
 again:
    ...
    if (!msdu_done || !msdu_end) {	    
            ...
            if (pdev->htt_rx_donebit_assertflag == 0) {
                ...
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "next_rx_desc %pK next_msdu %pK \n", rx_desc, next_msdu);
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Rx Attention of next_msdu 0x%x\n",
                    pdev->ar_rx_ops->get_attn_word(rx_desc));

            if (pdev->ar_rx_ops->attn_msdu_done(rx_desc)) {
                    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "RECOVERED FROM DONE BIT NOT SET ERROR. USING NEXT MSDU\n");
                ...
                msdu = htt_rx_netbuf_pop((rx_handle_t)pdev);
                    ...
                    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "DONE BIT NOT SET FOR CURRENT AND NEXT MSDU\n");
                HTT_ASSERT_ALWAYS(pdev->ar_rx_ops->attn_msdu_done(rx_desc));
                }
            }
        } else {
            if (pdev->htt_rx_donebit_assertflag == 1) {
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "rx_desc %pK msdu %pK \n", rx_desc, msdu);
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Rx Attention of msdu 0x%x\n",
                    pdev->ar_rx_ops->get_attn_word(rx_desc));
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "RECOVERED FROM DONE BIT NOT SET ERROR. USING SAME MSDU\n");
                pdev->htt_rx_donebit_assertflag = 0;
            }
            if ((*npackets) == 0) {
            msdu = htt_rx_netbuf_pop((rx_handle_t)pdev);
...
htt_rx_attach(struct htt_pdev_t *pdev)
{
    ...
    if (!pdev->cfg.is_high_latency) {
        ...
        if(pdev->ar_rx_ops == NULL) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:%d Invalid target",__func__,__LINE__);
            goto fail1;
        }
