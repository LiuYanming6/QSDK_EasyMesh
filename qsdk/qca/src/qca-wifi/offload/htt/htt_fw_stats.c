...
{
    struct ol_txrx_fw_stats_info *stats_buffer = (void *)buf;
    ...
    scn = stats_buffer->scn;

    /* vdev_id will be sent in htt response to specify if vdev specific stats are being sent */
    vdev_id = stats_buffer->vdev_id;

    htt_t2h_stats_print(stats_buffer->stats_info, 0, scn->target_type, vdev_id);
    ...
    wlan_dbg_rx_rate_info_t *rx_phy_info, int concise)
{
   ...
   /* SGI */
    ...
    qdf_print("NF (dBm): %d \n",rx_phy_info->nf_dbm);
    qdf_print("RSSI (dBm): %d \n",rx_phy_info->rssi_dbm);
...
htt_t2h_stats_pdev_tidq_stats_print(struct wlan_dbg_tidq_stats *wal_pdev_tidq_stats, int concise)
{
    ...
    if(wal_pdev_tidq_stats->wlan_dbg_tid_txq_status == 1)
      QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " Could not read TIDQ stats from firmware \n");
    else
    {
     qdf_print(" ============================= \n");
...
{
    qdf_print("\n########  COEX COMMON STATS  ########\n");
    qdf_print(" coex version             :    %u\n", coex_stats->coex_cmn_stats.coex_version);
    qdf_print(" is_coex_enabled          :    %u\n", coex_stats->coex_cmn_stats.is_coex_enabled);
    qdf_print(" num_coex_int_high        :    %u\n", coex_stats->coex_cmn_stats.num_coex_int_high);
    qdf_print(" num_coex_int_low         :    %u\n", coex_stats->coex_cmn_stats.num_coex_int_low);
    qdf_print(" num_coex_tx_aborts      :    %u\n", coex_stats->coex_cmn_stats.num_coex_tx_aborts);
    qdf_print(" num_beacons_missed       :    %u\n", coex_stats->coex_cmn_stats.num_beacons_missed);
    qdf_print(" is_priority_line_stuck   :    %u\n", coex_stats->coex_cmn_stats.is_priority_line_stuck);

    qdf_print("\n########  COEX DUTY CYCLE STATS  ########\n");
    qdf_print(" dc_duration              :    %u\n", coex_stats->coex_dc_stats.dc_duration);
    qdf_print(" configured_wlan_duration :    %u\n", coex_stats->coex_dc_stats.configured_wlan_duration);
    qdf_print(" actual_wlan_duration     :    %u\n", coex_stats->coex_dc_stats.actual_wlan_duration);
}
    ...
    wlan_halphy_dbg_stats_t *hal_stats, int concise)
{
    ...
    qdf_print("\n########  HALPHY TX STATS  ########\n");
    qdf_print(" dpd_hw_queued          :    %u\n", hal_stats->tx_stats.dpd_hw_queued);
    qdf_print(" dpd_hw_reaped          :    %u\n", hal_stats->tx_stats.dpd_hw_reaped);
    qdf_print(" dpd_tx_done            :    %u\n", hal_stats->tx_stats.dpd_tx_done);
    qdf_print(" dpd_tx_abort           :    %u\n", hal_stats->tx_stats.dpd_tx_abort);
    qdf_print(" dpd_tx_fail            :    %u\n", hal_stats->tx_stats.dpd_tx_fail);
    qdf_print(" ibf_hw_queued          :    %u\n", hal_stats->tx_stats.ibf_hw_queued);
    qdf_print(" ibf_hw_reaped          :    %u\n", hal_stats->tx_stats.ibf_hw_reaped);
    qdf_print(" ibf_tx_done            :    %u\n", hal_stats->tx_stats.ibf_tx_done);
    qdf_print(" ibf_tx_abort           :    %u\n", hal_stats->tx_stats.ibf_tx_abort);
    qdf_print(" ibf_tx_fail            :    %u\n", hal_stats->tx_stats.ibf_tx_fail);
    qdf_print(" tx_async_enqueue       :    %u\n", hal_stats->tx_stats.tx_async_enqueue);
    qdf_print(" tx_async_dequeue       :    %u\n", hal_stats->tx_stats.tx_async_dequeue);

    qdf_print("\n########      ANI STATS    ########\n");
    qdf_print(" ofdm_ANI_level         :    0x%x\n",hal_stats->ani_stats.ofdm_ANI_level);
    qdf_print(" cck_ANI_level          :    0x%x\n",hal_stats->ani_stats.cck_ANI_level);
    qdf_print(" th_d0_b                :    0x%x\n",hal_stats->ani_stats.th_d0_b);
    qdf_print(" th_d0_b_tf_est         :    0x%x\n",hal_stats->ani_stats.th_d0_b_tf_est);
    qdf_print(" firstep                :    0x%x\n",hal_stats->ani_stats.firstep);
    qdf_print(" firstep_low            :    0x%x\n",hal_stats->ani_stats.firstep_low);
    qdf_print(" rssi_thr1a             :    0x%x\n",hal_stats->ani_stats.rssi_thr1a);
    qdf_print(" cycpwr_thr1            :    0x%x\n",hal_stats->ani_stats.cycpwr_thr1);
    qdf_print(" relpwr                 :    0x%x\n",hal_stats->ani_stats.relpwr);
    qdf_print(" bk_in_backoff          :    0x%x\n",hal_stats->ani_stats.bk_in_backoff);
    qdf_print(" enable_rfsat_strong    :    0x%x\n",hal_stats->ani_stats.enable_rfsat_strong);
    qdf_print(" enable_srch_start_gain :    0x%x\n",hal_stats->ani_stats.enable_srch_start_gain);
    qdf_print(" use_self_corr_low      :    0x%x\n",hal_stats->ani_stats.use_self_corr_low);
}
