...
extern int ath_is_directlink(struct ol_txrx_pdev_t* pdev);
...
htt_h2t_frag_desc_bank_cfg_msg(struct htt_pdev_t *pdev)
{
    ...
    if (qdf_nbuf_put_tail(msg, sizeof(struct htt_tx_frag_desc_bank_cfg_t)) == NULL) {
        qdf_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_FRAG_DESC_BANK_CFG msg\n", __func__);
        return A_ERROR; /* failure */
    }
    ...
    *msg_word = 0;
    ...
    qdf_print("%s - HTT_H2T_MSG_TYPE_FRAG_DESC_BANK_CFG sent to FW for radio ID = %d\n",__func__,scn->radio_id);
    return A_OK;
}
...
htt_h2t_ver_req_msg(struct htt_pdev_t *pdev)
{
    ...
    if (qdf_nbuf_put_tail(msg, HTT_VER_REQ_BYTES) == NULL) {
        qdf_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_VERSION_REQ msg\n", __func__);
        return A_ERROR; /* failure */
    }
...
htt_h2t_rx_ring_cfg_msg_ll(struct htt_pdev_t *pdev)
{
    ...
    if (qdf_nbuf_put_tail(msg, HTT_RX_RING_CFG_BYTES(1)) == NULL) {
        qdf_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_RX_RING_CFG msg\n", __func__);
        return A_ERROR; /* failure */
    }
...
/* FIX THIS: if the FW creates a complete translated rx descriptor, then the MAC DMA of the HW rx descriptor should be disabled. */
   ...
   if(pdev->txrx_pdev && pdev->txrx_pdev->pl_dev) {
        enable_ctrl_data = 1;
        enable_mgmt_data = 1;
        enable_null_data = 1;
        enable_phy_data  = 1;
    ...
    u_int32_t vdev_id)
{
    ...
    if (qdf_nbuf_put_tail(msg, HTT_H2T_STATS_REQ_MSG_EXTN1_SZ) == NULL) {
        qdf_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_STATS_REQ msg\n", __func__);
        return -1;
    }
...
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
    if (pdev->nss_wifiol_ctx) {
        status = osif_nss_ol_stats_frame_send(txrx_pdev, pdev->nss_wifiol_ctx, pdev->nss_wifiol_id, msg);
    } else
#endif
    {
...
htt_h2t_sync_msg(struct htt_pdev_t *pdev, u_int8_t sync_cnt)
{
...
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
    if (pdev->nss_wifiol_ctx) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "NSS WiFi Offload :sending sync message not supported \n");
        return A_NO_MEMORY;
    }
#endif
    ...
    if (qdf_nbuf_put_tail(msg, HTT_H2T_SYNC_MSG_SZ) == NULL) {
        qdf_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_SYNC msg\n", __func__);
        htt_htc_pkt_free(pdev, pkt);
        return A_NO_MEMORY;
    }
...
void
fw_hang_failure(void *instance)
{
    qdf_nbuf_t msg;
    struct ol_ath_softc_net80211 *scn = (struct ol_ath_softc_net80211 *)instance;
    struct htt_pdev_t *pdev = (struct htt_pdev_t *)scn->pdev_txrx_handle->htt_pdev;
    u_int32_t *msg_word;
    int status;
    u_int32_t cookie ;
    HTC_FRAME_HDR       *pHtcHdr;
    ...
    msg = qdf_nbuf_alloc(
        ...
        HTC_HEADER_LEN + HTC_HDR_ALIGNMENT_PADDING, 4, FALSE);
    if(!msg) {
        qdf_print("%s: qdf_nbuf_alloc Failed\n", __func__);
        return;
    }

    /* set the length of the message */
    if (qdf_nbuf_put_tail(msg, 1) == NULL) {
        qdf_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_ASSERT msg\n", __func__);
        return;
    }
    ...
    *msg_word = 0;
    HTT_H2T_MSG_TYPE_SET(*msg_word, HTT_H2T_MSG_TYPE_ASSERT);
                     ...
                     int max_subfrms_amsdu)
{
    ...
    if (qdf_nbuf_put_tail(msg, HTT_AGGR_CFG_MSG_SZ) == NULL) {
        qdf_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_AGGR_CFG msg\n", __func__);
        return -1;
    }
        ...
        ol_txrx_vdev_handle  vdev)
{
    ...
    A_UINT8 tid, rate, retry, power, subtype_index;
   ...
   /* CAUTION:
    * Read umac cb before allocating mgmt tx descriptor.
    * After mgmt tx desc allocation, nbuf cb will be pointing to offload cb.
    */
...
#define MAX_MGMT_BUF_PKTS 512 /*TODO: This needs fine tuning */
    ...
    if (!msg) {
        qdf_spin_lock_bh(&hpdev->mgmtbufLock);
        if(qdf_nbuf_queue_len(&hpdev->mgmtbufQ) > MAX_MGMT_BUF_PKTS)
        {
            qdf_spin_unlock_bh(&hpdev->mgmtbufLock);
            return -1; /* Do not Queue more than allowed to avoid Host Memory Hogging */
        }
        qdf_nbuf_queue_add(&hpdev->mgmtbufQ, mgmt_frm);
        qdf_spin_unlock_bh(&hpdev->mgmtbufLock);
        return 0;
    }

    /* Map mgmt buffer now */
    qdf_nbuf_map_single(pdev->osdev, mgmt_frm, QDF_DMA_TO_DEVICE);
    ...
    if (subtype == IEEE80211_FC0_SUBTYPE_ACTION) {
        ...
        if ((ia->ia_category == IEEE80211_ACTION_CAT_PUBLIC) ||
            ...
            (ia->ia_category == IEEE80211_ACTION_CAT_WNM)) {
             paddr =  QDF_NBUF_CB_PADDR(mgmt_frm);
        } else {
             paddr = qdf_nbuf_get_frag_paddr(mgmt_frm, 0);
        }
    }
    else {
...
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
    if (pdev->nss_wifiol_ctx) {
        status = osif_nss_ol_mgmt_frame_send(pdev, pdev->nss_wifiol_ctx, pdev->nss_wifiol_id, desc_id, msg);
    } else {
    ...
    HTT_TX_PDEV_UNLOCK(&pdev->tx_lock);
     ...
     if(status) {
         htt_tx_mgmt_desc_free(hpdev, desc_id, IEEE80211_TX_ERROR, MGMT_PPDU_ID_NON_USED);
     }
                ...
                A_UINT32             len)
{
    ...
    if (qdf_nbuf_put_tail(msg, (sizeof(struct htt_tx_fetch_resp_t) - 8)) == NULL) {
        qdf_print("%s: Failed to expand head for HTT_H2T_MSG_TYPE_TX_FETCH_RESP msg\n", __func__);
        return NULL;
    }
