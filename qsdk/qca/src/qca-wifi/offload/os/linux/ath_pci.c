...
void ol_ath_pci_event(struct qcom_pcie_notify *notify)
{
    ...
    scn = (struct ol_ath_softc_net80211*) notify->data;
    ...
    case QCOM_PCIE_EVENT_LINKDOWN:
        ...
        OSIF_RADIO_DELIVER_EVENT_WATCHDOG(&(scn->sc_ic), ACFG_WDT_PCIE_ASSERT);
#endif
        break;
...
ol_ath_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    aps_osdev->netdev = qdf_dev->netdev;
    ...
    scn->pcie_notify.data = (void*)scn;
    scn->pcie_event.events = QCOM_PCIE_EVENT_LINKDOWN | QCOM_PCIE_EVENT_LINKUP;
    scn->pcie_event.user = pdev;
    scn->pcie_event.callback = ol_ath_pci_event;
    scn->pcie_event.notify = scn->pcie_notify;
    qcom_pcie_register_event(&scn->pcie_event);
#endif
    /* Stop the target after the probe. This will put the target in low power state. The target will be brought up automatically, while creating the first VAP
     * In testmode of qca_ol. This Stop target will not be called.
     */
    if (testmode == 0) {
        ol_ath_target_stop(&scn->sc_ic, 0);
    }
    return 0;
err_attach:
    if (flag) {
...
static int qca_napi_destroy (void *hif_ctx, int force)
{
    int rc = 0;
    int i;
    uint32_t hdd_napi_map = hif_napi_get_cemap(hif_ctx);
...
/**
 * is_target_ready() - function to check whether target is ready or not
 * @context: context from hif
 *
 * Return: true/false
 */
bool is_target_ready(void *context)
{
   struct _NIC_DEV *aps_osdev = (struct _NIC_DEV *)context;
   struct ol_ath_softc_net80211 *scn = ath_netdev_priv(aps_osdev->netdev);
        ...
        enum qdf_bus_type bus_type, bool reinit, qdf_device_t qdf_dev)
{
    ...
    struct hif_driver_state_callbacks cbk;
        ...
        cbk.is_target_ready = is_target_ready;
        cbk.context = qdf_dev->drv;
...
ol_ath_pci_remove(struct pci_dev *pdev)
{
    ...
    if (!(scn->down_complete)) {
        ...
        /* Suspend Target */
        qdf_print(KERN_INFO"Suspending Target - with disable_intr set :%s (sc %pK)\n",dev->name,sc);
        if (!ol_ath_suspend_target(scn, 1)) {
            u_int32_t  timeleft;
            qdf_print(KERN_INFO"waiting for target paused event from target :%s (sc %pK)\n",dev->name,sc);
            ...
            if(!timeleft || signal_pending(current)) {
                qdf_print("ERROR: Failed to receive target paused event :%s (sc %pK)\n",dev->name,sc);
                target_paused = FALSE;
            }
            /*
