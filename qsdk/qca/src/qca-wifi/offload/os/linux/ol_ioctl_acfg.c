                                  ...
                                  uint8_t *buffer, int32_t Length)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d] gpio_num %d input %d pull_type %d, intr_mode %d\n", __func__,__LINE__, ptr->gpio_num, ptr->input, ptr->pull_type, ptr->intr_mode);
                                  ...
                                  uint8_t *buffer, int32_t Length)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d] gpio_num %d set %d\n", __func__,__LINE__, ptr->gpio_num, ptr->set);
    if(ol_ath_ucfg_gpio_output(scn, ptr->gpio_num, ptr->set)) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:Unable to set GPIO\n", __func__);
        return -1;
    }
    return 0;
}
...
ol_acfg_handle_wifi_ioctl(struct net_device *dev, void *data, int *isvap_ioctl)
{
    ...
    if(!req){
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: mem alloc failed\n",__FUNCTION__);
        return ENOMEM;
    }
    if(copy_from_user(req, data, req_len)){
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: copy_from_user failed\n",__FUNCTION__);
        goto done;
    }
    ...
    if(req->cmd > ACFG_REQ_FIRST_WIFI && req->cmd < ACFG_REQ_LAST_WIFI)
    {
        *isvap_ioctl = 0;
        ...
        if ((cmd_found == false) || (ol_acfgdispatchentry[i].cmd_handler == NULL)) {
            status = -1;
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "OL ACFG ioctl CMD %d failed\n", req->cmd);
            goto done;
        }
        status = ol_acfgdispatchentry[i].cmd_handler(dev,
                        req->cmd, (uint8_t *)req, req_len);
        if(copy_to_user(data, req, req_len)) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "OL copy_to_user error\n");
            status = -1;
    	}
    }
    else
    {
        *isvap_ioctl = 1;
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: req->cmd=%d not valid for radio interface, it's for VAP\n",
                  __func__,req->cmd);
    }
done:
    kfree(req);
