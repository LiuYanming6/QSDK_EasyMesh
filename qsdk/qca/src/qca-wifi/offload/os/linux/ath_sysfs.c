...
#endif
    ret = qdf_create_dev_file(&scn->sc_osdev->device->kobj, diag_fsattr);
    if (ret) {
...
ath_sysfs_diag_fini(struct ol_ath_softc_net80211 *scn)
{
    ...
    if (diag_fsattr) {
        qdf_print("%s: diag_fsattr \n", __func__);
        qdf_vfs_delete_binfile(&scn->sc_osdev->device->kobj, diag_fsattr);
        scn->diag_ol_priv = NULL;
        OS_FREE(diag_fsattr);
    }
}
...
ath_sysfs_CE_init(struct ol_ath_softc *sc)
{
    ...
    ret = qdf_create_dev_file(&sc->dev->kobj, &CE_fsattr);
    BUG_ON(ret);
...
{
    qdf_vfs_delete_binfile(&sc->dev->kobj, &CE_fsattr);
