...
struct ol_txrx_nbuf_classify {
    uint16_t peer_id;
    uint8_t tid;
    uint8_t pkt_tid; /* Unmodified Packet TID */
    uint8_t pkt_tos; /* Unmodified Packet Tos */
    uint8_t pkt_dscp; /* Unmodified Packet dscp */
    uint8_t tos;
    uint8_t dscp;
    uint8_t is_mcast;
    uint8_t is_eap;
...
/**
 * ol_txrx_classify_fp - Txrx packet classifier function
 * @data_vdev - handle to the virtual device object
 * @ol_txrx_pkt_direction- classification required in tx or rx path
 * @nbuf_class - classification object
 * @msdu_list - list of network buffers
 */
typedef void (*ol_txrx_classify_fp)(ol_txrx_vdev_handle data_vdev,
					qdf_nbuf_t msdu_list,
					enum ol_txrx_pkt_direction,
...
struct ol_txrx_ops {
	...
	/* tx/rx Classify function */
	ol_txrx_classify_fp txrxclassify;
