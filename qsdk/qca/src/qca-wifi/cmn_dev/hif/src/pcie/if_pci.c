...
#ifdef CONFIG_WIN
extern int32_t frac, intval, ar900b_20_targ_clk, qca9888_20_targ_clk, ipq4019_20_targ_clk;
#endif
...
static void hif_msi_fw_handler(int irq, void *arg)
{
        struct hif_pci_softc *sc = (struct hif_pci_softc *)arg;
        struct hif_softc *scn = HIF_GET_SOFTC(sc);
        struct HIF_CE_state *hif_state = HIF_GET_CE_STATE(scn);
        ...
        (irqreturn_t) hif_fw_interrupt_handler(irq, scn);

        for (id = sc->num_msi_intrs; id < scn->ce_count; id++) {
                ce_dispatch_interrupt(id,  &hif_state->tasklets[id]);
        }
}

static irqreturn_t hif_pci_msi_handler(int irq, void *arg)
{
        struct hif_pci_softc *sc = (struct hif_pci_softc *)arg;
        struct hif_softc *scn = HIF_GET_SOFTC(sc);
        struct HIF_CE_state *hif_state = HIF_GET_CE_STATE(scn);
        ...
        if(MSI_ASSIGN_FW == (irq - sc->pdev->irq) ){
                hif_msi_fw_handler(irq, arg);
        }

        if(((irq - sc->pdev->irq ) > MSI_ASSIGN_FW) && ((irq - sc->pdev->irq) <= sc->num_msi_intrs))
        {
                id = irq -((sc->pdev->irq)+ MSI_ASSIGN_CE_INITIAL);
                ce_dispatch_interrupt(id,  &hif_state->tasklets[id]);
...
{
	qdf_debugfs_remove_file(sc->pm_dentry);
}
#else
...
void hif_set_hia_extnd(struct hif_softc *scn)
{
	...
	if (target_type == TARGET_TYPE_AR900B
		...
		if ((target_type == TARGET_TYPE_AR900B)
			...
			if (200000000 != ipq4019_20_targ_clk) {
			/* setting target clock speed to 250 Mhz */
			ipq4019_20_targ_clk = 250000000;
			}
...
int hif_pci_bus_configure(struct hif_softc *hif_sc)
{
	...
	/*
         * Configure the IRQ first if msienable=1, Legacy interrupt pins cannot be
	 * used with MSI, hence MSI shall need to enabled first
	 */
	if (!msienable) {
...
int hif_configure_msi(struct hif_pci_softc *sc)
{
	...
	/* for kernel version > 3.16,pci_enable_msi_range()returns
	 * values > 0 (number of msi it actually allocated).
	 * for kernel version < 3.16, pci_enable_msi_block() returns
	 * returns 0 if requested number of msi is allocated totally,
	 * else returns number of msi it actually allocated.
	 * Both functions return negative, if it couldn't allocate
	 * atleast one msi.
	 */
	 /*Multiple msi*/
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0))
	if (rv > 1) {
#else
	if ((rv > 1) || ((num_msi_desired > 1) && (rv == 0))) {
#endif
		int i;
		...
		for (i = MSI_ASSIGN_CE_INITIAL; i < num_msi_desired; i++) {
			ret = request_irq((sc->pdev->irq + i),
					  hif_pci_msi_handler, IRQF_SHARED,
					  "wlan_pci", sc);
			if (ret) {
				HIF_ERROR("%s: request_irq failed", __func__);
				goto err_intr;
...
#else
	} else if ((rv == 1) || ((num_msi_desired == 1) && (rv == 0))) {
#endif
...
static int hif_pci_configure_legacy_irq(struct hif_pci_softc *sc)
{
	...
	struct net_device *dev = scn->qdf_dev->netdev;
...
void hif_target_sync(struct hif_softc *scn)
{
	...
	if (LEGACY_INTERRUPTS(sc))
	{
		hif_write32_mb(scn->mem+(SOC_CORE_BASE_ADDRESS |
                ...
                (void)hif_read32_mb(scn->mem + (SOC_CORE_BASE_ADDRESS | PCIE_INTR_ENABLE_ADDRESS));
...
/**
 * hif_check_bus_link_state(): check bus
 *
 * This function checks the bus link status
 *
 * @scn: soft_sc struct
 * Return: IO Error/Success
 */
QDF_STATUS hif_pci_check_bus_link_state(struct hif_softc *scn)
{
 ...
 */
int hif_addr_in_boundary(hif_handle_t *hif_device, A_UINT32 offset)
{
    struct HIF_CE_state *hif_state;
    struct hif_pci_softc *sc;

    hif_state = (struct HIF_CE_state *)hif_device;
    sc =  HIF_GET_PCI_SOFTC(hif_state);
    if (unlikely(offset > sc->mem_len)) {
        qdf_print("Avoid reading mmio out of bounds at 0x%08x (max 0x%08zx)\n",
                   offset, sc->mem_len);
        return -EINVAL;
    }
    return 0;
