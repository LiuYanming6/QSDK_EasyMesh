...
#define WMI_HOST_VDEV_IS_BEACON_SUPPORTED(param) ((param) & WMI_HOST_VDEV_BEACON_SUPPORT)
#define WMI_HOST_VDEV_IS_WDS_LRN_ENABLED(param) ((param) & WMI_HOST_VDEV_WDS_LRN_ENABLED)
#define WMI_HOST_VDEV_IS_VOW_ENABLED(param) ((param) & WMI_HOST_VDEV_VOW_ENABLED)
...
typedef enum {
     ...
     */
    WMI_HOST_DFS_STATUS_CHECK_PASSED = 0,
...
struct vdev_scan_nac_rssi_params{
    A_UINT32 vdev_id;
    u_int8_t bssid_addr[IEEE80211_ADDR_LEN];
...
#define WMI_HOST_FW_FEATURE_VOW_FEATURES               0x4000
#define WMI_HOST_FW_FEATURE_VOW_STATS                  0x8000
	/**
 ...
 * @vdev_id: vdev id
 * @fd_period:  '0' - Disabled. non-zero - Period of FD frames.
 */
...
struct btcoex_cfg_params {
     ...
     * The host sets the appropriate bit(s) in btcoex_param_flags to indicate which
     * configuration parameters are valid within a particular BT coex config message,
...
typedef enum {
	...
	wmi_pdev_param_rx_mitigation,
	wmi_pdev_param_dyn_auto_burst,
...
typedef enum {
	...
	wmi_service_host_htt_assert_trigger_support,
...
typedef struct _wmi_host_dcs_im_tgt_stats {
	...
	uint32_t		     my_bss_rx_cycle_count;
        /* these fields are added here for backward compatibility instead of wmi_host_dcs_mib_stats_t */
 ...
 * @my_bss_rx_cycle_count: own bss rx cycle count
 * @tx_frame_count: tx frame count
 ...
 *      Intentionally started numbering from 1,
...
#define WMI_HOST_BOARD_MCN_STRING_BUF_SIZE (WMI_HOST_BOARD_MCN_STRING_MAX_SIZE+1) /* null-terminator */
...
/**
 * typedef enum WMI_HOST_SMART_LOG_SCENARIO - Smart log scenarios to be
 * enabled/disabled
...
/**
 * typedef enum WMI_HOST_FATAL_CONDITION_TYPE - Values that 'type' can take in
...
} WMI_HOST_FATAL_CONDITION_TYPE;

/**
 * typedef enum WMI_HOST_FATAL_CONDITION_SUBTYPE_TIMEOUTS - Possible subtypes
 * for WMI_HOST_FATAL_CONDITION_TIMEOUTS
...
} WMI_HOST_FATAL_CONDITION_SUBTYPE_TIMEOUTS;
...
} WMI_HOST_FATAL_CONDITION_SUBTYPE_CONNECTION_ISSUE;

/**
 * typedef enum WMI_HOST_FATAL_CONDITION_SUBTYPE_PACKET_LOG_CONFIG - Possible
 * subtypes for WMI_HOST_FATAL_CONDITION_PACKET_LOG_CONFIG
...
} WMI_HOST_FATAL_CONDITION_SUBTYPE_PACKET_LOG_CONFIG;
