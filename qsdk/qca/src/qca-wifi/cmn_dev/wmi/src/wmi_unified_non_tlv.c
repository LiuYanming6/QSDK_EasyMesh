					...
					struct vdev_install_key_params *param)
{
	...
	static const u_int8_t wmi_ciphermap[] = {
...
#else
		WMI_CIPHER_UNUSED,	/* IEEE80211_CIPHER_WAPI */
#endif
		...
		WMI_CIPHER_UNUSED,	/* IEEE80211_CIPHER_FILS_AEAD */
#endif
	...
	/* for big endian host, copy engine byte_swap is enabled
         ...
         */
	WMI_HOST_IF_MSG_COPY_CHAR_ARRAY(cmd->key_data, param->key_data, cmd->key_len);
 ...
 * Return: 0 for success or error code
 */
QDF_STATUS send_peer_cfr_capture_cmd_non_tlv(wmi_unified_t wmi_handle,
					...
					struct set_bridge_mac_addr_params *param)
{
	...
	memset( null_macaddr, 0, IEEE80211_ADDR_LEN );
	cmd = (wmi_peer_add_wds_entry_cmd *)wmi_buf_data(buf);
	...
	qdf_print("%s: set bridge_addr %s \n", __func__, ether_sprintf(param->bridge_addr));
	return wmi_unified_cmd_send(wmi_handle, buf, len,
			WMI_PEER_ADD_WDS_ENTRY_CMDID);
}
...
QDF_STATUS send_radar_found_indication_cmd_non_tlv(wmi_unified_t wmi_handle,
				struct dfs_radar_found_params *param)
{
	 ...
	 * out.
	 */
	cmd->pri_min_value = param->pri_min;
	cmd->pri_max_value = param->pri_max;
	cmd->duration_min_value = param->duration_min;
	cmd->duration_max_value = param->duration_max;
	cmd->sidx_min_value = param->sidx_min;
	cmd->sidx_max_value = param->sidx_max;

	if (wmi_unified_cmd_send(wmi_handle, buf, len,
				WMI_HOST_DFS_RADAR_FOUND_CMDID)) {
		qdf_print("%s:Failed to send WMI command \n",__func__);
		return QDF_STATUS_E_FAILURE;
	}
 ...
 *
 *  @param wmi_handle	: handle to WMI.
 *  @param param		: pointer to hold FD send cmd parameter
 *  @return QDF_STATUS_SUCCESS  on success and -ve on failure.
 */
QDF_STATUS send_fils_discovery_send_cmd_non_tlv(wmi_unified_t wmi_handle,
				struct fd_params *param)
{
	...
	wmi_unified_cmd_send(wmi_handle, wmi_buf, len, WMI_PDEV_SEND_FD_CMDID);
...
#define FREQUENCY_THRESH 1000
	...
	if (param->scan_offset_time) {
		...
		scan_start_offset->num_offset = sizeof(param->scan_offset_time)/sizeof(A_UINT32);
		WMI_HOST_IF_MSG_COPY_CHAR_ARRAY(&(scan_start_offset->start_tsf_offset),
				&(param->scan_offset_time),sizeof(param->scan_offset_time));
		tmp_ptr += (2 + sizeof(param->scan_offset_time)/sizeof(A_UINT32));
	}
	if (param->ie_len) {
...
/**
 * send_vdev_fils_enable_cmd_non_tlv- enable/Disable FD Frame
 * command to fw
 * @wmi_handle: wmi handle
 ...
 * Return: 0 for success or error code
 */
QDF_STATUS send_vdev_fils_enable_cmd_non_tlv(wmi_unified_t wmi_handle,
				struct config_fils_params *param)
{
	...
	qdf_print("Setting FD period to %d vdev id : %d \n",
		      param->fd_period, param->vdev_id);

	return wmi_unified_cmd_send(wmi_handle, buf, len, WMI_ENABLE_FILS_CMDID);
}
...
/**
 * send_set_atf_grp_ac_cmd_non_tlv() - send set atf grouping command to fw
 * @wmi_handle: wmi handle
 * @param: pointer to set atf grouping param
 *
 ...
 */
QDF_STATUS
send_set_atf_grp_ac_cmd_non_tlv(wmi_unified_t wmi_handle,
				struct atf_grp_ac_params *param)
{
	...
	int i, retval = QDF_STATUS_SUCCESS;
...
#define WHAL_NUM_BAND_EDGES_2G_40MHZ    6
	...
	if (param->target_type == TARGET_TYPE_AR900B || param->target_type == TARGET_TYPE_QCA9984 ||
		...
		if (param->is_2g) {
			...
			if (param->ctl_cmd_len !=
				...
				1 + (WHAL_NUM_CTLS_2G_20MHZ * WHAL_NUM_BAND_EDGES_2G_20MHZ) +
				(WHAL_NUM_CTLS_2G_40MHZ * 2) + (WHAL_NUM_BAND_EDGES_2G_40MHZ * 3) +
				(WHAL_NUM_CTLS_2G_40MHZ * WHAL_NUM_BAND_EDGES_2G_40MHZ) + 4 )) {
			...
			if (param->ctl_cmd_len !=
				...
				1 + (WHAL_NUM_CTLS_5G_HT20 * WHAL_NUM_BAND_EDGES_5G_HT20) +
				(WHAL_NUM_CTLS_5G_HT40 * 2) + (WHAL_NUM_BAND_EDGES_5G_HT40 * 3) +
				(WHAL_NUM_CTLS_5G_HT40 * WHAL_NUM_BAND_EDGES_5G_HT40) +
				(WHAL_NUM_CTLS_5G_HT80 * 2) + (WHAL_NUM_BAND_EDGES_5G_HT80 * 3) +
				(WHAL_NUM_CTLS_5G_HT80 * WHAL_NUM_BAND_EDGES_5G_HT80) +
				(WHAL_NUM_CTLS_5G_HT160 * 2) + (WHAL_NUM_BAND_EDGES_5G_HT160 * 3) +
				(WHAL_NUM_CTLS_5G_HT160 * WHAL_NUM_BAND_EDGES_5G_HT160))) {
				qdf_print("CTL array len not correct\n");
				return QDF_STATUS_E_FAILURE;
			}
		}
...
send_get_user_position_cmd_non_tlv(wmi_unified_t wmi_handle, uint32_t value)
{
	...
	qdf_mem_copy(&cmd->aid, &value, sizeof(value));

	if (wmi_unified_cmd_send(wmi_handle, buf,
		sizeof(wmi_peer_gid_userpos_list_cmd), WMI_PEER_GID_USERPOS_LIST_CMDID)) {
		return QDF_STATUS_E_FAILURE;
	}
 ...
 */
QDF_STATUS
send_reset_peer_mumimo_tx_count_cmd_non_tlv(wmi_unified_t wmi_handle, uint32_t value)
{
	wmi_buf_t buf;
	...
	if (wmi_unified_cmd_send(wmi_handle, buf,
		sizeof(wmi_peer_txmu_rstcnt_cmd), WMI_PEER_TX_MU_TXMIT_RSTCNT_CMDID)) {
		return QDF_STATUS_E_FAILURE;
	}
 ...
 */
QDF_STATUS
send_get_peer_mumimo_tx_count_cmd_non_tlv(wmi_unified_t wmi_handle, uint32_t value)
{
	wmi_buf_t buf;
	...
	if (wmi_unified_cmd_send(wmi_handle, buf,
		sizeof(wmi_peer_txmu_cnt_cmd), WMI_PEER_TX_MU_TXMIT_COUNT_CMDID)) {
		return QDF_STATUS_E_FAILURE;
	}
 ...
 */
QDF_STATUS
send_pdev_caldata_version_check_cmd_non_tlv(wmi_unified_t wmi_handle, uint32_t param)
{
	...
	if (wmi_unified_cmd_send(wmi_handle, buf, len, WMI_PDEV_CHECK_CAL_VERSION_CMDID))
		return QDF_STATUS_E_FAILURE;
 ...
 * @wmi_handle: wmi handle
 * @param evt_buf: pointer to event buffer
 * @param param: Pointer to hold esp event
 *
 * Return: 0 for success or error code
 */
QDF_STATUS extract_esp_estimation_ev_param_non_tlv(wmi_unified_t wmi_handle,
			void *evt_buf, wmi_host_esp_estimation_event *param)
{
...
/**
 * extract_peer_gid_userpos_list_ev_param_non_tlv() - extract gid user position from event
 * @wmi_handle: wmi handle
 * @param evt_buf: pointer to event buffer
 ...
 * Return: 0 for success or error code
 */
QDF_STATUS extract_peer_gid_userpos_list_ev_param_non_tlv(wmi_unified_t wmi_handle,
			void *evt_buf, wmi_host_peer_gid_userpos_list_event *param)
{
	wmi_peer_gid_userpos_list_event *event = (wmi_peer_gid_userpos_list_event *)evt_buf;
 ...
 * Return: 0 for success or error code
 */
QDF_STATUS extract_pdev_caldata_version_check_ev_param_non_tlv(wmi_unified_t wmi_handle,
			void *evt_buf, wmi_host_pdev_check_cal_version_event *param)
{
	wmi_pdev_check_cal_version_event *event = (wmi_pdev_check_cal_version_event *)evt_buf;
 ...
 * @wmi_handle		: wmi handle
 * @param evt_buf	: pointer to event buffer
 * @param vdev_id	: Pointer to hold vdev id
 *
 * Return: 0 for success or error code
 */
static QDF_STATUS extract_swfda_vdev_id_non_tlv(wmi_unified_t wmi_handle,
 ...
 * @param evt_buf: pointer to event buffer
 * @param index: Index into extended vdev stats
 * @param wmi_host_vdev_nac_rssi_event: Pointer to hold vdev NAC_RSSI stats
 *
 * Return: 0 for success or error code
 */
static QDF_STATUS extract_vdev_nac_rssi_stats_non_tlv(wmi_unified_t wmi_handle,
	void *evt_buf, wmi_host_vdev_nac_rssi_event *vdev_nac_rssi_stats)
{
	uint8_t *pdata = ((wmi_stats_event *)evt_buf)->data;
	...
	if (WMI_REQUEST_NAC_RSSI_STAT &
		...
		wmi_host_vdev_nac_rssi_event *ev = (wmi_host_vdev_nac_rssi_event *)
		((pdata) +
		((((wmi_stats_event *)evt_buf)->num_pdev_stats) *
					 sizeof(wmi_pdev_stats)) +
		((((wmi_stats_event *)evt_buf)->num_pdev_ext_stats) *
		...
		OS_MEMCPY(vdev_nac_rssi_stats, ev,
				sizeof(wmi_host_vdev_nac_rssi_event));
	}
	return QDF_STATUS_SUCCESS;
}
#endif
...
static QDF_STATUS extract_dfs_status_check_event_non_tlv(wmi_unified_t
        wmi_handle, void *evt_buf, A_UINT32 *fw_dfs_status_code)
{
    ...
    if ((ev->status == WMI_HOST_DFS_CHECK_PASSED) ||
        ...
        *fw_dfs_status_code = ev->status;
        ...
        qdf_print("%s..Invalid status code : %d received\n",__func__,ev->status);
        return QDF_STATUS_E_FAILURE;
    }
...
struct wmi_ops non_tlv_ops =  {
	...
	.send_set_atf_grp_ac_cmd = send_set_atf_grp_ac_cmd_non_tlv,
	...
	.send_radar_found_indication_cmd = send_radar_found_indication_cmd_non_tlv,
	.extract_dfs_status_from_fw = extract_dfs_status_check_event_non_tlv,
};
...
static void populate_non_tlv_service(uint32_t *wmi_service)
{
	...
	wmi_service[wmi_service_host_htt_assert_trigger_support] =
...
static void populate_non_tlv_events_id(uint32_t *event_ids)
{
	...
	event_ids[wmi_host_swfda_event_id] =
...
static void populate_pdev_param_non_tlv(uint32_t *pdev_param)
{
	...
	pdev_param[wmi_pdev_param_antenna_gain_half_db] = WMI_PDEV_PARAM_ANTENNA_GAIN_HALF_DB;
	pdev_param[wmi_pdev_param_per_peer_prd_cfr_enable] =
		WMI_PDEV_PARAM_PER_PEER_PERIODIC_CFR_ENABLE;
	pdev_param[wmi_pdev_param_tx_ack_timeout] =
		WMI_PDEV_PARAM_TX_ACK_TIMEOUT;
	pdev_param[wmi_pdev_param_rx_mitigation] =
		WMI_PDEV_PARAM_RX_INDICATION_MITIGATION;
	pdev_param[wmi_pdev_param_dyn_auto_burst] =
