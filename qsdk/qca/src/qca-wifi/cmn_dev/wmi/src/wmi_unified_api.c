...
/**
 * wmi_unified_peer_cfr_capture_send() - send cfr params to fw
 * @wmi: wmi handle
 * @param: pointer to hold per cfr config parameter
 *
 * Return: QDF_STATUS_SUCCESS on success and QDF_STATUS_E_FAILURE for failure
 */
QDF_STATUS wmi_unified_peer_cfr_capture_send(void *wmi_hdl,
...
QDF_STATUS wmi_unified_radar_found_indication_to_fw_cmd(void *wmi_hdl,
				struct dfs_radar_found_params *param)
{
	wmi_unified_t wmi_handle = (wmi_unified_t) wmi_hdl;

	if (wmi_handle->ops->send_radar_found_indication_cmd)
		return wmi_handle->ops->send_radar_found_indication_cmd(wmi_handle,
				  param);
...
/**
 *  wmi_unified_fils_discovery_send_cmd() - WMI FILS discovery send function
 *  @param wmi_handle	: handle to WMI.
 *  @param macaddr		: MAC address
				...
				struct fd_params *param)
{
	...
	if (wmi_handle->ops->send_fils_discovery_send_cmd)
		return wmi_handle->ops->send_fils_discovery_send_cmd(wmi_handle, param);
...
/**
 *  wmi_unified_vdev_config_fils_cmd_send(() - WMI config fils enable function
 *  @param wmi_handle      : handle to WMI.
 *  @param param    : pointer to hold config fils param
 *
 *  @return QDF_STATUS_SUCCESS on success and QDF_STATUS_E_FAILURE for failure
 */
QDF_STATUS wmi_unified_vdev_config_fils_cmd_send(void *wmi_hdl,
...
/**
 * wmi_send_set_atf_grp_ac_cmd() - send group AC config request command to fw
 * @param: pointer to set atf group AC config request param
 *
 * Return: QDF_STATUS_SUCCESS on success and QDF_STATUS_E_FAILURE for failure
 */
QDF_STATUS
wmi_send_set_atf_grp_ac_cmd(void *wmi_hdl,
		struct atf_grp_ac_params *param)
{
	wmi_unified_t wmi_handle = (wmi_unified_t) wmi_hdl;

	if (wmi_handle->ops->send_set_atf_grp_ac_cmd)
		return wmi_handle->ops->send_set_atf_grp_ac_cmd(wmi_handle,
					param);
...
wmi_send_get_user_position_cmd(void *wmi_hdl, uint32_t value)
{
	...
	if (wmi_handle->ops->send_get_user_position_cmd)
		return wmi_handle->ops->send_get_user_position_cmd(wmi_handle, value);

	return QDF_STATUS_E_FAILURE;
}

/**
 * wmi_send_get_peer_mumimo_tx_count_cmd() - send get mumio tx count command to fw
 * @wmi_handle: wmi handle
...
wmi_send_get_peer_mumimo_tx_count_cmd(void *wmi_hdl, uint32_t value)
{
	...
	if (wmi_handle->ops->send_get_peer_mumimo_tx_count_cmd)
		return wmi_handle->ops->send_get_peer_mumimo_tx_count_cmd(wmi_handle, value);

	return QDF_STATUS_E_FAILURE;
}

/**
 * wmi_send_reset_peer_mumimo_tx_count_cmd() - send reset peer mumimo tx count to fw
 * @wmi_handle: wmi handle
...
wmi_send_reset_peer_mumimo_tx_count_cmd(void *wmi_hdl, uint32_t value)
{
	...
	if (wmi_handle->ops->send_reset_peer_mumimo_tx_count_cmd)
		return wmi_handle->ops->send_reset_peer_mumimo_tx_count_cmd(wmi_handle, value);

	return QDF_STATUS_E_FAILURE;
}

/**
 * wmi_send_pdev_caldata_version_check_cmd() - send reset peer mumimo tx count to fw
 * @wmi_handle: wmi handle
 * @value:	value
 *
 * Return: QDF_STATUS_SUCCESS on success and QDF_STATUS_E_FAILURE for failure
...
wmi_send_pdev_caldata_version_check_cmd(void *wmi_hdl, uint32_t value)
{
	...
	if (wmi_handle->ops->send_pdev_caldata_version_check_cmd)
		return wmi_handle->ops->send_pdev_caldata_version_check_cmd(wmi_handle, value);
	...
	wmi_host_peer_txmu_cnt_event *param)
{
	...
	if (wmi_handle->ops->extract_mumimo_tx_count_ev_param)
		return wmi_handle->ops->extract_mumimo_tx_count_ev_param(wmi_handle,
			 evt_buf, param);
 ...
 * Return: QDF_STATUS_SUCCESS on success and QDF_STATUS_E_FAILURE for failure
 */
QDF_STATUS wmi_extract_esp_estimate_ev_param(void *wmi_hdl, void *evt_buf,
	wmi_host_esp_estimation_event *param)
{
	wmi_unified_t wmi_handle = (wmi_unified_t) wmi_hdl;

	if (wmi_handle->ops->extract_esp_estimation_ev_param)
		return wmi_handle->ops->extract_esp_estimation_ev_param(wmi_handle,
			 evt_buf, param);
...
/**
 * wmi_extract_peer_gid_userpos_list_ev_param() - extract peer userpos list from event
 * @wmi_handle: wmi handle
 * @param evt_buf: pointer to event buffer
 ...
 * Return: QDF_STATUS_SUCCESS on success and QDF_STATUS_E_FAILURE for failure
 */
QDF_STATUS wmi_extract_peer_gid_userpos_list_ev_param(void *wmi_hdl, void *evt_buf,
	wmi_host_peer_gid_userpos_list_event *param)
{
	wmi_unified_t wmi_handle = (wmi_unified_t) wmi_hdl;

	if (wmi_handle->ops->extract_peer_gid_userpos_list_ev_param)
		return wmi_handle->ops->extract_peer_gid_userpos_list_ev_param(wmi_handle,
			 evt_buf, param);
...
/**
 * wmi_extract_pdev_caldata_version_check_ev_param() - extract caldata from event
 * @wmi_handle: wmi handle
 * @param evt_buf: pointer to event buffer
 ...
 * Return: QDF_STATUS_SUCCESS on success and QDF_STATUS_E_FAILURE for failure
 */
QDF_STATUS wmi_extract_pdev_caldata_version_check_ev_param(void *wmi_hdl, void *evt_buf,
	wmi_host_pdev_check_cal_version_event *param)
{
	wmi_unified_t wmi_handle = (wmi_unified_t) wmi_hdl;

	if (wmi_handle->ops->extract_pdev_caldata_version_check_ev_param)
		return wmi_handle->ops->extract_pdev_caldata_version_check_ev_param(wmi_handle,
			 evt_buf, param);
...
/**
 * wmi_extract_swfda_vdev_id() - extract swfda vdev id from event
 * @wmi_handle		: wmi handle
 * @param evt_buf	: pointer to event buffer
 * @param vdev_map	: Pointer to hold vdev map
 *
...
QDF_STATUS wmi_extract_vdev_nac_rssi_stats(void *wmi_hdl, void *evt_buf,
		 wmi_host_vdev_nac_rssi_event *vdev_nac_rssi_stats)
{
	wmi_unified_t wmi_handle = (wmi_unified_t) wmi_hdl;
				...
				struct btcoex_cfg_params *param)
{
	...
	if (wmi_handle->ops->send_btcoex_wlan_priority_cmd)
		return wmi_handle->ops->send_btcoex_wlan_priority_cmd(wmi_handle,param);
				...
				struct btcoex_cfg_params *param)
{
	...
	if (wmi_handle->ops->send_btcoex_duty_cycle_cmd)
		return wmi_handle->ops->send_btcoex_duty_cycle_cmd(wmi_handle, param);
...
/**
 * wmi_extract_dfs_status_check() - extract dfs status check event
 * from FW.
 * @wmi_handle: wmi handle
 * @param evt_buf: pointer to event buffer
 * @param dfs_status_check: Pointer to hold the status event
 *
 * Return: QDF_STATUS_SUCCESS on success and QDF_STATUS_E_FAILURE for failure
 */

QDF_STATUS wmi_extract_dfs_status_from_fw(void *wmi_hdl, void *evt_buf,
	  A_UINT32 *dfs_status_check)
{
	wmi_unified_t wmi_handle = (wmi_unified_t) wmi_hdl;
