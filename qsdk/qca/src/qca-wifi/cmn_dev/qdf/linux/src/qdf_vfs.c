			    ...
			    struct attribute_group *attr)
{
	...
	ret = sysfs_create_group(devobj, attr);
...
QDF_STATUS
qdf_create_dev_file(struct kobject *devobj, struct bin_attribute *attr)
{
	int ret;
	...
	ret = sysfs_create_bin_file(devobj,
				    attr);

	return qdf_status_from_os_return(ret);
}

qdf_export_symbol(qdf_create_dev_file);

QDF_STATUS
qdf_vfs_delete_binfile(struct kobject *devobj, struct bin_attribute *attr)
{
	if (!devobj || !attr)
		return QDF_STATUS_E_INVAL;

	sysfs_remove_bin_file(devobj,
			      attr);
