...
ar9300_noise_floor_get(struct ath_hal *ah, int freq_mhz, int ch)
{
    ...
    ath_hal_printf(ah,
        ...
        (AH_PRIVATE(ah))->ah_mac_rev, freq_mhz, ch);
    return 0;
}
...
ar9300_get_spectral_params(struct ath_hal *ah, HAL_SPECTRAL_PARAM *ss)
{
    ...
    if (chan != NULL) {
        ...
        for (i = 0; i < NF_BUF_SIZE; i++) {
            ichain = (i % AH_SPECTRAL_MAX_CHAINS);
...
int
ar9300_is_spectral_supported(struct ath_hal *ah)
{
        u_int32_t val = 1;
        if (( AR_SREV_JET(ah))) {
            val = 0;
