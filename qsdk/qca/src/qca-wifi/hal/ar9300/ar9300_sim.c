    ...
    struct ar9300_txc* txc, u_int32_t num_bytes)
{
    ...
    struct qcn5500_rxs* jrxs = (struct qcn5500_rxs*)rx_buffer;
    ...
    rxs->status10 = 0;
    rxs->status11 = 0;
    if (AR_SREV_JET(dest_ah)) {
        jrxs->status12 = fake_rssi << QCN_rx_rssi_ant03_S |
            fake_rssi << QCN_rx_rssi_ant13_S;
        jrxs->status13 = 0;
        if ((txc->ds_ctl12 & AR_is_aggr) && is_first) {
            jrxs->status13 |= QCN_rx_first_aggr;
        }
        if (txc->ds_ctl12 & AR_is_aggr) {
            jrxs->status13 |= QCN_rx_aggr;
        }
        if (txc->ds_ctl12 & AR_more_aggr) {
            jrxs->status13 |= QCN_rx_more_aggr;
        }
        rx_buffer += sizeof(struct qcn5500_rxs);
            ...
            if (rx_protected_frame && !AH_PRIVATE(dest_ah)->ah_hal_sim.is_monitor) {
                if (tx_key_entry) {
                    ...
                    if (rx_key_entry) {
                        jrxs->status13 |=
                            QCN_rx_frame_ok |
                            QCN_rx_key_idx_valid |
                            (i << QCN_key_idx_S);
                    } else {
                        jrxs->status13 |= QCN_key_miss;
                    }
                } else {
                    jrxs->status13 |= QCN_key_miss;
                }
            } else {
                jrxs->status13 |= QCN_rx_frame_ok;
            }
        }

        jrxs->status13 |= QCN_rx_done;
    ...
    u_int32_t* frag_length)
{
    ...
    if (AR_SREV_JET(ah)) {
        jtxc = QCN5500TXC(AHSIM_AthBufToTxcDesc(ath_buf_ptr));
        tx_buffer =
            AHSIM_GetTxBufferVirtualAddress(ath_buf_ptr, (void*)jtxc->ds_data0);
    } else{
        tx_buffer =
    ...
    u_int32_t num_pkts, u_int64_t ba_mask)
{
    ...
    if (AR_SREV_JET(ah))
        txs->status2 |= (FAKE_RSSI_STRENGTH_GOOD << QCN_tx_rssi_ant03_S);
    ...
    u_int32_t dummy_pkt_bytes)
{
    ...
    if (AR_SREV_JET(ah)) {
        buffer[sizeof(struct qcn5500_rxs)] = 0x04;
        buffer[sizeof(struct qcn5500_rxs) + 1] = 0x00;
        OS_MEMCPY(
                buffer + sizeof(struct qcn5500_rxs) + 2,
                dummy_pkt,
                dummy_pkt_bytes);
    } else {
    ...
    jrxs = (struct qcn5500_rxs*)buffer;
    ...
    rxs->status11 = 0;
    if (AR_SREV_JET(ah)) {
        jrxs->status12 = 0x80 << QCN_rx_rssi_ant03_S |
            0x80 << QCN_rx_rssi_ant13_S;
        jrxs->status13 = QCN_rx_frame_ok | QCN_rx_done;
    } else {
