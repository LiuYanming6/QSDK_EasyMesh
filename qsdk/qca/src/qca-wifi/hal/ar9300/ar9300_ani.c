...
ar9300_ani_init_defaults(struct ath_hal *ah, HAL_HT_MACMODE macmode)
{
    ...
    /* Disable the bit 0 and bit 1 in register BB_mrc_cck_ctrl, fixes false CCK voting
     * due to spur magnitudes in scorpion and increases throughput in channel 6.
     */
    if (AR_SREV_SCORPION(ah)) {
        OS_REG_RMW_FIELD(ah, AR_PHY_MRC_CCK_CTRL,
            AR_PHY_MRC_CCK_ENABLE, 0);
        OS_REG_RMW_FIELD(ah, AR_PHY_MRC_CCK_CTRL,
            AR_PHY_MRC_CCK_MUX_REG, 0);
    }
                                  ...
                                  u_int8_t cck_noise_immunity_level)
{
     ...
     * It might possibly cause bb hang in dragonfly.
     * Fixes false CCK voting due to spur magnitudes in scorpion and increases throughput
     * in channel 6.
     */
    if ((!AR_SREV_DRAGONFLY(ah)) && (!AR_SREV_SCORPION(ah)) && (!AR_SREV_JET(ah)))
    {
