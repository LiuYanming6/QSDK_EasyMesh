...
void ar9300_wowoffload_download_devid_swar(struct ath_hal *ah)
{
    ...
    addr += 4;
    HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) pcie_000 = %08x\n",
    ...
    addr += 4;
    HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) pcie_008 = %08x\n",
    ...
    addr += 4;
    HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) pcie_02c = %08x\n",
    ...
    addr += 4;
    HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) pcie_70c = %08x\n",
    ...
    addr += 4;
    HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) pcie_040 = %08x\n",
...
u_int32_t ar9300_wowoffload_download_rekey_data(struct ath_hal *ah, u_int32_t *data, u_int32_t bytes)
{
    ...
    HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) %s, bytes=%d\n", __func__, bytes);
    if (AR_SREV_JUPITER(ah) &&
        (bytes > (AR_WOW_OFFLOAD_GTK_DATA_WORDS_JUPITER * 4)))
    {
        bytes = AR_WOW_OFFLOAD_GTK_DATA_WORDS_JUPITER * 4;
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) bytes truncated to %d\n", bytes);
    }
    /* Check if mailbox is busy */
    if (mbox_status != 0) {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "%s: Mailbox register busy! Reg = 0x%x", __func__, mbox_status);
        return 1;
    }
                                            ...
                                            u_int32_t* datap)
{
    ...
    HDPRINTF(ah, HAL_DBG_UNMASKABLE, "%s: id=%d, period=%d ms, size=%d bytes\n",
    ...
    for (i=0; i<n_data * 4; i+=4) {
        ...
        /*HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) %08x\n", *datap);*/
        datap++;
    }
}
...
#define AH_DEFAULT_BT_WAIT_TIMEOUT  3000000; /* 3 sec */
    ...
    HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) TIMEOUT: %d us\n", bt_handshake_timeout_us);
    ...
    if (pattern_enable & AH_WOW_USER_PATTERN_EN) {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - User pattern\n");
        val |= AR_EMB_CPU_WOW_ENABLE_PATTERN_MATCH;
    }
    else {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - User pattern\n");
    }
        ...
        )
    {
        ...
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - Magic pattern\n");
    }
    else {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - Magic pattern\n");
    }
        ...
        )
    {
        ...
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - Kepp alive fail\n");
    }
    else {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - Kepp alive fail\n");
    }
    if (pattern_enable & AH_WOW_BEACON_MISS) {
        val |= AR_EMB_CPU_WOW_ENABLE_BEACON_MISS;
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - Becon Miss\n");
    }
    else {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - Becon Miss\n");
    }
    ...
    if (!ath_hal_wait(ah, AR_MBOX_CTRL_STATUS, AR_MBOX_WOW_CONF, AR_MBOX_WOW_CONF, bt_handshake_timeout_us)) {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "%s: WoW offload handshake failed", __func__);
        return 0;
    }
    else {
        OS_REG_CLR_BIT(ah, AR_MBOX_CTRL_STATUS, AR_MBOX_WOW_CONF);
        HDPRINTF(ah, HAL_DBG_POWER_MGMT, "%s: WoW offload handshake successful",__func__);
    } 
    return 1;
}
#endif /* ATH_WOW_OFFLOAD */
...
#define POWER_UP_TIME   10000
    ...
    if (set_chip) {
        ...
        if ((OS_REG_READ(ah, AR_RTC_STATUS) & AR_RTC_STATUS_SHUTDOWN)) {
            if (ar9300_set_reset_reg(ah, HAL_RESET_POWER_ON) != true) {
                HALASSERT(0);
                return false;
            }
        }
        ...
        if (AH_PRIVATE(ah)->ah_caps.hal_mci_support) {
            if (ar9300_get_power_mode(ah) == HAL_PM_AWAKE) {
                    ...
                    !ahp->ah_mci_halted_bt_gpm)
                {
                    ...
                    ar9300_mci_send_coex_halt_bt_gpm(ah, true, true);
                }
            }
    ...
    bool offloadEnable)
{
    ...
    if (HAL_WOW_CTRL(ah, HAL_WOW_OFFLOAD_FORCE_BT_SLEEP)) {
        ...
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - BT SLEEP\n");
    } else {
        wow_feature_enable &= ~AR_WOW_OFFLOAD_ENA_BT_SLEEP;
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - BT SLEEP\n");
    }
        
    if (HAL_WOW_CTRL(ah, HAL_WOW_OFFLOAD_SW_NULL_DISABLE)) {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - SW NULL\n");
        wow_feature_enable &= ~AR_WOW_OFFLOAD_ENA_SW_NULL;
    } else {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - SW NULL\n");
        wow_feature_enable |= AR_WOW_OFFLOAD_ENA_SW_NULL;
    }

    if (HAL_WOW_CTRL(ah, HAL_WOW_OFFLOAD_DEVID_SWAR_DISABLE)) {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - DevID SWAR\n");
        wow_feature_enable &= ~AR_WOW_OFFLOAD_ENA_DEVID_SWAR;
    } else {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - DevID SWAR\n");
        wow_feature_enable |= AR_WOW_OFFLOAD_ENA_DEVID_SWAR;
    }

    if (pattern_enable & AH_WOW_ACER_KEEP_ALIVE_EN) {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - Acer SWKA\n");
        wow_feature_enable |= AR_WOW_OFFLOAD_ENA_SWKA;
    } else {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - Acer SWKA\n");
        wow_feature_enable &= ~AR_WOW_OFFLOAD_ENA_SWKA;
    }

    if (pattern_enable & AH_WOW_ACER_MAGIC_EN) {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - Standard Magic\n");
        wow_feature_enable &= ~AR_WOW_OFFLOAD_ENA_STD_MAGIC;
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - Acer Magic\n");
        wow_feature_enable |= AR_WOW_OFFLOAD_ENA_ACER_MAGIC;
    } else {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - Standard Magic\n");
        wow_feature_enable |= AR_WOW_OFFLOAD_ENA_STD_MAGIC;
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - Acer Magic\n");
        wow_feature_enable &= ~AR_WOW_OFFLOAD_ENA_ACER_MAGIC;
    }

    if ((pattern_enable & AH_WOW_4WAY_HANDSHAKE_EN) ||
        HAL_WOW_CTRL(ah, HAL_WOW_OFFLOAD_FORCE_4WAY_HS_WAKE)) {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - 4Way Handshake\n");
        wow_feature_enable |= AR_WOW_OFFLOAD_ENA_4WAY_WAKE;
    } else {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - 4Way Handshake\n");
        wow_feature_enable &= ~AR_WOW_OFFLOAD_ENA_4WAY_WAKE;
    }
    ...
    {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - AP loss wake\n");
        wow_feature_enable |= AR_WOW_OFFLOAD_ENA_AP_LOSS_WAKE;
    } else {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - AP loss wake\n");
        wow_feature_enable &= ~AR_WOW_OFFLOAD_ENA_AP_LOSS_WAKE;
    }
    ...
    {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - GTK error wake\n");
        wow_feature_enable |= AR_WOW_OFFLOAD_ENA_GTK_ERROR_WAKE;
    } else {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - GTK error wake\n");
        wow_feature_enable &= ~AR_WOW_OFFLOAD_ENA_GTK_ERROR_WAKE;
    }

    if (pattern_enable & AH_WOW_GTK_OFFLOAD_EN) {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - GTK offload\n");
        wow_feature_enable |= AR_WOW_OFFLOAD_ENA_GTK;
    } else {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - GTK offload\n");
        wow_feature_enable &= ~AR_WOW_OFFLOAD_ENA_GTK;
    }

    if (pattern_enable & AH_WOW_ARP_OFFLOAD_EN) {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - ARP offload\n");
        wow_feature_enable |= AR_WOW_OFFLOAD_ENA_ARP_OFFLOAD;
    } else {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - ARP offload\n");
        wow_feature_enable &= ~AR_WOW_OFFLOAD_ENA_ARP_OFFLOAD;
    }

    if (pattern_enable & AH_WOW_NS_OFFLOAD_EN) {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) ENA - NS offload\n");
        wow_feature_enable |= AR_WOW_OFFLOAD_ENA_NS_OFFLOAD;
    } else {
        HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) DIS - NS offload\n");
        wow_feature_enable &= ~AR_WOW_OFFLOAD_ENA_NS_OFFLOAD;
    }
    ...
    if (timeout_in_seconds) {
        ...
        printk("----- %s[%d] ------ TOValue %d IMR %x, IMR_S5 %x "
            "TSF_L32 = %x timer_next_reg = %x, timer_period = %x, "
            "timer_mode_reg = %x BSSID_lo = %x, BSSID_Hi = %x\n",
            __func__, __LINE__, timeout_in_seconds,
    ...
    if (offloadEnable) {
        ...
        AH9300(ah)->ah_chip_full_sleep = false;
...
#endif /* ATH_WOW_OFFLOAD */
    {
        ...
        AH9300(ah)->ah_chip_full_sleep = true;
    }
...
ar9300_wow_wake_up(struct ath_hal *ah, bool offloadEnabled)
{
    ...
    if (offloadEnabled) {
        ...
        if (val) {
            if (val & AR_EMB_CPU_WOW_STATUS_MAGIC_PATTERN) {
                HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) SW MAGIC_PATTERN\n");
                wow_status |= AH_WOW_MAGIC_PATTERN_EN;
            }
            if (val & AR_EMB_CPU_WOW_STATUS_PATTERN_MATCH) {
                HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) SW USER_PATTERN\n");
                wow_status |= AH_WOW_USER_PATTERN_EN;
            }
            if (val & AR_EMB_CPU_WOW_STATUS_KEEP_ALIVE_FAIL) {
                HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) SW KEEP_ALIVE_FAIL\n");
                wow_status |= AH_WOW_LINK_CHANGE;
            }
            if (val & AR_EMB_CPU_WOW_STATUS_BEACON_MISS) {
                HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) SW BEACON_FAIL\n");
...
#endif /* ATH_WOW_OFFLOAD */
    {
        ...
        if (val) {
            if (val & AR_WOW_MAGIC_PAT_FOUND) {
                HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) HW MAGIC_PATTERN\n");
                wow_status |= AH_WOW_MAGIC_PATTERN_EN;
            }
            if (AR_WOW_PATTERN_FOUND(val)) {
                ...
                //}
                wow_status |= AH_WOW_USER_PATTERN_EN;
                HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) HW USER_PATTERN\n");
            }
            if (val & AR_WOW_KEEP_ALIVE_FAIL) {
                HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) HW KEEP_ALIVE_FAIL\n");
                wow_status |= AH_WOW_LINK_CHANGE;
            }
            if (val & AR_WOW_BEACON_FAIL) {
                HDPRINTF(ah, HAL_DBG_UNMASKABLE, "(WOW) HW BEACON_FAIL\n");
                wow_status |= AH_WOW_BEACON_MISS;
            }
        }
    }
     ...
     */
    HDPRINTF(AH_NULL, HAL_DBG_UNMASKABLE, "%s: restore AR_WA\n", __func__);
    if (AH_PRIVATE(ah)->ah_is_pci_express == true) {
