...
#define OSPREY_EEPMISC_SW_RXDCO      0x04
#define OSPREY_CUSTOMER_DATA_SIZE    20
#define JET_NUM_CONFIGADDR           5
#define JET_NUM_POWEROFFSET_HT20     6
#define JET_NUM_POWEROFFSET_HT40     6
#define JET_BOARDID_YB676           3
#define JET_BOARDID_YB413           4
#define JET_BOARDID_YC637           6
...
typedef enum targetPowerHTRates {
    HT_TARGET_RATE_0_8_16_24,
    HT_TARGET_RATE_1_3_9_11_17_19_25_27,
    HT_TARGET_RATE_4,
    HT_TARGET_RATE_5,
    HT_TARGET_RATE_6,
    HT_TARGET_RATE_7,
...
const static int mapRate2Index[32]=
{
    0,1,1,1,2,
    3,4,5,0,1,
    1,1,6,7,8,
...
#define JET_MAX_MODAL_RESERVED 1
#define MAX_MODAL_RESERVED 5
#define MAX_MODAL_FUTURE 5
#define JET_MAX_MODAL_FUTURE 1
#define MAX_BASE_EXTENSION_FUTURE 2
#define JET_MAX_BASE_EXTENSION_FUTURE 1
...
typedef struct jet_BaseExtension_1 {
	u_int8_t  ant_div_control;
	u_int8_t  future[JET_MAX_BASE_EXTENSION_FUTURE];
    int8_t    eep_paprd_trainning_power; // half dB
	u_int8_t  misc_enable; //misc_enable : bit0 - Tx Gain Cap enable
                           // bit1 - uncompressed Checksum Enable
                           // bit 2 &3 - MinCCA PWR Enable 2G and 5G
	int8_t  tempslopextension[MAX_TEMP_SLOPE];
...
typedef struct jet_BaseExtension_2 {
	int8_t    temp_slope_low;
	int8_t    temp_slope_high;
    u_int8_t   xatten1_db_low[JET_MAX_CHAINS];           	// 4  //xatten1_db for merlin (0xa20c/b20c 5:0)
    u_int8_t   xatten1_margin_low[JET_MAX_CHAINS];          	// 4  //xatten1_margin for merlin (0xa20c/b20c 16:12
    u_int8_t   xatten1_db_high[JET_MAX_CHAINS];           	// 4  //xatten1_db for merlin (0xa20c/b20c 5:0)
    u_int8_t   xatten1_margin_high[JET_MAX_CHAINS];          	// 4  //xatten1_margin for merlin (0xa20c/b20c 16:12
} __packed JET_BASE_EXTENSION_2;
...
//Note the order of the fields in this structure has been optimized to put all fields likely to change together
typedef struct jetModalEepHeader {
    u_int32_t  ant_ctrl_common;                         // 4   idle, t1, t2, b (4 bits per setting)
    u_int32_t  ant_ctrl_common2;                        // 4    ra1l1, ra2l1, ra1l2, ra2l2, ra12
    u_int16_t  ant_ctrl_chain[JET_MAX_CHAINS];       // 8   idle, t, r, rx1, rx12, b (2 bits each)
    u_int8_t   xatten1_db[JET_MAX_CHAINS];           // 4  //xatten1_db for merlin (0xa20c/b20c 5:0)
    u_int8_t   xatten1_margin[JET_MAX_CHAINS];       // 4  //xatten1_margin for merlin (0xa20c/b20c 16:12
    int8_t     temp_slope;
    int8_t     voltSlope;
    u_int8_t   spur_chans[OSPREY_EEPROM_MODAL_SPURS];   // spur channels in usual fbin coding format
    int8_t     noise_floor_thresh_ch[JET_MAX_CHAINS];// 4    //Check if the register is per chain
    u_int8_t   obdboffst_aux1[JET_MAX_CHAINS];       // 4    //obdboffst2G_aux1 for dragonfly (0x16044/0x16444/0x16844, 5:3)
    u_int8_t   obdboffst_aux2[JET_MAX_CHAINS];       // 4    //obdboffst2G_aux2 for dragonfly (0x16044/0x16444/0x16844, 2:0)
    u_int8_t   temp_slope_ext[JET_MAX_CHAINS];       // 4 Temperature slope (per-chain)
    u_int8_t   reserved[JET_MAX_MODAL_RESERVED];
    int8_t     quick_drop;
    u_int8_t   xpa_bias_lvl;                            // 1
    u_int8_t   tx_frame_to_data_start;                  // 1
    u_int8_t   tx_frame_to_pa_on;                       // 1
    ...
    u_int8_t   thresh62;                                // 1
    u_int32_t  paprd_rate_mask_ht20;
    u_int32_t  paprd_rate_mask_4ss_ht20;
    u_int32_t  paprd_rate_mask_ht40;
    u_int32_t  paprd_rate_mask_4ss_ht40;
    u_int16_t  switchcomspdt;
    u_int8_t   xLNA_bias_strength;                      // bit: 0,1:chain0, 2,3:chain1, 4,5:chain2, 6,7:chain3
    u_int8_t   rf_gain_cap;
    u_int8_t   tx_gain_cap;                             // bit0:4 txgain cap, txgain index for max_txgain + 20 (10dBm higher than max txgain)
    u_int8_t   futureModal[JET_MAX_MODAL_FUTURE];
    int8_t     xtal_ppm_target;     // PPM target
    int8_t     xtal_ppm_tolerance;  // PPM tolerance
    int8_t     temp_slope_high_3;     // temp slope for chain3
    int8_t     eep_iqmask;      // iq mask
    // last 12 bytes stolen and moved to newly created base extension structure
} __packed JET_MODAL_EEP_HEADER;                    // == 100 B
...
typedef struct jetCalTargetPowerHt {
    u_int8_t  t_pow2x[18];
} __packed JET_CAL_TARGET_POWER_HT;
...
/* Structure definition for ATE thermal data Begin */
#define NUM_ATE_THERMAL_ADC_CAL_CHAINS  4

typedef struct otp_ate_parsed_pdadc_cal {
    u_int16_t      voltHigh;
    u_int16_t      voltLow;
    u_int16_t      pdadcHigh;
    u_int16_t      pdadcLow;
} __packed OTP_ATE_PARSED_PDADC_CAL;

typedef struct kgko_struct {
    int16_t   Kg; //thermAdcScaleGain;
    int8_t    Ko; //thermADCOffset;
    u_int8_t   notUsed;
} __packed OTP_ATE_KGKO;

typedef struct otp_ate_temp {
    u_int16_t  tempCode[NUM_ATE_THERMAL_ADC_CAL_CHAINS];
    u_int8_t   temp;
    u_int8_t   notUsed[3];
} __packed OTP_ATE_PARSED_TEMP;

typedef struct otp_ate_parsed_data {
    OTP_ATE_PARSED_PDADC_CAL  atePdadcCAL[NUM_ATE_THERMAL_ADC_CAL_CHAINS];
    OTP_ATE_PARSED_TEMP       ateTemp;
    //   OTP_ATE_KGKO              kgko;
} __packed OTP_ATE_PARSED_DATA;

typedef struct read_otp_ate_data {
    u_int8_t temp;

    u_int8_t tempCode0_l;  // chain 0
    u_int8_t tempCode0_h;
    u_int8_t tempCode1_l;  // chain 1
    u_int8_t tempCode1_h;
    u_int8_t tempCode2_l;  // chain 2
    u_int8_t tempCode2_h;
    u_int8_t tempCode3_l;  // chain 3
    u_int8_t tempCode3_h;


    u_int8_t  voltHigh0_l;
    u_int8_t  voltLow0_l;
    u_int8_t  voltHigh1_l;
    u_int8_t  voltLow1_l;
    u_int8_t  voltHigh2_l;
    u_int8_t  voltLow2_l;
    u_int8_t  voltHigh3_l;
    u_int8_t  voltLow3_l;

    u_int8_t    voltHigh0_h : 2,
                voltHigh1_h : 2,
                voltHigh2_h : 2,
                voltHigh3_h : 2;

    u_int8_t    voltLow0_h : 2,
                voltLow1_h : 2,
                voltLow2_h : 2,
                voltLow3_h : 2;

    u_int8_t  pdadcHigh0_l;
    u_int8_t  pdadcLow0_l;
    u_int8_t  pdadcHigh1_l;
    u_int8_t  pdadcLow1_l;
    u_int8_t  pdadcHigh2_l;
    u_int8_t  pdadcLow2_l;
    u_int8_t  pdadcHigh3_l;
    u_int8_t  pdadcLow3_l;

    u_int8_t    pdadcLow0_h : 1,
                pdadcLow1_h : 1,
                pdadcLow2_h : 1,
                pdadcLow3_h : 1,
                pdadcHigh0_h : 1,
...
}__packed OTP_ATE_READ_DATA;

#define ATE_THERMAL_ADC_CAL_OFFSET 1000
#define ATE_THERMAL_ADC_CAL_LEN     16

#define ATE_DATA_MAX               128
#define DFLT_FS                    700
#define DFLT_VREF                  500


//#define DFLT_ATE_CAL_TEMP  ((DFLT_CAL_TEMP << ATE_CAL_TEMP_LSB) | (DFLT_THERMAL_CODE << ATE_CAL_THERM_CODE1_LSB) | (DFLT_THERMAL_CODE << ATE_CAL_THERM_CODE0_LSB))

/* chipCalData.configFlag */
#define ATE_THERMAL_CAL_ENABLED          0x1
#define ATE_TPC_LOWER_PERFORM            0x2
#define ATE_THERMAL_TEMPINDEGC_ENABLED   0x4

#define DFLT_TEMP_OFFSET                   40
#define TEMP_SLOPE_SHIFT                   7
#define DFLT_TEMP_SLOPE                    216// 216= (1.69 * 2^(TEMP_SLOPE_SHIFT))//189=(1.48 * 2^(TEMP_SLOPE_SHIFT))
#define DFLT_THERMAL_CODE               0x93//0x7B   // likely room temperature
#define DFLT_CAL_TEMP                        65   // likely room temperature
#define ATE_CAL_TEMP_LSB                  18
#define ATE_CAL_TEMP_MASK               0xFF
#define ATE_CAL_THERM_CODE0_LSB    0
#define ATE_CAL_THERM_CODE2_LSB    0
#define ATE_CAL_THERM_MASK             0x1FF
#define ATE_CAL_THERM_CODE1_LSB            9
#define ATE_CAL_THERM_CODE3_LSB            9

typedef struct {
    int16_t     thermAdcScaledGain;
    int8_t      thermAdcOffset;
    u_int8_t     rbias;
    int16_t     thermAdcScaledGain1;
    int8_t      thermAdcOffset1;
    u_int8_t     ate_reserved1;
    int16_t     thermAdcScaledGain2;
    int8_t      thermAdcOffset2;
    u_int8_t     ate_reserved2;
    int16_t     thermAdcScaledGain3;
    int8_t      thermAdcOffset3;
    u_int8_t     ate_reserved3;
    u_int8_t     configFlag;
    u_int8_t     ate_reserved4[3];
    u_int32_t    ateCALTemp;
    u_int32_t    ateCALTemp1;
    u_int8_t     ate_reserved5[8];
} __packed CAL_DATA_PER_CHIP;
/* Structure definition for ATE thermal data End */
...
#ifndef ART_BUILD
} __packed osprey_eeprom_t;
#else
} __packed ar9300_eeprom_t;
#endif

typedef struct jetEeprom {
    u_int8_t  eeprom_version;
    u_int8_t  template_version;
    u_int8_t  mac_addr[6];
    u_int8_t  custData[OSPREY_CUSTOMER_DATA_SIZE];
    ...
    JET_MODAL_EEP_HEADER   modal_header_2g;
    JET_BASE_EXTENSION_1   base_ext1;
    u_int8_t            cal_freq_pier_2g[OSPREY_NUM_2G_CAL_PIERS];
    OSP_CAL_DATA_PER_FREQ_OP_LOOP cal_pier_data_2g[JET_MAX_CHAINS][OSPREY_NUM_2G_CAL_PIERS];
    u_int8_t cal_target_freqbin_cck[OSPREY_NUM_2G_CCK_TARGET_POWERS];
    u_int8_t cal_target_freqbin_2g[OSPREY_NUM_2G_20_TARGET_POWERS];
    u_int8_t cal_target_freqbin_2g_ht20[OSPREY_NUM_2G_20_TARGET_POWERS];
    u_int8_t cal_target_freqbin_2g_ht40[OSPREY_NUM_2G_40_TARGET_POWERS];
    CAL_TARGET_POWER_LEG cal_target_power_cck[OSPREY_NUM_2G_CCK_TARGET_POWERS];
    CAL_TARGET_POWER_LEG cal_target_power_2g[OSPREY_NUM_2G_20_TARGET_POWERS];
    JET_CAL_TARGET_POWER_HT cal_target_power_2g_ht20[OSPREY_NUM_2G_20_TARGET_POWERS];
    JET_CAL_TARGET_POWER_HT cal_target_power_2g_ht40[OSPREY_NUM_2G_40_TARGET_POWERS];
    u_int8_t   ctl_index_2g[OSPREY_NUM_CTLS_2G];
    u_int8_t   ctl_freqbin_2G[OSPREY_NUM_CTLS_2G][OSPREY_NUM_BAND_EDGES_2G];
    OSP_CAL_CTL_DATA_2G   ctl_power_data_2g[OSPREY_NUM_CTLS_2G];

    JET_MODAL_EEP_HEADER   modal_header_5g;
    JET_BASE_EXTENSION_2 base_ext2;
    u_int8_t            cal_freq_pier_5g[OSPREY_NUM_5G_CAL_PIERS];
    OSP_CAL_DATA_PER_FREQ_OP_LOOP cal_pier_data_5g[JET_MAX_CHAINS][OSPREY_NUM_5G_CAL_PIERS];
    u_int8_t cal_target_freqbin_5g[OSPREY_NUM_5G_20_TARGET_POWERS];
    u_int8_t cal_target_freqbin_5g_ht20[OSPREY_NUM_5G_20_TARGET_POWERS];
    u_int8_t cal_target_freqbin_5g_ht40[OSPREY_NUM_5G_40_TARGET_POWERS];
    CAL_TARGET_POWER_LEG cal_target_power_5g[OSPREY_NUM_5G_20_TARGET_POWERS];
    JET_CAL_TARGET_POWER_HT cal_target_power_5g_ht20[OSPREY_NUM_5G_20_TARGET_POWERS];
    JET_CAL_TARGET_POWER_HT cal_target_power_5g_ht40[OSPREY_NUM_5G_40_TARGET_POWERS];
    u_int8_t   ctl_index_5g[OSPREY_NUM_CTLS_5G];
    u_int8_t   ctl_freqbin_5G[OSPREY_NUM_CTLS_5G][OSPREY_NUM_BAND_EDGES_5G];
    OSP_CAL_CTL_DATA_5G   ctl_power_data_5g[OSPREY_NUM_CTLS_5G];
    ...
    u_int8_t    nart_version;
    u_int8_t    ref_file_version;
    u_int8_t    board_id;
    u_int8_t    jet_misc_flag; // bit0: use_cck_spur_mitigation
    u_int8_t    spur_rssi_thresh;
    u_int8_t    xtal_bibxosc;
    u_int8_t    xtal_len;
    u_int8_t    xtal_xoscrefv;
    u_int8_t    jet_eep_reserved[22];
    u_int32_t   configAddr[128]; // 128*4 = 512B
    int8_t      powerOffset2GHT20[6];
    int8_t      powerOffset2GHT40[6];
#ifndef ART_BUILD
} __packed ar9300_eeprom_t;
#else
} __packed ar5500_eeprom_t;
#endif
/*
** SWAP Functions
...
#define ar9300_get_ntxchains(_txchainmask) \
   (((_txchainmask >> 3) & 1) + ((_txchainmask >> 2) & 1) + ((_txchainmask >> 1) & 1) + (_txchainmask & 1))
...
#define AR9300_RATES_HT40_OFFSET    26
...
extern ar9300_eeprom_t *ar9300_eeprom_struct_default(void);
extern osprey_eeprom_t *osprey_eeprom_struct_default(int default_index);
extern osprey_eeprom_t *osprey_eeprom_struct_default_find_by_id(int ver);
extern int osprey_eeprom_struct_default_many(void);
#else
extern u_int16_t ar9300_eeprom_struct_size(struct ath_hal *ah);
extern ar9300_eeprom_t *ar9300_eeprom_struct_default(int default_index);
extern ar5500_eeprom_t *ar5500_eeprom_struct_default(void);
...
extern void osprey_eeprom_template_preference(int32_t value);
...
extern bool ar9300_calibration_data_read(struct ath_hal *ah, long address, void *buffer, int many);
...
extern void copy_jet_to_osprey(struct jetEeprom *jet_eeprom, struct ospreyEeprom *osprey_eeprom);

extern void copy_osprey_to_jet(struct jetEeprom *jet_eeprom, struct ospreyEeprom *osprey_eeprom);
extern void copy_osprey_base_extension_2_to_jet(JET_BASE_EXTENSION_2 *jet_base_ext2, OSPREY_BASE_EXTENSION_2 *osprey_base_ext2);
extern void copy_osprey_modal_eep_header_to_jet(JET_MODAL_EEP_HEADER *jet_modal_header, OSPREY_MODAL_EEP_HEADER *osprey_modal_header);
extern void copy_osp_cal_target_power_ht_to_jet(JET_CAL_TARGET_POWER_HT *jet_cal_target_power,
