    ...
    if ( (sc->sc_reset_type ==  ATH_RESET_NOLOSS) ){
        if (sc->sc_opmode == HAL_M_IBSS) {
            if (ath_hal_getrxfilter(sc->sc_ah) & HAL_RX_FILTER_PHYRADAR) {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: radar was enabled before reset\n", __func__);
                enable_radar = 1;
            }
        }
...
#endif
        if (enable_radar) {
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: enabling radar after reset\n", __func__);
...
void ath_wbuf_recycle(struct ath_softc *sc, struct ath_buf *bf, wbuf_t wbuf)
{
    ...
    u_int8_t head_frag = skb->head_frag;
#endif
    skb_dst_drop(skb);

    shinfo = skb_shinfo(skb);
    OS_MEMZERO(shinfo, offsetof(struct skb_shared_info, dataref));
    ...
    bf->bf_buf_addr[0] = bf->bf_dmacontext = virt_to_phys(skb->data);
...
bool ath_can_wbuf_recycle(wbuf_t wbuf)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 5, 0)
	printk("Review Functions ath_wbuf_recycle and ath_wbuf_can_recycle"
     ...
     */
    if (unlikely(skb->destructor != NULL)) {
        return false;
    }
    /* Since the size of rx skb's will be same,conditional check
       on it is neglected */
...
int ath_rx_indicate(struct ath_softc *sc, wbuf_t wbuf, ieee80211_rx_status_t *status, u_int16_t keyix)
{
    ...
    if (likely(requeue)){
            ...
            ATH_SET_RX_CONTEXT_BUF(nwbuf, bf);
            /* queue the new wbuf or current buffer to HW after reset wbuf fiels */
...
ath_dynamic_sysctl_register(struct ath_softc *sc)
{
	...
	char *devname = sc->sc_osdev->netdev->name ;
        ...
        if (sc->sc_sysctls == NULL) {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: no memory for sysctl table!\n", __func__);
                return;
        }
        ...
        if (!sc->sc_sysctl_header) {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: failed to register sysctls!\n", devname);
                kfree(sc->sc_sysctls);
                sc->sc_sysctls = NULL;
        }
}
...
int ath_proc_write_nodefixedrate(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Proc write: macaddr = %s\n", ether_sprintf(macaddr));
    fixedratecode = (u_int32_t)simple_strtoul(buffer+15, NULL, 16);
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Proc write: fixedratecode = %x\n", fixedratecode);

    for (i=0; i<num_global_scn; i++) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Searching for node in wifi%d\n", i);
...
{
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO"Removing athdebug proc file\n");
...
init_ath_dev(void)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO "%s: "
...
#if !ATH_RXBUF_RECYCLE
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 5, 0)
EXPORT_SYMBOL(ath_can_wbuf_recycle);
EXPORT_SYMBOL(ath_wbuf_recycle);
#endif
#endif
