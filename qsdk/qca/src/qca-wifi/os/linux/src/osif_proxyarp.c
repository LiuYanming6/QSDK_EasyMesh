...
int wlan_proxy_arp(wlan_if_t vap, wbuf_t wbuf)
{
    ...
    if (IEEE80211_VAP_IS_DELIVER_80211_ENABLED(vap)) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: IEEE80211_FEXT_DELIVER_80211 is not supported\n", __func__);
        goto pass;
    }
    if (eh == NULL)
        goto drop;
                ...
                if ((src_type == IPV6_ADDR_ANY) && (dst_type & IPV6_ADDR_MULTICAST)) {
                    ...
                    if (IS_MYNODE(vap, ni)) {
                        ...
                        if (ieee80211_node_add_ipv6(nt, ni, (u8 *)&msg->target)) {
                            QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Maximum multiple IPv6 addresses exceeded "
