...
void* ath_green_ap_attach( struct ieee80211com* ic)
{
    ...
    green_ap->power_save_mode       = GREEN_AP_MODE_NO_STA;  /* "NO STA" is the default mode */
    green_ap->num_nodes             = 0;                    /* Initial node count is zero */
    green_ap->num_nodes_multistream = 0;                    /* Initial multistream node count is zero */
...
static int ath_green_ap_ant_ps_mode_on_tmr_fun(void *ic_ptr)
{
     ...
     * the green_ap_ps_lock, which will case a double lock.
    ...
    /* Free the reference outside the green_ap_ps_lock taken above */
...
void ath_green_ap_state_mc(struct ieee80211com* ic, ath_ps_event event)
{
    ...
    switch(event) {
        ...
        case ATH_PS_EVENT_INC_MULTISTREAM_STA:
            green_ap->num_nodes_multistream++;
            break;

        case ATH_PS_EVENT_DEC_MULTISTREAM_STA:
            if (green_ap->num_nodes_multistream) {
                green_ap->num_nodes_multistream--;
            }
            break;
    ...
    switch( green_ap->ps_state ) {
        ...
        case ATH_PWR_SAVE_WAIT:
            ...
            } else if (((green_ap->power_save_mode == GREEN_AP_MODE_NO_STA) && green_ap->num_nodes)
                    || ((green_ap->power_save_mode == GREEN_AP_MODE_NUM_STREAM) && green_ap->num_nodes_multistream)) {
...
void ath_green_ap_sc_set_enable(struct ieee80211com *ic, int32_t val )
{
    ...
    if (val) {
        ...
        if(val == GREEN_AP_MODE_NUM_STREAM)
            green_ap->power_save_mode = GREEN_AP_MODE_NUM_STREAM;
        else
            green_ap->power_save_mode = GREEN_AP_MODE_NO_STA;
...
int32_t ath_green_ap_sc_get_enable(struct ieee80211com *ic)
{
    ...
    return green_ap->power_save_enable ? green_ap->power_save_mode : 0;
