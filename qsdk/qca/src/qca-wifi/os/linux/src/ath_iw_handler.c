...
*/
    ...
    */
    ...
    { ATH_PARAM_TWICE_ANTENNA_GAIN | ATH_PARAM_SHIFT,
        IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "twice_antgain" },
    { ATH_PARAM_TWICE_ANTENNA_GAIN | ATH_PARAM_SHIFT,
        0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_twice_antgain"},
    ...
    { ATH_PARAM_TT_TEMP | ATH_PARAM_SHIFT,
        IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0,    "force_temp" },
    { ATH_PARAM_TT_TEMP | ATH_PARAM_SHIFT,
        0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1,    "get_temp" },
    { ATH_PARAM_TT_LEVEL | ATH_PARAM_SHIFT,
        IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0,    "force_tt_level" },
    { ATH_PARAM_TT_LEVEL | ATH_PARAM_SHIFT,
        0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1,    "get_tt_level" },
#if ATH_PARAMETER_API
    { ATH_PARAM_PAPI_ENABLE |ATH_PARAM_SHIFT,
        IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0,       "set_papi" },
    { ATH_PARAM_PAPI_ENABLE |ATH_PARAM_SHIFT, 0,
                           ...
                           char *extra)
{
    ...
    return ath_ucfg_set_countrycode(scn, p);
}
...
static int ath_iw_set_dscp_tid_map(struct net_device *dev, struct iw_request_info *info, void *w, char *extra)
{
    ...
    ath_ucfg_set_dscp_tid_map(scn, tos, tid);
...
static int ath_iw_get_dscp_tid_map(struct net_device *dev, struct iw_request_info *info, void *w, char *extra)
{
    ...
    param[0] = ath_ucfg_get_dscp_tid_map(scn, tos);
...
static int ath_iw_sethwaddr(struct net_device *dev, struct iw_request_info *info, void *w, char *extra)
{
    ...
    if (!retval)
        retval = ath_ucfg_set_mac_address(scn, addr);
                           ...
                           char *extra)
{
    ...
    return ath_ucfg_set_smartantenna_param(scn, extra);
}
                           ...
                           char *extra)
{
    ...
    return ath_ucfg_get_smartantenna_param(scn, extra);
}
#endif
...
#define N(a) (sizeof(a)/sizeof(a[0]))
    ...
    if (sc == NULL) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "   Hardware queue dump not yet supported on %s.\n", ic->ic_osdev->netdev->name);
        return;
    }

#if 1
    /* HW queues */
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "HW TX Queues, %s:\n", ic->ic_osdev->netdev->name);
    for (i = 0; i < HAL_NUM_TX_QUEUES; i++) {
        ...
        if (txq->axq_depth > 0 || ath_hal_numtxpending(sc->sc_ah, txq->axq_qnum) > 0) {
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "hw_q=%d, axq_depth=%d, num_tx_pend=%d, axq_aggr_depth=%d, ",
                txq->axq_qnum, txq->axq_depth, ath_hal_numtxpending(sc->sc_ah, txq->axq_qnum), txq->axq_aggr_depth);
#if ATH_TX_BUF_FLOW_CNTL
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "axq_minfree=%03d, axq_num_buf_used=%03d, axq_totalq=%d\n",
                txq->axq_minfree, txq->axq_num_buf_used, txq->axq_totalqueued);
#else
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "axq_totalq=%d\n", txq->axq_totalqueued);
    ...
    /* AC queues*/
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "HW AC Queues:\n");
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "   ac-queue mapping: AC_BK(#%d)->txq %d, AC_BE(#%d)->txq %d, ",
        WME_AC_BK, scn->sc_ac2q[WME_AC_BK], WME_AC_BE, scn->sc_ac2q[WME_AC_BE]);
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "AC_VI(#%d)->txq %d, AC_VO(#%d)->txq %d\n",
    ...
    for (i = 0; i < HAL_NUM_TX_QUEUES; i++) {
        ...
        TAILQ_FOREACH(ac, &txq->axq_acq, ac_qelem) {
            ...
            if (tid) {
                ...
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "   ac_q=%d, ac=0x%pK, hw_q=%d, node=%pK, an_flags=0x%x, sched=%d, hwqcnt=%d, ",
                    ac->qnum, ac, i, ni, tid->an->an_flags, ac->sched, ac->hwqcnt);
            } else {
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "   ac_q=%d, ac=0x%pK, hw_q=%d, node=<null>, sched=%d, hwqcnt=%d, ",
                    ac->qnum, ac, i, ac->sched, ac->hwqcnt);
            }
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "filtered=%d, queued tids=%d\n",
                ac->filtered, depth);
            empty = 0;
        }
        ATH_TXQ_UNLOCK(txq);
    ...
    /* TID queues overview */
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "HW TID Queues:\n");
    for (i = 0; i < HAL_NUM_TX_QUEUES; i++) {
        txq = &sc->sc_txq[i];
        if (!txq) { continue; }
        ...
        TAILQ_FOREACH(ac, &txq->axq_acq, ac_qelem) {
            TAILQ_FOREACH_SAFE(tid, &ac->tid_q, tid_qelem, next_tid) {
                ...
                TAILQ_FOREACH(bf, &tid->buf_q, bf_list) {
                    ...
                    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "      bf: node=%x, fc0=0x%x, fc1=0x%x, status=%08x, flags=%08x, nframes=%d, isaggr=%d, ",
                        bf->bf_node, wh->i_fc[0], wh->i_fc[1], bf->bf_status, bf->bf_flags, bf->bf_nframes,
                        (bf->bf_isaggr ? 1 : 0));
                    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "isampdu=%d, data=%d, mc=%d, al=%d, frmlen=%d, retries=%d, seqno=%d\n",
                ...
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "   vap=%d, tidno=%d, tid=0x%pK, node=%pK, an_flags=0x%x, ac_q=%d, ",
