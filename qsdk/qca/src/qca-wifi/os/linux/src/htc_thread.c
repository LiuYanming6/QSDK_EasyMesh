...
static int ath_put_events(struct eventq *event_q, dummy_timer_func_t fn, void *arg)
{
    ...
    if (event_q->count >= MAX_TIME_EVENT_NUM) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: event queue %pK is full\n", __func__,event_q);
...
int ath_put_defer_item(osdev_t osdev, defer_func_t func, int func_id, void *param1, void *param2, void *param3)
{
    ...
    if (event_q->defer_count >= MAX_DEFER_ITEM_NUM) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: work item queue %pK is full\n", __func__,event_q);
        spin_unlock_irqrestore(&event_q->event_lock, flags);
        return 1;
    }
...
void ath_execute_defer_item(defer_t *work)
{
    ...
    if (work->func_id == WORK_ITEM_SET_MULTICAST ) {
        ...
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Unknown Defer Item, enlist yourself\n", __func__);
    }
}
...
static int ath_add_tasklets(osdev_t osdev, htc_tq_struct_t *tasklet, tasklet_callback_t fn, void *ctx)
{
    ...
    if (event_q->tasklet_count >= MAX_TASKLET_EVENT_NUM) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: tasklet queue %pK is full\n", __func__,event_q);
...
static int ath_del_tasklets(osdev_t osdev, htc_tq_struct_t *tasklet)
{
    ...
    for (index = 0; index < MAX_TASKLET_EVENT_NUM; index++) {
        if (event_q->taskletEvents[index].tq == tasklet) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: tasklet[%d] is deleted!\n", __func__, index);
    ...
    if (index == MAX_TASKLET_EVENT_NUM) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: tasklet is not exist!\n", __func__);
    }    
...
int ath_create_htc_thread(osdev_t osdev)
{
    ...
    if(osdev) {
        ...
        if(osdev->event_q == NULL) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s: allocate struct eventq fail\n", __func__);
            return 1;
        }
    ...
    *athHTC_task = kthread_run(ath_htc_thread, (void *)event_q, "athHTCThread");
    ...
    {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO "%s: kthread_run successfully, thread %pK\n", __func__, *athHTC_task);
        unlock_kernel();
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s: kthread_run fail\n", __func__);
    unlock_kernel();
...
void ath_htc_thread_stopping(osdev_t osdev)
{
    ...
    if(event_q == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s: stop htc thread fail\n", __func__);
        return;
    } 
...
void ath_terminate_htc_thread(osdev_t osdev)
{
	...
	if((athHTC_task == NULL) ||
       (event_q == NULL)) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s: terminate htc thread fail\n", __func__);
        return;
    } 
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO "%s: stop the thread\n", __func__);
}
...
int ath_htc_thread(void *data)
{
    ...
    {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s HTC Thread is not terminated by terminate funcion\n", __func__);
        BUG();
    }
...
int ath_put_txbuf(struct sk_buff *skb)
{
    ...
    if (htc_tx_q.htc_txbuf_count >= MAX_TXQ_BUF_NUM) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: txbuf queue is full\n", __func__);
...
static int ath_htc_tx_thread(void *data)
{
    ...
    {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s HTC Thread is not terminated by terminate funcion\n", __func__);
        BUG();
    }
...
int ath_create_htc_tx_thread(struct net_device *netdev)
{
    ...
    {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO "%s: Tx Thread creates successfully\n", __func__);
        unlock_kernel();
        return 0;
    }

    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s: Tx Thread creates fail\n", __func__);
    unlock_kernel();
