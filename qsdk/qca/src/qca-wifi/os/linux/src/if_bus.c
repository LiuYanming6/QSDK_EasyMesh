			...
			size_t size, int direction)
{
    ...
    QDF_BUS_CONTEXT	*bc = &(devhandle->bc);
    unsigned long	addr;

    if (bc->bc_bustype == QDF_BUS_TYPE_SIM) {
        return;
    }

    addr = (unsigned long) __va(dma_handle);
         
    if (!(devhandle->bdev) || bc->bc_bustype == QDF_BUS_TYPE_AHB || bc->bc_bustype == QDF_BUS_TYPE_SNOC) {
...
bus_alloc_consistent(void *hwdev, size_t size, dma_addr_t * dma_handle)
{
    ...
    if (bc->bc_bustype == QDF_BUS_TYPE_SIM || bc->bc_bustype == QDF_BUS_TYPE_SNOC) {
        void *addr = OS_MALLOC(hwdev, size, flags);
        *dma_handle = (uintptr_t)addr;
        return addr;
    }
    
    if(bc->bc_bustype == QDF_BUS_TYPE_AHB)
    {
        return ahb_alloc_consistent(NULL, size, dma_handle);
    }
    else if(bc->bc_bustype == QDF_BUS_TYPE_PCI) 
    {
        struct pci_dev *pdev;
...
bus_free_consistent(void *hwdev, size_t size, void *vaddr, dma_addr_t dma_handle)
{
    ...
    if (!(devhandle->bdev) || bc->bc_bustype == QDF_BUS_TYPE_AHB) {
...
bus_read_cachesize(osdev_t osdev, int *csz, int bustype)
{
    ...
    if (bustype == QDF_BUS_TYPE_SIM || bustype == QDF_BUS_TYPE_SNOC) {
        *csz=0;
        return ;
    }

    if (bustype == QDF_BUS_TYPE_AHB) {
...
{
    QDF_BUS_CONTEXT     *bc = &(osdev->bc);
    int bustype = bc->bc_bustype;

    if (bustype == QDF_BUS_TYPE_PCI) {
      return pci_suspend(osdev);
    } else {
      QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Not supported on this bus\n");
      return 1;
    }
}
