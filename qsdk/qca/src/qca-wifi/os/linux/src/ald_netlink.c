    ...
    if (ald_nl == NULL) {
...
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(3, 6, 0)
        ald_nl->ald_sock =
            (struct sock *)netlink_kernel_create(&init_net, NETLINK_ALD, THIS_MODULE, &cfg);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 24)
        ald_nl->ald_sock = (struct sock *)netlink_kernel_create(&init_net, NETLINK_ALD, 1,
                                                                &ald_nl_receive, NULL, THIS_MODULE);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 22)
        ald_nl->ald_sock = (struct sock *)netlink_kernel_create(NETLINK_ALD, 1, &ald_nl_receive,
                                                                (struct mutex *)NULL, THIS_MODULE);
#else
        ald_nl->ald_sock =
            (struct sock *)netlink_kernel_create(NETLINK_ALD, 1, &ald_nl_receive, THIS_MODULE);
#endif
        ...
        if (ald_nl->ald_sock == NULL) {
            ...
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                           "%s NETLINK_KERNEL_CREATE FAILED\n", __func__);
...
int ald_destroy_netlink(void) {
    ...
    if (!atomic_dec_return(&ald_nl->ald_refcnt)) {
        if (ald_nl->ald_sock) netlink_kernel_release(ald_nl->ald_sock);
...
void ald_notify(wlan_if_t vap, u_int32_t info_cmd, u_int32_t info_len, void *info_data) {
    struct sk_buff *skb;
    struct nlmsghdr *nlh = NULL;
    u_int8_t *nldata = NULL;
    u_int32_t pid;

    if (ald_nl == NULL || vap == NULL) return;
...
#if LINUX_VERSION_CODE == KERNEL_VERSION(3, 12, 59) && defined(QCA_PARTNER_PLATFORM) && \
    defined(USE_LITEPATH)
    netlink_unicast(ald_nl->ald_sock, skb, pid, MSG_DONTWAIT, 0);
#else
...
static void ald_nl_hifitbl_update(wlan_if_t vap, struct nlmsghdr *nlh) {
    ...
    if (nlh->nlmsg_len <= NLMSG_LENGTH(0)) {
        ...
        if (ic->nss_funcs) {
            ...
            ic->nss_funcs->ic_osif_nss_vdev_me_update_hifitlb(vap->iv_txrx_handle,
                                                              &vap->iv_me->me_hifi_table);
        }
#endif
    }
#if ATH_SUPPORT_ME_FW_BASED
    if (vap->iv_ic->ic_is_mode_offload(vap->iv_ic)) vap->iv_ic->ic_hifitbl_update_target(vap);
#endif
...
int ieee80211_ioctl_ald_getStatistics(struct net_device *dev, struct iw_request_info *info, void *w,
                                      char *extra) {
    ...
    IEEE80211_DPRINTF(vap, IEEE80211_MSG_DEBUG, "%s parameter is %s 0x%x get1\n", __func__,
