...
bool osif_nss_ni_stats_update(struct ol_ath_softc_net80211 *scn, struct sk_buff * nbuf, void *rx_mpdu_desc, uint8_t htt_rx_status, struct nss_wifi_append_statsv2_metahdr *metaheader)
{
    ...
    switch (htt_rx_status) {
        case HTT_RX_IND_MPDU_STATUS_OK:
            if (peer) {
                ni->ni_stats.ns_rx_data += msdus;
                ni->ni_stats.ns_rx_bytes += bytes;
                ni->ni_stats.ns_rx_mpdus += mpdus;
                ni->ni_stats.ns_rx_retries += retries;
                ni->ni_stats.ns_rx_ppdus++;
#if UMAC_SUPPORT_STA_STATS_ENHANCEMENT
                if (is_mcast) {
                    ni->ni_stats.ns_rx_mcast += msdus;
                    ni->ni_stats.ns_rx_mcast_bytes += bytes;
                } else {
                    ni->ni_stats.ns_rx_ucast += msdus;
                    ni->ni_stats.ns_rx_ucast_bytes += bytes;
                }
#endif
                /*  Updating vap stats  */
                mac_stats->ims_rx_data_packets += msdus;
                mac_stats->ims_rx_data_bytes += bytes;
                mac_stats->ims_rx_datapyld_bytes += (bytes -
                        ETHERNET_HDR_LEN);

                mac_stats->ims_rx_packets += msdus;

                mac_stats->ims_rx_bytes += bytes;
                scn->scn_stats.rx_packets += msdus;
                scn->scn_stats.rx_num_data += msdus;
                scn->scn_stats.rx_bytes += bytes;
                scn->scn_stats.rx_aggr = pdev->stats.priv.rx.normal.rx_aggr;
            }
            break;
...
/*
 * osif_nss_update_statsv2()
 * Update statsv2 per ppdu
 */
void osif_nss_update_statsv2(struct ol_ath_softc_net80211 *scn, struct sk_buff * nbuf, void *rx_mpdu_desc, uint8_t htt_rx_status)
{
    struct ol_txrx_pdev_t *pdev = scn->pdev_txrx_handle;
    struct htt_pdev_t *htt_pdev = pdev->htt_pdev;
    uint32_t attn = htt_pdev->ar_rx_ops->get_attn_word(rx_mpdu_desc);
    uint8_t is_first_msdu = htt_rx_msdu_first_msdu_flag(htt_pdev, rx_mpdu_desc);
    struct nss_wifi_append_statsv2_metahdr *metaheader;

    if (rx_mpdu_desc == NULL) {
        qdf_print("%pK Rx desc is NULL \n",scn);
        return;
    }

    if (scn->enable_statsv2) {
        if ((attn & RX_FIRST_MPDU_MASK) && is_first_msdu) {
            dma_unmap_single (NULL, virt_to_phys(nbuf->tail), sizeof(struct nss_wifi_append_statsv2_metahdr), DMA_FROM_DEVICE);
            metaheader = (struct nss_wifi_append_statsv2_metahdr *)nbuf->tail;
...
osif_nss_vdev_data_receive(struct net_device *dev, struct sk_buff *skb, __attribute__((unused)) struct napi_struct *napi)
{
    ...
    pdev = vdev->pdev;
    /*
     * avn is just a typecast of vap and hence vap NULL check
     * is enough.
     */
    avn = OL_ATH_VAP_NET80211(vap);
    scn = avn->av_sc;
...
int32_t osif_nss_ol_vap_create(struct ol_txrx_vdev_t * vdev, struct ieee80211vap *vap, osif_dev *os_dev, uint32_t if_num)
{
    ...
    os_dev->nss_ifnum = if_num;
    QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "vap create %pK : if_num %d \n", os_dev, os_dev->nss_ifnum );
...
int32_t osif_nss_ol_vap_delete(osif_dev *osif)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "vap detach %pK: if_num %d ",osif, osif->nss_ifnum);
    /*
...
int32_t osif_nss_ol_vap_xmit(osif_dev *osif, struct sk_buff *skb)
{
    ...
    nss_ifnum = osif->nss_ifnum;
    if (nss_ifnum == -1) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " vap transmit called on invalid interface %pK \n",osif);
        return -1;
    }
    qdf_nbuf_count_dec(skb);
...
void osif_nss_vdev_process_mpsta_rx_to_tx(struct net_device *netdev, struct sk_buff *skb, __attribute__((unused)) struct napi_struct *napi)
{
    ...
    if (unlikely(skb_headroom(skb) < sizeof(struct nss_wifi_vdev_mpsta_per_packet_tx_metadata))){
        if(pskb_expand_head(skb, sizeof(struct nss_wifi_vdev_mpsta_per_packet_tx_metadata), 0, GFP_ATOMIC)){
            QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Unable to allocated skb data%pK head%pK tail%pK end %pK\n", skb->data, skb->head, skb->tail, skb->end);
            qdf_nbuf_free(skb);
            return ;
        }
    }
...
void osif_nss_vdev_process_extap_tx(struct net_device *netdev, struct sk_buff *skb)
{
    ...
    if (unlikely(skb_headroom(skb) < sizeof(struct nss_wifi_vdev_extap_per_packet_metadata))){
        if(pskb_expand_head(skb, sizeof(struct nss_wifi_vdev_extap_per_packet_metadata), 0, GFP_ATOMIC)){
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Unable to allocated skb data%pK head%pK tail%pK end %pK\n", skb->data, skb->head, skb->tail, skb->end);
            qdf_nbuf_free(skb);
            return ;
        }
    }
