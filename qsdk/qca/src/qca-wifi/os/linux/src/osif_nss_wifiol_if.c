...
extern int ol_ath_mgmt_null_handler(ol_scn_t scn, wbuf_t wbuf, ol_ath_ieee80211_rx_status_t *rx_status);
...
int osif_nss_ol_process_bs_peer_state_msg(struct ol_ath_softc_net80211 *scn,  struct nss_wifi_bs_peer_activity *msg)
{
    ...
    for (i = 0 ; i < msg->nentries ; i++) {
        ...
        if (peer) {
            ol_txrx_mark_peer_inact(peer, false); //TODO : for successful tx too
...
int osif_nss_ol_process_ol_timestats_msg(struct ol_ath_softc_net80211 *scn,  struct nss_wifi_ol_peer_time_msg *msg)
{
    ...
    for (pindex = 0 ; pindex < msg->npeers ; pindex++) {
        peer = ol_txrx_peer_find_by_id(pdev, msg->tstats[pindex].peer_id);
        if (peer) {
    ...
    for (i = 0 ; i < msg->npeers ; i++) {
        ...
        if (peer) {
            ...
            if (vap) {
                ...
                mac_stats->ims_tx_bcast_data_packets += bcast_cnt;
...
void osif_nss_ol_event_sync(struct ol_ath_softc_net80211 *scn, int response)
{
    scn->osif_nss_ol_wifi_cfg_complete.response = response;
...
void osif_nss_ol_event_receive(struct ol_ath_softc_net80211 *scn, struct nss_cmn_msg *wifimsg)
{
    ...
    switch (msg_type) {
        case NSS_WIFI_POST_RECV_MSG:
            {
                if (response == NSS_CMN_RESPONSE_EMSG) {
                    ...
                    osif_nss_ol_event_sync(scn, NSS_TX_FAILURE);
                    return;
                }
                ...
                osif_nss_ol_event_sync(scn, NSS_TX_SUCCESS);
            }
            break;
        ...
        case NSS_WIFI_ENABLE_OL_STATSV2_MSG:
            if (response == NSS_CMN_RESPONSE_EMSG) {
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "NSS send to host configure msg failed for NSS radio %d\n", scn->nss_wifiol_id);
            }
            break;
...
static void osif_nss_ol_wifi_ce_callback_func(struct ol_ath_softc_net80211 *scn, struct sk_buff * nbuf, __attribute__((unused)) struct napi_struct *napi) {
    ...
    if (scn == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: scn is NULL free the nbuf: %pK\n", __func__, nbuf);
        goto fail;
    }
    ...
    if (htt_pdev == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: htt_pdev is NULL free the nbuf: %pK for radio ID: %d\n", __func__, nbuf, scn->radio_id);
        goto fail;
    }
...
static void osif_nss_ol_wifi_ext_callback_func(struct ol_ath_softc_net80211 *scn, struct sk_buff * nbuf,  __attribute__((unused)) struct napi_struct *napi)
{
    ...
    switch (type) {
        case NSS_WIFI_RX_EXT_PKTLOG_TYPE:
            {
                ...
                if ((peer_id > OL_TXRX_MAX_PEER_IDS) && (peer_id != HTT_INVALID_PEER)) {
                    qdf_print("%s: Invalid Peer Id peer_id=%d \n",__func__, peer_id);
                    break;
                }
                peer = ol_txrx_peer_find_by_id(pdev, peer_id);
        ...
        case NSS_WIFI_RX_STATS_V2_EXCEPTION:
            {
                status = wrem->htt_rx_status;
                osif_nss_update_statsv2(scn, nbuf, rx_mpdu_desc, status);
            }
            break;

        case NSS_WIFI_RX_MGMT_NULL_TYPE:
            {
                /* do not send fcs errored null data frame to mgmt_handler */
                if (!(htt_pdev->ar_rx_ops->is_fcs_err(rx_mpdu_desc))) {
                    ol_ath_ieee80211_rx_status_t rx_status;
                    memset(&rx_status, 0, sizeof(rx_status));
                    memset(nbuf->cb, 0, sizeof(nbuf->cb));
                    nbuf->protocol = ETH_P_CONTROL;
                    ol_ath_mgmt_null_handler(scn, nbuf, &rx_status);
                    discard = 0;
                }
...
int osif_nss_ol_wds_extn_peer_cfg_send(struct ol_txrx_pdev_t *pdev, struct ol_txrx_peer_t *peer)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s %pK %d Peer CFG ol stats enable \n",__func__, nss_wifiol_ctx, radio_id);
    ...
    status = nss_wifi_tx_msg(nss_wifiol_ctx, &wifimsg);
    if (status != NSS_TX_SUCCESS) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "WIFI WDS Vendor Ext  message send failed  %d \n", status);
        return -1;
    }
...
void osif_nss_gre_rx_stats_callback(struct net_device *netdev, struct sk_buff *skb)
{
    ...
    rx_desc_size = scn->htt_pdev->ar_rx_ops->sizeof_rx_desc();

    if (scn->enable_statsv2) {
        dma_unmap_single (NULL, virt_to_phys(rx_mpdu_desc), rx_desc_size, DMA_FROM_DEVICE);
        osif_nss_update_statsv2(scn, skb, rx_mpdu_desc, HTT_RX_IND_MPDU_STATUS_OK);
    }
}
 ...
 */
void osif_nss_ol_enable_ol_statsv2(struct ol_ath_softc_net80211 *scn, uint32_t value)
{
    int ifnum=-1;
    struct nss_wifi_msg wifimsg;
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s %pK %pK %d stats V2 enabled %d\n",__func__, nss_wifiol_ctx, scn, radio_id, value);
...
int osif_nss_ol_wifi_monitor_set_filter(struct ieee80211com* ic, uint32_t filter_type)
{
    ...
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s %pK %pK %d %d \n",__func__,nss_wifiol_ctx, scn, radio_id, filter_type);
...
int osif_nss_ol_stats_cfg(struct ol_ath_softc_net80211 *scn, uint32_t cfg)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s %pK %pK %d ol stats enable %d\n",__func__, nss_wifiol_ctx, scn, radio_id, cfg);
    ...
    status = nss_wifi_tx_msg(nss_wifiol_ctx, &wifimsg);
    if (status != NSS_TX_SUCCESS) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "WIFI OL Stats Config message send failed  %d \n", status);
        return -1;
    }
...
int osif_nss_ol_pktlog_cfg(struct ol_ath_softc_net80211 *scn, int enable)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s %pK %pK %d PKT log enable %d buf 64 \n",__func__,nss_wifiol_ctx, scn, radio_id, enable);
    ...
    status = nss_wifi_tx_msg(nss_wifiol_ctx, &wifimsg);
    if (status != NSS_TX_SUCCESS) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "NSS_WIFI_PKTLOG_CFG message send failed  %d \n", status);
        return -1;
    }
    return 0;
}
...
int osif_nss_ol_wifi_pause(struct ol_ath_softc_net80211 *scn)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s %pK %pK %d \n",__func__,nss_wifiol_ctx, scn, radio_id);
...
int osif_nss_ol_wifi_tx_capture_set(struct ol_txrx_pdev_t *pdev, uint8_t tx_capture_enable)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s %pK %d %d \n",__func__,nss_wifiol_ctx,radio_id,tx_capture_enable);
...
#define CE_HTT_TX_CE            4
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "NSS %s :sc %pK NSS ID %d  Taret type %x \n",__FUNCTION__, osc, scn->nss_wifiol_id, scn->target_type);
    ...
    nss_wifiol_ctx   = nss_register_wifi_if(ifnum,
     ...
     scn->nss_wifiol_ctx= nss_wifiol_ctx;
     QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "NSS-WifiOffload: nss_wifiol_ctx %pK \n",nss_wifiol_ctx);
    ...
    ret = wait_for_completion_timeout(&scn->osif_nss_ol_wifi_cfg_complete.complete, msecs_to_jiffies(OSIF_NSS_WIFI_CFG_TIMEOUT_MS));
    if (ret == 0) {
        printk("Waiting for NSS WIFI INIT MSG ack timed out\n");
        return -1;
    }
     ...
     */
    if (NSS_TX_FAILURE == scn->osif_nss_ol_wifi_cfg_complete.response) {
        printk("nack for NSS WIFI INIT MSG\n");
        return -1;
    }
...
void osif_nss_ol_post_recv_buffer(struct ol_ath_softc_net80211 *scn)
{
    ...
    ret = wait_for_completion_timeout(&scn->osif_nss_ol_wifi_cfg_complete.complete, msecs_to_jiffies(OSIF_NSS_WIFI_CFG_TIMEOUT_MS));
    if (ret == 0) {
        printk("Waiting for NSS WIFI POST RECV MSG ack timed out\n");
        return;
    }
     ...
     */
    if (NSS_TX_FAILURE == scn->osif_nss_ol_wifi_cfg_complete.response) {
        printk("nack for NSS WIFI POST RECV MSG\n");
        return;
    }
        ...
        uint32_t pmap_addr)
{
    ...
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s :NSS %pK NSS ID %d  target type  \n",__FUNCTION__, nss_wifiol_ctx, radio_id);
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s :NSS %pK NSS ID %d \n",__FUNCTION__, nss_wifiol_ctx, radio_id);
    ...
    ret = wait_for_completion_timeout(&scn->osif_nss_ol_wifi_cfg_complete.complete, msecs_to_jiffies(OSIF_NSS_WIFI_CFG_TIMEOUT_MS));
    if (ret == 0) {
        printk("Waiting for NSS WIFI TX INIT MSG ack timed out\n");
        return -1;
    }
     ...
     */
    if (NSS_TX_FAILURE == scn->osif_nss_ol_wifi_cfg_complete.response) {
        printk("nack for NSS WIFI TX INIT MSG\n");
        return -1;
    }
...
int osif_ol_nss_htc_send(struct ol_txrx_pdev_t *pdev, void *nss_wifiol_ctx, int nssid, qdf_nbuf_t netbuf, uint32_t transfer_id)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s :NSS %pK NSS ID %d \n",__FUNCTION__, nss_wifiol_ctx, nssid);
