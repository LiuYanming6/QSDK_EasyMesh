                                    ...
                                    ath_lro_hw_info_t *lro_info)
{
    ...
    *tcp_header = (struct tcphdr *)(l3_start + lro_info->tcp_udp_offset);
}
                               ...
                               ath_lro_hw_info_t *lro_info)
{
    ...
    le->mss = tcp_data_len;
    le->active = 1;
    le->tcp_payload_ck_sum = lro_info->tcp_udp_cksum;
    le->dev = nbuf->dev;
                               ...
                               ath_lro_hw_info_t *lro_info)
{
    ...
    ath_lro_add_common(le, ip_header, tcp_header, tcp_data_len, lro_info);
                                  ...
                                  ath_lro_hw_info_t *lro_info)
{
    ...
    if (le->tcp_header->source == tcp_header->source ||
        ...
        if (le->ipv6_proto && lro_info->ipv6_proto) {
        ...
        else if(!le->ipv6_proto && !lro_info->ipv6_proto) {
                                        ...
                                        ath_lro_hw_info_t *lro_info)
{
    ...
    for (i = 0; i < LRO_MAX_ENTRY; i++) {
        ...
        if (tmp_le->active) {
            if (!ath_lro_check_tcp_conn(tmp_le, ip_header, tcp_header, lro_info)) {
                 le = tmp_le;
                 return (le);
            }
        }
...
unsigned int ath_lro_process_nbuf(qdf_nbuf_t nbuf, void *priv)
{
    ...
    lro_info = NULL;
    lro_info = (ath_lro_hw_info_t *)qdf_nbuf_get_cb(nbuf);

    /* if packet is not a TCP packet,
     * hand it over to stack immediately
     */
    if(!lro_info->tcp_proto) {
        return (ATH_TCP_LRO_NOT_SUPPORTED);
    }
    ath_get_nbuf_hdr(nbuf, (void *)&ip_header, (void *)&tcp_header, lro_info);

    le = ath_retrieve_le(os_if->ath_le_arr, ip_header, tcp_header, lro_info);
    if (!le) {
        return (ATH_TCP_LRO_FAILURE);
    }
    if (!le->active) {
        if (ath_lro_elegibility_check((void *)ip_header, tcp_header, NULL, lro_info)) {
            return (ATH_TCP_LRO_FAILURE);
        }
        ath_lro_init_entry(le, nbuf, ip_header, tcp_header, lro_info);
    ...
    if (ath_lro_elegibility_check(ip_header, tcp_header, le, lro_info)) {
        ath_lro_flush(le);
        return (ATH_TCP_LRO_FAILURE);
    }

    ath_lro_add_packet(le, nbuf, ip_header, tcp_header, lro_info);
    os_if->aggregated++;
