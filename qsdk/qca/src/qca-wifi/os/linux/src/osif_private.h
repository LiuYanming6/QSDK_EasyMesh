...
void osif_update_info_to_app(struct ieee80211com *ic, char *buf);
...
#define OSIF_PERIODICSCAN_COEXT_PERIOD      (180000)
    ...
    void (*tx_dev_lock_acquire)(void *); /* API to acquire tx data path lock - currently used by OL only */
    void (*tx_dev_lock_release)(void *); /* API to release tx data path lock - currently used by OL only */
#ifdef ATH_SUPPORT_LINUX_VENDOR
#define OSIF_VENDOR_RAWDATA_SIZE        (24)
    ...
    ol_txrx_classify_fp iv_vap_classify;
...
#endif
    u_int32_t    os_supplicant_state_period;
    os_timer_t    os_supplicant_state_timer;
...
#define OSIF_SUPPLICANT_STATE_TIMEOUT 1000  /*ms*/
