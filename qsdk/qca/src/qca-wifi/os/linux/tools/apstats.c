...
static int nodestats_hierarchy_init(vaplevel_stats_t *vapstats, int sockfd)
{
    ...
    if (remaining_len >= sizeof(struct ieee80211req_sta_info)) {
        ...
        do {
            ...
            /* As an optimization, copy ieee80211req_sta_info right-away. */
                                 ...
                                 apstats_recursion_t recursion)
{
    ...
    vapstats->mgmt_tx_fail = stats->mgmt_tx_fail_cnt;
...
#if MAC_SUPPORT_CHANUTIL_MEASUREMENT
                                  ...
                                  apstats_recursion_t recursion)
{
    ...
    APSTATS_PRINT_GENERAL("\n");
    APSTATS_PRINT_STAT64("Mgmt Tx Fail", vapstats->mgmt_tx_fail);
    APSTATS_PRINT_GENERAL("\n");
                                  ...
                                  nodelevel_stats_t *nodestats)
{
    ...
    /* Is the sta connected to DA or OFFLOAD interface */
    ...
    req.data.tr181_req.cmdid = TR181_GET_LAST_ACK_RSSI;
    req.data.tr181_req.data_addr = &stats;
    req.data.tr181_req.data_size = sizeof(struct ieee80211req_sta_stats);
    iwr.u.data.pointer = (void *) &req;
    iwr.u.data.length = (sizeof(struct ieee80211req_athdbg));
    if (ioctl(sockfd, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
        perror("tr181 get last ack rssi  failed \n");
        close(sockfd);
        return -1;
    }

    req.cmd = IEEE80211_DBGREQ_TR181;
    req.data.tr181_req.cmdid = TR181_GET_LAST_BEACON_RSSI;
    req.data.tr181_req.data_addr = &stats;
    req.data.tr181_req.data_size = sizeof(struct ieee80211req_sta_stats);
    iwr.u.data.pointer = (void *) &req;
    iwr.u.data.length = (sizeof(struct ieee80211req_athdbg));
    if (ioctl(sockfd, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
        perror("tr181 get bcn ack rssi  failed \n");
        close(sockfd);
        return -1;
    }


    memcpy(&nodestats->rssi_bcn, &ns->rssi_bcn, sizeof(rssi_snapshot_t));
    memcpy(&nodestats->rssi_ack, &ns->rssi_ack, sizeof(rssi_snapshot_t));
...
static int nodelevel_display_stats(nodelevel_stats_t *nodestats)
{
    ...
    APSTATS_PRINT_STAT64("Avg ppdu Tx rate(kbps)",nodestats->ppdu_tx_rate);
    APSTATS_PRINT_STAT64("Avg ppdu Rx rate(kbps)",nodestats->ppdu_rx_rate);
    }
#if UMAC_SUPPORT_TXDATAPATH_NODESTATS
    ...
    APSTATS_PRINT_STAT32("Seq no of last rx ack", nodestats->rssi_ack.count);
    APSTATS_PRINT_STAT32("Seq no of last rx bcn", nodestats->rssi_bcn.count);

    APSTATS_PRINT_GENERAL("Last 4 ACK RSSI: %d %d %d %d\n", nodestats->rssi_ack.rssi[0],
                                                            nodestats->rssi_ack.rssi[1],
                                                            nodestats->rssi_ack.rssi[2],
                                                            nodestats->rssi_ack.rssi[3]);

    APSTATS_PRINT_GENERAL("Timestamps of Last 4 ACK RSSI: %d %d %d %d\n", nodestats->rssi_ack.time_s[0],
                                                            nodestats->rssi_ack.time_s[1],
                                                            nodestats->rssi_ack.time_s[2],
                                                            nodestats->rssi_ack.time_s[3]);

    APSTATS_PRINT_GENERAL("Last 4 BCN RSSI: %d %d %d %d\n", nodestats->rssi_bcn.rssi[0],
                                                            nodestats->rssi_bcn.rssi[1],
                                                            nodestats->rssi_bcn.rssi[2],
                                                            nodestats->rssi_bcn.rssi[3]);

    APSTATS_PRINT_GENERAL("Timestamps of Last 4 BCN RSSI: %d %d %d %d\n", nodestats->rssi_ack.time_s[0],
                                                            nodestats->rssi_ack.time_s[1],
                                                            nodestats->rssi_ack.time_s[2],
                                                            nodestats->rssi_ack.time_s[3]);
...
void nodelevel_display_extended_stats(char *mac, char *interface)
{
    struct iwreq iwr = {0};
    struct wlanstats_uapi_ioctl iocmd = {0};

    APSTATS_PRINT_GENERAL("\n-----------------extended stats---------------------- \n");
    /* Display extended peer tx stats */
    wlanstats_peer_tx_stats_prepare(&iocmd, mac);
    if (wlanstats_send_ioctl_command(interface, &iwr, &iocmd) < 0)
        return;
    wlanstats_peer_tx_stats_handle(&iocmd);

    APSTATS_PRINT_GENERAL("--------------------------------------- \n");
    /* Display extended peer rx stats */
    wlanstats_peer_rx_stats_prepare(&iocmd, mac);
    if (wlanstats_send_ioctl_command(interface, &iwr, &iocmd) < 0)
        return;
    wlanstats_peer_rx_stats_handle(&iocmd);
                             ...
                             nodelevel_stats_t *nodestats)
{
    ...
    nodelevel_display_extended_stats(nodestats->macaddr.ether_addr_octet,
            (((radiolevel_stats_t *)(((vaplevel_stats_t*)nodestats->parent)->parent))->ifname));
