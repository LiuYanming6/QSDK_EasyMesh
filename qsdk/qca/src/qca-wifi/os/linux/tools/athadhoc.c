...
int process_events()
{
    ...
    while (1) {
        ...
        if (recv_len > 0) {
            memcpy(&cmd,&buffer[50],2);
            if(cmd == IWEVCUSTOM) {
                memcpy(&subtype,&buffer[54],2);

                printf("Received buffer: \n");
                switch(subtype) {
                    case IEEE80211_EV_RECV_PROBEREQ:
                        probe_ev = (struct ev_recv_probereq *) &buffer[56];
                        printf("Probe Event: \n");
                    ...
                    case IEEE80211_EV_STA_AUTHORIZED:
                        authorize_ev = (struct ev_node_authorized *) &buffer[56];
                    ...
                    case IEEE80211_EV_STA_LEAVE:
                        staleave_ev = (struct ev_sta_leave *) &buffer[56];
                        printf("Leve Event \n");
