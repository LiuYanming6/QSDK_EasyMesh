...
#define ATH_CREATE_DELAYED_WORK(a,b,c)   qdf_create_delayed_work(0, a, b, c)
#define ATH_QUEUE_WORK(a,b)      qdf_queue_work(0, a, b)
#define ATH_QUEUE_DELAYED_WORK(a,b,c)   qdf_queue_delayed_work(0, a, b, c)
...
#define ATH_INIT_TQUEUE(a,b,c)      qdf_create_bh(0, a, b, c)
#define ATH_SCHEDULE_TQUEUE(a,b)    qdf_sched_bh(0, a)
...
{\
netif_trans_update(((__sc)->sc_osdev->netdev)); \
}
#else
#define OS_NETDEV_UPDATE_TRANS(__sc)\
{\
...
#define OS_SYNC_CANCEL_TIMER(_timer)   qdf_timer_sync_cancel(_timer)
...
netlink_unicast((struct sock*) ssk, skb, pid,nonblock, 0);
#else
