...
void wlan_mlme_remove_ie_list(wlan_mlme_app_ie_t app_ie_handle)
{
    ...
    if (app_ie_handle == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: handle is NULL. Do nothing.\n", __func__);
        return;
    }
...
int wlan_mlme_app_ie_delete(wlan_mlme_app_ie_t app_ie_handle, ieee80211_frame_type ftype, u_int8_t *app_ie)
{
    ...
    if(app_ie == NULL)
    {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: appie is NULL. Do nothing.\n", __func__);
        return -EINVAL;
    }
...
static void ieee80211_check_duplicate_ie_delete(struct ieee80211vap *vap, ieee80211_frame_type ftype, u_int8_t *new_app_ie, u_int32_t new_app_ie_length)
{
    struct app_ie_entry  *ie_entry,*next_entry;
    LIST_FOREACH_SAFE(ie_entry, &vap->iv_app_ie_list[ftype], link_entry,next_entry)
    {
    ...
    u_int8_t                identifier)
{
    ...
    {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "NULL IE or zero length received");
        error = -EINVAL;
        return error;
    }
    if (ftype >= IEEE80211_FRAME_TYPE_MAX)
    {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Invalid frame type");
        error = -EINVAL;
        return error;
    }
    IEEE80211_VAP_LOCK(vap);
    ...
    if(ie_entry == NULL)
    {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Not able to allocate memory");
        error = -ENOMEM;
        goto exit;
    }
...
int wlan_mlme_app_ie_set_check(struct ieee80211vap *vap, ieee80211_frame_type ftype, u_int8_t *app_ie, u_int32_t app_ie_length, u_int8_t identifier)
{
    int i = 0;
    int j=0;
    int block_length = 0;
    ...
    if (vap->vie_handle == NULL)
    {
        ...
        if (vie_handle == NULL)
        {
            QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "not able to get vie_handle");
            error = -ENOMEM;
            return error;
        }
    }
    ...
    LIST_FOREACH_SAFE(ie_entry, &vap->iv_app_ie_list[ftype], link_entry,next_entry)
    {
        if(list_all || ie_entry->app_ie.ie[0] == temp_buf[0])
        {
            if(list_all || temp_buf[0] != IEEE80211_ELEMID_VENDOR || !(OS_MEMCMP(&ie_entry->app_ie.ie[2],&temp_buf[2],3)))
            {
                ...
                {
                    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "buffer length exceeded");
                    error = -EINVAL;
                    goto out;
                }
                buf[(*ielen)] = ftype;
