                        ...
                        struct ieee80211_rx_status *rs)
{
    ...
    if ((ni == vap->iv_bss) && ( ni_temp = ieee80211_find_node(&vap->iv_ic->ic_sta, wh->i_addr2))){

        ni_temp->ni_stats.ns_last_rx_mgmt_rate = rs->rs_datarate;
        ni_temp->ni_stats.ns_rx_mgmt_rssi = rs->rs_rssi;
    ...
    if (snd_prb_resp) {
        ...
        if ((extractx.fils_sta || extractx.oce_sta) && channel_time && vap->iv_bpr_enable &&
            ...
            /* If channel time is bigger than beancon interval, slightly discard the probe-req as beacon will be sent instead */
        ...
        } else if ((extractx.fils_sta || extractx.oce_sta) && channel_time &&
