...
ieee80211_flag_beacon_sent(struct ieee80211vap *vap) {
    ...
    if (qdf_ktime_to_ns(qdf_ktime_add(qdf_hrtimer_get_remaining(bpr_hrtimer), qdf_ktime_get())) >
        ...
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,
            ...
            __func__, __LINE__, vap->iv_bpr_delay, qdf_ktime_to_ns(vap->iv_next_beacon_tstamp), \
            ic->ic_intval, qdf_ktime_to_ns(qdf_ktime_add(qdf_ktime_get(), qdf_hrtimer_get_remaining(bpr_hrtimer))), \
            qdf_hrtimer_callback_running(bpr_hrtimer), qdf_hrtimer_is_queued(bpr_hrtimer));
                      ...
                      u_int8_t *frm)
{
    ...
    *frm++ = IEEE80211_ELEMID_SSID;
    ...
    if (IEEE80211_VAP_IS_WDS_ENABLED(vap) && !ieee80211_vap_map_is_set(vap))
    {
...
ieee80211_csa_interop_phy_update(struct ieee80211_node *ni, int rx_bw)
{
    ...
    switch (rx_bw) {
        ...
        case IEEE80211_CWM_WIDTH80:
            ...
            qdf_print("%s: [%s, %pM] upgrading CW %d -> %d (chan_bw=%d)\n",
                   __func__,
                   vap->iv_netdev_name,
                   ni->ni_macaddr,
                   ni->ni_chwidth,
            ...
            break;
        case -1:
            qdf_print("%s: [%s, %pM] downgrading CW %d -> %d\n",
                   __func__,
                   vap->iv_netdev_name,
                   ni->ni_macaddr,
                   ni->ni_chwidth,
            ...
            break;
        default:
            qdf_print("%s: [%s, %pM] unsupported CW %d -> %d, ignoring\n",
                   __func__,
                   vap->iv_netdev_name,
                   ni->ni_macaddr,
                   ni->ni_chwidth,
...
void
free_ni_list(void *arg, struct ieee80211_node *ni)
{
...
#endif
{
    ...
    static int pmf_disassoc_timeout = 3; /* 3 seconds */
#define DELAY_BEACON_TIME (pmf_disassoc_timeout*NUM_MILLISEC_PER_SEC)
#ifdef ATH_SUPPORT_RDKB
    int interop_bss_desired;
#endif

    if (ieee80211_vap_delayed_beacon_is_set(vap))
    {
        now = qdf_system_ticks_to_msecs(qdf_system_ticks());
        desired_time = vap->iv_last_vap_up_time + DELAY_BEACON_TIME;
        if((qdf_system_time_after_eq(now, desired_time)) ||
                (ic->ic_prevchan->ic_ieee !=ic->ic_curchan->ic_ieee)) {
            ieee80211_vap_ready_set(vap);
            ieee80211_vap_delayed_beacon_clear(vap);
            ieee80211_vap_pmf_mgmt_xtry_clear(vap);
        } else {
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,
                    "Not queing beacon as time has not expired "
                    " Now =%lu sec,desired_time=%lu sec\n",
                    (now/1000),(desired_time/1000) );
...
#endif
#if DK_DEBUG_OCE
    /* [FIXME] Consider using ktime_to_us() instead to increase accuracy */
#endif
    ...
    */
    ...
    if ((vap->iv_flags & IEEE80211_F_CHANSWITCH) && (vap->iv_chanchange_count == ic->ic_chanchange_tbtt) &&
        ...
        /* If csa_deauth_mode is set to 1, send deauth to all ni's as unicast */
        if (vap->iv_csa_deauth_mode == 1) {
            wlan_iterate_station_list(vap, sta_deauth, NULL);
        } else if (vap->iv_csa_deauth_mode == 2) { /* if set to 2, send deauth as broadcast */
            ieee80211_send_deauth(vap->iv_bss, IEEE80211_REASON_AUTH_LEAVE);
            /* Free all the ni's after deauth */
            wlan_iterate_station_list(vap, free_ni_list, NULL);
        }
        ...
        if (ic->ic_curchan != c) {
            ...
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,
                    "%s: Prev Chan=%u New Chan=%u mainsta=%pK enh_ind=%u\n",
    ...
    if (vap->iv_opmode == IEEE80211_M_HOSTAP || vap->iv_opmode == IEEE80211_M_IBSS) {
        ...
        if (ieee80211_ic_doth_is_set(ic) && (ic->ic_flags & IEEE80211_F_CHANSWITCH) && (ic->ic_chanchange_channel) &&
            ...
            if (!vap->iv_chanchange_count) {
                ...
                if (bo->bo_ext_bssload)
                    bo->bo_ext_bssload += csa_ecsa_len;
                if(bo->bo_vhttxpwr != NULL)
                ...
                if (bo->bo_ap_chan_rpt)
                    bo->bo_ap_chan_rpt += csa_ecsa_len;
                if (bo->bo_rnr)
                    bo->bo_rnr += csa_ecsa_len;
                if (bo->bo_mbo_cap)
