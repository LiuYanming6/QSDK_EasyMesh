...
DEFINE_RATELIMIT_STATE(assoc_deny_ratelimit,
        ASSOC_DENY_PRINT_RATE_LIMIT_PERIOD,
        ASSOC_DENY_PRINT_RATE_LIMIT_BURST_DEFAULT);

int print_assoc_deny_ratelimit(void)
{
    return __ratelimit(&assoc_deny_ratelimit);
}
    ...
    if (ni &&(vap->iv_opmode == IEEE80211_M_HOSTAP) &&
        ...
        if(!ni->is_sta_node) {
            ...
            if(wlan_node_get_max_nss(ni) > 1)
                ath_green_ap_state_mc(ic, ATH_PS_EVENT_INC_MULTISTREAM_STA);
            ni->is_sta_node = 1;
        }
...
int ieee80211_beacon_intersect(struct ieee80211_node *ni, u_int8_t *bcn_frm_body, u_int16_t bcn_body_len, struct ieee80211_frame *wh)
{
    ...
    u_int8_t *athextcap, *opmode, *xcaps, *bwnss_oui, *supp_chan, *supp_op_cl = NULL;
    IEEE80211_COUNTRY_ENTRY country;
...
ieee80211_fils_discovery_init(struct ieee80211_node *ni, u_int8_t *frm)
{
    ...
    if (ieee80211_vap_oce_check(vap) && ieee80211_non_oce_ap_present (vap)) {
        fd_cntl_subfield |= IEEE80211_FD_FRAMECNTL_NON_OCE_AP_PRESENT;
    }
    ...
    if(vap->iv_cur_mode == IEEE80211_MODE_11AC_VHT80_80)    {
        fd_cntl_subfield |= IEEE80211_FD_FRAMECNTL_CH_CENTERFREQ;
        fd_cntl_subfield |= IEEE80211_FD_FRAMECNTL_PRIMARY_CH;
    }
    ...
    if (IEEE80211_FD_IS_FRAMECNTL_PRIMARY_CH(fd_cntl_subfield))
    {
        op_class = regdmn_get_opclass (ic->ic_country_iso, ic->ic_curchan);
        prim_chan = ic->ic_curchan->ic_ieee;
        *frm = op_class;
        frm++;
        *frm = prim_chan;
        frm++;
    ...
    if(IEEE80211_FD_IS_FRAMECNTL_CH_CENTERFREQ(fd_cntl_subfield))
    {
        /* spec has seg0 and seg1 naming while we use seg1 and seg2 */
        ch_seg1 = ic->ic_curchan->ic_vhtop_ch_freq_seg2;
        *frm = ch_seg1;
        frm++;
    }
    /* Mobility Domain */
