...
static OS_TIMER_FUNC(ieee80211_synced_channel_switch)
{
    ...
    if (vap->iv_cswitch_rxd && !ieee80211_num_apvap_running(ic)) {
        ...
        if (!IEEE80211_IS_CHAN_SWITCH_STARTED(ic) && (chan != NULL)) {
            ...
            if (err == EOK) {
#if UMAC_SUPPORT_CBS
                if ((IEEE80211_IS_CHAN_DFS(chan) && (!ic->ic_dfs_state.ignore_dfs))) {
                   QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " CSA: Triggering RADAR detect and DEAUTH event\n");
#else
                           ...
                           struct ieee80211_extendedchannelswitch_ie *echanie)
{
        ...
        if((vap->iv_opmode == IEEE80211_M_STA)
                ...
                /* Update node channel setting */
                ieee80211_node_set_chan(ni);
                if ((ic->ic_opmode == IEEE80211_M_HOSTAP) && (ieee80211_num_apvap_running(ic)))
                        ic->ic_flags |= IEEE80211_F_CHANSWITCH;
        }
    ...
    if ((ni == vap->iv_bss) &&
        ...
        if(se_sonadv != NULL) {
            ...
            if(vap->iv_whc_root_ap_distance > 0) {
                if(ic->ic_serving_ap_backhaul_rate != se_apinfo_uplink_rate) {
                    /* Get the serving AP backhaul rate from scan entry */
                    ic->ic_serving_ap_backhaul_rate = se_apinfo_uplink_rate;
                    whc_apinfo_ie_update = true;
                }
                if(vap->iv_whc_root_ap_distance != se_sonadv->whc_apinfo_root_ap_dist + 1) {
                    if(se_sonadv->whc_apinfo_root_ap_dist != WHC_INVALID_ROOT_AP_DISTANCE) {
                        vap->iv_whc_root_ap_distance = se_sonadv->whc_apinfo_root_ap_dist + 1 ;
                    }
                    else {
        ...
        */
        if (((chanie != NULL) || (echanie != NULL)) && chan && (chan != ic->ic_curchan)) {
                ieee80211_process_csa(ni, chan, chanie, echanie);
        }
    ...
    )
{
    ...
    *(u_int16_t *)frm = htole16(ic->ic_lintval);
    ...
    if(ieee80211_vap_map_is_set(vap)){
        ...
        frm = ieee80211_add_multi_ap_ie(frm,vap,&ie_len);
    }
