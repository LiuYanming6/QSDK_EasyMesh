...
sta_deauth(void *arg, struct ieee80211_node *ni)
{
    ...
    if(ieee80211_is_pmf_enabled(vap, ni)) {
        ...
        IEEE80211_DELIVER_EVENT_MLME_DEAUTH_INDICATION(vap,
                macaddr, associd, IEEE80211_REASON_AUTH_LEAVE);
    }
}
...
void
sta_node_leave(void *arg, struct ieee80211_node *ni)
{
    struct ieee80211vap    *vap = ni->ni_vap;
    u_int8_t macaddr[6];
...
#define MAX_DISASSOC_WAIT_COUNT 11
    ...
    if (flags & WLAN_MLME_STOP_BSS_F_FORCE_STOP_RESET) {
        ...
        error = ieee80211_vap_stop(vap, TRUE);
    } else {
        ...
        {
            if(wlan_vap_is_pmf_enabled(vap) &&
                    (vap->iv_opmode==IEEE80211_M_HOSTAP) && vap->iv_sta_assoc > 0) {
                u_int8_t count=0;
                IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,"%s..PMF enabled vap"
                        " No of associated stas=%d\n",__func__,vap->iv_sta_assoc);
                while(1)
                {
                    if(vap->iv_sta_assoc > 0 && count < MAX_DISASSOC_WAIT_COUNT
                            && !(in_irq() || in_softirq())) {
                        schedule_timeout_interruptible(OSIF_DISCONNECT_TIMEOUT);
                        count++;
                    } else {
                            IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,
                                    "%s No of associated stas=%d, count=%d\n",
                                    __func__,vap->iv_sta_assoc,count);
                            if(ieee80211_vap_pmf_mgmt_xtry_is_set(vap)) {
                                ieee80211_vap_delayed_beacon_set(vap);
                                vap->iv_last_vap_up_time =
                                    qdf_system_ticks_to_msecs(qdf_system_ticks());
                                IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,
                                        "%s..Setting Delayed beacon"
                                        " Value of now: %lu sec",
                                        __func__,(vap->iv_last_vap_up_time/1000));
                            }
                        break;
                    }
                }
                wlan_iterate_all_sta_list(vap, sta_node_leave, NULL);
                error = ieee80211_vap_stop(vap, FALSE);
            }
            else {
                error = ieee80211_vap_stop(vap, FALSE);
            }
    ...
    if (!(flags & WLAN_MLME_STOP_BSS_F_NO_RESET) && !error)
...
int wlan_mlme_start_monitor(wlan_if_t vaphandle)
{
    ...
    if ((vap->iv_des_chan[vap->iv_des_mode] != IEEE80211_CHAN_ANYC) && (vap->iv_des_chan[vap->iv_des_mode])) {
    /* Follow random channel of ap vap, if spoof dfs fails in host */
        if(!((ic->ic_spoof_dfs_check_failed == 1) && (ieee80211_get_num_vaps_up(ic)))) {
...
void wlan_mlme_connection_up(wlan_if_t vaphandle)
{
    ...
    if ((!ic->ic_is_mode_offload(ic)) && (ic->ic_global_list->num_stavaps_up > 0)) {
        ic->ic_thermal_timer(ic);
    }
...
void wlan_mlme_connection_down(wlan_if_t vaphandle)
{
	...
	if (ic->ic_global_list->num_stavaps_up == 1) {
	    ic->ic_global_list->is_dbdc_rootAP = 0;
            ...
            for (i=0; i < MAX_RADIO_CNT; i++) {
               OS_MEMZERO(&ic->ic_global_list->preferred_mac_list[i][0], IEEE80211_ADDR_LEN);
               OS_MEMZERO(&ic->ic_global_list->denied_mac_list[i][0], IEEE80211_ADDR_LEN);
            }
        }
...

{
    ...
    vap->iv_stats.mlme_auth_attempt++;
    ...
    case IEEE80211_M_BTAMP:
        if (ni != vap->iv_bss) {
            ...
            if(IEEE80211_NODE_LEAVE(ni)) {
                ...
                ieee80211_bsteering_send_disconnect_event(vap, ni->ni_macaddr,
...
void wlan_mlme_external_auth_timer_fn(void *arg)
{
    ...
    IEEE80211_DPRINTF(vap, IEEE80211_MSG_AUTH, "external auth timer expire\n");
}
   ...
   case MLME_REQ_JOIN_ADHOC:
    ...
    case MLME_REQ_AUTH:
        ...
        if (vap->iv_sta_external_auth_enabled) {
            wlan_assoc_external_auth(vap, IEEE80211_EXTERNAL_AUTH_ABORT);
        }
...
#if OL_ATH_SMART_LOGGING
        {
            ...
            if (ic->ic_smart_log_connection_fail_start) {
                /* Currently, we will not alter the rest of the state machine
                 * flow based on whether we succeeded or failed in starting
                 * connection failure logging. Hence we do not check for
                 * failure. We rely on the API to print failure messages if any.
                 * Engineering action would have to be taken separately to
                 * analyze logging API failures.
                 */
                ic->ic_smart_log_connection_fail_start(ic);
            }
        }
