                                       ...
                                       u_int8_t reassoc,u_int8_t *vendor_ie, wbuf_t wbuf)
{
    ...
    */
    ...
    if (ni->ni_assocstatus != IEEE80211_STATUS_SUCCESS) {
        ...
        vap->iv_stats.authorize_attempt++;
...
#if ATH_BAND_STEERING
        // Copy nbuf to use later for MAP
        if (wbuf && ieee80211_vap_map_is_set(vap)) {
            ni->assoc_frame = qdf_nbuf_copy(wbuf);
        }
#endif
...
static int ieee80211_mlme_create_infra_continue(struct ieee80211vap *vap)
{
    ...
    if (vap->iv_state_info.iv_state == IEEE80211_S_ABORTING) {
        skip_dfs_cac = true;
    }
    ...
    if (skip_dfs_cac || ieee80211_dfs_cac_start(vap->iv_ic)) {
        if (!ic->ic_nl_handle) {
            ...
            status = ieee80211_vap_start(vap);
        } else {
            status = ieee80211_state_event(vap, IEEE80211_STATE_EVENT_DFS_WAIT);
        }
    }
...
mlme_create_infra_bss(struct ieee80211vap *vap, u_int8_t restart)
{
    ...
    */
    ...
    /*
     * issue a vap start request to resource manager via state machine.
     ...
     */
    error = ieee80211_state_event(vap, 
                (!restart) ? IEEE80211_STATE_EVENT_START : IEEE80211_STATE_EVENT_RESTART);
    if (error == EOK) { /* no resource manager in place */
        ...
        if (numvaps_up == 0) {
            if (error == EOK ) {
                IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME, "%s: Setting channel number %d\n", __func__, vap->iv_bsschan->ic_ieee);
                ieee80211_set_channel(ic, vap->iv_bsschan);
            }
...
void ieee80211_mlme_create_infra_continue_async(struct ieee80211vap *vap, int32_t status)
{
    ...
    if (!error) {
        ...
        qdf_atomic_set(&(vap->init_in_progress), 0);
        /* RUN failed, its likely there was a down before start-response from FW.
         * Stop and Reset the VAP */
