...
int ieee80211_fill_bcnreq_info(void *arg, wlan_scan_entry_t se)
{
    ...
    ssid = ieee80211_scan_entry_ssid(se, &ssid_len);

    start = (u_int8_t *)(cb_info->frm);

    if(cb_info->ssid_len > 0) {
        if(ssid && (cb_info->ssid_len == ssid_len) && !OS_MEMCMP(ssid, cb_info->ssid, cb_info->ssid_len)) {
...
/**
  ...
  * @return MAC address 
  */
int wlan_get_rrm_sta_list(wlan_if_t vap, u_int8_t *macaddr, u_long *param)
{
    struct ieee80211_node *ni  = NULL;
    ...
    if(IEEE80211_ADDR_EQ(macaddr, IEEE80211_GET_BCAST_ADDR(vap->iv_ic)))
    {
        ...
        dest_stats.dest_addr = (u_int8_t *)(uintptr_t)param[1];
        dest_stats.count = 0;
...
static void ieee80211_rrm_free_scan_resource(ieee80211_rrm_t rrm)
{
    ...
    if (rc != EOK) {
        ...
        "%s: wlan_scan_unregister_event_handler() failed handler=%pK,%pK rc=%x\n",
