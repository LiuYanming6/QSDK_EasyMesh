...
typedef struct ieee80211req_map_apcap_data *apcap;
 ...
 *
 * @return EOK on success; -EINVALID if client is not connected.
 *         +ve number in case memeory is not enough to copy.
 *
 */
int wlan_bsteering_get_assoc_frame(struct ieee80211vap *vap,
 ...
 */
void ieee80211_bsteering_send_rrm_frame_bcnrpt_event(struct ieee80211vap *vap, u_int32_t token,
                                               const u_int8_t *macaddr, const u_int8_t *bcnrpt,
                                               size_t report_len, u_int8_t num_rpt_elem);
...
void ieee80211_bsteering_send_opmode_update_event(struct ieee80211vap *vap,
                                                  const struct ieee80211_node *ni);
/*
 * @brief Send Basic radio cap required for MAP.
 *
 * @param [in] vap  the VAP on which the change occurred
 * @param [in] apcap struct to fill info.
 * @param [out] bool EOK for success otherwise EFAIL.
 */
bool wlan_bsteering_get_apcap(struct ieee80211vap *vap, mapapcap_t *apcap);

/**
 * @brief Set the RSSI threshold and Hysteresis Value to trigger event to upper
 *        layer when STA RSSI varies according to these values
 *
 * @param [inout] vap  the VAP whose overload status changes
 * @param [in] req  request from user space containing the RSSI and RSSI Hysteresis
 *
...
/*
 * @brief Send operable channels in a opclass for MAP.
 *
 * @param [in] vap  the VAP on which we need to get the op channels
 * @param [in] map_op_chan  struct to fill info.
 * @param [out] bool EOK for success otherwise EFAIL.
 */
...
static inline bool ieee80211_bsteering_is_vap_enabled(struct ieee80211vap *vap)
{
    return false;
}
...
static inline int wlan_bsteering_get_apcap(struct ieee80211vap *vap, mapapcap_t *apcap) {
    return -EINVAL;
}

static inline int wlan_bsteering_get_assoc_frame(struct ieee80211vap *vap, const u_int8_t *macaddr,
                                                 char *info, u_int16_t *length) {
    return -EINVAL;
}

static inline int wlan_bsteering_get_operable_channels(struct ieee80211vap *vap, map_op_chan_t *map_op_chan)
{
    return -EINVAL;
}

static inline int wlan_bsteering_set_map_rssi_policy(struct ieee80211vap *vap,
                                                     const struct ieee80211req_athdbg *req)
{
    return -EINVAL;
}
