    ...
    )
{
    ...
    do {
         ...
         */
        if ((des_ssid->len != 0) && (candidate_ssid != NULL)) {
            /* 
            ...
            if (candidate_ssid_len < 0 || candidate_ssid_len > IEEE80211_NWID_LEN) {
                IEEE80211_DPRINTF(vaphandle, IEEE80211_MSG_SCANENTRY, "%s", "- Reject (Invalid SSID Length)"
        ...
        if (! ieee80211_aplist_get_accept_any_bssid(pconfig)) {
            ...
            bIsAcceptable = false;
            bssid = ieee80211_scan_entry_bssid(scan_entry);
            ...
            for (i = 0; i < ieee80211_aplist_get_desired_bssid_count(pconfig); i++) {
                ...
                if (IEEE80211_ADDR_EQ(bssid, desired_bssid) == true) {
                    ...
                    if (candidate_ssid == NULL)
                        update_candidate_aplist_ssid_entries(scan_entry,
                                                 des_ssid->ssid, des_ssid->len);
                    bIsAcceptable = true;
                    break;
                }
            }
    ...
    )
{
    ...
    if (ieee80211_scan_entry_status(scan_entry) & AP_STATE_BAD) {
        ...
        if ((!pconfig->bad_ap_timeout) || (CONVERT_SYSTEM_TIME_TO_MS(OS_GET_TIMESTAMP() - ieee80211_scan_entry_bad_ap_time(scan_entry)) >
    ...
    if(ieee80211_candidate_aplist_match_ssid(vaphandle,
                                             pconfig,
                                             ssid,
                                             ssid_len,
                                             mac_address,
    ...
    /* Match data rates */
    {
        ...
        ieee80211_build_rate_set(standard_rates_ie, 
                                 extended_rates_ie,
                                 &merged_rate_set);
        if(ieee80211_scan_entry_htcap(scan_entry))
        {
            is_ht = 1;
        }
    ...
    )
{
    ...
    {
        tstBSSID = ieee80211_scan_entry_bssid(ap_list[i]);
