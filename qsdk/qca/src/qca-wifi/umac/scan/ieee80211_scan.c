...
struct ieee80211_scanner {
    ...
    struct ieee80211_ath_channel            *ss_chans[IEEE80211_SCAN_MAX];
    u_int16_t                           ss_nallchans;                      /* # all chans's to scan */
    struct ieee80211_ath_channel            *ss_all_chans[IEEE80211_SCAN_MAX];
...
static inline bool
is_passive_channel(struct ieee80211_ath_channel *channel,
...
#define FREQUENCY_THRESH 1000
    ...
    if (ss->ss_nreqchans == 0)
    {
        ...
        if ((ss->ss_flags & (IEEE80211_SCAN_ALLBANDS)) == IEEE80211_SCAN_ALLBANDS) {
            ...
            OS_MEMCPY(ss->ss_chans, ss->ss_all_chans, sizeof(struct ieee80211_ath_channel *) * ss->ss_nallchans);
#endif
        } else {
            for (i = 0; i < ss->ss_nallchans; ++ i) {
    ...
    if ((! ss->ss_common.ss_info.si_in_home_channel) || (ss->ss_suspending_traffic)) {
        ...
        default:     /* Resource Manager not active. */
            ...
            if (ss->ss_vap->iv_bsschan) {
                ...
                {
                    /*
                     * Skip set channel & chip reset if scan triggered to get channel utilization
                     * periodically of home channel only.
                     */
                    if ((wlan_channel_ieee(ss->ss_vap->iv_bsschan) ==
                          wlan_channel_ieee(ss->ss_ic->ic_curchan)) &&
                          (ss->ss_nchans == 1)) {
                            IEEE80211_SCAN_PRINTF(ss, IEEE80211_MSG_SCAN,
                                                  "%s: No channel change for channel:%d nchans:%d\n",
                                                  __func__, wlan_channel_ieee(ss->ss_chans[ss->ss_next - 1]),
                                                  ss->ss_nchans);
                    } else {
...
static void scanner_state_foreign_channel_entry(void *context)
{
    ...
    if (!ieee80211_resmgr_active(ss->ss_ic) ||
         ...
         * periodically of home channel only.
         */
        if ((wlan_channel_ieee(ss->ss_ic->ic_curchan) ==
                     wlan_channel_ieee(ss->ss_chans[ss->ss_next])) &&
                      (wlan_channel_ieee(ss->ss_vap->iv_bsschan) ==
                        wlan_channel_ieee(ss->ss_chans[ss->ss_next])) &&
                                                    (ss->ss_nchans == 1)) {
            IEEE80211_SCAN_PRINTF(ss, IEEE80211_MSG_SCAN,
                                  "%s: No channel change for channel:%d nchans:%d\n",
                                  __func__, wlan_channel_ieee(ss->ss_chans[ss->ss_next]),
                                  ss->ss_nchans);
        } else {
