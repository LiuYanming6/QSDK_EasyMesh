                    ...
                    IEEE80211_SCAN_ID        *scan_id)
{
    ...
    if (ic->ic_flags & IEEE80211_F_CHANSWITCH) {
        qdf_print("%s: CSA in progress, ignore scan\n", __func__);
        return -EAGAIN;
    }
#ifdef ATH_SUPPORT_RDKB
    /* Enable channel event notification to get
     * channel utilization counters from FW.
     */
    params->flags |= IEEE80211_SCAN_CHAN_EVENT;
#endif
    ...
    STA_VAP_DOWNUP_UNLOCK(vaphandle->iv_ic);
#endif
    if (vaphandle->iv_ic->no_chans_available
#if ATH_SUPPORT_DFS && QCA_DFS_NOL_VAP_RESTART
        || vaphandle->iv_ic->ic_pause_stavap_scan
#endif
) {
        IEEE80211_DPRINTF(vaphandle, IEEE80211_MSG_SCAN,"%s do not start_scan because no channel is set \n",__func__);
        return -EINVAL;
    }
