...
#define NUM_TRACE_BUF_SE           (1 << 5)    /*32 recent adds and removes */
    ...
    struct ieee80211_ath_channel          *se_chan;           /* channel where sta found */
    union {
        u_int8_t     data[8];
        u_int64_t    tsf;
    ...
    struct ieee80211_ath_channel          *se_alt_beacon_chan;    /* channel where alternative frame was found */
...
#define IEEE80211_DUMP_TRACESE(_entry) do {          \
    ...
    for (; i < NUM_TRACE_BUF_SE && (_entry)->t_func[i]; i++) {     \
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "***** %s(%d,%d)=> called by %s:%d, SSID=%.*s,refcnt:%d,ptr:0x%pK\n", \
           (_entry)->t_self[i], i,                      \
...
void update_candidate_aplist_ssid_entries(ieee80211_scan_entry_t scan_entry,
                                          u_int8_t *ssid, int len)
{
    OS_MEMZERO(scan_entry->se_ssid, sizeof(scan_entry->se_ssid));
    scan_entry->se_ssid[1] = len;
    OS_MEMCPY(&scan_entry->se_ssid[2], ssid, len);
}
...
struct ieee80211_ath_channel *ieee80211_scan_entry_channel( ieee80211_scan_entry_t scan_entry)
{
...
u_int8_t *ieee80211_scan_entry_mboie(ieee80211_scan_entry_t scan_entry)
{
    return scan_entry->se_ie_list.mboie;
}
bool ieee80211_scan_entry_oce_cap(ieee80211_scan_entry_t scan_entry)
{
    return scan_entry->se_oce_cap;
}

u_int32_t ieee80211_channel_frequency(struct ieee80211_ath_channel *chan)
{
    return chan->ic_freq;    /* frequency in Mhz */
}

u_int32_t ieee80211_channel_ieee(struct ieee80211_ath_channel *chan)
{
...
ieee80211_is_p2p_scan_entry_match(wlan_if_t                         vaphandle,
                                  ...
                                  const struct ieee80211_ath_channel    *beacon_chan,
                                  bool                              *pssid_match)
{
    bool    b_found = false;
    ...
    do {
        ...
        if (IEEE80211_ADDR_EQ(scan_entry->se_macaddr, beacon_macaddr))
        {
            ...
            if ((scan_entry->se_ssid[1] == 0) || (beacon_ssid_ie == NULL)) {
                b_found = false;
                IEEE80211_SCANENTRY_PRINTF(vaphandle->iv_ic, IEEE80211_MSG_SCANENTRY,
                                           "%s: Reject P2P entry: NULL SSID: MAC[" MACSTR "], beacon_ssid_ie=0x%pK, se_ssid[1]=%d\n",
                                           __func__,
                                           MAC2STR( (unsigned char*)&scan_entry->se_macaddr[0] ),
...
#define AVERAGE_RSSI_TIME_LIMIT    5000    /* 5 seconds */
    ...
    if (strlen(vaphandle->iv_filter_ssid) > 0 && strcmp(&(scan_entry->se_ssid[2]), vaphandle->iv_filter_ssid) == 0) {
        IEEE80211_SCANENTRY_PRINTF(vaphandle->iv_ic, IEEE80211_MSG_SCANENTRY,
                                    "%s ESSID getting filtered out\n", vaphandle->iv_filter_ssid);
        return scan_entry;
    }
...
#define convert_pointer(_address, _base1, _base2) \
    ...
    scan_entry->se_ie_list.mboie       = convert_pointer(scan_entry->se_ie_list.mboie,     beacon_frame, scan_entry->se_beacon_data);
#if QCA_LTEU_SUPPORT
    scan_entry->se_oce_cap = scan_entry_parameters->oce_cap;
#endif
    scan_entry->se_ie_list.extender_ie = convert_pointer(scan_entry->se_ie_list.extender_ie, beacon_frame, scan_entry->se_beacon_data);
...

    {
        ...
        LIST_FOREACH(current_scan_entry, &(scan_table->st_hash[hash]), se_hash) {
            if(current_scan_entry->se_chan == NULL || scan_entry_parameters->chan == NULL ){
                ...
                IEEE80211_DPRINTF(vaphandle, IEEE80211_MSG_ANY,"%s: ==se_chan=%pK==\n",__FUNCTION__,current_scan_entry->se_chan);
...
u_int8_t *ieee80211_scan_entry_extenderie(ieee80211_scan_entry_t scan_entry)
{
    return scan_entry->se_ie_list.extender_ie;
}

u_int8_t *wlan_scan_entry_extenderie(wlan_scan_entry_t scan_entry)
{
    return ieee80211_scan_entry_extenderie(scan_entry);
}
#endif
