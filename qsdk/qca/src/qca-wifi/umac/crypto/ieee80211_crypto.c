...
ieee80211_crypto_attach(struct ieee80211com *ic)
{
    ...
    ieee80211_crypto_register_fils(ic);
#endif
...
ieee80211_crypto_register(struct ieee80211com *ic, const struct ieee80211_cipher *cip)
{
    ...
    if (cip->ic_cipher >= IEEE80211_CIPHER_MAX) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: cipher %s has an invalid cipher index %u\n",
               __func__, cip->ic_name, cip->ic_cipher);
        return;
    }
    if (ic->ciphers[cip->ic_cipher] != NULL && ic->ciphers[cip->ic_cipher] != cip) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: cipher %s registered with a different template\n",
...
#define    N(a)    (sizeof(a) / sizeof(a[0]))
    ...
    if (((vap->iv_caps & cipher2cap(cipher)) == 0) || (flags & IEEE80211_KEY_PERSTA)) {
        ...
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_FILS,
                          "%s IEEE80211_KEY_SWCRYPT flag 0x%x cipher : 0x%x\n",
                          __func__, flags, cipher);
...
#if UMAC_SUPPORT_FILS
    if (cipher == IEEE80211_CIPHER_FILS_AEAD) {
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_FILS, "%s:FILS: SW only crypt."
                          "Not to be set in HW\n", __func__);
        return 1;
    }
#endif
...
static int
_ieee80211_crypto_clearkey(struct ieee80211vap *vap, struct ieee80211_key *key,
                         struct ieee80211_node *ni)
{
    u_int16_t keyix;
    KASSERT(key->wk_cipher != NULL, ("No cipher!"));
...
int
ieee80211_crypto_clearkey(struct ieee80211vap *vap, struct ieee80211_key *key,
                        struct ieee80211_node *ni)
{
    int status;
    ieee80211_key_update_begin(vap);
    status = _ieee80211_crypto_clearkey(vap, key, ni);
    ieee80211_key_update_end(vap);
    return status;
}
/*
        ...
        if (ret) {
            ...
            if (vap->iv_key_map) {
                ...
                if(ni && ni->ni_table == NULL) {
                    ...
                    IEEE80211_CRYPTO_KEY_UNLOCK_BH(key);
                    QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Warning: node not in table 0x%pK\n", ni);
                    return ret;
                }
