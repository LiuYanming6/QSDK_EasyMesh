...
wbuf_t ieee80211_tx_coalescing(struct ieee80211_node *ni, wbuf_t wbuf);
#endif
                       ...
                       struct ieee80211_tx_status *ts)
{
    ...
    if ((ts->ts_flags != 0) && !(ts->ts_flags & IEEE80211_TX_XRETRY)) {
        ...
        if(ts->ts_flags & IEEE80211_TX_ERROR)
            IEEE80211_NODE_STAT(ni, is_tx_not_ok);
#endif
    }
...
ieee80211_release_wbuf_internal(struct ieee80211_node *ni, wbuf_t wbuf, struct ieee80211_tx_status *ts )
{
    ...
    if (vap) {
		...
		QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s vap is null node ni 0x%pK\n", __func__, ni);
    }
