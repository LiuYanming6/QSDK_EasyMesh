...
int wlan_p2p_GO_set_appie(wlan_p2p_go_t p2p_go_handle, ieee80211_frame_type ftype, u_int8_t *buf, u_int16_t buflen, u_int8_t identifier)
{
    ...
    while (pos + 1 < end) {
        if (pos + 2 + pos[1] > end) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: invalid ie setup by os shim\n", __func__);
            OS_FREE(ie_buf);
            return EINVAL;
        }
        if (pos[0] == IEEE80211_P2P_SUB_ELEMENT_VENDOR &&
...
{
    if (p2p_go_handle->p2p_go_ps == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: not starting test since p2p_go_handle->p2p_go_ps is NULL\n", __func__);
        return 0;
    }
