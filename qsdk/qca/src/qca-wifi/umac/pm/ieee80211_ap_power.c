...
ieee80211_ap_power_vattach(struct ieee80211vap *vap)
{
    ...
    if (!vap->iv_tim_bitmap) {
        ...
        vap->iv_tim_bitmap = (u_int8_t *) OS_MALLOC(vap->iv_ic->ic_osdev, MAX_TIM_BITMAP_LENGTH + TIM_BITMAP_GAURD, 0);
...
int ieee80211_power_alloc_tim_bitmap(struct ieee80211vap *vap)
{
    ...
    vap->iv_tim_len = howmany(vap->iv_max_aid, 8) * sizeof(u_int8_t);
    tim_bitmap = OS_MALLOC(vap->iv_ic->ic_osdev, MAX_TIM_BITMAP_LENGTH + TIM_BITMAP_GAURD, 0);
    if(!tim_bitmap) {
