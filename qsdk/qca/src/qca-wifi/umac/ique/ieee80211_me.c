...
static int ieee80211_me_remove_expired_member(struct MC_GROUP_LIST_ENTRY* grp_list,
...
#if ATH_SUPPORT_ME_FW_BASED
    /* Do not remove group from the snoop table if offload path since timestamp is not updated */
    if (vap->iv_ic->ic_is_mode_offload(vap->iv_ic )) {
        return;
    }
#endif /*ATH_SUPPORT_ME_FW_BASED*/
...
static void ieee80211_me_SnoopShowDenyTable(struct ieee80211vap *vap)
{
    ...
    for (idx = 0 ; idx < ps->msl_deny_count; idx ++) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%d - addr : %d.%d.%d.%d, mask : %d.%d.%d.%d\n", idx + 1,
...
ieee80211_me_SnoopListDump(struct ieee80211vap* vap)
{
        ...
        snp_list != NULL)
    {
        IEEE80211_SNOOP_LOCK(snp_list,&lock_state);
        TAILQ_FOREACH(grp_list,&snp_list->msl_node,grp_list){
            QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "group addr %x:%x:%x:%x:%x:%x \n",grp_list->group_addr[0],
            ...
            TAILQ_FOREACH(grp_member_list, &grp_list->src_list,member_list){
                QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "    src_ip_addr %d:%d:%d:%d \n",((grp_member_list->src_ip_addr >> 24) & 0xff),
                                                        ((grp_member_list->src_ip_addr >> 16) & 0xff),
                                                        ((grp_member_list->src_ip_addr >>  8) & 0xff),
                                                        ((grp_member_list->src_ip_addr) & 0xff));
                QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "    member addr %x:%x:%x:%x:%x:%x\n",grp_member_list->grp_member_addr[0],
                ...
                QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "    Type 0x%x \n",grp_member_list->type);
                if (grp_member_list->type == ETHERTYPE_IP) {
                    QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "   ipv4 addr=0x%x\r\n",grp_member_list->u.grpaddr_ip4);
                } else {
                    int i;
                    QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "   ipv6 addr=");
                    for (i = 0; i < MLD_IP_ADDR_LENGTH; i++) {
                        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%x ",grp_member_list->u.grpaddr_ip6[i]);
                    }
                    QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "\r\n-------------------------------\r\n");
                }
                QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "    Mode %d \n",grp_member_list->mode);
            }
        }
        IEEE80211_SNOOP_UNLOCK(snp_list, &lock_state);
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
...
static void ieee80211_me_SnoopListInit(struct ieee80211vap *vap)
{
    atomic_set(&vap->iv_me->ieee80211_me_snooplist.msl_group_list_limit, vap->iv_me->ieee80211_me_snooplist.msl_list_max_length+1);
