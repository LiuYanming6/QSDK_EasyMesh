...
static int ieee80211_hbr_getstate(wlan_hbr_sm_t sm, void *arg1, void *arg2)
{
    ...
    block = sm->hbr_block;
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%d\t %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\t%s\t%s\t%s\t%d\n", 
...
{
    if (ni) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\t%s\n",
...
static void ieee80211_hbr_getstate_all(struct ieee80211vap * vap)
{
    int i;
    i = 0;
    QDF_PRINT_INFO( vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "HBR list dump\nNode\tAddress\t\t\tState\tTrigger\tBlock\tDropped VI frames\n");
