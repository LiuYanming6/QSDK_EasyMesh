...
static struct ieee80211_ath_channel* checkchan(wlan_if_t vaphandle,
...
extern void channel_change_by_precac(struct ieee80211com *ic);
#endif
...
void wlan_debug_dump_nodes_tgt(wlan_if_t vap);
#endif
...
int ieee80211_ucfg_set_freq_internal(wlan_if_t vap, int ieeechannel)
{
...
#if ATH_SUPPORT_ZERO_CAC_DFS
    if (((vap->iv_des_mode == IEEE80211_MODE_11AC_VHT80) ||
         (vap->iv_des_mode == IEEE80211_MODE_11AC_VHT160)) &&
        ieeechannel && ic->ic_precac_inter_chan) {
        freq = ieee80211_ieee2mhz(ic, ieeechannel, 0);

        if (vap->iv_des_mode == IEEE80211_MODE_11AC_VHT160)
            c = ieee80211_find_channel(ic, freq, 0, IEEE80211_CHAN_11AC_VHT160);
        else
            c = ieee80211_find_channel(ic, freq, 0, IEEE80211_CHAN_11AC_VHT80);

        /* If precac is done on this channel use it, else use a intermediate
         * non-DFS channel and trigger a precac on this channel */
        if(ic->ic_precac_enable && c) {
            if (IEEE80211_IS_CHAN_DFS(c) && !ieee80211_dfs_is_precac_done(ic, c)) {
                uint32_t freq2;
                struct ieee80211_ath_channel *c2;

                freq2 = ieee80211_ieee2mhz(ic, ic->ic_precac_inter_chan, 0);
                c2 = ieee80211_find_channel(ic, freq2, 0, IEEE80211_CHAN_11AC_VHT80);
                if(c2 != NULL) {
                    ieee80211_set_precac_prefered_channel(vap, ieeechannel);
                    ieeechannel = ic->ic_precac_inter_chan;
                }
            } else {
                ic->ic_precac_inter_chan = c->ic_ieee;
            }
        }
    }
#endif
    ...
    if(ic->ic_primary_allowed_enable && (ieeechannel != 0) &&
        ...
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY,
                QDF_TRACE_LEVEL_INFO,
                "%s: channel %d is not a primary allowed channel \n",
    ...
    if (vap->iv_opmode == IEEE80211_M_IBSS) {
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_IOCTL, "%s : IBSS desired channel(%d)\n",
                __func__, ieeechannel);
        retval =  wlan_set_desired_ibsschan(vap, ieeechannel);
        goto bad;
    }
    else if (vap->iv_opmode == IEEE80211_M_HOSTAP || vap->iv_opmode == IEEE80211_M_MONITOR)
    {
        ...
        if (!vap->iv_special_vap_mode) {
            TAILQ_FOREACH(tmpvap, &ic->ic_vaps, iv_next) {
                if (tmpvap->iv_opmode == IEEE80211_M_STA) {
                    ...
                    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Set freq vap %d stop send + %pK\n",tmpvap->iv_unit, tmpvap);
                    ...
                    if (ic->recovery_in_progress) {
                        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                               "%s: FW Crash on vap %d ...returning\n", __func__, tmpvap->iv_unit);
                        return -EINVAL;
                    }
			...
			wlan_debug_dump_nodes_tgt(tmpvap);
#endif
                        continue;
                    }
                    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Set wait done --%pK\n",tmpvap);
        ...
        retval = wlan_set_channel(vap, ieeechannel, vap->iv_des_cfreq2);
        /* Consider a case where channel change to a DFS channel is issued
         * following which spoof pulses are injected by FW after
         * vdev_start_resp. set_freq acquires "CHANCHANGE_STARTED"
         * lock and on radar found, the same lock is tried to acquire again.
         * As channel change lock is not released by ucfg_set_freq,
         * process_radar_event fails to return "success" on radar found, DFS
         * timer on FW expires and DFS channels are blocked in the host.
         * Hence clearing the "CHANCHANGE_STARTED" lock after set_channel and
         * before vap init.
         */
...
#define IEEE80211_FD_INTERVAL_MIN       20      /* min FILS Discovery Interval */
int ieee80211_ucfg_setparam(wlan_if_t vap, int param, int value, char *extra)
{
    ...
    case IEEE80211_PARAM_BSS_CHAN_INFO:
        ...
        if (ic->ic_ath_bss_chan_info_stats)
            ...
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Not supported for DA\n");
            return -EINVAL;
        }
	break;
    ...
    case IEEE80211_PARAM_COEXT_DISABLE:
        ...
        osif_restart_for_config(ic, NULL);
    ...
    case IEEE80211_PARAM_AUTHMODE:
        ...
        */
        if ((value != IEEE80211_AUTH_OPEN) && (value != IEEE80211_AUTH_SHARED)
                && (value != IEEE80211_AUTH_AUTO) && (value != IEEE80211_AUTH_SAE))
        {
            if (osifp->m_count)
    ...
    */
    ...
    case IEEE80211_PARAM_SEND_DEAUTH:
        ...
        break;
    case IEEE80211_PARAM_STEALTH_DOWN:
        retv = wlan_set_param(vap, IEEE80211_STEALTH_DOWN, value);
        break;
    ...
    case IEEE80211_PARAM_RANDOMGEN_MODE:
        ...
        {
         QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY,
             QDF_TRACE_LEVEL_INFO, "INVALID mode please use between modes 1 to 3\n");
         break;
        }
        ic->ic_random_gen_mode = value;
        break;
#endif
    ...
    case IEEE80211_IOCTL_GREEN_AP_PS_ENABLE:
        wlan_set_device_param(ic, IEEE80211_DEVICE_GREEN_AP_PS_ENABLE, value);
        retv = 0;
        break;
    ...
    case IEEE80211_PARAM_EXTAP:
    {
        ...
        bool is_offload = ic->ic_is_mode_offload(ic);
        if (value) {
            if (value == 3 /* dbg */) {
            ...
            if (!is_offload) {
                scn->sc_ops->set_extap(scn->sc_dev, 1);
            }
            ...
            if (!is_offload) {
                scn->sc_ops->set_extap(scn->sc_dev, 0);
            }
...
#undef IBSSDFS_RECOVER_MASK
   ...
   break;
    ...
    case  IEEE80211_PARAM_ATF_OPT:
        ...
        } else if ((ic->ic_is_mode_offload(ic)) &&
                    (ic->ic_atf_tput_tbl_num) && (retv == EOK)) {
            struct net_device *tmpdev = NULL;
            TAILQ_FOREACH(tmpvap, &ic->ic_vaps, iv_next) {
                tmpdev = ((osif_dev *)tmpvap->iv_ifp)->netdev;
                retv = IS_UP(tmpdev) ? -osif_vap_init(tmpdev, RESCAN) : 0;
    ...
    case  IEEE80211_PARAM_ATF_SSID_GROUP:
        ieee80211_atf_ssidgroup_set_reset(vap, !!value);
    break;
    ...
    case IEEE80211_PARAM_PROBEREQ_EVENT:
        if(!value)
            vap->iv_probe_ev_enable = 0;
        else if(value == 1)
            vap->iv_probe_ev_enable = 1;
        else
            return -EINVAL;
        break;
#if MESH_MODE_SUPPORT
    ...
    case IEEE80211_PARAM_WHC_APINFO_SFACTOR:
        if((vap->iv_opmode == IEEE80211_M_STA) && (value >= 1) && (value <= 100))
            ic->ic_whc_scaling_factor = value;
        break;
        ...
        if(ic->ic_is_mode_offload(ic)){
            if(IEEE80211_FC0_TYPE_MGT ==frame_type)
            {
                if(0xff != frame_subtype)
                {
                    ...
                    {
                        retv = ic->ic_vap_set_param(vap, IEEE80211_CONFIG_VAP_TXPOW, (frame_type<<IEEE80211_FRAMETYPE_TXPOW_SHIFT)+(frame_subtype<<IEEE80211_SUBTYPE_TXPOW_SHIFT)+transmit_power);
                    }
                    else
                    {
                        vap->iv_txpow_mgt_frm[(frame_subtype >> IEEE80211_FC0_SUBTYPE_SHIFT)] = transmit_power;
    ...
    case IEEE80211_PARAM_BCAST_PROBE_RESPONSE_ENABLE:
        ...
        if (vap->iv_opmode == IEEE80211_M_HOSTAP && !IEEE80211_VAP_IS_HIDESSID_ENABLED(vap)) {
            ...
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                                    "Invalid. Allowed only in HOSTAP & non-hidden SSID mode \n");
        }
        break;
    ...
    case IEEE80211_PARAM_ENABLE_FILS:
        if(val[1])
        {
            fils_en_period |= (1 << IEEE80211_FILS_ENABLE_BIT);
            val[2] &= 0xFFFF;
        } else {
            fils_en_period &= ~(1 << IEEE80211_FILS_ENABLE_BIT);
            val[2] = 0;
        }
        if (!((val[2] >= IEEE80211_FD_INTERVAL_MIN) &&
             (val[2] < vap->iv_bss->ni_intval) &&
             (vap->iv_bss->ni_intval % val[2] == 0))) {
            qdf_print("%sInvalid FD Interval : %d. Disabling FD\n",
                        __func__, val[2]);
            val[2] = 0;
        }
        fils_en_period |= val[2];
    ...
    case IEEE80211_PARAM_SOFTBLOCK_WAIT_TIME:
        if (value >= SOFTBLOCK_WAIT_TIME_MIN && value <= SOFTBLOCK_WAIT_TIME_MAX) {
            ...
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                "Allowed value between (%d, %d)\n", SOFTBLOCK_WAIT_TIME_MIN, SOFTBLOCK_WAIT_TIME_MAX);
            retv = -EINVAL;
        }
        break;
    case IEEE80211_PARAM_SOFTBLOCK_ALLOW_TIME:
        if (value >= SOFTBLOCK_ALLOW_TIME_MIN && value <= SOFTBLOCK_ALLOW_TIME_MAX) {
            ...
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                "Allowed value between (%d, %d)\n", SOFTBLOCK_ALLOW_TIME_MIN, SOFTBLOCK_ALLOW_TIME_MAX);
            retv = -EINVAL;
        }
        break;
#endif
    ...
    case IEEE80211_PARAM_RSN_OVERRIDE:
        ...
        break;
    case IEEE80211_PARAM_DFS_DISABLE_CHAN_CHANGE:
        ic->ic_tst_disable_chanchange_post_radar_detect = value;
        break;
    case IEEE80211_PARAM_DFS_SELECT_RANDOM_CHAN_AFTER_SPOOF_TEST_FAIL:
        ic->ic_tst_choose_random_chan = value;
        break;
    case IEEE80211_PARAM_MAP:
        if (val) {
            if (ieee80211_vap_map_is_set(vap))
                retv = -EALREADY;
            else {
                ieee80211_vap_map_set(vap);
                retv = ENETRESET;
            }
        } else {
            if (ieee80211_vap_map_is_clear(vap))
                retv = -EALREADY;
            else {
                ieee80211_vap_map_clear(vap);
                retv = ENETRESET;
            }
        }
        break;
    case IEEE80211_PARAM_MAP_BSS_TYPE:
        if (val) {
            if (value & MAP_FRONTHAUL_BSS) {
                if (ieee80211_vap_mapfh_is_set(vap))
                    retv = -EALREADY;
                else {
                    ieee80211_vap_mapfh_set(vap);
                    retv = ENETRESET;
                }
            }

            if (value & MAP_BACKHAUL_BSS) {
                if (ieee80211_vap_mapbh_is_set(vap))
                    retv = -EALREADY;
                else {
                    ieee80211_vap_mapbh_set(vap);
                    retv = ENETRESET;
                }
            }

            if (value & MAP_BACKHAUL_STA) {
                if (ieee80211_vap_mapbsta_is_set(vap))
                    retv = -EALREADY;
                else {
                    ieee80211_vap_mapbsta_set(vap);
                    retv = ENETRESET;
                }
            }
        } else {
            if (ieee80211_vap_mapfh_is_clear(vap) || ieee80211_vap_mapbh_is_clear(vap) ||
                ieee80211_vap_mapbsta_is_clear(vap)) {
                retv = -EALREADY;
            } else {
                ieee80211_vap_mapfh_clear(vap);
                ieee80211_vap_mapbh_clear(vap);
                ieee80211_vap_mapbsta_clear(vap);
                retv = ENETRESET;
            }
        }
        break;
    case IEEE80211_PARAM_DROP_MDNS:
        wlan_set_param(vap, IEEE80211_DROP_MDNS, value);
        break;
#ifdef ATH_SUPPORT_RDKB
    ...
    case IEEE80211_PARAM_MAP_VAP_BEACONING:
        if (value) {
            if (ieee80211_vap_mapvapup_is_set(vap))
                retv = -EALREADY;
            else {
                ieee80211_vap_mapvapup_set(vap);
                retv = ENETRESET;
            }
        } else {
            if (ieee80211_vap_mapvapup_is_clear(vap))
                retv = -EALREADY;
            else {
                ieee80211_vap_mapvapup_clear(vap);
                retv = ENETRESET;
            }
        }
        break;
    ...
    default:
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                       "%s : parameter 0x%x not supported \n", __func__, param);
        retv = -EOPNOTSUPP;
        break;
    }
...
int ieee80211_ucfg_getparam(wlan_if_t vap, int param, int *value)
{
    ...
    int tmp;
#endif
    if (!osifp || osifp->is_delete_in_progress) return -EINVAL;

    switch (param) {
        case IEEE80211_PARAM_MAXSTA:
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                           "Getting Max Stations: %d\n", vap->iv_max_aid - 1);
    ...
    case IEEE80211_PARAM_SEND_DEAUTH:
        *value = wlan_get_param(vap, IEEE80211_SEND_DEAUTH);
        break;
    case IEEE80211_PARAM_STEALTH_DOWN:
        *value = wlan_get_param(vap, IEEE80211_STEALTH_DOWN);
        break;
    case IEEE80211_PARAM_WEP_KEYCACHE:
...
#endif
    case IEEE80211_IOCTL_GREEN_AP_PS_ENABLE:
        *value = wlan_get_device_param(ic, IEEE80211_DEVICE_GREEN_AP_PS_ENABLE);
        break;
    case IEEE80211_IOCTL_GREEN_AP_PS_TIMEOUT:
	...
	*value = wlan_get_param(vap, IEEE80211_ME_TIMEOUT);
    ...
    case IEEE80211_PARAM_XBSS_LOAD:
        *value = wlan_get_param(vap, IEEE80211_XBSS_LOAD);
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,"Extended BSS Load: %s\n",*value? "Enabled":"Disabled");
        if (*value)
        {
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,"MU-MIMO capable STA Count: %d\n", vap->iv_mu_bformee_sta_assoc);
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,"Spatial Stream Underutilization: %d\n", ic->ic_ss_uu);
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,"Observable secondary 20MHz Utilization: %d\n", ic->ic_sec_20u);
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,"Observable secondary 40MHz Utilization: %d\n", ic->ic_sec_40u);
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,"Observable secondary 80MHz Utilization: %d\n", ic->ic_sec_80u);
        }
        break;
#endif /* UMAC_SUPPORT_XBSSLOAD */
#if UMAC_SUPPORT_CHANUTIL_MEASUREMENT
 ...
 case IEEE80211_PARAM_VAP_ENHIND:
    ...
    case IEEE80211_PARAM_WHC_APINFO_ROOT_DIST:
        *value = vap->iv_whc_root_ap_distance;
        break;
    case IEEE80211_PARAM_WHC_APINFO_SFACTOR:
        *value = ic->ic_whc_scaling_factor;
        break;
    ...
    case IEEE80211_PARAM_WHC_APINFO_RATE:
        *value = (int)ieee80211_rep_datarate_estimator(ic->ic_serving_ap_backhaul_rate,
                ic->ic_uplink_rate,
                (vap->iv_whc_root_ap_distance - 1),
                ic->ic_whc_scaling_factor);
        break;
   ...
   case IEEE80211_PARAM_MACCMD_SEC:
    ...
    case IEEE80211_PARAM_BCAST_PROBE_RESPONSE_STATS:
        *value = 0;
        if (vap->iv_opmode == IEEE80211_M_HOSTAP) {
            ...
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                           "| Timer expires in           - %lld |\n", qdf_ktime_to_ns(qdf_ktime_add(qdf_ktime_get(),                                                     qdf_hrtimer_get_remaining(&vap->bpr_timer))));
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
    ...
    case IEEE80211_PARAM_ACTIVITY:
        *value = wlan_get_param(vap, IEEE80211_GET_VAP_ACTIVITY);
        break;
#if ATH_ACL_SOFTBLOCKING
    case IEEE80211_PARAM_SOFTBLOCK_WAIT_TIME:
    ...
    case IEEE80211_PARAM_TIMEOUTIE:
        *value = wlan_get_param(vap, IEEE80211_SUPPORT_TIMEOUTIE);
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY,
            QDF_TRACE_LEVEL_INFO, "Getting TIMEOUTIE: %x\n",*value);
        break;
    case IEEE80211_PARAM_PMF_ASSOC:
        *value = wlan_get_param(vap, IEEE80211_SUPPORT_PMF_ASSOC);
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY,
            QDF_TRACE_LEVEL_INFO, "PMF_ASSOC: %x\n",*value);
        break;
    case IEEE80211_PARAM_DFS_INFO_NOTIFY_APP:
    ...
    case IEEE80211_PARAM_MAP:
        *value = ieee80211_vap_map_is_set(vap);
        break;
    case IEEE80211_PARAM_MAP_BSS_TYPE:
        *value = 0;
        if (ieee80211_vap_mapfh_is_set(vap)) {
            *value |= MAP_FRONTHAUL_BSS;
        }
        if (ieee80211_vap_mapbh_is_set(vap)) {
            *value |= MAP_BACKHAUL_BSS;
        }

        if (ieee80211_vap_mapbsta_is_set(vap)) {
            *value |= MAP_BACKHAUL_STA;
        }
        break;
    case IEEE80211_PARAM_DROP_MDNS:
        *value = wlan_get_param(vap, IEEE80211_DROP_MDNS);
        break;
#ifdef ATH_SUPPORT_RDKB
    case IEEE80211_PARAM_CSA_INTEROP_PHY:
...
#endif
    case IEEE80211_PARAM_MAP_VAP_BEACONING:
        *value = ieee80211_vap_mapvapup_is_set(vap);
        break;
    case IEEE80211_PARAM_SAE_AUTH_ATTEMPTS:
...
int ieee80211_ucfg_get_maxphyrate(wlan_if_t vaphandle)
{
 ...
 return(!ieee80211_vap_ready_is_set(vap) ? 0:ic->ic_get_maxphyrate(ic, vap->iv_bss));
}
...
#if QCA_AIRTIME_FAIRNESS
/**
 * ieee80211_ucfg_atf_flushtable: Flush ATF table
 * Clears all ATF configuration, sets group & subgroup for deletion
 * before vap init.
 *
 ...
 *         -1 on Error
 */
int ieee80211_ucfg_atf_flushtable(wlan_if_t vap)
{
    struct ieee80211com *ic = vap->iv_ic;
    wlan_if_t tmpvap;
    ...
    memset(&(ic->atfcfg_set.peer_id[0]), 0, (sizeof(ic->atfcfg_set.peer_id[0]) * ATF_ACTIVED_MAX_CLIENTS));
    memset(&(ic->atfcfg_set.vap[0]), 0, (sizeof(ic->atfcfg_set.vap[0]) * ATF_CFG_NUM_VDEV));
    memset(&(ic->atfcfg_set.atfgroup[1]), 0, (sizeof(ic->atfcfg_set.atfgroup[0]) * (ATF_ACTIVED_MAX_ATFGROUPS-1)));

    ic->atfcfg_set.grp_num_cfg = 1;
    ic->atfcfg_set.peer_num_cfg = 0;
    ic->atfcfg_set.peer_num_cal = 0;
    ic->atfcfg_set.peer_cal_bitmap = 0;
    ic->atfcfg_set.vap_num_cfg = 0;
    TAILQ_FOREACH(tmpgroup, &ic->ic_atfgroups, group_next) {
        if(!ieee80211_atf_is_default_group(tmpgroup->group_name))
        {
            ieee80211_atf_mark_del_all_sg(ic, tmpgroup);
            tmpgroup->group_del = 1;
        }
    }
    OS_SET_TIMER(&ic->atfcfg_timer, 0);
...
/**
 * ieee80211_atf_ssidgroup_set_reset: Enable/Disable ATF SSID Grouping feature
 * Flushes the ATF table if previous configuration exists
 * before enabling/disabling the feature
 *
 * @ic: Pointer to ic structure
 * @val: enable/Disable as set by the user
 *
 * Return: None
 */
void ieee80211_atf_ssidgroup_set_reset(wlan_if_t vap, int32_t val)
{
    int8_t atfgroup_state = 0;
    struct ieee80211com *ic = vap->iv_ic;

    /* Check if previous & current val are the same */
    if (ic->ic_atf_ssidgroup != val) {
        atfgroup_state = 1;
    }
    ...
    if (atfgroup_state) {
        ...
        if ((!val && ic->atfcfg_set.grp_num_cfg)) {
            qdf_print("Group configuration exists already. Flushing ATF configuration\n");
            ieee80211_ucfg_atf_flushtable(vap);
        }

        /* If Grouping is being enabled & ATF configuration(SSID/PEER) exists */
        if (val &&
            (ic->atfcfg_set.peer_num_cfg || ic->atfcfg_set.vap_num_cfg
             || ic->atfcfg_set.peer_num_cal)) {
            qdf_print("Flushing existing ATF configuration\n");
            ieee80211_ucfg_atf_flushtable(vap);
        }
    }
    ic->ic_atf_ssidgroup = val;
}

/**
 * ieee80211_vap_iter_atf_ssid_validate : Validate SSID provide by user
 * Sets the Flag 'ssid_exist' if configured ssid already exists.
 *
 * @arg: Pointer to ATF SSID config structure
...
/**
 * ieee80211_atf_ssid_in_group() - Check if ssid is part of any group
 * @ic: Pointer to the radio structure
 * @grp_name: ssid name to compare
 *
 * Return: TRUE if ssid part of any group
 *         FALSE if ssid is not part of group
 */
int ieee80211_atf_ssid_in_group(struct ieee80211com *ic, u_int8_t *ssid)
{
    int i = 0, k = 0;
    ...
    for (i = 0; i < ic->atfcfg_set.grp_num_cfg; i++) {
        ...
        for (k = 0; k < ic->atfcfg_set.atfgroup[i].grp_num_ssid; k++) {
            if ((strlen(ic->atfcfg_set.atfgroup[i].grp_ssid[k]) == strlen(ssid)) &&
                !(strncmp( (char *)(ic->atfcfg_set.atfgroup[i].grp_ssid[k]), (char *)(ssid), strlen(ssid)))) {
                return TRUE;
            }
        }
    }
...
/**
 * ieee80211_atf_mark_del_all_sg() - Mark all subgroups for deletion
 * Parse the group/subgroup list & sel del flag for all SGs
 * @ic: Pointer to the radio structure
 * @group: pointer to the group list entry
 *
 * Return: 0 on success
 *         -EINVAL on ERROR
 */
...
{
    struct subgroup_list *tmpgroup = NULL;
    struct subgroup_list *tmpgroup_next = NULL;

    if (TAILQ_EMPTY(&group->atf_subgroups))
        return -EINVAL;

    TAILQ_FOREACH_SAFE(tmpgroup, &group->atf_subgroups, sg_next, tmpgroup_next) {
        tmpgroup->sg_del = 1;
        group->num_subgroup--;
    }
...
/**
 * ieee80211_atf_new_subgroup_list() - Create new subgroup
 * @ic: Pointer to the radio structure
 * @group: pointer to the group list entry
 * @param: Peer mac incase of Peer based ATF Config
 *         AC id incase of AC based ATF config
 * @val: user configured airtime
 * @sg_type: Subgroup type (SSID/Peer/AC)
 *
 * Return: NULL on error
 *         Pointer to subgroup on success
 */
struct subgroup_list *ieee80211_atf_new_subgroup_list(struct ieee80211com *ic, struct group_list *group,
                                            void *param, u_int32_t val, u_int32_t sg_type)
{
    struct subgroup_list *subgroup = NULL;

    if ((sg_type == IEEE80211_ATF_SUBGROUP_TYPE_PEER ||
        sg_type == IEEE80211_ATF_SUBGROUP_TYPE_AC) && !param) {
        qdf_print("%s param cannot be NULL.. CHeck \n",__func__);
        return NULL;
    }
    ...
    if (subgroup == NULL) {
        qdf_print("%s %d subgroup alloc failed. Return error\n",__func__, __LINE__);
        return NULL;
    }
    subgroup->sg_id = group->subgroup_count++;
    snprintf(subgroup->sg_name, IEEE80211_NWID_LEN, "%s%d%s%s", "sg_", (subgroup->sg_id), "_", group->group_name);
    ...
    if (sg_type == IEEE80211_ATF_SUBGROUP_TYPE_PEER) {
        OS_MEMCPY((char *)(subgroup->sg_peermac),(char *)(param),IEEE80211_ADDR_LEN);
    } else if (sg_type == IEEE80211_ATF_SUBGROUP_TYPE_AC) {
...
/**
 * ieee80211_atf_new_group_list() - create a new group list entry
 * @ic: Pointer to the radio structure
 * @ssid: ssid to add
 * @groupindex: index of the group in the ic structure
 *
 * Return: NULL if group could not be created
 *         pointer to the group structure
 */
struct group_list *ieee80211_atf_new_group_list(struct ieee80211com *ic, char *ssid, u_int32_t groupindex)
{
    ...
    strlcpy(group->group_name, ssid, sizeof(group->group_name));
...
/**
 * ieee80211_atf_group_exist() - Check if group exists
 * @ic: Pointer to the radio structure
 * @grp_name: group name to compare
 * @groupindex: If found, index of the group
 *
 * Return: TRUE if group exists
 *         FALSE if group not found
 */
int ieee80211_atf_group_exist(struct ieee80211com *ic, u_int8_t *grp_name, int *groupindex)
{
    int grp_index = 0;

    if (!grp_name) {
        qdf_print("%s group name is NULL. Returning \n",__func__);
        return FALSE;
    }

    for (grp_index = 0; grp_index < ic->atfcfg_set.grp_num_cfg; grp_index++) {
        /* Check if group already exists */
        if ((strlen(ic->atfcfg_set.atfgroup[grp_index].grpname) == strlen(grp_name)) &&
             !(strncmp((char *)(ic->atfcfg_set.atfgroup[grp_index].grpname), (char *)(grp_name), strlen(grp_name)))) {
            *groupindex = grp_index;
            return TRUE;
        }
    }
...
/**
 * iee80211_atf_mark_del_group_list() - Mark group list entries for deletion
 * group entry will be deleted in timer context
 * @ic: Pointer to the radio structure
 * @group: Pointer to the group entry
 *
 * Return: 0 on success
 *         1 on error
 */
int ieee80211_atf_mark_del_group_list(struct ieee80211com *ic, struct group_list *group)
{
    if(group == NULL)
    {
        qdf_print("%s group is NULL. Can't delete \n", __func__);
        return 1;
    }
    group->group_del = 1;
...
/**
 * iee80211_atf_mark_del_subgroup_list() - Mark subgroup list entries for deletion
 * subgroup entry will be deleted in timer context
 * @ic: Pointer to the radio structure
 * @peer_mac: MAC address of the peer(incase of peer based config)
 *
 * Return: 0 on success
 */
int ieee80211_atf_mark_del_subgroup_list(struct ieee80211com *ic, void *peer_mac)
{
    struct group_list *group = NULL;
    struct subgroup_list *subgroup = NULL;

    TAILQ_FOREACH(group, &ic->ic_atfgroups, group_next) {
        if (ieee80211_atf_subgroup_exist(ic, group, peer_mac, &subgroup, IEEE80211_ATF_SUBGROUP_TYPE_PEER)) {
            subgroup->sg_del = 1;
        }
    }
...
/**
 * ieee80211_atf_del_ssid_group() - Delete group from ic structure
 * @ic: Pointer to the radio structure
 * @ssid: ssid name to del
 * @groupindex: index of group
 *
 * Return: 0 on success
 *         -EINVAL on error
 */
int ieee80211_atf_del_ssid_group(struct ieee80211com *ic, u_int8_t *ssid, u_int32_t groupindex)
{
    u_int32_t i = 0;

    if (groupindex >= ATF_ACTIVED_MAX_ATFGROUPS) {
        qdf_print("%s Invalid group index \n",__func__);
        return -EINVAL;
    }

    if (!ic->atfcfg_set.atfgroup[groupindex].grp_num_ssid) {
        qdf_print("%s no SSIDs attached to this group %s\n",
                __func__, ic->atfcfg_set.atfgroup[groupindex].grpname);
        return -EINVAL;
    }

     memset (&(ic->atfcfg_set.atfgroup[groupindex]), 0, sizeof(ic->atfcfg_set.atfgroup[0]));

     if (groupindex >= ic->atfcfg_set.grp_num_cfg) {
        qdf_print("%s Invalid index \n",__func__);
        return -EINVAL;
     }

     if (ic->atfcfg_set.grp_num_cfg > ATF_ACTIVED_MAX_ATFGROUPS) {
        qdf_print("%s Invalid grp_num_cfg : %d \n", __func__, ic->atfcfg_set.grp_num_cfg);
        return -EINVAL;
     }

     /* if group is not the last, move the other group indexes */
     if ((groupindex != (ic->atfcfg_set.grp_num_cfg - 1)) &&
        (groupindex < ATF_ACTIVED_MAX_ATFGROUPS)) {
        for (i = groupindex; (i < ic->atfcfg_set.grp_num_cfg) && (i < ATF_ACTIVED_MAX_ATFGROUPS); i++) {
            if (i != (ic->atfcfg_set.grp_num_cfg - 1)) {
                memcpy(&ic->atfcfg_set.atfgroup[i], &ic->atfcfg_set.atfgroup[i+1], (sizeof(ic->atfcfg_set.atfgroup[0]) * (ic->atfcfg_set.grp_num_cfg - i - 1)) );
            }
        }
    }
    memset (&(ic->atfcfg_set.atfgroup[ic->atfcfg_set.grp_num_cfg - 1]), 0,
           sizeof(ic->atfcfg_set.atfgroup[0]));
...
/**
 * ieee80211_atf_add_ssid_group() - Add ssid to group structure
 * @ic: Pointer to the radio structure
 * @ssid: ssid name to add
 * @groupindex: index of group
 * @group: pointer to group list entry
 *
 * Return: 0 on success
 *         -EINVAL on error
 */
int ieee80211_atf_add_ssid_group(struct ieee80211com *ic, u_int8_t *ssid, u_int32_t groupindex, struct group_list *group)
{
    u_int32_t groupssidindex = 0;

    if (ic->atfcfg_set.atfgroup[groupindex].grp_num_ssid >= ATF_CFG_NUM_VDEV) {
        qdf_print("%s %d num_ssid(%d) > NUM_VDEV(%d) \n",
                __func__, __LINE__, ic->atfcfg_set.atfgroup[groupindex].grp_num_ssid, ATF_CFG_NUM_VDEV);
        return -EINVAL;
    }
    ...
    strlcpy(ic->atfcfg_set.atfgroup[groupindex].grp_ssid[groupssidindex], (char *)(ssid) ,IEEE80211_NWID_LEN+1);
...
/**
 * ieee80211_atf_config_group_val_sched() - Configure default group scheduling policy and airtime value
 * @ic: Pointer to the radio structure
 * @value: Airtime value configured for the group
 * @groupindex: index of group
 *
 * Return 0 on success
 */
int ieee80211_atf_config_group_val_sched(struct ieee80211com *ic, u_int32_t value, u_int32_t groupindex)
{
    ic->atfcfg_set.atfgroup[groupindex].grp_sched_policy = IEEE80211_ATF_GROUP_SCHED_FAIR;
    ic->atfcfg_set.atfgroup[groupindex].grp_cfg_value = value;

    return 0;
}

/**
 * dump_atf_group_subgroup() - Debug command to dump all groups & subgroups
 * @ic: Pointer to the radio structure
 *
 * Return: None
 */
void dump_atf_group_subgroup(struct ieee80211com *ic)
{
    ...
    qdf_print("********** DUMP GROUP - START **********\n");
    for(i = 0; i < ic->atfcfg_set.grp_num_cfg ; i++)
    {
        qdf_print("group name : %s grp_num_ssid : %d \n", ic->atfcfg_set.atfgroup[i].grpname, ic->atfcfg_set.atfgroup[i].grp_num_ssid);
        qdf_print("SSIDs : \t");
        for(k = 0; k < ic->atfcfg_set.atfgroup[i].grp_num_ssid; k++)
        {
            qdf_print(" %s \n", ic->atfcfg_set.atfgroup[i].grp_ssid[k]);
        }
        group = ic->atfcfg_set.atfgroup[i].grplist_entry;
        if (group == NULL) {
            qdf_print("index : %d group is null \n",i);
        } else {
            qdf_print("index : %d group list name : %s \n",i,  group->group_name);
        }

        group = ic->atfcfg_set.atfgroup[i + 1].grplist_entry;
        if (group == NULL) {
            qdf_print("index : %d group is null \n",i + 1);
        } else {
            qdf_print("index : %d group list name : %s \n", i + 1, group->group_name);
        }
    }

    TAILQ_FOREACH(group, &ic->ic_atfgroups, group_next) {
        qdf_print("Group : %s Val : %d num subgroup : %d \n", group->group_name, group->group_airtime, group->subgroup_count );
        TAILQ_FOREACH(subgroup, &group->atf_subgroups, sg_next) {
            qdf_print("Subgroup : %s ", subgroup->sg_name);
        }
        qdf_print("\n");
    }
    qdf_print("********** DUMP GROUP - END **********\n");
}

int ieee80211_atf_subgroup_match_peer(struct subgroup_list *sgroup, char *mac)
{
    if (sgroup->sg_type == IEEE80211_ATF_SUBGROUP_TYPE_PEER &&
        IEEE80211_ADDR_EQ((char *)(sgroup->sg_peermac), mac)) {
        return TRUE;
    }
...
int ieee80211_atf_subgroup_match_ac(struct subgroup_list *sgroup, u_int8_t ac_id)
{
     if (sgroup->sg_type == IEEE80211_ATF_SUBGROUP_TYPE_AC &&
         ac_id == sgroup->ac_id) {
        return TRUE;
     }
...
int ieee80211_atf_subgroup_exist(struct ieee80211com *ic, struct group_list *group, void *param, struct subgroup_list **subgroup, int8_t cfg_type )
{
    struct subgroup_list *sgroup = NULL;

    if (!group) {
        qdf_print("%s group is NULL. Return \n",__func__);
        return -EINVAL;
    }

    TAILQ_FOREACH(sgroup, &group->atf_subgroups, sg_next) {
        switch (cfg_type) {
            case IEEE80211_ATF_SUBGROUP_TYPE_SSID:
               break;
            case IEEE80211_ATF_SUBGROUP_TYPE_PEER:
                if (ieee80211_atf_subgroup_match_peer(sgroup, (char *)param)) {
                    *subgroup = sgroup;
                    return TRUE;
                }
                break;
            case IEEE80211_ATF_SUBGROUP_TYPE_AC:
                if (ieee80211_atf_subgroup_match_ac(sgroup, *(u_int8_t*)param)) {
                    *subgroup = sgroup;
                    return TRUE;
                }
                break;
            default:
                qdf_print("%s Invalid cfg_type : %d \n",__func__, cfg_type);
                return -EINVAL;
        }
    }
...
int ieee80211_atf_modify_sta_cfg_index(struct ieee80211com *ic, u_int32_t index)
{
    int i = 0, j = 0;

    for (i = 0; i < ATF_ACTIVED_MAX_CLIENTS; i++) {
        for (j = index + 1; j < ATF_CFG_NUM_VDEV; j++) {
            ic->atfcfg_set.peer_id[i].sta_cfg_value[j] =
                ic->atfcfg_set.peer_id[i].sta_cfg_value[j + 1];
        }
    }
...
/**
 * ieee80211_atf_del_ic_group() - Delete a group list entry
 * @ic: Pointer to the radio structure
 * @grp_name: group name
 * @index: group index
 *
 ...
 *         1 on error
 */
int ieee80211_atf_del_ic_group(struct ieee80211com *ic, char *group_name, u_int32_t index)
{
    int ret = 0;

    memset(ic->atfcfg_set.atfgroup[index].grpname, 0, sizeof(ic->atfcfg_set.atfgroup[index].grpname));
    if (index != (ic->atfcfg_set.grp_num_cfg - 1)) {
        memcpy(&ic->atfcfg_set.atfgroup[index], &ic->atfcfg_set.atfgroup[index+1],
                  (sizeof(ic->atfcfg_set.atfgroup[0]) * (ic->atfcfg_set.grp_num_cfg - index - 1)));
        memset(&(ic->atfcfg_set.atfgroup[ic->atfcfg_set.grp_num_cfg - 1]), 0, sizeof(ic->atfcfg_set.atfgroup[0]));
    }
    ieee80211_atf_modify_sta_cfg_index(ic, index);

    if (ic->atfcfg_set.grp_num_cfg) {
        ic->atfcfg_set.grp_num_cfg--;
    } else {
        qdf_print("%s ERROR!! ic->atfcfg_set.grp_num_cfg is %d \n",__func__, ic->atfcfg_set.grp_num_cfg);
        ret = 1;
    }
...
/**
 * ieee80211_atf_init_ac_val() - Initialize ATF AC config structure
 * @ic: Pointer to the radio structure
 * @index: index to group/vap
 *
 * Return: 0 on SUCCESS
 */
int ieee80211_atf_init_ac_val(struct ieee80211com *ic, int index)
{
    int i = 0;
    struct atf_ac_config *atfac = NULL;

    if (ic->ic_atf_ssidgroup) {
        atfac = &ic->atfcfg_set.atfgroup[index].atf_cfg_ac;
    } else {
        atfac = &ic->atfcfg_set.vap[index].atf_cfg_ac;
    }

    if (!atfac) {
        qdf_print("%s atfac is NULL. ERROR> Check \n",__func__);
        return -EINVAL;
    }
...
/**
 * ieee80211_atf_add_ic_group() - Add new group in radio structure
 * @ic: Pointer to the radio structure
...
int ieee80211_atf_add_ic_group(struct ieee80211com *ic, char *group_name)
{
    ...
    strlcpy(ic->atfcfg_set.atfgroup[groupindex].grpname, group_name, sizeof(ic->atfcfg_set.atfgroup[groupindex].grpname));
    ieee80211_atf_init_ac_val(ic, groupindex);
    ic->atfcfg_set.grp_num_cfg++;
...
/**
 * ieee80211_atf_del_group_list_entry() - Delete group list entry
 * @ic: Pointer to the radio structure
 * @group: pointer to the group list entry to be delete
 *
 * Return: 1 on error
 *         0 on success
 */
int ieee80211_atf_del_group_list_entry(struct ieee80211com *ic, struct group_list *group)
{
...
/**
 * ieee80211_ucfg_setatfssid() - configure SSID based ATF
 * @vap: Pointer to vap structure
 * @val: User configured airtime
 *
 * Return: 1 on error
...
int ieee80211_ucfg_setatfssid(wlan_if_t vap, struct ssid_val *val)
{
    ...
    if (ic->ic_atf_ssidgroup) {
        printk("ATF SSID group is enabled. Cannot configure Airtime for SSID\n");
        return -EFAULT;
    }
    ...
    */
    ...
    if (!val->ssid_exist) {
       QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
               "Airtime configuration applied for a non-existing SSID - %s:%d %%\n\r", val->ssid, (val->value/10));
    }
    ...
    if (cumulative_vap_cfg_value > PER_UNIT_1000) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                " WRONG CUMULATIVE CONFIGURATION VALUE %d, MAX VALUE IS 1000!!!!! \n", cumulative_vap_cfg_value);
        return -EFAULT;
    }
    ...
    if (!ic->ic_atf_ssidgroup) {
        ...
        if (!ieee80211_atf_group_exist(ic, val->ssid, &groupindex)  &&
            !ieee80211_atf_ssid_in_group(ic, val->ssid)) {

            if (ic->atfcfg_set.grp_num_cfg >= ATF_ACTIVED_MAX_ATFGROUPS)
            {
                qdf_print("%s %d grp_num(%d) > MAX_GROUPS(%d) \n",
                        __func__, __LINE__, ic->atfcfg_set.grp_num_cfg, ATF_ACTIVED_MAX_ATFGROUPS);
                return -EINVAL;
            }

            groupindex = ic->atfcfg_set.grp_num_cfg;
            ieee80211_atf_add_ic_group(ic, val->ssid);
            group = ieee80211_atf_new_group_list(ic, val->ssid, groupindex);
            if (group == NULL) {
                ieee80211_atf_del_ic_group(ic, val->ssid, groupindex);
                return 1;
            }

            subgroup = ieee80211_atf_new_subgroup_list(ic, group, NULL, val->value, IEEE80211_ATF_SUBGROUP_TYPE_SSID);
            if(subgroup == NULL) {
                ieee80211_atf_del_ic_group(ic, val->ssid, groupindex);
                ieee80211_atf_del_group_list_entry(ic, group);
                return 1;
            }

            ieee80211_atf_add_ssid_group(ic, val->ssid, groupindex, group);
            ieee80211_atf_config_group_val_sched(ic, val->value, groupindex);
            group->group_airtime = val->value;
        } else {
            /* if the ssid is already part of a Group, modify the airtime */
            ic->atfcfg_set.atfgroup[groupindex].grp_cfg_value = val->value;
            group = ic->atfcfg_set.atfgroup[groupindex].grplist_entry;
            group->group_airtime = val->value;

            subgroup = TAILQ_FIRST(&group->atf_subgroups);
            if(subgroup) {
                subgroup->usr_cfg_val = val->value;
            }
        }
    }
...
/**
 * ieee80211_ucfg_delatfssid() - Delete SSID based ATF
 * @vap: Pointer to vap structure
 * @val: User configured airtime
 *
 * Return: 1 on error
...
int ieee80211_ucfg_delatfssid(wlan_if_t vap, struct ssid_val *val)
{
    ...
    if (i == ATF_CFG_NUM_VDEV) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO," atf The input ssid does not exist\n");
    } else {
        ...
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO," Peer percentage linked to this SSID is cleared.\n");
        ic->atfcfg_set.vap_num_cfg--;
    }
    ...
    if (!ic->ic_atf_ssidgroup) {
        if (ieee80211_atf_group_exist(ic, val->ssid, &groupindex) ) {
            ...
            group = ic->atfcfg_set.atfgroup[groupindex].grplist_entry;
            ieee80211_atf_mark_del_all_sg(ic, group);
            if (ieee80211_atf_ssid_in_group(ic, val->ssid)) {
                ieee80211_atf_del_ssid_group(ic, val->ssid, groupindex);
            }
...
/**
 * ieee80211_atf_verify_sta_cumulative() - cumlative val of peer config
 * Return total val of peer config per vap/group index
 * @ic: Pointer to radio structure
 * @index: vap/group index
 * @val: User configured airtime
 *
 * Return: cumulative value
 */
int ieee80211_atf_verify_sta_cumulative(struct ieee80211com *ic, u_int32_t index, struct sta_val *val)
{
    int i = 0;
    ...
    for (i = 0; i < ATF_ACTIVED_MAX_CLIENTS; i++)
    {
        if (!IEEE80211_ADDR_EQ((char *)(ic->atfcfg_set.peer_id[i].sta_mac), (char *)val->sta_mac))
            cumulative += ic->atfcfg_set.peer_id[i].sta_cfg_value[index + 1];
    }
...
/**
 * ieee80211_ucfg_check_sta_per() - Validate STA configuration
 * @ic: Pointer to the radio structure
 * @val: user configuration
 *
 * Return: 0 on success
 *         ATF_PER_NOT_ALLOWED/ATF_SSID_NOT_EXIST on error
 */
int ieee80211_ucfg_check_sta_per(struct ieee80211com *ic, struct sta_val *val)
{
    u_int8_t  j;
    u_int32_t groupindex = 0;

    if (!val->ssid[0]) {
        qdf_print("%s Peer atf configuration should have an ssid associated \n",__func__);
    ...
    if (ic->ic_atf_ssidgroup) {
        if (ieee80211_atf_group_exist(ic, val->ssid, &groupindex)) {
            if (ieee80211_atf_verify_sta_cumulative(ic, groupindex, val))
                return ATF_PER_NOT_ALLOWED;
        } else {
            qdf_print("Group %s does not exist\n",val->ssid);
            return ATF_SSID_NOT_EXIST;
        }
    } else if(val->ssid[0]) {
        ...
        {
            qdf_print("ATF SSID config doesn't exist for %s. Configure airtime for SSID followed by STA\n",val->ssid);
            return ATF_SSID_NOT_EXIST;
        } else {
            return ieee80211_atf_verify_sta_cumulative(ic, j, val);
        }
    }

    /* allow setting global percentage */
    return 0;
}

/**
 * ieee80211_atf_new_sta_cfg() - Add ATF STA configuration in ic structure
 * @ic: Pointer to the radio structure
 * @val: user configuration
 ...
 *         ATF_PER_NOT_ALLOWED/ATF_SSID_NOT_EXIST on error
 */
int ieee80211_atf_new_sta_cfg(struct ieee80211com *ic, struct sta_val *val, u_int32_t peer_index )
{
    u_int32_t cfg_index = 0;
    u_int8_t global_config = 0;
    u_int64_t calbitmap = 0;

    if  (!val->ssid) {
        global_config = 1;
    } else if (ic->ic_atf_ssidgroup) {
        if (!ieee80211_atf_group_exist(ic, val->ssid, &cfg_index)) {
            qdf_print("%s group %s not found \n",__func__, val->ssid);
            return -1;
        }
        ic->atfcfg_set.peer_id[peer_index].sta_cfg_value[cfg_index + 1] = val->value;
    } else {
        /* adding new configuration */
        if(val->ssid[0])
        {
            for (cfg_index = 0; (cfg_index < ATF_CFG_NUM_VDEV); cfg_index++)
            {
                if (ic->atfcfg_set.vap[cfg_index].cfg_flag)
                {
                    if (strcmp((char *)(ic->atfcfg_set.vap[cfg_index].essid), (char *)(val->ssid)) == 0)
                        break;
                }
            }
            ...
            ic->atfcfg_set.peer_id[peer_index].sta_cfg_value[cfg_index + 1] = val->value;
            ic->atfcfg_set.peer_id[peer_index].index_vap = cfg_index + 1;
        }
    }

    if(global_config) {
        /* storing global configuration */
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,"Peer percentage should have an SSID\n");
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,"Percentage without ssid/group is deprecated \n");
        ic->atfcfg_set.peer_id[peer_index].sta_cfg_value[ATF_CFG_GLOBAL_INDEX] = val->value;
        if (ic->ic_atf_ssidgroup) {
            ic->atfcfg_set.peer_id[peer_index].index_group = 0xff;
        } else {
            ic->atfcfg_set.peer_id[peer_index].index_vap = 0xff;
        }
    }

    ic->atfcfg_set.peer_id[peer_index].sta_cfg_mark = 1;
    ic->atfcfg_set.peer_id[peer_index].sta_assoc_status = 0;
    ic->atfcfg_set.peer_cal_bitmap |= (calbitmap << peer_index);

    return 0;
}

/**
 * ieee80211_atf_modify_sta_cfg() - Modify existing STA based ATF config
 * @ic: Pointer to the radio structure
 * @val: user configuration
 * @peer_index: peer index in ic structure
 *
 ...
 *         -EINVAL on error
 */
int ieee80211_atf_modify_sta_cfg(struct ieee80211com *ic, struct sta_val *val, u_int32_t peer_index )
{
    u_int32_t cfg_index = 0;
    u_int8_t global_config = 0;
...
/**
 * dump_atf_ic_sta () - Debug API to dump ATF STA configuration
 * @ic: Pointer to the radio structure
 *
 * Return: None
 */
void dump_atf_ic_sta(struct ieee80211com *ic)
{
    int i = 0, j = 0;

    qdf_print("************dump_atf_ic_sta START ***************\n");
    for (i = 0; i < 4; i++) {
        qdf_print("entry %d node : %s cfg : %d \n",
                i, ether_sprintf(ic->atfcfg_set.peer_id[i].sta_mac),  ic->atfcfg_set.peer_id[i].sta_cfg_mark);
        for (j= 0; j < ATF_CFG_NUM_VDEV; j++)
            qdf_print(" %d:%d ", j, ic->atfcfg_set.peer_id[i].sta_cfg_value[j]);
        qdf_print("\n");
    }
    qdf_print("************dump_atf_ic_sta END ***************\n");
}

/**
 * ieee80211_ucfg_setatfsta() - configure peer based ATF
 * @vap: pointer to vap structure
 * @sta_val: user configuration
 *
 * Return: 0 on success
 *         -1 on error
 */
int ieee80211_ucfg_setatfsta(wlan_if_t vap, struct sta_val *val)
{
    ...
    switch(ieee80211_ucfg_check_sta_per(ic,val)){
        case ATF_PER_NOT_ALLOWED:
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,"Percentage not allowed as cumulative sum exceeds 100\% \n");
            return -1;
    ...
    if (i == ATF_ACTIVED_MAX_CLIENTS) {
        if (staIndex != 0xff) {
            ...
            ic->atfcfg_set.peer_num_cfg++;
            ic->atfcfg_set.peer_id[i].cfg_flag = 1;
            ieee80211_atf_new_sta_cfg(ic, val, staIndex);
        }
        else
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,"\n STA number over %d \n",ATF_ACTIVED_MAX_CLIENTS);
    } else {
        ieee80211_atf_modify_sta_cfg(ic, val, i);
    }
   ...
   if (ieee80211_atf_group_exist(ic, val->ssid, &groupindex)) {
       ...
       if (!ieee80211_atf_subgroup_exist(ic, group, val->sta_mac, &subgroup, IEEE80211_ATF_SUBGROUP_TYPE_PEER)) {
           subgroup = ieee80211_atf_new_subgroup_list(ic, group, val->sta_mac, val->value, IEEE80211_ATF_SUBGROUP_TYPE_PEER);
           if (subgroup == NULL) {
               ret = -EINVAL;
               goto fail;
           }
       ...
       if (!subgroup) {
           qdf_print("%s %d subgroup is NULL. Something Wrong. Return \n",__func__, __LINE__);
           ret = -EINVAL;
           goto fail;
       }
   } else {
       qdf_print("%s ERROR!!! shouldn't have reached here ieee80211_atf_group_exist : %d ieee80211_atf_ssid_in_group : %d\n",
           __func__, ieee80211_atf_group_exist(ic, val->ssid, &groupindex), ieee80211_atf_ssid_in_group(ic, val->ssid));
   }
...
/**
 * ieee80211_ucfg_delatfsta() - Delete peer based ATF configuration
 * @vap: pointer to vap structure
 * @sta_val: user configuration
 *
 * Return: 0 on success
...
int ieee80211_ucfg_delatfsta(wlan_if_t vap, struct sta_val *val)
{
    ...
    ieee80211_atf_mark_del_subgroup_list(ic, val->sta_mac);
    //dump_atf_group_subgroup(ic);
...
/**
 * ieee80211_atf_init_group() - Initialize group list variable
 * @group: Pointer to the group structure
 *
 * Return: 0 on success
 *         -1 on error
 */
int ieee80211_atf_init_group(struct group_list *group)
{
...
/**
 * ieee80211_atf_get_vap_index() - Get Vap index of ssid 
 * @ic: Pointer to ic structure
 * @ssid: ssid name
 *
 * Return: 0 on success
 *         -1 on error
 */
int ieee80211_atf_get_vap_index(struct ieee80211com *ic, u_int8_t *ssid)
{
    int vap_index = 0, num_vaps = 0;
    ...
    for (vap_index = 0; (vap_index < ATF_CFG_NUM_VDEV) && (num_vaps != 0); vap_index++)
    {
        if(ic->atfcfg_set.vap[vap_index].cfg_flag)
        {
            if (!strcmp((char *)(ic->atfcfg_set.vap[vap_index].essid), (char *)ssid))
            {
                return vap_index;
            }
            num_vaps--;
...
/**
 * ieee80211_ucfg_delatfac() - Delete ATF AC configuration
 * @vap: pointer to vap
 ...
 *         -EINVAL on error
 */
int ieee80211_ucfg_delatfac(wlan_if_t vap, struct atf_ac *buf)
{
    struct ieee80211com *ic = vap->iv_ic;
    int groupindex = 0, vapindex = 0;
    struct group_list *group = NULL;
    struct subgroup_list *subgroup = NULL;
    int ret = 0;
    ...
    if (!ieee80211_atf_group_exist(ic, buf->ac_ssid, &groupindex) ) {
        qdf_print("%s group %s Doesn't exist. Invalid Configuration \n",__func__, buf->ac_ssid);
        return -EINVAL;
    }

    if (buf->ac_id > (WME_NUM_AC - 1)) {
        qdf_print("%s invalid ac_id : %d \n",__func__, buf->ac_id);
        return -EINVAL;
    }

    /* Clear AC val in ic config structure */
    if (ic->ic_atf_ssidgroup) {
        atf_ac = &ic->atfcfg_set.atfgroup[groupindex].atf_cfg_ac;
    } else {
        vapindex = ieee80211_atf_get_vap_index(ic, buf->ac_ssid);
        if (vapindex < 0) {
            qdf_print("%s Vap index could not be found. Return Error \n",__func__);
            return -EINVAL;
        }
        atf_ac = &ic->atfcfg_set.vap[vapindex].atf_cfg_ac;
    }

    if (!atf_ac) {
        qdf_print("%s %d atf_ac is NULL. Return \n",__func__, __LINE__);
        return -EINVAL;
    }
    ...
    group = ic->atfcfg_set.atfgroup[groupindex].grplist_entry;
    if (!group) {
        qdf_print("%s The group is null. Returning \n",__func__);
        ret = -EINVAL;
        goto fail;
    }

    /* Mark subgroup for deletion */
    if (ieee80211_atf_subgroup_exist(ic, group, &buf->ac_id, &subgroup, IEEE80211_ATF_SUBGROUP_TYPE_AC)) {
        subgroup->sg_del = 1;
    } else {
        qdf_print("%s AC subgroup Doesn't exist. Cannot Delete\n",__func__);
        ret = -EINVAL;
    }
...
/**
 * ieee80211_atf_validate_acval() - Validate ATF AC configuration val
 * @atf_ac: pointer to atf ac config struct
 * @ac_id: AC id
 ...
 *         -EINVAL on error
 */
int ieee80211_atf_validate_acval(struct atf_ac_config *atf_ac, int8_t ac_id, u_int32_t ac_val)
{
    int i = 0;
    int ac_cumulative = 0;
    ...
    if (ac_cumulative + ac_val > PER_UNIT_1000) {
        qdf_print("%s Total Airtime for ACs %d > 1000 . Error \n",__func__, ac_cumulative + ac_val);
        return -1;
    }
...
/**
 * ieee80211_ucfg_addatfac() - Add ATF AC configuration
 * @vap: pointer to vap
 * @buf: user configuration
 *
 * Return: 0 on success
 *         -EINVAL on error
 */
int ieee80211_ucfg_addatfac(wlan_if_t vap, struct atf_ac *buf)
{
    struct ieee80211com *ic = vap->iv_ic;
    int groupindex = 0, vapindex = 0;
    struct group_list *group = NULL;
    ...
    /* AC can be configured only after SSID or GROUP is configured */
    if (!ieee80211_atf_group_exist(ic, buf->ac_ssid, &groupindex) ) {
        qdf_print("ATF SSID config doesn't exist for %s. Configure airtime for SSID followed by AC\n",buf->ac_ssid);
        return -EINVAL;
    }
    ...
    if (ieee80211_atf_validate_acval(atf_ac, buf->ac_id, buf->ac_val) < 0) {
        qdf_print("Total Airtime assigned to AC's in a group/ssid cannot exceed 1000\n");
        return -EINVAL;
    }
    ...
    if (!ieee80211_atf_subgroup_exist(ic, group, &buf->ac_id, &subgroup, IEEE80211_ATF_SUBGROUP_TYPE_AC)) {
        subgroup = ieee80211_atf_new_subgroup_list(ic, group, &buf->ac_id, buf->ac_val, IEEE80211_ATF_SUBGROUP_TYPE_AC);
        if (subgroup == NULL) {
            qdf_print("%s %d subgroup is NULL. Returning \n",__func__, __LINE__);
            ret = -EINVAL;
            goto fail;
        }
    } else {
...
/**
 * ieee80211_ucfg_configatfgroup() - Configure airtime for atfgroup
 * @vap: vap structure
 * @buf: user configuration
 * @airtime_group: Total airtime configured for all groups
 *
 * Return: 0 on success
 *         -EINVAL on error
 */
int ieee80211_ucfg_configatfgroup(wlan_if_t vap, struct atf_group *buf, u_int32_t airtime_group)
{
    struct ieee80211com *ic = vap->iv_ic;
    ...
    for (i = 0; i < ic->atfcfg_set.grp_num_cfg; i++) {
        ...
        if ((strlen(ic->atfcfg_set.atfgroup[i].grpname) == strlen(buf->name)) &&
            ...
            if ((airtime_group -  ic->atfcfg_set.atfgroup[i].grp_cfg_value + buf->value) <= availableairtime ) {
                ieee80211_atf_config_group_val_sched(ic, buf->value, i);
                if (!ic->ic_is_mode_offload(ic)) {
                ...
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                       "Error!! Total Airtime configured for groups cannot be greater than %d \n\r", availableairtime/10);
                error = 1;
            }
            break;
        }
...
/**
 * ieee80211_ucfg_atfgroupsched() - Configure airtime for atfgroup
 * @vap: vap structure
 * @buf: user configuration
 *
 * Return: 0 on success
 *         -EINVAL on error
 */
int ieee80211_ucfg_atfgroupsched(wlan_if_t vap, struct atf_group *buf)
{
    struct ieee80211com *ic = vap->iv_ic;
    u_int32_t error = 0, i = 0;
    struct group_list *group = NULL;
    ...
    for (i = 0; i < ic->atfcfg_set.grp_num_cfg; i++) {
        ...
        if ((strlen(ic->atfcfg_set.atfgroup[i].grpname) == strlen(buf->name)) &&
            !(strncmp( (char *)(ic->atfcfg_set.atfgroup[i].grpname), (char *)(buf->name), strlen(buf->name))) )  {
            ic->atfcfg_set.atfgroup[i].grp_sched_policy = buf->value;
               ...
               if (group != NULL) {
                ...
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                        "Error!! Not a valid scheduling Policy\n");
                error = 1;
            }
            break;
        }
...
/**
 * ieee80211_ucfg_addatfgroup() - Add ATF group
 * @vap: pointer to vap
 * @buf: user configuration
 *
 * Return: 0 on success
 *         -EINVAL on error
 */
int ieee80211_ucfg_addatfgroup(wlan_if_t vap, struct atf_group *buf)
{
    struct ieee80211com *ic = vap->iv_ic;
    u_int32_t groupindex = 0, error =0;
    u_int8_t addssid = 1, addgroup = 1;
    struct group_list *group = NULL;
    struct subgroup_list *subgroup = NULL;

    if (ieee80211_atf_is_default_group(buf->name)) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                "%s: Group name cannot be %s \n\r",__func__, buf->name);
        return -EINVAL;
    }

    if (ieee80211_atf_group_exist(ic, buf->name, &groupindex)) {
        addgroup = 0;
        group = ic->atfcfg_set.atfgroup[groupindex].grplist_entry;
    }

    if (ieee80211_atf_ssid_in_group(ic, buf->ssid)) {
        addssid = 0;
        error = 1;
    }
    ...
    if (addgroup) {
        if (ic->atfcfg_set.grp_num_cfg >= ATF_ACTIVED_MAX_ATFGROUPS) {
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                            "Error!Cannot configure more than %d groups\n\r", ATF_ACTIVED_MAX_ATFGROUPS);
            kfree(buf);
            return -EINVAL;
        }
        groupindex = ic->atfcfg_set.grp_num_cfg;
        ieee80211_atf_add_ic_group(ic, buf->name);
        group = ieee80211_atf_new_group_list(ic, buf->name, groupindex);
        if (group == NULL) {
            ieee80211_atf_del_ic_group(ic, buf->name, groupindex);
            qdf_print("%s %d Failed to add group \n",__func__, __LINE__);
            return 1;
        }

        ieee80211_atf_init_group(group);
        subgroup = ieee80211_atf_new_subgroup_list(ic, group, NULL, buf->value, IEEE80211_ATF_SUBGROUP_TYPE_SSID);
        if (subgroup == NULL) {
            /* Delete group entry if subgroup could not be added */
            qdf_print("%s %d Failed to add group \n",__func__, __LINE__);
            ieee80211_atf_del_ic_group(ic, buf->name, groupindex);
            ieee80211_atf_del_group_list_entry(ic, group);
            return 1;
    ...
    if (addssid) {
        ieee80211_atf_add_ssid_group(ic, buf->ssid, groupindex, group);
    }
...
/**
 * ieee80211_ucfg_delatfgroup() - Delete ATF group
 * @vap: pointer to vap
 * @buf: user configuration
 *
 * Return: 0 on success
 *         -EINVAL on error
 */
int ieee80211_ucfg_delatfgroup(wlan_if_t vap, struct atf_group *buf)
{
    struct ieee80211com *ic = vap->iv_ic;
    struct group_list *group = NULL;
    u_int32_t groupindex = 0;
    ...
    if (!ieee80211_atf_group_exist(ic, buf->name, &groupindex)) {
        qdf_print("%s group : %s not found. Return \n",__func__, buf->name);
        return -1;
    }

    group = ic->atfcfg_set.atfgroup[groupindex].grplist_entry;
    ieee80211_atf_del_ic_group(ic, buf->name, groupindex);
    ieee80211_atf_mark_del_group_list(ic, group);
...
int ieee80211_rep_datarate_estimator(u_int16_t backhaul_rate,
        u_int16_t ap_rate,
        u_int8_t root_distance,
...
int ieee80211_get_bssid_info(void *arg, wlan_scan_entry_t se)
{
    ...
    if((se_ssid_len == 0) ||
            (bestUL->in.essid_len != se_ssid_len) ||
            OS_MEMCMP(se_ssid, bestUL->in.essid, se_ssid_len) != 0)
        return 0;
    ...
    if(se_rootdistance == 0) {
        ...
        tmprate = ieee80211_rep_datarate_estimator(se_uplinkrate,
                se_currentrate,
                se_rootdistance,
                ic->ic_whc_scaling_factor);
    }
...
int ieee80211_ucfg_find_best_uplink_bssid(wlan_if_t vap, char *bssid)
{
    ...
    bestUL.in.essid_len = vap->iv_des_ssid[0].len;
    ...
    max_rate = ieee80211_ucfg_get_maxphyrate(vap) / 1000;
    current_rate = ieee80211_rep_datarate_estimator(ic->ic_serving_ap_backhaul_rate,
            ic->ic_uplink_rate,
            (vap->iv_whc_root_ap_distance - 1),
            ic->ic_whc_scaling_factor);
    bestUL_hyst = ic->ic_bestUL_hyst ? ic->ic_bestUL_hyst : BEST_UL_HYST_DEF;
    ...
    if(wlan_scan_table_iterate(vap, ieee80211_get_bssid_info, &bestUL) == 0)
    {
        ...
        {
            if (bestUL.rate_estimate < (current_rate + ((max_rate * bestUL_hyst) / 100)))
            {
...
int ieee80211_get_cap_bssid(void *arg, wlan_scan_entry_t se)
{
    ...
    /* make sure ssid is same */
    if((se_ssid_len == 0) ||
            (rootinfo->essid_len != se_ssid_len) ||
            OS_MEMCMP(se_ssid, rootinfo->essid, se_ssid_len) != 0)
        return 0;
...
int ieee80211_ucfg_find_cap_bssid(wlan_if_t vap, char *bssid)
{
    ...
    rootinfo.essid_len = vap->iv_des_ssid[0].len;
...
int ieee80211_get_otherband_uplink_bssid(void *arg, wlan_scan_entry_t se)
{
    ...
    /* make sure ssid is same */
    if((se_ssid_len == 0) ||
            (uplink->essid_len != se_ssid_len) ||
            OS_MEMCMP(se_ssid, uplink->essid, se_ssid_len) != 0)
        return 0;
...
int ieee80211_ucfg_get_otherband_uplink_bssid(wlan_if_t vap, char *bssid)
{
    ...
    uplink.essid_len = vap->iv_des_ssid[0].len;
