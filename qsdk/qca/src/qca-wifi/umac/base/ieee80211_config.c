...
wlan_set_param(wlan_if_t vaphandle, ieee80211_param param, u_int32_t val)
{
    ...
    if ((ic->ic_curchan == NULL) || (ic->ic_curchan == IEEE80211_CHAN_ANYC)) {
        qdf_print("%s: Channel Pointer is invalid\n",__func__);
        return -EINVAL;
    }
	...
	case IEEE80211_ME:
        if (vap->iv_me) {
            if(val  == MC_HYFI_ENABLE) {
               ...
               return -EINVAL;
            } else if(val == MC_SNOOP_DISABLE_CMD) {
               QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Please use value 0 to disbale snooping  \n");
               return -EINVAL;
            } else {
    ...
    case IEEE80211_SEND_DEAUTH:
            ...
            break;
    case IEEE80211_STEALTH_DOWN:
            vap->iv_stealth_down = val;
            break;
    ...
    case IEEE80211_FIXED_NSS:
         ...
         /*For offload vap->iv_nss set based on return value of
           ic_vap_set_param() of wmi_vdev_param_nss  */
    break;
...
#undef DEFAULT_MIN_SCAN_DWELL_TIME
    ...
    case IEEE80211_ATF_SSID_SCHED_POLICY:
        if (vap->iv_opmode == IEEE80211_M_HOSTAP) {
            if (val < 0 || val >= IEEE80211_ATF_GROUP_NUM_SCHED_POLICIES) {
                IEEE80211_DPRINTF(vap, IEEE80211_MSG_IOCTL, "Invalid Sched Policy.\n \
                        set 0 for 'Fair', 1 for 'Strict' or '2' for 'fair with Upper bound'\n");
                return -EINVAL;
            }

            if (ic->ic_atf_ssidgroup) {
                IEEE80211_DPRINTF(vap, IEEE80211_MSG_IOCTL, "Cannot configure SSID sched policy when grouping is enabled\n");
                return -EINVAL;
            }

            vap->iv_vap_atf_sched = val;
            ieee80211_atf_group_vap_sched(vap, val);
            if (ic->ic_is_mode_offload(ic)){
    ...
    case IEEE80211_FEATURE_SON:
        if (val) {
            ...
            /* When SON mode is enabled, disable same ssid feature support */
            GLOBAL_IC_LOCK(ic->ic_global_list);
            if (ic->ic_global_list->same_ssid_support == 1) {
...
#undef MAX_TRAFFIC_INTERVAL
    ...
    case IEEE80211_FEATURE_FILS:
        if ( val & (1 << IEEE80211_FILS_ENABLE_BIT) ) {
            qdf_print("Enable FILS\n");
            IEEE80211_VAP_FILS_ENABLE(vap);
        } else {
            qdf_print("Disable FILS\n");
    ...
    case IEEE80211_SUPPORT_RSN_OVERRIDE:
        ...
        break;
    case IEEE80211_DROP_MDNS:
        vap->iv_drop_mdns = val;
        break;
    default:
        break;
    }
    ...
    if (ic->ic_vap_set_param) {
         ...
         */
        if (EOK != retv) {
            int is_up = ((ieee80211_vap_active_is_set(vap)) || (ieee80211_vap_dfswait_is_set(vap)));
            switch (param) {
                case IEEE80211_MCAST_RATE:
                    if (is_up)
...
wlan_get_param(wlan_if_t vaphandle, ieee80211_param param)
{
    ...
    struct ieee80211com *ic = vap->iv_ic;
    u_int32_t val = 0;
    struct ieee80211_mib_cycle_cnts *pcnts;
   ...
   case IEEE80211_CONFIG_RDG_ENABLE:
    ...
    case IEEE80211_GET_VAP_ACTIVITY:
        pcnts = (struct ieee80211_mib_cycle_cnts *)qdf_mem_malloc(
                                    sizeof(struct ieee80211_mib_cycle_cnts));
        if (pcnts) {
           ic->ic_rmgetcounters(ic, pcnts);
            val = ((pcnts->rx_clear_count - (pcnts->rx_frame_count + pcnts->tx_frame_count)) * 100) / pcnts->cycle_count;
        }
        qdf_mem_free(pcnts);
        break;
...
/*
 * Get the cfr capture params
 * @param vaphandle  Vap interface handle
 * @param macaddress Mac Address of the sta
 * @param cfrinfo cfr params to get for the particular station.
 */
int wlan_get_cfr_capture(wlan_if_t vaphandle, u_int8_t *macaddr, ieee80211_cfr_param_info *cfrinfo)
{
    struct ieee80211vap *vap = vaphandle;
    struct ieee80211com *ic = vap->iv_ic;
    struct ol_ath_softc_net80211 *scn = OL_ATH_SOFTC_NET80211(ic);
    ...
    if(!ic->ic_is_mode_offload(ic) || !scn->cfr_support) {
        qdf_print("CFR is not supported on this Chip\n");
        return -EINVAL;
    }
    ...
    if (ni == NULL) {
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_OUTPUT,
                          "%s: could not get cfr param, no node found for %s\n",
                          __func__, ether_sprintf(macaddr));
        return -EINVAL;
    }

    cfrinfo->cfr_request = ni->ni_cfr.request;
    cfrinfo->cfr_bw      = ni->ni_cfr.bw;
    cfrinfo->cfr_period  = ni->ni_cfr.period;
    cfrinfo->cfr_method  = ni->ni_cfr.method;
...
/*
 * Set the cfr capture params
 * @param vaphandle  Vap interface handle
 * @param macaddress Mac Address of the sta
 * @param cfrinfo cfr params to be set for the particular station.
 */
int wlan_set_cfr_capture(wlan_if_t vaphandle, const u_int8_t *macaddr, ieee80211_cfr_param_info *cfrinfo)
{
    struct ieee80211vap *vap = vaphandle;
    struct ieee80211com *ic = vap->iv_ic;
    struct ol_ath_softc_net80211 *scn = OL_ATH_SOFTC_NET80211(ic);
    struct ieee80211_node *ni = NULL;
    ni_cfr_param_t *pointer = NULL;
    int retv = 0;
    ...
    if (ni == NULL) {
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_OUTPUT,
                          "%s: could not set cfr param, no node found for %s\n",
                          __func__, ether_sprintf(macaddr));
        return -EINVAL;
    }
    pointer = (ni_cfr_param_t *)(&ni->ni_cfr);

    if (!cfrinfo->cfr_request) {
        pointer->request = PEER_CFR_CAPTURE_DISABLE;
        if (pointer->period)
            ic->ic_cfr_max_clients--;
        else {
            ieee80211_free_node(ni);
            return -EINVAL;
        }
    } else {
        if (cfrinfo->cfr_bw > ni->ni_chwidth) {
            ieee80211_free_node(ni);
            return -EINVAL;
        }

        if ((cfrinfo->cfr_period < 0) || (cfrinfo->cfr_period > MAX_CFR_PRD) || (cfrinfo->cfr_period % 10)
                     || ((cfrinfo->cfr_period == 0) && (ic->ic_cfr_timer_enable))) {
            ieee80211_free_node(ni);
            return -EINVAL;
        }

        if (cfrinfo->cfr_method >= cfr_method_max) {
            ieee80211_free_node(ni);
            return -EINVAL;
        }

        if (cfrinfo->cfr_period == 0)
            pointer->period = PEER_CFR_ONE_SHOT_REQUEST;
        else {
            if (ic->ic_cfr_max_clients >= MAX_CFR_CLIENTS) {
                ieee80211_free_node(ni);
                return -EINVAL;
            }
            ic->ic_cfr_max_clients++;
            pointer->period = cfrinfo->cfr_period;
        }
        pointer->request = PEER_CFR_CAPTURE_ENABLE;
        pointer->bw = cfrinfo->cfr_bw;
        pointer->method = cfrinfo->cfr_method;
    }

    if (ic->ic_node_cfr_capture)
        retv = ic->ic_node_cfr_capture(ni);
                          ...
                          ieee80211_ssid *ssidlist)
{
    ...
    for (i=0; i < MAX_RADIO_CNT; i++) {
        ...
        if (tmp_ic) {
            TAILQ_FOREACH(tmpvap, &tmp_ic->ic_vaps, iv_next) {
                if (IEEE80211_VAP_IS_SON_ENABLED(tmpvap)) {
                    son_enabled = 1;
                    break;
                }
            }
    ...
    for (i = 0; i < nssid; i++) {
        ...
        if (vap->iv_des_ssid[i].len) {
            OS_MEMZERO(vap->iv_des_ssid[i].ssid, sizeof(vap->iv_des_ssid[i].ssid));
		...
		if (ret) {
                    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Same ssid support enabled ssid:%s len:%d\n",vap->iv_des_ssid[i].ssid,vap->iv_des_ssid[i].len);
		    GLOBAL_IC_LOCK(ic_list);
...
#if ATH_SUPPORT_NAC_RSSI
{
    ...
    if ( is_nac_valid_mac(vap_nac_rssi->bssid_mac)){
        ...
        printk("%s:nac_rssi bssid configured \n", __func__);
        return NAC_STATUS_FAILED_ADD_OR_DEL;
    }
}
#endif
...
static int wlan_nac_rssi_add_mac(wlan_if_t vap, char *bssid_macaddr, char *client_macaddr, u_int8_t  chan_num, int *chagrchan)
{
    ...
    *chagrchan = 0;
    ...
    {
        if ((c->ic_freq - 5000)/5 !=  chan_num) /*home channel is same as configured channel number, no need to switch channel*/
        {
            *chagrchan = 1;
        }
    }
    ...
    {
        if ((c->ic_freq - 2407)/5 !=  chan_num) /*home channel is same as configured channel number, no need to switch channel*/
        {
            *chagrchan = 1;
        }
    }
    ...
    if ((IEEE80211_ADDR_EQ(vap_nac_rssi_info->bssid_mac, nullmac)) && (IEEE80211_ADDR_EQ(vap_nac_rssi_info->client_mac, nullmac)))
    {
        ...
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_IOCTL, "%s  Added BSSID Bytes[0][5]=%2x %2x client Bytes[0][5]=%2x %2x \n", __func__,
        vap_nac_rssi_info->bssid_mac[0], vap_nac_rssi_info->bssid_mac[5], vap_nac_rssi_info->client_mac[0], vap_nac_rssi_info->client_mac[5],chan_num);

        /* Fill in respective handler and  send to target for adding bssid  */
        if(vap->iv_scan_nac_rssi) {
            /* Bssid idx starts from 1 in target */
            vap->iv_scan_nac_rssi(vap , IEEE80211_NAC_RSSI_PARAM_ADD, bssid_macaddr, client_macaddr, chan_num);
        }
    }
    else if ( !(IEEE80211_ADDR_EQ(vap_nac_rssi_info->bssid_mac, bssid_macaddr)) || !(IEEE80211_ADDR_EQ(vap_nac_rssi_info->client_mac, client_macaddr))) {

        printk("%s:Existed entry: BSSID byte[0][5]=%2x:%2x  Client byte[0][5]=%2x:%2x  \n", __func__,
                vap_nac_rssi_info->bssid_mac[0], vap_nac_rssi_info->bssid_mac[5],
...
static int wlan_nac_rssi_del_mac(wlan_if_t vap, char *bssid_macaddr, char *client_macaddr)
{
    ...
    IEEE80211_DPRINTF(vap, IEEE80211_MSG_IOCTL, "%s BSSID Bytes[0][5]=%2x %2x client Bytes[0][5]=%2x %2x removed \n", __func__,
                          bssid_macaddr[0], bssid_macaddr[5], client_macaddr[0], client_macaddr[5]);
     /* Fill in respective handler and  send to target for deleting bssid  */
     if(vap->iv_scan_nac_rssi ) {
         /* delete BSSID in target*/
         vap->iv_scan_nac_rssi(vap , IEEE80211_NAC_RSSI_PARAM_DEL, bssid_macaddr, client_macaddr, 0);
     }
    return 0;
}
...
int wlan_nac_rssi_chanchgr(wlan_if_t vap, u_int8_t  chan_num)
{
    ...
    if (wlan_acs_set_user_chanlist(vap,list.chan) != EOK) {
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_NORMAL, IEEE80211_MSG_EXTIOCTL_CHANSSCAN, "%s Failed to set user channel after ", __func__);
        return -EINVAL;
    }
...
int wlan_set_nac_rssi(wlan_if_t vaphandle, enum ieee80211_nac_param param, void *in_nac)
{
    ...
    switch(param) {
        case IEEE80211_NAC_RSSI_PARAM_ADD:
            ...
            if (! (((nac_rssi->chan_num >= 30) && (nac_rssi->chan_num <= 165)) || ((nac_rssi->chan_num >= 1) && (nac_rssi->chan_num <= 14)))){

                printk("%s:chan num should be between 1~14, or 30~165 \n", __func__);
                return -1;
	        }
            ret = wlan_nac_rssi_add_mac(vap, nac_rssi->mac_bssid, nac_rssi->mac_client, nac_rssi->chan_num, &changeChannel);
...
int wlan_list_nac_rssi(wlan_if_t vaphandle, enum ieee80211_nac_param param, void *in_nac) {
    ...
    if(vap->iv_scan_nac_rssi) {
         /* Bssid idx starts from 1 in target */
         vap->iv_scan_nac_rssi(vap , IEEE80211_NAC_RSSI_PARAM_LIST, vap_nac_rssi_info->bssid_mac, vap_nac_rssi_info->client_mac, 0);
    }
