...
#if ATH_SUPPORT_WIFIPOS_ONE_VAP
{
    ...
    else
    {
		...
		qdf_hrtimer_init(&vap->bpr_timer, ieee80211_send_bcast_resp_handler, QDF_CLOCK_MONOTONIC, QDF_HRTIMER_MODE_REL, QDF_CONTEXT_TASKLET);
        }
#endif

    vap->offchan_requestor = 0;

    qdf_nbuf_queue_init(&vap->iv_mgmt_offchan_txnbufqueue);
...
ieee80211_vap_detach(struct ieee80211vap *vap)
{
    ...
    ieee80211_vap_cancel_supplicant_state_timer(vap);
        ...
        if (MBSSID_BINTVAL_CHECK(ic->ic_intval, ic->ic_num_ap_vaps, scn->bcn_mode)) {
            ...
            if(ic->ic_num_ap_vaps <= IEEE80211_BINTVAL_VAPCOUNT1){
                ...
                if(scn->bcn_mode == 1) { //burst mode enabled so limit the beacon interval value to 100
                    ...
                    /* burst mode disabled (staggered) ,
                     * limit the beacon interval to 200 for Offload chipsets
                     * But for DA chipsets limit the beacon interval to 400ms.
                     */
                    if (!(ic->ic_is_mode_offload(ic))) {
                        ic->ic_intval = IEEE80211_BINTVAL_MINVAL_RANGE4;
                    } else {
                ...
                void                   *osifp_handle)
{
    ...
    /* Initialize the reject_mgmt array */
    for (i = 0; i < IEEE80211_REJECT_MGMT_MAX; i++) {
        qdf_mem_set(vap->iv_conf_rej_mgmt[i].addr, IEEE80211_ADDR_LEN, 0xff);
    }
...
void ic_reset_params(struct ieee80211com *ic)
{
    ...
    ic->ic_cfr_timer_enable = 0;
    ic->ic_cfr_max_clients = 0;
    ...
    ic->ic_curchan = ieee80211_get_channel(ic, 0);
...
{
    ieee80211_reset_radar_event_log_counter(ic);
    ic->ic_stop(ic, 1);
    ic_reset_params(ic);
...
u_int16_t wlan_get_uplinkrate(wlan_if_t vaphandle, u_int8_t snr)
{
    ...
    if((chwidth == wlan_chwidth_160) &&
        ...
        if(vap->iv_bss->ni_ext_nss_support && vap->iv_bss->ni_bw160_nss)
            serving_ap_nss = vap->iv_bss->ni_bw160_nss;
        else
