...
ieee80211_ieee2mhz(struct ieee80211com *ic, u_int chan, u_int flags)
{
    ...
    c = ic->ic_curchan;
    if ((c != NULL) && (c != IEEE80211_CHAN_ANYC) && (c->ic_ieee == chan)) {
        return c->ic_freq;
    }
    ...
    for (i = 0; i < ic->ic_nchans; i++) {
        ...
        if (mode == IEEE80211_MODE_AUTO) {
            ...
            if (IEEE80211_IS_CHAN_2GHZ(c)) {
                 ...
                 * 11NG to maintain the same precedence as above.
                 */
             ...
             */
            if(IS_CHAN_IN_PUBLIC_SAFETY_BAND(c->ic_freq)) {
...
bool ieee80211_is_skip_curchan_needed(struct ieee80211com *ic,
        struct ieee80211_ath_channel *channel,
        struct ieee80211_channel_list *chan_info)
{
    int i, j;
    struct ieee80211_channel_list tmp_chan_info;

    ieee80211_get_extchaninfo(ic, channel, &tmp_chan_info);

    for (i = 0; (i < chan_info->cl_nchans) && chan_info->cl_channels[i]; i++)
        for (j = 0; (j < tmp_chan_info.cl_nchans) && (tmp_chan_info.cl_channels[j]); j++)
            if(chan_info->cl_channels[i]->ic_freq == tmp_chan_info.cl_channels[j]->ic_freq)
                return true;

    return false;
}

static int find_alternate_mode_channel(struct ieee80211com *ic,
        u_int32_t alt_chan_mode, int chan_count, u_int8_t skip_curchan)
{
   ...
   /* find n-th valid channel in alt_chan_mode */
    ...
    for (i = 0; i < ic->ic_nchans; i++) {
        ...
        if(skip_curchan)
        {
            ...
            ret_val = ieee80211_is_skip_curchan_needed(ic, &ic->ic_channels[i], &chan_info);
            if (ret_val)
                continue;
        }
...
int ieee80211_random_channel(struct ieee80211com *ic, u_int8_t is_select_nondfs, u_int8_t skip_curchan, bool use_des_chanflags)
{
    int chanStart,n=0;
    ...
    int       chan_count    = 0;
    struct ieee80211_ath_channel *tmp_chan = NULL;

    osdev_t dev = ic->ic_osdev;
    if ((available_chan_idx = OS_MALLOC(ic->ic_osdev, (IEEE80211_CHAN_MAX+1) * sizeof(int) , GFP_KERNEL)) == NULL) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: cannot allocate memory\n", __func__);
        return ret_val;
    }
    ...
    * the list, followed by all A channels
    ...
    /* In case of spoof failure the ic_curchan->ic_flags has already been reset
     * to zero. Thus we are reading from a copy (ic_curchan_flags) to get
     * the original channel flags to ensure that new random channel to be
     * chosen is picked up with the same user configured mode after spoof
     * test failure.
     */
    if (ic->ic_spoof_dfs_check_failed) {
       curChanFlags = ic->ic_curchan_flags;
    } else {
    ...
    if (use_des_chanflags) {
        curChanFlags = (ic->ic_des_chan_flags) & IEEE80211_CHAN_ALL;
    }
#endif
    ...
    for (n = 0; n < ic->ic_nchans; chanStart++, n++)
	{
        ...
        {
            /* Skip curchan when choosing apriori random channel */
            ret_val = ieee80211_is_skip_curchan_needed(ic, &ic->ic_channels[chanStart], &chan_info);
            if (ret_val)
                continue;
        }
#if ATH_SUPPORT_PRIMARY_ALLOWED_CHAN
        if(ic->ic_primary_allowed_enable &&
                !ieee80211_check_allowed_prim_chanlist(
                    ic, ic->ic_channels[chanStart].ic_ieee)) {
            continue;
        }
#endif
...
#undef ADJACENT_WEATHER_RADAR_CHANNEL
        ...
        tmp_chan = &ic->ic_channels[chanStart];
        if ((chan_flags & IEEE80211_CHAN_STURBO) ||
            ...
            (chan_flagext & IEEE80211_CHAN_DFS &&
             ic->ic_flags_ext & IEEE80211_FEXT_BLKDFSCHAN )||
            ((IEEE80211_IS_CHAN_DFS(tmp_chan) ||
             ((IEEE80211_IS_CHAN_11AC_VHT160(tmp_chan) ||
               IEEE80211_IS_CHAN_11AC_VHT80_80(tmp_chan)) &&
              IEEE80211_IS_CHAN_DFS_CFREQ2(tmp_chan))) && is_select_nondfs))
            continue;
    ...
    if(available_chan_count)
    {
        ...
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d] idx %d chan %d flags 0x%08x\n", __func__, __LINE__,
                available_chan_idx[n], ic->ic_channels[available_chan_idx[n]].ic_ieee, ic->ic_channels[available_chan_idx[n]].ic_flags);
        }
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d] random %d avail_count %d rand_idx %d\n", __func__, __LINE__, random_byte, available_chan_count, j);
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d] Selected chan %d flags 0x%08x chanStart %d \n", __func__, __LINE__,
        ...
        if (chan_count) {
            ret_val = find_alternate_mode_channel(ic, alt_chan_mode, chan_count, skip_curchan);
            if (ret_val == -1) {
...
ieee80211_update_channellist(struct ieee80211com *ic, int exclude_11d)
{
    ...
    if( (exclude_11d) && (ic->ic_curchan != NULL)) {
        ...
        if( (i >= ic->ic_nchans) && (first_valid_c != NULL))
...
wlan_set_desired_phymode(wlan_if_t vaphandle, enum ieee80211_phymode mode)
{
    ...
    if (!IEEE80211_SUPPORT_PHY_MODE(vap->iv_ic, mode)) {
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Rejecting mode %d: This mode may be invalid or may be unavailable for the requested values of parameters such as channel and chainmasks\n", mode);
            return -EINVAL;
    }
    ...
    if (chan != IEEE80211_CHAN_ANYC) {
        ...
        /* This NULL check is needed in case of STA vap if it fails to find a channel with
         * vap's desired mode.
         */
        if ( chan != NULL) {
...
#if ATH_SUPPORT_DFS && QCA_DFS_NOL_VAP_RESTART
    if ((chan != IEEE80211_CHAN_ANYC) && chan) {
        ic->ic_des_chan_flags = chan->ic_flags;
    }
#endif
    return 0;
}
...
wlan_get_current_channel(wlan_if_t vaphandle, bool hwChan)
{
    ...
    } else if (ic->ic_is_host_dfs_check_enabled &&
            ic->ic_spoof_dfs_check_failed == 1 &&
            DFS_FCC_DOMAIN == ic->ic_get_dfsdomain(ic)
            && ieee80211_get_num_vaps_up(ic)) {
...
* @param channel    handle to ieee80211_ath_channel
*
 ...
 *
 * @param p1: lower bound value
 * @param p2: upper bound value
 * @param cp: value to be checked
 *
 * @return 1 if cp within p1 and p2
 */
int
...
/*
 * Check whether given frequency centre/channel width entry
 * overlap with frequency spread in any way.
 *
 * @param f1       :   current channel centre frequency.
 * @param w        :   current channel width
 * @param f2_l     :   overlap frequency lower bound
 * @param f2_h     :   overlap frequency upper bound
 *
 * @return 1 if overlap found, else returns 0
 */
int
ol_check_etsi_overlap(int f1,int w,int f2_l, int f2_h)
{
	int f1_l;
	int f1_h;
	int ret;
...
/*
 * Check whether current channel frequecy spread overlaps with
 * EN 302 502 radar type freqiemcy range.
 *
 * @param ic: ic structure to reference centre frequency of current channel
 * @return 1 if overlap present, else return 0
 */
int
ol_is_en302_502_applicable(struct ieee80211com *ic)
{
	int chan_freq;
	int chan_width;
	int overlap = 0;
	...
	chan_freq = ieee80211_get_chan_centre_freq(ic, ic->ic_curchan);

	/* Grab width */
	chan_width = ieee80211_get_chan_width(ic->ic_curchan);

	if (IEEE80211_IS_CHAN_11AC_VHT80_80 (ic->ic_curchan)) {
		/* HT80_80 mode has 2 segments and each segment must
		 * be checked for control channel first.
		 */
		overlap = ol_check_etsi_overlap(chan_freq, chan_width/2,
						ETSI_RADAR_EN302_502_FREQ_LOWER,
						ETSI_RADAR_EN302_502_FREQ_UPPER);

		/* check for extension channel */
		chan_freq = ieee80211_ieee2mhz(ic,
		...
		overlap = ol_check_etsi_overlap(chan_freq, chan_width,
						ETSI_RADAR_EN302_502_FREQ_LOWER,
						ETSI_RADAR_EN302_502_FREQ_UPPER);
	}
...
ieee80211_print_nolhistory(struct ieee80211com *ic)
{
    ...
    for (i = 0; i < ic->ic_nchans; i++) {
        ...
        if(IEEE80211_IS_CHAN_HISTORY_RADAR(c)) {
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "nolhistory:%d channel=%d MHz Flags=%X \n",j,c->ic_freq,c->ic_flags);
            j++;
        }
    }
    return;
