...
#ifdef ATH_SUPPORT_RDKB
    if (ic->ic_wlanstats_peer_init)
        ic->ic_wlanstats_peer_init(ic, ni);
...
#ifdef ATH_SUPPORT_RDKB
    if (ic->ic_wlanstats_peer_deinit)
        ic->ic_wlanstats_peer_deinit(ic, ni);
#endif
...
_ieee80211_free_node(struct ieee80211_node *ni)
{
    ...
    if (ieee80211_vap_map_is_set(vap) && ni->assoc_frame) {
        qdf_nbuf_free(ni->assoc_frame);
    }
#endif

    if (print_assoc_deny_ratelimit()) {
        IEEE80211_NOTE(vap, IEEE80211_MSG_ASSOC, ni,
                       "%s: vap=0x%x, nodecount=%d, ni=0x%x, ni_bss_node=0x%x bss_ref=%d \n",__func__,
                       vap, vap->iv_node_count, ni, ni->ni_bss_node, ni->ni_bss_node->ni_refcnt);
    }
...
ieee80211_node_authorize(struct ieee80211_node *ni)
{
    ...
    if (ic->ic_node_authorize) {
        if (vap->iv_rsn.rsn_keymgmtset == RSN_AUTH_KEY_MGMT_OWE) {
            /* Set the 1th position bit of authorization parameter value
             * so that FW should set the peer authorization bit forcibly.
             */
    ...
    vap->iv_stats.authorize_success++;
}
...
ieee80211_node_unauthorize(struct ieee80211_node *ni)
{
    ...
    if (ic == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:WARN ni->ni_ic is NULL for ni(%pK) ni_macddr %s\n",
    ...
    { /* Add join node */
       for (i = 0, calbitmap = 1; i < ATF_ACTIVED_MAX_CLIENTS; i++)
       {
        ...
        if(!ic->ic_is_mode_offload(ic))
        {
            ...
            /* Point node to the default subgroup list */
...
#if IEEE80211_DEBUG_REFCNT
EXPORT_SYMBOL(ieee80211_sta_leave_debug);
#else
...
#if QCA_AIRTIME_FAIRNESS
    group = TAILQ_FIRST(&ic->ic_atfgroups);
    selbs->ni_atf_group = group;
    atf_assign_subgroup(selbs, group, 0, ATF_CFGTYPE_SSID);
#endif
...
{
    if (!ieee80211_vap_deleted_is_set(vap)) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: vap is not deleted by user, vap: 0x%pK, vap->iv_bss: 0x%pK\n",
...
u_int8_t wlan_node_get_rx_spatial(wlan_node_t node)
{
    struct ieee80211_node *ni = node;
    return ni->ni_rxstreams;
}

u_int8_t wlan_node_get_tx_spatial(wlan_node_t node)
{
    struct ieee80211_node *ni = node;
    return ni->ni_txstreams;
}
...
EXPORT_SYMBOL(wlan_node_get_max_nss);
...
#define MAX_ACK_RSSI_AGE 0xff
    ...
    age = jiffies - ni->ni_ctrl_stats.ni_last_ack_jiffies;
    age = jiffies_to_msecs(age) / MSEC_PER_SEC;
    if (age > MAX_ACK_RSSI_AGE)
        age = MAX_ACK_RSSI_AGE;

    ni->ni_ctrl_stats.ni_last_ack_rssi = (ni->ni_ctrl_stats.ni_last_ack_rssi << 8) | rssi;
    ni->ni_ctrl_stats.ni_last_ack_age = (ni->ni_ctrl_stats.ni_last_ack_age << 8) | age;
    ni->ni_ctrl_stats.ni_last_ack_jiffies = jiffies;
    ni->ni_ctrl_stats.ni_last_ack_cnt = ni->ni_ctrl_stats.ni_last_ack_cnt + 1;
