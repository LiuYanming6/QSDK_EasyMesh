...
static int ev_fill_info(struct sk_buff * skb, struct net_device *dev,
                        int type, wnm_netlink_event_t *event)
{
    ...
    if (unlikely(nla_put(skb, IFLA_WIRELESS, sizeof(wnm_netlink_event_t) -
                IEEE80211_WNM_NETLINKBUF + event->datalen, event) < 0))
        goto nla_put_failure;
...
int wnm_netlink_init(ieee80211_wnm_t wnm)
{
    if (wnm->wnm_nl_sock == NULL) {
        ...
        if (wnm->wnm_nl_sock == NULL) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: %d NETLINK_KERNEL_CREATE failed!\n", __func__, __LINE__);
            return -ENODEV;
        }
    }
    return 0;
...
int wlan_wnm_set_bssmax(wlan_if_t vaphandle, void *data)
{
    ...
    if (vap->iv_opmode == IEEE80211_M_HOSTAP) {
        ...
        } else if (bssmax->idleoption > 3) {
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_WNM,
                              "%s: Idle option (0, 1, 2 or 3\n", __func__);
        } else {
    ...
    while (tclass_len > 0) {
        ...
        if (IEEE80211_ELEMID_TCLAS == elemid) {
            ...
            if (tclas == NULL) {
                ...
                QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: %d OS_MALLOC failed!\n", __func__, __LINE__);
                return ENOMEM;
            }
                     ...
                     tclas->type == IEEE80211_WNM_TCLAS_CLASSIFIER_TYPE4) 
            {
                ...
                wlan_wnm_free_tclass(tclass_head);
                QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: %d unknown tclass classifier type %d\n",
            ...
            wlan_wnm_free_tclass(tclass_head);
            QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: %d unknown elementid %d\n", __func__, __LINE__, elemid);
            return -EINVAL;
        }
    }
...
int wlan_wnm_set_fms(wlan_if_t vaphandle, void *data)
{
    ...
    while (fmsreqelem_count < fms->num_fmsreq) {
        ...
        if (fms_req == NULL) {
            ...
            QDF_PRINT_INFO(vaphandle->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: %d OS_MALLOC failed!\n", __func__, __LINE__);
            return -ENOMEM;
        }
        ...
        while (subelem_count < req_fmsreq->num_subelements) {
            ...
            while (tclas_elem_count < req_subelement->num_tclas_elements) {
                ...
                if (rv != 0) {
                    ...
                    OS_FREE(fms_req);
                    OS_FREE(fmssub);
                    QDF_PRINT_INFO(vaphandle->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: wlan_wnm_set_tclass failed!\n", __func__);
                    return -ENOMEM;
                }
                tclas_elem_count++;
            }
    ...
    if (ni == NULL) {
        QDF_PRINT_INFO(vaphandle->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : %d ieee80211_vap_find_node failed !\n", __func__, __LINE__);
        return -EINVAL;
    }
    event = OS_MALLOC(vap->iv_ic->ic_osdev, sizeof(*event), GFP_KERNEL);
...
int ieee80211_wnm_sleepreq_to_app(wlan_if_t vap, u_int8_t action, u_int16_t intval)
{
    ...
    if (action == IEEE80211_WNMSLEEP_ACTION_ENTER)
        ...
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Not supported WNM-Sleep action %d\n", action);
        return -1;
    }
...
void wlan_btm_sta_count(void *arg, wlan_node_t node)
{
    ieee80211_sta_info_t *sta_info = (ieee80211_sta_info_t *)arg;
...
/**
  ...
  * @return MAC address
  */
int wlan_get_btm_sta_list(wlan_if_t vap, u_int8_t *macaddr, u_long *param)
{
    ...
    {
        wlan_iterate_station_list(vap, wlan_btm_sta_count, &dest_stats);
        param[0] = dest_stats.count;
        return 0;
    } else {
