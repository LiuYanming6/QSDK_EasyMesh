...
void ieee80211_lowi_send_wru_frame (u_int8_t *data)
{
    ...
    if ((!ic)||(!ni))
    {
        qdf_print("%s: Could not find node[%s] in associated nodes table.\n", __func__, ether_sprintf(&wru->sta_mac[0]));
        return;
    }
    /* Get VAP where this node is associated */
    vap = ni->ni_vap;
    /* Make sure this VAP is active and in AP mode */
    if ((!ieee80211_vap_active_is_set(vap)) || (IEEE80211_M_HOSTAP != vap->iv_opmode)){
    ...
    qdf_print("Where Are you frame successfully sent for %s\n", ether_sprintf(&wru->sta_mac[0]));
...
void ieee80211_lowi_send_ftmrr_frame (u_int8_t *data)
{
    ...
    ieee80211_send_mgmt(vap, ni, wbuf, false);
    qdf_print("FTMRR frame successfully sent for %s\n", ether_sprintf(&ftmrr->sta_mac[0]));
    ieee80211_free_node(ni);    /* reclaim node */
    return;
}
