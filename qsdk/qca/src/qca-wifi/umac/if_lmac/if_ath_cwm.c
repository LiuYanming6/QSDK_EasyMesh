...
ath_cwm_attach(struct ath_softc_net80211 *scn, struct ath_reg_parm *ath_conf_parm)
{
	...
	acw = (struct ath_cwm *)OS_MALLOC(scn->sc_osdev, 
                                      ...
                                      GFP_KERNEL);
	if (acw == NULL) {
		QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: no memory for cwm attach\n", __func__);
		return ENOMEM;
	}
...
ath_cwm_detach(struct ath_softc_net80211 *scn)
{
    ...
    if (acw == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: error - acw NULL. Possible attach failure\n", __func__);
        return;
    }
...
cwm_stoptxdma(struct ath_softc *scn)
{
	...
	ath_hal_intrset(ah, 0);     	
		...
		if (!ath_hal_aborttxdma(sc->sc_ah)) {
			QDF_PRINT_INFO(scn->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: unable to abort tx dma\n", __func__);
		}
	} else {
		int i;
