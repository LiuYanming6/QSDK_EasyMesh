...
int ath_net80211_update_ack_rssi(ieee80211_node_t node, int rssi);
...
static void ath_modify_cts_rate(struct ieee80211vap *vap);
static int ath_net80211_get_node_numstreams(ieee80211_node_t node);
#if ATH_DEBUG
/*
...
ath_net80211_flowmac_notify_state (ieee80211_handle_t ieee, int en)
{
    ...
    TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {
        if (vap) {
            vap->iv_flowmac = en;
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Notifying VAP %pK  enabled state %d \n", vap, en);
        }
    }
    spin_unlock(&ic->ic_lock);
}
        ...
        struct ieee80211vap *vap)
{
    ...
    /*
     *  Set four stream capability if all of the following are true
     *  - HAL supports four streams
     *  - four chains are available
     *  - remote node has advertised support for four streams
     */
    if (scn->sc_ops->have_capability(scn->sc_dev, ATH_CAP_QS) &&
        (numtxchains == 4) && (ni->ni_streams >= 4))
    {
        ratecap |= ATH_RC_QS_FLAG;
    }
    /*
     *  Set three stream capability if all of the following are true
     *  - HAL supports three streams
     *  - three  or more chains are available
     *  - remote node has advertised support for three or more streams
     */
    ...
    if((ni->ni_htcap & IEEE80211_HTCAP_C_SM_MASK) == IEEE80211_HTCAP_C_SMPOWERSAVE_STATIC) {
        ratecap &= ~(ATH_RC_TS_FLAG|ATH_RC_DS_FLAG|ATH_RC_QS_FLAG);
    }
    /*
...
#define ATH_NSCAN_PSTA_VAPS 0
    ...
    if (vap->iv_mpsta) {
        ...
        if (ic->ic_global_list->psta_creation_timer_flag == 0) {
             /* Start this timer only when any radio is configured on qwrap mode*/
             OS_SET_TIMER(&(ic->ic_global_list->psta_creation_timer), IEEE80211_PSTA_CHECK_TIME);
             ic->ic_global_list->psta_creation_timer_flag = 1;
        }
    ...
    vap->iv_modify_cts_rate = ath_modify_cts_rate;
    ...
    /* complete setup */
    if (ieee80211_vap_attach(vap))
    {
#if WAR_DELETE_VAP
        scn->sc_ops->remove_interface(scn->sc_dev, avn->av_if_id, vap->iv_athvap);
#else
        scn->sc_ops->remove_interface(scn->sc_dev, avn->av_if_id);
#endif
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "vap_attach failed\n");
        OS_FREE_VAP(avn);
        return NULL;
    }
...
#if DBDC_REPEATER_SUPPORT
        GLOBAL_IC_LOCK(ic->ic_global_list);
        if (ic->ic_global_list->psta_creation_timer_flag == 1) {
             OS_CANCEL_TIMER(&(ic->ic_global_list->psta_creation_timer));
             ic->ic_global_list->psta_creation_timer_flag = 0;
        }
        GLOBAL_IC_UNLOCK(ic->ic_global_list);
#endif
    }
...
ath_net80211_node_getairtime(const struct ieee80211_node *ni)
{
    ...
    u_int32_t airtime = 0, used_tokens = 0;
    
    subgroup = ni->ni_ac_list_ptr[WME_AC_BE];
    if (!subgroup) {
        return -EINVAL;
    }

    /* 1-1 mapping between peer & subgroup exists only if
     * the subgroup is of Peer type (ATF peer configuration exists
               ...
               ieee80211_tx_control_t *txctl)
{
    ...
    if (wbuf_is_pos(wbuf)) {
        ...
        if (txctl->wifiposdata) {
            ...
            if(update_txchainmask == 0xf)
                txctl_flag_mask |= HAL_TXDESC_POS_TXCHIN_4;
...
void ath_net80211_tx_complete_compact(struct ieee80211_node *ni, wbuf_t wbuf, ieee80211_tx_status_t *ts)
{
    ...
    void *arg;
    ieee80211_xmit_status tx_status = {0};
    tx_status.ts_flags = ts->flags;
    ...
    if (type == IEEE80211_FC0_TYPE_MGT || subtype == IEEE80211_FC0_SUBTYPE_NODATA) {
        ...
        if (handler) {
            handler(ni->ni_vap,wbuf,arg,wh->i_addr1,wh->i_addr2,wh->i_addr3,&tx_status);
        }
    }
                           ...
                           int countrycode)
{
    ...
    if ((countrycode == CTRY_DEFAULT) || (cmd == CLIST_NEW_COUNTRY)) {
        ...
        for (i = 0; i < nchan; i++) {
            ...
            OS_MEMZERO(ichan, sizeof(struct ieee80211_ath_channel));
...
#if QCA_AIRTIME_FAIRNESS
static u_int8_t
ath_net80211_atf_adjust_tokens(ieee80211_node_t node, int8_t ac, u_int32_t actual_duration, u_int32_t est_duration)
{
    struct subgroup_list *subgroup;
    u_int8_t ret = 0;
    struct ieee80211_node *ni = (struct ieee80211_node *)node;
    u_int32_t diff_duration = 0;
    ...
    if (actual_duration > est_duration) {
        ...
        if (subgroup->tx_tokens >= diff_duration) {
            ...
            subgroup->tokens_excess_consumed += diff_duration;
        }
...
/*
 * Description  : Check if the respective subgroup has sufficient tokens
 * Input        : Pointer to node, ac, packet duration
 * Output       : 0 if tokens available > packet duration
                  1 if tokens available < packet duration
*/
static u_int8_t
ath_net80211_atf_account_tokens(ieee80211_node_t node, int8_t ac, u_int32_t duration)
{
    struct ieee80211_node *ni = (struct ieee80211_node *)node;
    u_int8_t ret = 0;
    struct subgroup_list *subgroup;
...
static u_int8_t
ath_net80211_get_atf_allocations(ieee80211_node_t node, void *sg, u_int32_t *tx_tokens,
				 u_int32_t *alloted_tx_tokens, u_int32_t *unassigned_txtokens)
{
    struct ieee80211_node *ni = (struct ieee80211_node *)node;
    struct subgroup_list *subgroup = (struct subgroup_list *)sg;
...
void *ath_net80211_atf_find_subgroup(ieee80211_node_t node, int8_t tidno)
{
    struct ieee80211_node *ni = (struct ieee80211_node *)node;
    struct subgroup_list *subgroup = NULL;
    int8_t ac = 0;
                        ...
                        RATE_TYPE type, const HAL_RATE_TABLE *rt)
{
    ...
    for (i = 0; (i < maxrates) && (i < IEEE80211_RATE_MAXSIZE); i++) {
...
static void ath_net80211_rate_newstate(ieee80211_handle_t ieee, ieee80211_if_t if_data)
{
    ...
    if (ieee80211vap_get_opmode(vap) != IEEE80211_M_STA) {
        ...
        if((vap->iv_bsschan == NULL) ||
            ...
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "VAP channel is not set for vap %pK investigate!\n", vap);
            return;
        }
        /*
...
static void
ath_modify_cts_rate(struct ieee80211vap *vap)
{
    struct ieee80211com *ic = vap->iv_ic;
    struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(ic);
    struct ath_vap_net80211 *avn = ATH_VAP_NET80211(vap);

    scn->sc_ops->modify_cts_rate(scn->sc_dev, avn->av_if_id, vap->iv_mgt_rate, vap->iv_disabled_legacy_rate_set);
    return;
}
...
struct ieee80211_ops net80211_ops = {
    ...
    ath_net80211_atf_find_subgroup,             /* atf_find_subgroup */
#endif
    ...
    .update_ack_rssi = ath_net80211_update_ack_rssi, /* update_ack_rssi */
    ath_net80211_get_node_numstreams,            /* get_node_numstreams */
};
 ...
 */
void
ath_net80211_mark_dfs(ieee80211_handle_t ieee, struct ieee80211_ath_channel *ichan)
{
    struct ieee80211com *ic = NET80211_HANDLE(ieee);
    struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(ic);
    struct ath_softc *sc = ATH_DEV_TO_SC(scn->sc_dev);
...
ath_net80211_change_channel(ieee80211_handle_t ieee, struct ieee80211_ath_channel *chan)
{
    struct ieee80211com *ic = NET80211_HANDLE(ieee);
    ic->ic_curchan = chan;
    ic->ic_curchan_ieee = chan->ic_ieee;
    ic->ic_curchan_cfreq2 = chan->ic_vhtop_ch_freq_seg2;
...
ath_set_spatialstream(struct ath_softc_net80211 *scn)
{
    ...
    if (scn->sc_ops->have_capability(scn->sc_dev, ATH_CAP_QS)){
        stream = 4;
    } else if (scn->sc_ops->have_capability(scn->sc_dev, ATH_CAP_TS)) {
        stream = 3;
    } else if (scn->sc_ops->have_capability(scn->sc_dev, ATH_CAP_DS)) {
        stream = 2;
    } else {
        stream = 1;
    }
...
#if QCA_AIRTIME_FAIRNESS
static void ath_net80211_atf_node_unblock( struct ieee80211com *ic,struct ieee80211_node *ni, struct atf_stats *stats)
{
    struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(ic);
    struct ath_node *an;

    an = (struct ath_node *)((ATH_NODE_NET80211(ni))->an_sta);
    scn->sc_ops->ath_atf_node_unblock(an, stats);
}
...
static int32_t ath_net80211_atf_debug_nodestate(struct ieee80211com *ic, struct ieee80211_node *ni, struct atf_nodestate *atfnodestate)
{
    struct ath_node *an;
    struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(ic);

    an = (struct ath_node *)((ATH_NODE_NET80211(ni))->an_sta);
    return scn->sc_ops->ath_atf_debug_nodestate(an, atfnodestate);
}
...
#if ATH_SUPPORT_WRAP && DBDC_REPEATER_SUPPORT
static
OS_TIMER_FUNC(ath_psta_creation_timeout)
{
           ...
           struct hal_reg_parm *hal_conf_parm, IEEE80211_REG_PARAMETERS *ieee80211_conf_parm)
{
...
#if QCA_AIRTIME_FAIRNESS
    ic->ic_atf_node_unblock = ath_net80211_atf_node_unblock;
    ...
    ic->ic_wmi_radar_found_indication_to_fw = NULL;
    ic->ic_dfs_remove_spoof_chan_from_nol = NULL;
    ic->ic_restart_vaps_with_non_dfs_chan = NULL;
    ic->ic_rebuild_chan_list_with_non_dfs_channels = NULL;
   ...
   ic->ic_scan_enable_txq = ath_net80211_scan_enable_txq;
    ...
    ic->ic_thermal_timer = ath_net80211_thermal_timer;
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Set global_ic[%d]..gloabl_ic ptr:%pK\n", __func__, ic_list.num_global_ic,&ic_list);
...
#if  ATH_SUPPORT_WRAP
    if (ic_list.num_global_ic == 1) {
        OS_INIT_TIMER(ic->ic_osdev, &(ic_list.psta_creation_timer), ath_psta_creation_timeout,
    ...
    /* initialize the random seed mode to 3 for now, applicable only
     * for plaforms where, there is no HRNG.
     */
#if ATH_GEN_RANDOMNESS
    sc = ATH_DEV_TO_SC(scn->sc_dev);
#if LINUX_VERSION_CODE <= KERNEL_VERSION(3,3,8)
    if (sc->sc_quadstream_support ) {
        ic->ic_random_gen_mode = 1;
    }
    else
    {
        ic->ic_random_gen_mode = 3;
    }
#else
    ic->ic_random_gen_mode = 1;
#endif
...
ath_detach(struct ath_softc_net80211 *scn)
{
    ...
    for (i=0; i < MAX_RADIO_CNT; i++) {
        ...
        if (tmp_ic && (ic == tmp_ic)) {
            ...
            QDF_PRINT_INFO(tmp_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: remove global_ic[%d]..gloabl_ic ptr:%pK\n", __func__, ic_list.num_global_ic,&ic_list);
            GLOBAL_IC_UNLOCK(&ic_list);
        }
    }
    for (i=0; i < MAX_RADIO_CNT; i++) {
    ...
    struct dfs_nol_chan_entry *nollist, int nentries)
{
	...
	ieee80211_enumerate_channels(chan, ic, i) {
		...
		if (nol_found && (! IEEE80211_IS_CHAN_RADAR(chan)))
			qdf_print("%s: radar found = chan=%pK, freq=%d, "
			...
			qdf_print("%s: NOL clear = chan=%pK, freq=%d, "
			    "vht freq1=%d, freq2=%d, flags=0x%x\n",
			    __func__,
			    chan,
			    chan->ic_freq,
		 ...
		 * in this driver to translate between ieee80211_ath_channel
...
ath_net80211_set_max_sta(ieee80211_handle_t ieee, int value)
{
    ...
    if ((value > 0) && (value < (IEEE80211_33_AID + 1)))
        ic->ic_num_clients = value;
    else {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "QCA_LOWMEM_PLATFORM: Range 1-33 clients");
        return -EINVAL;
    }
#else
    if ((value > 0) && (value < (IEEE80211_128_AID + 1)))
        ic->ic_num_clients = value;
    else {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Max clients Range 1-128 clients");
        return -EINVAL;
    }
#endif
...
static int
ath_net80211_get_node_numstreams(ieee80211_node_t node)
{
    struct ieee80211_node *ni = (struct ieee80211_node *)node;
    return wlan_node_get_max_nss(ni);
}
...
void ath_net80211_thermal_timer(struct ieee80211com *ic)
{
    struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(ic);
    struct ath_softc  *sc  = ATH_DEV_TO_SC(scn->sc_dev);

    if (sc->thermal_param.th_cfg.enable) {
