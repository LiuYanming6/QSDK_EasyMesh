...
// This is an ugly way to get the driver definitions for some enums and will
// need to be removed ultimately in preference to locally defined enums.
...
struct steerexecImplCmnLegacyPriv_t {
    ...
    /// Timer used to age out devices marked as legacy upgrade steering unfriendly.
    steerexecImplCmnTimerStruct_t legacyUpgradeUnfriendlyTimer;
};
...
typedef struct steerexecImplCmnSteeringStateLegacy_t {
    ...
    /// The time when the legacy upgrade steering unfriendly timer expires.
    time_t legacyUpgradeUnfriendlyExpiryTime;

    /// Flag indicating if this device is legacy upgrade steering unfriendly
    LBD_BOOL legacyUpgradeUnfriendly;
...
typedef struct steerexecImplCmnSteeringStateBTM_t {
    ...
    /// Disassoc Timer to force steer a STA when timer expires
    struct evloopTimeout disassoc_timer;
...
typedef struct steerexecImplCmnSteeringState_t {
    ...
    /// related messaging.
    steerexec_transactionId_t msgTransaction;
...
static LBD_BOOL steerexecImplCmnIsSelfServing(
...
static LBD_BOOL steerexecImplCmnSTAAllowedToSteer(steerexecImplCmnSteeringState_t *state,
...
static void steerexecImplCmnStartLegacyUpgradeUnfriendly(
        steerexecImplCmnHandle_t exec,
        steerexecImplCmnSteeringState_t *state,
        const struct ether_addr *staAddr,
        steerexecImplCmnTimerStruct_t *timer,
...
static void steerexecImplCmnLegacyUpgradeUnfriendlyTimeoutHandler(void *cookie);
...
static void steerexecImplCmnBTMDisassocTimeoutHandler(void *cookie);
...
static void steerexecImplCmnHandleBTMQueryEvent(struct mdEventNode *event);
#endif
...
static u_int8_t steerexecImplCmnSteerReasonToMboReason(u_int8_t steerReason);
// ====================================================================
// Public API
// ====================================================================
...
steerexecImplCmnHandle_t steerexecImplCmnCreate(const steerexecImplCmnConfig_t *config,
    ...
    if (exec) {
        ...
        /* Initialize the BTM dialog token */
        exec->btm.dialogToken = 1;
        ...
        evloopTimeoutCreate(&exec->legacy.legacyUpgradeUnfriendlyTimer.timer,
                            "steerexecImplCmnLegacyUpgradeUnfriendlyTimeout",
                            steerexecImplCmnLegacyUpgradeUnfriendlyTimeoutHandler,
                            exec);
...
#if defined(LBD_MODULE_SONEVENT) && defined(IEEE80211_SUPPORT_BTM_QUERY)
        mdListenTableRegister(mdModuleID_WlanIF, wlanif_event_btm_query,
                              steerexecImplCmnHandleBTMQueryEvent);
#endif
...
LBD_STATUS steerexecImplCmnAbort(steerexecImplCmnHandle_t exec,
    ...
    if (state->steerType == steerexecImplCmnSteeringType_none) {
        ...
        if (abortReason == steerexecImplCmnSteeringStatusType_channel_change) {
            ...
            return steerexecImplCmnRemoveAllBlacklists(entry, state, addr);
        } else {
            return LBD_NOK;
        }
    }
...
LBD_STATUS steerexecImplCmnAllowAssoc(
    ...
    // Ensure STA is not locally associated and is allowed to steer for
    // pre-association steering
    stadbEntry_bssStatsHandle_t stats = stadbEntry_getServingBSS(entry, NULL);
    if (steerexecImplCmnIsSelfServing(stats) ||
        !steerexecImplCmnSTAAllowedToSteer(state, exec, entry, staAddr,
...
#ifdef LBD_MODULE_SONEVENT
    if (exec->sonEventEnabled) {
        soneventserviceBlklistSteerCmdEvent steerEvent;

        memset(&steerEvent, 0, sizeof(steerEvent));
        lbCopyMACAddr(staAddr->ether_addr_octet,
                      steerEvent.staAddr.ether_addr_octet);
        steerEvent.blklistType = steerexecImplCmnBlacklist_channel;
        steerEvent.blklistTime = exec->config.legacy.blacklistTime;
        mdCreateEvent(mdModuleID_SonEventService, mdEventPriority_Low,
                      soneventservice_event_blklist_steer_cmd,
                      &steerEvent, sizeof(steerEvent));
    }
#endif
...
LBD_STATUS steerexecImplCmnSteer(steerexecImplCmnHandle_t exec,
    ...
    if (!assocBSS) {
        dbgf(exec->dbgModule, DBGINFO, "%s: Ignoring steer command for " lbMACAddFmt(
                                           ":") " due to STA not associated",
             __func__, lbMACAddData(staAddr->ether_addr_octet));
        if (ignored) {
            *ignored = LBD_TRUE;
        }
    ...
    LBD_BOOL preparationComplete = LBD_FALSE;
    LBD_BOOL blacklistAutoClear =
    ...
    if (steerexecImplPrepareForSteering(entry, staAddr, candidateCount,
        ...
        steerexecImplCmnEnableAllDisabledCandidates(entry,
                                                    state,
                                                    candidateCount,
                                                    candidateList,
                                                    staAddr);
...
void steerexecImplCmnHandleAssocUpdate(steerexecImplCmnHandle_t exec,
    ...
    stadbEntry_bssStatsHandle_t stats = stadbEntry_getServingBSS(entry, NULL);
    if (!steerexecImplCmnIsSelfServing(stats)) {
        // Not actually associated locally
        return;
    }
...
steerexec_steerEligibility_e steerexecImplCmnDetermineSteeringEligibility(
    ...
    // Check if the STA is actually associated to an AP
    stadbEntry_bssStatsHandle_t stats = stadbEntry_getServingBSS(entry, NULL);
    if (!stats) {
...
LBD_STATUS steerexecImplCmnUpdateBTMCapabilityWrapper(
    struct steerexecImplCmnPriv_t *exec, stadbEntry_handle_t entry,
    const struct ether_addr *staAddr) {
    if (!exec || !entry || !staAddr) {
        return LBD_NOK;
    }
    LBD_BOOL isBTMSupported = stadbEntry_isBTMSupported(entry);
    steerexecImplCmnSteeringState_t *state =
    ...
    if (steerexecImplCmnUpdateBTMCapability(state, entry, staAddr, isBTMSupported) !=
        LBD_OK) {
        // Not able to update the BTM steering capability
        return LBD_NOK;
    }
    return LBD_OK;
}
...
LBD_STATUS steerexecImplCmnClearPreparedSteer(
    ...
    LBD_STATUS status = steerexecImplCmnRemoveAllBlacklists(entry, state, staAddr);
...
void steerexecImplCmnDestroy(steerexecImplCmnHandle_t exec) {
    if (exec) {
        ...
        evloopTimeoutUnregister(&exec->legacy.legacyUpgradeUnfriendlyTimer.timer);
        free(exec);
    }
}
...
LBD_BOOL steerexecImplCmnIsBTMSteer(steerexecImplCmnSteeringType_e steerType) {
    if ((steerType == steerexecImplCmnSteeringType_btm) ||
        ...
        (steerType == steerexecImplCmnSteeringType_btm_be_active_force_steer)) {
        return LBD_TRUE;
    }
...
LBD_BOOL steerexecImplCmnIsBlacklistSteer(steerexecImplCmnSteeringType_e steerType) {
...
/**
 * @brief Create the BTM Disassoc Timer for a specific STA.
 *
 * @param [in] state steering state for the STA
 * @param [in] entry stadb entry for the STA
 */
static void steerexecImplCmnCreateBTMDisassocTimer(
        steerexecImplCmnSteeringState_t *state, stadbEntry_handle_t entry) {
    evloopTimeoutCreate(&state->btm->disassoc_timer,
                        "steerexecImplCmnBTMDisassocTimer",
                        steerexecImplCmnBTMDisassocTimeoutHandler,
                        entry);
}
...
static LBD_STATUS steerexecImplCmnSetupBTMState(steerexecImplCmnSteeringState_t *state,
    ...
    evloopTimeoutCreate(&state->btm->timer,
                        ...
                        steerexecImplCmnBTMTimeoutHandler,
                        entry);
    steerexecImplCmnCreateBTMDisassocTimer(state, entry);
...
static void steerexecImplCmnManageSteeringStateLifecycleCB(
    ...
    if (handle) {  // record the remaining time
        ...
        if (statePtr->btm) {
            evloopTimeoutRemaining(&statePtr->btm->timer,
                                   &btmSecs, &btmUsecs);
            evloopTimeoutRemaining(&statePtr->btm->disassoc_timer,
                                   &disassocSecs, &disassocUsecs);
        }
    }
    ...
    if (statePtr->btm) {
        ...
        evloopTimeoutUnregister(&statePtr->btm->disassoc_timer);
    }
    ...
    if (!handle) {  // destroy
        ...
        if (statePtr->btm) {
            ...
            steerexecImplCmnCreateBTMDisassocTimer(state, handle);
            ...
            if (disassocSecs || disassocUsecs) {
                evloopTimeoutRegister(&statePtr->btm->disassoc_timer,
                                      disassocSecs, disassocUsecs);
            }
        }
    }
...
static steerexecImplCmnSteeringType_e steerexecImplCmnDetermineBTMSteeringType(
    ...
    if (active) {
        if(stadbEntryReadActiveLoadBalancingRejected(entry) == stadbEntryLoadBalancingStaState_markedForceSteer) {
            return steerexecImplCmnSteeringType_btm_be_active_force_steer;
        }
...
static steerexecImplCmnSteeringType_e steerexecImplCmnDetermineLegacySteeringType(
    LBD_BOOL isBestEffort, LBD_BOOL disableLegacySteering) {

    if (disableLegacySteering)
 ...
 *
 * @param [in] servingBSS  the currently serving BSS for the STA (may be
 *                         NULL)
 *
 ...
 *
 * NOTE: This only covers general checks, BTM-specific checks
 ...
 *                                      steering
 *
 * @return LBD_TRUE if this STA can be steered; LBD_FALSE otherwise
 */
static LBD_BOOL steerexecImplCmnSTAAllowedToSteer(steerexecImplCmnSteeringState_t *state,
                                                  steerexecImplCmnHandle_t exec,
                                                  stadbEntry_handle_t entry,
                                                  const struct ether_addr *staAddr,
                                                  LBD_BOOL reportReasonNotEligible) {
    // if STA steering flag is set to true, then this STA cannot be steered.
    if (stadbEntry_isSteeringDisallowed(entry)) {
        return LBD_FALSE;
    }
 ...
 * @brief Determine the type of steering to use for this STA
 *
 * NOTE: This function is only for post-association steering
 *
 * @param [in] state steering state for the STA
 * @param [in] exec steering executor instance
 * @param [in] entry staDB entry for the STA
 * @param [in] staAddr MAC address of the STA
 * @param [in] stats  serving BSS stats handle for the STA
...
static steerexecImplCmnSteeringType_e steerexecImplCmnDetermineSteeringType(
    ...
    if (!steerexecImplCmnSTAAllowedToSteer(state, exec, entry, staAddr,
                                           reportReasonNotEligible)) {
        return steerexecImplCmnSteeringType_none;
    }
    ...
    if (!stadbEntry_isBTMSupported(entry)) {
        ...
        steerType = steerexecImplCmnDetermineLegacySteeringType(isBestEffort,
                                                                exec->config.legacy.disableLegacySteering);
    } else {
        if (eligibilityOnly) {
 ...
 *        marked as legacy upgrade steering unfriendly.
 *
 * @param [in] exec  the executor instance to use
 * @param [in] state  the object that captures the steering state
 * @param [in] staAddr address of the STA marked legacy upgrade steering
 *                     unfriendly
 * @param [in] timer  timer to start
        ...
        time_t *expiryTime)
{
    ...
    lbGetTimestamp(&ts);
    u_int32_t baseTime;
    if (!exec->config.legacy.legacyUpgradeUnfriendlyTime) {
        return;
    }
    state->legacy.legacyUpgradeUnfriendly = LBD_TRUE;
    timer->countEntries++;
    baseTime = exec->config.legacy.legacyUpgradeUnfriendlyTime + 1;
    *expiryTime = ts.tv_sec + baseTime;
    steerexecImplCmnStartTimer(exec->dbgModule, timer, *expiryTime, baseTime);
}
...
static void steerexecImplCmnSteerEnd(steerexecImplCmnSteeringState_t *state,
...
#ifdef LBD_MODULE_SONEVENT
    if (state->context->sonEventEnabled) {
        if (steerexecImplCmnIsBlacklistSteer(state->steerType) ||
            state->steerType == steerexecImplCmnSteeringType_preassociation) {
            soneventserviceBlklistSteerCmdResultEvent blklistSteerResultEvent;

            memset(&blklistSteerResultEvent, 0, sizeof(blklistSteerResultEvent));
            lbCopyMACAddr(staAddr->ether_addr_octet,
                          blklistSteerResultEvent.staAddr.ether_addr_octet);
            blklistSteerResultEvent.blklistCmdResult =
                (status == steerexecImplCmnSteeringStatusType_success) ? LBD_TRUE : LBD_FALSE;
            mdCreateEvent(mdModuleID_SonEventService, mdEventPriority_Low,
                          soneventservice_event_blklist_steer_cmd_result,
                          &blklistSteerResultEvent, sizeof(blklistSteerResultEvent));
        }
    }
#endif

    // Start the prohibit timer if steerType is not force steer
    if((stadbEntryReadActiveLoadBalancingRejected(entry) != stadbEntryLoadBalancingStaState_markedForceSteer) ||
    ...
    if (isLocalAbort && (status != steerexecImplCmnSteeringStatusType_success) &&
        ...
        steerexecImplAbort(state->msgTransaction, staAddr, status, state->steerType);
    }
...
/**
 * @brief Examine a single entry to see if its legacy upgrade steering
 *        unfriendly period has elapsed.
 *
 * @param [in] entry  the entry to examine
 * @param [in] cookie  the executor handle
 */
static void steerexecImplCmnLegacyUpgradeUnfriendlyIterateCB(stadbEntry_handle_t entry,
        void *cookie) {
    if (!stadbEntry_isInNetwork(entry)) {
        return;
    }
    ...
    steerexecImplCmnHandle_t exec = (steerexecImplCmnHandle_t) cookie;
    steerexecImplCmnSteeringState_t *state = stadbEntry_getSteeringState(entry);
    if (state && state->legacy.legacyUpgradeUnfriendly) {
        if (steerexecImplCmnTimerExpiryCheck(exec,
                    &exec->legacy.legacyUpgradeUnfriendlyTimer,
                    state->legacy.legacyUpgradeUnfriendlyExpiryTime)) {

            const struct ether_addr *addr = stadbEntry_getAddr(entry);
            lbDbgAssertExit(exec->dbgModule, addr);

            dbgf(exec->dbgModule, DBGINFO,
                    "%s: Cleared Legacy Upgrade unfriendly flag for " lbMACAddFmt(":"),
                    __func__, lbMACAddData(addr->ether_addr_octet));

            state->legacy.legacyUpgradeUnfriendlyExpiryTime = 0;
            state->legacy.legacyUpgradeUnfriendly = LBD_FALSE;
        }
    }
}
...
/**
 * @brief Handle the periodic timer that signals we should check how many
 *        entries are still waiting to have their Legacy upgrade unfriendly
 *        flag cleared.
 *
 * @param [in] cookie  the executor handle
 */
static void steerexecImplCmnLegacyUpgradeUnfriendlyTimeoutHandler(void *cookie) {
    ...
    steerexecImplCmnTimerStartIterate(exec, &exec->legacy.legacyUpgradeUnfriendlyTimer,
            steerexecImplCmnLegacyUpgradeUnfriendlyIterateCB,
            exec->config.legacy.legacyUpgradeUnfriendlyTime);
}
...
LBD_STATUS steerexecImplCmnTimeoutBlacklistCandidate(
    ...
    for (i = 0; i < blacklistCount; i++) {
        ...
        if (wlanif_setCandidateStateForSTA(1,
                                           ...
                                           staAddr,
                                           lastServingESS,
                                           LBD_TRUE /* enable */, servingBSS, 0) != LBD_OK) {
            dbgf(state->context->dbgModule, DBGERR,
...
static LBD_STATUS steerexecImplCmnTimeoutBlacklist(
    ...
    if (state->blacklistType == steerexecImplCmnBlacklist_channel) {
        if (steerexecImplCmnTimeoutBlacklistChannel(entry, exec, state, addr) != LBD_OK) {
            return LBD_NOK;
        }
        ...
        // Candidate blacklist
        lbd_bssInfo_t candidateList[WLANIF_MAX_RADIOS];
        u_int8_t candidateCount;
        stadbEntry_bssStatsHandle_t bssStats = stadbEntry_getServingBSS(entry, NULL);
        const lbd_bssInfo_t *servingBSS = stadbEntry_resolveBSSInfo(bssStats);
        ...
        return steerexecImplCmnTimeoutBlacklistCandidate(exec, state, entry, addr, candidateCount,
                                                      &candidateList[0]);
    }
}
...
void steerexecImplCmnBlacklistIterateCB(stadbEntry_handle_t entry,
    ...
    if (state && state->blacklistType != steerexecImplCmnBlacklist_none) {
        ...
        if (ts.tv_sec - state->legacy.lastSteeringTime >
            ...
            LBD_STATUS status = steerexecImplCmnTimeoutBlacklist(entry, exec, state, addr);
            if (status != LBD_OK) {
                // Selective blacklist didn't work, try disabling the
                // whole blacklist
                status = steerexecImplCmnRemoveAllBlacklists(entry, state, addr);
            }
...
static LBD_STATUS steerexecImplCmnRemoveAllBlacklists(
    ...
    if (state->legacy.disabledChannelCount) {
        ...
        status = steerexecImplCmnEnableAllDisabledChannels(entry, state, staAddr);
        ...
        status = steerexecImplCmnEnableAllDisabledCandidates(entry,
                                                             state,
                                                             state->candidateCount,
                                                             &state->candidateList[0],
                                                             staAddr);
...
static LBD_STATUS steerexecImplCmnAbortSteerImpl(
    ...
    result = steerexecImplCmnRemoveAllBlacklists(entry, state,staAddr);
    if (result == LBD_NOK) {
        dbgf(exec->dbgModule, DBGERR,
...
static void steerexecImplCmnLowRSSIObserver(stadbEntry_handle_t entry, void *cookie) {
    ...
    stadbEntry_bssStatsHandle_t stats = stadbEntry_getServingBSS(entry, NULL);
    if (!stats || !steerexecImplCmnIsSelfServing(stats)) {
    ...
    if (rssi != LBD_INVALID_RSSI &&
        ...
        if (LBD_OK == steerexecImplCmnRemoveAllBlacklists(entry, state,
                                                            staAddr)) {
            dbgf(exec->dbgModule, DBGINFO,
...
static void steerexecImplCmnCleanupBlacklistBTM(steerexecImplCmnSteeringState_t *state,
    ...
    steerexecImplCmnRemoveAllBlacklists(entry, state, staAddr);
...
/**
 * @brief Timeout handler for BTM Disassoc timeouts
 *
 *
 *
 * @param cookie the steering state
...
static void steerexecImplCmnBTMDisassocTimeoutHandler(void *cookie) {
    ...
    stadbEntryMarkActiveLoadBalancingRejected(entry, stadbEntryLoadBalancingStaState_disassocTimerExpired);
    // @todo Remove these when satisfied that there are no more lurking
    //       issues with spurious timer expiries.
    if (!state && staAddr) {
        dbgf(NULL, DBGERR,
    ...
    dbgf(NULL, DBGERR, "%s: Disassoc Timer Expired--> Initiating Disassociation of"
         lbMACAddFmt(":") " (entry=%p)",
         __func__, lbMACAddData(staAddr->ether_addr_octet), entry);
    ...
    if (wlanif_disassociateSTA(assocBSS, staAddr, LBD_FALSE /* local */) != LBD_OK) {
        /*== handle steer fail */
        //steerexecImplCmnAbortInProgress(NULL, state, entry, staAddr, steerexecImplCmnSteeringStatusType_prepare_fail, LBD_TRUE /*isLocalAbort*/, NULL);
        if (steerexecImplCmnIsBTMOnlySteer(state->steerType)) {
            steerexecImplCmnSteerEnd(state, staAddr, steerexecImplCmnSteeringStatusType_prepare_fail, LBD_TRUE /*isLocaliAbort*/, entry);
        }
        state->candidateCount = 0;
        state->steerType = steerexecImplCmnSteeringType_none;
        state->state = steerexecImplCmn_state_idle;
...
static LBD_STATUS steerexecImplCmnStartSteer(
    ...
    if (steerexecImplCmnIsLegacySteer(state->steerType)) {
        LBD_BOOL ineligible = LBD_FALSE;
        if((state->reason == steerexec_reason_idleUpgrade) || (state->reason == steerexec_reason_activeUpgrade))
        {
            if (!(state->legacy.legacyUpgradeUnfriendly)) {
                if (stadbUpdateLegacyUpgradeSteeringTimestamp(entry))
                {
                    steerexecImplCmnStartLegacyUpgradeUnfriendly(state->context, state, staAddr,
                            &state->context->legacy.legacyUpgradeUnfriendlyTimer,
                            &state->legacy.legacyUpgradeUnfriendlyExpiryTime);
                }
            } else {
                dbgf(exec->dbgModule, DBGINFO,
                     "Client ineligible for Legacy upgrade steering since it has reached the legacyUpgradeAllowedCnt \n");
                status = LBD_NOK;
                ineligible = LBD_TRUE;
            }
        }
        if (ineligible != LBD_TRUE) {
    ...
    if (status == LBD_OK) {
        ...
        if (exec->sonEventEnabled) {
            if (steerexecImplCmnIsBlacklistSteer(state->steerType)) {
                soneventserviceBlklistSteerCmdEvent blklistSteerEvent;

                memset(&blklistSteerEvent, 0, sizeof(blklistSteerEvent));
                lbCopyMACAddr(staAddr->ether_addr_octet,
                              blklistSteerEvent.staAddr.ether_addr_octet);
                blklistSteerEvent.blklistType = steerexecImplCmnBlacklist_candidate;
                blklistSteerEvent.blklistTime = exec->config.legacy.blacklistTime;
                mdCreateEvent(mdModuleID_SonEventService, mdEventPriority_Low,
                              soneventservice_event_blklist_steer_cmd,
                              &blklistSteerEvent, sizeof(blklistSteerEvent));
            }

            if (!steerexecImplCmnIsLegacySteer(state->steerType)) {
                soneventserviceBSTMReqCmdEvent bstmReqEvent;

                memset(&bstmReqEvent, 0, sizeof(bstmReqEvent));
                lbCopyMACAddr(staAddr->ether_addr_octet,
                              bstmReqEvent.staAddr.ether_addr_octet);
                mdCreateEvent(mdModuleID_SonEventService, mdEventPriority_Low,
                              soneventservice_event_bstm_req_cmd,
                              &bstmReqEvent, sizeof(bstmReqEvent));
            }
        }
#endif
...
static LBD_STATUS steerexecImplCmnSteerBTM(
    ...
    u_int8_t steerReason = IEEE80211_BSTM_REQ_REASON_UNSPECIFIED;
    u_int8_t forceSteer =
        (state->steerType == steerexecImplCmnSteeringType_btm_be_active_force_steer) ? 1
                                                                                     : 0;
    // Store the current association
    lbCopyBSSInfo(assocBSS, &state->btm->initialAssoc);

    if (lbIsBSSLocal(assocBSS)) {
        ...
        steerReason = steerexecImplCmnSteerReasonToMboReason(state->reason);
        dbgf(exec->dbgModule, DBGINFO,
             "%s: Sending BTM request to " lbMACAddFmt(
                 ":") " with forceSteer=%d and MBO Reason=%d",
             __func__, lbMACAddData(staAddr->ether_addr_octet), forceSteer, steerReason);
        ...
        exec->btm.dialogToken++;
        exec->btm.dialogToken = exec->btm.dialogToken ? exec->btm.dialogToken : 1;
    } else {
        if (steerexecImplStartRemoteSteer(staAddr, assocBSS, forceSteer,
                                          state->candidateCount, state->candidateList,
                                          &state->msgTransaction) != LBD_OK) {
            dbgf(exec->dbgModule, DBGERR,
                 "%s: Can't steer for " lbMACAddFmt(
                     ":") " failed to start remote steering (transaction %d)",
                 __func__, lbMACAddData(staAddr->ether_addr_octet), state->transaction);
            return LBD_NOK;
        }
    }
    ...
    evloopTimeoutRegister(&state->btm->timer, exec->config.btm.responseTime,
                          0 /* USec */);
    if(forceSteer) {
        evloopTimeoutRegister(&state->btm->disassoc_timer, 1, 0 /* USec */);
        stadbEntryMarkActiveLoadBalancingRejected(entry, stadbEntryLoadBalancingStaState_disassocSent);
        dbgf(exec->dbgModule, DBGERR, "%s: disassoc_timer Registered for 1 second", __func__);
    }
...
static LBD_STATUS steerexecImplCmnSteerLegacy(
    ...
    if (lbIsBSSLocal(assocBSS)) {
        ...
        if (steerexecImplStartRemoteSteer(staAddr, assocBSS, LBD_FALSE /* forceSteer */,
                                          state->candidateCount, state->candidateList,
                                          &state->msgTransaction) != LBD_OK) {
            dbgf(exec->dbgModule, DBGERR,
                 "%s: Can't steer for " lbMACAddFmt(
...
static LBD_BOOL steerexecImplCmnHandleAssocUpdateBTM(
    ...
    if (steerexecImplCmnIsAssociationOnCandidate(state, entry, assocBSS, staAddr,
        ...
        if (stadbEntryReadActiveLoadBalancingRejected(entry) == stadbEntryLoadBalancingStaState_disassocTimerExpired) {
        stadbEntryMarkActiveLoadBalancingRejected(entry, stadbEntryLoadBalancingStaState_forceSteerComplete);
        }
...
static void steerexecImplCmnHandleAssocPreAssoc(
    ...
    if (assocBand == wlanif_band_5g) {
        if (steerexecImplCmnEnableAllDisabledChannels(entry, state, staAddr) != LBD_OK) {
            return;
        }
...
static void steerexecImplCmnUpdateBlacklistOnSteerCompletion(
    ...
    lbd_bssInfo_t blacklist[WLANIF_MAX_RADIOS];
    u_int8_t blacklistCount;
    ...
    lbd_bssInfo_t enableList[WLANIF_MAX_RADIOS];
    for (i = 0; i < blacklistCount; i++) {
    ...
    if (enableCount) {
        if (wlanif_setCandidateStateForSTA(enableCount, &enableList[0],
                                           staAddr,lastServingESS, LBD_TRUE /* enable */, servingBSS, 0) != LBD_OK) {
            dbgf(state->context->dbgModule, DBGERR,
    ...
    if (state->candidateCount + enableCount > WLANIF_MAX_RADIOS) {
        dbgf(state->context->dbgModule, DBGERR,
             "%s: state->candidateCount (%d) +  enableCount (%d) for " lbMACAddFmt(":")
             " should be less than WLANIF_MAX_RADIOS",
    ...
    steerexecImplCmnBlacklistCandidates(entry, state, staAddr);
}
...
static void steerexecImplCmnAssocBlacklistClear(steerexecImplCmnHandle_t exec,
    ...
    steerexecImplCmnUpdateBlacklistOnSteerCompletion(entry, state, assocBSS, staAddr);
...
static void steerexecImplCmnHandleBTMResponseEvent(struct mdEventNode *event) {
...
#ifdef LBD_MODULE_SONEVENT
    soneventserviceBSTMRespEvent respEvent;
#endif
...
#ifdef LBD_MODULE_SONEVENT
    if (state->context->sonEventEnabled) {
        memset(&respEvent, 0, sizeof(respEvent));
        lbCopyMACAddr(resp->sta_addr.ether_addr_octet,
                      respEvent.staAddr.ether_addr_octet);
        respEvent.status = resp->status;
        mdCreateEvent(mdModuleID_SonEventService, mdEventPriority_Low,
                      soneventservice_event_bstm_resp,
                      &respEvent, sizeof(respEvent));
    }
#endif
    ...
    if (resp->status != IEEE80211_WNM_BSTM_RESP_SUCCESS) {
        dbgf(state->context->dbgModule, DBGINFO,
             "%s: Received BTM response from " lbMACAddFmt(":")
             " with non-success code (%d) reject code (%d) (transaction %d)",
             __func__, lbMACAddData(resp->sta_addr.ether_addr_octet), resp->status,
             resp->reject_code,
             state->transaction);
        if((resp->reject_code > IEEE80211_BSTM_REJECT_REASON_UNSPECIFIED) && (resp->reject_code < IEEE80211_BSTM_REJECT_REASON_INVALID))
        {
            state->steerType = steerexecImplCmnSteeringType_btm_be;
            if(state->reason == steerexec_reason_activeOffload &&
              (stadbEntryReadActiveLoadBalancingRejected(staHandle) == stadbEntryLoadBalancingStaState_initial)) {
                stadbEntryMarkActiveLoadBalancingRejected(staHandle, stadbEntryLoadBalancingStaState_markedForceSteer);
            }
        }
    ...
    else if((stadbEntryReadActiveLoadBalancingRejected(staHandle) == stadbEntryLoadBalancingStaState_disassocSent) ||
            (stadbEntryReadActiveLoadBalancingRejected(staHandle) == stadbEntryLoadBalancingStaState_disassocTimerExpired)) {
    evloopTimeoutUnregister(&state->btm->disassoc_timer);
    stadbEntryMarkActiveLoadBalancingRejected(staHandle, stadbEntryLoadBalancingStaState_forceSteerComplete);
    dbgf(state->context->dbgModule, DBGERR, "%s: Received BTM Response SUCCESS : %d", __func__, resp->status);
    }
...
#if defined(LBD_MODULE_SONEVENT) && defined(IEEE80211_SUPPORT_BTM_QUERY)
/**
 * @brief React to an event that a BTM query was received.
 *
 * @param [in] event event received
 */
static void steerexecImplCmnHandleBTMQueryEvent(struct mdEventNode *event)
{
    const wlanif_btmQueryEvent_t *query =
        (const wlanif_btmQueryEvent_t *)event->Data;
    soneventserviceBSTMQueryEvent queryEvent;
    stadbEntry_handle_t staHandle;

    lbDbgAssertExit(NULL, query);

    staHandle = stadb_find(&query->sta_addr);
    if (!staHandle) {
        return;
    }

    memset(&queryEvent, 0, sizeof(queryEvent));
    lbCopyMACAddr(query->sta_addr.ether_addr_octet,
                  queryEvent.staAddr.ether_addr_octet);
    queryEvent.reason = query->reason;
    mdCreateEvent(mdModuleID_SonEventService, mdEventPriority_Low,
                  soneventservice_event_bstm_query,
                  &queryEvent, sizeof(queryEvent));
}
#endif
    ...
    if (!steerexecImplCmnIsBTMOnlySteer(steerType)) {
        ...
        if (updatedState) {
            ...
            if (wlanif_setNonCandidateStateForSTA(candidateCount, candidateList,
                                               staAddr, lastServingESS, LBD_FALSE /* enable */,
                                               LBD_FALSE /* probeOnly */,
                                               servingBSS) != LBD_OK) {
                dbgf(exec->dbgModule, DBGERR,
             ...
             if (disableCount) {
                 ...
                 if (wlanif_setCandidateStateForSTA(disableCount, &disableList[0],
                                                    staAddr,lastServingESS, LBD_FALSE /* enable */, servingBSS, 0) != LBD_OK) {
                     dbgf(exec->dbgModule, DBGERR,
...
#ifdef LBD_MODULE_SONEVENT
LBD_STATUS steerexecImplCmnEnableSonEvent(steerexecImplCmnHandle_t exec,
                                 LBD_BOOL enabled) {
    if (!exec) {
        return LBD_NOK;
    }

    exec->sonEventEnabled = enabled;
    dbgf(exec->dbgModule, DBGINFO, "Son event service enabled = %d",exec->sonEventEnabled);
...
/**
 * @brief Map steerexec_reason to MBO Reason Code before sending it to WLAN Driver.
 *
 * This should be called before sending it to WLAN Driver for using steer reason in
 * BTM Request as MBO Reason Code.
 *
 * @param [in] steerReason, steerexec reason decided by LBD.
 * @return steerMBOReason, mapped reason code.
 */
static u_int8_t steerexecImplCmnSteerReasonToMboReason(
        u_int8_t steerReason) {
    u_int8_t steerMBOReason = IEEE80211_BSTM_REQ_REASON_UNSPECIFIED;

    switch (steerReason)
    {
        case steerexec_reason_activeOffload:
            steerMBOReason = IEEE80211_BSTM_REQ_REASON_LOAD_BALANCING;
            break;
        case steerexec_reason_activeDowngradeRSSI:
        case steerexec_reason_idleDowngrade:
            steerMBOReason = IEEE80211_BSTM_REQ_REASON_LOW_RSSI;
            break;
        case steerexec_reason_activeDowngradeRate:
            steerMBOReason = IEEE80211_BSTM_REQ_REASON_INSUFFICIENT_BANDWIDTH;
            break;
        case steerexec_reason_activeUpgrade:
            steerMBOReason = IEEE80211_BSTM_REQ_REASON_PREMIUM_AP;
            break;
        case steerexec_reason_interferenceAvoidance:
            steerMBOReason = IEEE80211_BSTM_REQ_REASON_HIGH_INTERFERENCE;
            break;
        case steerexec_reason_user:
            steerMBOReason = IEEE80211_BSTM_REQ_REASON_UNSPECIFIED;
            break;
        case steerexec_reason_invalid:
        default:
            steerMBOReason = IEEE80211_BSTM_REQ_REASON_INVALID;
            break;
    }
    return steerMBOReason;
}
...
void steerexecImplCmnDumpLegacyHeader(struct cmdContext *context,
    ...
    cmdf(context, "  Current # STAs marked as legacy upgrade steering unfriendly: %u\n",
                     exec->legacy.legacyUpgradeUnfriendlyTimer.countEntries);

    if (exec->legacy.legacyUpgradeUnfriendlyTimer.countEntries &&
            exec->config.legacy.legacyUpgradeUnfriendlyTime > 0) {
        cmdf(context, "    Next unfriendly update: %u seconds\n",
                exec->legacy.legacyUpgradeUnfriendlyTimer.nextExpiry - ts.tv_sec);
    }
    ...
    cmdf(context, "%-18s%-12s%-17s%-10s%-11s%-11s%-11s%-8s%-10s%-21s%-19s\n",
         "MAC", "Transaction", "Secs since steer", "State", "# Auth Rej",
         "Prohibited", "Unfriendly", "T_Steer", "Blacklist", "Consecutive Failures","Upgrade Unfriendly");
}
...
static void steerexecImplCmnDumpLegacyEntryState(struct cmdContext *context,
    ...
    cmdf(context, "%-19s", state->legacy.legacyUpgradeUnfriendly ? "yes" : "no");
