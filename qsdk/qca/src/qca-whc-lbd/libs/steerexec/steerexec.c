...
static struct profileElement steerexecElementDefaultTable[] = {
    ...
    // 6 hours
    { STEEREXEC_LEGACY_UPGRADE_UNFRIENDLY_TIME_KEY,  "21600" },
    { STEEREXEC_DISABLE_LEGACY_STEERING, "0"},
    { NULL, NULL }
};
...
LBD_STATUS steerexec_updateBTMCapability(stadbEntry_handle_t entry,
    ...
    return steerexecImplCmnUpdateBTMCapabilityWrapper(steerexecState.steerexecImplCmn,
                                                      entry, staAddr);
}
...
LBD_BOOL steerexec_isSubjectToSteeringLimit(steerexec_reason_e reason) {
    if (reason == steerexec_reason_activeAPSteering ||
        reason == steerexec_reason_idleAPSteering) {
        return LBD_TRUE;
    }
    return LBD_FALSE;
}
...
static LBD_STATUS steerexecReadConfig(steerexecImplCmnConfig_t *config,
    ...
    config->legacy.disableLegacySteering =
        profileGetOptsInt(mdModuleID_SteerExec,
                          STEEREXEC_DISABLE_LEGACY_STEERING,
                          steerexecElementDefaultTable);
    ...
    config->legacy.legacyUpgradeUnfriendlyTime =
        profileGetOptsInt(mdModuleID_SteerExec,
                          STEEREXEC_LEGACY_UPGRADE_UNFRIENDLY_TIME_KEY,
                          steerexecElementDefaultTable);
...
LBD_STATUS steerexecSonEventServiceStart(int enabled) {
    ...
    if (!steerexecState.steerexecImplCmn) {
        dbgf(steerexecState.dbgModule, DBGERR,"%s: No exec handle", __func__);
        return LBD_NOK;
    }

    return steerexecImplCmnEnableSonEvent(steerexecState.steerexecImplCmn, enabled);
}
#endif
