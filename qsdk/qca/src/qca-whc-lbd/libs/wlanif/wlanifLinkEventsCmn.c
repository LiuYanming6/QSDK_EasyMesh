...
//QCA file included only when SON Lib not enabled
...
void wlanifLinkEventsCmnProcessChannelChange(
    ...
    dbgf(state->dbgModule, DBGINFO,
         "%s: Channel change to %u", __func__, ((const struct iw_event *)event)->u.freq.m);
    ...
    // restart us.
    wlanif_band_e newBand = wlanifMapFreqToBand(((const struct iw_event *)event)->u.freq.m);
    if (newBand != band) {
    ...
    if (wlanifBSteerControlUpdateChannel(state->bsteerControlHandle,
                                         band,
                                         sysIndex,
                                         ((const struct iw_event *)event)->u.freq.m) != LBD_OK) {
         dbgf(state->dbgModule, DBGERR,
...
void wlanifLinkEventsCmnGenerateDisassocEvent(
        wlanifLinkEventsHandle_t state, void *event,
        int sysIndex) {
    const struct iw_event *disassocEvent;
    disassocEvent = (const struct iw_event *)event;
    wlanif_assocEvent_t assocEvent;
    lbCopyMACAddr(disassocEvent->u.addr.sa_data, assocEvent.sta_addr.ether_addr_octet);
    ...
    dbgf(state->dbgModule, DBGINFO, "%s: Client " lbMACAddFmt(":")
         ...
         __func__, lbMACAddData(disassocEvent->u.addr.sa_data),
        ...
        switch (event.cmd) {
            ...
            case IWEVEXPIRED:
            {
                if (event.len == IW_EV_LCP_LEN + sizeof(event.u)) {
                    ...
                    dbgf(state->dbgModule, DBGERR,
                         "%s: Invalid event length %u (expected %tu)",
                    ...
                    if (event.u.data.flags == IEEE80211_EV_CHAN_CHANGE) {
                        if (event.u.data.length == sizeof(u_int8_t) &&
                            ...
                            dbgf(state->dbgModule, DBGERR,
                                 ...
                                 "remaining=%tu)", __func__, event.u.data.length,
...
static void wlanifLinkEventsCmnHandleNewLink(wlanifLinkEventsHandle_t state,
    ...
    if (attrLen != 0) {
        dbgf(state->dbgModule, DBGERR, "%s: Did not consume all attributes: %zu bytes left",
             __func__, attrLen);
    }
}
