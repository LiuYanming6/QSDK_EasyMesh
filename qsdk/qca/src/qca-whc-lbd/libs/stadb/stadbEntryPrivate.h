...
typedef struct stadbEntryPriv_bssStats_t {
    struct {
        ...
        /// The estimated uplink full capacity on this channel,
...
typedef struct stadbEntryPriv_inNetworkInfo_t {
    ...
    struct {
        ...
        /// The AP on which the device is currently associated (if any).
        lbd_apId_t apId;
    ...
    /// State information related to MAP Service.
    /// This should be considered opaque to all but the MAP Service module.
    void *mapServiceState;

    /// State information related to monitoring coordination between nodes.
    /// This should be considered opaque to all but the monitoring messaging
    /// module.
    void *monitorState;
    ...
    /// Function to invoke to destroy the MAP Service state prior to
    /// destroying the overall entry.
    stadbEntry_stateLifecycleCB_t mapServiceStateLifecycleCB;

    /// Function to invoke to destroy the monitoring messaging state prior to
    /// destroying the overall entry.
    stadbEntry_stateLifecycleCB_t monitorStateLifecycleCB;

    // Whether STA rejected BTM Request made for offloading purpose //
    stadbEntryLoadBalancingStaState_t loadbalancingAttemptedButRejected;

    /// Number of steering attempted for the STA
    u_int8_t numSteerAttempts;
...
typedef struct stadbEntryPriv_t {
    ...
    /// The best PHY mode supported across all bands (HE, VHT, HT or basic)
    wlanif_phymode_e bestPHYMode : 3;
    ...
    /// Whether the Legacy Upgrade Steering count is reached
    LBD_BOOL isLegacyUpgradeCntReached : 1;

    /// An array which holds the timestamp of each legacy upgrade steering
    struct timespec *steeringTimeHistory;

    /// Index which increments during every Legacy upgrade steering
    u_int8_t steeringTimeId;
 ...
 * @param [in] trackRemoteAssoc  whether the station database is configured
 *                               to track STAs that are associated on remote
 *                               APs
...
stadbEntry_handle_t stadbEntryCreate(
        ...
        wlanif_capStateUpdate_e rrmStatus, LBD_BOOL trackRemoteAssoc,
...
stadbEntry_handle_t stadbEntryChangeEntryType(
        ...
        LBD_BOOL trackRemoteAssoc, size_t numRadiosLocal,
 ...
 * that band or the new capabilities are better than those previously stored.
 *
 * @param [in] entry  the STA to set the PHY capabilities for
 * @param [in] band  the band on which the capabilities should be set
 * @param [in] phyCapInfo  the capabilities for the band
 *
 * @return LBD_OK on success; otherwise LBD_NOK
 */
...
/**
 * @brief Mark the given entry with the given band capabilites
 *
 * @param [in] handle  the handle to the STA entry
 * @param [in] band capabilities of the STA 2.4G (or) 5G (or) both
 *
 * @return LBD_OK on success, otherwise return LBD_NOK
 */

LBD_STATUS stadbEntryMarkGivenBandSupported(stadbEntry_handle_t handle, u_int8_t bandCap);
