...
static struct {
    struct {
        ...
        u_int32_t minAssocAgeForStatsAssocUpdate;
    } config;
    ...
    u_int8_t legacyUpgradeAllowedCnt;
    ...
    u_int32_t legacyUpgradeMonitorDur;
...
static void stadbReadConfig(void) {
    ...
    stadbState.config.minAssocAgeForStatsAssocUpdate =
        profileGetOptsInt(mdModuleID_StaDB, STADB_MIN_ASSOC_AGE_STATS_ASSOC_UPDATE,
                          stadbElementDefaultTable);
    ...
    stadbState.legacyUpgradeAllowedCnt =
        profileGetOptsInt(mdModuleID_StaDB, STADB_LEGACY_UPGRADE_ALLOWED_CNT,
                          stadbElementDefaultTable);
    stadbState.legacyUpgradeMonitorDur =
        profileGetOptsInt(mdModuleID_StaDB, STADB_LEGACY_UPGRADE_MONITOR_DUR,
                stadbElementDefaultTable);
...
void stadbGetLegacyUpgradeSteeringConfig(int *legacyUpgradeAllowedCnt, int *legacyUpgradeAllowedTime) {
    if(legacyUpgradeAllowedCnt != NULL)
        *legacyUpgradeAllowedCnt = stadbState.legacyUpgradeAllowedCnt;
    if(legacyUpgradeAllowedTime != NULL)
        *legacyUpgradeAllowedTime = stadbState.legacyUpgradeMonitorDur;
}
...
void stadbUpdateAssoc(const struct ether_addr *addr,
    ...
    if (entry) {
        ...
        if ((stadbState.legacyUpgradeAllowedCnt) && (entry->steeringTimeHistory == NULL)) {
            entry->steeringTimeHistory = (struct timespec *)malloc((stadbState.legacyUpgradeAllowedCnt) * sizeof(struct timespec));
...
stadbEntry_bssStatsHandle_t stadb_findOrCreateServingBSSStats(stadbEntry_handle_t entry,
    ...
    if (wasAssociated &&
        (secSinceAssoc < stadbState.config.minAssocAgeForStatsAssocUpdate ||
