 ...
 */
void stadbGetLegacyUpgradeSteeringConfig(int *legacyUpgradeAllowedCnt, int *legacyUpgradeAllowedTime);

/**
 * @brief Get the serving BSS handle for a STA.
 *
 * The measurement report (e.g. STA Link Metrics) may indicate a different BSS from the
 * BSS stored in staDB, possibly as a result of missed association update. In this case,
 * take the latter of the association record time and the measurement time, and update
 * association information if necessary.
 *
 * @param [in] entry  the STA entry
 * @param [in] bssidRx  the BSSID received (e.g. in link metrics report)
 * @param [in] secSinceMeasurement  the elapsed time (in sec) since the measurement
 * @param [out] assocUpdated  whether the serving BSS was updated for this STA
 *
 * @return stats handle of the serving BSS, or NULL if cannot resolve the serving BSS.
 */
stadbEntry_bssStatsHandle_t stadb_findOrCreateServingBSSStats(
    stadbEntry_handle_t entry, const struct ether_addr *bssidRx,
    time_t secSinceMeasurement, LBD_BOOL *assocUpdated);

/**
 * @brief Store the PHY capabilities for a given STA on a band, updating the
 *        other band with inferred capabilities if so configured.
 *
...
#define STADB_TRACK_REMOTE_ASSOC_KEY     "TrackRemoteAssoc"
...
#define STADB_MAX_NON_SERVING_BSSES      "NumNonServingBSSes"
...
#define STADB_LEGACY_UPGRADE_ALLOWED_CNT "LegacyUpgradeAllowedCnt"
#define STADB_LEGACY_UPGRADE_MONITOR_DUR "LegacyUpgradeMonitorDur"
#define STADB_DISABLE_STEERING_STA_LIST  "DisallowSteeringStaListFile"
#define STADB_MIN_ASSOC_AGE_STATS_ASSOC_UPDATE "MinAssocAgeForStatsAssocUpdate"
