...
acfg_os_send_req(uint8_t *ifname, acfg_os_req_t  *req)
{
    ...
    if(strlen((const char *)ifname) < IFNAMSIZ) {
        acfg_os_strcpy((uint8_t *)ifr.ifr_name, ifname,
                       IFNAMSIZ);
    }
    else
...
acfg_set_ssid(uint8_t     *vap_name, acfg_ssid_t  *ssid)
{
    ...
    ptr->len = acfg_os_strcpy((uint8_t *)ptr->ssid, (uint8_t *)ssid->ssid, IEEE80211_NWID_LEN + 1);
...
acfg_get_ssid(uint8_t  *vap_name, acfg_ssid_t  *ssid)
{
    ...
    if (status == QDF_STATUS_SUCCESS)
        ssid->len = acfg_os_strcpy((uint8_t *)ssid->ssid, (uint8_t *)ptr->ssid, IEEE80211_NWID_LEN + 1);
...
void acfg_wlan_vap_profile_vlan_remove(acfg_wlan_profile_vap_params_t *cur_vap_params)
{
    ...
    snprintf(iface_name, sizeof(iface_name), "%s.%d",
             cur_vap_params->vap_name, cur_vap_params->vlanid);
...
acfg_vap_atf_addsta(uint8_t *vap_name,
        uint8_t *mac, uint32_t percent, char *ssid)
{
    uint32_t status = QDF_STATUS_SUCCESS;
        ...
        *cur_radio_params)
{
    ...
    if (radio_params->amsdu != cur_radio_params->amsdu) {
        status = acfg_set_radio_param(radio_params->radio_name,
                ACFG_PARAM_RADIO_AMSDU,
                !!radio_params->amsdu);
        if (status != QDF_STATUS_SUCCESS) {
            return QDF_STATUS_E_FAILURE;
        }
