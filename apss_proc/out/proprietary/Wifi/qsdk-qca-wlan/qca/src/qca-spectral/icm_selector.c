...
static int icm_best_cfreq2_selection_helper(ICM_INFO_T* picm,
                                            int pri20_chan_80_80);

static int icm_init_cfreq2_seldbg_decinfo(ICM_SELDBG_DECISION_INFO_T *decdbginfo,
                                   int arraysize);

static int icm_cfreq2_seldbg_process(ICM_DEV_INFO_T* pdev,
                              ICM_SELDBG_DECISION_INFO_T *decdbginfo,
                              int arraysize);

static int icm_cfreq2_seldbg_consoledump(ICM_DEV_INFO_T* pdev,
                                  ICM_SELDBG_DECISION_INFO_T *decdbginfo,
                                  int arraysize);
...
int icm_get_reg_domain(ICM_INFO_T* picm)
{
    ...
    if (strlcpy(iwr.ifr_name, picm->dev_ifname, IFNAMSIZ) >= IFNAMSIZ) {
   ...
   ICM_DPRINTF(pdev,
    ...
    for (chan_idx = 0; chan_idx < num_chan_obss_disp; chan_idx++) {
        ...
        ICM_DPRINTF(pdev,
                ...
                ICM_MODULE_ID_SELECTOR,
                "----------------------------------------------------\n"
                "Channel %d - OBSS counts from scan:\n"
    ...
    for (chan_idx = 0; chan_idx < num_chan_to_scan; chan_idx++) {
        ...
        adjusted_msrd_usability = ICM_GET_CHANNEL_CHANNEL_LOAD(picm, ng_chan[chan_idx]);
        ...
        if (ch_bw != ICM_CH_BW_20) {
           ...
           relchandbginfo =
            ...
            adjusted_msrd_usability = ICM_GET_CHANNEL_CHANNEL_LOAD(picm, ext_chan[chan_idx]); 
            ICM_SELDBG_SETFIELD( \
        ...
        ICM_CH_BW_T ch_bw, u_int8_t seg_id, int pri20_chan_80_80)
{
    ...
    for (chan_idx = 0; chan_idx < chanlist->count; chan_idx++) {
        ...
        ICM_DPRINTF(pdev,
                ...
                ICM_GET_CHANNEL_CHANNEL_LOAD(picm, chan_num));
        ...
        /* Check if 80/80+80/160 MHz has been requested and if the current
           ...
           ensure that the PHY spec in this case is 11AC. So we don't factor it
           into the if check.
         */
        if (((ch_bw == ICM_CH_BW_80) && !(IEEE80211_IS_CHAN_11AC_VHT80(ch))) ||
            ((ch_bw == ICM_CH_BW_80_PLUS_80) && (seg_id == 0) && !(IEEE80211_IS_CHAN_11AC_VHT80_80(ch))) ||
        ...
        if (ch_bw == ICM_CH_BW_80) {
            ...
            } else if (chan_num == (ch->ic_vhtop_ch_freq_seg2_160 + 14)) {
            ...
            if (bondspan_chan_num != chan_num) {
                ...
                if (ch_bw == ICM_CH_BW_40 ||
                    ...
                    if ((IEEE80211_IS_CHAN_11AC_VHT40PLUS(ch) &&
                        ...
                        ICM_SELDBG_SETFIELD(relchandbginfo->relation,
                                            ICM_CHAN_RELATIONSHIP_SEC20);
                    } else if (((chan_num < ch->ic_vhtop_ch_freq_seg2_160) &&
                                (bondspan_chan_num <
                                        ch->ic_vhtop_ch_freq_seg2_160)) ||
                               ((chan_num > ch->ic_vhtop_ch_freq_seg2_160) &&
                                (bondspan_chan_num >
                                        ch->ic_vhtop_ch_freq_seg2_160)))  {
                        ICM_SELDBG_SETFIELD(relchandbginfo->relation,
                                           ICM_CHAN_RELATIONSHIP_SEC40);
        ...
        if (ch_bw < ICM_CH_BW_80) {
            prepnl_msrd_usability = ICM_GET_CHANNEL_CHANNEL_LOAD(picm, chan_num);
                ...
                if ((bondspan_chan_adj > adj_check_min_adj) &&
                    ...
                    prepnl_msrd_usability =
                        MIN(prepnl_msrd_usability,
                            ICM_GET_CHANNEL_CHANNEL_LOAD(picm, bondspan_chan_num));
                }
            }
            ...
            */
            ...
            for (bondspan_chan_adj = adj_check_min_adj;
                ...
                if (bondspan_chan_adj == adj_check_min_adj)
                {
                    ...
                    if ((picm->enable_sec40_occ_check == true) &&
                            chanlist->ch[bondspan_chan_idx].num_wnw) {
                        if (relchandbginfo != NULL) {
                            ICM_SELDBG_SETFIELD(relchandbginfo->is_obss_pri20,
                                                true);
...

{
    ...
    if ((emiwar80p80 = icm_get_emiwar80p80(picm)) < 0) {
    ...
    for (chan_idx = 0; chan_idx < chanlist->count; chan_idx++) {
        ch = &chanlist->ch[chan_idx];
        /* As per regulatory, if 80 MHz operation is allowed on a given channel,
         * and two such non-contiguous 80MHz segments are available, then 80+80
         * MHz mode is allowed on this combination. In case EMIWAR80P80_FC1GTFC2
         * is set, we use the above to ascertain 80+80 support (subject to
         * exceptions in the WAR) in the absence of the full list of channel
         * combinations from the driver.
         */
        if ((IEEE80211_IS_CHAN_11AC_VHT80_80(ch) ||
             ((EMIWAR_80P80_FC1GTFC2 == emiwar80p80) &&
               IEEE80211_IS_CHAN_11AC_VHT80(ch))) &&
            ...
            ((ch->ic_vhtop_ch_freq_seg1 <
             ...
             (ch->ic_vhtop_ch_freq_seg1 >
                (ch_pri20_80_80->ic_vhtop_ch_freq_seg1 + 16))))
        {
             if(((EMIWAR_80P80_FC1GTFC2 == emiwar80p80) && \
                     ICM_IS_EMIWAR80P80_FC1GTFC2(ch_pri20_80_80->ic_vhtop_ch_freq_seg1,
                     ch->ic_vhtop_ch_freq_seg1)) || ((EMIWAR_80P80_BANDEDGE == emiwar80p80) && \
                     ICM_IS_EMIWAR80P80_BANDEDGE(ch_pri20_80_80->ic_vhtop_ch_freq_seg1,
                         ch->ic_vhtop_ch_freq_seg1))) {
                /*Skip the Secondary Channel*/
                continue;
            }
    ...
    for (st80_chan_idx = 0; st80_chan_idx < st80_chan_count; st80_chan_idx++) {
        ...
        for (bondspan_chan_adj = bond_range_start_idx;
             ...
             */
            if (bondspan_chan_num == chan_num) {
                min_msrd_channel_usability = ICM_GET_CHANNEL_CHANNEL_LOAD(picm,
                                                bondspan_chan_num);
            } else {
                tmp_msrd_channel_usability = ICM_GET_CHANNEL_CHANNEL_LOAD(picm,
                                                bondspan_chan_num);
                                   ...
                                   int arraysize)
{
    ...
    for(i = 0; i < arraysize; i++) {
        ...
        ICM_SELDBG_SETFIELD(decdbginfo[i].physpec, ICM_PHY_SPEC_11AC);
                              ...
                              int arraysize)
{
    ...
    icm_cfreq2_seldbg_consoledump(pdev, decdbginfo, arraysize);
                                  ...
                                  int arraysize)
{
    ...
    ICM_DPRINTF(pdev,
                ...
                "PHY Spec",
                icm_phy_spec_str[ICM_PHY_SPEC_11AC],
                "Desired b/w",
