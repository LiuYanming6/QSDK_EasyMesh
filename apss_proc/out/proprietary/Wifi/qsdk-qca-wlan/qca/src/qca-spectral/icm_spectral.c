...
int icm_init_spectral(ICM_INFO_T* picm)
{
    ...
    if (strlcpy(psinfo->atd.ad_name, picm->radio_ifname, sizeof(psinfo->atd.ad_name)) >= sizeof(psinfo->atd.ad_name)) {
...
/*
 * Function     : is_spectral_supported
...
int icm_is_spectral_supported(ICM_INFO_T* picm)
{
    u_int32_t status =  0;
    struct ifreq ifr;
    ICM_DEV_INFO_T* pdev = get_pdev();
    ...
    psinfo->atd.ad_id = SPECTRAL_IS_SUPPORTED | ATH_DIAG_DYN;
    psinfo->atd.ad_in_data = NULL;
    psinfo->atd.ad_in_size = 0;
    psinfo->atd.ad_out_data = (void*)&status;
    psinfo->atd.ad_out_size = sizeof(u_int32_t);
    ...
    if (ioctl(pnlinfo->sock_fd, SIOCGATHPHYERR, &ifr) < 0){
         return FAILURE;
         perror("icm : ioctl fail (SPECTRAL_IS_SUPPORTED)");
    }
