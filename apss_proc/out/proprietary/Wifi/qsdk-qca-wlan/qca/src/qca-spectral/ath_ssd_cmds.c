...
/*
 * Function     : ath_ssd_is_spectral_supported
 * Description  : Return whether spectral capability
 *                is available on the
 *                given radio interface
 * Input params : pointer to ath_ssd_info_t
 * Output params: whether spectral capability is
 *                available, filled into pointer to boolean
 *                passed
 * Return       : SUCCESS/FAILURE
 *
 */
int ath_ssd_is_spectral_supported(ath_ssd_info_t* pinfo)
{
    struct ifreq ifr;
    u_int32_t status =  0;
    ath_ssd_nlsock_t *pnlinfo = GET_ADDR_OF_NLSOCKINFO(pinfo);
