...
char ap_macaddr_hex[3][20];
char bh_sta_macaddr_hex[3][20];
char action_sta_macaddr_hex[20];
...
static void sig_handler(int, void *eloop_ctx);
...
int get_stamac_addr()
{
    ...
    {
        snprintf(buffer,sizeof(buffer),"wlanconfig %s list sta | tail -n  1 | cut -f1 -d\" \" > /var/run/sta_macaddr",ap_ifname[i]);
        system(buffer);
    ...
    if(file==0)
    {
        printf("Couldnot open sta_config_file\n");
        return -1;
    }
    else
    {
        ...
        if(ret!=EOF)
        {
            ...
            tmp_action_sta_macaddr_hex = &(action_sta_macaddr_hex);
...
/* function to start ACTION based sequence for BH vaps
   ...
   */
start_action_in_root( struct son_driver_data *drv)
{
    int timeout;
    char buf[256];
    union son_event_data event;
    ...
    const u8 *frm , char *ifname, int iface_id)
{
    ...
    if(actionargs->category ==  WLAN_ACTION_VENDOR_SPECIFIC) {
        ...
        return set80211priv(drv, IEEE80211_IOCTL_SEND_MGMT, mgmt_frm, mgmt_frm->buflen, ifname );
    }
    else
        return -1;
}
...
void sig_handler(int signal, void *eloop_ctx)
{
    struct son_driver_data *drv = (struct son_driver_data*)eloop_ctx;
...

    {
        ...
        while ((x = fgetc(file)) != EOF)
        {
            ...
            if(strcmp(mode,"sta") == 0)
            {
                ...
                tmp_bh_sta_macaddr_hex=&(bh_sta_macaddr_hex[num_bh_sta_vaps]);
               ...
               
            {
                ...
                if (&ap_macaddr[num_bh_vaps] == NULL)
                    goto out;
                tmp_ap_macaddr_hex=&(ap_macaddr_hex[num_bh_vaps]);
