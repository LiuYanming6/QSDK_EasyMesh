...
#define WRAP_MAX_PSTA_NUM_AR900B  21 /* 24 : 1 mointor + 1 ap vap + 1 sta vap */
...
wrapd_psta_add(struct wrap_wifi_iface *aptr, const char *parent, char *child, const u8 *addr, u_int32_t flags)
{
    ...
    if (aptr->proxy_noack_war) {
        if (aptr->psta_counter < WRAP_MAX_PSTA_NUM_AR900B) {
        ...
        wrapd_printf("Cannot create more than %d Proxy STAs", aptr->psta_counter);
        return PROXY_STA_LIMIT_EXCEEDED;
      }
    }
#endif
				                        ...
				                        char *data, int len)
{
	...
	if((!hapd) && (iwe->cmd != IWEVCUSTOM)) {
		...
		if (((opcode == IEEE80211_EV_ASSOC_IND_AP) || (opcode == IEEE80211_EV_REASSOC_IND_AP)) && (hapd != NULL)) {
		    /*
		    ...
		    if(wrapg->wrapd_psta_config == 2)
		    {
			wrapd_psta_add(wrapg->max_aptr, wrapg->max_aptr->wifi_ifname, child, msg->addr, flags);
		    }
		    else if (wrapg->wrapd_psta_config == 1)
		    {
			for(dev = wrapg->wrap_wifi_list; dev; dev=dev->next)
			{
			    wrapd_psta_add(dev, dev->wifi_ifname, child, msg->addr, flags);
			}
		    }
		    else
		    {
			wrapd_psta_add(aptr, aptr->wifi_ifname, child, msg->addr, flags);
		    }
		}
...
int wrapd_add_sta(struct wrapd_global *wptr, char *cmd)
{
	...
	ifr.ifr_data = (caddr_t) &extended_cmd;
	extended_cmd.data = (caddr_t)&sta_vap_up;
	if ((ret = ioctl(wrapg->ioctl_sock, SIOCGATHEXTENDED, &ifr)) != 0) {
	    wrapd_printf("ret=%d ioctl SIOCGATHEXTENDED getsta_vap_up err", ret);
	} else {
	    wrapd_printf("initial stavap(%s) connection: %d", aptr->wifi_ifname,sta_vap_up);
	}
	if (sta_vap_up == 1) {
...
wrapd_hostapd_ctrl_iface_process(struct ctrl_conn *hapd, char *msg)
{
    ...
    if (os_strncmp(msg + HOSTAPD_MSG_ADDR_OFF, "AP-DISABLED ", 12) == 0) {
