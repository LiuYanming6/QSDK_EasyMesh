                   ...
                   acfg_nf_dbr_dbm_t *nf_dbr_dbm)
{
    ...
    for (j = 0; j < 4; j++)
    {
        ...
        for (i = 0; i < 4; i++)
        {
            snprintf((char *)buf, sizeof(buf), "%d\t%d", nf_dbr_dbm->nfdbr[4*j+i], nf_dbr_dbm->nfdbm[4*j+i]);
            acfg_logger(buf);
        }
    }
                   ...
                   acfg_packet_power_t *power_info)
{
    uint8_t buf[128];
    uint32_t status = QDF_STATUS_SUCCESS;
    snprintf((char *)buf, sizeof(buf), "%s: packet power (dBm) = %f",ifname,power_info->packet_power/2.0);
    acfg_logger(buf);
    return status;
}
        ...
        acfg_smart_log_fw_pktlog_stop_t *slfwpktlog_stop)
{
    ...
    switch(slfwpktlog_stop->reason) {
        ...
        case ACFG_SMART_LOG_FW_PKTLOG_STOP_HOSTREQ:
            snprintf((char *)buf, sizeof(buf),
                    "%s: ACFG Event Smart Log FW initiated packetlog stop "
                    "event: Stop due to a higher priority host request\n",
                    ifname);
            break;
