...
sensor_setting_t def_ipq8074 = {
	...
	.t = {
		{
			.lvl_trig = 115, /* Level trigger temp */
			.lvl_clr = 105, /* Level clear temp */
			.num_actions = 1, /* No. of actions */
			.actions = {
				{
					.action = CPU_FREQ, /* Action type */
					.info = 1017600 /* Action info/value */
				}
			}
		},
...
void update_def_sensor_settings(thermal_setting_t *settings)
{
	...
	switch (settings->soc_id) {
		...
		case THERM_IPQ_8078:
			msg("==== IPQ8074 ===\n");
			def_sensor_setting = &def_ipq8074;
			break;
	...
	while (pos && (*pos != '\0') && (pos < maxpos)) {
		...
		case '[':
			...
			*idx = '\0';
			...
			if ( settings->soc_id == THERM_IPQ_8072
