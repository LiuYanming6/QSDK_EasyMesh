...
{
    if (isCfg80211) {
        wlanIfWd = wlanif_config_init(WLANIF_CFG80211,
                                      WSPLCD_NL80211_CMD_SOCK,
                                      WSPLCD_NL80211_EVENT_SOCK);
    } else {
        wlanIfWd = wlanif_config_init(WLANIF_WEXT, 0, 0);
    }
    if ( !wlanIfWd )
    {
        return -1;
...
int apacHyfi20GetDeviceMode(apacHyfi20IF_t *pIF) {
    ...
    if (wlanIfWd->ops->isAP(wlanIfWd->ctx, ifName, &result) < 0) {
...
int apacHyfi20GetFreq(apacHyfi20IF_t *pIF) {
    ...
    if (wlanIfWd->ops->getFreq(wlanIfWd->ctx ,pIF->ifName , &freq) < 0) {
        dprintf(MSG_ERROR, "%s: ioctl() failed, ifName: %s.\n", __func__, pIF->ifName);
        goto err;
    }
...
int apacHyfi20GetChannel(apacHyfi20AP_t *pAP) {
    ...
    if (wlanIfWd->ops->getFreq(wlanIfWd->ctx ,pAP->ifName , &freq) < 0) {
        dprintf(MSG_ERROR, "%s: ioctl() failed, ifName: %s.\n", __func__, pAP->ifName);
        goto err;
    }
...
int apacHyfi20GetAPMode(apacHyfi20AP_t *pAP) {
    ...
    if (wlanIfWd->ops->getWirelessMode(wlanIfWd->ctx,pAP->ifName,mode,sizeof(mode)) < 0) {
        dprintf(MSG_ERROR, "%s: ioctl() failed, ifName: %s.\n", __func__, pAP->ifName);
        goto err;
    }
...
static int get80211ChannelInfo(const char *ifName, struct ieee80211req_chaninfo *chans) {
    ...
    if(wlanIfWd->ops->getChannelInfo(wlanIfWd->ctx, ifName, chans, len) < 0) {
        dprintf(MSG_ERROR, "%s: ioctl() failed, ifName: %s.\n", __func__, ifName);
        goto err;
    }
...
int apacHyfi20GetAPChannelInfo( const char *iface, apacHyfi20ChanInfo_t *chaninfo)
{
    ...
    if (wlanIfWd->ops->getFreq(wlanIfWd->ctx ,iface , &freq) < 0) {
        dprintf(MSG_ERROR, "%s: ioctl() failed, ifName: %s.\n", __func__, iface);
        goto err;
    }
    ...
    if(wlanIfWd->ops->getChannelWidth(wlanIfWd->ctx, iface, &chwidth) < 0) {
        dprintf(MSG_ERROR, "%s: ioctl() failed, ifName: %s.\n", __func__, iface);
        goto err;
    }

    dprintf(MSG_MSGDUMP, "%s: Interface %s, channel width %d\n", __func__, iface, chwidth);
    if(wlanIfWd->ops->getChannelExtOffset(wlanIfWd->ctx, iface, &choffset) < 0) {
        dprintf(MSG_ERROR, "%s: ioctl() failed, ifName: %s.\n", __func__, iface);
        goto err;
    }
    ...
    case IEEE80211_CWM_WIDTH160: //160MHz or 80p80MHz
        if (wlanIfWd->ops->getCfreq2(wlanIfWd->ctx, iface, &cfreq2) < 0) {
 ...
 * Sample string: ath0@wifi0:WLAN,ath1@wifi1:WLAN
...
void apacHyfi20CmdLogFileModeName(int argc, char **argv)
{
    ...
    for (;;) {
        c = getopt(argc, argv, "f:r:l:o:c:m:M:wa");
        if (c < 0)
            break;
        switch (c) {
        case 'w':
