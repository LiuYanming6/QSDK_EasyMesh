...
int apac_atf_config_line_getparam(const char *buf, char delim, char *out, size_t outLen) {
    int len;
    char *newpos;
...
int apac_atf_config_line_lex(char *buf, ATF_CONFIG *atf_cfg) {
    ...
    for (pos = buf; ; pos++) {
       ...
       buf = pos;
       if (*pos >= UNICODE_START || isgraph(*pos)) break;
    }
    while ((*pos >= UNICODE_START || isgraph(*pos)) && *pos != '=') pos++;
    if (*pos == '=') {
    ...
    /* get RE identifier */
    pos = buf;
    len = apac_atf_config_line_getparam(pos, '_', atf_cfg->ident, sizeof(atf_cfg->ident));
    ...
    /* get Command */
    pos += len + 1;
    len = apac_atf_config_line_getparam(pos, '_', atf_cfg->cmd, sizeof(atf_cfg->cmd));
    if (len == -1) {
    ...
    pos += len + 1;
    len =
        apac_atf_config_line_getparam(pos, '_', atf_cfg->subcmd, sizeof(atf_cfg->subcmd));
    if (len == -1) {
        return 0;
    }
...
int apac_atf_re_mac_update(void *ptrData, ATF_CONFIG *atfcfg, int index ) {
    ...
    strlcpy(pConfig->atfConf[num_repeaters].identifier, atfcfg->ident, sizeof(pConfig->atfConf[num_repeaters].identifier));
...
int apac_atf_radio_params_update(void* ptrData, ATF_CONFIG *atfcfg, int index ) {
    ...
    len = strlen(atfcfg->value);
    dprintf(MSG_MSGDUMP, "%s index : %d subcmd : %s len : %d\n", __func__, cmd_index, atfcfg->subcmd, len);
    if(streq(atfcfg->subcmd, "name")) {
        if(strlen(atfcfg->value) <= ATF_INTERFACE_MAX_LEN ) {
            strlcpy(pConfig->atfConf[rep_index].radioparams[cmd_index].interface, atfcfg->value,
                    sizeof(pConfig->atfConf[rep_index].radioparams[cmd_index].interface));
            pConfig->atfConf[rep_index].radioparams[cmd_index].interface[len] = '\0';
        } else {
...
int apac_atf_ssid_config_update(void* ptrData, ATF_CONFIG *atfcfg, int index ) {
    ...
    if(streq(atfcfg->subcmd, "entry")) {
        if(strlen(atfcfg->value) <= IEEE80211_NWID_LEN ) {
            strlcpy(pConfig->atfConf[rep_index].vap[cmd_index].ssid, atfcfg->value,
                   sizeof(pConfig->atfConf[rep_index].vap[cmd_index].ssid));
            pConfig->atfConf[rep_index].vap[cmd_index].ssid[len] = '\0';
        } else {
        ...
        if(strlen(atfcfg->value) <= ATF_INTERFACE_MAX_LEN ) {
            strlcpy(pConfig->atfConf[rep_index].vap[cmd_index].interface, atfcfg->value,
                   sizeof(pConfig->atfConf[rep_index].vap[cmd_index].interface));
            pConfig->atfConf[rep_index].vap[cmd_index].interface[len] = '\0';
        } else {
        ...
        if(strlen(atfcfg->value) <= ATF_CMD_MAX_LEN ) {
            strlcpy(pConfig->atfConf[rep_index].vap[cmd_index].cmd, atfcfg->value,
                sizeof(pConfig->atfConf[rep_index].vap[cmd_index].cmd));
            pConfig->atfConf[rep_index].vap[cmd_index].cmd[len] = '\0';
        } else {
...
int apac_atf_sta_config_update( void* ptrData, ATF_CONFIG *atfcfg, int index ) {
    ...
    if(streq(atfcfg->subcmd, "entry")) {
        ...
        if(strlen(atfcfg->value) <= ATF_INTERFACE_MAX_LEN ) {
            strlcpy(pConfig->atfConf[rep_index].peer[cmd_index].interface, atfcfg->value,
                   sizeof(pConfig->atfConf[rep_index].peer[cmd_index].interface));
            pConfig->atfConf[rep_index].peer[cmd_index].interface[len] = '\0';
        } else {
            dprintf(MSG_ERROR, "%s Interface len cannot be more than %d\n",__func__, ATF_INTERFACE_MAX_LEN);
            return 1;
        }
        ...
        if(strlen(atfcfg->value) <= ATF_CMD_MAX_LEN ) {
            strlcpy(pConfig->atfConf[rep_index].peer[cmd_index].cmd, atfcfg->value,
                    sizeof(pConfig->atfConf[rep_index].peer[cmd_index].cmd));
            pConfig->atfConf[rep_index].peer[cmd_index].cmd[len] = '\0';
        } else {
            dprintf(MSG_ERROR, "%s Command len cannot be more than %d\n",__func__, ATF_CMD_MAX_LEN );
            return 1;
        }
        ...
        strlcpy(pConfig->atfConf[rep_index].peer[cmd_index].ssid, atfcfg->value,
              sizeof(pConfig->atfConf[rep_index].peer[cmd_index].ssid));
        pConfig->atfConf[rep_index].peer[cmd_index].ssid[len] = '\0';
...
int apac_atf_group_config_update(void* ptrData, ATF_CONFIG *atfcfg, int index ) {
    ...
    if(streq(atfcfg->subcmd, "entry")) {
        if(strlen(atfcfg->value) <= IEEE80211_NWID_LEN ) {
            strlcpy(pConfig->atfConf[rep_index].group[cmd_index].grpname, atfcfg->value,
                   sizeof(pConfig->atfConf[rep_index].group[cmd_index].grpname));
            pConfig->atfConf[rep_index].group[cmd_index].grpname[len] = '\0';
        } else {
        ...
        if(strlen(atfcfg->value) <= ATF_INTERFACE_MAX_LEN ) {
            strlcpy(pConfig->atfConf[rep_index].group[cmd_index].interface, atfcfg->value,
                   sizeof(pConfig->atfConf[rep_index].group[cmd_index].interface));
            pConfig->atfConf[rep_index].group[cmd_index].interface[len] = '\0';
        } else {
            dprintf(MSG_ERROR, "%s Interface len cannot be more than %d\n",__func__, ATF_INTERFACE_MAX_LEN);
            return 1;
        }
        ...
        if(strlen(atfcfg->value) <= ATF_CMD_MAX_LEN ) {
            strlcpy(pConfig->atfConf[rep_index].group[cmd_index].cmd, atfcfg->value,
                   sizeof(pConfig->atfConf[rep_index].group[cmd_index].cmd));
            pConfig->atfConf[rep_index].group[cmd_index].cmd[len] = '\0';
        } else {
            dprintf(MSG_ERROR, "%s Command len cannot be more than %d\n",__func__, ATF_CMD_MAX_LEN);
            return 1;
        }
        ...
        if( (grp_index < ATF_MAX_SSID_GROUP ) &&
            ...
            strlcpy(pConfig->atfConf[rep_index].group[cmd_index].grp_ssid[grp_index], atfcfg->value,
                   sizeof(pConfig->atfConf[rep_index].group[cmd_index].grp_ssid[grp_index]));
            pConfig->atfConf[rep_index].group[cmd_index].grp_ssid[grp_index][len] = '\0';
    ...
    if( pConfig->atf_config_enabled ) {
        ...
        for(i = 0; i < num_repeaters; i++) {
            ...
            for(k = 0; k < pConfig->atfConf[i].peer_num_cfg; k++) {
                dprintf(MSG_INFO, "cmd : %s, intf : %s sta_mac : %s val : %d ssid : %s\n",
                        pConfig->atfConf[i].peer[k].cmd, pConfig->atfConf[i].peer[k].interface,
                        print_macaddr(pConfig->atfConf[i].peer[k].sta_mac),
                        pConfig->atfConf[i].peer[k].val, pConfig->atfConf[i].peer[k].ssid);
            }
    ...
    if (pData->config.sendOnAllIFs == APAC_FALSE) {
        ...
        for (i = 0; i < APAC_MAXNUM_HYIF; i++) {
           if (apacHyfi20SendL2Packet(&pIF[i], frame, frameLen) < 0) {
                perror("apacHyfi20SendAtfConfig-onAllIFs");
                return -1;
            }
...
int apacHyfi20ConfigureAtf(ATF_REP_CONFIG *apAtfConfig) {
    ...
    for(k = 0; k < apAtfConfig->peer_num_cfg; k++) {
        dprintf(MSG_INFO, "cmd : %s, intf : %s sta_mac : %s val : %d ssid : %s\n",
                apAtfConfig->peer[k].cmd, apAtfConfig->peer[k].interface, print_macaddr(apAtfConfig->peer[k].sta_mac),
                apAtfConfig->peer[k].val, apAtfConfig->peer[k].ssid);
        if(streq (apAtfConfig->peer[k].cmd, "addsta")) {
            ret = apacHyfi20ConfigAtf_addsta(apAtfConfig->peer[k].interface, apAtfConfig->peer[k].sta_mac,
                                             apAtfConfig->peer[k].val,apAtfConfig->peer[k].ssid);
...
int apacHyfi20ConfigAtf_radioparams( char *iface, u_int32_t param, u_int32_t val) {
    ...
    if(wlanIfWd->ops->setParam(wlanIfWd->ctx, iface, (param | ATH_PARAM_SHIFT), &val , sizeof(val)) < 0) {
...
int apacHyfi20ConfigAtf_addgroup( char *ifname, char *grpname, char *grpssid ) {
    ...
    if (wlanIfWd->ops->getGenericInfoAtf(wlanIfWd->ctx, ifname, IEEE80211_IOCTL_ATF_ADDGROUP,  &set_group, sizeof(set_group)) < 0) {
...
int apacHyfi20ConfigAtf_delgroup(const char *ifname, char *grpname) {
    ...
    if (wlanIfWd->ops->getGenericInfoAtf(wlanIfWd->ctx, ifname, IEEE80211_IOCTL_ATF_DELGROUP, &del_group, sizeof(del_group)) < 0) {
        dprintf(MSG_ERROR, "Unable to add ATF group\n");
        return -1;
    }
...
int apacHyfi20ConfigAtf_configgroup(const char *ifname, char *grpname, u_int32_t val) {
    ...
    if (wlanIfWd->ops->getGenericInfoAtf(wlanIfWd->ctx, ifname, IEEE80211_IOCTL_ATF_CONFIGGROUP, &config_group, sizeof(config_group)) < 0) {
...
int apacHyfi20ConfigAtf_addssid(const char *ifname, char *ssid, u_int32_t val) {
    ...
    if (wlanIfWd->ops->getGenericInfoAtf(wlanIfWd->ctx, ifname, IEEE80211_IOCTL_ATF_ADDSSID, &set_atp, sizeof(set_atp)) < 0) {
...
int apacHyfi20ConfigAtf_delssid(const char *ifname, char *ssid) {
    ...
    if (wlanIfWd->ops->getGenericInfoAtf(wlanIfWd->ctx, ifname, IEEE80211_IOCTL_ATF_DELSSID, &set_atp, sizeof(set_atp)) < 0) {
...
int apacHyfi20ConfigAtf_addsta(const char *ifname, u8 *macaddr, u_int32_t val, const char *ssid) {
    ...
    os_memcpy(&(set_sta.ssid[0]), ssid, strlen(ssid));
    set_sta.id_type = IEEE80211_IOCTL_ATF_ADDSTA;

    if (wlanIfWd->ops->getGenericInfoAtf(wlanIfWd->ctx, ifname, IEEE80211_IOCTL_ATF_ADDSTA, &set_sta, sizeof(set_sta)) < 0) {
...
int apacHyfi20ConfigAtf_delsta(const char *ifname, u8 *macaddr) {
    ...
    if (wlanIfWd->ops->getGenericInfoAtf(wlanIfWd->ctx, ifname, IEEE80211_IOCTL_ATF_DELSTA, &set_sta, sizeof(set_sta)) < 0) {
...
int apacHyfi20atfGroupEnable( const char *iface, int val) {
    ...
    if(wlanIfWd->ops->setParam(wlanIfWd->ctx, iface, IEEE80211_PARAM_ATF_SSID_GROUP, &val , sizeof(val)) < 0) {
        dprintf(MSG_ERROR, "%s: ioctl() failed, ifName: %s\n", __func__, iface);
        goto err;
    }
...
int apacHyfi20atfCommit( const char *iface, int val) {
    ...
    if(wlanIfWd->ops->setParam(wlanIfWd->ctx, iface, IEEE80211_PARAM_ATF_OPT, &val , sizeof(val)) < 0) {
        dprintf(MSG_ERROR, "%s: ioctl() failed, ifName: %s\n", __func__, iface);
        goto err;
    }
