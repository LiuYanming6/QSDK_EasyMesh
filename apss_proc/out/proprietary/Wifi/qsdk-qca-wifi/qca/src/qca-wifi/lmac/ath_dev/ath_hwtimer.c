                    ...
                    void *arg)
{
    ...
    timer = (struct ath_gen_timer *) OS_MALLOC(sc->sc_osdev,
                                               ...
                                               GFP_KERNEL);
    if (timer == NULL) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: can't alloc timer object\n", __func__);
        return NULL;
    }
    ...
    if (index < 0) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: no hardware generic timer slot available\n", __func__);
        OS_FREE(timer);
        return NULL;
    }
