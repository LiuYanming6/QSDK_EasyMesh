...
/*
 * for tgt iram address
 * As AR9888, AR900B based platforms use the same addresses, using
 * any of the respective soc header file will suffice.
 */
...
extern A_STATUS hif_pktlog_subscribe(
                struct ol_ath_softc_net80211 *scn);

#if ATH_SUPPORT_WRAP && DBDC_REPEATER_SUPPORT
static OS_TIMER_FUNC(ol_psta_creation_timeout);
#endif
...
int count_bits_set (u_int8_t val);
void node_all_unblock (void *arg, struct ieee80211_node *ni);
void node_all_block (void *arg, struct ieee80211_node *ni);
#endif

#if  QCN_ESP_IE
int ol_ath_esp_estimate_event_handler (ol_scn_t scn, u_int8_t *data, u_int16_t datalen);
#endif
...
int ol_ath_target_stop(struct ieee80211com *ic, bool flush);
extern int emu_type;
...
#else
    /* ARM PLATFORM */
#if  LINUX_VERSION_CODE >= KERNEL_VERSION(3,14,0) && !QCA_PARTNER_PLATFORM
...
ol_target_failure(void *instance, QDF_STATUS status)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "[%s][0x%08X]: XXX TARGET ASSERTED XXX\n", scn->sc_osdev->netdev->name, scn->target_type);
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
    ...
    if(dbglog_buf.length) {
        ...
        if (dbglog_data) {
                        ...
                        dbglog_buf.length)!= QDF_STATUS_SUCCESS)
            {
                ...
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "dbglog_hdr.dbuf=%pK dbglog_data=%pK dbglog_buf.buffer=%pK dbglog_buf.length=%u\n",
...
ol_transfer_swap_struct(struct ol_ath_softc_net80211 *scn, ATH_SWAP_INFO swap_info,char *bin_filename) {
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO"scn=%pK  target_write_addr=%x seg_info=%pK \n", scn,target_write_addr,(u_int8_t *)seg_info);
                      ...
                      u_int32_t address, bool compressed)
{
    ...
    if (status) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Board data download failed, download address: %pK\n",
                 __func__, (void *)(uintptr_t)address);
    } else {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Board data file: %d successfully downloaded, download address: %pK\n",
...
static int ol_ath_handle_host_mem_req(struct ol_ath_softc_net80211 *scn, uint8_t *evt_buf)
{
    ...
    if (num_mem_reqs) {
        ...
        for (i = 0; i < FW_PRIORITY_MAX; i++) {
            for (idx = 0; idx < num_mem_reqs; ++idx) {
                 ...
                 */
                if (mem_reqs[idx].req_id == IRAM_BKP_PADDR_REQ_ID) {
                    iram_bkp_req = TRUE;
                    continue;
                }
                if (((i == FW_MEM_HIGH_PRIORITY) && (mem_reqs[idx].num_unit_info & HOST_CONTIGUOUS_MEM_CHUNK_REQUIRED))
                    ...
                    if (status == A_ERROR) {
                        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s() scn:(%pK) num_mem_chunk exceeds supported number\n", __func__, scn);
                        return -1;
                    } else if (status == A_NO_MEMORY) {
                        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s() scn:(%pK) Mem allocation failure \n", __func__, scn);
                        return -1;
                    }
                }
            }
    ...
    /*
     * Populate entry for the address of tgt iram-bkp.
     * This request is special as host doesnt need to
     * allocate any m/r.Only the paddr for the FW iram bkp
     * in host needs to be passed to FW.
     */
    if (iram_bkp_req) {
           uint8_t num_chunks = scn->num_mem_chunks;
           scn->mem_chunks[num_chunks].paddr = scn->tgt_iram_bkp_paddr;
           scn->mem_chunks[num_chunks].vaddr = NULL;
           scn->mem_chunks[num_chunks].len = 0;
           scn->mem_chunks[num_chunks].req_id = IRAM_BKP_PADDR_REQ_ID;
           scn->num_mem_chunks += 1;
    }
    return 0;
}
...
ol_ath_service_ready_event(ol_scn_t scn_handle, uint8_t *evt_buf, uint16_t len)
{
    ...
    ic->ic_is_host_dfs_check_enabled =
        wmi_service_enabled(scn->wmi_handle,wmi_service_host_dfs_check_support);
#if ATH_SUPPORT_CFR
    if (wmi_service_enabled(scn->wmi_handle, wmi_service_cfr_capture_support)){
        scn->cfr_support    = 1;
    }
#endif
    ...
    if (wmi_service_enabled(scn->wmi_handle, wmi_service_ext_res_cfg_support)) {
        ...
        if (scn->carrier_vow_config) {
                qdf_print("WMI_HOST_FW_FEATURE_VOW_FEATURES %d\n",WMI_HOST_FW_FEATURE_VOW_FEATURES);
                scn->wlan_ext_resource_config.fw_feature_bitmap |= WMI_HOST_FW_FEATURE_VOW_FEATURES;
                if (scn->fw_vow_stats_enable)
                        scn->wlan_ext_resource_config.fw_feature_bitmap |= WMI_HOST_FW_FEATURE_VOW_STATS;
        }
...
/*          scn->wlan_resource_config.num_vdevs = CFG_TGT_NUM_VDEV_ATF;
    ...
    if (scn->btcoex_support) {
        if (ol_ath_pdev_set_param(scn, wmi_pdev_param_enable_btcoex,scn->btcoex_enable, 0) == EOK ) {
            qdf_print("Set  btcoex_enable:%d \n",scn->btcoex_enable);
        } else {
            qdf_print("Failed to send btcoex enable cmd :%d \n",scn->btcoex_enable);
        }
    }
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,5,0)
             qdf_assert_always(ic->ic_smart_log_fw_pktlog_enable != NULL);
             ic->ic_smart_log_fw_pktlog_enable(ic);
#else /* LINUX_VERSION_CODE >= KERNEL_VERSION(3,5,0) */
             qdf_sched_work(scn->qdf_dev, &ic->smart_log_fw_pktlog_enable_work);
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(3,5,0) */
#else /* REMOVE_PKT_LOG */
             qdf_print("Not enabling smart log FW initiated packetlog feature "
                       "since packetlog is not available at host\n");
...
int
ol_ath_set_atf_grp_ac(struct ieee80211com *ic)
{
	struct ol_ath_softc_net80211 *scn = OL_ATH_SOFTC_NET80211(ic);
	struct atf_grp_ac_params grp_ac_params;
	int i,retval = 0;

	qdf_mem_set(&grp_ac_params, sizeof(grp_ac_params), 0);

	grp_ac_params.num_groups = ic->wmi_atf_grp_ac_req.num_groups;

	for (i =0; i< ic->wmi_atf_grp_ac_req.num_groups; i++)
	{
		OS_MEMCPY((void *)&(grp_ac_params.group_info[i]),(void *)&(ic->wmi_atf_grp_ac_req.atf_group_wmm_ac_info[i]),sizeof
					(struct atf_group_wmm_ac_info));
	}

    if(!ic->ic_atf_tput_based){
		retval = wmi_send_set_atf_grp_ac_cmd(scn->wmi_handle, &grp_ac_params);
	}
	return retval;
}
...
ol_ath_fips_event_handler(ol_scn_t scn, u_int8_t *evt_buf, u_int16_t datalen)
{
    ...
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s ic->ic_output_fips %pK\n", __func__, ic->ic_output_fips);
...
#define QCA_WIFITOOL_IRAM_DUMP_COMMAND 164
    if (arg == QCA_WIFITOOL_IRAM_DUMP_COMMAND){
        qdf_print("Command not supported\n");
        return;
    }
#endif
...
{
    struct ol_ath_softc_net80211 *scn = OL_ATH_SOFTC_NET80211(ic);
    return scn->scn_stats.tx_retries;
}
...
{
    struct ol_ath_softc_net80211 *scn = OL_ATH_SOFTC_NET80211(ic);
    return (scn->scn_stats.tx_num_data + scn->scn_stats.tx_mgmt);
}
...
extern void ol_ath_set_vap_dscp_tid_map_partner(uint8_t* dscp_to_tid_map,int vap_id);
#endif
int
...
#if QCA_PARTNER_DIRECTLINK
    ol_ath_set_vap_dscp_tid_map_partner(param.dscp_to_tid_map,avn->av_if_id);
#endif
 ...
 *
 * @param ic - pointer to struct ieee80211com for the physical device
 *
 ...
 * been started), QDF_STATUS_E_** on error (an error message will be printed).
 */
static int32_t ol_smart_log_connection_fail_start(struct ieee80211com *ic)
{
    int32_t ret = 0;
    struct ol_ath_softc_net80211 *scn = NULL;
    ...
    if (ret != QDF_STATUS_SUCCESS) {
        qdf_print("Error %d: Could not successfully send command to start "
                  "connection failure logging. Smart logging or rest of "
                  "system may no longer function as expected. Investigate!!.\n"
                  "Preserving connection failure logging status as stopped "
 ...
 * @return QDF_STATUS_SUCCESS on success or if the API is inapplicable (since
 * smart logging is disabled, or connection failure smart logging has already
 * been stopped), QDF_STATUS_E_** on error (an error message will be printed).
 */
static int32_t ol_smart_log_connection_fail_stop(struct ieee80211com *ic)
{
    int32_t ret = 0;
    struct ol_ath_softc_net80211 *scn = NULL;
    ...
    if (ret != QDF_STATUS_SUCCESS) {
        qdf_print("Error %d: Could not successfully send command to stop "
                  "connection failure logging. Smart logging or rest of "
                  "system may no longer function as expected. Investigate!!.\n"
                  "Marking connection failure logging status as stopped "
                  "within host. This may not be consistent with FW state due "
 ...
 * @param ic - pointer to struct ieee80211com for the physical device
 *
 * @return QDF_STATUS_SUCCESS on success, QDF_STATUS_E_** on error (an error
 * message will be printed).
 */
static int32_t ol_smart_log_fw_pktlog_enable(struct ieee80211com *ic)
{
    int32_t ret = 0;
    ...
    if (!ic->smart_logging) {
        qdf_print("%s: Smart logging not enabled. Ignoring FW packetlog enable "
                  "request.\n", __func__);
 ...
 * message will be printed).
 */
static int32_t ol_smart_log_fw_pktlog_disable(struct ieee80211com *ic)
{
    struct ol_ath_softc_net80211 *scn = NULL;
    int32_t ret = 0;
    ...
    if (!ic->smart_log_fw_pktlog_enabled) {
        qdf_print("%s: Smart log FW initiated packetlog not enabled. Ignoring "
                  "FW packetlog disable request.\n", __func__);
        ret = QDF_STATUS_E_INVAL;
        goto done;
    }
    ...
    if (scn->pl_dev && ic->smart_logging &&
           ...
           ((ret = smart_log_fw_pktlog_stop(ic,
                            SMART_LOG_FW_PKTLOG_STOP_DISABLE)) !=
                QDF_STATUS_SUCCESS)) {
        qdf_print("%s: Could not successfully stop FW initiated packet "
                  "logging. Investigate!\n"
 ...
 * message will be printed).
 */
static int32_t ol_smart_log_fw_pktlog_start(struct ieee80211com *ic,
        u_int32_t fw_pktlog_types)
{
    ...
    if (!scn->pl_dev) {
        qdf_print("%s: Packetlog pl_dev unavailable. Ignoring FW packetlog "
                  "start request.\n", __func__);
        ret = QDF_STATUS_E_INVAL;
        goto done;
    }
    ...
    if (!ic->smart_logging) {
        qdf_print("%s: Smart logging not enabled. Ignoring FW packetlog start "
                  "request.\n", __func__);
        ret = QDF_STATUS_E_INVAL;
        goto done;
    }
    ...
    if (!ic->smart_log_fw_pktlog_enabled) {
        qdf_print("%s: Smart log FW initiated packetlog not enabled. Ignoring "
                  "FW packetlog start request. Investigate!\n", __func__);
        ret = QDF_STATUS_E_INVAL;
        goto done;
    }

    if (ic->smart_log_fw_pktlog_blocked) {
        qdf_print("%s: Smart log FW initiated packetlog is blocked due to host "
                  "initiated packetlog. Ignoring FW packetlog start request.\n",
                  __func__);
        ret = QDF_STATUS_E_INVAL;
        goto done;
    }

    if (ic->smart_ant_enable && (scn->target_type == TARGET_TYPE_AR9888)) {
         qdf_print("%s: Host initiated smart antenna packet logging already in "
    ...
    if (ic->smart_log_fw_pktlog_started) {
        qdf_print("%s: FW initiated packet logging already in progress. "
                  "Ignoring new FW packetlog start request.\n", __func__);
        ret = QDF_STATUS_E_INVAL;
        goto done;
    }
    ...
    if(fw_pktlog_types & WMI_HOST_PKTLOG_EVENT_SMART_ANTENNA) {
        qdf_print("%s - Warning: Unexpected packet log type "
    ...
    /* TODO: Apply restrictions for WMI_HOST_PKTLOG_EVENT_TX_DATA_CAPTURE if
     * required. However, for now FW is expected to know what it is doing in
    ...
    if (scn->pl_dev->pl_funcs->pktlog_enable(scn, host_pktlog_types) == 0) {
        ic->smart_log_fw_pktlog_started = true;
        ret = QDF_STATUS_SUCCESS;
    } else {
        qdf_print("%s: Error - Could not successfully start FW initiated "
                  "packetlog. Smart logging or rest of system may no longer "
                  "function as expected. Investigate!\n"
                  "Maintaining FW initiated packetlog status as stopped "
                  "within host. This may or may not be consistent with FW "
                  "state.\n",  __func__);
 ...
 *
 * @return QDF_STATUS_SUCCESS on success, QDF_STATUS_E_** on error (an error
 * message will be printed by the helper function used by this function).
 */
static int32_t ol_smart_log_fw_pktlog_stop(struct ieee80211com *ic)
{
    int32_t ret = 0;
    ...
    qdf_mutex_acquire(&ic->smart_log_fw_pktlog_mutex);
    ret = smart_log_fw_pktlog_stop(ic, SMART_LOG_FW_PKTLOG_STOP_NORMAL);
    qdf_mutex_release(&ic->smart_log_fw_pktlog_mutex);
 ...
 * inapplicable, as well), QDF_STATUS_E_** on error (an error message will
 * be printed).
 */
static int32_t ol_smart_log_fw_pktlog_stop_and_block(struct ieee80211com *ic,
        int32_t host_pktlog_types, bool block_only_if_started)
{
    ...
    if (scn->pl_dev && ic->smart_logging &&
        ...
        if (!host_pktlog_types) {
            qdf_print("Stopping FW initiated packetlog due to host initiated "
                      "stop request\n");
        } else {
            qdf_print("Stopping FW initiated packetlog due to host initiated "
                      "start request\n");
        ...
        if ((ret = smart_log_fw_pktlog_stop(ic,
                        SMART_LOG_FW_PKTLOG_STOP_HOSTREQ)) !=
                QDF_STATUS_SUCCESS) {
            qdf_print("%s: Could not successfully stop FW initiated packet "
                      "logging. Investigate!\n"
                      "However, FW initiated packetlog has been marked as "
 ...
 * @param ic - pointer to struct ieee80211com for the physical device
 */
static void ol_smart_log_fw_pktlog_unblock(struct ieee80211com *ic)
{
    qdf_assert_always(ic != NULL);
...
static void ol_unauthorized_sta_timeout(struct ieee80211_node_table *nt)
{
    struct ieee80211_node *ni;
    u_int gen;
    systime_t now;
...
static OS_TIMER_FUNC(ol_ath_timeout)
{
    struct ieee80211com *ic;
    OS_GET_TIMER_ARG(ic, struct ieee80211com *);
    ol_unauthorized_sta_timeout(&ic->ic_sta);
...
ol_ath_get_bw_nss_mapping(struct ieee80211vap *vap, struct ieee80211_bwnss_map *nssmap, u_int8_t chainmask)
{
    ...
    if ((error != -EINVAL) && (vap->iv_nss != 0)) {
        nssmap->bw_nss_160 = QDF_MIN(vap->iv_nss, nssmap->bw_nss_160);
        if ((nssmap->bw_nss_160) == (vap->iv_nss)) {
                   ...
                   IEEE80211_REG_PARAMETERS *ieee80211_conf_parm)
{
...
#ifdef ATH_SUPPORT_RDKB
    wlanstats_init(&ic->wlanstats_ctx);
#endif
    ...
    ic->ic_whal_kbps_to_mcs = whal_kbps_to_mcs;
    ic->ic_whal_ratecode_to_kbps = whal_ratecode_to_kbps;
    ic->ic_get_supported_rates = whal_get_supported_rates;
    ...
    ic->ic_set_atf_grp_ac = ol_ath_set_atf_grp_ac;
    ...
    ic->ic_whc_scaling_factor = WHC_DEFAULT_SFACTOR;

#if OL_ATH_SMART_LOGGING
    ic->ic_smart_log_connection_fail_start = ol_smart_log_connection_fail_start;
    ic->ic_smart_log_connection_fail_stop = ol_smart_log_connection_fail_stop;
#endif /* OL_ATH_SMART_LOGGING */
...
#endif
        qdf_dev_release_mem((struct qdf_dev *)scn->sc_osdev->device, (void *)seg_info);
    }
}
void
...
/*
 * API to read the iram content from the
 * target memory and write into the F/W
 * dump location.
 */
static void
ol_ath_copy_tgt_iram(struct ol_ath_softc_net80211 *scn)
{

#if QCA_TGT_FW_IRAM_DUMP_ENABLE
    uint32_t tgt_iram_paddr = TARG_IRAM_START;
    A_UINT32 *tgt_iram_bkp_vaddr;
    A_UINT32 fw_iram_bkp_addr = 0;
    A_UINT32 offset = (scn->radio_id * 0x200000); /* 2MB per radio */
    ...
    dev_node = of_find_node_by_name (NULL, "wifi_dump");
    if (dev_node) {
        if((ret = of_property_read_u32_array(dev_node,"reg", &fw_iram_bkp_addr, 1))) {
    ...
    switch (scn->target_type) {
        ...
        case TARGET_TYPE_QCA9888:
        case TARGET_TYPE_QCA9984:
            fw_iram_bkp_addr = FW_IO_MEM_ADDR_AR900B;
            break;
        case TARGET_TYPE_IPQ4019:
            fw_iram_bkp_addr = FW_IO_MEM_ADDR_IPQ4019;
            break;
    }
#endif /* LINUX_VERSION_CODE */
    fw_iram_bkp_addr += offset;

    scn->tgt_iram_bkp_paddr = fw_iram_bkp_addr;
    tgt_iram_bkp_vaddr = ioremap_nocache(scn->tgt_iram_bkp_paddr, (TARG_IRAM_SZ + 1));
    printk("\n%s() iram read paddr: 0x%x host paddr 0x%x Radio id %d\n",
             __func__, tgt_iram_paddr, scn->tgt_iram_bkp_paddr, scn->radio_id);
    if (hif_diag_read_mem(scn->hif_hdl,
                       tgt_iram_paddr,
                       (A_UCHAR *)tgt_iram_bkp_vaddr,
                       TARG_IRAM_SZ
                       )!= QDF_STATUS_SUCCESS)
        printk("%s(): HifDiagReadMem FW IRAM content failed\n",__func__);

    iounmap(tgt_iram_bkp_vaddr);
#else /* if the FW IRAM dump feature is disabled*/
    scn->tgt_iram_bkp_paddr = TGT_FW_IRAM_DUMP_DISABLED;
#endif /* QCA_TGT_FW_IRAM_DUMP_ENABLE */
    return;
}
...
#define HOST_CALDATA_SIZE (16 * 1024)
    ...
    {
        qdf_print("\n scn->cal_mem %pK AH_CAL_LOCATIONS_PCI %x HOST_CALDATA_SIZE %x\n",
    ...
    /*
     * Time to read target iram content and create the backup.
     * This feature is supported for BL family only
     */
    if (scn->is_ar900b) {
        ol_ath_copy_tgt_iram(scn);
    }
    /* Spoof DFS check happens in the first vap (configured in DFS channel)
     * after every FW download.So resetting the variables here
     */
    scn->sc_ic.ic_spoof_dfs_check_failed = 0;
    scn->sc_ic.ic_spoof_test_done = 0;
    scn->sc_ic.ic_rebuilt_chanlist = 0;
    return 0;
...
int htc_wmi_init(struct ol_ath_softc_net80211 *scn)
{
    ...
    htcInfo.FWhangFailure = fw_hang_failure;
    ...
    qdf_print(KERN_INFO"%s() HT Create . %pK\n", __func__, scn->htc_handle);
...
#ifdef ATH_SUPPORT_RDKB
int
ol_wlan_txstat_start(struct ol_ath_softc_net80211 *scn)
{
    wmi_buf_t buf;
              ...
              ol_ath_update_fw_config_cb cfg_cb)
{
    ...
    qdf_print(KERN_INFO"%s() WMI attached. wmi_handle %pK \n", __func__, scn->wmi_handle);
...
#ifndef REMOVE_PKT_LOG
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,5,0)
    qdf_create_work(scn->qdf_dev, &ic->smart_log_fw_pktlog_enable_work,
            smart_log_fw_pktlog_enable_work_handler, (void *)ic);
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(3,5,0) */
    qdf_mutex_create(&ic->smart_log_fw_pktlog_mutex);
    ic->ic_smart_log_fw_pktlog_enable = ol_smart_log_fw_pktlog_enable;
    ic->ic_smart_log_fw_pktlog_disable = ol_smart_log_fw_pktlog_disable;
    ic->ic_smart_log_fw_pktlog_start = ol_smart_log_fw_pktlog_start;
    ic->ic_smart_log_fw_pktlog_stop = ol_smart_log_fw_pktlog_stop;
    ic->ic_smart_log_fw_pktlog_stop_and_block =
        ol_smart_log_fw_pktlog_stop_and_block;
    ic->ic_smart_log_fw_pktlog_unblock = ol_smart_log_fw_pktlog_unblock;
#endif /* REMOVE_PKT_LOG */
#endif /* OL_ATH_SMART_LOGGING */

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,5,0)
    /* In case ol_ath_service_ready_event() is no longer registered under
     * WMI_RX_WORK_CTX in the future, take care to enable smart log FW initiated
     * packetlog from a separate work queue.
     */
	...
	ol_ath_lowi_wmi_event_attach(ic);
...
#if QCN_ESP_IE
        wmi_unified_register_event_handler(scn->wmi_handle, wmi_esp_estimate_event_id,
        ...
        wmi_unified_register_event_handler(scn->wmi_handle,
                ...
                ol_ath_wmi_dfs_status_check_event_handler, WMI_RX_UMAC_CTX);
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Set global_ic[%d] ..ptr:%pK\n", __func__, ic_list.num_global_ic,&ic_list);
    ...
    for (i=0; i < MAX_RADIO_CNT; i++) {
        memset(&ic_list.preferred_mac_list[i][0], 0, IEEE80211_ADDR_LEN);
        memset(&ic_list.denied_mac_list[i][0], 0, IEEE80211_ADDR_LEN);
    }
#if ATH_SUPPORT_WRAP
    ic_list.psta_creation_timer_flag = 0;
#endif
...
#if  ATH_SUPPORT_WRAP
    if (ic_list.num_global_ic == 1) {
        OS_INIT_TIMER(ic->ic_osdev, &(ic_list.psta_creation_timer), ol_psta_creation_timeout,
                (void *)ic->ic_global_list, QDF_TIMER_TYPE_WAKE_APPS);
    }
#endif
#endif
    ...
    if(IS_WLANSTATS_CFG_ACTIVE(ic)) {
        status = ol_wlan_txstat_start(scn);
        if (status != EOK)
            qdf_print("ps: failed to start txstat: %d\n", status);
    }
#endif
    return EOK;
...
#ifndef REMOVE_PKT_LOG
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,5,0)
    qdf_flush_work(scn->qdf_dev, &ic->smart_log_fw_pktlog_enable_work);
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(3,5,0) */
#endif /* REMOVE_PKT_LOG */
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,5,0)
    qdf_destroy_work(scn->qdf_dev, &ic->smart_log_fw_pktlog_enable_work);
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(3,5,0) */
#endif /* REMOVE_PKT_LOG */
#endif /* OL_ATH_SMART_LOGGING */
...
#endif
    /*
     * If tgt iram bkp was created, the num_mem_chunks
     * for scn would have been incremented by 1 to take
     * care of the same.However, the mem_chunks array in
     * scn doesnt have an entry for this.Hence, the freeing
     * needs to happen accordingly.
     */
    if (scn->tgt_iram_bkp_paddr && scn->num_mem_chunks) {
        num_mem_chunks = scn->num_mem_chunks - 1;
    } else {
...
ol_ath_detach(struct ol_ath_softc_net80211 *scn, int force)
{
...
#if  ATH_SUPPORT_WRAP
    if (ic_list.num_global_ic == 1) {
        OS_FREE_TIMER(&(ic_list.psta_creation_timer));
    }
#endif
    ...
    for (i=0; i < MAX_RADIO_CNT; i++) {
        ...
        if (tmp_ic && (ic == tmp_ic)) {
            ...
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: remove global_ic[%d]..gloabl_ic ptr:%pK\n", __func__, ic_list.num_global_ic,&ic_list);
            GLOBAL_IC_UNLOCK(&ic_list);
        }
    }
    for (i=0; i < MAX_RADIO_CNT; i++) {
...
#if LINUX_VERSION_CODE == KERNEL_VERSION(3,14,77)
    qcom_pcie_deregister_event(&scn->pcie_event);
#endif
...
int ol_ath_target_stop(struct ieee80211com *ic, bool flush)
{
    ...
    if (flush) {
        flush_work(&scn->pci_reconnect_work);
    }
    ...
    if (ic->recovery_in_progress == 0) {
        qdf_print("Suspending Target  scn=%pK\n", scn);
        if (!ol_ath_suspend_target(scn, 1)) {
            ...
            if(!timeleft || signal_pending(current)) {
                qdf_print("ERROR: Failed to receive target paused event scn=%pK \n", scn);
                target_paused = FALSE;
            }
            /*
...
void *ol_hif_open(struct device *dev, void *bdev, void *bid,
        enum qdf_bus_type bus_type, bool reinit, qdf_device_t qdf_dev, void *mem);
int ol_ath_target_start(struct ol_ath_softc_net80211 *scn)
{
    ...
    if(scn->target_type == TARGET_TYPE_IPQ4019) {
	    scn->hif_hdl = ol_hif_open(&pdev->dev, pdev, (void *)scn->pdevid, QDF_BUS_TYPE_AHB, 0,scn->qdf_dev, NULL);
    } else {
	    pci_set_drvdata(pcidev, NULL);
	    scn->hif_hdl = ol_hif_open(&pcidev->dev, pdev, (void *)scn->pdevid, QDF_BUS_TYPE_PCI, 0,scn->qdf_dev, NULL);
	    pci_set_drvdata(pcidev, dev);
    }
                                      ...
                                      A_UINT32 interference_type)
{
...
#if UMAC_SUPPORT_CBS
    int cbs_csa = ieee80211_cbs_get_param(ic->ic_cbs, IEEE80211_CBS_CSA_ENABLE);
#endif
    ...
    if (scn->scn_icm_active) {
         ...
         */
            TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {
                if ((vap->iv_opmode == IEEE80211_M_HOSTAP) &&
                        ieee80211_vap_active_is_set(vap)) {
...
/* Block all Access category of the node */
void node_all_block (void *arg,struct ieee80211_node *ni)
{
    ni->ni_block_bitmap = ATF_TRAFFIC_CTRL_AC_ALL;
}

/* Unblock all Access category of the node */
void node_all_unblock (void *arg,struct ieee80211_node *ni)
{
    ni->ni_block_bitmap = 0;
}
    ...
    if(evt.peer_ast_idx == WMI_HOST_INVALID_PEER_AST_INDEX) {
        ...
        if (vap) {
            switch (evt.ctrl_cmd) {
                case WMI_HOST_TX_DATA_TRAFFIC_CTRL_UNBLOCK:
                    wlan_iterate_station_list(vap, node_all_unblock, NULL);
                    vap->iv_block_tx_traffic = 0; //allow traffic
                    vap->ac_blk_cnt = 0;
                    break;
                case WMI_HOST_TX_DATA_TRAFFIC_CTRL_BLOCK:
                    wlan_iterate_station_list(vap, node_all_block, NULL);
                    vap->iv_block_tx_traffic = 1; //stop traffic
                    vap->ac_blk_cnt = vap->iv_sta_assoc * WME_NUM_AC;
                    break;
                default:
                    break;
            }
        ...
        if(peer) {
            if(vap) {
                ...
                if(ni) {
                     ac_id = (evt.wmm_ac & 0xF);
                     switch (evt.ctrl_cmd) {
                         case WMI_HOST_TX_DATA_TRAFFIC_CTRL_UNBLOCK:
                            IEEE80211_DPRINTF(ni->ni_vap, IEEE80211_MSG_ATF,
                             "%s WMI_HOST_TX_DATA_TRAFFIC_CTRL_UNBLOCK , ac id is %x ,WMM AC is %x , node is %02x:%02x:%02x:%02x:%02x:%02x\n",
                                          __func__, ac_id, evt.wmm_ac,peer->mac_addr.raw[0],peer->mac_addr.raw[1],peer->mac_addr.raw[2],
                                          peer->mac_addr.raw[3],peer->mac_addr.raw[4],peer->mac_addr.raw[5] );
                             if(ni->ni_block_bitmap) {
                                 if(vap->ac_blk_cnt && ac_id == ATF_TRAFFIC_CTRL_AC_ALL) {
                                     block_bitset = count_bits_set(ni->ni_block_bitmap);
                                     ni->ni_block_bitmap &= ~(ATF_TRAFFIC_CTRL_AC_ALL);
                                     vap->ac_blk_cnt -= block_bitset;
                                 } else {
                                     ni->ni_block_bitmap &= ~(1 << (ac_id-1));
                                     if(vap->ac_blk_cnt)
                                         vap->ac_blk_cnt--;
                                 }
                             }
                         ...
                         case WMI_HOST_TX_DATA_TRAFFIC_CTRL_BLOCK:
                            IEEE80211_DPRINTF(ni->ni_vap, IEEE80211_MSG_ATF,
                             "%s WMI_HOST_TX_DATA_TRAFFIC_CTRL_BLOCK , ac id is %x , WMM AC is %x , node is %02x:%02x:%02x:%02x:%02x:%02x\n",
                               __func__, ac_id, evt.wmm_ac,peer->mac_addr.raw[0],peer->mac_addr.raw[1],peer->mac_addr.raw[2],
                               peer->mac_addr.raw[3],peer->mac_addr.raw[4],peer->mac_addr.raw[5] );

                             if(ac_id == ATF_TRAFFIC_CTRL_AC_ALL) {
                                 block_bitset = count_bits_set(ni->ni_block_bitmap);
                                 ni->ni_block_bitmap |= ATF_TRAFFIC_CTRL_AC_ALL;
                                 vap->ac_blk_cnt += (WME_NUM_AC - block_bitset);
                             } else {
                                 ni->ni_block_bitmap |= (1 << (ac_id-1));
                                 vap->ac_blk_cnt++;
                             }
...
#if ATH_SUPPORT_WRAP && DBDC_REPEATER_SUPPORT
static
OS_TIMER_FUNC(ol_psta_creation_timeout)
{
    struct global_ic_list *glist;
    OS_GET_TIMER_ARG(glist, struct global_ic_list *);
    ieee80211_validate_psta_creation(glist);
    OS_SET_TIMER(&(ic_list.psta_creation_timer), IEEE80211_PSTA_CHECK_TIME);
}
#endif
...
#define WMI_MAX_DUMP_LEN 200
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Dumping %pK - %d words \n", ptr, len);
...
int ol_ath_set_tx_capture (struct ol_ath_softc_net80211 *scn, int val) {
...
#if OL_ATH_SMART_LOGGING
    /* TODO: We use scn->pl_dev->tgt_pktlog_enabled inline with existing design
     * for this call. However, if access semantics for
    ...
    if ((val == 1) && (!tx_capture)) {
        ...
        if (scn->pl_dev->tgt_pktlog_enabled) {
             ...
             * remains in effect.
             */
            if (ic->ic_smart_log_fw_pktlog_stop_and_block &&
                    (ic->ic_smart_log_fw_pktlog_stop_and_block(ic,
                        ATH_PKTLOG_TX_CAPTURE_ENABLE, true) !=
                     QDF_STATUS_SUCCESS)) {
                return -1;
            }
        } else {
             ...
             if (ic->ic_smart_log_fw_pktlog_stop_and_block &&
                    (ic->ic_smart_log_fw_pktlog_stop_and_block(ic,
                        ATH_PKTLOG_TX_CAPTURE_ENABLE, false) !=
                     QDF_STATUS_SUCCESS)) {
                return -1;
            }
        }
...
#endif
#if OL_ATH_SMART_LOGGING
            if (ic->ic_smart_log_fw_pktlog_unblock) {
                ic->ic_smart_log_fw_pktlog_unblock(ic);
            }
#endif /* OL_ATH_SMART_LOGGING */
            return 0;
        }
    ...
    if (cfg == 1) {
        if (ic->smart_logging == 1) {
            ...
            if (smart_log_init(scn) == 0) {
                ...
                ic->ic_smart_log_fw_pktlog_enable(ic);
#else /* REMOVE_PKT_LOG */
                qdf_print("Not enabling smart log FW initiated packetlog "
                          "feature since packetlog is not available at host\n");
#endif /* REMOVE_PKT_LOG */
        ...
        if (ic->smart_logging == 0) {
            ...
            if (ic->smart_logging_p1pingfail_started) {
               qdf_print("\nP1 ping failure logging in progress. Stopping it."
                         "\n");
               if ((send_fatal_cmd(scn,
                       WMI_HOST_FATAL_CONDITION_CONNECTION_ISSUE,
                       WMI_HOST_FATAL_SUBTYPE_P1_PING_FAILURE_STOP_DEBUG)) !=
                       QDF_STATUS_SUCCESS) {
                    qdf_print("Error: Could not successfully send command to "
                              "stop P1 ping failure logging. Smart logging or "
                              "rest of system may no longer function as "
                              "expected. Investigate!!.\n"
                              "Marking P1 ping failure as stopped within host. "
                              "This may not be consistent with FW state due "
                              "to command send failure\n");
               }
               ic->smart_logging_p1pingfail_started = false;
            }

            /* We do not use the wrapper API exported to other modules to stop
             * logging because of the need for customized error prints and also
             * to avoid any inapplicable book-keeping that might be added to the
             * API.
             */
            if (ic->smart_logging_connection_fail_started) {
               qdf_print("\nConnection failure logging in progress. Stopping "
                         "it.\n");
               if ((send_fatal_cmd(scn,
                       WMI_HOST_FATAL_CONDITION_CONNECTION_ISSUE,
                       WMI_HOST_FATAL_SUBTYPE_CONNECTION_FAILURE_STOP_DEBUG)) !=
                       QDF_STATUS_SUCCESS) {
                    qdf_print("Error: Could not successfully send command to "
                              "stop connection failure logging. Smart logging "
                              "or rest of system may no longer function as "
                              "expected. Investigate!!.\n"
                              "Marking connection failure logging as stopped "
                              "within host. This may not be consistent with FW "
                              "state due to command send failure\n");
               }
               ic->smart_logging_connection_fail_started = false;
            }

            qdf_assert_always(ic->ic_smart_log_fw_pktlog_disable != NULL);
            ic->ic_smart_log_fw_pktlog_disable(ic);
 ...
 */
int32_t send_fatal_cmd(struct ol_ath_softc_net80211 *scn, uint32_t type,
        uint32_t subtype)
{
...
#if QCN_ESP_IE
int
ol_ath_esp_estimate_event_handler (ol_scn_t scn, u_int8_t *data, u_int16_t datalen)
{
    wmi_host_esp_estimation_event event, *ev = &event;
    struct ieee80211com *ic = &scn->sc_ic;

    if(wmi_extract_esp_estimate_ev_param(scn->wmi_handle, data, ev)) {
        return -1;
    }
