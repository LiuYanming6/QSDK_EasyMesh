...
int get_radiotap_total_len(u_int8_t ht, u_int8_t vht)
{
    ...
    len = ALIGN(len, 2);
    len += sizeof(struct ieee80211_radiotap_vendor_ns_ath);
    return len;
}
...
static u_int32_t get_ht_mcs_from_ratecode(u_int8_t ht_ratecode)
{
    ...
    if (ht_mcs == 0xff) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: no mcs found for ht ratecode=0x%x\n",__FUNCTION__, ht_ratecode);
        return -EINVAL;
    }
...
static u_int32_t get_pream_from_ratecode(u_int8_t ratecode)
{
    ...
    if (pream == IEEE80211_T_MAX) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: preamble not found for ratecode=0x%x\n",__FUNCTION__, ratecode);
        return -EINVAL;
    }
    return pream;
}

static int add_radiotap_header_da(os_if_t osif, struct sk_buff *skb, ieee80211_recv_status *rs)
{
    ...
    case IEEE80211_T_HT:
        ...
        break;
    default:
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: pream=%d not valid\n",__FUNCTION__, pream);
        qdf_nbuf_free(skb);
        return -EINVAL;
    }
    /* populate rh */
...
*/
int osif_mon_add_radiotap_header(os_if_t osif, struct sk_buff *skb, ieee80211_recv_status *rs)
{
    osif_dev *osifp = (osif_dev *)osif;
    struct ieee80211vap *vap = osifp->os_if;
    struct ieee80211com *ic = wlan_vap_get_devhandle(vap);
    ...
    struct ieee80211_radiotap_header *rthdr = NULL;
    struct sk_buff *tskb = NULL;
    struct ieee80211_radiotap_vendor_ns_ath *radiotap_vendor_ns_ath;
    u_int32_t *it_present_p = NULL;
    u_int32_t it_present_mask=0;
    u_int8_t *p = NULL;
    ...
    if (!osifp->osif_is_mode_offload) {
        ...
        return add_radiotap_header_da(osif, skb, rs);
    }
    ...
    *p = rs->rs_rssi;
    ...
    p += 4;
    // We are concentrating on 3rd and 4th bits in AMPDU Status flags
    ...
    radiotap_vendor_ns_ath = (struct ieee80211_radiotap_vendor_ns_ath *)p;
    /*
    ...
    put_unaligned_le32(scn->device_id, &radiotap_vendor_ns_ath->device_id);

    return EOK;
}


int osif_mon_add_prism_header(os_if_t osif, struct sk_buff *skb, ieee80211_recv_status *rs)
{
    osif_dev *osifp = (osif_dev *)osif;
    struct ieee80211vap *vap = osifp->os_if;
    ...
    if (skb_headroom(skb) < sizeof(wlan_ng_prism2_header)) {
        tskb = qdf_nbuf_realloc_headroom(skb, sizeof(wlan_ng_prism2_header));
        if(tskb!=NULL){
            skb = tskb;
        } else {
