...
int ol_ath_mgmt_null_handler(ol_scn_t scn, wbuf_t wbuf, ol_ath_ieee80211_rx_status_t *rx_status);
/*
...
#endif
    ol_ath_mgmt_null_handler(scn, wbuf, &rx_status);
    return 0;
...
int ol_ath_mgmt_null_handler(ol_scn_t scn, wbuf_t wbuf, ol_ath_ieee80211_rx_status_t *rx_status)
{
    ...
    if (ni == NULL || (IEEE80211_IS_PROBEREQ(wh) && IEEE80211_IS_BROADCAST(wh->i_addr3))) {
        ...
        if (IEEE80211_IS_BEACON(wh)) {
            /*Call ieee80211_input_all is not efficent.
              For beacon, no need to process for each AP VAP.
              Only pass& process beacon once on one AP VAP.
             */
            last_vap = TAILQ_LAST(&ic->ic_vaps, ic_vaps_head);
            TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {
                if (vap != last_vap) {
                    if (vap->iv_opmode == IEEE80211_M_HOSTAP) {
                        if (bcn_processed_apvap != 1) {
                            wbuf1 = wbuf_clone(vap->iv_ic->ic_osdev, wbuf);
                            if (wbuf1 == NULL) {
                                return -1;
                            }
                        }
                ...
                if (vap->iv_opmode == IEEE80211_M_HOSTAP) {
                    if (bcn_processed_apvap != 1){
                        ni = vap->iv_bss;
                        ieee80211_input(ni, wbuf1, &rs);
                        bcn_processed_apvap = 1;
                    }
                } else {
                    ni = vap->iv_bss;
                    ieee80211_input(ni, wbuf1, &rs);
...
#endif  /*end of BCN_PROCESS_OPTIMIZE*/
...
struct ol_ath_iter_update_fd_arg {
    struct ieee80211com *ic;
    int if_id;
};

/*
 * Send FILS Discovery Frames
 */
void
ol_ath_fils_discovery_send(struct ol_ath_softc_net80211 *scn,
        int vid,
        wbuf_t wbuf)
{
    ...
    if(!avn->av_restart_in_progress) {
        wmi_unified_fils_discovery_send_cmd(scn->wmi_handle, &param);
    }
    return;
}

/*
 * Handler for Host SWFDA events in host mode
 */
static int
ol_ath_fils_discovery_swfda_handler(struct ol_ath_softc_net80211 *scn,
                           uint8_t *data)
{
    struct ieee80211com *ic = &scn->sc_ic;
    ...
    if(wmi_extract_swfda_vdev_id(scn->wmi_handle, data, &vdev_id)) {
        qdf_print("Unable to extact vdev id from swfda event\n");
        return -1;
    }

    /* Get the VAP corresponding to the id */
    TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {
        if ((OL_ATH_VAP_NET80211(vap))->av_if_id == vdev_id) {
            break;        }
    }
    ...
    if (avn->av_fd_wbuf == NULL) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "FD Buffer is NULL. Ignore SWFDA Event \n");
    ...
    if (!ieee80211_vap_active_is_set(vap)) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "vap %d: drop SWFDA event, since vap is not active  \n", vdev_id);
        qdf_spin_unlock(&avn->avn_lock);
        return -1;
    }
    if (!ieee80211_fils_discovery_update(vap, avn->av_fd_wbuf)) {
        if (avn->is_fd_dma_mapped) {
            struct ol_txrx_vdev_t *vdev = vap->iv_txrx_handle;
            struct ol_txrx_pdev_t *pdev = vdev->pdev;

            qdf_nbuf_unmap_single(pdev->osdev, avn->av_fd_wbuf,
                                  QDF_DMA_TO_DEVICE);
            avn->is_fd_dma_mapped = 0;
        }
        ol_ath_fils_discovery_send(scn, vdev_id, avn->av_fd_wbuf);
        avn->is_fd_dma_mapped = 1;
    }
    qdf_spin_unlock(&avn->avn_lock);
...
/* WMI FD related Event APIs */
static int
ol_fils_discovery_swfda_handler(ol_scn_t scn, u_int8_t *data, u_int16_t datalen)
{
    return ol_ath_fils_discovery_swfda_handler(scn, data);
}

/*
 * Function to move FILS discovery buffer to deferred_fd_list
 * and wait for stopped event from Target.
 * FD buffer in deferred_fd_list gets freed on receiving
 * vap stopped event from target
 */
static void
ol_ath_vap_defer_fd_buf_free(struct ol_ath_vap_net80211 *avn)
{
    struct fd_buf_entry* buf_entry;

    buf_entry = (struct fd_buf_entry *)qdf_mem_malloc(
                                sizeof(struct fd_buf_entry));
    ...
    if (buf_entry) {
        ...
        buf_entry->is_dma_mapped = avn->is_fd_dma_mapped;
        avn->is_fd_dma_mapped = 0;
        buf_entry->fd_buf = avn->av_fd_wbuf;
        TAILQ_INSERT_TAIL(&avn->deferred_fd_list, buf_entry, deferred_fd_list_elem);
        avn->av_fd_wbuf =  NULL;
        qdf_spin_unlock(&avn->avn_lock);
    }
    else {
...
/*
 * Function to allocate FILS Discovery Frames
 */
static void
ol_ath_vap_iter_fils_discovery_alloc(void *arg, wlan_if_t vap)
{
    struct ol_ath_iter_update_fd_arg* params = (struct ol_ath_iter_update_fd_arg *)arg;
    struct ol_ath_vap_net80211 *avn = OL_ATH_VAP_NET80211(vap);
    struct ieee80211_node *ni;
    ...
    if (avn->av_if_id == params->if_id) {
        ...
        ol_ath_vap_defer_fd_buf_free(avn);
        avn->av_fd_wbuf = ieee80211_fils_discovery_alloc(ni);
        if (avn->av_fd_wbuf == NULL) {
           QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ERROR ieee80211_fils_discovery_alloc failed in %s:%d\n", __func__, __LINE__);
           A_ASSERT(0);
        }
    }
...
/*
 * Function to allocate buffer for FILS Discovery Frames
 */
void
ol_ath_fils_discovery_alloc(struct ieee80211com *ic, int if_id)
{
    struct ol_ath_iter_update_fd_arg params;

    params.ic = ic;
    params.if_id = if_id;
    wlan_iterate_vap_list(ic,ol_ath_vap_iter_fils_discovery_alloc,(void *) &params);
}

/* Move the FILS discovery buffer to deferred_fd_list */
void
ol_ath_fd_stop(struct ol_ath_softc_net80211 *scn,
                   struct ol_ath_vap_net80211 *avn)
{
    ol_ath_vap_defer_fd_buf_free(avn);
    return;
...
/*
 * Function to free fils discovery in host mode
 */
static void
ol_ath_vap_iter_fd_free(void *arg, wlan_if_t vap)
{
    struct ol_ath_iter_update_fd_arg* params = (struct ol_ath_iter_update_fd_arg *)arg;
    struct ol_ath_vap_net80211 *avn = OL_ATH_VAP_NET80211(vap);
    ...
    if (avn->av_if_id == params->if_id) {
       struct fd_buf_entry* buf_entry,*buf_temp;
       struct ieee80211_tx_status ts;
       ts.ts_flags = 0;
       ts.ts_retries = 0;

       qdf_spin_lock(&avn->avn_lock);
       TAILQ_FOREACH_SAFE(buf_entry, &avn->deferred_fd_list, deferred_fd_list_elem,buf_temp) {
           TAILQ_REMOVE(&avn->deferred_fd_list, buf_entry, deferred_fd_list_elem);
           if (buf_entry->is_dma_mapped == 1) {
               qdf_nbuf_unmap_single(scn->qdf_dev, buf_entry->fd_buf,
                                     QDF_DMA_TO_DEVICE);
               buf_entry->is_dma_mapped = 0;
           }
           ieee80211_complete_wbuf(buf_entry->fd_buf, &ts);
           qdf_mem_free(buf_entry);
       }
       qdf_spin_unlock(&avn->avn_lock);
    }
...
/*
 * Free FILS FD buffers
 */
void
ol_ath_fd_free(struct ieee80211com *ic, int if_id)
{
    struct ol_ath_iter_update_fd_arg params;

    params.ic = ic;
    params.if_id = if_id;
    wlan_iterate_vap_list(ic,ol_ath_vap_iter_fd_free,(void *) &params);
}
#endif
...
#if UMAC_SUPPORT_FILS
    wmi_unified_register_event_handler(scn->wmi_handle, wmi_host_swfda_event_id,
                                            ol_fils_discovery_swfda_handler, WMI_RX_UMAC_CTX);
#endif
