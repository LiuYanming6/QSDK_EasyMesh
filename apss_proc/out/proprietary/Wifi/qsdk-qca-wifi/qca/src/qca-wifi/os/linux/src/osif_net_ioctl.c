...
__qdf_net_wifi_getath_stats(struct net_device *netdev, struct ifreq *ifr)
{
	...
	if (user->size <= 0 || !user->address) {
		QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : Invalid len or addr\n",__FUNCTION__);
...
__qdf_net_vap_getscan_results(struct net_device *netdev, struct ifreq * ifr)
{
	...
	if (iwr->u.data.length < 0 ){
		QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : Invalid Len\n",__FUNCTION__);
		error = QDF_STATUS_E_INVAL;
		goto done;
	}
...
__qdf_net_eth_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd)
{
	switch (cmd) {
		default:
			QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:Unhandled IOCTL = 0x%x\n", __func__, cmd);
		 break;
	}
	return -ENOTSUPP;
}
