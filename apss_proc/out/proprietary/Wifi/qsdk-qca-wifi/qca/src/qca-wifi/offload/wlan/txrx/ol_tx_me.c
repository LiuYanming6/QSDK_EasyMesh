...
ol_tx_me_init(struct ol_txrx_pdev_t *pdev)
{
    ...
    if (pdev->me_buf.vaddr == NULL) {
        ...
        if (pdev->me_buf.vaddr == NULL) {
            ...
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Error allocating memory pool\n");
            return 1;
        }
        ...
        p->next = NULL;
        qdf_spin_unlock_bh(&pdev->tx_mutex);
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ME Pool succesfully initialized vaddr - %pK paddr - %x\n"
...
#endif
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:Already Enabled!!\n", __func__);
    }
    return 0;
}
...
ol_tx_me_exit(struct ol_txrx_pdev_t *pdev)
{
    ...
    if (pdev->me_buf.vaddr) {
        uint16_t wait_cnt = 0;
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Disabling Mcastenhance. This may take some time...\n");
        qdf_spin_lock_bh(&pdev->tx_mutex);
        ...
        if (pdev->me_buf.buf_in_use > 0) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Tx-comp pending for %d ME frames after waiting %ds!!\n",
        ...
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO"%s: Already Disabled !!!\n", __func__);
    }
}
