...
static void ieee80211_bsteering_send_rrm_frame_report_event(
    struct ieee80211vap *vap, const struct bs_rrm_frame_report_ind *report, size_t len);
...
{
    bool bsteer = ieee80211_bsteering_is_valid(ic) && atomic_read(&ic->ic_bsteering->bs_enabled);

    /* band steering can be disabled when complete band is teared down which is
     * possible in MAP . Add check to see if VAP is up on any band and return
     * true for events to be received */
    if (!bsteer && ic) {
        struct ieee80211vap *vap;
        if (!TAILQ_EMPTY(&ic->ic_vaps)) {
            TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {
                if (ieee80211_vap_mapvapup_is_set(vap) ||
                    (vap->iv_map && ieee80211_vap_ready_is_set(vap))) {
                    bsteer = TRUE;
                }
            }
        }
 ...
 *         false
 */
bool ieee80211_bsteering_is_vap_enabled(struct ieee80211vap *vap)
{
    bool bsteer = ((ieee80211_bsteering_is_vap_valid(vap) &&
            ieee80211_bsteering_is_event_enabled(vap)) ||
            ieee80211_vap_mapvapup_is_set(vap));

    if (!bsteer && vap->iv_map && ieee80211_vap_ready_is_set(vap) ) {
        bsteer = TRUE;
    }
...
/**
 * @brief Sends bsteering client disconnect event
 *
 * @param [in] vap  the VAP on which the ioctl was made
 * @param [in] mac_addr  MAC address of the client disconnecting
 * @param [in] src   from where disconnect was initiated
 * @param [in] type  type of disconnection
 * @param [in] reason  reason_code
 *
 * @return void
 */
...
#undef MIN_SAMPLE_PERIOD_DIRECT_ATTACH
    ...
    /*
     * in case the vap is enabled already, we should not treat it as an error
     * this is a common case when rung more then one hyd instances
     */
    if(retval == -EBUSY)
        retval = EOK;
                                     ...
                                     const struct ieee80211req_athdbg *req)
{
    ...
    if (req->data.bsteering_probe_resp_wh) {
        ret = wlan_acl_reset_probe_rssi_thresholds(vap, req->dstmac);
        if (ret < 0) {
            return ret;
        }
...
/**
 * @brief Retrieved Assoc frame for Map implementation
 *
 * @param [in] vap  iface on which client is connected
 *
 * @return EOK on success; -EINVAL if client is not connected.
 *         +ve number in case memory is not enough to copy.
 *
 */
int wlan_bsteering_get_assoc_frame(struct ieee80211vap *vap, const u_int8_t *macaddr, char *info,
                                   u_int16_t *length) {
...
#define MAP_SUCCESS 0x00
    ...
    if (ni->assoc_frame) {
        if (wbuf_get_pktlen(ni->assoc_frame) <= *length) {
            memcpy(info, wbuf_header(ni->assoc_frame), wbuf_get_pktlen(ni->assoc_frame));
            *length = wbuf_get_pktlen(ni->assoc_frame);
            ieee80211_free_node(ni);
            return MAP_SUCCESS;
        }
    }
 ...
 *                       request
 * @param [in] probe_rssi  RSSI value for the incoming probe request
 *
                                          ...
                                          u_int8_t probe_rssi)
{
    ...
    if (ieee80211_acl_flag_check(vap, mac_addr, IEEE80211_ACL_FLAG_PROBE_RESP_WH)) {
        if (wlan_acl_is_probe_resp_wh(vap, mac_addr, probe_rssi)) {
            return true;
        }
    }
 ...
 *        was sent, blocked or rejected with a failure code.
 *
 ...
 * @param [in] reason  reason code for auth rejection
 *                     (0 if blocked with no reply)
                                                    ...
                                                    const struct ieee80211_node *ni)
{
    ...
    wlan_acl_apply_node_rssi_thresholds(vap, ni->ni_macaddr);
 ...
 */
void ieee80211_bsteering_send_rrm_frame_bcnrpt_event(
    struct ieee80211vap *vap, u_int32_t token, const u_int8_t *macaddr,
    const u_int8_t *bcnrpt, size_t report_len, u_int8_t num_rpt_elem) {
    ...
    event = (struct bs_rrm_frame_report_ind*)OS_MALLOC(vap->iv_ic->ic_osdev, event_len, 0);

    if(!event) {
        printk("%s: Failed to create event buffer\n", __func__);
        return;
    }
    ...
    IEEE80211_ADDR_COPY(event->macaddr, macaddr);
    OS_MEMCPY(&event->meas_rpt_data, bcnrpt, report_len);

    ieee80211_bsteering_send_rrm_frame_report_event(vap, event, event_len);
    OS_FREE(event);
}
...
static void ieee80211_bsteering_send_rrm_frame_report_event(
    ...
    IEEE80211_DELIVER_BSTEERING_EVENT(vap, ATH_EVENT_BSTEERING_RRM_FRAME_REPORT,
 ...
 */
void ieee80211_bsteering_send_wnm_bstm_query_event(struct ieee80211vap *vap, u_int32_t token,
                                                  u_int8_t *macaddr,
    ...
    event.dialog_token = token;
    IEEE80211_ADDR_COPY(event.macaddr, macaddr);
    OS_MEMCPY(&event.data, bstm_query, sizeof(struct bs_wnm_bstm_query));
 ...
 * @param [in] rssi  the measured RSSI
 */
static void ieee80211_bsteering_send_rssi_xing_map_event(struct ieee80211vap *vap,
                                                         const u_int8_t *mac_addr, u_int8_t rssi,
...
void ieee80211_bsteering_record_rssi(struct ieee80211_node *ni, u_int8_t rssi)
{
    ...
    do {
        ...
        low_rssi_threshold =
            bsteering->bs_config_params.low_rssi_crossing_threshold;
        ...
        wlan_acl_override_rssi_thresholds(ni,
                                            &inact_rssi_low_threshold,
                                            &inact_rssi_high_threshold,
                                            &low_rssi_threshold,
                                            &low_rate_rssi_threshold,
    ...
    /* Check if MAP is set */
    if (ieee80211_vap_map_is_set(ni->ni_vap)) {
        if (bsteering->map_config_params.rssi_threshold) {
            low_rate_rssi_threshold_map = bsteering->map_config_params.rssi_threshold -
                bsteering->map_config_params.rssi_hysteresis;
            high_rate_rssi_threshold_map = bsteering->map_config_params.rssi_threshold +
                bsteering->map_config_params.rssi_hysteresis;
            ...
            if (ni->ni_bs_rssi_prev_xing_map != map_xing) {
                ni->ni_bs_rssi_prev_xing_map = map_xing;
                ieee80211_bsteering_send_rssi_xing_map_event(ni->ni_vap, ni->ni_macaddr, rssi,
                                                             map_xing);
            }
        }
    }
...
bool wlan_bsteering_get_apcap(struct ieee80211vap *vap, mapapcap_t *apcap) {
    struct ieee80211_ath_channel * channelmin = NULL, *channelmax = NULL;
    u_int8_t minch, maxch;
    int8_t maxpow, minpow, pow;

    struct ieee80211com *ic = NULL;

    u_int32_t rx_streams = ieee80211_get_rxstreams(vap->iv_ic, vap);
    u_int32_t tx_streams = ieee80211_get_txstreams(vap->iv_ic, vap);

    ic = vap->iv_ic;

    channelmax = &(vap->iv_ic->ic_channels[vap->iv_ic->ic_nchans - 1]);
    channelmin = &(vap->iv_ic->ic_channels[0]);

    minch = ieee80211_chan2ieee(vap->iv_ic, channelmin);
    maxch = ieee80211_chan2ieee(vap->iv_ic, channelmax);
    maxpow = channelmax->ic_maxregpower;
    minpow = channelmin->ic_maxregpower;
    pow = maxpow > minpow ? maxpow : minpow;

    apcap->hwcap.num_supported_op_classes =
        regdmn_get_map_opclass(vap->iv_ic, minch, maxch, apcap, pow, NULL);
    // keeping it to max now , should be changed later
    apcap->hwcap.max_supported_bss = CFG_MAX_VAPS - 1;
    ...
    if (ieee80211_vap_wme_is_set(vap) &&
        ...
        if (vap->iv_des_mode >= 9 && vap->iv_des_mode <= 14) {
...
/**
 * @brief Check the RSSI of a given station and report if it crosses a threshold.
 *
 * @param [inout] vap  the VAP whose rssi changes
 * @param [in] req  request from user space containing the rssi threshold and rssi hysteresis
 *
 * @return EOK if success
 */
int wlan_bsteering_set_map_rssi_policy(struct ieee80211vap *vap,
                                       const struct ieee80211req_athdbg *req) {
    ieee80211_bsteering_t bsteering = NULL;
    ...
    bsteering = vap->iv_ic->ic_bsteering;
    spin_lock_bh(&bsteering->bs_lock);
    bsteering->map_config_params.rssi_threshold = req->data.map_rssi_policy.rssi;
    bsteering->map_config_params.rssi_hysteresis = req->data.map_rssi_policy.rssi_hysteresis;
    spin_unlock_bh(&bsteering->bs_lock);
    return EOK;
}

bool wlan_bsteering_get_operable_channels(struct ieee80211vap *vap, map_op_chan_t *map_op_chan) {
    struct ieee80211_ath_channel *channelmin = NULL, *channelmax = NULL;
    u_int8_t minch, maxch;
    ...
    minch = ieee80211_chan2ieee(vap->iv_ic, channelmin);
    maxch = ieee80211_chan2ieee(vap->iv_ic, channelmax);
    return regdmn_get_map_opclass(vap->iv_ic, minch, maxch, NULL, 0, map_op_chan);
}
#endif
