/*
 * Copyright (c) 2011, 2015-2016, 2017 Qualcomm Innovation Center, Inc.
 * All Rights Reserved.
...
#ifdef OL_TXRX_AST_DEBUG
    qdf_print("%s %pK: peer mac %02x:%02x:%02x:%02x:%02x:%02x\n", __func__, pdev, mac_addr->raw[0],mac_addr->raw[1],mac_addr->raw[2],mac_addr->raw[3],mac_addr->raw[4],mac_addr->raw[5]);
#endif
    ...
    u_int16_t peer_id)
{
    ...
    if ((peer->vdev) && (peer->vdev->pdev != pdev)) {
        qdf_print("%s: Peer lookup returned invalid peer %pK peer_id %d, pdev %pK\n", __func__, peer, peer_id, pdev);
        return;
    }
