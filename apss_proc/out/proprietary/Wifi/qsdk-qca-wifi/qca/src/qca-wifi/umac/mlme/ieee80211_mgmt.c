...
ieee80211_send_deauth(struct ieee80211_node *ni, u_int16_t reason)
{
    ...
    wlan_vap_complete_buf_handler handler = NULL;
#endif
...
#if ATH_SUPPORT_DEFERRED_NODE_CLEANUP
    if ((ni != ni->ni_bss_node) && (ieee80211_is_pmf_enabled(vap, ni))) {
...
/* Callback handler for disassoc and deauth completion.
   If vap is PMF enabled, the status of the sent disassoc/deauth
   is monitored. If deauth/disassoc ends with XRETRY, deauth/disassoc
   is sent again after which node leave is triggered. This is to
   ensure that these management frames are received by powersaving client
   */


#if ATH_SUPPORT_DEFERRED_NODE_CLEANUP
#define DEAUTH_TX_XRETRY_THRESHOLD 2
#define DISASSOC_TX_XRETRY_THRESHOLD 2
void ieee80211_mlme_frame_complete_handler(wlan_if_t vap, wbuf_t wbuf,
        void *arg,u_int8_t *dst_addr, u_int8_t *src_addr, u_int8_t *bssid,
        ieee80211_xmit_status *ts)
{
    struct ieee80211_node *ni = (struct ieee80211_node *)arg;
    struct ieee80211_frame *wh = (struct ieee80211_frame *)wbuf_header(wbuf);
    ...
    if(ni && (ni->ni_flags & IEEE80211_NODE_DELAYED_CLEANUP)) {
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_NODE, "%s: frame send to %s\n",
        ...
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,"%s:ts_flags=%d,ni_mac=%s\n",
                __func__,ts->ts_flags, ether_sprintf(ni->ni_macaddr));
        if ((vap->iv_opmode == IEEE80211_M_HOSTAP) &&
                !(ni->ni_flags & IEEE80211_NODE_LEAVE_ONGOING)){
            ...
            if(subtype == IEEE80211_FC0_SUBTYPE_DISASSOC) {
                if (ieee80211_is_pmf_enabled(vap, ni)) {
                    if(ts->ts_flags == IEEE80211_TX_XRETRY) {
                        if(ni->ni_disassoc_tx_xretry < DISASSOC_TX_XRETRY_THRESHOLD) {
                            ni->ni_disassoc_tx_xretry++;
                            IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,"%s Retrying "
                                    "disassoc for ni_mac=%s,num_retry=%d\n",
                                    __func__,ether_sprintf(ni->ni_macaddr),
                                    ni->ni_disassoc_tx_xretry);
                            ieee80211_send_disassoc(ni, IEEE80211_REASON_ASSOC_LEAVE);
                            return;
                        } else {
                            ieee80211_vap_pmf_mgmt_xtry_set(vap);
                        }
                    }
                }
...
#endif
                IEEE80211_NODE_LEAVE(ni);
                IEEE80211_DELIVER_EVENT_MLME_DISASSOC_COMPLETE(vap, dst_addr,
                        ni->ni_reason_code, IEEE80211_STATUS_SUCCESS);
            }

            if(subtype == IEEE80211_FC0_SUBTYPE_DEAUTH) {
                if (ieee80211_is_pmf_enabled(vap, ni)) {
                    if(ts->ts_flags == IEEE80211_TX_XRETRY) {
                        if((ni->ni_deauth_tx_xretry < DEAUTH_TX_XRETRY_THRESHOLD)) {
                            ni->ni_deauth_tx_xretry++;
                            IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,"%s:Retrying "
                                    "deauth for ni_mac=%s,num_retry=%d\n",
                                    __func__,ether_sprintf(ni->ni_macaddr),
                                    ni->ni_deauth_tx_xretry);
...
ieee80211_send_disassoc(struct ieee80211_node *ni, u_int16_t reason)
{
    ...
    if ( (ni != ni->ni_bss_node) && (ieee80211_is_pmf_enabled(vap, ni))) {
        ...
        handler = ieee80211_mlme_frame_complete_handler;
        ni->ni_reason_code = reason;
        retval = ieee80211_send_disassoc_with_callback(ni, reason,
                handler, (void *)ni);
    } else {
...
#define BEACON_INFO_ELEMENT_OFFSET    (offsetof(struct ieee80211_beacon_frame, info_elements))
    ...
    while (remaining_ie_length >= sizeof(struct ieee80211_ie_header)) {
        ...
        case IEEE80211_ELEMID_VENDOR:
            ...
            else if (ismbooui((u_int8_t *) info_element)) {
                mboie = (u_int8_t *) info_element;
            }
#if DBDC_REPEATER_SUPPORT
            else if (isextender_oui((u_int8_t *)info_element)) {
                /* location where Extender IE Present */
		if (ic_list->same_ssid_support) {
		    extender_info = (u_int8_t *)(((u_int8_t *)info_element) + 6);
		    scan_entry_parameters->ie_list.extender_ie = extender_info;
			...
			if (mlme_priv->im_connection_up) {
				...
				if ((ic_list->extender_info & ROOTAP_ACCESS_MASK) != ((*extender_info) & ROOTAP_ACCESS_MASK)) {
    ...
    scan_entry_parameters->ie_list.mboie = mboie;
#if QCA_LTEU_SUPPORT
    if (mboie) {
        scan_entry_parameters->oce_cap = ieee80211_oce_capable(mboie);
    }
#endif
...
#define MAX_BCN_RSSI_AGE 0xff
    ...
    age = jiffies - ni->ni_mgmt_stats.ni_last_bcn_jiffies;
    age = jiffies_to_msecs(age) / MSEC_PER_SEC;
    if (age > MAX_BCN_RSSI_AGE)
        age = MAX_BCN_RSSI_AGE;

    ni->ni_mgmt_stats.ni_last_bcn_rssi = (ni->ni_mgmt_stats.ni_last_bcn_rssi << 8) | rssi;
    ni->ni_mgmt_stats.ni_last_bcn_age = (ni->ni_mgmt_stats.ni_last_bcn_age << 8) | age;
    ni->ni_mgmt_stats.ni_last_bcn_jiffies = jiffies;
    ni->ni_mgmt_stats.ni_last_bcn_cnt = ni->ni_mgmt_stats.ni_last_bcn_cnt + 1;
}
...
#define MGMT_OFFCHANNEL_TX 1
#if MGMT_OFFCHANNEL_TX
#define OFFCHAN_MGMT_TX_POWER       (63)
#define OFFCHAN_MGMT_TX_CHAIN       (-1)
#define OFFCHAN_MGMT_TX_RETRY       (7)
static
void wlan_offchan_mgmt_tx_drain(wlan_if_t vap_handle)
{
    wbuf_t wbuf = NULL;
    if (vap_handle == NULL) {
        qdf_print("%s: vap_handle is NULL\n",__FUNCTION__);
        return;
    }

    if (qdf_atomic_read(&vap_handle->iv_mgmt_offchan_cmpl_pending)) {
        qdf_spin_lock_bh(&vap_handle->iv_mgmt_offchan_tx_lock);
        while(!qdf_nbuf_is_queue_empty(&vap_handle->iv_mgmt_offchan_txnbufqueue)) {
            wbuf = qdf_nbuf_queue_remove(&(vap_handle->iv_mgmt_offchan_txnbufqueue));
            if (wbuf) {
	        struct ieee80211_tx_status ts;
                    ts.ts_flags = IEEE80211_TX_ERROR;
                    ts.ts_retries=0;
	         ...
	         * complete buf will decrement the pending count.
	         */
                wbuf_set_node(wbuf, ieee80211_ref_node(vap_handle->iv_bss));
                if (qdf_atomic_read(&vap_handle->iv_mgmt_offchan_cmpl_pending))
...
void wlan_offchan_mgmt_tx_send(struct ieee80211vap *vap)
{
    ...
    if (!qdf_nbuf_is_queue_empty(&vap->iv_mgmt_offchan_txnbufqueue)) {
        wbuf = qdf_nbuf_queue_remove(&(vap->iv_mgmt_offchan_txnbufqueue));
        ...
        if (wbuf) {
            ...
            if (IEEE80211_IS_CHAN_2GHZ(vap->iv_ic->ic_curchan) && vap->iv_mgt_rate) {
                if (vap->iv_mgt_rate == 6000)       /* 6 Mbps */
                    wbuf_set_tx_rate(wbuf, 0, RATECODE_PREAM_OFDM, 3);
                else if (vap->iv_mgt_rate == 5500)  /* 5.5 Mbps */
                    wbuf_set_tx_rate(wbuf, 0, RATECODE_PREAM_CCK, 1);
                else if (vap->iv_mgt_rate == 2000)  /* 2 Mbps */
                    wbuf_set_tx_rate(wbuf, 0, RATECODE_PREAM_CCK, 2);
                else                                  /* 1 Mbps */
                    wbuf_set_tx_rate(wbuf, 0, RATECODE_PREAM_CCK, 3);
            }
            /* Increase frame retries for DPP packets */
            wbuf_set_tx_ctrl(wbuf, OFFCHAN_MGMT_TX_RETRY, OFFCHAN_MGMT_TX_POWER, OFFCHAN_MGMT_TX_CHAIN);
...
extern void ieee80211_ioctl_send_action_frame_cb(wlan_if_t vaphandle, wbuf_t wbuf,
        void *arg, u_int8_t *dst_addr,
        u_int8_t *src_addr, u_int8_t *bssid, ieee80211_xmit_status *ts);
static
void wlan_offchan_mgmt_tx_handler(wlan_if_t vaphandle, wbuf_t wbuf,
        void *arg, u_int8_t *dst_addr,
        u_int8_t *src_addr, u_int8_t *bssid, ieee80211_xmit_status *ts)
{
    ...
    if (osifp->is_stop_event_pending) {
        wlan_offchan_mgmt_tx_drain(vaphandle);
        if (wlan_scan_in_progress(vap)) {
            wlan_scan_cancel(vap, osifp->offchan_scan_requestor, IEEE80211_VAP_SCAN, IEEE80211_SCAN_CANCEL_SYNC);
        }
    }

    ieee80211_ioctl_send_action_frame_cb(vaphandle, wbuf, arg, dst_addr, src_addr, bssid, ts);
    
    if (qdf_atomic_read(&vaphandle->iv_mgmt_offchan_cmpl_pending) != 0) {
        wlan_offchan_mgmt_tx_send(vaphandle);
    } else if( ts->ts_flags && qdf_atomic_read(&vaphandle->iv_mgmt_offchan_cmpl_pending) == 0) {
        wlan_scan_cancel(vap, osifp->offchan_scan_requestor, IEEE80211_VAP_SCAN, IEEE80211_SCAN_CANCEL_SYNC);
    }
...
static
int wlan_offchan_mgmt_tx_add(wlan_if_t vap_handle, u_int32_t freq,
                            ...
                            const u_int8_t *src_addr, const u_int8_t *bssid, const u_int8_t *data, u_int32_t data_len)
{
    ...
    qdf_spin_lock_bh(&vap_handle->iv_mgmt_offchan_tx_lock);
    qdf_nbuf_queue_add(&vap_handle->iv_mgmt_offchan_txnbufqueue, wbuf);
    qdf_spin_unlock_bh(&vap_handle->iv_mgmt_offchan_tx_lock);
...
static
void wlan_offchan_mgmt_tx_scan_event_handler(struct ieee80211vap *orig_vap,
...
//    osif_dev *osifp = (osif_dev *)vap->iv_ifp;
    ...
    case IEEE80211_SCAN_FOREIGN_CHANNEL:
        ...
        qdf_print("MGMT_OFF_TX foreign :: %s[%d] \n", __func__, __LINE__);
        break;
    case IEEE80211_SCAN_COMPLETED:
        qdf_print("MGMT_OFF_TX :: %s[%d] \n", __func__, __LINE__);
        rc = wlan_scan_unregister_event_handler(vap, &wlan_offchan_mgmt_tx_scan_event_handler,(void *)arg);
        ieee80211_ic_offchanscan_clear(vap->iv_ic);
        if (rc != EOK) {
            qdf_print("MGMT_OFF_TX :: %s[%d] ERROR \n", __func__, __LINE__);
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_ANY,
                              "%s: acfg_scan_unregister_event_handler() failed handler=%08p,%08p rc=%08X\n",
                              __func__, &wlan_offchan_mgmt_tx_scan_event_handler, vap, rc);
        }

        if (qdf_atomic_read(&vap->iv_mgmt_offchan_cmpl_pending)) {
            qdf_print("MGMT_OFF_TX :: %s[%d] ERROR \n", __func__, __LINE__);
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_ANY,
...
static
int wlan_offchan_mgmt_tx_scan_req(wlan_if_t vaphandle, void *netdev, u_int32_t chan,
                                u_int16_t dwell_time, u_int16_t scan_requestor, u_int32_t *scan_id)
{
    ...
    scan_params->flags |= IEEE80211_SCAN_OFFCHAN_MGMT_TX | IEEE80211_SCAN_OFFCHAN_DATA_TX | IEEE80211_SCAN_PROM_MODE | IEEE80211_SCAN_CONTINUOUS;
    scan_params->type = IEEE80211_SCAN_FOREGROUND;
    scan_params->min_dwell_time_passive = dwell_time;
    scan_params->max_dwell_time_passive = dwell_time + 5;
    ...
    qdf_print("MGMT_OFF_TX :: %s[%d] \n", __func__, __LINE__);
    rc = wlan_scan_register_event_handler(vap, &wlan_offchan_mgmt_tx_scan_event_handler,(void *)netdev);
    if (rc != EOK) {
        qdf_print("MGMT_OFF_TX :: %s[%d] ERROR \n", __func__, __LINE__);
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_ANY,
    ...
    if (wlan_scan_start(vap, scan_params, scan_requestor, scan_priority, scan_id) != 0 ) {
        qdf_print("MGMT_OFF_TX :: %s[%d] ERROR scan fail\n", __func__, __LINE__);
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_SCAN,
            "%s: Issue a scan fail.\n",
            __func__);
        OS_FREE(scan_params);
        wlan_scan_unregister_event_handler(vap, &wlan_offchan_mgmt_tx_scan_event_handler,(void *)netdev);
        return -1;
    }
...
{
    osif_dev *osifp = (osif_dev *)vap_handle->iv_ifp;
    struct ieee80211vap *vap = vap_handle;

    qdf_print("MGMT_OFF_TX :: %s[%d] cancel wait request", __func__, __LINE__);
    if (ieee80211_ic_offchanscan_is_set(vap_handle->iv_ic)) {
             /* Cancel current scan and send tx failures for all queued packets */
            qdf_print("MGMT_OFF_TX :: %s[%d] Cancel current scan \n", __func__, __LINE__);
            wlan_scan_cancel(vap, osifp->offchan_scan_requestor, IEEE80211_VAP_SCAN, IEEE80211_SCAN_CANCEL_SYNC);
    }
}

int wlan_offchan_mgmt_tx_start(wlan_if_t vap_handle, u_int32_t freq, u_int32_t dwell_time,
                            wlan_action_frame_complete_handler handler, void *arg,const u_int8_t *dst_addr,
                            const u_int8_t *src_addr, const u_int8_t *bssid, const u_int8_t *data, u_int32_t data_len)
{
    osif_dev *osifp = (osif_dev *)vap_handle->iv_ifp;
    ...
    handler = wlan_offchan_mgmt_tx_handler;
    qdf_print("MGMT_OFF_TX :: %s[%d] request freq %d current freq %d\n", __func__, __LINE__,
                                       freq, ieee80211_chan2freq(ic,ic->ic_curchan));
    if (freq != 0 && freq !=  ieee80211_chan2freq(ic,ic->ic_curchan) &&
		    ieee80211_ic_offchanscan_is_set(vap_handle->iv_ic)) {
             /* Cancel current scan and send tx failures for all queued packets */
            qdf_print("MGMT_OFF_TX :: %s[%d] Cancel current scan \n", __func__, __LINE__);
            wlan_scan_cancel(vap, osifp->offchan_scan_requestor, IEEE80211_VAP_SCAN, IEEE80211_SCAN_CANCEL_SYNC);
    }
    if (!wlan_offchan_mgmt_tx_add(vap_handle, freq, handler, arg, dst_addr,
                            src_addr, bssid, data, data_len)) {
        qdf_print("MGMT_OFF_TX :: %s[%d] \n", __func__, __LINE__);

	if(!freq)
	    wlan_offchan_mgmt_tx_send(vap_handle);
        else if (freq !=  ieee80211_chan2freq(ic,ic->ic_curchan) &&
		    !ieee80211_ic_offchanscan_is_set(vap_handle->iv_ic)) {
            /* freq different and no offchannel scan in progress*/
	    if (!dwell_time)
                dwell_time = 80;
            qdf_print("MGMT_OFF_TX :: %s[%d] scan request freq %d\n", __func__, __LINE__, freq);
            wlan_offchan_mgmt_tx_scan_req(vap_handle, osifp, ieee80211_mhz2ieee(ic, freq, 0), dwell_time, osifp->offchan_scan_requestor, &(osifp->offchan_scan_id));
        } else if (freq !=  ieee80211_chan2freq(ic,ic->ic_curchan) &&
		    ieee80211_ic_offchanscan_is_set(vap_handle->iv_ic)) {

            /* freq different and offchannel scan in progress
             * Cancel current scan and send tx failures for all queued packets */
            wlan_scan_cancel(vap, osifp->offchan_scan_requestor, IEEE80211_VAP_SCAN, IEEE80211_SCAN_CANCEL_SYNC);
            qdf_print("MGMT_OFF_TX :: %s[%d] offchannel already progress in different channel \n", __func__, __LINE__);
            qdf_print("MGMT_OFF_TX :: %s[%d] Drin the Tx queued packet and cancel the ongoing scan\n", __func__, __LINE__);
            wlan_offchan_mgmt_tx_drain(vap_handle);
        } else if (ieee80211_ic_offchanscan_is_set(vap_handle->iv_ic)) {
            /* currently offchannel scan in progress */
            qdf_print("MGMT_OFF_TX :: %s[%d] offchannel already in progress \n", __func__, __LINE__);
            wlan_offchan_mgmt_tx_send(vap_handle);
        } else {
            /* default case :: this won't execute */
            qdf_print("MGMT_OFF_TX :: freq == 0 %s[%d] \n", __func__, __LINE__);
            wlan_offchan_mgmt_tx_send(vap_handle);
        }
		return 0;
    } else {
        qdf_print("MGMT_OFF_TX :: %s[%d] add frame failed \n", __func__, __LINE__);
		return -1;
    }
}
...
#if DBDC_REPEATER_SUPPORT
void ieee80211_bringdown_secondary_stavap(struct ieee80211com *ic)
{
    int i = 0;
	...
	if(tmp_ic && tmp_ic->ic_sta_vap) {
	    ...
	    if (wlan_is_connected(tmp_stavap) && (tmp_stavap != ic_list->max_priority_stavap_up)) {
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "RootAP access status changed, bring down secondary stavap:%s\n",ether_sprintf(tmp_stavap->iv_myaddr));
