...
ar9300_proc_tx_desc(struct ath_hal *ah, void *txstatus)
{
    ...
    */
    ...
    if (AR_SREV_JET(ah))
        tx_ba_status = ads->status3 & QCN_tx_ba_status;
    else
        tx_ba_status = ads->status2 & AR_tx_ba_status;
    ...
    if (AR_SREV_JET(ah)){
        ts->ts_rssi_ctl3 = MS(ads->status2, QCN_tx_rssi_ant03);
        ts->ts_rssi_ext3 = MS(ads->status3, QCN_tx_rssi_ant13);
    } else {
        ts->ts_rssi_ctl3 = HAL_RSSI_BAD;
        ts->ts_rssi_ext3 = HAL_RSSI_BAD;
    }
...
{
    struct ar9300_txc *ads = AR9300TXC(ds);
    struct qcn5500_txc *jds = QCN5500TXC(ds);

    if (AR_SREV_JET(ah))
        jds->ds_ctl23 |= SM((1 << chain_num), QCN_paprd_chain_mask);
    else
...
#endif
{
    ...
    if (AR_SREV_JET(ah))
        cal_pkt = (jds->ds_ctl23 & QCN_paprd_chain_mask)?1:0;
    else
    ...
    if (AR_SREV_JET(ah)) {
        jds->ds_ctl23 = set_Jet_tx_chainmask(series, 0)
                             |  set_Jet_tx_chainmask(series, 1)
                             |  set_Jet_tx_chainmask(series, 2)
                             |  set_Jet_tx_chainmask(series, 3)
                             |  (cal_pkt ? QCN_paprd_chain_mask : 0);
    } else {
        ads->ds_ctl18 |= set_11n_tx_chainmask(series, 0)
                            | set_11n_tx_chainmask(series, 1)
                            | set_11n_tx_chainmask(series, 2)
                            | set_11n_tx_chainmask(series, 3);
    }

#if ATH_SUPPORT_WIFIPOS
    if (AR_SREV_JET(ah)) {
        if (flags & (HAL_TXDESC_POS_TXCHIN_1 | HAL_TXDESC_POS_TXCHIN_2 |
                    HAL_TXDESC_POS_TXCHIN_3 | HAL_TXDESC_POS_TXCHIN_4) ) {
            /* if tx chainmask is specified in measurement request for locationing
             * copy the the chainmask to TX control descriptor chain selects */
            jds->ds_ctl23 = jds->ds_ctl23 & (~QCN_chain_sel0);
            jds->ds_ctl23 = jds->ds_ctl23 & (~QCN_chain_sel1);
            jds->ds_ctl23 = jds->ds_ctl23 & (~QCN_chain_sel2);
            jds->ds_ctl23 = jds->ds_ctl23 & (~QCN_chain_sel3);
            if (flags & HAL_TXDESC_POS_TXCHIN_1) {
                jds->ds_ctl23 = jds->ds_ctl23 | (1 << QCN_chain_sel0_S);
                jds->ds_ctl23 = jds->ds_ctl23 | (1 << QCN_chain_sel1_S);
                jds->ds_ctl23 = jds->ds_ctl23 | (1 << QCN_chain_sel2_S);
                jds->ds_ctl23 = jds->ds_ctl23 | (1 << QCN_chain_sel3_S);
            }
            if (flags & HAL_TXDESC_POS_TXCHIN_2) {
                jds->ds_ctl23 = jds->ds_ctl23 | (3 << QCN_chain_sel0_S);
                jds->ds_ctl23 = jds->ds_ctl23 | (3 << QCN_chain_sel1_S);
                jds->ds_ctl23 = jds->ds_ctl23 | (3 << QCN_chain_sel2_S);
                jds->ds_ctl23 = jds->ds_ctl23 | (3 << QCN_chain_sel3_S);
            }
            if (flags & HAL_TXDESC_POS_TXCHIN_3) {
                jds->ds_ctl23 = jds->ds_ctl23 | (7 << QCN_chain_sel0_S);
                jds->ds_ctl23 = jds->ds_ctl23 | (7 << QCN_chain_sel1_S);
                jds->ds_ctl23 = jds->ds_ctl23 | (7 << QCN_chain_sel2_S);
                jds->ds_ctl23 = jds->ds_ctl23 | (7 << QCN_chain_sel3_S);
            }
            if (flags & HAL_TXDESC_POS_TXCHIN_4) {
                jds->ds_ctl23 = jds->ds_ctl23 | (15 << QCN_chain_sel0_S);
                jds->ds_ctl23 = jds->ds_ctl23 | (15 << QCN_chain_sel1_S);
                jds->ds_ctl23 = jds->ds_ctl23 | (15 << QCN_chain_sel2_S);
                jds->ds_ctl23 = jds->ds_ctl23 | (15 << QCN_chain_sel3_S);
            }
        }
    } else {
        if (flags & (HAL_TXDESC_POS_TXCHIN_1 | HAL_TXDESC_POS_TXCHIN_2 |
...
ar9300_get_desc_info(struct ath_hal *ah, HAL_DESC_INFO *desc_info)
{
    ...
    if (AR_SREV_JET(ah)) {
        desc_info->txctl_numwords = JET_TXCTL_NUMWORDS(ah);
        desc_info->rxstatus_numwords = JET_RXSTATUS_NUMWORDS(ah);
    } else {
