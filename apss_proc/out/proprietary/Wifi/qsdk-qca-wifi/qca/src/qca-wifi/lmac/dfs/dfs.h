...
/* Cascade has issue with reported duration especially when there is a crossover
   of chirp from one segment to another. It may report a value of duration that is
   well below 50us for a valid FCC type 5 chirping pulse.
   For now changing minimum duration as a work around.
   This will affect all chips but since we detect chirp with Merlin+,
   we may be OK for now.
...
#define ATH_DFSNOL_LOCK(_dfs)           qdf_spin_lock_bh(&dfs->dfs_nol_lock);
#define ATH_DFSNOL_UNLOCK(_dfs)         qdf_spin_unlock_bh(&dfs->dfs_nol_lock);
#define ATH_DFSNOL_LOCK_CREATE(_dfs)    qdf_spinlock_create(&dfs->dfs_nol_lock);
#define ATH_DFSNOL_LOCK_DESTROY(_dfs)   qdf_spinlock_destroy(&dfs->dfs_nol_lock);
...
#define DFS_SECOND_SEGMENT_RADAR_EN		0x20000000	/* Radar detect in second segment is capable */
#endif
...
struct dfs_event {
	...
	u_int32_t  re_freq_hi;    /* Upper bounds of frequency, KHz */
        /* enhancement for false detection log */
...
struct dfs_delayline {
	...
	u_int32_t	dl_numelems;		/* Number of elements in the delay line */
        /* The following is to handle fractional PRI pulses that can cause false detection */
	u_int32_t	dl_seq_num_start;	/* sequence number of first pulse that was part of threshold match */
	u_int32_t	dl_seq_num_stop;	/* sequence number of last pulse that was part of threshold match */
        /* The following is required because the first pulse may or may not be in the delay line but we will find */
        /* it iin the pulse line using dl_seq_num_second's diff_ts value */
	u_int32_t	dl_seq_num_second;	/* isequence number of second pulse that was part of threshold match */
        u_int32_t       dl_search_pri;          /* we need final search PRI to identify possible fractional PRI issue */
        int16_t         dl_min_sidx;            /* minimum sidx value of pulses used to match thershold. used for sidx spread check */
        int8_t          dl_max_sidx;            /* maximum sidx value of pulses used to match thershold. used for sidx spread check */
        u_int8_t        dl_delta_peak_match_count; /* number of pulse in the delay line that had valid delta peak value */
} qdf_packed;
#ifdef WIN32
...
struct dfs_filter {
        ...
        /* To reduce false detection, look at frequency spread. For now we will use sidx spread. But for HT160 */
        /* frequency spread will be a better measure */
        u_int16_t       rf_sidx_spread; /* Maximum SIDX value spread in a matched sequence excluding FCC Bin 5 */
        int8_t          rf_check_delta_peak;/*Minimum allowed delta_peak value for a pulse to be considetred for this filter's match */
} qdf_packed;
#ifdef WIN32
...
struct dfs_state {
	struct ieee80211_ath_channel	rs_chan;		/* Channel info */
...
#define DFS_RCSA_INTVAL         (100)     /* 100 millisecond */
#define DFS_WAIT_FOR_CSA_TIME   (2000)    /* 2sec (roundtrip(1sec) + grace period(1sec)) */
#define DFS_RCSA_INIT_COUNT     (5)       /* Send 5 RCSA */
#define DFS_WAIT_FOR_RCSA_COMPLETION   ((DFS_RCSA_INIT_COUNT + 1) * DFS_RCSA_INTVAL)    /* 600ms = 500ms(5 RCSAs) + 100 ms grace period after last RCSA */
...
#if ATH_SUPPORT_ZERO_CAC_DFS
        u_int32_t       num_seg_two_radar_detects; /* total num. of radar detected in secondary segment */
#endif
...
struct dfs_event_log {
      ...
      u_int8_t   dur;        /* duration of radar pulse */
        /* enhancement for false detection log */
        int      is_chirp;
        u_int8_t seg_id;        /* HT80_80/HT160 use */
        int      sidx;
...
struct ath_dfs {
    ...
    u_int32_t
        ...
        ath_dfs_waitfor_csa_sched:1;        /* Waiting for CSA from the uplink */
    ...
    bool       is_radar_found_on_secondary_seg; /* Radar found on secondary VHT80 in VHT80_80 or VHT160*/
    bool       is_radar_during_precac;  /* Radar found during precac */
#endif
#endif
    os_timer_t ath_dfs_task_timer;   /* dfs wait timer */
    os_timer_t ath_dfs_tx_rcsa_timer;   /* dfs RCSA(reverse CSA) timer */
    os_timer_t ath_dfs_waitfor_csa_timer;   /* Wait for CSA from parent Repeater/Root timer */
    int        dur_multiplier;
    ...
    qdf_spinlock_t  dfs_nol_lock;       /* lock to protect nol list */
    int       ath_dfs_host_wait_timeout_override; /* To over-ride the def host wait time */
    /* Averg. PRI, Duration, Sidx of the radar pulse */
    u_int32_t  ath_dfs_average_pri;
    u_int32_t  ath_dfs_average_duration;
    u_int32_t  ath_dfs_average_sidx;
#if ATH_SUPPORT_DFS && QCA_DFS_NOL_VAP_RESTART
    os_timer_t ath_dfs_no_chan_vap_restart;     /* vap restart timer */
#endif
};
...
enum {
	...
	ATH_DEBUG_DFS_FALSE_DET 	= 0x00080000,	/* false detection debug related prints */
	ATH_DEBUG_DFS_FALSE_DET2 	= 0x00100000,	/* second level check to confirm poisitive detection */
};
...
/*TODO For now considered 100ms, must experiment
 *to freeze a concrete value.
 */
...
int         dfs_process_radarevent(struct ath_dfs *dfs, struct ieee80211_ath_channel *chan);

/* Non occupancy (NOL) handling prototypes */
void        dfs_nol_addchan(struct ath_dfs *dfs, struct ieee80211_ath_channel *chan, u_int32_t dfs_nol_timeout);
...
#endif
#if ATH_SUPPORT_ZERO_CAC_DFS
void         dfs_print_precaclists(struct ieee80211com *ic,struct ath_dfs *dfs);
void         dfs_reset_precaclists(struct ieee80211com *ic,struct ath_dfs *dfs);
#endif
...
void        dfs_process_ar_event(struct ath_dfs *dfs, struct ieee80211_ath_channel *chan);
...
extern u_int8_t dfs_is_channel_in_nol(struct ath_dfs *dfs, u_int32_t chan_freq);
