    ...
    case HTT_T2H_MSG_TYPE_STATS_CONF:
        {
            ...
            ol_txrx_fw_stats_handler(pdev->txrx_pdev, cookie, stats_info_list, vdev_id);
            break;
        }
#endif
    ...
    case HTT_T2H_MSG_TYPE_PKTLOG:
        {
            u_int32_t *pl_hdr;
            pl_hdr = (msg_word + 1);
            wdi_event_handler(WDI_EVENT_OFFLOAD_ALL, pdev->txrx_pdev, pl_hdr,
    ...
    case HTT_T2H_MSG_TYPE_TX_COMPL_IND:
        {
            ...
            int ack_rssi_filled = 0, ppdu_peer_id_filled = 0;
...
#if QCA_PARTNER_DIRECTLINK
            if (ath_is_directlink((struct ol_txrx_pdev_t*)pdev->txrx_pdev)) {
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ERROR: TX_INSPECT_IND over Directlink\n");
                A_ASSERT(0);
            }
#endif /* QCA_PARTNER_DIRECTLINK */
            ol_tx_inspect_handler(pdev->txrx_pdev, num_msdus, msg_word + 1);
    ...
    case HTT_T2H_MSG_TYPE_MGMT_TX_COMPL_IND:
        {
            ...
            A_UINT8 *frm_buf = NULL;
            qdf_nbuf_t tx_mgmt_frm = NULL;
            int vdev_id, subtype, ret;
            struct ieee80211_frame *wh = NULL;

            ret = 0;
            htt_tx_mgmt_desc_free(pdev, compl_msg->desc_id, compl_msg->status, compl_msg->ppdu_id);
            OL_TX_LL_FAST_UPDATE_HTT_COMPLS(*num_htt_cmpls, 1);

            qdf_spin_lock_bh(&pdev->mgmtbufLock);
            if (!qdf_nbuf_is_queue_empty(&pdev->mgmtbufQ)) {
            ...
            /*
             * drop dequeued probe resp frame if desc pool is 75% allocated, and
             * SW Q is 50% full.
             */
#define MAX_MGMT_BUF_PKTS 512
            wh = (struct ieee80211_frame *)wbuf_header(tx_mgmt_frm);
            subtype =  wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK;

            if ((subtype == IEEE80211_FC0_SUBTYPE_PROBE_RESP) &&
                            (htt_tx_mgmt_pending_frames_cnt(pdev) > (HTT_MAX_NUM_MGMT_DESCS*3)/4)) {
                if(qdf_nbuf_queue_len(&pdev->mgmtbufQ) > MAX_MGMT_BUF_PKTS/2) {
                    ret = -1;
                    goto skip_send;
                }
            }
            ...
            vap = wbuf_get_node(tx_mgmt_frm)->ni_vap;
            vdev_id = (OL_ATH_VAP_NET80211(vap))->av_if_id;

            ret = htt_h2t_mgmt_tx(pdev->txrx_pdev,
                                  tx_mgmt_frm,
            ...
            skip_send:
                if (ret != 0) {
                    ...
                    ts.ts_retries = 0;
                    ieee80211_complete_wbuf(tx_mgmt_frm, &ts);
                }
    ...
    case HTT_T2H_MSG_TYPE_STATS_CONF:
        {
            ...
            stats_info_list = (u_int8_t *) (msg_word + 3);
            if ((ol_txrx_fw_stats_handler(pdev->txrx_pdev, cookie, stats_info_list, vdev_id)) == 0) {
                (*num_htt_cmpls)++;
    ...
    case HTT_T2H_MSG_TYPE_EN_STATS:
        {
            ...
            ol_ath_smart_ant_stats_handler(pdev->txrx_pdev, msg_word, qdf_nbuf_len(htt_t2h_msg));
#endif
#if ENHANCED_STATS
            ol_ath_enh_stats_handler(pdev->txrx_pdev, msg_word, qdf_nbuf_len(htt_t2h_msg));
#endif
            break;
        }
    case HTT_T2H_MSG_TYPE_CHAN_CHANGE:
        {
            ...
            pdev->txrx_pdev->rx_mon_recv_status->rs_freq =
            ...
            //printk("\nfreq %d bc_f1 %d bc_f2 %d mode %d\n",scn->sc_chan_freq,scn->sc_chan_band_center_f1,scn->sc_chan_band_center_f2,scn->sc_chan_phy_mode);
            break;
        }
    ...
    case HTT_T2H_MSG_TYPE_TX_MODE_SWITCH_IND:
        {
            ...
            if (ath_is_directlink((struct ol_txrx_pdev_t*)(pdev->txrx_pdev))) {
                ...
                qdf_print("Switching to Tx Mode-%u Threshold %d\n", mode, threshold);
                break;
            }
#endif
            if(mode_enable) {
                ...
                if(mode == HTT_TX_MODE_PUSH_NO_CLASSIFY) {
                    ...
                    qdf_print("Switching to Tx Mode-%u\n", mode);
                    ol_tx_switch_mode_flush_buffers(radiopdev);
    ...
    case HTT_T2H_MSG_TYPE_TX_FETCH_IND:
        {
            ...
            radiopdev->pstats[HTTFETCH].value++;
            ...
            if(num_resp_id_records == 0)
                radiopdev->pstats[HTTFETCH_NOID].value++;
            /* free response ids  */
            ...
            if (!tx_fetch_desc) {
                radiopdev->pstats[HTTDESC_ALLOC_FAIL].value++;
                break;
            }

            radiopdev->pstats[HTTDESC_ALLOC].value++;
#if PEER_FLOW_CONTROL_DEBUG
            qdf_print("\n HTT FETCH Rx Start\n");
            qdf_print("\n Fetch seq_num %u num_fetch_records %u num_resp_id_records %u\n",seq_num,
            ...
            num_resp_records = ol_tx_ll_fetch_sched(pdev->txrx_pdev, num_fetch_records,
            ...
            radiopdev->pstats[HTTFETCH_RESP].value++;
            htt_h2t_fetch_resp_tx(pdev->txrx_pdev, tx_fetch_desc);
    ...
    case HTT_T2H_MSG_TYPE_TX_FETCH_CONF:
        {
            ...
            radiopdev->pstats[HTTFETCH_CONF].value++;
...
#endif
#if ATH_SUPPORT_CFR
    /* TODO: Do we have to move this in htt_t2h_msg_handler_fast  */
    case HTT_T2H_MSG_TYPE_CFR_DUMP_COMPL_IND:
        {
            ...
            struct htt_cfr_info htt_cfr;

            if (scn->cfr_support == 0){
                qdf_print("CFR capture not enabled/supported in Host SW\n");
                break;
            }

            memset(&htt_cfr,0,sizeof(struct htt_cfr_info));

            /* This CFR msg info(bit 8:31)should be checked for any new element added in
             * this capture type.Ex:Rssi .TODO:pass msg info,capture type and msgword+3 to CFR file,
             * to avoid additional copy? */

            htt_cfr.msg_info        = *msg_word;
            htt_cfr.capture_type    = *(msg_word + 1);
            switch (htt_cfr.capture_type){
                u_int8_t *peeraddr;
                case HTT_PEER_CFR_CAPTURE_MSG_TYPE_1:

                        /*TODO : use pcfrtype= &htt_cfr.u.cfr_type_1 like that */
                        htt_cfr.u.cfr_type_1.mem_req_id          = HTT_T2H_CFR_DUMP_TYPE1_MEM_REQ_ID_GET(*(msg_word + 2));
                        htt_cfr.u.cfr_type_1.cap_status          = HTT_T2H_CFR_DUMP_TYPE1_STATUS_GET(*(msg_word + 2));
                        htt_cfr.u.cfr_type_1.capture_bw          = HTT_T2H_CFR_DUMP_TYPE1_CAP_BW_GET(*(msg_word + 2));
                        htt_cfr.u.cfr_type_1.capture_mode        = HTT_T2H_CFR_DUMP_TYPE1_MODE_GET(*(msg_word + 2));
                        htt_cfr.u.cfr_type_1.sts                 = HTT_T2H_CFR_DUMP_TYPE1_STS_GET(*(msg_word + 2));
                        htt_cfr.u.cfr_type_1.channel_bw          = HTT_T2H_CFR_DUMP_TYPE1_CHAN_BW_GET(*(msg_word + 2));
                        htt_cfr.u.cfr_type_1.cap_type            = HTT_T2H_CFR_DUMP_TYPE1_CAP_TYPE_GET(*(msg_word + 2));
                        htt_cfr.u.cfr_type_1.vdev_id             = HTT_T2H_CFR_DUMP_TYPE1_VDEV_ID_GET(*(msg_word + 2));

                        peeraddr = htt_t2h_mac_addr_deswizzle((u_int8_t *) (msg_word+3),&(htt_cfr.u.cfr_type_1.peeraddr[0]));
                        memcpy(&(htt_cfr.u.cfr_type_1.peeraddr[0]),peeraddr,6);

                        htt_cfr.u.cfr_type_1.index               = *(msg_word + 5);
                        htt_cfr.u.cfr_type_1.length              = *(msg_word + 6);
                        htt_cfr.u.cfr_type_1.timestamp           = *(msg_word + 7);
                        htt_cfr.u.cfr_type_1.counter             = *(msg_word + 8);
                        htt_cfr.u.cfr_type_1.chan_mhz            = *(msg_word + 9);
                        htt_cfr.u.cfr_type_1.band_center_freq1   = *(msg_word + 10);
                        htt_cfr.u.cfr_type_1.band_center_freq2   = *(msg_word + 11);
                        htt_cfr.u.cfr_type_1.chan_mode           = *(msg_word + 12);

                        if (prdev->cfr_callback.cfr_rx_htt_ind)
                            prdev->cfr_callback.cfr_rx_htt_ind(prdev->cfr_callback.ctxt,&htt_cfr,htt_cfr.u.cfr_type_1.length);
                        
                        break;
                default:
                        qdf_print(" Unsupported CFR capture type:%d\n",htt_cfr.capture_type);
                        break;
            }
...
#define HTT_T2H_MSG_BUF_REINIT(_buf, _scn)                              \
    do {                                                                \
            uint32_t paddr_lo;                                          \
            uint8_t *buf_end;                                           \
            ...
            qdf_nbuf_init((_buf));                                      \
            OS_SYNC_SINGLE((_scn)->sc_osdev, paddr_lo,                  \
                            (buf_end - (_buf)->data),                   \
    ...
    for (i = 0; i < num_cmpls; i++) {
        ...
        if (pdev->nss_wifiol_ctx) {
            if ((msg_type == HTT_T2H_MSG_TYPE_TX_INSPECT_IND) ||
                ...
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "NSS Wifi Offload : Unhandled %x\n", msg_type);
                qdf_nbuf_free(htt_t2h_msg);
                continue;
            ...
            case HTT_T2H_MSG_TYPE_TX_COMPL_IND:
                {
                    ...
                    enum htt_tx_status status;
                    int ack_rssi_filled = 0, ppdu_peer_id_filled = 0;
                    HTT_TX_PDEV_LOCK(&(pdev->txrx_pdev)->tx_lock);
...
#if QCA_PARTNER_DIRECTLINK
                    if (ath_is_directlink((struct ol_txrx_pdev_t*)pdev->txrx_pdev)) {
                        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ERROR: TX_COMPL_IND received over Directlink\n");
                        A_ASSERT(0);
                    }
#endif /* QCA_PARTNER_DIRECTLINK */
                    ol_tx_completion_handler(
                    ...
                    HTT_TX_PDEV_UNLOCK(&(pdev->txrx_pdev)->tx_lock);
#if PEER_FLOW_CONTROL
                    PFLOW_CTRL_PDEV_STATS_ADD(pdev->txrx_pdev, TX_COMPL_IND, num_htt_cmpls);
#endif
...
#endif
        {
            ...
            HTT_T2H_MSG_BUF_REINIT(htt_t2h_msg, scn);
        }
