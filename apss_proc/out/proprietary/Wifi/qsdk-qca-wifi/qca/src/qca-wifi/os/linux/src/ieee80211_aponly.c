...
#endif
int wrap_tx_bridge_da ( wlan_if_t *vap , osif_dev **osdev,  struct sk_buff **skb);
...
#if USE_LITEPATH
extern struct sk_buff* ath_dpath_rxbuf_alloc ( struct net_device *dev, int32_t size, int32_t reserve, int32_t align );
extern struct sk_buff* ath_dpath_rxbuf_alloc_da ( struct net_device *dev, int32_t size, int32_t reserve, int32_t align );
#endif
                          ...
                          ath_bufhead *bf_q, struct ath_tx_status *ts, int txok)
{
     ...
     */
   {
...
#ifdef ATH_SUPPORT_TxBF
                ath_tx_complete_buf(sc, defer_completion.bf, &defer_completion.bf_head, ts, !defer_completion.txfail, ts->ts_txbfstatus, ts->ts_tstamp);
#else
                ath_tx_complete_buf(sc, defer_completion.bf, &defer_completion.bf_head, ts, !defer_completion.txfail);
#endif
            }
            /*
	...
	if (unlikely(sendbar && !ath_vap_pause_in_progress(sc))) {
		...
		QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:txq pause is in progress\n",__func__);
	}
...
#ifdef ATH_SUPPORT_TxBF
                        ath_tx_complete_buf(sc, bf, &bf_head, &txstat, txok, 0, 0);
#else
                        ath_tx_complete_buf(sc, bf, &bf_head, &txstat, txok);
#endif
...
#endif
        ath_edma_free_complete_buf(sc,txq, &bf_headfree, &ts, txok);
...
ieee80211_input_data_aponly(struct ieee80211_node *ni, wbuf_t wbuf, struct ieee80211_rx_status *rs, int subtype, int dir)
{
    ...
    u_int16_t hdrspace;
    u_int16_t hdrsize;
    uint8_t is_amsdu = 0, is_mcast, is_ipv6_mcast, is_mdns = 0;
    u_int16_t associd = 0;
    uint32_t last_mdns_msecs = 0, curr_mdns_msecs = 0, mdns_time_diff = 0;
    uint8_t mdns_mac[] = {0x01, 0x00, 0x5e, 0x00, 0x00, 0xfb};
    uint8_t mdns_mac_ipv6[] = {0x33, 0x33, 0, 0, 0, 0xfb}; /* IPv6 uses the prefix 33:33:* */
    ...
    if (dir == IEEE80211_FC1_DIR_DSTODS)
    {
        ...
        {
             QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "mac %s should not in here\n", ether_sprintf(IEEE80211_WH4(wh)->i_addr4));
                        goto bad;
        }
    }

    is_ipv6_mcast = IEEE80211_IS_IPV6_MULTICAST(IEEE80211_WH4(wh)->i_addr3);

    if (vap->iv_drop_mdns) {
        if (is_mcast || is_ipv6_mcast) {
            uint8_t mcast_addr[IEEE80211_ADDR_LEN];
            IEEE80211_ADDR_COPY(mcast_addr, IEEE80211_WH4(wh)->i_addr3);
            if ((IEEE80211_ADDR_EQ(mcast_addr, (uint8_t *)mdns_mac)) ||
                    (IEEE80211_ADDR_EQ(mcast_addr, (uint8_t *)mdns_mac_ipv6))) {
                is_mdns = 1;
            }
        }

        if(is_mdns) {
            last_mdns_msecs = (uint32_t) CONVERT_SYSTEM_TIME_TO_MS(ni->ni_last_mdns_rx);
            curr_mdns_msecs = (uint32_t) CONVERT_SYSTEM_TIME_TO_MS(OS_GET_TIMESTAMP());
            mdns_time_diff = curr_mdns_msecs - last_mdns_msecs;

            if(mdns_time_diff > MDNS_DROP_THRESHOLD) {
                ni->ni_last_mdns_rx = OS_GET_TIMESTAMP();
            } else {
                goto bad;
            }
        }
...
ath_net80211_rx_aponly(ieee80211_handle_t ieee, wbuf_t wbuf, ieee80211_rx_status_t *rx_status, u_int16_t keyix)
{
    ...
    if (unlikely(IEEE80211_IS_DATA(wh) && rx_status->nomoreaggr)) {
        ...
        an->an_rxrateflags = rx_status->flags & 0xc0;  /* similar to info in OL rateflags currently*/
    }
...
#define DESC2PA(_sc, _va)\
    ...
    if (rxenable && !atomic_read(&sc->sc_in_reset)) {
        ...
        if (ath_hal_getrxbuf(ah, 0) == 0)
        {
            ...
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: FATAL ERROR: NULL RXDP while enabling RX.\n", __func__);
            ASSERT(FALSE);
        } else
#endif
        ath_hal_rxena(ah);
...
ath_rx_removebuffer_aponly(struct ath_softc *sc, HAL_RX_QUEUE qtype)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        if (wbuf) {
            ...
            if (!bf) {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d] PANIC wbuf %pK Index %d\n", __func__, __LINE__, wbuf, i);
            } else {
    ...
    if (rxedma->rxfifodepth)
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "PANIC depth non-zero %d\n", rxedma->rxfifodepth);
}
static void
...
ath_rx_intr_aponly(ath_dev_t dev, HAL_RX_QUEUE qtype)
{
    ...
    do {
        ...
        if (unlikely(rxedma->rxfifodepth == 0))
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ath_rx_intr: depth 0 PANIC\n");
...
extern qdf_nbuf_t ath_rxbuf_alloc_cbm_partner(struct ath_softc *sc, u_int32_t len);
#endif

#if USE_LITEPATH
qdf_nbuf_t ath_rxbuf_dp_alloc(struct ath_softc *sc, u_int32_t len)
{
    qdf_nbuf_t nbf;
    struct net_device *dev = sc->sc_osdev->netdev;

    if (sc->sc_opmode == HAL_M_MONITOR) {
        nbf = ( qdf_nbuf_t )ath_dpath_rxbuf_alloc_da ( dev, len + sizeof(wlan_ng_prism2_header),
...
#if LIMIT_RXBUF_LEN_4K
        nbf = ( qdf_nbuf_t )ath_dpath_rxbuf_alloc_da ( dev, len - NET_SKB_PAD, NET_SKB_PAD, 0 );
#else
        nbf = ( qdf_nbuf_t )ath_dpath_rxbuf_alloc_da ( dev, len, NET_SKB_PAD, sc->sc_cachelsz );
#endif
...
static inline int ath_rx_indicate_aponly(struct ath_softc *sc, wbuf_t wbuf, ieee80211_rx_status_t *status, u_int16_t keyix)
{
    ...
    if (unlikely(nwbuf == NULL)) {

		if (sc->sc_rxbuf_reuse_enable && ath_can_wbuf_recycle(wbuf)) {
			 ...
			 */
			indicate    = 0;
			ath_wbuf_recycle(sc, bf, wbuf);
			sc->sc_stats.ast_rx_nobuf++;
			nwbuf = wbuf;
		
		}else{

			/* Could not allocate the buffer and also not able to
			 * reuse current buffeir.Refill it when its avilable latter */
			requeue = 0;
		}
	}
    ...
    if (likely(requeue)){
        ...
        ath_rx_requeue(sc, nwbuf);
            ...
            /* refill buffer to HW latter when its avilable */
            ath_rx_refill_queue(sc, bf); 
    }
        ...
        ieee80211_rx_status_t *rx_status, u_int8_t *chainreset)
{
    ...
    if(!selevm) {
            ...
            rx_status->htsig[6] = rxs->evm2  & 0xFF;
            rx_status->htsig[7] = rxs->evm3 >> 24 & 0xFF;
            rx_status->htsig[8] = rxs->evm3 >> 16 & 0xFF;
            rx_status->htsig[9] = rxs->evm3 >> 8 & 0xFF;
    }
     ...
     */
    {
        ...
        if (ic->ic_green_ap_get_print_level(ic)) {
            if (!(print_now & 0xff)) {
                  QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Rx rssi0 %d rssi1 %d rssi2 %d rssi3 %d\n",
                          (int8_t)rxs->rs_rssi_ctl0,
                          (int8_t)rxs->rs_rssi_ctl1,
                          (int8_t)rxs->rs_rssi_ctl2,
                          (int8_t)rxs->rs_rssi_ctl3);
            }
        }
        print_now++;
    }
    ...
    if (likely(sc->sc_hashtsupport)) {
        ...
        if (rxs->rs_moreaggr == 0) {
            ...
            if (rxs->rs_rssi_ctl3 != ATH_RSSI_BAD) {
                rx_status->rssictl[3]  = rxs->rs_rssi_ctl3;
                rssi_chain_valid_count++;
                sc->sc_stats.ast_rx_rssi_ctl3 = rxs->rs_rssi_ctl3;
            }
            ...
            if (rxs->rs_flags & HAL_RX_2040) {
                ...
                if (rxs->rs_rssi_ext3 != ATH_RSSI_BAD) {
                    rx_status->rssiextn[3]  = rxs->rs_rssi_ext3;
                    rssi_extn_valid_count++;
                    sc->sc_stats.ast_rx_rssi_ext3 = rxs->rs_rssi_ext3;
                }
    ...
    /* Update HT Rx Rate STATS */
    if ((rxs->rs_rate >= 0x80) && ((rxs->rs_rate <= 0x97))) {
         sc->sc_stats.ast_rx_rate_packets[rxs->rs_rate - 0x80]++;
    }
    ...
    if (type == IEEE80211_FC0_TYPE_DATA) {
        ...
        sc->sc_stats.ast_rx_data_bytes += buf_len;
    }
...
static inline void ath_rx_wifipos_isresp_aponly(struct ath_rx_status *rxs, struct ath_softc *sc, wbuf_t wbuf, unsigned char pkt_type)
{
    ...
    wifiposdata.rssi3 = rxs->rs_rssi_ctl3;
...
rx_next:
    ...
    if (unlikely(chainreset)) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Reset rx chain mask. Do internal reset. (%s)\n", __func__);
        ath_internal_reset(sc);
    }
...
ath_aggr_check_aponly(ath_dev_t dev, ath_node_t node, u_int8_t tidno)
{
    ...
    if ((tid->addba_exchangecomplete) || (tid->addba_exchangeinprogress))  return 0;
    tid->addba_retry++;
    if (!tid->addba_exchangeinprogress &&
		    ((tid->addba_exchangeattempts < ADDBA_EXCHANGE_ATTEMPTS) || (tid->addba_retry % 10000 == 1 )))
    {
...
ath_tx_start_dma_aponly(wbuf_t wbuf, sg_t *sg, u_int32_t n_sg, void *arg)
{
    ...
    for (i = 0; i < n_sg; i++, sg++) {
        ...
        atf_based_alloc = 0;
        if (an->an_node) {
            subgroup = (struct subgroup_list *)sc->sc_ieee_ops->atf_find_subgroup((ieee80211_node_t)((an)->an_node), txctl->tidno);
            atf_commit = sc->sc_ieee_ops->get_atf_allocations((ieee80211_node_t)((an)->an_node), subgroup,
            ...
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: no ni for an sending packet\n", __func__);
            retval = ath_tx_get_buf_aponly(sc, sg, &bf, &bf_head, buf_used);
        }
#else
        retval = ath_tx_get_buf_aponly(sc, sg, &bf, &bf_head, buf_used);
        ...
        if ((likely(retval != ATH_BUF_NONE)) && (atf_based_alloc == 1)
            ...
            if (bf->bf_atf_accounting) {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: buffer already marked for atf accounting\n", __func__);
            }
            bf->bf_atf_accounting = 1;
            bf->bf_atf_sg = (void *)subgroup;
        ...
        bf->bf_state.bfs_is_proberesp =
		                (txctl->atype == HAL_PKT_TYPE_PROBE_RESP) ? 1:0;
    ...
    if (firstbf) {
		...
		if (!no_wait_for_vap_pause) {
			...
			if(sc->sc_vap_pause_timeout){
				ath_vap_pause_txq_use_dec(sc);
				QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:txq_pause in progress flag is set\n",__func__);
				goto bad;
			}
#endif
		}
...
osif_vap_hardstart_aponly(struct sk_buff *skb, struct net_device *dev)
{
...
#endif
    {
        ...
        if (vap->iv_drop_mdns) {
            ...
            if (IEEE80211_IS_MULTICAST(eh->ether_dhost) || IEEE80211_IS_IPV6_MULTICAST(eh->ether_dhost)) {
                is_mcast = 1;
            }

            if (is_mcast) {
                if ((IEEE80211_ADDR_EQ((eh)->ether_dhost, (uint8_t *)mdns_mac)) ||
                        (IEEE80211_ADDR_EQ((eh)->ether_dhost, (uint8_t *)mdns_mac_ipv6))) {
                    is_mdns = 1;
                    last_mdns_msecs = (uint32_t) CONVERT_SYSTEM_TIME_TO_MS(vap->iv_last_mdns_tx);
                    curr_mdns_msecs = (uint32_t) CONVERT_SYSTEM_TIME_TO_MS(OS_GET_TIMESTAMP());
                    mdns_time_diff = curr_mdns_msecs - last_mdns_msecs;
                    if(mdns_time_diff > MDNS_DROP_THRESHOLD) {
                        vap->iv_last_mdns_tx = OS_GET_TIMESTAMP();
                    } else {
                        goto bad;
                    }
                }
                                          ...
                                          struct ath_tx_status *ts)
{
    ...
    if (sc->sc_ieee_ops->smart_ant_update_txfeedback) {
        ...
        tx_feedback.rssi[3] = ((ts->ts_rssi_ctl3 | ts->ts_rssi_ext3 << 8) | INVALID_RSSI_WORD);
...
static inline uint32_t ath_smart_ant_rxfeedback_aponly(struct ath_softc *sc,  wbuf_t wbuf, struct ath_rx_status *rxs, uint32_t pkts)
{
    ...
    if (sc->sc_ieee_ops->smart_ant_update_rxfeedback) {
        ...
        rx_feedback.rx_rssi[3] = ((rxs->rs_rssi_ctl3 | rxs->rs_rssi_ext3 << 8) | INVALID_RSSI_WORD);
...
static inline void
ath_tx_seqno_set_wds(struct ath_node *an, u_int16_t tidno, wbuf_t wbuf, bool isqosdata, bool isctl)
{
    struct ieee80211_frame *wh = (struct ieee80211_frame *)wbuf_header(wbuf);
...
#define ath_tx_seqno_set_wds(an, tidno, wbuf, isqosdata, isctl)  /* */
 ...
 */
static INLINE void
ath_rate_set_minrate_wds(struct ath_softc *sc,
        ieee80211_tx_control_t *txctl, const HAL_RATE_TABLE *rt,
        struct ath_rc_series *rcs)
{
    if (txctl->min_rate != 0)
 ...
 */
static INLINE int
__ath_tx_prepare_wds(struct ath_softc *sc, wbuf_t wbuf, ieee80211_tx_control_t *txctl)
{
    struct ath_node *an;
    u_int8_t rix;
    struct ath_txq *txq = NULL;
    ...
    if (txctl->isdata &&
        ...
        if (txctl->ismcast) {
             ...
             */
            rcs[0].tries = 1;
            ath_tx_seqno_set_wds(an, WME_MGMT_TID, wbuf, 0, 0);
        } else {
            /*
             * For aggregation enabled nodes there is no need to do rate find
             * on each of these frames.
            ...
            if (
                ...
                if (likely(!txctl->use_minrate)) {
                    ...
                    ath_rate_set_minrate_wds(sc, txctl, rt, rcs);
                }
                ...
                if (
                    ...
                    ath_tx_seqno_set_wds(an, txctl->tidno, wbuf, txctl->isqosdata, 0);
                }
            } else {
                /*
                 * For HT capable stations, we save tidno for later use.
        ...
        if(wbuf_is_eapol(wbuf)){
            ...
            *(u_int16_t *)wh->i_seq = htole16(tid->seq_next << IEEE80211_SEQ_SEQ_SHIFT);
            ...
            ath_tx_seqno_set_wds(an, txctl->tidno, wbuf, 0, !txctl->ismgmt);
        }
        ...
        {
            rcs[0].rix = ath_rate_findrix_aponly(rt, 12);
        }
        if(txctl->flags & HAL_TXDESC_POS)
        {
            ...
            if (txctl->wifiposdata) {
                ...
                rcs[0].rix = ath_rate_findrix_aponly(rt, map_rate_wifipos[wifipos_rix]);
                rcs[1].rix = rcs[0].rix;
...
#if ATH_SUPPORT_VOWEXT
static ath_get_buf_status_t
ath_tx_get_buf_wds(struct ath_softc *sc, sg_t *sg, struct ath_buf **pbf,
 ...
 */
static inline int
ath_tx_start_dma_wds(wbuf_t wbuf, sg_t *sg, u_int32_t n_sg, void *arg)
{
    ieee80211_tx_control_t *txctl = (ieee80211_tx_control_t *)arg;
    struct ath_softc *sc = (struct ath_softc *)txctl->dev;
    struct ath_node *an = txctl->an;
...
#undef IEEE80211_NODE_ATH_PAUSED
    ...
    for (i = 0; i < n_sg; i++, sg++) {
        ...
        if (!vicontention)
        {
            retval = ath_tx_get_buf_wds(sc, sg, &bf, &bf_head,
                    txctl->qnum, buf_used);
        }
        else
        {
...
#else
        retval = ath_tx_get_buf_wds(sc, sg, &bf, &bf_head, buf_used);
#endif
#else
#if ATH_SUPPORT_VOWEXT
        retval = ath_tx_get_buf_wds(sc, sg, &bf, &bf_head, txctl->qnum);
#else
        retval = ath_tx_get_buf_wds(sc, sg, &bf, &bf_head);
#endif
#endif
...
#endif
            {
                ...
                if (!send_to_cabq) {
...
#if UMAC_SUPPORT_WNM
                    {
                        ...
                        ath_tx_mcastqaddbuf_internal_aponly(sc, tmp_txq, &bf_head);
...
static int
wbuf_map_sg_wds(osdev_t osdev, qdf_nbuf_t nbf, dma_addr_t *pa, void *arg)
{
    struct scatterlist sg;
    int ret;
    ...
    *pa = bus_map_single(osdev, nbf->data, UNI_SKB_END_POINTER(nbf) - nbf->data, BUS_DMA_TODEVICE);
    ...
    ret = ath_tx_start_dma_wds(nbf, &sg, 1, arg);
    if (ret) {
        /*
         * NB: common code doesn't tail drop frame
         * because it's not allowed in NDIS 6.0.
...
static int
ath_tx_start_wds(ath_dev_t dev, wbuf_t wbuf, ieee80211_tx_control_t *txctl)
{
    struct ath_softc *sc = ATH_DEV_TO_SC(dev);
    int err;
#ifdef ATH_SUPPORT_DFS
    ...
    err = __ath_tx_prepare_wds(sc, wbuf, txctl);
    if (err){
        return err;
    }
     ...
     * or asynchrounsly once DMA is complete.
     */
    err = wbuf_map_sg_wds(sc->sc_osdev, wbuf,
            OS_GET_DMA_MEM_CONTEXT(txctl, dmacontext),
            txctl);
if (err) {
#if ATH_RIFS
 ...
 */
static void ieee80211_ibss2ap_header_wds(struct ieee80211_frame* wh){
...
static wbuf_t
ieee80211_encap_80211_wds(struct ieee80211_node *ni, wbuf_t wbuf)
{
    struct ieee80211com *ic = ni->ni_ic;
    struct ieee80211vap *vap = ni->ni_vap;
    struct ieee80211_key *key=NULL;
    ...
    if(ic->ic_softap_enable && ibssaddr && (type == IEEE80211_FC0_TYPE_DATA)){
        ieee80211_ibss2ap_header_wds(wh);
    }
    datalen = wbuf_get_pktlen(wbuf) - (hdrsize + sizeof(struct llc));  /* NB: w/o 802.11 header */
     ...
     * And wbuf_set_eapol has been called in wlan_vap_send_wds.
     */
    ...
    if (wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_QOS) {
        ...
        /*
         * XXX: we assume a QoS frame must come from ieee80211_encap_8023_wds() function,
         * meaning it's already padded. If OS sends a QoS frame (thus without padding),
         * then it'll break.
         */
        if (ic->ic_flags & IEEE80211_F_DATAPAD) {
...
struct ieee80211_node *
__ieee80211_find_wds_node_wds(struct ieee80211_node_table *nt, const u_int8_t *macaddr, struct ieee80211_wds_addr **wds)
{
    struct ieee80211_node *ni;
    u_int8_t stag=0;
    ...
    rwlock_state_t lock_state;
    OS_RWLOCK_WRITE_LOCK(&nt->nt_wds_nodelock, &lock_state);
    ni = _ieee80211_find_wds_node_wds(nt, macaddr, wds, &stag);
    if (!(nt->nt_ic->ic_is_mode_offload(nt->nt_ic))) {
        /* find wds node should return the pointer the ni, for some reasons,
         * wds entry would have been found on staging. At that instance
         * probably we do not have the node pointer. It, means, there is no
...
struct ieee80211_node *
ieee80211_find_wds_node_wds(struct ieee80211_node_table *nt, const u_int8_t *macaddr)
{
    struct ieee80211_wds_addr *wds = NULL;

    return __ieee80211_find_wds_node_wds(nt, macaddr, &wds);
}

static systime_t
_ieee80211_get_wds_node_time_wds(
        struct ieee80211_node_table *nt,
        const u_int8_t *macaddr)
{
    struct ieee80211_wds_addr *wds;
...
static u_int32_t
ieee80211_find_wds_node_age_wds(struct ieee80211_node_table *nt, const u_int8_t *macaddr)
{
    systime_t wds_time;
    u_int32_t wds_age = 0;
    ...
    OS_RWLOCK_WRITE_LOCK(&nt->nt_wds_nodelock, &lock_state);
    wds_time = _ieee80211_get_wds_node_time_wds(nt, macaddr);
...
static inline int
_ieee80211_add_wds_addr_wds(struct ieee80211_node_table *nt,
...
static inline int
ieee80211_add_wds_addr_wds(struct ieee80211_node_table *nt,
        struct ieee80211_node *ni, const u_int8_t *macaddr,
        u_int32_t flags)
{
    return _ieee80211_add_wds_addr_wds(nt, ni, macaddr, flags);
}

static void
ath_node_del_wds_entry_wds(struct ieee80211com *ic, u_int8_t *dest_mac, u_int32_t flags)
{
    return;
}

static void
ieee80211_set_wds_node_time_wds(struct ieee80211_node_table *nt, const u_int8_t *macaddr)
{
    struct ieee80211_wds_addr *wds;
    int hash;
...
static inline void
ieee80211_remove_wds_addr_wds(struct ieee80211_node_table *nt,
        const u_int8_t *macaddr, u_int32_t flags)
{
    ...
    if (is_node_delete)
    {
        ...
        ath_node_del_wds_entry_wds(ic,destmac,flags);
    }
}
 ...
 */
static INLINE wbuf_t
ieee80211_encap_8023_wds(struct ieee80211_node *ni, wbuf_t wbuf)
{
    struct ieee80211vap *vap = ni->ni_vap;
    struct ieee80211com *ic = ni->ni_ic;
    struct ieee80211_rsnparms *rsn = &vap->iv_rsn;
        ...
        case IEEE80211_M_STA:
                    ...
                    (IEEE80211_VAP_IS_WDS_ENABLED(vap)))
            {
                ...
                if(IEEE80211_IS_MULTICAST(eh.ether_dhost))
                {
                    ...
                    struct ieee80211_node *ni_wds=NULL;
                    nt = &ic->ic_sta;
                    ni_wds = ieee80211_find_wds_node_wds(nt,eh.ether_shost);
                    if(ni_wds)
                    {
                        ieee80211_set_wds_node_time_wds(nt,eh.ether_shost);
                        ieee80211_free_node(ni_wds); /* Decr ref count */
                    }
                    else
                    {
                        ieee80211_add_wds_addr_wds(nt, ni, eh.ether_shost,
    ...
    *(u_int16_t *)wh->i_dur = 0;
    ...
    if((vap->iv_opmode == IEEE80211_M_HOSTAP) && (IEEE80211_VAP_IS_WDS_ENABLED(vap)))
    {
        ...
        ni_wds = ieee80211_find_wds_node_wds(&ic->ic_sta, eh.ether_shost);
        /* Last call increments ref count if !NULL */
        if (ni_wds != NULL) {
            /* if 4 address source pkt reachable through same node as dest
             * then remove the source addr from wds table
            ...
            if(use4addr && (ni_wds == ni))
            {
                ieee80211_remove_wds_addr_wds(&ic->ic_sta, eh.ether_shost, IEEE80211_NODE_F_WDS_REMOTE);
            }
            else if (isMulticast)
            {
                wds_age = ieee80211_find_wds_node_age_wds(&ic->ic_sta, eh.ether_shost);
                if(wds_age > 2000)
                {
                    ieee80211_remove_wds_addr_wds(&ic->ic_sta, eh.ether_shost, IEEE80211_NODE_F_WDS_REMOTE);
                }
    ...
    if((vap->iv_opmode == IEEE80211_M_STA) && (IEEE80211_VAP_IS_WDS_ENABLED(vap)))
    {
        ...
        ni_wds = ieee80211_find_wds_node_wds(&ic->ic_sta, eh.ether_dhost);
        /* Last call increments ref count if !NULL */
        if (ni_wds != NULL) {
            ieee80211_remove_wds_addr_wds(&ic->ic_sta, eh.ether_dhost,IEEE80211_NODE_F_WDS_BEHIND);
            ieee80211_free_node(ni_wds); /* Decr ref count */
        }
    }
...
#endif /* UMAC_SUPPORT_WNM */
    return ieee80211_encap_80211_wds(ni, wbuf);
...
static inline int
ieee80211_80211frm_amsdu_check_wds(wbuf_t wbuf)
{
    struct ieee80211_frame *wh;
...
static inline int
ieee80211_8023frm_amsdu_check_wds(wbuf_t wbuf)
{
    struct ether_header *eh;
 ...
 */
static inline int
ieee80211_amsdu_check_wds(struct ieee80211vap *vap, wbuf_t wbuf)
{
    if (wbuf_is_uapsd(wbuf) || wbuf_is_moredata(wbuf)) {
        return 1;
    }
    if (IEEE80211_VAP_IS_SEND_80211_ENABLED(vap)) {
        return ieee80211_80211frm_amsdu_check_wds(wbuf);
    }
    else {
        return ieee80211_8023frm_amsdu_check_wds(wbuf);
    }
}
#endif /* ATH_AMSDU */
 ...
 */
static inline wbuf_t
ath_amsdu_send_wds(wbuf_t wbuf)
{
    struct ieee80211_node *ni = wbuf_get_node(wbuf);
    struct ieee80211com *ic = ni->ni_ic;
    struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(ic);
    ...
    framelen = roundup(wbuf_get_pktlen(wbuf), 4);
    amsdu_deny = ieee80211_amsdu_check_wds(vap, wbuf);
    /* Set the tx status flags */
    ts.ts_flags = 0;
    ts.ts_retries = 0;
#ifdef ATH_SUPPORT_TxBF
...
static inline void
ieee80211_protect_set_tim_wds(struct ieee80211vap *vap)
{
 ...
 */
static inline void
ieee80211_set_tim_wds(struct ieee80211_node *ni, int set,bool isr_context)
{
    struct ieee80211vap *vap = ni->ni_vap;
    u_int16_t aid;
    rwlock_state_t lock_state;
    ...
    if (!isr_context)
    {
        OS_EXEC_INTSAFE(vap->iv_ic->ic_osdev, ieee80211_protect_set_tim_wds, vap);
...
static INLINE void
ath_uapsd_pwrsave_check_wds(wbuf_t wbuf, struct ieee80211_node *ni)
{
    wlan_if_t vap = ni->ni_vap;
    if (WME_UAPSD_AC_ISDELIVERYENABLED(wbuf_get_priority(wbuf), ni))
    {
        ...
        if ((vap->iv_set_tim != NULL) && IEEE80211_NODE_UAPSD_USETIM(ni)) {
            ieee80211_set_tim_wds(ni, 1, false);
        }
    }
}

static inline wbuf_t
__ieee80211_encap_wds(struct ieee80211_node *ni, wbuf_t wbuf)
{
    struct ieee80211vap *vap = ni->ni_vap;
    ...
    if (IEEE80211_VAP_IS_SEND_80211_ENABLED(vap) || wbuf_is_encap_done(wbuf)) {
        ...
        return ieee80211_encap_8023_wds(ni, wbuf);
    }
}


#ifdef ENCAP_OFFLOAD
#define ieee80211_encap_wds( _ni , _wbuf)   (_wbuf)
#else
#define ieee80211_encap_wds( _ni , _wbuf)   __ieee80211_encap_wds( _ni, _wbuf)
#endif
...
ath_tx_send_wds(wbuf_t wbuf)
{
    ...
    ath_uapsd_pwrsave_check_wds(wbuf, ni);
    ...
    if (wbuf_is_uapsd(wbuf))
        wbuf = __ieee80211_encap_wds(ni, wbuf);
    else
#endif
    wbuf = ieee80211_encap_wds(ni, wbuf);

    if (wbuf == NULL) {
        DPRINTF(sc,ATH_DEBUG_ANY,"%s: ieee80211_encap failed \n",__func__);
        goto bad;
    }
    ...
    while (wbuf != NULL) {
        ...
        if (error == 0) {
            if (ath_tx_start_wds(scn->sc_dev, wbuf, txctl) != 0) {
                goto bad;
            }
            else {
                HTC_WBUF_TX_DATA_COMPLETE_STATUS(ic);
...
int ath_netdev_hardstart_wds(struct sk_buff *skb, struct net_device *dev)
{
    struct ath_softc_net80211 *scn = ath_netdev_priv(dev);
    struct ath_softc *sc = ATH_DEV_TO_SC(scn->sc_dev);
    struct ieee80211_cb *cb;
     ...
     * to be dropped and station disconnects.
     */
    DPRINTF(sc, ATH_DEBUG_RESET, "skb->priority=%d cb->u_tid=%d addba_status=%d txq_aggr_depth=%d txq_depth=%d\n",skb->priority, cb->u_tid, addba_status, txq_aggr_depth, txq_depth);
...
if (ismulti && early_drop) {
    ...
    if (error) {
        DPRINTF(sc, ATH_DEBUG_XMIT, "%s: Tx failed with error %d\n",
                __func__, error);
    }
    return 0;
...
static void
ieee80211_sta_power_tx_start_wds(struct ieee80211vap *vap)
{
    struct ieee80211com *ic = vap->iv_ic;
...
static void
ieee80211_sta_power_tx_end_wds(struct ieee80211vap *vap)
{
    struct ieee80211com *ic = vap->iv_ic;
...
static INLINE int
ieee80211_send_wbuf_internal_wds(struct ieee80211vap *vap, wbuf_t wbuf)
{
    int retval;
    os_if_t osif;
    /*
    ...
    vap->iv_lastdata = OS_GET_TIMESTAMP();
    vap->iv_txrxbytes += wbuf_get_pktlen(wbuf);
    ieee80211_sta_power_tx_start_wds(vap);
    ...
    /*
     * call back to shim layer to queue it to hardware device.
     * Assuming that common_evt_handler is same for both AP
     * and STA mode as per recent implementation.
     */
    osif = vap->iv_ifp;
    retval = ath_netdev_hardstart_wds( wbuf, ((osif_dev *)osif)->os_comdev);
    ieee80211_sta_power_tx_end_wds(vap);

    return retval;
}

static INLINE int
ieee80211_is_tspecActive_wds(struct ieee80211com *ic)
{
    return ic->ic_tspec_active;
}

static int
ieee80211_classify_wds(struct ieee80211_node *ni, wbuf_t wbuf)
{
    struct ieee80211vap                 *vap = ni->ni_vap;
    struct ieee80211com                 *ic = ni->ni_ic;
    int                                 ac = WME_AC_BE;
    ...
    if (vap->iv_opmode == IEEE80211_M_STA) {
        ...
        while ((ac != WME_AC_BK)
                ...
                && !ieee80211_is_tspecActive_wds(ic)) {
...
struct ieee80211_node *
#if IEEE80211_DEBUG_REFCNT
ieee80211_find_txnode_wds_debug(struct ieee80211vap *vap, const u_int8_t *macaddr,
        const char *func, int line, const char *file)
#else
ieee80211_find_txnode_wds(struct ieee80211vap *vap, const u_int8_t *macaddr)
#endif
{
    struct ieee80211_node_table *nt = &vap->iv_ic->ic_sta;
    struct ieee80211_node *ni = NULL;
        ...
        if (vap->iv_opmode == IEEE80211_M_STA ||
            ...
            if (ni == NULL) {
                if( (vap->iv_opmode == IEEE80211_M_HOSTAP)  && wlan_get_param(vap, IEEE80211_FEATURE_WDS) ) {
                    ni = ieee80211_find_wds_node_wds(nt, macaddr);
                }
            }
        }
    }
...
static void ieee80211_vap_txrx_deliver_event_wds(ieee80211_vap_t vap,ieee80211_vap_txrx_event *evt)
{
 ...
 */
static inline int
wlan_vap_send_wds(wlan_if_t vap, wbuf_t wbuf)
{
    struct ieee80211_node *ni=NULL;
    u_int8_t *daddr;
    int is_data,retval;
    ...
    ni = ieee80211_find_txnode_wds(vap, daddr);
    if (ni == NULL) {
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_OUTPUT,
                "%s: could not send packet, NI equal to NULL for %s\n",
                __func__, ether_sprintf(daddr));
        /* NB: ieee80211_find_txnode_wds does stat+msg */
        goto bad;
    }

    /* calculate priority so driver can find the tx queue */
    if (ieee80211_classify_wds(ni, wbuf)) {
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_OUTPUT,
                "%s: discard, classification failure (%s)\n",
                __func__, ether_sprintf(daddr));
        goto bad;
    ...
    if (vap->iv_txrx_event_info.iv_txrx_event_filter & IEEE80211_VAP_OUTPUT_EVENT_DATA) {
        ...
        ieee80211_vap_txrx_deliver_event_wds(vap,&evt);
    }
    ...
    ieee80211_vap_pause_update_xmit_stats(vap,wbuf); /* update the stats for vap pause module */
    retval = ieee80211_send_wbuf_internal_wds(vap, wbuf);
...
{ \
    if (qdf_unlikely(wlan_is_mpsta(_vap)))   { \
        if (wrap_tx_bridge_da (&_vap, _osdev , _skb))  { \
            goto bad;\
        }\
        if (*(_skb) == NULL) {\
            goto bad;\
...
int
osif_vap_hardstart_wds(struct sk_buff *skb, struct net_device *dev)
{
    osif_dev  *osdev = ath_netdev_priv(dev);
    wlan_if_t vap = osdev->os_if;
    struct net_device *comdev = osdev->os_comdev;
...
#endif
    {
        if (osdev->osif_is_mode_offload) {
...

            {
                ...
                //skb = wlan_vap_send_wds(osdev->iv_txrx_handle, skb);
            }
            /*
            ...
            if (do_proxy_arp(vap, skb))
                goto bad;
#endif /* UMAC_SUPPORT_PROXY_ARP */
            send_err = wlan_vap_send_wds(vap, (wbuf_t)skb);
        }
    }
#if ATH_SUPPORT_FLOWMAC_MODULE
