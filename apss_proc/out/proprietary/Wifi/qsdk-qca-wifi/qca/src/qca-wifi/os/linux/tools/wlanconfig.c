...
static void list_hw_capabilities(const char *ifname);
...
static void set_atfgroupsched(const char *ifname, char *groupname, char *val);
static void set_delatfgroup(const char *ifname, char *groupname);
static void showatfgroup(const char *ifname);
static void showatfsubgroup(const char *ifname);
...
static int atf_addac(const char *ifname, char *ssid, char *ac1, char *ac2, char *ac3, char *ac4);
static int atf_delac(const char *ifname, char *ssid, char *ac1, char *ac2, char *ac3, char *ac4);
#endif
...
#endif
static int handle_addie(const char *ifname, IEEE80211_WLANCONFIG_CMDTYPE cmdtype, char *ftype, int tot_len, char *data);
...
static int handle_nac_rssi(const char *ifname, IEEE80211_WLANCONFIG_CMDTYPE cmdtype,	int argc, char *argv[]);
#endif
int	verbose = 0;
		...
		if (argc == 6 && streq(argv[3], "add-addr")) {
			...
			return handle_hmwds(ifname, IEEE80211_WLANCONFIG_HMWDS_READ_TABLE,
					NULL, NULL);
                        ...
                        return handle_hmwds(ifname, IEEE80211_WLANCONFIG_HMWDS_SET_BRIDGE_ADDR,
                                        argv[4], NULL);
...
#define ARG_COUNT_ADD_IE 9    /* check wlanconfig help */
        ...
        if (argc == ARG_COUNT_ADD_IE) {
            if (streq(argv[3], "ftype") && streq(argv[5], "len") &&
                streq(argv[7], "data")) {
                return handle_addie (ifname, IEEE80211_WLANCONFIG_ADD_IE, argv[4], atoi(argv[6]), argv[8]);
            } else {
        ...
        if(streq(argv[3], "add") && streq(argv[4], "bssid") && streq(argv[6], "client") && streq(argv[8], "channel") && (argc == 10)){
            return handle_nac_rssi (ifname, IEEE80211_WLANCONFIG_NAC_RSSI_ADDR_ADD,
                                        argc , argv);

        } else if(streq(argv[3], "del") && streq(argv[4], "bssid") && streq(argv[6], "client")&& ( argc ==8)){
            return handle_nac_rssi (ifname, IEEE80211_WLANCONFIG_NAC_RSSI_ADDR_DEL	,
                                        argc , argv);

        } else if(streq(argv[3], "show_rssi") && (argc == 4 )){
            return handle_nac_rssi (ifname, IEEE80211_WLANCONFIG_NAC_RSSI_ADDR_LIST,
                                        argc , argv);
        } else {
            ...
            if (streq(argv[2], "addssid"))
            {
               ...
               showairtime(ifname);
                ...
                if(argc != 5)
                {
                    ...
                    set_atfgroupsched(ifname, argv[3], argv[4]);
                }
               ...
               if ((argc < 5) || (argc > 8)) {
                  ...
                  atf_addac(ifname, argv[3], argv[4], argv[5], argv[6], argv[7]);
               }
               ...
               if ((argc < 5) || (argc > 8)) {
                  ...
                  atf_delac(ifname, argv[3], argv[4], argv[5], argv[6], argv[7]);
               }
            } else if (streq(argv[2], "showatfsubgroup")) {
                fprintf(stderr,"\n\n                      SHOW   ATF    GROUP  \n");
                showatfsubgroup(ifname);
            }
        }
    }
#endif
...
usage(void)
{
	...
	fprintf(stderr, "usage: wlanconfig athX destroy\n");
        fprintf(stderr, "usage: wlanconfig athX list hwcaps\n");
#if UMAC_SUPPORT_NAWDS
...
static const char *
getxcaps(struct ieee80211req_sta_info * si)
{
        uint32_t capinfo=0;
        uint32_t capinfo2=0;
        uint8_t capinfo3=0;
...
printies(const u_int8_t *vp, int ielen, int maxcols)
{
	while (ielen > 0) {
		...
		case IEEE80211_ELEMID_VENDOR:
			...
			/* display 11N capability */
            		printie(" 11N", vp, 2+vp[1], maxcols);
            		break;
        	case IEEE80211_ELEMID_VHTCAP:
			/* display 11AC capability */
            		printie(" 11AC", vp, 2+vp[1], maxcols);
            		break;
		default:
                ...
                if( ((!strncmp(ssid,set_atp.atf_info[i].ssid, ssid_length ) && (strlen(set_atp.atf_info[i].ssid) == ssid_length)) || !strncmp("Others   ",set_atp.atf_info[i].ssid, strlen(OTHER_SSID))) || set_atp.atf_group)
                {
                    if(set_atp.atf_info[i].info_mark == 0)
                    {
                        ...
                        if( set_atp.atf_info[i].cfg_value !=0)
                            ...
                            fprintf(stderr,"\n");
                        if (set_atp.atf_info[i].atf_ac_cfg) {
                            ac_configured = 1;
                        }
                    } else {
...
static void set_atfgroupsched(const char *ifname, char *groupname, char *val)
{
    ...
    iwr.u.data.pointer = ((void *) &group_sched);
    iwr.u.data.length = sizeof(group_sched);
...
static void showatfsubgroup(const char *ifname)
{
    ...
    u_int8_t subgroup_type_str[3][5] = {"SSID", "PEER", "AC"};
    u_int8_t ac_str[8];
    u_int32_t quo_cfg = 0, rem_cfg = 0;
    u_int32_t quo_allot = 0, rem_allot = 0;
    const char *ntoa = NULL;
            ...
            for (j = 0; j < list_group.atf_list[i].num_subgroup; j++)
            {
                ...
                fprintf(stderr,"                                                    %s                            %d.%d                      %d.%d                  %s",
                    ...
                    if (list_group.atf_list[i].sg_table[j].subgrp_type == 0x1) {
                        ntoa = ieee80211_ntoa(list_group.atf_list[i].sg_table[j].peermac);
                        fprintf(stderr,"           %s",(ntoa != NULL) ? ntoa:"WRONG MAC");
...
static void showatfgroup(const char *ifname)
{
    ...
    if(list_group.info_cnt)
    {
        fprintf(stderr,"\n          Group           Airtime        Sched Policy     SSID List\n");
        for (i =0; i < list_group.info_cnt; i++)
        {
...
int del_atf_ac(const char *ifname, char *ssid, int ac_id)
{
    ...
    strlcpy(atfac_set.ac_ssid, ssid, IEEE80211_NWID_LEN);
    ...
    iwr.u.data.pointer = (void *)&atfac_set;
    iwr.u.data.length = sizeof(struct atfac_val);

    if (ioctl(s, IEEE80211_IOCTL_CONFIG_GENERIC, &iwr) < 0) {
        printf("unable to set ATF AC Configuration\n");
        close(s);
        return -1;
    }
...
int configure_atf_ac(const char *ifname, char *ssid, int ac_id, int val)
{
    struct atfac_val atfac_set;
    struct iwreq iwr;
    int s = 0;
    ...
    if (ioctl(s, IEEE80211_IOCTL_CONFIG_GENERIC, &iwr) < 0) {
        printf("ATF AC Configuration Failed for AC: %d SSID : %s\n",
                ac_id, ssid);
        close(s);
        return -1;
    } else {
...
static int atf_delac(const char *ifname, char *ssid, char *ac1, char *ac2, char *ac3, char *ac4)
{
    ...
    if(!ac1) {
        ...
        if (del_atf_ac(ifname, ssid, ac_id) < 0) {
            printf("%s ATF AC %d configuration failed \n",__func__, ac_id);
            ret = -1;
            goto fail;
        }
    }
    ...
    if (!ac2) {
        ...
        if (ac_id < 0) {
            printf("atf_fill_ac_val returned error \n");
            ret = -1;
            goto fail;
        }
        if (del_atf_ac(ifname, ssid, ac_id) < 0) {
...
static int atf_addac(const char *ifname, char *ssid, char *ac1, char *ac2, char *ac3, char *ac4)
{
    ...
    if(!ac1) {
        ...
        if (configure_atf_ac(ifname, ssid, ac_id, val) < 0) {
            ret = -1;
            goto fail;
        }
    }
...
list_stations(const char *ifname, int ps_activity)
{
	...
	cp = buf;
        if (ps_activity == 0)
	{
	        printf("%-17.17s %4s %4s %4s %4s %4s %7s %7s %4s %6s %6s %5s %5s %12s %7s %8s %14s %6s %6s %9s %6s %6s %24s %5s %5s\n"
		, "ADDR"
		, "AID"
		, "CHAN"
		, "TXRATE"
	...
	if (ps_activity == 1)
	{
		printf("%-17.17s %4s %4s %4s %4s %4s %7s %7s %4s %6s %6s %5s %5s %12s %7s %8s %14s %6s %9s %9s %6s %6s %24s %5s %5s %6s %9s\n"
		, "ADDR"
		, "AID"
		, "CHAN"
		, "TXRATE"
	...
	do {
		...
		if (ps_activity == 0)
		{
			printf(" %3d %d %d",si->isi_ps,si->isi_rx_nss,si->isi_tx_nss);
		}
		else if (ps_activity == 1)
		{
			printf ("  %d     %d     %d",si->isi_ps,si->isi_rx_nss,si->isi_tx_nss);
...
#endif
                printf (" SNR\t\t\t\t: %d\n", si->isi_rssi);
                printf(" Operating band\t\t\t: %s\n", (ieee80211_mhz2ieee(si->isi_freq) > IEEE80211_MAX_2G_CHANNEL)? "5GHz": "2.4 GHz");
		...
		if (si->isi_nr_channels != 0) {
			...
			for ( chan = si->isi_first_channel; chan < si->isi_nr_channels; chan++ ) {
				if ( chan >= 1 && chan <= 14 ) {
					if ( chan == 14 )
						freq = 2484;
					else
...
list_stations_human_format(const char *ifname)
{
	...
	do {
        ...
        if (maxrate & 0x80) maxrate &= 0x7f;
                ntoa = ieee80211_ntoa(si->isi_macaddr);
		printf("ADDR :%s AID:%4u CHAN:%4d TXRATE:%3dM RXRATE:%6dM RSSI:%4d IDLE:%4d TXSEQ:%6d RXSEQ:%7d CAPS:%5.4s XCAPS:%5.4s ACAPS:%-5.5s ERP:%3x STATE:%10x MAXRATE(DOT11):%14d HTCAPS:%14.6s VHTPCAPS:%14.6s ASSOCTIME:%02u:%02u:%02u"
			, (ntoa != NULL) ? ntoa:"WRONG MAC"
			, IEEE80211_AID(si->isi_associd)
			, ieee80211_mhz2ieee(si->isi_freq)
			, txrate
...
#define SCAN_RESULTS_BUFFER_LENGTH 0xFFFF

static void
list_scan(const char *ifname)
{
	uint8_t buf[SCAN_RESULTS_BUFFER_LENGTH];
	struct iwreq iwr;
	char ssid[14];
	uint8_t *cp;
	int len;
...
static void
print_chaninfo(const struct ieee80211_ath_channel *c, const struct ieee80211_ath_channel *c_160)
{
    char buf[50];
    char buf1[4];
...
{
    /* seg1 in ieee80211_ath_channel structure is common to both 80 Mhz and 160 Mhz,
...
void
print_hw_caps(unsigned hw_caps)
{
...
static void
list_hw_capabilities(const char *ifname)
{
        u_int32_t hwcaps;

        if (get80211param(ifname, IEEE80211_PARAM_MODE, &hwcaps, sizeof(hwcaps)) < 0)
              errx(1, "unable to get driver hw capabilities");
        print_hw_caps(hwcaps);
}
...
int
config_get_param_value(char *buf, char *pos, char *param, int param_len, char *value, int value_len)
{
    char *pos2, *pos3;
...
static int handle_addie(const char *ifname, IEEE80211_WLANCONFIG_CMDTYPE cmdtype, char *ftype, int tot_len, char *data)
{
    ...
    iwr.u.data.length = (sizeof(struct ieee80211_wlanconfig_ie) + strlen(ie_data));
...
static int handle_nac_rssi(const char *ifname, IEEE80211_WLANCONFIG_CMDTYPE cmdtype, int argc, char *argv[])
{
    ...
    if (cmdtype == IEEE80211_WLANCONFIG_NAC_RSSI_ADDR_ADD ||
        ...
        if (strlcpy(macaddr, argv[5], MACSTR_LEN) >= MACSTR_LEN) {
        ...
        memcpy(config.data.nac_rssi.mac_bssid, macaddr, IEEE80211_ADDR_LEN);

        if (strlcpy(macaddr, argv[7], MACSTR_LEN) >= MACSTR_LEN) {
            printf("Invalid MAC address (format: xx:xx:xx:xx:xx:xx), %s\n", argv[7]);
            return -1;
        }
        ...
        memcpy(config.data.nac_rssi.mac_client, macaddr, IEEE80211_ADDR_LEN);
    }
    if (cmdtype == IEEE80211_WLANCONFIG_NAC_RSSI_ADDR_ADD) {
    ...
    iwr.u.data.length = sizeof(struct ieee80211_wlanconfig);
    if (ioctl(getsocket(), IEEE80211_IOCTL_CONFIG_GENERIC, &iwr) < 0) {
        perror(" rssi_nac command failed");
        return -1;
    }
