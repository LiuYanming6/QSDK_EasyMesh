 ...
 */
static
int acfg_ev_fill_info(struct sk_buff * skb, struct net_device *dev, \
        int type, void *event, u_int32_t event_type, u_int32_t event_len, u_int32_t pid)
{
    struct ifinfomsg *r;
    struct nlmsghdr  *nlh = NULL;

    if(event_type == ACFG_NETLINK_MSG_RESPONSE){
        nlh = nlmsg_put(skb, pid, 0, type, NLMSG_LENGTH(event_len), 0);
        if (nlh == NULL)
            return -EMSGSIZE;
        memcpy(OS_NLMSG_DATA(nlh), event, event_len);
    }else if(event_type == ACFG_NETLINK_SENT_EVENT){
        nlh = nlmsg_put(skb, pid, 0, type, sizeof(*r), 0);
        if (nlh == NULL)
            return -EMSGSIZE;
        ...
        if (unlikely(nla_put(skb, IFLA_WIRELESS, event_len, event) < 0))
            goto nla_put_failure;
    }
...
static void acfg_event_work_process(void *arg)
{
    ...
    if (acfg_nl == NULL || acfg_nl->acfg_sock == NULL) {
       QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: acfg_nl (or) acfg_sock is NULL !!\n", __func__);
       return;
    }
#endif
    net = (struct net *)kmalloc(sizeof(struct net), GFP_ATOMIC);
    if (NULL == net){
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: mem alloc failed for net\n",__FUNCTION__);
        return;
    }

    nlh = (struct nlmsghdr *)kmalloc(sizeof(struct nlmsghdr), GFP_ATOMIC);
    if (NULL == nlh){
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: mem alloc failed for nlh\n",__FUNCTION__);
        kfree(net);
        return;
    }
                    ...
                    u_int32_t event_type, int send_iwevent, u_int32_t pid)
{
    ...
    if(!osdev->acfg_netlink_wq_init_done){
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Work queue not ready, drop acfg event!\n");
        return QDF_STATUS_E_FAILURE;
    }
    ...
    if(event_len > NLMSG_DEFAULT_SIZE){
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Event length bigger than NLMSG_DEFAULT_SIZE(%d)\n",__FUNCTION__,(int)NLMSG_DEFAULT_SIZE);
        kfree_skb(skb);
        return QDF_STATUS_E_FAILURE ;
    }

    err = acfg_ev_fill_info(skb, dev, RTM_NEWLINK, event, event_type, event_len, pid);
    if (err < 0) {
        kfree_skb(skb);
        return QDF_STATUS_E_FAILURE ;
    }
...
{
    if(osdev->acfg_netlink_wq_init_done){
        qdf_disable_work(NULL,&(osdev->acfg_event_os_work));
        qdf_flush_work(NULL,&(osdev->acfg_event_os_work));
...
#endif
		if (acfg_ev_sock == NULL) {
			QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: netlink create failed\n", __func__);
			return QDF_STATUS_E_FAILURE;
		}
	}
	return QDF_STATUS_SUCCESS;
...
void osif_radio_evt_radar_detected_ap(void *event_arg, struct ieee80211com *ic)
{
    ...
    nolinfo = kmalloc(sizeof(struct dfsreq_nolinfo), GFP_ATOMIC);
    if (nolinfo == NULL){
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: mem alloc failed for nolinfo\n",__FUNCTION__);
        kfree(acfg_event);
        return;
    }
