...
#endif
#define IEEE80211_AKM_SUITE_SAE 0x000fac08
    ...
    IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME, "%s: action %d\n", __func__, action);
    ...
    params.keymgmt_suite = cpu_to_be32(IEEE80211_AKM_SUITE_SAE);
                                        ...
                                        u_int32_t             ie_len)
{
...
#if DBDC_REPEATER_SUPPORT
            else if (isextender_oui((u_int8_t *)frm)) {
    ...
    if (!ieee80211_setup_rates(ni, rates, xrates,
                                 ...
                                 IEEE80211_F_DOXSECT)) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: association failed (rate set mismatch)\n", __func__);
        return -EINVAL;
    }
    ...
    if (whc_apinfo != NULL) {
        ...
        if (vap->iv_whc_root_ap_distance == WHC_INVALID_ROOT_AP_DISTANCE) {
    ...
    if ((htcap != NULL) && (ht_rates_allowed)) {
        ...
        if(!ieee80211_parse_htcap(ni, htcap)) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: association failed (Rx MCS  set mismatch)\n", __func__);
            return -EINVAL;
        }
    ...
    if ((IEEE80211_IS_CHAN_11AC(vap->iv_bsschan) || IEEE80211_IS_CHAN_11NG(vap->iv_bsschan)) &&
        ...
        if (vhtcap != NULL) {
            ...
            if (vap->iv_ext_nss_capable && ni->ni_ext_nss_support && !validate_extnss_vhtcap(&ni->ni_vhtcap)) {
                qdf_print("%s: Association failed invalid vhtcap\n",__func__);
                return -EINVAL;
            }

            if ((vhtop != NULL) && (htinfo != NULL)) {
            ...
            if (!ieee80211_setup_vht_rates(ni, vhtcap,
                ...
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: association failed (vht rate set mismatch)\n", __func__);
                return -EINVAL;
            }
    ...
    if (vap->iv_ext_nss_capable  && (htinfo != NULL)) {
        ap_htinfo = (struct ieee80211_ie_htinfo_cmn *)htinfo;
    }

    if (vap->iv_ext_nss_capable  && (vhtcap != NULL)) {
    ...
    if (vap->iv_des_mode != IEEE80211_MODE_AUTO) {
        ...
        case IEEE80211_MODE_11AC_VHT160:
            ...
            if((ap_htinfo != NULL) && (ap_vhtcap != NULL) && (ap_vhtop != NULL)) {
                if ((vap->iv_ext_nss_capable && peer_ext_nss_capable(ap_vhtcap) && extnss_80p80_validate_and_seg2_indicate(&ni->ni_vhtcap, ap_vhtop,ap_htinfo)) ||
...
bool is_subset_channel(struct ieee80211com *ic, struct ieee80211_ath_channel *curchan,
