            ...
            uint8_t *buffer, int32_t Length)
{
    ...
    data = ptr->buff;
    ptr->buff = kzalloc(ptr->len, GFP_KERNEL);
    if(!ptr->buff)
        return -ENOMEM;

    if(copy_from_user(ptr->buff, data, ptr->len))
        goto done;

    status = ieee80211_ucfg_set_encode(vap, ptr->len, ptr->flags, ptr->buff);
    ...
    while ((scn_num < ol_num_global_scn) && (scn_idx < GLOBAL_SCN_SIZE)) {
        ...
        if (!TAILQ_EMPTY(&ic->ic_vaps)) {
            TAILQ_FOREACH_SAFE(vap, &ic->ic_vaps, iv_next, tvap) {
                osifp = (osif_dev *)vap->iv_ifp;
                if (osifp && osifp->netdev && !strcmp(osifp->netdev->name, vap_name)) {
                    pvap = vap;
                    goto found;
                }
            }
    ...
    if (ic->ic_acfg_handle == NULL) {
        ...
        for (i = 0; i < num_global_scn; i++) {
            ...
            if (ic_tmp && ic_tmp->ic_acfg_handle != NULL) {
                ...
                if (acfg_nl_tmp->acfg_sock != NULL) {
                    ...
                    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: using existing sock %pK\n", __func__, acfg_nl->acfg_sock);
                    return 0;
                }
            }
        }
        ...
        while ((scn_num < ol_num_global_scn) && (scn_idx < GLOBAL_SCN_SIZE)) {
            ...
            if (ic_tmp->ic_acfg_handle != NULL) {
                ...
                if (acfg_nl_tmp->acfg_sock != NULL) {
                    acfg_nl->acfg_sock = acfg_nl_tmp->acfg_sock;
                    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Offload using existing sock %pK\n", __func__, acfg_nl->acfg_sock);
                    return 0;
                }
            }
        }
        ...
        if (acfg_nl->acfg_sock == NULL) {
            ...
            if (acfg_nl->acfg_sock == NULL) {
                ...
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO"%s: %d: Netlink socket created:%pK\n",
