...
#define QCA_PARTNER_DIRECTLINK_WDS 1
#include "ath_carr_pltfrm.h"
#undef QCA_PARTNER_DIRECTLINK_WDS
#endif
...
#ifdef BIG_ENDIAN_HOST
    wds_table = (u_int8_t *)wds_entry_dump_event->wds_entry;
    offset = (sizeof(wmi_pdev_wds_entry_dump_event) - sizeof(wmi_pdev_wds_entry))/sizeof(u_int32_t);
    for (i = offset; i < (len / sizeof(u_int32_t)); i++, wds_table += sizeof(u_int32_t))
...
#if QCA_PARTNER_DIRECTLINK
        wds_peer_cfg_add_partner(scn->pdev_txrx_handle,&peer_mac, &dest_mac);
...
#if QCA_PARTNER_DIRECTLINK
            wds_peer_cfg_remove_partner(scn->pdev_txrx_handle,peer_mac, dest_mac);
#endif /* QCA_PARTNER_DIRECTLINK */
...
wds_peer_event_handler(ol_scn_t sc, u_int8_t *datap, u_int16_t len)
{
    ...
    if(wmi_extract_wds_addr_event(scn->wmi_handle, datap, len, &wds_addr_event))
    {
        ...
        if(wds_addr_event.event_type[0] == 1) {
            ...
            ast_entry =  ol_txrx_ast_find_hash_find(scn->pdev_txrx_handle, wds_addr_event.dest_mac, 0);
            if (ast_entry) {
...
#if QCA_PARTNER_DIRECTLINK
            wds_peer_cfg_add_partner(scn->pdev_txrx_handle,(uint8_t *)wds_addr_event.peer_mac,
...
#if QCA_PARTNER_DIRECTLINK
            wds_peer_cfg_remove_partner(scn->pdev_txrx_handle,(uint8_t *)wds_addr_event.peer_mac,
                                (uint8_t *)wds_addr_event.dest_mac);
#endif /* QCA_PARTNER_DIRECTLINK */
