...
#define	PKTLOG_MOD_INC_USE_COUNT					\
	if (!try_module_get(THIS_MODULE)) {					\
		QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_WARNING "try_module_get failed\n"); \
	}
                ...
                ppos)
{
    ...
    struct ieee80211com *ic = &(((struct ath_softc_net80211 *)scn)->sc_ic);
    ...
    if (pl_dev && ((pl_dev->pl_info->log_state) & ATH_PKTLOG_TX_CAPTURE_ENABLE)) {
         ...
         * pktlog flags.
         */
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Cannot enable tx capture : Check the flag value 0x%x\n", (pl_dev->pl_info->log_state));
        return -1;
    }
    ...
    if (write) {
        ret = ATH_SYSCTL_PROC_DOINTVEC(ctl, write, filp, buffer,
                                       lenp, ppos);
        if (ret == 0) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Enable in %s:%d\n", __FUNCTION__, enable);
            if (pl_dev && (((pl_dev->pl_info->log_state) & ATH_PKTLOG_TX_CAPTURE_ENABLE) == 0)) {
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "pl_dev:%pK; pl_funcs:%pK; pktlog_enable:%pK; scn:%pK\n",
...
#if OL_ATH_SMART_LOGGING
                if (!enable && ic->ic_smart_log_fw_pktlog_unblock) {
                ...
                return ret;
                 ...
                 QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Cannot enable pktlog as Tx Capture has been enabled\n");
            }
        } else {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, PKTLOG_TAG "%s:proc_dointvec failed\n", __FUNCTION__);
        }
    } else {
        ret = ATH_SYSCTL_PROC_DOINTVEC(ctl, write, filp, buffer,
                                       lenp, ppos);
...
#	define set_ctl_name(a, b)	pl_info_lnx->sysctls[a].ctl_name = b
    ...
    pl_info_lnx->sysctls[13].extra1 = scn;
    ...
    pl_info_lnx->sysctls[14].extra1 = scn;
    ...
    if (!pl_info_lnx->sysctl_header) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: failed to register sysctls!\n", proc_name);
        return -1;
    }
...
pktlogmod_init(void)
{
    ...
    if (g_pktlog_pde == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, PKTLOG_TAG "%s: proc_mkdir failed\n", __FUNCTION__);
        return -1;
    }
    ...
    for (i = 0; i < MAX_WLANDEV; i++) {
        ...
        if ((dev = dev_get_by_name(dev_name)) != NULL) {
            ...
            if (!pl_dev || pktlog_attach(get_scn(dev)))
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, PKTLOG_TAG "%s: pktlog_attach failed for %s\n",
            ...
            if(pl_dev && pl_dev->tgt_pktlog_enabled) {
                if(pl_dev->pl_funcs) {
                   if(pl_dev->pl_funcs->pktlog_enable(scn, 0)) {
                        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Unable to Disable pktlog in the target\n");
                    }
                    /* detach for offload */
...
pktlog_attach(ath_generic_softc_handle scn)
{
    ...
    if (pl_dev != NULL) {
        ...
        if (pl_info_lnx == NULL) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, PKTLOG_TAG "%s:allocation failed for pl_info\n"
                                                    , __FUNCTION__);
            return -ENOMEM;
        }
    ...
    proc_entry = proc_create_data(proc_name, PKTLOG_PROC_PERM, g_pktlog_pde, &pktlog_fops, &pl_info_lnx->info);
#else
    proc_entry = create_proc_entry(proc_name, PKTLOG_PROC_PERM, g_pktlog_pde);
#endif
    if (proc_entry == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, PKTLOG_TAG "%s: create_proc_entry failed for %s\n",
    ...
    if (pktlog_sysctl_register(scn)) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, PKTLOG_TAG "%s: sysctl register failed for %s\n",
               __FUNCTION__, proc_name);
        goto attach_fail2;
    }
    return 0;
...
__pktlog_read(struct file *file, char *buf, size_t nbytes, loff_t *ppos)
{
    ...
    if (!pktlog_hdr_size) {
        ...
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: WARNING: pl_info->pktlog_hdr_size = %u, Using pktlog_hdr_size as: %zu\n",
...
int pktlog_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
	...
	if (address == 0UL) {
        	QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, PKTLOG_TAG "%s: page fault out of range\n", __FUNCTION__);
...
static int pktlog_mmap(struct file *file, struct vm_area_struct *vma)
{
    ...
    if (!pl_info->buf) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, PKTLOG_TAG "%s: Log buffer unavailable\n", __FUNCTION__);
        return -ENOMEM;
    }
