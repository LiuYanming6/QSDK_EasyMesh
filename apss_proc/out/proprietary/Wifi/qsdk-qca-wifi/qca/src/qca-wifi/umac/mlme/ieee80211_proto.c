...
static void ieee80211_vap_scan_event_handler(struct ieee80211vap *originator, ieee80211_scan_event *event, void *arg)
{
    ...
    if(wlan_get_param(vap, IEEE80211_START_ACS_REPORT)) {
        IEEE80211_STATE_LOCK(vap->iv_ic);
        vap_state = vap->iv_state_info.iv_state;
        IEEE80211_STATE_UNLOCK(vap->iv_ic);
        if (vap_state != IEEE80211_S_SCAN) {
            return;
        }
    }
...
ieee80211_state_event(struct ieee80211vap *vap, enum ieee80211_state_event event)
{
    ...
    */
    ...
    switch(event) {
        ...
        case IEEE80211_STATE_EVENT_RESTART:
            restart = 1;
            break;
        default:
            break;
    ...
    case IEEE80211_S_INIT:
	   ...
	   case IEEE80211_STATE_EVENT_CHAN_SET:
              ...
              IEEE80211_STATE_UNLOCK(ic);
               return 0;
               break;
           case IEEE80211_STATE_EVENT_START:
           case IEEE80211_STATE_EVENT_RESTART:  /* notice fallthrough */
               chan = ieee80211_derive_chan(vap);
               if (chan == NULL) {
                   IEEE80211_DPRINTF(vap, IEEE80211_MSG_STATE,
                                     "Failed to derive Channel\n");
                   vap->iv_state_info.iv_sm_running = false;
                   IEEE80211_STATE_UNLOCK(ic);
                   ASSERT(0);
                   return EINVAL;
               }
               nstate = IEEE80211_S_STARTING;
               break;
           default:
               break;
        }
        break;
    case IEEE80211_S_STARTING:
        switch(event) {
           case IEEE80211_STATE_EVENT_JOIN:
               nstate = IEEE80211_S_JOIN;
               break;
           ...
           case IEEE80211_STATE_EVENT_DOWN:
               nstate = IEEE80211_S_ABORTING;
               break;
           default:
               break;
        }
        break;
    case IEEE80211_S_ABORTING:
        switch(event) {
        case IEEE80211_STATE_EVENT_JOIN:
            vap->iv_state_info.iv_sm_running = false;
            IEEE80211_STATE_UNLOCK(ic);
    ...
    switch(nstate) {
    case IEEE80211_S_STARTING:
        error = ieee80211_resmgr_vap_start(ic->ic_resmgr,vap,chan,0,0,restart);
        if (error == EOK) {
            /* set iv_bsschan here in case of DA */
            vap->iv_bsschan = chan;
        }
        break;
    ...
    case IEEE80211_S_INIT:
     ...
     case IEEE80211_S_RUN:
         ...
         if(vap->iv_dbeacon) {
             ...
             /* Do not suspend beacon for DFS channels  or hiddedn ssid not enabled */
             if (IEEE80211_IS_CHAN_DFS(ic->ic_curchan) || !IEEE80211_VAP_IS_HIDESSID_ENABLED(vap)) {
    ...
    case IEEE80211_S_DFS_WAIT:
        ...
        break;
    case IEEE80211_S_ABORTING:
        /* return an error to mlme, so that reset_bss is skipped */
        error = EBUSY;
        break;
    default:
        break;
...
{
    if (force) {
        return ieee80211_state_event(vap, IEEE80211_STATE_EVENT_FORCE_STOP);
    } else {
        return ieee80211_state_event(vap, IEEE80211_STATE_EVENT_DOWN);
    }
}
