...
enum ieee80211_state {
    ...
    IEEE80211_S_STARTING,       /* state while waiting for start response from FW*/
    IEEE80211_S_ABORTING,       /* Abort Start due to Stop event in STOPPING State */
...
enum ieee80211_state_event {
    ...
    IEEE80211_STATE_EVENT_START      ,
    IEEE80211_STATE_EVENT_RESTART      ,
};
...
int  ieee80211_vap_stop(struct ieee80211vap *vap, bool force);
...
wbuf_t ieee80211_fils_discovery_alloc(struct ieee80211_node *ni);
int ieee80211_fils_discovery_update(struct ieee80211vap *vap, wbuf_t wbuf);
#endif
...
/*
 * Add MultiAP IE in Assoc Res
 * @param [in] frm  the place in the frame to which to start writing the IE
 * @param [in] Pointer to length which gets total IE length
 *
 */
u_int8_t *ieee80211_add_multi_ap_ie(u_int8_t *frm, struct ieee80211vap *vap, u_int16_t *ie_len);
...
INLINE static int
isextender_oui(u_int8_t *frm)
{
    return ((frm[1] > 4) && (LE_READ_4(frm+2) == ((QCA_OUI_EXTENDER_TYPE<<24)|QCA_OUI)));
