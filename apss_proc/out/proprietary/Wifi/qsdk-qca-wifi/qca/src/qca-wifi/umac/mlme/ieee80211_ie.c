...
/*
 * Add Multi-AP IE format
 */

u_int8_t *
ieee80211_add_multi_ap_ie(u_int8_t *frm, struct ieee80211vap *vap, u_int16_t* ie_len)
{
    u_int8_t attribute = 0;
    u_int8_t attributelen = 0;
    u_int8_t attriburevalue = 0;

    static const u_int8_t oui[4] = {
        (WFA_OUI_IE & 0xff), ((WFA_OUI_IE >> 8) & 0xff), ((WFA_OUI_IE >> 16) & 0xff),
        WFA_OUI_MAP_TYPE_IE
    };
    u_int8_t *plen = NULL;
    *frm++ = IEEE80211_ELEMID_VENDOR;
    ...
    *plen += 3;
    ...
    if (ieee80211_vap_mapbsta_is_set(vap))
        attriburevalue = MAP_BACKHAUL_STA;

    if(ieee80211_vap_mapfh_is_set(vap))
       attriburevalue |= MAP_FRONTHAUL_BSS;

    if (ieee80211_vap_mapbh_is_set(vap))
        attriburevalue |= MAP_BACKHAUL_BSS;
    ...
    *plen += 22;
    if (vap->iv_whc_root_ap_distance == 0) {
        ...
        *frm++ = 1;
        ...
        /* ROOTAP indicator */
        *frm++ = 0;
        uplink_rate = ieee80211_rep_datarate_estimator(vap->iv_ic->ic_serving_ap_backhaul_rate,
                                                       vap->iv_ic->ic_uplink_rate,
                                                       vap->iv_whc_root_ap_distance - 1,
                                                       vap->iv_ic->ic_whc_scaling_factor);
    }
    /* # of connected REs */
                ...
                if (!(ni->ni_htcap & IEEE80211_HTCAP_C_CHWIDTH40)) {
                    ...
                    if(vap->iv_cur_mode == IEEE80211_MODE_11AC_VHT160) {
                        if (vap->iv_ext_nss_support) {
                            if (peer_ext_nss_capable(vhtcap) &&
                        ...
                        if (vap->iv_ext_nss_support) {
                            if (peer_ext_nss_capable(vhtcap) && ext_nss_80p80_supported(&ni->ni_vhtcap)){
                                ni->ni_chwidth = IEEE80211_CWM_WIDTH160;
                            } else {
                                ni->ni_chwidth = IEEE80211_CWM_WIDTH80;
                            }
...
#define RSN_SEL(x)  (((x)<<24)|RSN_OUI)
    ...
    case RSN_SEL(AKM_SUITE_TYPE_OWE):
    ...
    if (chan && (pcsaIe || pecsaIe)) {
            ...
            (chan != vap->iv_bsschan))
        {
            ...
            if (err == EOK) {
                    ...
                    ieee80211_process_csa(ni, chan, pcsaIe, pecsaIe);
            } else if (err == EBUSY) {
                    err = EOK;
            }
        }
                    ...
                    struct ieee80211com *ic,  u_int8_t subtype, u_int8_t is_subelement)
{
    ...
    /* Most architectures should use 2s complement, but we utilize an
...
void ieee80211_process_supp_chan_ie(struct ieee80211_node *ni, u_int8_t *ie)
{
    ...
    if (supp_chan->supp_chan_len != 2) {
        IEEE80211_DISCARD_IE(ni->ni_vap,
            IEEE80211_MSG_ELEMID,
            "802.11h channel supported IE", "invalid len %u",
           supp_chan->supp_chan_len);
        return;
    }
...
ieee80211_process_extender_ie(struct ieee80211_node *ni, const u_int8_t *ie, ieee80211_frame_type ftype)
{
	...
	if (ic_list->num_stavaps_up == 0) {
            apvaps_cnt = *ie++;
            stavaps_cnt = *ie++;
            mac_list = (u_int8_t *)ic_list->preferred_mac_list;
            for (i=0; ((i < apvaps_cnt)&&(ie_len > 0)); i++) {
                GLOBAL_IC_LOCK(ic_list);
		IEEE80211_ADDR_COPY((mac_list+(i*IEEE80211_ADDR_LEN)), ie);
                GLOBAL_IC_UNLOCK(ic_list);
		QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Preferred mac[%d]:%s\n",i,ether_sprintf(mac_list+(i*IEEE80211_ADDR_LEN)));
		ie += IEEE80211_ADDR_LEN;
		ie_len -= IEEE80211_ADDR_LEN;
	    }
            mac_list = (u_int8_t *)ic_list->denied_mac_list;
            for (i=0; ((i < stavaps_cnt)&&(ie_len > 0)); i++) {
                GLOBAL_IC_LOCK(ic_list);
		IEEE80211_ADDR_COPY((mac_list+(i*IEEE80211_ADDR_LEN)), ie);
                GLOBAL_IC_UNLOCK(ic_list);
		QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Denied mac[%d]:%s\n",i,ether_sprintf(mac_list+(i*IEEE80211_ADDR_LEN)));
		ie += IEEE80211_ADDR_LEN;
		ie_len -= IEEE80211_ADDR_LEN;
	    }
