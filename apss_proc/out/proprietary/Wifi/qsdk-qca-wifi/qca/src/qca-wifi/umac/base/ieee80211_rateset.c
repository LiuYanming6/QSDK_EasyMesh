 ...
 */
bool
ieee80211_derive_nss_from_cap(struct ieee80211_node *ni)
{
 ...
 */
	...
	case IEEE80211_EXTNSS_MAP_23_80F2_160F1_80P80F1:
            ni->ni_rxstreams *= 2;
            ni->ni_bw160_nss = (ni->ni_rxstreams / 2);
            ni->ni_bw80p80_nss = (ni->ni_rxstreams / 2);
            break;

        case IEEE80211_EXTNSS_MAP_13_80F2_160F2_80P80F1:
            ni->ni_rxstreams *= 2;
            ni->ni_bw160_nss = (ni->ni_rxstreams);
            ni->ni_bw80p80_nss = (ni->ni_rxstreams / 2);
            break;

        case IEEE80211_EXTNSS_MAP_03_80F1_160FDOT75_80P80FDOT75:
            ni->ni_bw160_nss = ((ni->ni_rxstreams * 3) / 4);
            ni->ni_bw80p80_nss = ((ni->ni_rxstreams * 3) / 4);
            break;
	...
	case IEEE80211_EXTNSS_MAP_01_80F1_160FDOT5_80P80NONE:
        ...
        case IEEE80211_EXTNSS_MAP_20_80F1_160F1_80P80F1:
            ni->ni_bw160_nss = ni->ni_rxstreams;
            ni->ni_bw80p80_nss = ni->ni_rxstreams;
            break;
                         ...
                         int flags)
{
    ...
    /* Check to ensure that max peer txstreams is taken if the HTCapabilities and VTH Capabilities have populated different rate values */
...
ieee80211_disable_legacy_rates(struct ieee80211vap *vap)
{
        ...
        if (!is_mgt_rate_set) {
            for (k = 0; k < basicpureg[0].rs_nrates; k++) {
                if ((ni->ni_rates.rs_rates[i] & IEEE80211_RATE_VAL) == basicpureg[0].rs_rates[k]) {
                    vap->iv_mgt_rate = ((basicpureg[0].rs_rates[k] * 1000) / 2);
                    is_mgt_rate_set = 1;
                    break;
                }
            }
    ...
    if (vap->iv_bcn_rate) {
        for (i = 0; i < ni->ni_rates.rs_nrates; i++) {
            ...
            if (vap->iv_bcn_rate == rate_in_kbps) {
                found = 1;
                break;
            }
        }
        ...
        if(!(vap->iv_ic->ic_is_mode_offload(vap->iv_ic))){
            if (found) {
                ...
                vap->iv_bcn_rate = vap->iv_mgt_rate;
                value = vap->iv_modify_bcn_rate(vap);

                qdf_print("%s:MSG: User has disabled the previously set modified rate for beacon.\n",__func__);
                qdf_print("%s:MSG: Hence going for the lowest available basic rate for beacon: %d (Kbps)\n",__func__,vap->iv_bcn_rate);
            }
