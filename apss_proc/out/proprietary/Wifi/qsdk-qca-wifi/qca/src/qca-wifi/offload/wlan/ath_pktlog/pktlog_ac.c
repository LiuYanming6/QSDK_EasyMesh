/*
 * Copyright (c) 2012, 2015, 2017 Qualcomm Innovation Center, Inc.
 * All Rights Reserved.
    ...
    if(pl_info) {
        ...
        if (pl_dev) {
            ...
            if (scn->is_ar900b) {
                ...
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " %s: Initializing Pktlog for AR900B, pktlog_hdr_size = %zd\n", __func__, pl_dev->pktlog_hdr_size);
                ...
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " %s: Initializing Pktlog for AR9888, pktlog_hdr_size = %zd\n", __func__, pl_dev->pktlog_hdr_size);
            } else {
