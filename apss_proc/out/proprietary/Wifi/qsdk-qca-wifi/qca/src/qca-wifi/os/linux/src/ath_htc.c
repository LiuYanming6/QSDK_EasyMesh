...
{   
    ...
    switch(evt_id) {
        ...
        case WMI_BMISS_EVENTID:
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "WMI_BMISS_EVENTID\n");
        ...
        case WMI_WLAN_TXCOMP_EVENTID:
            ...
            if(ps_null_addr) {
                ...
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_DEBUG "WMI_WLAN_TXCOMP_EVENTID: %02x %02x %02x %02x %02x %02x\n",
                ...
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "##dbg [%s] fail to allocate buffer, line %d\n",__func__,__LINE__);
            }
            break;
        ...
        case WMI_ACTION_TXFAIL_EVENTID:
            ...
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "WMI_ACTION_TXFAIL_EVENTID\n");
...
host_htc_rdy_handler(osdev_t osdev, u_int32_t TotalCredits)
{
    ...
    if (HTCWaitForHtcRdy(osdev->host_htc_handle) != A_OK) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "HTCWaitForHtcRdy failed, init failed!\n");
        return A_ERROR;
    }
    ...
    if (A_FAILED(status)) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "wmi connect failed!");
        return A_ERROR;
    }
    ...
    if (A_FAILED(status)) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Failed to connect services!");
        return A_ERROR;
    }
    ...
    if (A_FAILED(status)) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Failed to start HTC!");
        return A_ERROR;
    } 
    return A_OK;
...
//    osdev->host_wmi_handle = wmi_init(osdev->bdev, osdev, &dispatch_magpie_sys_evts);
    ...
    if (A_FAILED(status)) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : host_htc_rdy_handler() call failed!", __func__);
    }
...
{
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: MpWmiStop\n", __func__);
    MpWmiStop(osdev);
}
...
void MpHtcAddNodeTarget(osdev_t osdev)
{
    ...
    TAILQ_FOREACH_SAFE(ni, &_nt->nt_node, ni_list, next) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO "%s: %02x %02x %02x %02x %02x %02x\n", __func__,
        ...
        if (i == HTC_MAX_NODE_NUM) {
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: can't find node in the node_bitmap\n", __func__);
            continue;
        }
