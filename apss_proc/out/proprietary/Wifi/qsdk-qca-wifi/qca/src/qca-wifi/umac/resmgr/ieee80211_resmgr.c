...
static void _ieee80211_resmgr_create_complete(ieee80211_resmgr_t resmgr)
{
    ...
    ASSERT(rc == EOK);
    if (rc != EOK) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : ieee80211_scan_register_event_handler() failed rc=%08X\n",
            __func__, rc);
    }
}
...
static void _ieee80211_resmgr_delete_prepare(ieee80211_resmgr_t resmgr)
{
    ...
    if (rc != EOK) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: wlan_scan_unregister_event_handler() failed handler=%pK,%pK rc=0x%x\n",
...
ieee80211_resmgr_t _ieee80211_resmgr_create(struct ieee80211com *ic, ieee80211_resmgr_mode mode)
{
    ...
    if (ic->ic_tsf_timer == NULL) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Multi-Channel Scheduler unavailable since no TSF Timer.\n", __func__);
    }
    else {
    ...
    if (ieee80211_resmgr_sm_create(resmgr) != EOK) {
        OS_FREE(resmgr);
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : ieee80211_resmgr_sm_create failed \n", __func__);
        return NULL;
    }
    ...
    if (ic->ic_caps_ext & IEEE80211_CEXT_MULTICHAN) {
        if (ieee80211_resmgr_oc_scheduler_create(resmgr, OFF_CHAN_SCHED_POLICY_ROUND_ROBIN) != EOK) {
            OS_FREE(resmgr);
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : ieee80211_resmgr_oc_scheduler_create failed \n", __func__);
            return NULL;
        }
        ...
        /* Do not support P2P */
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Error: %s: no Multi-chan hw caps. Disabling Off Channel scheduler.\n", __func__);
    }
