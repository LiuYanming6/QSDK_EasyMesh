/*
 * Copyright (c) 2013-2016, 2017-2019 Qualcomm Innovation Center, Inc.
 * All Rights Reserved.
...
unsigned int carrier_vow_config = 0;
module_param(carrier_vow_config, uint, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
MODULE_PARM_DESC(carrier_vow_config,
        "VoW Configuration");
EXPORT_SYMBOL(carrier_vow_config);

unsigned int fw_vow_stats_enable = 0;
module_param(fw_vow_stats_enable, uint, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
MODULE_PARM_DESC(fw_vow_stats_enable,
        "Firmware VoW stats control");
EXPORT_SYMBOL(fw_vow_stats_enable);
...
unsigned int wlanstats_config = 0;
module_param(wlanstats_config, uint, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
MODULE_PARM_DESC(wlanstats_config,
        "Wlanstats configuration");
EXPORT_SYMBOL(wlanstats_config);
#endif
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
...
extern int32_t __ol_ath_thermal_mitigation_attach (struct ol_ath_softc_net80211 *scn);
extern int32_t __ol_ath_thermal_mitigation_detach (struct ol_ath_softc_net80211 *scn);
extern int32_t ol_ath_ioctl_set_thermal_handler (struct ol_ath_softc_net80211 *scn, caddr_t *param);
extern int32_t ol_ath_ioctl_get_thermal_handler (struct ol_ath_softc_net80211 *scn, caddr_t *param);
...
int ol_ath_verify_vow_config(struct ol_ath_softc_net80211 *scn)
{
    ...
    if(vow_config && (scn->target_type != TARGET_TYPE_AR9888)) {
	    scn->vow_config = 0;
	    qdf_print("vow_config not supported on this platform\n");
	    return 0;
    }
...
ol_swap_seg_alloc (struct ol_ath_softc_net80211 *scn, struct swap_seg_info **ret_seg_info, u_int64_t **scn_cpuaddr, const char* filename, int type)
{
    ...
    qdf_dev_alloc_mem(scn->sc_osdev->device,(void **)&seg_info, sizeof(*seg_info),
                                               GFP_KERNEL);
    if (!seg_info) {
...
ol_swap_wlan_memory_expansion(struct ol_ath_softc_net80211 *scn, struct swap_seg_info *seg_info,const char*  filename, u_int32_t *target_addr)
{
    ...
    */
    while ((size_left && fw_temp) && (dma_size_left > 0)) {
        ...
        length = *(int *)fw_temp;
#endif
    qdf_print("%s: length:%d size_left:%d dma_size_left:%d fw_temp:%pK fw_entry_size:%zd\n",
...
#else
        qdf_print("%s: dma_virt_addr :%pK fw_temp: %pK length: %d\n",
...
__ol_ath_attach(void *hif_hdl, struct ol_attach_t *ol_cfg, osdev_t osdev, qdf_device_t qdf_dev)
{
...
#ifdef ATH_SUPPORT_RDKB
    ic->ic_wlanstats_config = wlanstats_config;
#endif
    /*
    ...
    scn->carrier_vow_config = carrier_vow_config;
    scn->fw_vow_stats_enable = fw_vow_stats_enable;
...
#endif
    /* TODO: Need to enable Nelink if this feature is eabled through UCI/module param?
     * Need to add init/delete in DA path also */
#if ATH_SUPPORT_CFR
    cfr_netlink_init();
#endif
...
ol_transfer_target_eeprom_caldata(struct ol_ath_softc_net80211 *scn, u_int32_t address, bool compressed)
{
    ...
    if ( scn->target_version != AR9887_REV1_VERSION) {
        ...
        return -EINVAL;
            ...
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s %d: Download Target EEPROM caldata len %zd\n",
                    ...
                    u_int32_t address, bool compressed)
{
    ...
    if(file != ATH_FLASH_FILE) {
        ...
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO"%s %d: downloading file %d, Download data len %zd\n", __func__, __LINE__, file, fw_entry->size);
    }
...
#ifdef AH_CAL_IN_FILE_HOST
	{
        ...
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s %d: Download Flash data len %zd\n", __func__, __LINE__, fw_entry_size);
    }
    ...
    if ((file == ATH_BOARD_DATA_FILE) || (file == ATH_FLASH_FILE))
    {
        ...
        if ((board_ext_address) && (fw_entry_size == (board_data_size + board_ext_data_size)))
        {
            ...
            status = BMIWriteMemory(scn->hif_hdl, board_ext_address,
                    (u_int8_t *)(((A_UINT32)(uintptr_t)tempEeprom) + board_data_size), board_ext_data_size, scn);
...
#endif
{
    ...
    nbytes = min((uint32_t)count, scn->last_rxlen);
...
#endif
    ret = qdf_create_dev_file(&scn->sc_osdev->device->kobj, BMI_fsattr);
    if (ret) {
...
ol_ath_wait_for_bmi_user_agent(struct ol_ath_softc_net80211 *scn)
{
    ...
    qdf_vfs_delete_binfile(&scn->sc_osdev->device->kobj, BMI_fsattr);
        ...
        struct device *dev, int dev_id)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s Total Firmware file allocated %zd\n", __func__, (*fw_entry)->size);
