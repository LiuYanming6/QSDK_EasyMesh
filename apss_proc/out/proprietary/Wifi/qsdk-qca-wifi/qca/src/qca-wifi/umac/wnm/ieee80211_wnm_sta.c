                                ...
                                u_int8_t *frm, int frm_len)
{
    ...
    event = (wnm_netlink_event_t *)qdf_mem_alloc_outline(NULL, sizeof(wnm_netlink_event_t));
    if (event == NULL) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d]: Memory allocation failed\n", __func__, __LINE__);
        return -ENOMEM;
    }

    fmsrsp_msg = (struct ieee80211_wnm_fmsresp *)qdf_mem_alloc_outline(NULL, sizeof(struct ieee80211_wnm_fmsresp));
    if (fmsrsp_msg == NULL) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d]: Memory allocation failed\n", __func__, __LINE__);
        qdf_mem_free(event);
        return -ENOMEM;
            ...
            if (IEEE80211_FMS_STATUS_SUBELE == status_subele->subelementid)
            {
                ...
                if (tclass_len > 0) {
                    ...
                    if (rv != 0) {
                        ...
                        OS_FREE(&fms_rsp);
                        ieee80211_wnm_free_fmsrsp(&fmsrsp_head);
                        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:%d ieee80211_tclass_element_parse failed %d\n",
                                ...
                                u_int8_t *frm, int frm_len)
{
    ...
    tfsrsp = (struct ieee80211_wnm_tfsresp *)qdf_mem_alloc_outline(NULL, sizeof(struct ieee80211_wnm_tfsresp));
    if (tfsrsp == NULL) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d]: Memory allocation failed\n", __func__, __LINE__);
        qdf_mem_free(event);
        return -ENOMEM;
