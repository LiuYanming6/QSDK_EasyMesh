...
static void wlan_acs_exttool_config(wlan_if_t vap,
        wifi_scan_custom_request_cmd_t *acs_config, int set)
{
    ...
    if (wlan_acs_start_scan_report(vap, set, IEEE80211_MIN_DWELL_ACS_REPORT,
            (void *)&acs_config->min_dwell_time) != EOK) {
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_NORMAL,
            IEEE80211_MSG_EXTIOCTL_CHANSSCAN,
            "%s Failed to %s min dwell time", __func__, (set ? "set" : "get"));
    }

    /* Setting the user configured max dwell time */
    if (wlan_acs_start_scan_report(vap, set, IEEE80211_MAX_DWELL_ACS_REPORT,
           (void *)&acs_config->max_dwell_time) != EOK) {
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_NORMAL,
            IEEE80211_MSG_EXTIOCTL_CHANSSCAN,
            "%s Failed to %s max dwell time", __func__, (set ? "set" : "get"));
    }

    /* Setting the user configured rest time */
    if (wlan_acs_start_scan_report(vap, set, IEEE80211_SCAN_REST_TIME,
           (void *)&acs_config->rest_time) != EOK) {
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_NORMAL,
            IEEE80211_MSG_EXTIOCTL_CHANSSCAN,
            "%s Failed to %s rest time", __func__, (set ? "set" : "get"));
    }
#if QCA_LTEU_SUPPORT
    if (wlan_acs_start_scan_report(vap, set, IEEE80211_SCAN_IDLE_TIME,
           (void *)&acs_config->idle_time) != EOK) {
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_NORMAL,
            IEEE80211_MSG_EXTIOCTL_CHANSSCAN,
            "%s Failed to %s idle time", __func__, (set ? "set" : "get"));
    }
#endif
    if (wlan_acs_start_scan_report(vap, set, IEEE80211_MAX_SCAN_TIME_ACS_REPORT,
           (void *)&acs_config->max_scan_time) != EOK) {
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_NORMAL,
            IEEE80211_MSG_EXTIOCTL_CHANSSCAN,
            "%s Failed to %s max scan time", __func__, (set ? "set" : "get"));
    }

    /* Setting the user configured scan mode */
    if (wlan_acs_start_scan_report(vap, set, IEEE80211_SCAN_MODE,
           (void *)&acs_config->scan_mode) != EOK) {
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_NORMAL,
            IEEE80211_MSG_EXTIOCTL_CHANSSCAN,
            "%s Failed to %s scan_mode", __func__, (set ? "set" : "get"));
    }
...
int ieee80211_extended_ioctl_chan_scan (struct net_device *dev,
        ...
        caddr_t *param)
{
    wifi_scan_custom_request_cmd_t request = {0};
    wifi_scan_custom_request_cmd_t old_config = {0};
    ...
    if (error) {
        ...
        if (wlan_acs_start_scan_report(vap, 1, IEEE80211_START_ACS_REPORT, (void *)&val) != EOK) {
            ...
            /* Do not return from here proceed to restore scan params */
            error = EINVAL;
        }
