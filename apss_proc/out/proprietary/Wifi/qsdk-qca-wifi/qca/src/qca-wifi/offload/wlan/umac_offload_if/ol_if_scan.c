...
#include "qdf_mem.h"
#define IEEE80211_APPIE_MAX    1024    /* max appie buffer size taken as defined */
...
struct ieee80211_scanner {
    ...
    struct ieee80211_ath_channel            *ss_all_chans[IEEE80211_CHAN_MAX];
                    ...
                    IEEE80211_SCAN_ID        scan_id)
{
    ...
    /** Channel scan offset time*/
    scan_param.scan_offset_time = params->scan_offset_time;
...
#define FREQUENCY_THRESH        1000
    ...
    if (params->num_bssid && params->num_bssid <= IEEE80211_SCAN_PARAMS_MAX_BSSID) {
                    ...
                    IEEE80211_SCAN_ID        *scan_id)
{
    ...
    u_int8_t *ie_data;                    /* ie_data received from upper layer*/
    u_int8_t *org_f_app_ie;               /* start of f_app_ie buffer */
    u_int8_t *f_app_ie;                   /* f_app_ie buffer  */
    int f_app_ie_len = 0;                 /* f_app_ie buffer length */
    int ret;
    ...
    *scan_id =  *scan_id | WMI_HOST_P_SCAN_REQ_ID_PREFIX ;
    ...
    f_app_ie = (u_int8_t *) qdf_mem_malloc(params->ie_len + IEEE80211_APPIE_MAX);
    if (!f_app_ie) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : App_ie memory alloction failed\n", __func__);
        return ENOMEM;
    }

    org_f_app_ie = f_app_ie;
    ie_data = params->ie_data;
    OS_MEMZERO(f_app_ie, params->ie_len + IEEE80211_APPIE_MAX);

    if(params->ie_len > 0) {
        OS_MEMCPY(f_app_ie, params->ie_data, params->ie_len);
        f_app_ie += params->ie_len;
        f_app_ie_len += params->ie_len;
    }

    if (vap->iv_app_ie[IEEE80211_FRAME_TYPE_PROBEREQ].length) {
        OS_MEMCPY(f_app_ie, vap->iv_app_ie[IEEE80211_FRAME_TYPE_PROBEREQ].ie,
                  vap->iv_app_ie[IEEE80211_FRAME_TYPE_PROBEREQ].length);
        f_app_ie += vap->iv_app_ie[IEEE80211_FRAME_TYPE_PROBEREQ].length;
        f_app_ie_len += vap->iv_app_ie[IEEE80211_FRAME_TYPE_PROBEREQ].length;
    }
    /* Add the Application IE's */
    f_app_ie = ieee80211_mlme_app_ie_append(vap, IEEE80211_FRAME_TYPE_PROBEREQ, f_app_ie);
    f_app_ie_len += vap->iv_app_ie_list[IEEE80211_FRAME_TYPE_PROBEREQ].total_ie_len;

    params->ie_data = org_f_app_ie;
    params->ie_len = f_app_ie_len;
    ...
    ret = ol_ath_scan_start_send(scn, avn->av_if_id, params, add_cck_rates, requestor, priority, *scan_id);

    qdf_mem_free(org_f_app_ie);
    params->ie_data = ie_data;
...
#if ATH_SUPPORT_ZERO_CAC_DFS
        if (ic->ic_precac_enable && ic->ic_precac_secondary_freq) {
