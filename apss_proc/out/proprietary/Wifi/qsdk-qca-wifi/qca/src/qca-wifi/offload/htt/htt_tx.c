...
#include <osdep_adf_internal.h>          /* u_int32_t, offsetof, etc. */
...
#define QCA_PARTNER_DIRECTLINK_HTT_TX 1
#include "ath_carr_pltfrm.h"
#undef QCA_PARTNER_DIRECTLINK_HTT_TX
#endif
    ...
    if(pdev && pdev->txrx_pdev) {
        ...
        /* If the trylock fails, it means that some other thread is
           ...
           Besides, it reduces the overhead since we are in IRQ context */
            if (qdf_spin_trylock(&pdev->txrx_pdev->peermap_lock)) {
                ...
                qdf_spin_unlock(&pdev->txrx_pdev->peermap_lock);
            }
        }
...
htt_peer_map_timer_init(struct ol_txrx_pdev_t *pdev)
{
    ...
    qdf_print("%s Enter pdev %pK hrtimer %pK\n",__FUNCTION__, pdev, &pdev->qmap_ctxt);
...
htt_alloc_peer_map_mem(struct htt_pdev_t *pdev, int desc_pool_elems)
{
...
#if QCA_PARTNER_DIRECTLINK
    if (ath_is_directlink((struct ol_txrx_pdev_t*)pdev->txrx_pdev)) {
        pdev->host_q_mem.pool_vaddr = ol_get_peer_map_base_partner();
    } else
#endif /* QCA_PARTNER_DIRECTLINK */
    ...
    qdf_spinlock_create(&pdev->txrx_pdev->peermap_lock);
...
#endif
        qdf_spinlock_destroy(&pdev->txrx_pdev->peermap_lock);
    /* Peer map cancel the timer and unmap the shared memory between host/target */
#if QCA_PARTNER_DIRECTLINK
    if (ath_is_directlink((struct ol_txrx_pdev_t*)pdev->txrx_pdev)) {
        /* peermap mem free will be done by Directlink*/
        return;
    }
#endif /* QCA_PARTNER_DIRECTLINK */
...
htt_tx_mgmt_desc_free(struct htt_pdev_t *pdev, A_UINT8 desc_id, A_UINT32 status, A_UINT16 ppdu_id)
{
    ...
    if (delayed_free == HTT_TX_MGMT_DELAYED_FREE_PENDING){
         ...
         */
        if (mgmt_frm && (status != IEEE80211_TX_ERROR)
                && (pdev->txrx_pdev->tx_capture)
                                   ...
                                   A_UINT32 status)
{
    ...
    for(desc_id = 0; desc_id < HTT_MAX_NUM_MGMT_DESCS; desc_id++) {
         ...
         *    unlock the htt_tx_mutex, and goes on to call umac-cb for the mgmt frame.
         *    (This umac-cb might try to send some mgmt frames again, there by trying to acqiure
         *     htt_tx_mutex again; So, it is important umac-cb is called without holding
