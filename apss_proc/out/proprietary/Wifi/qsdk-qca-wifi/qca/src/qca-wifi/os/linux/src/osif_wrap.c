...
#else
ssize_t osif_wrap_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
#endif
{
    char buf[4];
...
int osif_wrap_attach(wlan_dev_t ic)
{
    ...
    osif_wrap_reinit(ic);
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " Wrap Attached: Wrap_com =%pK ic->ic_wrap_com=%pK &wrap_com->wc_devt=%pK \n", wrap_com, ic->ic_wrap_com, &wrap_com->wc_devt);
    return ret;
}
