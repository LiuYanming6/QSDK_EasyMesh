...
struct wps_radio_t {
    ...
    int num_ap_vaps;
    struct vap_t *ap_vaplist_ptr;
    int num_sta_vaps;
    struct vap_t *sta_vaplist_ptr;
    int conn_sta_start_time;
    int conn_sta_duration;
    int dconn_sta_start_time;
    int dconn_sta_duration;
    int conn_ap_start_time;
    int conn_ap_duration;
    int dconn_ap_start_time;
    int dconn_ap_duration;
    int is_one_sta_disconnect;
 ...
 * And, make sure that the radio interface is valid.
 */
...
{
    int current_time = 0, i = 0, j;
    struct wps_cfg_t *cfg, *cfg_ap, *cfg_sta;
    char wps_completed_vap[IFNAMSIZ];
    FILE *f;
    struct vap_t *p_stalist = NULL;
    struct vap_t *p_sta = NULL;
    int ret, is_prev_sta_connected = 0;
        ...
        if (current_time == work[i].time) {
            ...
            if (work[i].activation > 0) {
                ...
                case WPS_MASTER:
                    ...
                    if (cfg->option & ABANDON_IF_CONN) {
                        p_sta = p_stalist;
                        while(p_sta && !is_prev_sta_connected) {
                            if(is_sta_connected(sock, p_sta->vap_iface)){
                                /*Previous STA Got Connected*/
                                is_prev_sta_connected = 1;
                                break;
                            }
                            p_sta = p_sta->next_vap_ptr;
                        }
                        if(is_prev_sta_connected)
                            break;
                    }
                ...
                case WPS_MANAGED:
                    ...
                    if (cfg->option & ABANDON_IF_CONN) {
                        ...
                        if(!is_sta_connected(sock, cfg->vap)){
                            p_sta = (struct vap_t*)malloc(sizeof(struct vap_t));
                            if (p_sta) {
                                strlcpy(p_sta->vap_iface, cfg->vap, IFNAMSIZ);
                                p_sta->mode = WPS_MANAGED;
                                p_sta->is_up = 0;
                                p_sta->next_vap_ptr = p_stalist;
                                p_stalist = p_sta;
                            }
                        }
                    }
...
static void free_radiolist()
{
    ...
    while (head) {
        if (head->ap_vaplist_ptr) {
            pvaplist = head->ap_vaplist_ptr;
            while (pvaplist) {
            ...
            head->ap_vaplist_ptr = NULL;
        }
        if (head->sta_vaplist_ptr) {
            pvaplist = head->sta_vaplist_ptr;
            while (pvaplist) {
                pvap = pvaplist->next_vap_ptr;
                log_msg(WPS_DBG, "Freeing vap(%pK)\n", pvaplist);
                free(pvaplist);
...
static void print_radiolist()
{
    ...
    temp = radiohead_ptr->enabled_radio_ptr;
    while (temp != NULL) {
        log_msg(WPS_DBG, "%s: p(%pK) %s num_sta_vaps(%d)\n", __func__, temp,
            temp->radio_iface, temp->num_sta_vaps);
        if (temp->num_sta_vaps) {
            p_vaplist = temp->sta_vaplist_ptr;
            pvap = p_vaplist;
            while (pvap) {
...
/*
 * Inserts vap name to radiolist
 * Before inserting vap, it checks if it is part of enabled
 * radio iface. Inserts vap for only wps enabled radios.
 *
 * It invokes wpa_supplicant to get sta vaps
 */
static void insert_vap_to_radiolist(char *vap, int ioctl_sock)
{
    struct vap_t *p_vap = NULL;
    struct wps_radio_t *p_temp;
    struct iwreq iwr;
    struct ifreq ifr;
    char radio_iface[IFNAMSIZ];
    int parent_ifindex = 0;
    int found = 0, is_vap_up = 0;

    /* get radio iface name of vap */
    memset(&iwr, 0, sizeof(iwr));
    strlcpy(iwr.ifr_name, vap, IFNAMSIZ);
    ...
    log_msg(WPS_INF, "vap(%s) parent if(%d)\n", vap, parent_ifindex);
    ...
    log_msg(WPS_INF, "vap(%s) radio(%s)\n", vap, radio_iface);

    /* Check if vap is admin down or up */
    memset(&ifr, 0, sizeof(ifr));
    strlcpy(ifr.ifr_name, vap, IFNAMSIZ);
    if (ioctl(ioctl_sock, SIOCGIFFLAGS, &ifr) < 0) {
        log_msg(WPS_INF, "Failed to get ioctl value SIOCGIFFLAGS\n");
        return;
    }
    is_vap_up = (ifr.ifr_flags & IFF_UP);
    log_msg(WPS_INF, "vap(%s) is_admin_up(%d)\n", vap, is_vap_up);
    ...
    strlcpy(p_vap->vap_iface, vap, IFNAMSIZ);
    p_vap->mode = find_interface_type(ioctl_sock, vap, radio_iface);
    p_vap->is_up = is_vap_up;

    if(WPS_MANAGED == (p_vap->mode)) {
        if(!p_vap->is_up)
            goto end_list_add;

        /* p_vap->is_up has different significance for STA and AP VAP.
         * In case of AP VAP, it indicates admin up/ down and in case
         * of STA VAP, it indicates the connection status.
         */

        p_vap->is_up = is_sta_connected(ioctl_sock, vap);
        log_msg(WPS_INF, "vap(%s) connection status : %d\n",vap,p_vap->is_up);
    }
    ...
    while (p_temp) {
        if (!strncmp(p_temp->radio_iface, radio_iface, IFNAMSIZ)) {
            ...
            if((p_vap->mode) == WPS_MANAGED) {
                p_vap->next_vap_ptr = p_temp->sta_vaplist_ptr;
                p_temp->sta_vaplist_ptr = p_vap;
                (p_temp->num_sta_vaps)++;
                (radiohead_ptr->num_all_sta_vaps)++;
            } else if ((p_vap->mode) == WPS_MASTER) {
                p_vap->next_vap_ptr = p_temp->ap_vaplist_ptr;
                p_temp->ap_vaplist_ptr = p_vap;
                (p_temp->num_ap_vaps)++;
            }
            break;
        }
...
static int get_sta_vaps(int sock_fd)
{
    ...
    if(radiohead_ptr && (radiohead_ptr->num_all_sta_vaps)){
        return radiohead_ptr->num_all_sta_vaps;
    }
                ...
                for (i=0, j=0; i < nbytes; i++) {
                    ...
                    if (*start == '\0' || *start == '\n') {
                        if (j > 0 && j < IFNAMSIZ) {
                            ...
                            insert_vap_to_radiolist(sta_iface, sock_fd);
                        }
                        j = 0;
                    } else j++;
                }
                        ...
                        struct wps_work_t **p_work, struct wps_cfg_t **p_head)
{
    ...
    char *start, *end;
    int n, ret = 0;
    struct wps_cfg_t *cfg, *head = NULL, *p_workhead = NULL, *ptemp_cfg = NULL, *tmp_head = NULL;
    ...
    unsigned int global_option = 0;
    int vap_list_iterate;

    struct wps_opt_t *radio_list_ptr;
...
next_line:
        ...
        if (wps_use_cases_start) {
            ...
            if (!strncmp(start, "ALL_DISCONNECTED", (end - start))) {
                ...
                log_msg(WPS_INF, "ATLEAST 1 DISCONN case (%d)\n", found_case);
            }
            if ((prev_use_case == STA_ATLEAST_ONE_DISCONN) &&
                            (prev_use_case == found_case)) {
                struct wps_radio_t *p_temp = NULL;
                char   *tmp_end = NULL, *tmp_start = NULL;
                int    tmp_start_time, tmp_duration;

                if (!strncmp(start, "abandon_if_connection_succeed", (end - start)))
                    global_option |= ABANDON_IF_CONN;

                do {
                    tmp_start = start;
                    tmp_end = end = strstr(tmp_start, ":");
                    if (!end){
                        break;
                    }
                    /* Start time */
                    tmp_start = tmp_end + 1;
                    if (*tmp_start == '\0' || *tmp_start == '\n') {
                        break;
                    }

                    tmp_end = strstr(tmp_start, ":");
                    if (!tmp_end || (n = tmp_end - tmp_start) >= sizeof(time) || n <= 0) {
                        break;
                    }

                    memcpy(time, tmp_start, n);
                    time[n] = '\0';

                    if (n == 1 && time[0] == '-') {
                        log_msg(WPS_INF, "%s:Line[%d]: empty start_time for the vap\n",__func__,__LINE__);
                        break;
                    }
                    tmp_start_time = strtoul(time, NULL, 0);
                    ...
                    tmp_end = line_buf + strlen(line_buf) - 1;
                    if (tmp_end && (*tmp_end != '\n'))
                        tmp_end++; // '\0'
                    ...
                    if (n == 1 && time[0] == '-') {
                        log_msg(WPS_INF, "%s:Line [%d] :empty duration for the vap\n",__func__,__LINE__);
                        break;
                    }
                    tmp_duration = strtoul(time, NULL, 0);

                    p_temp = radiohead_ptr->enabled_radio_ptr;
                    if (!strncmp(start, "D_STA",(end - start))) {
                        while (p_temp) {
                            p_temp->dconn_sta_start_time = tmp_start_time;
                            p_temp->dconn_sta_duration = tmp_duration;
                            p_temp = p_temp->next_radio_ptr;
                        }
                    } else if (!strncmp(start, "C_STA",(end - start))) {
                        while (p_temp) {
                            p_temp->conn_sta_start_time = tmp_start_time;
                            p_temp->conn_sta_duration = tmp_duration;
                            p_temp = p_temp->next_radio_ptr;
                        }
                    } else if (!strncmp(start, "D_AP",(end - start))) {
                        while (p_temp) {
                            p_temp->dconn_ap_start_time = tmp_start_time;
                            p_temp->dconn_ap_duration = tmp_duration;
                            p_temp = p_temp->next_radio_ptr;
                        }
                    } else if (!strncmp(start, "C_AP",(end - start))) {
                        while (p_temp) {
                            p_temp->conn_ap_start_time = tmp_start_time;
                            p_temp->conn_ap_duration = tmp_duration;
                            p_temp = p_temp->next_radio_ptr;
                        }
                    }
                } while (0);
            ...
            if ((found_case != prev_use_case) || (wps_use_cases_end)) {
                ...
                if (prev_use_case == STA_ATLEAST_ONE_DISCONN) {
                    DIR *p_sys_class_net = NULL;
                    struct dirent *p_dent = NULL;
                    char radio_iface[IFNAMSIZ];
                    int parent_ifindex = 0;
                    struct wps_radio_t *p_temp = NULL;
                    ...
                    /*Simulate the cfg option specifically given for STA_ATLEAST_ONE_DISCONN case*/
                    p_sys_class_net = opendir("/sys/class/net");
                    if (!p_sys_class_net) {
                        log_msg(WPS_WARN, "%s[%d]:can't open directory\n", __func__,__LINE__);
                        return -1;
                    }

                    while ((p_dent = readdir(p_sys_class_net)) != NULL) {
                        if (!strncmp("ath",p_dent->d_name,strlen("ath"))) {
                            log_msg(WPS_WARN, "%s[%d]:Inserting vap: %s to radiolist\n",
                                    __func__,__LINE__,p_dent->d_name);
                            insert_vap_to_radiolist(p_dent->d_name,sock);
                        }
                    }

                    closedir(p_sys_class_net);
                    p_temp = radiohead_ptr->enabled_radio_ptr;

                    while (p_temp) {
                        struct vap_t *p_all_vap_list[] = {p_temp->sta_vaplist_ptr,p_temp->ap_vaplist_ptr};
                        struct vap_t *pvap = NULL;

                        for (vap_list_iterate = 0; vap_list_iterate < 2; ++vap_list_iterate) {
                            pvap = p_all_vap_list[vap_list_iterate];
                            ...
                            while (pvap) {
                                ...
                                memcpy(cfg->vap, pvap->vap_iface, IFNAMSIZ);
                                memcpy(cfg->radio, p_temp->radio_iface, IFNAMSIZ);
                                cfg->type = pvap->mode;

                                if (WPS_MANAGED == (pvap->mode)) {
                                    cfg->start_time = pvap->is_up ?
                                        p_temp->conn_sta_start_time : p_temp->dconn_sta_start_time;
                                    cfg->duration =  pvap->is_up ?
                                        p_temp->conn_sta_duration : p_temp->dconn_sta_duration;
                                    p_temp->is_one_sta_disconnect |= pvap->is_up ? 0 : 1;
                                } else if (WPS_MASTER == (pvap->mode)) {
                                    cfg->start_time = p_temp->is_one_sta_disconnect ?
                                        p_temp->dconn_ap_start_time : p_temp->conn_ap_start_time;
                                    cfg->duration = p_temp->is_one_sta_disconnect ?
                                        p_temp->dconn_ap_duration : p_temp->conn_ap_duration;
                                }
                                cfg->radio_ptr = NULL;
                                cfg->wps_active = 0;
                                cfg->wid_act = cfg->wid_deact = -1;
                ...
                if (((p_worklist) && (head != NULL)) || (wps_use_cases_end)) {
                    ...
                    if (p_worklist->work_list_ptr) {
                        ...
                        log_msg(WPS_DBG, "p_worklist->worklist_p(%pK) head(%pK)\n",
                            p_worklist->work_list_ptr, p_worklist->work_list_ptr->cfg_ptr);
