...
#define IEEE80211_BINTVAL_MINVAL_RANGE3     200   /*For Offload: if vap is <=16 use this value as min allowed bintval value*/
#define IEEE80211_BINTVAL_MINVAL_RANGE4     400   /*For DA chipset: if vap is <16 used this value as min allowed bintval value*/
...
#define MBSSID_BINTVAL_CHECK(_bintval, _numvaps, _bmode) \
        ...
        ((_numvaps <= IEEE80211_BINTVAL_VAPCOUNT3) && (_bmode == 0) && \
         ((_bintval >= IEEE80211_BINTVAL_MINVAL_RANGE3) || (_bintval >= IEEE80211_BINTVAL_MINVAL_RANGE4))) || \
...
#define IEEE80211_AUTH_MAX_MGMT_TIME            600 /* 3 times the MAX_MGMT_TIME 200 */
...
#define IEEE80211_ATF_WAIT                     1   /* build up allocate table (secs)*/
#define ATF_NUM_ITER_CONTRIBUTE_LEVEL_INCR     4   /* Should be a power of 2.
                                                   Number of iterations after which the contribute level increments */
#endif
...
#define IEEE80211_FILS_ENABLE_BIT   31
...
#define IEEE80211_REJECT_MGMT_MAX 16
...
/* CSA DEAUTH Support Values */
typedef enum _csa_deauth_type {
    CSA_DEAUTH_NONE,
    CSA_DEAUTH_UNICAST,
    CSA_DEAUTH_BROADCAST,
} csa_deauth_type;
...
#define ATF_TRAFFIC_CTRL_AC_ALL              0xF
...
struct wmi_pdev_atf_grp_wmm_ac_cfg_req {
    u_int32_t    num_groups;
    struct{
        u_int32_t    atf_config_ac_be;    /* Relative ATF % for BE traffic */
        u_int32_t    atf_config_ac_bk;    /* Relative ATF % for BK traffic */
        u_int32_t    atf_config_ac_vi;    /* Relative ATF % for VI traffic */
        u_int32_t    atf_config_ac_vo;    /* Relative ATF % for VO traffic */
...
struct atf_ac_config {
    u_int32_t   ac[WME_NUM_AC][1]; /* Airtime corresponding to each AC id*/
    u_int8_t    ac_bitmap;      /* Bitmap to indicate configured ACs */
    u_int8_t    ac_cfg_flag;    /* Flag to indicate AC configuration */
};
...
struct atf_config {
    struct {
        ...
        u_int8_t     sta_cfg_mark;      /* label sta cfg from user input until re-update table to decide if remove this mark or remove this entry */
        ...
        u_int32_t    sta_cal_ac_value[WME_NUM_AC];     /* airtime calculated for ac configured STA */
    ...
    struct {
        ...
        struct atf_ac_config    atf_cfg_ac; /* AC configuration */
    ...
    struct {
        ...
        u_int8_t    grp_sched_policy; /* 0-Fair, 1- Strict, 2- Fair with UB */
        u_int8_t    grp_ssid[ATF_CFG_NUM_VDEV][IEEE80211_NWID_LEN+1]; /* List of SSIDs in the group */
        struct atf_ac_config    atf_cfg_ac; /* AC configuration */
        struct group_list *grplist_entry; /* Pointer to group list */
...
typedef struct chan_util_stats {
    ...
    u_int8_t non_wifi;     /*Non-WIFI channel utilization */
...
struct scan_chan_stats {
    struct scan_chan_entry_stats entry_stats;
    struct channel_stats stats[MAX_DUAL_BAND_SCAN_CHANS];
};
#endif
...
typedef struct ieee80211com {
    ...
    TAILQ_HEAD(ic_vaps_head, ieee80211vap)    ic_vaps; /* list of vap instances */
...
#define CONSIDER_OBSS_LONG_SLOT_DEFAULT 0
    ...
    u_int8_t                      ic_host_dfs_status_wait_timer_running :1,
                                  ic_spoof_dfs_check_failed :1,
                                  ic_sent_wmi_dfs_radar_found :1,/*Sent radar_found wmi to FW and expecting a response */
                                  ic_spoof_test_done      :1,
                                  ic_tst_disable_chanchange_post_radar_detect :1,
                                  ic_tst_choose_random_chan   :1,
                                  ic_rebuilt_chanlist : 1;
    u_int8_t                      ic_tempchan:1; /* Indication that a temp chan is assigned and must find a proper one */
...
#define IC_IEEE80211_FIND_NODE(_ic,_nt,_macaddr)  (_ic)->ic_ieee80211_find_node((_nt),(_macaddr))
    ...
    /* Channel on which radar pulses are injected */
    struct ieee80211_ath_channel      *ic_radar_found_chan;
...
#define MAX_TX_PPDU_SIZE 32
    ...
    int                     (*ic_node_cfr_capture)(struct ieee80211_node *);
...
#ifdef ATH_SUPPORT_RDKB
    void                     (*ic_wlanstats_peer_init)(struct ieee80211com *, struct ieee80211_node *);
    void                     (*ic_wlanstats_peer_deinit)(struct ieee80211com *, struct ieee80211_node *);
#endif
    /* scanning support */
...
#if QCA_AIRTIME_FAIRNESS
    void            (*ic_atf_node_unblock)(struct ieee80211com *ic, struct ieee80211_node *ni, struct atf_stats *stats);
    ...
    int             (*ic_set_atf_grp_ac)(struct ieee80211com *ic);
    ...
    int32_t        (*ic_atf_debug_nodestate)(struct ieee80211com *ic, struct ieee80211_node *ni, struct atf_nodestate *atfnodestate);
#endif
...
#define ATH_HMMC_CNT_MAX 8
    ...
    int                              (*ic_random_channel)(struct ieee80211com *ic, u_int8_t is_select_nondfs, u_int8_t skip_curchan, bool use_des_chanflags);
    void                             (*ic_dfs_notify_radar)(struct ieee80211com *ic, struct ieee80211_ath_channel *chan);
    void                             (*ic_dfs_notify_etsi_precac_radar)(struct ieee80211com *ic, uint8_t is_radar_found_on_secondary_seg);
    void                             (*ic_channel_notify_to_app)(struct ieee80211com *ic);
    ...
    void                             (*ic_dfs_reset_precaclist)(struct ieee80211com *ic);
    ...
    uint8_t                          ic_precac_prefered_chan; /*channel to change after precac*/
    uint8_t                          ic_precac_inter_chan; /*intermediate non-DFS channel while precac is being done*/
    ...
    struct ieee80211_ath_channel *       (*ic_dfs_find_precac_secondary_vht80_chan)(struct ieee80211com *ic);
#endif
    ...
    void                             (*ic_dfs_unmark_radar)(struct ieee80211com *ic, struct ieee80211_ath_channel *chan);
    u_int8_t                         (*ic_dfs_chan_in_nol)(struct ieee80211com *ic, u_int32_t chan_freq);
#endif
    int                              (*ic_wmi_radar_found_indication_to_fw) (struct ieee80211com *ic,
                                        struct dfs_radar_found_params *radar_params);
    void                             (*ic_dfs_remove_spoof_chan_from_nol) (struct ieee80211com *ic);
    void                             (*ic_dfs_action_on_radar_found) (struct ieee80211com *ic);
    void                             (*ic_restart_vaps_with_non_dfs_chan) (struct ieee80211com *ic, int);
    int                              (*ic_rebuild_chan_list_with_non_dfs_channels) (struct ieee80211com *ic);
    bool                             ic_is_host_dfs_check_enabled;

    void                             (*ic_get_ext_chan_info)(struct ieee80211com *ic, struct ieee80211_ath_channel *chan,
                                             struct ieee80211_channel_list *chan_info);
    struct ieee80211_ath_channel *(*ic_find_channel)(struct ieee80211com *ic, int freq, u_int8_t des_cfreq2, u_int32_t flags);
    ...
    void                                (*ic_thermal_timer)(struct ieee80211com *ic);
...
#endif
    u_int32_t                           (*ic_get_activiy)(struct ieee80211com *ic);
    ...
    struct  wmi_pdev_atf_grp_wmm_ac_cfg_req  wmi_atf_grp_ac_req;
    ...
    u_int32_t                           ic_atf_num_active_sg;   /* total number of active subgroups */
    u_int32_t                           ic_atf_num_active_sg_strict; /* total number of active subgroups in strict group*/
    u_int32_t                           ic_atf_num_active_sg_fair_ub; /* total number of active subgroups in fair_ub group */
    u_int32_t                           atf_num_sg_borrow_strict; /* total subgroups looking to borrow in strict group */
    u_int32_t                           atf_num_sg_borrow_fair_ub; /* total subgroups looking to borrow in fair_ub group */
    u_int32_t                           ic_atf_contrlevel_incr; /*number of consecutive iterations after which the contribution level increments*/
#endif
...
int (*ic_print_scan_config)(wlan_if_t vaphandle, struct seq_file *m);
    ...
    /* See documentation for functions registered to below smart log function
     * pointers.
     */
    int32_t                           (*ic_smart_log_connection_fail_start)\
                                          (struct ieee80211com *ic);
    int32_t                           (*ic_smart_log_connection_fail_stop)\
                                          (struct ieee80211com *ic);

#ifndef REMOVE_PKT_LOG
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,5,0)
    /* Note: In the interest of readability, we do not bother about extremely
     * old Linux kernels where Work Queues were not available, since these are
     * not known to be part of currently supported products, and since rest of
     * code has no such checks.
     * But add such checks if required in the future.
     */
    qdf_work_t                          smart_log_fw_pktlog_enable_work;
     ...
     * pointers.
     */
    int32_t                           (*ic_smart_log_fw_pktlog_enable)\
                                          (struct ieee80211com *ic);
    int32_t                           (*ic_smart_log_fw_pktlog_disable)\
                                          (struct ieee80211com *ic);
    int32_t                           (*ic_smart_log_fw_pktlog_start)\
                                          (struct ieee80211com *ic,
                                           u_int32_t fw_pktlog_types);
    int32_t                           (*ic_smart_log_fw_pktlog_stop)\
                                          (struct ieee80211com *ic);
    int32_t                           (*ic_smart_log_fw_pktlog_stop_and_block)\
                                          (struct ieee80211com *ic,
                                           int32_t host_pktlog_types,
                                           bool block_only_if_started);
    void                              (*ic_smart_log_fw_pktlog_unblock)\
                                          (struct ieee80211com *ic);
#endif /* REMOVE_PKT_LOG */
#endif /* OL_ATH_SMART_LOGGING */
    u_int8_t                            tx_capture_da;      /* Enable Tx capture for DA radios if monitor mode is enabled */
    u_int8_t                            ic_bestUL_hyst;
    u_int8_t                            ic_dfs_info_notify_channel_available; /* Enable DFS info notify to app */
    ...
    chan_util_stats_t                   ic_ch_util;
    u_int8_t                            ic_curchan_ieee; /* current channel number */
    u_int8_t                            ic_curchan_cfreq2;/* current channel cfreq2 */
    int32_t                             acs_noisefloor_threshold;
    u_int8_t                            ic_tx_capture;  /* whether tx capture is enabled */
#if ATH_SUPPORT_DFS && QCA_DFS_NOL_VAP_RESTART
    u_int32_t                           ic_des_chan_flags; /* Desired channel flags */
    void                                (*ic_bringup_ap_vaps_after_nol)(struct ieee80211com *ic);
    u_int8_t                            ic_pause_stavap_scan; /*pause stavap scan*/
#endif
#ifdef ATH_SUPPORT_RDKB
    struct wlanstats_ctx                wlanstats_ctx;
    uint32_t                            ic_wlanstats_config;
    bool                                ic_wlanstats_enabled;
    int                                 ic_poll_q_util_interval_ms;
    unsigned long                       (*ic_poll_q_util)(struct ieee80211com *ic);
    int                                 wlanstats_rix;
    int                                 wlanstats_sgi;
    int                                 wlanstats_first_mpdu;
    int                                 ic_uniform_rssi;
    struct channel_stats                ic_home_chan_stats;
    struct scan_chan_stats              ic_scan_chan_stats;
#endif
    bool                                ic_cfr_timer_enable;
    u_int16_t                           ic_cfr_max_clients;
...
#if QCA_AIRTIME_FAIRNESS
#define IEEE80211_ATF_SUBGROUP_TYPE_SSID    0x0
#define IEEE80211_ATF_SUBGROUP_TYPE_PEER    0x1
#define IEEE80211_ATF_SUBGROUP_TYPE_AC      0x2

#define IEEE80211_ATF_SG_ACTIVE             0x1 /* Indicates subgroup is active */

#define IEEE80211_ATF_SET_SG_ACTIVE(_v)      ((_v)->sg_flag |= IEEE80211_ATF_SG_ACTIVE)
#define IEEE80211_ATF_RESET_SG_ACTIVE(_v)    ((_v)->sg_flag &= ~IEEE80211_ATF_SG_ACTIVE)
#define IEEE80211_ATF_IS_SG_ACTIVE(_v)       ((_v)->sg_flag & IEEE80211_ATF_SG_ACTIVE)
#define IEEE80211_ATF_SET_PEER_CFG(_v)       (_v |= 1)
#define IEEE80211_ATF_RESET_PEER_CFG(_v)     (_v &= ~(1))

struct subgroup_list {
    TAILQ_ENTRY(subgroup_list)  sg_next;    /* list of ATF group instances */
    u_int8_t    sg_name[IEEE80211_NWID_LEN + 1]; /* subgroup name */
    u_int32_t   sg_id;      /* subgroup id */
    int8_t      sg_type;    /* 0 - SSID, 1 - PEER, 2 - AC */
    int8_t      sg_peermac[6]; /* Peer MA. for Peer config entry */
    int8_t      ac_id;      /*Access category ID. for AC config entry */
    int8_t      sg_del;     /* Mark for deletion */
    u_int32_t   usr_cfg_val;    /* Value configured by the user */
    u_int32_t   sg_atf_units;   /* Alloted Airtime for this subgroup */
    ...
    u_int32_t   tokens_excess_consumed; /* Transmit took more time than estimated
                                        (actualairtime - estimated airtime) */
    u_int32_t   tokens_less_consumed;   /* Transmit completion happened faster than estimated
                                                        (estimatedairtime - actual airtime */
    u_int32_t   sg_shadow_tx_tokens;      /* copy of tx_tokens. Used a temp variable for calc */
    u_int32_t   sg_raw_tx_tokens;         /* tx tokens. user conf + channel availability */
    u_int32_t   sg_act_tx_tokens;         /* tokens available, after adjustemnt of excess consumed in prev cycle */
    u_int32_t   sg_atfdata_logged;        /* data log(history) available */
    u_int32_t   sg_atfindex;              /* index to the data log */
    u_int32_t   sg_borrowedtokens;        /* tokens borrowed in the previous iteration */
    u_int32_t   sg_contributedtokens;     /* tokens contributed in the previous iteration */
    u_int32_t   sg_unusedtokenpercent[5]; /* Unused tokens(weighted %) for last 3 iterations */
    u_int32_t   sg_flag;                  /* Flags to indicate subgroup status */
    u_int16_t   sg_atf_debug_mask;        /* Size of the history of statistics */
    u_int16_t   sg_atf_debug_id;          /* Write index into the history of statistics */
    u_int16_t   sg_allowed_bufs;        /* Buffers that this subgroup can hold at max*/
    u_int16_t   sg_num_buf_held;        /* Buffers held by a subgroup currently  */
    u_int16_t   sg_num_bufs_sent;       /* Buffers transmitted (or failed) */
    u_int16_t   sg_num_bytes_sent;      /* Bytes transmitted (or failed) */
    u_int16_t   sg_max_num_buf_held;    /* Max buffers held by a subgroup */
    u_int16_t   sg_min_num_buf_held;    /* Min buffers held by a subgroup */
    u_int16_t   sg_pkt_drop_nobuf;      /* Packets dropped as subgroup is holding max allowed buffers */
    u_int8_t    sg_allowed_buf_updated; /* Set after an_allowed_bufs has been computed once */
    u_int8_t    sg_atfborrow;           /* Flag. node is looking to borrow tokens */
    u_int32_t   sg_contribute_id;       /* index used for contribution level */
    u_int32_t   sg_contribute_level;    /* level of contribution */
    u_int32_t   sg_atftidstate;         /* TID state. Applicable only when 1:1 mapping between node & subgroup*/
    qdf_spinlock_t  atf_sg_lock;       /* lock to protect atf subgroup */
    struct atf_stats sg_atf_stats;      /* Statistics */
    struct atf_stats *sg_atf_debug;     /* History of statistics */
};

struct group_list {
    TAILQ_ENTRY(group_list)       group_next;    /* list of ATF group instances */
    u_int8_t        group_name[IEEE80211_NWID_LEN + 1]; //group name
    u_int32_t   atf_num_sg_borrow;         /* number of clients looking to borrow tokens */
    ...
    u_int32_t   subgroup_count;                 /* Number of subtroups in this group */
    u_int32_t   num_subgroup;                   /* Number of subgroups in this group */
    u_int32_t   tot_airtime_peer;               /* Total airtime assigned to all peer subgroups within a group */
    u_int32_t   tot_airtime_ac;                 /* Total airtime assigned to all ac subgroups withing a group */
    u_int32_t   atf_num_active_sg;              /* number of active subgroups in a group */
    u_int8_t    group_sched;                    /* Group scheduling policy. 0-Fair, 1-Strict, 2-FairWithUB */
    TAILQ_HEAD(, subgroup_list) atf_subgroups;  /* List of ATF subgroup instances */
};
#endif
...
#define MDNS_DROP_THRESHOLD 1000                         /* msecs */
...
/* not multiple thread safe */
    u_int32_t                         iv_deleted:1,  /* if the vap deleted by user */
                                      ...
                                      iv_qbssload_update:1, /* update bssload IE in beacon */
    ...
    u_int32_t                         iv_sgi:1,        /* Short Guard Interval Enable:1 Disable:0 */
                                      ...
                                      iv_vap_atf_sched:2,   /* per ssid atf scheduling policy */
...
#endif
    u_int8_t                          iv_map:1, /* flag to check if map is enabled or not */
    ...
    u_int8_t                          iv_mapvapup:1; /* flag to check if vap is up */
...
/* multiple thread safe */
    ...
    u_int8_t                          iv_ss_under_util_20;       /* spatial stream underutilize for primary 20 */
    u_int8_t                          iv_sec_20_util;            /* Secondary 20MHz utilization */
    u_int8_t                          iv_sec_40_util;            /* Secondary 40MHz utilization */
    u_int8_t                          iv_sec_80_util;            /* Secondary 80MHz utilization */
    ...
    systime_t                         iv_last_vap_up_time;         /* absolute system time; not TSF */
...
#define IEEE80211_F_WOW_DISASSOC           2               /* Disassoc Pattern */
    ...
    void                              (*iv_modify_cts_rate)(struct ieee80211vap *);
...
#define IBSS_RSSI_CLASS_MAX 7
    ...
    u_int8_t iv_stealth_down; /* for stealth mode interface down without explicitly disconnecting STAs */
...
*/
 ...
 u_int8_t                   iv_pause_scan;
...
#endif
    u_int8_t                   mhdr_len;
    u_int8_t                   iv_probe_ev_enable:1;    /* custom probe event enabled for this vap */
#if MESH_MODE_SUPPORT
    ...
    int                       (*iv_scan_nac_rssi)(struct ieee80211vap *, enum ieee80211_nac_rssi_param ,
...
#if QCA_AIRTIME_FAIRNESS
    u_int8_t                   ac_blk_cnt; /* number of ac under this vap blocked to transmit */
#endif
...
#define SOFTBLOCK_ALLOW_TIME_MAX 100000
    ...
    bool                       is_country_changed; /* flag to indicate country change */
    qdf_nbuf_queue_t      iv_mgmt_offchan_txnbufqueue;
    qdf_spinlock_t        iv_mgmt_offchan_tx_lock;
    qdf_atomic_t          iv_mgmt_offchan_cmpl_pending;
    bool                       iv_supplicant_state; /* 0 - supplicant active
                                                       1 - supplicant inactive */
    u_int8_t                   iv_state_check_cnt;
    u_int8_t                   iv_drop_mdns;       /* flag to drop mdns packets */
    systime_t                  iv_last_mdns_tx;   /* Last MDNS packet transmit time */
#ifdef ATH_SUPPORT_RDKB
    ...
    u_int8_t                 iv_sae_max_auth_attempts;     /* indicate maximum no.pf auth attempts that STA can */
...
struct global_ic_list {
    ...
    u_int8_t      extender_info;  /* 0x00->no stavap connection and RootAP access
                                     0x0F->invalid state
                                     0xF0->stavap connection is there, but no rootap access
                                     0xFF->stavap connection and rootap access is there
                                   */
    u_int8_t      preferred_mac_list[MAX_RADIO_CNT][IEEE80211_ADDR_LEN];
                                  /*Mac list of RE to which this AP has to connect*/
    u_int8_t      denied_mac_list[MAX_RADIO_CNT][IEEE80211_ADDR_LEN];
                                  /*Mac list of RE STAvaps to which AP should deauth*/
    u_int8_t      ap_preferrence;
                                 /* 0 - No preferrence
                                    1 - connect to RootAP
                                    2 - connect to RE, whose bssid matches with preferred_mac_list
                                  */
    systime_t     rootap_access_downtime;
#if ATH_SUPPORT_WRAP
    u_int8_t      psta_creation_timer_flag;
    os_timer_t    psta_creation_timer;
#endif
};
#endif
...
#define IEEE80211_ATF_GROUP_SCHED_FAIR             0    /* Can Borrow & contribute */
#define IEEE80211_ATF_GROUP_SCHED_STRICT           1    /* Cannot borrow & contribute */
#define IEEE80211_ATF_GROUP_SCHED_FAIR_UB          2    /* Can contribute, cannot borrow */
#define IEEE80211_ATF_GROUP_NUM_SCHED_POLICIES     3    /* Max number of supported sched policies */
...
#else
#define GLOBAL_IC_LOCK(_rl)
#define GLOBAL_IC_UNLOCK(_rl)
#endif
...
VAP_FLAG_FUNCS(mbo)
VAP_FLAG_FUNCS(oce)
VAP_FLAG_FUNCS(delayed_beacon)
VAP_FLAG_FUNCS(pmf_mgmt_xtry)
VAP_FLAG_FUNCS(map)
VAP_FLAG_FUNCS(mapfh)
VAP_FLAG_FUNCS(mapbh)
VAP_FLAG_FUNCS(mapbsta)
VAP_FLAG_FUNCS(mapvapup)
...
void ieee80211_vap_cancel_supplicant_state_timer(struct ieee80211vap *vap);
...
do {                                                                                 \
    ...
    TAILQ_FOREACH(_vap, &ic->ic_vaps, iv_next) {                                     \
        ...
        if (_vap->iv_ic != ic) {                                                     \
           qdf_print("%s() Vap %pK for ic %pK is not valid ,cnt %d\n",                 \
...
#define IEEE80211_PSTA_CHECK_TIME 60000 /*1 min*/
void ieee80211_validate_psta_creation(struct global_ic_list *glist);
#endif
