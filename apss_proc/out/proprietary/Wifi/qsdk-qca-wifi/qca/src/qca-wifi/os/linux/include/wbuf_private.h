...
static INLINE void
__wbuf_set_tx_rate(struct sk_buff *skb, uint8_t nss, uint8_t preamble, uint8_t mcs)
{
    if (!skb) return;

#define ASSEMBLE_RATE(_mcs, _nss, _pream)     \
            (((_pream) << 6) | ((_nss) << 4) | (_mcs))

    ((struct ieee80211_cb *)skb->cb)->_uu.tx_ctrl.rate = 
        ASSEMBLE_RATE(mcs & 0xF, nss & 0x3, preamble & 0x3);;
}
...
__wbuf_set_tx_ctrl(struct sk_buff *skb, uint8_t retry, uint8_t power, uint8_t chain)
{
    ...
    ((struct ieee80211_cb *)skb->cb)->_uu.tx_ctrl.retries = retry;
    ((struct ieee80211_cb *)skb->cb)->_uu.tx_ctrl.txchainmask = chain;
    ((struct ieee80211_cb *)skb->cb)->_uu.tx_ctrl.txpower = power;
}
...
__wbuf_get_tx_ctrl(struct sk_buff *skb, uint8_t *retry, uint8_t *power, uint8_t *chain)
{
    ...
    if (retry)
        *retry = ((struct ieee80211_cb *)skb->cb)->_uu.tx_ctrl.retries;
    if (chain)
        *chain = ((struct ieee80211_cb *)skb->cb)->_uu.tx_ctrl.txchainmask;
    if (power)
        *power = ((struct ieee80211_cb *)skb->cb)->_uu.tx_ctrl.txpower;
}

static INLINE void
__wbuf_get_tx_rate(struct sk_buff *skb, uint8_t *rate)
{
    if (!skb) return;

    if (rate)
        *rate = ((struct ieee80211_cb *)skb->cb)->_uu.tx_ctrl.rate;
}
...
__wbuf_set_pid(struct sk_buff *skb, u_int32_t pid) 
{
    ...
    ((struct ieee80211_cb *)skb->cb)->request_id = pid;
}
...
__wbuf_get_pid(struct sk_buff *skb, u_int32_t *pid) 
{
    ...
    *pid = ((struct ieee80211_cb *)skb->cb)->request_id;
}
