...
/**
 * ieee80211_atf_wmi_configured_peer() - set bit for explicitly configured clients
 * @ic: Pointer to the radio structure
 * @wmi_peer_ext_req: Pointer to wmi peer struct
 * @peer_index: Peer index in the ic config struct
 * @wmi_peer_index: Index to the wmi peer struct
 *
 * Return: 0 on success
 */
int32_t
ieee80211_atf_wmi_configured_peer(struct ieee80211com *ic, struct wmi_pdev_atf_peer_ext_request *wmi_peer_ext_req,
                                u_int8_t peer_index, u_int8_t wmi_peer_index)
{
    int32_t retv = 0, index = 0;

    if (ic->atfcfg_set.peer_id[peer_index].index_group == 0xff) {
        wmi_peer_ext_req->atf_peer_ext_info[wmi_peer_index].group_index = 0;
        return retv;
    }

    if (ic->ic_atf_ssidgroup) {
        /* SSID grouping enabled */
        index = ic->atfcfg_set.peer_id[peer_index].index_group;
    } else {
        /* VAP based configuration */
        index = ic->atfcfg_set.peer_id[peer_index].index_vap;
    }

    if ((index < ATF_CFG_NUM_VDEV) &&
         ic->atfcfg_set.peer_id[peer_index].cfg_flag &&
         (ic->atfcfg_set.peer_id[peer_index].sta_cfg_value[index] ||
          ic->atfcfg_set.peer_id[peer_index].sta_cfg_value[ATF_CFG_GLOBAL_INDEX])) {
         IEEE80211_ATF_SET_PEER_CFG(wmi_peer_ext_req->atf_peer_ext_info[wmi_peer_index].atf_units_reserved);
    } else {
        IEEE80211_ATF_RESET_PEER_CFG(wmi_peer_ext_req->atf_peer_ext_info[wmi_peer_index].atf_units_reserved);
    }
...
build_atf_for_fm(struct ieee80211com *ic)
{
    ...
    struct     wmi_pdev_atf_grp_wmm_ac_cfg_req *wmi_wmm_ac_req = &(ic->wmi_atf_grp_ac_req);
    int32_t    retv = 0;
       ...
       for (i = 0, j = 0; (i < ATF_ACTIVED_MAX_CLIENTS)&&(j < ic->atfcfg_set.peer_num_cal); i++)
       {
            if(ic->atfcfg_set.peer_id[i].sta_assoc_status == 1)
            {
                if (ic->atfcfg_set.peer_id[i].index_group == 0xff)
                    wmi_peer_ext_req->atf_peer_ext_info[j].group_index = 0;
                else
                    wmi_peer_ext_req->atf_peer_ext_info[j].group_index = (ic->atfcfg_set.peer_id[i].index_group - 1);

                wmi_peer_ext_req->atf_peer_ext_info[j].atf_units_reserved = 0;
                ieee80211_atf_wmi_configured_peer(ic, wmi_peer_ext_req, i, j);
                ...
                IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_NORMAL, IEEE80211_MSG_ATF, "Peer mac : %s Groupindex : %d airtime : %d atf_units_reserved : 0x%08x\n",
                        ether_sprintf(ic->atfcfg_set.peer_id[i].sta_mac), wmi_peer_ext_req->atf_peer_ext_info[j].group_index,
                        wmi_req->atf_peer_info[j].percentage_peer, wmi_peer_ext_req->atf_peer_ext_info[j].atf_units_reserved);
                j++;
            }
       }
    }
    ...
    if(ic->ic_atf_ssidgroup)
    {
        wmi_wmm_ac_req->num_groups = ic->atfcfg_set.grp_num_cfg;
        for (i = 0; (i < ic->atfcfg_set.grp_num_cfg); i++)
        {
            acbitmap = ic->atfcfg_set.atfgroup[i].atf_cfg_ac.ac_bitmap;
            for(k = 0; k < WME_NUM_AC; k++)
            {
                if (!(1 & acbitmap))
                    ic->atfcfg_set.atfgroup[i].atf_cfg_ac.ac[k][0] = 0;
                acbitmap = acbitmap >> 1;
            }
            wmi_wmm_ac_req->atf_group_wmm_ac_info[i].atf_config_ac_be = ic->atfcfg_set.atfgroup[i].atf_cfg_ac.ac[WME_AC_BE][0];
            wmi_wmm_ac_req->atf_group_wmm_ac_info[i].atf_config_ac_bk = ic->atfcfg_set.atfgroup[i].atf_cfg_ac.ac[WME_AC_BK][0];
            wmi_wmm_ac_req->atf_group_wmm_ac_info[i].atf_config_ac_vi = ic->atfcfg_set.atfgroup[i].atf_cfg_ac.ac[WME_AC_VI][0];
            wmi_wmm_ac_req->atf_group_wmm_ac_info[i].atf_config_ac_vo = ic->atfcfg_set.atfgroup[i].atf_cfg_ac.ac[WME_AC_VO][0];
        }
    }
    else
    {
        wmi_wmm_ac_req->num_groups = ic->atfcfg_set.vap_num_cfg + 1; //default group (+1)
        for (i = 0, j = 1; i < ic->atfcfg_set.vap_num_cfg; i++, j++)
        {
            acbitmap = ic->atfcfg_set.vap[i].atf_cfg_ac.ac_bitmap;
            for(k = 0; k < WME_NUM_AC; k++)
            {
                if (!(1 & acbitmap))
                    ic->atfcfg_set.vap[i].atf_cfg_ac.ac[k][0] = 0;
                acbitmap = acbitmap >>1;
            }
            wmi_wmm_ac_req->atf_group_wmm_ac_info[j].atf_config_ac_be = ic->atfcfg_set.vap[i].atf_cfg_ac.ac[WME_AC_BE][0];
            wmi_wmm_ac_req->atf_group_wmm_ac_info[j].atf_config_ac_bk = ic->atfcfg_set.vap[i].atf_cfg_ac.ac[WME_AC_BK][0];
            wmi_wmm_ac_req->atf_group_wmm_ac_info[j].atf_config_ac_vi = ic->atfcfg_set.vap[i].atf_cfg_ac.ac[WME_AC_VI][0];
            wmi_wmm_ac_req->atf_group_wmm_ac_info[j].atf_config_ac_vo = ic->atfcfg_set.vap[i].atf_cfg_ac.ac[WME_AC_VO][0];
        }
    }
    ...
    for (i = 0; (i < ATF_ACTIVED_MAX_ATFGROUPS) && (i < ic->atfcfg_set.grp_num_cfg); i++)
    {
        ...
        wmi_group_req->atf_group_info[i].atf_group_units_reserved = ic->atfcfg_set.atfgroup[i].grp_sched_policy;
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_NORMAL, IEEE80211_MSG_ATF, "Groupindex : %d Airtime : %d Sched policy : %d\n",
                i, wmi_group_req->atf_group_info[i].percentage_group, wmi_group_req->atf_group_info[i].atf_group_units_reserved);
    }
...
/**
 * atf_assign_ac_subgroup() - Assign subgroups to AC based config
 * @ic: Pointer to the radio structure
 * @ni: Pointer to node structure
 * @group: pointer to group entry
 * @acbitmap: bitmap to indicate AC configuration
 *
 * Return: 0 on success
 */
int
atf_assign_ac_subgroup(struct ieee80211com *ic, struct ieee80211_node *ni, struct group_list *group, u_int8_t acbitmap)
{
    ...
    for (ac_id = 0; ac_id < WME_NUM_AC; ac_id++)
    {
        ...
        if (1 & acbitmap) {
            if (!ieee80211_atf_subgroup_exist(ic, group, &ac_id, &subgroup, IEEE80211_ATF_SUBGROUP_TYPE_AC)) {
                qdf_print("%s AC subgroup doesn't exist in group %s for ac : %d \n",
                        __func__, group->group_name, ac_id);
                return -EINVAL;
            } else {
                ni->ni_ac_list_ptr[ac_id] = subgroup;
            }
        } else {
            ni->ni_ac_list_ptr[ac_id] = subgroup;
        }
...
/**
 * atf_assign_peer_subgroup() - Assign subgroups to node with Peer based config
 * @ic: Pointer to the radio structure
 * @ni: Pointer to node structure
 * @group: pointer to group entry
 *
 ...
 */
int
atf_assign_peer_subgroup(struct ieee80211com *ic, struct ieee80211_node *ni, struct group_list *group)
{
    struct subgroup_list *subgroup = NULL;
    u_int8_t ac_id = 0;

    if (!ieee80211_atf_subgroup_exist(ic, group, ni->ni_macaddr, &subgroup, IEEE80211_ATF_SUBGROUP_TYPE_PEER)) {
        qdf_print("%s %d Peer subgroup doesn't exit. ERROR!!! \n",__func__, __LINE__);
        return -1;
    }
...
/**
 * atf_assign_default_subgroup() - Assign default subgroup to Nodes
 * @ic: Pointer to the radio structure
 * @ni: Pointer to node structure
 * @group: pointer to group entry
 *
 ...
 */
int
atf_assign_default_subgroup(struct ieee80211com *ic, struct ieee80211_node *ni, struct group_list *group)
{
    struct subgroup_list *subgroup = NULL;
    u_int8_t ac_id = 0;
...
/**
 * atf_assign_subgroup() - Assign subgroups to Nodes
 * @ni: Pointer to node structure
 * @group: pointer to group entry
 * @acbitmap: bitmap to indicate AC configuration
 * @cfg_type: Config type (SSID/Peer/AC)
 *
 * Return: 0 on success
 */
int atf_assign_subgroup(struct ieee80211_node *ni, struct group_list *group, u_int8_t acbitmap, u_int8_t cfg_type)
{
    struct ieee80211com *ic = ni->ni_ic;

    if (group == NULL) {
        qdf_print("%s group is NULL. returning \n", __func__);
        return -EINVAL;
    }

    if (cfg_type == ATF_CFGTYPE_PEER) {
        atf_assign_peer_subgroup(ic, ni, group);
    } else  if (cfg_type == ATF_CFGTYPE_AC) {
        atf_assign_ac_subgroup(ic, ni, group, acbitmap);
    } else {
        atf_assign_default_subgroup(ic, ni, group);
    }
...
/**
 * ieee80211_get_atf_peer_subgroup_cfgtype () - Get config type of peer
 * @ic: Pointer to ic structure
 * @atfac: Pointer to AC configuration structure
 * @Peerindex: Peer index in ic struct
 *
 * Return: configuration type
 */
u_int32_t
ieee80211_get_atf_peer_subgroup_cfgtype(struct ieee80211com *ic, struct atf_ac_config *atfac, u_int32_t peerindex)
{
    u_int32_t cfg_type = 0;
    u_int32_t index = 0;
    ...
    if (ic->atfcfg_set.peer_id[peerindex].cfg_flag &&
        ic->atfcfg_set.peer_id[peerindex].sta_cfg_value[index]) {
...
build_atf_alloc_tbl(struct ieee80211com *ic)
{
    ...
    int32_t    retv = 0;
    u_int8_t   i,vap_index = 0, k = 0, cfg_type = 0, ac_bitmap = 0;
    ...
    /* Reset airtime alloted to all subgroups */
    ieee80211_atf_reset_airtime_sg_all(ic);
    /* delete any group/subgroups marked */
    ieee80211_atf_del_subgroup(ic);
    ieee80211_atf_del_group(ic);

    /* Assign BSS node to default group */
    ...
    */
    if(ic->atfcfg_set.peer_num_cal!= 0)
    {
        TAILQ_FOREACH(ni, &nt->nt_node, ni_list) {
            ...
            if( (ic->ic_atf_ssidgroup) || (ic->atfcfg_set.grp_num_cfg) )
            {
                /*
                  Search for this vap in the atfgroup table & find the group id
                */
                group_index = 0xFF;
                for (i = 0; (i < ic->atfcfg_set.grp_num_cfg) && (i < ATF_ACTIVED_MAX_ATFGROUPS); i++)
                {
                    for (k = 0; (k < ic->atfcfg_set.atfgroup[i].grp_num_ssid) && (k < ATF_CFG_NUM_VDEV); k++)
                    {
            ...
            for (i = 0, vap_index = 0xff; (i < ic->atfcfg_set.vap_num_cfg) && (i < ATF_CFG_NUM_VDEV); i++)
            {
                 ...
                 for (i = 0; i < ATF_ACTIVED_MAX_CLIENTS; i++)
                 {
                    if(ic->atfcfg_set.peer_id[i].index_vap != 0)
                    {
                        if(OS_MEMCMP(ic->atfcfg_set.peer_id[i].sta_mac,ni->ni_macaddr,IEEE80211_ADDR_LEN) == 0)
                        {
                            ...
                            if (ic->ic_atf_ssidgroup) {
                                if (group_index != 0xFF) {
                                    atf_ac = &ic->atfcfg_set.atfgroup[group_index - 1].atf_cfg_ac;
                                } else {
                                    atf_ac = NULL;
                                }
                            } else {
                                if (vap_index != 0xFF) {
                                    atf_ac = &ic->atfcfg_set.vap[vap_index - 1].atf_cfg_ac;
                                } else {
                                    atf_ac = NULL;
                                }
                            }
                            ...
                            if(group_index == 0xFF)
                            {
                                ...
                                ni->ni_atf_group = group;
                                /* Assign node to peer subgroup if ThroughputBased
                                 * configuration exists for the node
                                 */
                                if (!ni->ni_atf_tput) {
                                    cfg_type = ieee80211_get_atf_peer_subgroup_cfgtype(ic, atf_ac, i);
                                } else {
                                    cfg_type = ATF_CFGTYPE_PEER;
                                }
                                atf_assign_subgroup(ni, group, ac_bitmap, cfg_type);
                            } else if (group_index <= ATF_ACTIVED_MAX_ATFGROUPS) {
                                group = ni->ni_atf_group = ic->atfcfg_set.atfgroup[group_index - 1].grplist_entry;
                                /* Assign node to peer subgroup if ThroughputBased
                                 * configuration exists for the node
                                 */
                                if (!ni->ni_atf_tput) {
                                ...
                                qdf_print("%s %d Invalid group index : %d \n",
                                    __func__, __LINE__, group_index);
                            }
...
/**
 * ieee80211_sta_cal_unconfigured_peers - Calculate airtime for unconfigured peer
 * @ic: Pointer to the radio structure
 * @tot_airtime: total airtime
 * @peer_cnt: unconfigure peer count
 * @peerbitmap: Peer bitmap indicating unconfigured peers
 *
 * Return: total airtime
 */
u_int32_t ieee80211_sta_cal_unconfigured_peers(struct ieee80211com *ic, u_int32_t tot_airtime, u_int32_t peer_cnt, u_int32_t peerbitmap)
{
    ...
    for (j = 0; j<ATF_ACTIVED_MAX_CLIENTS; j++) {
        if (peerbitmap & (calbitmap << j)) {
            ...
            tot_airtime -= ic->atfcfg_set.peer_id[j].sta_cal_value;
        }
    }
...
/**
 * ieee80211_sta_cal_unconfigured_peers - Calculate airtime for configured peer
 * @ic: Pointer to the radio structure
 * @peer_index: Index of peer
 * @cfg_value: user configured value
 * @cfg_index : index of vap/group
 *
 * Return: ATF_PEER_CFG_HIERARCHY: If STA Hierarchy config exists
           ATF_PEER_CFG_GLOBAL: If STA global config exists
           ATF_PEER_CFG_UNCONFIGURED: If STA is unconfigured
           ATF_PEER_CFG_NOTASSOCIATED: If STA is not associated
 */
int
ieee80211_sta_cal_configured_peers(struct ieee80211com *ic, u_int32_t peer_index, u_int32_t cfg_value, u_int32_t cfg_index)
{
    int ret = 0;
    u_int32_t sta_cal = 0, peer_global_per = 0;

    if (ic->atfcfg_set.peer_id[peer_index].sta_assoc_status == 1) {
        sta_cal = ((cfg_value * ic->atfcfg_set.peer_id[peer_index].sta_cfg_value[cfg_index])
                         /ic->atfcfg_set.percentage_unit);
        peer_global_per = ic->atfcfg_set.peer_id[peer_index].sta_cfg_value[ATF_CFG_GLOBAL_INDEX];
        if (sta_cal) {
            ic->atfcfg_set.peer_id[peer_index].sta_cal_value = sta_cal;
            ret = ATF_PEER_CFG_HIERARCHY;
        } else if(peer_global_per) {
            ic->atfcfg_set.peer_id[peer_index].sta_cal_value = peer_global_per;
            ret = ATF_PEER_CFG_GLOBAL;
        } else {
            ic->atfcfg_set.peer_id[peer_index].sta_cal_value = 0;
...
/**
 * get_total_ac_config_val() - Total airtime configured for AC
 * @atfac: Pointer to atf AC config structure
 *
 * Return: total configured value
 */
int get_total_ac_config_val(struct atf_ac_config *atfac)
{
    u_int8_t acbitmap = 0;
    u_int32_t config_val = 0, i = 0;
...
/**
 * assign_airtime_default_group() - Assign airtime to default group
 * @ic: Pointer to the radio structure
 * @airtime_unconfigured: unconfigured airtime
 *
 * Return: 0 on Success
 */
int
assign_airtime_default_group(struct ieee80211com *ic, u_int32_t airtime_unconfigured)
{
    struct group_list *group = NULL;

    TAILQ_FOREACH (group, &ic->ic_atfgroups, group_next) {
        if (ieee80211_atf_is_default_group(group->group_name)) {
            group->group_airtime = airtime_unconfigured;
            ic->atfcfg_set.atfgroup[0].grp_cfg_value = group->group_airtime;
        }
    }
...
/**
 * cal_atf_ac_config() - Calculate airtime for AC
 * @ic: Pointer to the radio structure
 * @index: vap/group index
 * @peerbitmap: Peer Bitmap to indicate unconfigured peers
 * @residual_airtime: Airtime left after deducting STA based config
 * @cfg_type: configuration type (SSID/GROUP)
 *
 * Return: 0 on Success
 */
int
cal_atf_ac_config(struct ieee80211com *ic, u_int32_t index, u_int64_t peerbitmap, u_int32_t residual_airtime, u_int8_t cfg_type)
{
    u_int32_t j = 0, i = 0, calbitmap = 0;
    struct atf_ac_config *atfac = NULL;
    u_int32_t clientindex = 0;
    u_int8_t acbitmap = 0;
    u_int32_t config_ac_val = 0, tot_ac_val = 0, airtime_vap = 0 ;
    ...
    for (j = 0, calbitmap = 1; j < ATF_ACTIVED_MAX_CLIENTS; j++) {
        ...
        if (cfg_type == ATF_CFGTYPE_SSID) { // ssid config
            clientindex = ic->atfcfg_set.peer_id[j].index_vap;
            airtime_vap = ic->atfcfg_set.vap[index].vap_cfg_value;
        } else {
            clientindex = ic->atfcfg_set.peer_id[j].index_group;
            airtime_vap = ic->atfcfg_set.atfgroup[index].grp_cfg_value;
        }
            ...
            if ((clientindex - 1) == index) {
                ...
                while (acbitmap) {
                    if (1 & acbitmap) {
                        ...
                        ic->atfcfg_set.peer_id[j].sta_cal_ac_value[i] = ((config_ac_val * residual_airtime) /  ic->atfcfg_set.percentage_unit);
                    }
                    i++;
                    acbitmap = acbitmap >> 1;
                }
    ...
    tot_ac_val = get_total_ac_config_val(atfac);
...
{
    int32_t    retv = 0, ac_config_val = 0;
    u_int8_t   calcnt = 0,stacnt = 0,j,i = 0;
    ...
    u_int32_t  grp_per_unit = 0, peer_cfg_type = 0;
    ...
    if (ic->ic_atf_ssidgroup) {
        u_int32_t grp_index = 0, peer_index = 0;

        ieee80211_atf_del_subgroup(ic);
        ieee80211_atf_del_group(ic);
        peer_total_cnt = ic->atfcfg_set.peer_num_cal;
        grp_num = ic->atfcfg_set.grp_num_cfg - 1;
        peer_cfg_cnt = ic->atfcfg_set.peer_num_cfg;
        per_unit = ic->atfcfg_set.percentage_unit;
        for (grp_index = 1; (grp_index < ATF_ACTIVED_MAX_ATFGROUPS)&&(grp_num != 0); grp_index++)
        {
            grp_per_unit = ic->atfcfg_set.atfgroup[grp_index].grp_cfg_value;
            per_unit -= grp_per_unit;
            grp_num--;
            stacnt = peerbitmap = 0;
            calbitmap = 1;
            for (peer_index = 0; peer_index < ATF_ACTIVED_MAX_CLIENTS; peer_index++) {
                if (ic->atfcfg_set.peer_id[peer_index].index_group == (grp_index + 1)) {
                    if (ic->atfcfg_set.peer_id[peer_index].cfg_flag) {
                        peer_cfg_type = ieee80211_sta_cal_configured_peers(ic, peer_index,
                              ic->atfcfg_set.atfgroup[grp_index].grp_cfg_value, ic->atfcfg_set.peer_id[peer_index].index_group);
                       if ((peer_cfg_type == ATF_PEER_CFG_HIERARCHY) ||
                           (peer_cfg_type == ATF_PEER_CFG_GLOBAL)) {
                            grp_per_unit -= ic->atfcfg_set.peer_id[peer_index].sta_cal_value;
                            peer_total_cnt--;
                        } else if (peer_cfg_type == ATF_PEER_CFG_UNCONFIGURED) {
                            peerbitmap |= (calbitmap << peer_index);
                            stacnt++;
                        } else if (peer_cfg_type == ATF_PEER_CFG_NOTASSOCIATED) {
                        }
                        peer_cfg_cnt--;
                    } else {
                        peerbitmap |= (calbitmap << peer_index);
            ...
            if (stacnt) {
                ac_config_val = cal_atf_ac_config(ic, grp_index, peerbitmap, grp_per_unit, ATF_CFGTYPE_GROUP);
                if ((ac_config_val >= 0) && (ac_config_val > ic->atfcfg_set.percentage_unit)) {
                    qdf_print("%s wrong ac config %d. Cannot be more than %d \n",__func__, ac_config_val, ic->atfcfg_set.percentage_unit);
                } else if (ac_config_val >= 0) {
                    grp_per_unit = ((ic->atfcfg_set.percentage_unit - ac_config_val) * grp_per_unit) / ic->atfcfg_set.percentage_unit;
                }
                ieee80211_sta_cal_unconfigured_peers(ic, grp_per_unit, stacnt, peerbitmap);
                peer_total_cnt -= stacnt;
            }
        }
        /* Allocate unconfigured airtime to default group */
        assign_airtime_default_group(ic, per_unit);

        if(peer_total_cnt) {
            qdf_print("All clients are expected to be part of a group. stacnt : %d peer_total_cnt : %d Check \n", stacnt, peer_total_cnt);
        }
        if(peer_cfg_cnt) {
            qdf_print("All clients are expected to be part of a group. peer_cfg_cnt : %d Check \n", peer_cfg_cnt);
        }
         ...
         for (i = 0; (i< ATF_CFG_NUM_VDEV)&&(vap_num != 0); i++)
         {
             if(ic->atfcfg_set.vap[i].cfg_flag)
             {
                  ...
                  if (stacnt)
                  {
                     ac_config_val = cal_atf_ac_config(ic, i, peerbitmap, vap_per_unit, ATF_CFGTYPE_SSID);
                     if ((ac_config_val >= 0) && (ac_config_val > ic->atfcfg_set.percentage_unit)) {
                         qdf_print("%s wrong ac config %d. Cannot be more than %d \n",__func__, ac_config_val, ic->atfcfg_set.percentage_unit);
                     } else if (ac_config_val >= 0) {
                         vap_per_unit = ((ic->atfcfg_set.percentage_unit - ac_config_val) * vap_per_unit) / ic->atfcfg_set.percentage_unit;
                     }
...
/*         }else{
        ...
        /* printk("cal_atf_alloc_tbl -- NO VAP host config mode\n"); */
         ...
         /* Allocate unconfigured airtime to default group */
            assign_airtime_default_group(ic, ic->atfcfg_set.percentage_unit);
         ...
         if(ic->atfcfg_set.peer_num_cfg)
         {
           ...
           for (i=0, calcnt=ic->atfcfg_set.peer_num_cfg, calbitmap = 1; ((i<ATF_ACTIVED_MAX_CLIENTS)&& (calcnt!=0)); i++)
           {
              if(ic->atfcfg_set.peer_id[i].cfg_flag)
              {
                 if(ic->atfcfg_set.peer_id[i].sta_cfg_value[ATF_CFG_GLOBAL_INDEX] <= per_unit )
                 {
                     ...
                     {
                         /* Get the global  percentage for this SSID if present */
                         peer_global_per = ic->atfcfg_set.peer_id[i].sta_cfg_value[ATF_CFG_GLOBAL_INDEX];
                         /* This is when we are in no vap host config mode so will use global config if set for a  peer*/
                         if(peer_global_per)
                         {
                             ic->atfcfg_set.peer_id[i].sta_cal_value = peer_global_per;
                             per_unit -=peer_global_per;
                         }else{
                            /* No global percentage so lets treat these as unconfigured peers */
                            peer_with_zero_val++;
                         }
                     }else{
           ...
           if (ic->atfcfg_set.peer_num_cal >= (ic->atfcfg_set.peer_num_cfg - un_assoc_cfg_peer))
           {
              ...
              if(calcnt)
              {
                  ...
                  {
                     peer_global_per = ic->atfcfg_set.peer_id[i].sta_cfg_value[ATF_CFG_GLOBAL_INDEX];
                     /* Allow non configured stations or stations with no global percentage */
                     if(ic->atfcfg_set.peer_id[i].cfg_flag == 0 || ((ic->atfcfg_set.peer_id[i].cfg_flag == 1) && !(peer_global_per)))
                     {
...
void log_atf_alloc(struct ieee80211com *ic)
{
    ...
    for (i=0, j=0; (i < ATF_ACTIVED_MAX_CLIENTS) && (j < ic->atfcfg_set.peer_num_cal); i++)
    {
        if(ic->atfcfg_set.peer_id[i].sta_assoc_status == 1)
        {
            ...
            ssid_name = "Others";
            if (group_index && (group_index != 0xFF ) &&
                (group_index < ATF_ACTIVED_MAX_ATFGROUPS)) {
                ssid_name = ic->atfcfg_set.atfgroup[group_index - 1].grp_ssid[0]; // First SSID in group
            }
            else if (vap_index && (vap_index != 0xFF) &&
                    (vap_index < ATF_CFG_NUM_VDEV)) {
                ssid_name = ic->atfcfg_set.vap[vap_index - 1].essid;
            }
            if (ic->ic_atf_ssidgroup) {
                if (group_index && (group_index < ATF_ACTIVED_MAX_ATFGROUPS)) {
                    atfac = &ic->atfcfg_set.atfgroup[group_index - 1].atf_cfg_ac;
                }
            } else {
                if (vap_index && (vap_index < ATF_CFG_NUM_VDEV)) {
                    atfac = &ic->atfcfg_set.vap[vap_index - 1].atf_cfg_ac;
                }
            }
            if (atfac) {
                acbitmap = atfac->ac_bitmap;
            ...
            // Log output:
            if (((vap_index < ATF_CFG_NUM_VDEV) &&
                  ic->atfcfg_set.peer_id[i].cfg_flag &&
                  ic->atfcfg_set.peer_id[i].sta_cfg_value[vap_index]) ||
                  !acbitmap) {
                ...
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                    "client %s connected to SSID %s has following ACs configured - ",
                    ether_sprintf(ic->atfcfg_set.peer_id[i].sta_mac), ssid_name);
                ac_id = 0;
                while (acbitmap) {
                    if (1 & acbitmap) {
                        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                            "  AC%d:%d\%", ac_id, ic->atfcfg_set.peer_id[i].sta_cal_ac_value[ac_id] / 10);
                    }
                    ac_id++;
                    acbitmap = acbitmap >> 1;
                }
...
/**
 * dump_atf_subgroup_allocation() - dump subgroup airtime allocation
 * @ic: Pointer to the radio structure
 *
 * Return: None
 */
void dump_atf_subgroup_allocation(struct ieee80211com *ic)
{
    struct subgroup_list *subgroup = NULL;
    struct group_list *group = NULL;

    qdf_print("********** DUMP SUBGROUP ALLOC - START **********\n");
    TAILQ_FOREACH(group, &ic->ic_atfgroups, group_next) {
        TAILQ_FOREACH(subgroup, &group->atf_subgroups, sg_next) {
            qdf_print("Group : %s Subgroup : %s Airtime : %d ", group->group_name, subgroup->sg_name, subgroup->sg_atf_units);
        }
        qdf_print("\n");
    }
    qdf_print("********** DUMP SUBGROUP ALLOC - END **********\n");
...
OS_TIMER_FUNC(ieee80211_atfcfg_timer)
{
    ...
    retv = assign_airtime_subgroup(ic);
    retv = node_atf_capable_indicate(ic);
...
exit_atf_timer:
    ...
    //dump_atf_subgroup_allocation(ic);
}
...
static OS_TIMER_FUNC(wlan_atf_token_allocate_timeout_handler)
{
    ...
    if (ic->ic_atf_sched & IEEE80211_ATF_SCHED_STRICT) {
        ieee80211_atf_strictq_algo(ic);
    } else {
        ieee80211_atf_fairq_algo(ic);
    }
...
static OS_TIMER_FUNC(ieee80211_poll_q_util)
{
    ...
    for (i = 0; i < 10; i++)
        if (mask & BIT(i))
            ic->wlanstats_ctx.q_util.q[i]++;

    ic->wlanstats_ctx.q_util.cnt++;
}
#endif
...
#define DEFAULT_TRAFFIC_RATE     300
	...
	*/
    ...
    group = ieee80211_atf_create_default_group(ic);
    ieee80211_atf_add_ic_group(ic, group->group_name);
    if (!group) {
        qdf_print("%s error in creating ATF default group\n",__func__);
        return -1;
    }

    /* Create default subgroup */
    subgroup = ieee80211_atf_create_default_subgroup(ic, group);
    if (!subgroup) {
        qdf_print("%s error in creating ATF default subgroup\n",__func__);
        return -1;
    }
    ...
    ic->ic_atf_contrlevel_incr = ATF_NUM_ITER_CONTRIBUTE_LEVEL_INCR;
...
ieee80211_iter_vap_opmode(void *arg, struct ieee80211vap *vaphandle)
{
    ...
    default:
        ...
        QDF_PRINT_INFO(vaphandle->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s vap=%pK unknown opmode=%d\n",
...
ieee80211_sta_assoc_in_progress(struct ieee80211com *ic)
{
    ...
    if (vap_opmode_count.sta_count) {
        ...
        wlan_iterate_vap_list(ic,ieee80211_vap_iter_associated,(void *) &is_sta_associated);
                        ...
                        u_int16_t dwell_time, u_int16_t scan_requestor, u_int32_t *scan_id)
{
    ...
    ic->ic_curchan_ieee = channel->ic_ieee;
    ic->ic_curchan_cfreq2 = channel->ic_vhtop_ch_freq_seg2;
    ...
    if (ic->ic_curchan != vap->iv_bsschan){
        ...
        ic->ic_curchan_ieee = vap->iv_bsschan->ic_ieee;
        ic->ic_curchan_cfreq2 = vap->iv_bsschan->ic_vhtop_ch_freq_seg2;
    }
    error = ic->ic_set_channel(ic);
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "error = %d, channel now is %d\n", error, ic->ic_curchan->ic_freq);
