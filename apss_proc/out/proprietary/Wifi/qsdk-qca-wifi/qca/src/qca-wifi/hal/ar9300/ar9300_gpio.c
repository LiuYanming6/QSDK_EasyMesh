    ...
    HAL_GPIO_OUTPUT_MUX_TYPE hal_signal_type)
{
    ...
    static const u_int32_t    mux_signal_conversion_table[] = {
        ...
        AR_GPIO_OUTPUT_MUX_AS_SMART_ANT_SERIAL_STROBE,
                    /* HAL_GPIO_OUTPUT_MUX_AS_SMART_ANT_SERIAL_DATA */
        AR_GPIO_OUTPUT_MUX_AS_SMART_ANT_SERIAL_DATA,
...
#define ATH_WLAN_DENY_GPIOFUNC_VAL 4
#define ATH_WLAN_DENY_GPIO  1
        /* Get the pin and func values for BT Test */
        switch (ah_signal_type)
        {
            case AR_GPIO_OUTPUT_MUX_AS_RX_CLEAR_EXTERNAL:
                gpio = ATH_WLAN_DENY_GPIO;
                ah_signal_type = ATH_WLAN_DENY_GPIOFUNC_VAL;
                break;
            case AR_GPIO_OUTPUT_MUX_AS_OUTPUT:
                gpio = ATH_WLAN_DENY_GPIO;
                ah_signal_type = ATH_WLAN_DENY_GPIOFUNC_VAL;
                break;
            default:
    ...
    switch (ah_signal_type)
    {
        case AR_GPIO_OUTPUT_MUX_AS_SMARTANT_CTRL0:
            gpio =  ah->sa_gpio_conf.gpio_pin[0];
            ah_signal_type = ah->sa_gpio_conf.gpio_func[0];
            smart_ant = 1;
            break; 
        case AR_GPIO_OUTPUT_MUX_AS_SMARTANT_CTRL1:
            gpio =  ah->sa_gpio_conf.gpio_pin[1];
            ah_signal_type = ah->sa_gpio_conf.gpio_func[1];
            smart_ant = 1;
            break;
        case AR_GPIO_OUTPUT_MUX_AS_SMARTANT_CTRL2:    
            gpio =  ah->sa_gpio_conf.gpio_pin[2];
            ah_signal_type = ah->sa_gpio_conf.gpio_func[2];
            smart_ant = 1;
            break;
#if ATH_SMARTANTENNA_ROUTE_SWCOM_TO_GPIO
        case AR_GPIO_OUTPUT_MUX_AS_SWCOM3:
            gpio =  ah->sa_gpio_conf.gpio_pin_swcom3
            ah_signal_type = ah->sa_gpio_conf.gpio_func_swcom3;
            smart_ant = 1;
            break;
#endif
#if UNIFIED_SMARTANTENNA            
        case AR_GPIO_OUTPUT_MUX_AS_SMART_ANT_SERIAL_STROBE:
            gpio =  ah->sa_gpio_conf.gpio_pin_strobe;
            ah_signal_type = ah->sa_gpio_conf.gpio_func_strobe;
            smart_ant = 1;
            break;
        case AR_GPIO_OUTPUT_MUX_AS_SMART_ANT_SERIAL_DATA:
            gpio =  ah->sa_gpio_conf.gpio_pin_data;
            ah_signal_type = ah->sa_gpio_conf.gpio_func_data;
            smart_ant = 1;
            break;
#endif                
        default:
...
void ar9300_gpio_show(struct ath_hal *ah) 
{
    ath_hal_printf(ah, "--- 9382 GPIOs ---(ah=%pK)\n", ah );
    ath_hal_printf(ah,
        "AH9300(_ah)->ah_hostifregs:%pK\r\n", &(AH9300(ah)->ah_hostifregs));
    ath_hal_printf(ah,
        "GPIO_OUT:         0x%08X\n",
