...
#define IS_WLANSTATS_CFG_ACTIVE(_ic) (_ic->ic_wlanstats_config & 0x1)

#define IS_WLANSTATS_ENABLED(_ic) (_ic->ic_wlanstats_config && _ic->ic_wlanstats_enabled)
#endif
...
/* req_id for getting the address where f/w iram content is available */
...
struct ol_ath_softc_net80211 {
    ...
    */
    ...
    u_int32_t               carrier_vow_config;
    u_int32_t               fw_vow_stats_enable;
    u_int32_t               max_peers;
...
#ifdef ATH_SUPPORT_RDKB
   wdi_event_subscribe      wlanstats_event_sub;
   bool                     wlanstats_txstat_discard;
   u_int32_t                wlanstats_txstat_ppdu_end[PPDU_END_SIZE];
#endif
   int8_t                   cfr_support;
   struct wmi_host_mem_chunk       cfr_mem_chunk;
...
struct fd_buf_entry {
    ...
    wbuf_t                        fd_buf;
    TAILQ_ENTRY(fd_buf_entry)     deferred_fd_list_elem;
};
...
struct ol_ath_vap_net80211 {
    ...
    wbuf_t                      av_fd_wbuf;         /* FILS Discovery Frame Buffer */
    TAILQ_HEAD(, fd_buf_entry)  deferred_fd_list;   /* List of deferred FILS Discovery buffers */
    u_int32_t                   av_fd_period;       /* FD Period(TUs) */
    A_BOOL                      is_fd_dma_mapped;
#endif
...
struct ol_ath_node_net80211 {
    ...
    u_int32_t                   an_ni_tx_sgi_count;
    u_int8_t                    an_ni_tx_ratecode;
    u_int8_t                    an_ni_tx_flags;
    u_int8_t                    an_ni_rx_ratecode;
    u_int8_t                    an_ni_rx_flags;
...
PREPACK struct htt_cfr_type_1_info
{
    uint8_t     mem_req_id;
    uint8_t     cap_status;
    uint8_t     capture_bw;
...
struct htt_cfr_info
{
    ...
    union {
        struct htt_cfr_type_1_info cfr_type_1;
    } u;
};
...
void ol_ath_cfr_htt_attach(struct ieee80211com *ic);
void ol_ath_cfr_htt_detach(struct ieee80211com *ic);
...
int ol_ath_set_atf_grp_ac(struct ieee80211com *ic);
...
void ol_ath_fils_discovery_alloc(struct ieee80211com *ic, int if_id);

void ol_ath_fd_stop(struct ol_ath_softc_net80211 *scn,
                   struct ol_ath_vap_net80211 *avn);

void ol_ath_fd_free(struct ieee80211com *ic, int if_id);
