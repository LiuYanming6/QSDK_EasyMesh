...
#define ADJ_CHANS 8
    ...
    if((sec_chan != ieee_chan) && (sec_chan >= 0)) {
        ...
        if( (acs->acs_noisefloor[sec_chan] != NF_INVALID)
                && ( acs->acs_noisefloor[sec_chan] >= ic->acs_noisefloor_threshold) ) {
    ...
    if(sec_chan_40_1 & sec_chan_40_2) {
       ...
       if (sec_chan_40_1 >= 0) {
           if( (acs->acs_noisefloor[sec_chan_40_1] != NF_INVALID)
                    && (acs->acs_noisefloor[sec_chan_40_1] >= ic->acs_noisefloor_threshold )) {
       ...
       if (sec_chan_40_2 >= 0) {
           if( (acs->acs_noisefloor[sec_chan_40_2] !=NF_INVALID)
                   && (acs->acs_noisefloor[sec_chan_40_2] >= ic->acs_noisefloor_threshold )) {
    ...
    if(sec_chan_80_1 && sec_chan_80_2 && sec_chan_80_3 && sec_chan_80_4) {
        if (sec_chan_80_1 >= 0) {
            if( (acs->acs_noisefloor[sec_chan_80_1] != NF_INVALID)
                     && (acs->acs_noisefloor[sec_chan_80_1] >= ic->acs_noisefloor_threshold )) {
        ...
        if (sec_chan_80_2 >= 0) {
            if( (acs->acs_noisefloor[sec_chan_80_2] !=NF_INVALID)
                    && (acs->acs_noisefloor[sec_chan_80_2] >= ic->acs_noisefloor_threshold )) {
        ...
        if (sec_chan_80_3 >= 0) {
            if( (acs->acs_noisefloor[sec_chan_80_3] !=NF_INVALID)
                    && (acs->acs_noisefloor[sec_chan_80_3] >= ic->acs_noisefloor_threshold )) {
        ...
        if (sec_chan_80_4 >= 0) {
            if( (acs->acs_noisefloor[sec_chan_80_4] !=NF_INVALID)
                    && (acs->acs_noisefloor[sec_chan_80_4] >= ic->acs_noisefloor_threshold )) {
    ...
    for (k = first_adj_chan ; (k <= last_adj_chan) && (k > 0); k += 2) {
        ...
        if((acs->acs_noisefloor[k] != NF_INVALID) && (acs->acs_noisefloor[k] >= ic->acs_noisefloor_threshold )){
...
#undef DCS_DISABLE_THRESH
    ...
    for (i = 0; i < acs->acs_nchans; i++) {
        ...
        if ((acs->acs_noisefloor[cur_chan] != NF_INVALID) && (acs->acs_noisefloor[cur_chan] >= ic->acs_noisefloor_threshold) ) {
...
#define MAX_2G_SUPPORTED_CHAN 13
#define OVERLAPPING_INDEX_MAX 5

struct centerchan {
    u_int8_t count;                              /* number of chans to average the rssi */
    u_int8_t chanlist[OVERLAPPING_INDEX_MAX];    /* the possible beacon channels around center chan */
};
...
static void ieee80211_get_11ng_overlapping_chans(struct centerchan *centerchans, ieee80211_acs_t acs)
{
    ...
    first_chfreq = acs->acs_chans[0]->ic_freq;
    acs_nchfreq = acs->acs_chans[acs->acs_nchans - 1]->ic_freq;
    min_freq = first_chfreq;

    /* When the scan is happening on dual-band, the first channels added to acs_chans will be of 2GHZ
     */
    for(i = 0; min_freq <= acs_nchfreq && i < acs->acs_nchans &&
            i < MAX_2G_SUPPORTED_CHAN ; min_freq += 5, i++)
    {
       low_freq = (min_freq - 10) < first_chfreq ? first_chfreq : (min_freq - 10);
       high_freq = (min_freq + 10) > acs_nchfreq ? acs_nchfreq : (min_freq + 10);
       for(j = 0; low_freq <= high_freq && j < OVERLAPPING_INDEX_MAX; j++)
       {
...
ieee80211_acs_find_best_11ng_centerchan(ieee80211_acs_t acs)
{
    ...
    struct ieee80211com *ic = acs->acs_ic;
    struct centerchan centerchans[MAX_2G_SUPPORTED_CHAN];
...
#if ATH_SUPPORT_PRIMARY_ALLOWED_CHAN
                    if(ic->ic_primary_allowed_enable) {
                            channel = ieee80211_find_dot11_channel(acs->acs_ic, acs->acs_ic->ic_primary_chanlist->chan[0], 0, acs->acs_vap->iv_des_mode);
                    } else
#endif
                            channel = ieee80211_find_dot11_channel(acs->acs_ic, 0, 0, acs->acs_vap->iv_des_mode);
                    acs->acs_status = ACS_FAILED_RNDM;
...
{
    struct ieee80211_ath_channel *scan_chan = ieee80211_scan_entry_channel(se);
...
ieee80211_get_chan_neighbor_list(void *arg, wlan_scan_entry_t se)
{
    ...
    if ((channel_ieee_se == chan_neighbor_list->chan)) {
        ...
        if (chan_neighbor_list->nbss >= IEEE80211_MAX_NEIGHBOURS) {
            /* Caller allocated memory buffer is not enough increment the neighbour
             * count to indicate caller the memory needed for this channel*/
            if (chan_neighbor_list->neighbor_size <= sizeof(ieee80211_neighbor_info) * (chan_neighbor_list->nbss)) {
                chan_neighbor_list->nbss++;
                chan_neighbor_list->status = E2BIG;
                return EOK;
           }
       }
...
ieee80211_acs_get_channel_maxrssi_n_secondary_ch(void *arg, wlan_scan_entry_t se)
{
    ...
    struct ieee80211_ath_channel *channel_se = ieee80211_scan_entry_channel(se);
...
static void ieee80211_acs_construct_chan_list(ieee80211_acs_t acs, enum ieee80211_phymode mode)
{
    ...
    } else if (mode == IEEE80211_MODE_NONE) {
        ieee80211_acs_get_phymode_channels(acs, IEEE80211_MODE_11NG_HT20);
        nchans_2ghz = acs->acs_nchans;
        ieee80211_acs_get_phymode_channels(acs, IEEE80211_MODE_11NA_HT20);
...
int ieee80211_acs_init(ieee80211_acs_t *acs, wlan_dev_t devhandle, osdev_t osdev)
{
    ...
    (*acs)->acs_ic->acs_noisefloor_threshold = ACS_NOISE_FLOOR_THRESH;
...
void ieee80211_acs_retrieve_chan_info(struct ieee80211com *ic, ieee80211_scan_event_type type, wlan_chan_t chan)
{
    u_int32_t now = 0;
    u_int8_t flags;
    ieee80211_acs_t acs = ic->ic_acs;

    if (type == IEEE80211_SCAN_FOREIGN_CHANNEL_GET_NF) {
        now = (u_int32_t) CONVERT_SYSTEM_TIME_TO_MS(OS_GET_TIMESTAMP());
        flags = ACS_CHAN_STATS_NF;
        /* Get the noise floor value */
        acs->acs_noisefloor[ieee80211_chan2ieee(ic, chan)] = ic->ic_get_cur_chan_nf(ic);
    ...
    if ( type == IEEE80211_SCAN_FOREIGN_CHANNEL ) {
...
static void ieee80211_acs_scan_evhandler(struct ieee80211vap *originator, ieee80211_scan_event *event, void *arg)
{
     ...
     */
    ieee80211_acs_retrieve_chan_info(ic, event->type, event->chan);
    ...
    */
    ...
    if(acs->acs_scan_req_param.acs_scan_report_active) {
        ...
        if(acs->acs_scan_req_param.idle_time) {
            scan_params.idle_time = acs->acs_scan_req_param.idle_time;
        }
        if(acs->acs_scan_req_param.max_scan_time) {
            scan_params.max_scan_time  = acs->acs_scan_req_param.max_scan_time;
        }
    }
    ...
    if(acs->acs_uchan_list.uchan_cnt &&
        ...
        scan_params.chan_list =
            (u_int32_t *) OS_MALLOC(acs->acs_osdev, sizeof(u_int32_t)*alloc_count, 0);
        ...
        */
        ...
        {
		wlan_chan_t chan =
                ieee80211_find_dot11_channel(ic, acs->acs_uchan_list.uchan[i], 0, IEEE80211_MODE_AUTO);
                if (!chan) {
                    continue;
                }

            /*Allow the channels only supported by the radio*/
            if ((scan_params.flags & IEEE80211_SCAN_ALLBANDS) ||
                    ((scan_params.flags & IEEE80211_SCAN_2GHZ) ? IEEE80211_IS_CHAN_2GHZ(chan) :
                    ((scan_params.flags & IEEE80211_SCAN_5GHZ) ? IEEE80211_IS_CHAN_5GHZ(chan) : 0))) {

                acs->acs_ieee_chan[scan_params.num_channels] =
                    scan_params.chan_list[scan_params.num_channels] = acs->acs_uchan_list.uchan[i];
                scan_params.num_channels++;
            }
        }
        acs->acs_nchans_scan = scan_params.num_channels;
    ...
    wlan_scan_table_flush(vap);
    ieee80211_acs_flush_olddata(acs);
    if (skip_scan || (ret = wlan_scan_start(vap,
                &scan_params,
                acs->acs_scan_requestor,
                IEEE80211_SCAN_PRIORITY_HIGH,
                &(acs->acs_scan_id))) != EOK) {
        ...
        struct ieee80211_chan_stats *chan_stats)
{
    ...
    eacs_trace(EACS_DBG_LVL1,("%d.%03d | %d: \n",  now / 1000, now % 1000, flags));
    if(acs == NULL) {
...
int ieee80211_acs_scan_report(struct ieee80211com *ic, struct ieee80211vap *vap, struct ieee80211_acs_dbg *acs_r, u_int8_t internal)
{
    ...
    acs_r->chan_radar_noise = dfs_is_channel_in_nol((struct ath_dfs *) ic->ic_dfs, acs_r->chan_freq);
    acs_r->chan_non_80211_noise = 0; /* Not supported by HW */

    if(acs->acs_cycle_count[ieee_chan] != 0) {
            acs_r->chan_utilization = (acs->acs_chan_load[ieee_chan]) / (acs->acs_cycle_count[ieee_chan] / 100);
            acs_r->chan_utilization_total = (acs->acs_chan_load[ieee_chan]) / (acs->acs_cycle_count[ieee_chan] / 100);
            acs_r->chan_utilization_busy = (acs->acs_chan_load[ieee_chan]) / (acs->acs_cycle_count[ieee_chan] / 100);
            if (ieee_chan ==  ieee80211_chan2ieee(ic, ic->ic_curchan)) {
                    acs_r->chan_utilization_busy_tx = (acs->acs_chan_load[ieee_chan]) / (acs->acs_cycle_count[ieee_chan] / 100);
                    acs_r->chan_utilization_busy_rx = (acs->acs_chan_load[ieee_chan]) / (acs->acs_cycle_count[ieee_chan] / 100);
            } else {
                    acs_r->chan_utilization_busy_tx = (scn->mib_cycle_cnts.tx_frame_count) / (acs->acs_cycle_count[ieee_chan] / 100);
                    acs_r->chan_utilization_busy_rx = (scn->mib_cycle_cnts.rx_frame_count) / (acs->acs_cycle_count[ieee_chan] / 100);
            }
            acs_r->chan_utilization_busy_self = acs_r->chan_utilization_busy_tx + acs_r->chan_utilization_busy_rx;
            acs_r->chan_utilization_busy_ext = (acs->acs_chan_load[ieee_chan]) / (acs->acs_cycle_count[ieee_chan] / 100);
    } else {
            eacs_trace(EACS_DBG_LVL0,("ieee_chan : %d, acs_cycle_count is zero \n", ieee_chan));
            acs_r->chan_utilization = 0;
            acs_r->chan_utilization_total = 0;
            acs_r->chan_utilization_busy = 0;
            acs_r->chan_utilization_busy_tx = 0;
            acs_r->chan_utilization_busy_rx = 0;
            acs_r->chan_utilization_busy_self = 0;
            acs_r->chan_utilization_busy_ext = 0;
    }
    ...
    if (acs_r->neighbor_list == NULL) {
        eacs_trace(EACS_DBG_LVL0,("Memory not available for neighbour list, not populating the list\n"));
        return 0;
    }
    ...
    acs_neighbor_list->neighbor_list = acs_r->neighbor_list;
    acs_neighbor_list->neighbor_size = acs_r->neighbor_size;

    status = wlan_scan_table_iterate(acs->acs_vap, ieee80211_get_chan_neighbor_list, (void *)acs_neighbor_list);
    acs_r->chan_nbss = acs_neighbor_list->nbss;
    if (acs_neighbor_list->status == E2BIG) {
        qdf_print("%s: Insufficent memory required buffer size %d\n",__func__,
                    sizeof(ieee80211_neighbor_info)*(acs_neighbor_list->nbss));
        status = E2BIG;
    }
...
int wlan_acs_scan_report(wlan_if_t vaphandle,void *acs_rp)
{
    ...
    acs_r = qdf_mem_malloc(sizeof(struct ieee80211_acs_dbg));
    if (acs_r == NULL) {
        QDF_PRINT_INFO(vaphandle->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY,
             QDF_TRACE_LEVEL_INFO, "%s: Unable to allocate memory\n", __func__);
        return -ENOMEM;
    }

    if (copy_from_user(acs_r, acs_rp, sizeof(struct ieee80211_acs_dbg))) {
        ret_val = -EFAULT;
        goto end;
    }

    if (ieee80211_acs_scan_report(vaphandle->iv_ic, vaphandle, acs_r, 0) == EOK) {
        if (copy_to_user(acs_rp, acs_r, sizeof(struct ieee80211_acs_dbg))) {
           ret_val = -EFAULT;
           goto end;
        }
    }
...
wlan_acs_channel_allowed(wlan_if_t vap, struct ieee80211_ath_channel *c, enum ieee80211_phymode mode)
{
    ...
    /* real phymode */
    if (mode == IEEE80211_MODE_AUTO) {
        modes[0] = IEEE80211_MODE_11AC_VHT160;
        modes[1] = IEEE80211_MODE_11AC_VHT80;
        modes[2] = IEEE80211_MODE_11AC_VHT40PLUS;
        modes[3] = IEEE80211_MODE_11AC_VHT40MINUS;
        modes[4] = IEEE80211_MODE_11AC_VHT20;
        modes[5] = IEEE80211_MODE_11NG_HT40PLUS;
        modes[6] = IEEE80211_MODE_11NG_HT40MINUS;
        modes[7] = IEEE80211_MODE_11NA_HT40PLUS;
        modes[8] = IEEE80211_MODE_11NA_HT40MINUS;
        modes[9] = IEEE80211_MODE_11NG_HT20;
        modes[10] = IEEE80211_MODE_11NA_HT20;
        modes[11] = IEEE80211_MODE_11G;
        modes[12] = IEEE80211_MODE_11A;
        modes[13] = IEEE80211_MODE_11B;
        n_modes = 14;
    } else if (mode == IEEE80211_MODE_11NG_HT40) {
...
int ieee80211_acs_api_update(struct ieee80211com *ic, ieee80211_scan_event_type type, wlan_chan_t chan)
{
    ieee80211_acs_retrieve_chan_info(ic, type, chan);
