...
ath_tx_init(ath_dev_t dev, int nbufs)
{
    ...
    do {
        ...
        if (error != 0) {
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "failed to allocate tx descriptors: %d\n", error);
            break;
        }
        ...
        if (error != 0) {
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "failed to allocate beacon descripotrs: %d\n", error);
            break;
        }
#if UMAC_SUPPORT_WNM
        ...
        if (error != 0) {
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "failed to allocate tim descripotrs: %d\n", error);
            break;
        }
#endif
        ...
        if (error != 0) {
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "failed to allocate UAPSD descripotrs: %d\n", error);
            break;
        }
#endif
        ...
        if (error != 0) {
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "failed to allocate tx status descriptors: %d\n", error);
            break;
        }
        ...
        if (ath_hal_hasmci(sc->sc_ah)) {
            ...
            if (error != 0) {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "failed to allocate MCI buffers: %d\n", error);
                break;
            }
        }
#endif
...
#define    N(a)    (sizeof(a)/sizeof(a[0]))
    ...
    if (qnum >= N(sc->sc_txq)) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "hal qnum %u out of range, max %u!\n",
...
ath_set_tx(struct ath_softc *sc)
{
    if (!ath_tx_setup(sc, HAL_WME_AC_BK)) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "unable to setup xmit queue for BE traffic!\n");
 ...
 */
    ...
    if (sc->sc_wifiposq_oc == NULL) {
        DPRINTF(sc, ATH_DEBUG_CALIBRATE, "%s: unable to setup WIFIPOS  xmit queue!\n",
                __func__);
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d]: unable to setup WIFIPOS  xmit queue!\n", __func__, __LINE__);
    }
    else {
    ...
    if (sc->sc_wifiposq_hc == NULL) {
        DPRINTF(sc, ATH_DEBUG_CALIBRATE, "%s: unable to setup WIFIPOS HC  xmit queue!\n",
                __func__);
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: unable to setup WIFIPOS  xmit queue!\n", __func__);
    }
    else {
    }
   }
...
#define    N(a)    (sizeof(a)/sizeof(a[0]))
    ...
    if (haltype >= N(sc->sc_haltype2q)) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "HAL AC %u out of range, max %zu!\n",
...
ath_txq_update(ath_dev_t dev, int qnum, HAL_TXQ_INFO *qi0)
{
    ...
    if (!ath_hal_settxqueueprops(ah, qnum, &qi)) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: unable to update hardware queue %u!\n",
               __func__, qnum);
        error = -EIO;
    } else {
...
_ath_tx_txqaddbuf(struct ath_softc *sc, struct ath_txq *txq, ath_bufhead *head)
{
    ...
    if (ath_hal_gettxbuf(ah, txq->axq_qnum) == 0) {
        ...
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: FATAL ERROR: NULL TXDP while enabling TX.\n", __func__);
        ASSERT(FALSE);
    } else
#endif
...
#define DESC2PA(_sc, _va)	\
    ...
    if (sc->sc_reg_parm.c3WarTimerPeriod &&
        ...
        if (!sc->c3_war_timer_active) {
            ...
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: sc_c3_war_timer is started!!\n", __func__);
        }
    ...
    if (sc->sc_enhanceddmasupport) {
        ...
        if (txq->axq_depth == HAL_TXFIFO_DEPTH) {
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "[%d] TXQ MAX DEPTH!!!\n", txq->axq_qnum);
            return -ENOMEM;
        }
...
*/
...
#ifdef ATH_SUPPORT_TxBF
                    ath_tx_complete_buf(sc, bf, head, NULL, 0, 0, 0);
#else
                    ath_tx_complete_buf(sc, bf, head, NULL, 0);
#endif
                }
#ifdef ATH_SWRETRY
                if(bf->bf_isswretry) {
        ...
        {
            /* This will cause an NMI since TXDP is 0 */
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: FATAL ERROR: NULL TXDP while enabling TX.\n", __func__);
            ASSERT(FALSE);
        } else
#endif
...
#ifdef ATH_SUPPORT_TxBF
        ath_tx_complete_buf (sc, bf, &bf_head_temp, NULL, 0, 0, 0);
#else
        ath_tx_complete_buf (sc, bf, &bf_head_temp, NULL, 0);
#endif
...
void flush_txq_for_eapol(wbuf_t wbuf, ieee80211_tx_control_t *txctl)
{
...
EXPORT_SYMBOL(flush_txq_for_eapol);
/*
...
#ifdef ATH_SUPPORT_TxBF
void
_ath_tx_complete_buf(struct ath_softc *sc, struct ath_buf *bf, ath_bufhead *bf_q, struct ath_tx_status *ts, int txok, u_int8_t txbf_status, u_int32_t tstamp)
#else
void
_ath_tx_complete_buf(struct ath_softc *sc, struct ath_buf *bf, ath_bufhead *bf_q, struct ath_tx_status *ts, int txok)
#endif
{
    ...
    if (sc->sc_reg_parm.c3WarTimerPeriod && bf->bf_isdata) {
        ...
        if (!atomic_read(&sc->sc_pending_tx_data_frames)) {
            ...
            if (sc->c3_war_timer_active) {
                ...
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: sc_c3_war_timer is stopped!!\n", __func__);
            }
            spin_unlock(&sc->sc_c3_war_lock);
        }
    }
                                      ...
                                      struct ath_tx_status *ts)
{
    ...
    if (bf->bf_state.bfs_is_proberesp == 1) {
        sc->sc_stats.ast_tx_hw_pr_retries += hwretries_estimate;
    }
...
#define VALID_TXBF_RATE(_rate,_nss) ((_rate >= 0x80) && (_rate < ((_nss == 2)?0x90:0x88)))
    ...
    if (ts->ts_status == 0)  {
        ...
        if (sc->sc_hashtsupport) {
            ...
            */
            ...
            sc->sc_stats.ast_tx_rssi_ctl3 = ts->ts_rssi_ctl3;
            ...
            sc->sc_stats.ast_tx_rssi_ext3 = ts->ts_rssi_ext3;
        ...
        if (bf->bf_state.bfs_is_proberesp == 1) {
            sc->sc_stats.ast_tx_hw_pr_success++;
        }
	...
	if (bf->bf_state.bfs_is_proberesp == 1) {
            sc->sc_stats.ast_tx_pr_xretries++;
        }
        ...
        if (sc->sc_enhanceddmasupport) {
            ...
            if (bf == NULL) {
                if (txq->axq_headindex != txq->axq_tailindex)
                    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ath_tx_draintxq: ERR head %d tail %d\n",
                                  ...
                                  uint8_t chainmask, uint8_t rate)
{
    if (sc->sc_enableapm && (sc->sc_curchan.channel_flags & CHANNEL_5GHZ) &&
        ...
        /* if optional chainmask is subset of specified chainmask, use opt one */
...
ath_buf_set_rate(struct ath_softc *sc, struct ath_buf *bf)
{
    ...
    if (an && (an->an_avp->av_ctsrts_ratecode)) {
        ctsrate = an->an_avp->av_ctsrts_ratecode |
        ...
        if (an && an->an_tx_chainmask) {
            ...
            if (sc->sc_tx_numchains > max_tx_numchains) {
                ...
                if (an && ((an->an_smmode == ATH_SM_PWRSAV_STATIC) &&
                    ((bf->bf_rcs[i].flags & (ATH_RC_DS_FLAG | ATH_RC_TS_FLAG | ATH_RC_QS_FLAG)) == 0)))
                {
                    /*
	...
	//Do not enable CTS to Self for positioning frames
         ...
         */
        if (dynamic_mimops &&
                (bf->bf_rcs[i].flags & (ATH_RC_DS_FLAG | ATH_RC_TS_FLAG | ATH_RC_QS_FLAG)))
        {
            series[i].RateFlags |= HAL_RATESERIES_RTS_CTS;
        }
    }
    ...
    if (bf->bf_flags & HAL_TXDESC_POS_TXCHIN_4)
    {
        wifipos_tmpflags = bf->bf_flags & HAL_TXDESC_POS_TXCHIN_4;
        flags = flags | wifipos_tmpflags;
    }
#endif
...
void ath_print_tbf_usage(struct ath_softc *sc)
{
    ...
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Txq Id\taxq_depth\taxq_fifo\ttid_bufq\tpaused_tid\tbar_paused\n");
    for (i = 0; i < HAL_NUM_TX_QUEUES; i++) {
        ...
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "txq[%d]\t%d\t\t%d\t\t%d\t\t0x%x\t\t0x%x\n", i,
               txq->axq_depth, num_axq_fifo, num_tid_bufq, paused, bar_paused);
        sum += num_tid_bufq;
    }
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Total: %d\n", sum);
}
#endif
