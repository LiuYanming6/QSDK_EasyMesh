...
osif_send_to_nss(os_if_t osif, struct sk_buff *skb, int nwifi)
{
     ...
     */
    if (osifp->iv_txrx_handle && osifp->iv_txrx_handle->carrier_vow_config) {
        if (osifp->iv_vap_classify)
            osifp->iv_vap_classify(osifp->iv_txrx_handle, skb, rx_direction, &nbuf_class);

        if (nbuf_class.pkt_tid == QDF_TID_VI)
            PFLOW_CTRL_PDEV_VIDEO_STATS_ADD(osifp->iv_txrx_handle->pdev, RX_VIDEO_TID_MSDU_DELIVERED_TO_STACK, 1);
