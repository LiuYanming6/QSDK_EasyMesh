                                ...
                                void *rx_mpdu_desc )
{
    ...
    if (ni && ni != ni->ni_bss_node) {
        if (ieee80211_vap_wnm_is_set(ni->ni_vap) && ieee80211_wnm_bss_is_set(ni->ni_vap->wnm)) {
...
static inline void
ol_rx_aggregate_bukt_stats_update(ol_txrx_pdev_handle pdev,
...
static inline void
ol_rx_amsdu_bukt_stats_update(ol_txrx_pdev_handle pdev,
...
static inline void
ol_rx_mcast_stats_update(ol_txrx_pdev_handle pdev,
        struct ol_txrx_peer_t *peer, struct ol_txrx_nbuf_classify *nbuf_class, u_int32_t npackets)
{
    if (nbuf_class->pkt_tid == QDF_TID_VI) {
        /* Increment total VI packets  */
        PFLOW_CTRL_PDEV_VIDEO_STATS_ADD(pdev, RX_VIDEO_TID_MSDU_TOTAL_FROM_FW, npackets);

        /* Increment multicast VI packets  */
        if (nbuf_class->is_mcast) {
            PFLOW_CTRL_PDEV_VIDEO_STATS_ADD(pdev, RX_VIDEO_TID_MSDU_MCAST_FROM_FW, 1);
        }
    }

    if (nbuf_class->is_mcast) {
        PFLOW_TXRX_TIDQ_STATS_ADD(peer, nbuf_class->pkt_tid, RX_MSDU_MCAST_FROM_FW, 1);
    }

    ol_rx_amsdu_bukt_stats_update(pdev, peer, nbuf_class->pkt_tid, npackets);
    PFLOW_TXRX_TIDQ_STATS_ADD(peer, nbuf_class->pkt_tid, RX_MSDU_TOTAL_FROM_FW, npackets);
}

#ifdef ATH_SUPPORT_RDKB
void
wlanstats_inc_rx_rssi(struct ieee80211com *ic,
                    struct wlan_rx_stats *stats,
                    void *rxd,
                    int rix)
{
    ...
    if (qdf_unlikely(WARN_ON_ONCE(qdf_unlikely(!ar_rx_ops->get_rssi))))
        return;
...
static void
ol_wlanstats_inc_rx_stats(struct ieee80211com *ic,
        ...
        int msdu_chaining)
{
    ...
    if (!wlanstats_svc_is_enabled(ic, WLAN_UAPI_IOCTL_SVC_PEER_RX_STATS))
        return;
    ...
    if (qdf_unlikely(WARN_ON_ONCE(qdf_unlikely(!ar_rx_ops->is_data))) ||
            qdf_unlikely(WARN_ON_ONCE(qdf_unlikely(!ar_rx_ops->is_first_mpdu))) ||
            qdf_unlikely(WARN_ON_ONCE(qdf_unlikely(!ar_rx_ops->is_retry))) ||
            qdf_unlikely(WARN_ON_ONCE(qdf_unlikely(!ar_rx_ops->is_fcs_err))) ||
            qdf_unlikely(WARN_ON_ONCE(qdf_unlikely(!ar_rx_ops->get_cmn_ri))))
        return;
    ...
    if (first_mpdu) {
        ...
        ic->wlanstats_rix = wlanstats_cmn_calc_rix(&ri);
        ic->wlanstats_sgi = ri.sgi;
        ic->wlanstats_first_mpdu = 1;
    }
    ...
    rix = ic->wlanstats_rix;

    if (ic->wlanstats_first_mpdu) {
        ic->wlanstats_first_mpdu = 0;
        ...
        wlanstats_inc_rx_rssi(ic, stats, rxd, rix);
    ...
    int num_mpdu_ranges)
{
...
#ifdef ATH_SUPPORT_RDKB
    if (htt_rx_ind_flush(rx_ind_msg) && peer && vdev) {
#else
    ...
    for (mpdu_range = 0; mpdu_range < num_mpdu_ranges; mpdu_range++) {
        ...
        struct ol_txrx_nbuf_classify nbuf_class = {0};
                ...
                if (scn->carrier_vow_config) {
                    ...
                    while (temp_msdu) {
                        ...
                        if (pdev->delay_counters_enabled)
                            __net_timestamp(temp_msdu);

                        if (nbuf_class.is_mcast && (tid >= OL_TX_PFLOW_CTRL_MAX_TIDS))
                            n_mcast_packets++;
                        else if (tid < OL_TX_PFLOW_CTRL_MAX_TIDS) {
...
out_stats:
                    ...
                    if (n_mcast_packets) {
                        ol_rx_mcast_stats_update(pdev, peer, &nbuf_class, n_mcast_packets);
                    }
                } /* carrier_vow_config */

#if ATH_SUPPORT_RDKB
                if (IS_WLANSTATS_ENABLED(ic)) {
                    ol_wlanstats_inc_rx_stats(ic,
                            ni,
                            head_msdu,
                            tail_msdu,
                            npackets,
                 ...
                 */
                if (ath_is_directlink((struct ol_txrx_pdev_t*)pdev)) {
                    rx_mpdu_desc = htt_rx_msdu_desc_retrieve(htt_pdev, head_msdu);
                } else
...
#if UMAC_SUPPORT_WNM
                if ((vdev != NULL) && (vdev->opmode != wlan_op_mode_sta)) {
                ...
                if (vdev && (OL_CFG_NONRAW_RX_LIKELINESS(
		           ...
		           while (1) {
                        qdf_nbuf_t next;
                        if (vdev && scn->carrier_vow_config) {
            ...
            for (i = 0; i < num_mpdus; i++) {
...
#if ATH_SUPPORT_RDKB
                if (IS_WLANSTATS_ENABLED(ic) && npackets) {
                    ol_wlanstats_inc_rx_stats(ic,
                            ni,
                            msdu,
                            tail_msdu,
                 ...
                 * call partner API for get rx descriptor.
                 */
                if (ath_is_directlink((struct ol_txrx_pdev_t*)htt_pdev->txrx_pdev)) {
                    rx_mpdu_desc = htt_rx_msdu_desc_retrieve(htt_pdev,msdu);
                } else
#endif /* QCA_PARTNER_DIRECTLINK */
                {
...
normalproc:
                ...
                /* Concentrate only on ampdu last subframe and last subframe known
                   for this we are using 10th and 11th bits of rs_flags */
                ...
                if ((status == htt_rx_status_ctrl_mgmt_null) && (peer == NULL)) {
                    ...
                    if (len >= sizeof(struct ieee80211_frame)) {
                        if ((type == IEEE80211_FC0_TYPE_DATA) && (subtype == IEEE80211_FC0_SUBTYPE_NODATA)) {
                            /*
                             * Check for FCS error bit and Null data bit in Rx Attention word.
                             * Send frames only with no fcs error
                             */
                            if (!(htt_pdev->ar_rx_ops->is_fcs_err(rx_desc))) {
                                qdf_nbuf_t next = qdf_nbuf_next(msdu);
                                msdu->next = NULL;
                                ol_ath_mgmt_null_handler(scn, msdu, &rx_status);
                                msdu = next ;
                                while (msdu) {
                                    qdf_nbuf_t next = qdf_nbuf_next(msdu);
                                    qdf_nbuf_free(msdu);
...
//TODO Need to debug why it is crashing in QVIT
        ...
        if (htt_rx_msdu_first_msdu_flag(htt_pdev, rx_desc) && peer) {
