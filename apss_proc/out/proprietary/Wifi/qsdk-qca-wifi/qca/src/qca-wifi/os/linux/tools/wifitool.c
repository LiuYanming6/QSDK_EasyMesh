...
usage(void)
{
    ...
    fprintf(stderr, "cmd: [get_rrmutil] \n");
    ...
    fprintf(stderr, "cmd: [csa_deauth_mode] \n");
    fprintf(stderr, "cmd: [g_csa_deauth_mode] \n");
    fprintf(stderr, "cmd: [filter_ssid] \n");
    fprintf(stderr, "cmd: [g_filter_ssid] \n");
    fprintf(stderr, "cmd: [cfr_capture] \n");
    fprintf(stderr, "cmd: [cfr_list] \n");
#if UMAC_SUPPORT_VI_DBG
    fprintf(stderr, "cmd: [vow_debug_set_param] \n");
    fprintf(stderr, "cmd: [vow_debug] \n");
#endif    
#if ATH_ACL_SOFTBLOCKING
    fprintf(stderr, "cmd: [softblocking dstmac enable_flag] \n");
    fprintf(stderr, "cmd: [softblocking_get dstmac] \n");
#endif
#if ATH_SUPPORT_PRIMARY_ALLOWED_CHAN
    fprintf(stderr, "cmd: [setprimarychans ch1 ch2 ... chn] \n");
#endif
	exit(-1);
}
...
usage_cfr_list(void)
{
    fprintf(stderr, "usage: wifitool athX cfr_list <mac_addr> \n");
    exit(-1);
}

static void
usage_cfr_capture(void)
{
    fprintf(stderr, "usage: wifitool athX cfr_capture <mac_addr> <request> <bw> <period> <method> \n");
    fprintf(stderr, "       <bw> <period> <method> not significant when <request> is 0 \n");
    exit(-1);
}
...
#if ATH_SUPPORT_PRIMARY_ALLOWED_CHAN
static void set_primary_allowed_chanlist(const char *ifname, int argc, char *argv[])
{
    struct iwreq iwr;
    int s, i;
    struct ieee80211req_athdbg req;
...
static void get_primary_allowed_chanlist(const char *ifname, int argc, char *argv[])
{
#define MAX_PARAM  3
    struct iwreq iwr;
    int s, i;
...
#define MIN_CTRL_PARAM_COUNT  4
    ...
    req.data.param[0] = (u_long)&ctrl_tbl; /*typecasting to avoid warning */
...
/**
 * @brief To display the MAC address of the RRM or BTM capable STA
 *
 * @return
 */
static void get_sta_list(const char *ifname, int argc, char *argv[], int sta_type)
{
...
#define MAX_PARAM 3
    ...
    ieee80211_sta_info_t *sta_stats;
    if (argc > MAX_PARAM)
    {
        if (sta_type == RRM_STA)
            usage_rrmstalist();
    ...
    iwr.u.data.pointer = (void *) &req;
    iwr.u.data.length = (sizeof(struct ieee80211req_athdbg));
    sta_stats = (void *)malloc(sizeof(ieee80211_sta_info_t));
    if (sta_type == RRM_STA)
        req.cmd = IEEE80211_DBGREQ_GET_RRM_STA_LIST;
    else
        req.cmd = IEEE80211_DBGREQ_GET_BTM_STA_LIST;
    req.data.param[0] = (u_long)sta_stats;
    ...
    if (ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
        perror("set STA list failed \n");
        goto MEMORY_FAIL;
    }
    if(sta_stats->count > MAX_CLIENTS) {
        perror("can not display long list __investigate__\n");
        goto MEMORY_FAIL;
    }
    addr_list = (u_int8_t *) malloc(IEEE80211_ADDR_LEN * (sta_stats->count));
    if(addr_list == NULL) {
        perror("Memory allocation failed  __investigate__\n");
        goto MEMORY_FAIL;
    }
    count = sta_stats->count;
    req.data.param[1] = (u_long) addr_list; /*typecasted to avoid warning */
    memset(req.dstmac,0x00,IEEE80211_ADDR_LEN);
    if (ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
        perror("STA LIST failed \n");
        free(addr_list);
        goto MEMORY_FAIL;
    }
    printf("\t %s Capable station's Mac address ", (sta_type==RRM_STA)?"RRM":"BTM");
...
/**
 * @brief To display the MAC address of the BTM capable STA
 *
 * @return
 */
static void btm_sta_list(const char *ifname, int argc, char *argv[])
{
...
#define MAX_PARAM 3
    ...
    req.data.param[0] = (u_long)node_info;
    memset(req.dstmac,0xff,IEEE80211_ADDR_LEN); /* Assigning broadcast address to get the STA count */
    ...
    req.data.param[1] = (u_long)addr_list; /*typecasted to avoid warning */
    req.data.param[3] = (u_long)noise_stats;
    memset(req.dstmac,0x00,IEEE80211_ADDR_LEN);
    if (ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
...
#define ATH_NUM_TID 17
    ...
    struct ieee80211req_athdbg req;
    int ret, i;
    struct atf_nodestate *atfnodestate = NULL;
    ...
    atfnodestate = malloc(sizeof(struct atf_nodestate));
    if (!atfnodestate) {
        printf("out of memory\n");
        close(sock);
        return;
    }
    memset(atfnodestate, 0, sizeof(struct atf_nodestate));
    req.data.atf_dbg_req.ptr = atfnodestate;
    req.data.atf_dbg_req.size = (sizeof(struct atf_nodestate));
    ...
    for (i = 0; i < WME_NUM_AC; i++) {
        if (atfnodestate->subgroup_name) {
            printf("   %d           %s \n", i, atfnodestate->subgroup_name[i]);
        }
    }
    printf("\n");

    printf("Node state : 0x%08x\n", atfnodestate->nodestate);
    for (i = 0; i < ATH_NUM_TID; i++)
    {
        if( (atfnodestate->nodestate >> i) & 0x1)
        {
            printf("tid : %d paused \n", i);
        }
    }
    printf("ATF TID Bitmap: 0x%08x\n", atfnodestate->atf_tidbitmap);
    close(sock);
#endif
}

static void
atf_get_group_subgroup(const char *ifname, struct atfgrouplist_table *list_group)
{
#if QCA_AIRTIME_FAIRNESS
    struct iwreq iwr;
    int sock;
...
static void atf_dump_debug(const char *ifname, int argc, char *argv[])
{
    ...
    for (grp = 0; grp < list_group.info_cnt; grp++) {
        for (sg = 0; sg < list_group.atf_list[grp].num_subgroup; sg++) {
            ...
            printf("====================\n");
            printf("                     total   actual                    total                         max      min     nobuf   txbufs  txbytes                                        contribute\n");
            printf("    time    allot    allot    allot   unused  contrib  contrib   borrow    allow     held     held     drop     sent     sent        up     wup     raw-tok max-tput  level   TID-state\n");
            for (i = 0; i < size; i++) {
...
static void acs_report(const char *ifname, int argc, char *argv[])
{
    ...
    acs->neighbor_list = (void *)malloc(sizeof(ieee80211_neighbor_info) * IEEE80211_MAX_NEIGHBOURS);
    if(NULL == acs->neighbor_list) {
        printf("insufficient memory \n");
        goto cleanup;
    }
    fprintf (stdout, "wifitool: acs->nchans: %d\n", acs->nchans);
    fprintf(stdout," Chan|chpool|noisefloor|radarNF|maxrssi|!80211NF|util|utiltotal|busy|busytx|busyrx|busyself|busyext\n");
    fprintf(stdout,"-----------------------------------------------------------------------------------------------------\n");
    ...
    for (i = 0; i < acs->nchans; i++) {
        ...
        fprintf(stdout," %4d    %4d    %4d    %4d    %4d    %4d    %4d    %4d    %4d    %5d    %5d    %5d    %5d\n",
                acs->ieee_chan,
                acs->chan_in_pool,
                acs->noisefloor,
                acs->chan_radar_noise,
                acs->chan_maxrssi,
                acs->chan_non_80211_noise,
                acs->chan_utilization,
                acs->chan_utilization_total,
                acs->chan_utilization_busy,
                acs->chan_utilization_busy_tx,
                acs->chan_utilization_busy_rx,
                acs->chan_utilization_busy_self,
                acs->chan_utilization_busy_ext);
    }

    memset(acs->neighbor_list, 0, sizeof(ieee80211_neighbor_info) * IEEE80211_MAX_NEIGHBOURS);
    acs->neighbor_size = sizeof(ieee80211_neighbor_info) * IEEE80211_MAX_NEIGHBOURS;

    fprintf(stdout,"---------------------------------------------------------------------\n");
    fprintf(stdout, "Additonal Channel Stat \n");
    fprintf(stdout,"---------------------------------------------------------------------\n");
    ...
    for (i = 0; i < acs->nchans; i++) {
        acs->entry_id = i;
        req.cmd = IEEE80211_DBGREQ_GETACSREPORT;
        ret = ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr);
        ...
        for (j = 0; j <  acs->chan_nbss; j++) {
            fprintf(stdout," %d                %3d     %d      %s      %02x:%02x:%02x:%02x:%02x:%02x           %d      %d    \n",
               j+1,
                acs->ieee_chan,
                acs->chan_nbss,
...
static void get_rrmutil(const char *ifname, int argc, char *argv[])
{
        ...
        rrmutil = (void *)malloc(sizeof(ieee80211_rrmutil_t));
        if(rrmutil == NULL){
           printf("Insufficient memory \n");
           close(s);
           return;
        ...
        req.data.tr069_req.data_addr = rrmutil;
        req.data.tr069_req.data_size = sizeof(ieee80211_rrmutil_t);
        iwr.u.data.pointer = (void *) &req;
        iwr.u.data.length = (sizeof(struct ieee80211req_athdbg));
        if (ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
            perror("get_rrmutil  failed \n");
            free(rrmutil);
            close(s);
            return;
        }
...
static void getapMacAddressControlMode(const char *ifname, int argc, char *argv[])
{
    struct iwreq iwr = { 0 };
    struct ieee80211req_athdbg req = { 0 };
    int s = 0;
    ...
    if (argc != 3) {
        fprintf(stderr, "usage: wifitool athX get_acl_mode \n");
        return;
    }
    memset(&req, 0, sizeof(struct ieee80211req_athdbg));
    s = socket(AF_INET, SOCK_DGRAM, 0);
    ...
    acl_mode = (void *)malloc(sizeof(int));
    if (acl_mode == NULL) {
        printf("insufficient memory");
        close(s);
        return;
    ...
    *acl_mode  = 0;
    ...
    req.cmd = IEEE80211_DBGREQ_TR181;
    req.data.tr181_req.cmdid = TR181_GET_ACL_MODE;
    req.data.tr181_req.data_addr = acl_mode;
    req.data.tr181_req.data_size = sizeof(int);
    iwr.u.data.pointer = (void *) &req;
    iwr.u.data.length = (sizeof(struct ieee80211req_athdbg));
    if (ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
        perror("tr181_get_acl_mode  failed \n");
        free(acl_mode);
        close(s);
        return;
    }
    ...
    if (*acl_mode == 0)
        ...
        printf("Filter as whitelist\n");
    else if (*acl_mode == 2)
        printf("Filter as blacklist\n");
    else
        printf("Unknown mode\n");
...
static void wifi_pushRadioChannel2(const char *ifname, int argc, char *argv[])
{
#define NUM_OF_ARGUMENTS 3
    ...
    if (argc != 6) {
        fprintf(stderr, "usage: wifitool athX push_radio_channel2 <channel> <channel_width> <beacon_count>\n");
        return;
    }
    memset(&req, 0, sizeof(struct ieee80211req_athdbg));
    s = socket(AF_INET, SOCK_DGRAM, 0);
    ...
    args = (int *) malloc (sizeof(int) * NUM_OF_ARGUMENTS);
    if (args == NULL) {
        printf("insufficient memory");
        close(s);
        return;
    ...
    args[0] = atoi(argv[3]);  // Channel
    args[1] = atoi(argv[5]);  // Beacon count
    args[2] = atoi(argv[4]);  // Channel width

    req.cmd = IEEE80211_DBGREQ_TR181;
    req.data.tr181_req.cmdid = TR181_PUSH_RADIO_CHANNEL2;
    req.data.tr181_req.data_addr = args;
    req.data.tr181_req.data_size = sizeof(int) * NUM_OF_ARGUMENTS;
    iwr.u.data.pointer = (void *) &req;
    iwr.u.data.length = (sizeof(struct ieee80211req_athdbg));
    if (ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
        perror("tr181 push radio channel2  failed \n");
        free(args);
        close(s);
        return;
    }
    ...
    close(s);
    return;
#undef NUM_OF_ARGUMENTS
}

static void wifi_getCsaDeauthMode(const char *ifname, int argc, char *argv[])
{
    struct iwreq iwr;
    struct ieee80211req_athdbg req = { 0 };
    int s = 0;
    ...
    if (argc != 3) {
        fprintf(stderr, "usage: wifitool athX g_csa_deauth_mode \n");
        return;
    }
    ...
    req.cmd = IEEE80211_DBGREQ_TR181;
    req.data.tr181_req.cmdid = TR181_GET_CSA_DEAUTH_MODE;
    req.data.tr181_req.data_addr = &mode;
    req.data.tr181_req.data_size = sizeof(int);
    iwr.u.data.pointer = (void *) &req;
    iwr.u.data.length = (sizeof(struct ieee80211req_athdbg));
    if (ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
        perror("tr181 get deauth mode csa failed \n");
        close(s);
        return;
    }

    printf(" %s:      CSA_DEAUTH_MODE :%d\n", argv[1], mode);

    close(s);
    return;
}

static void wifi_getFilterSsid(const char *ifname, int argc, char *argv[])
{
    struct iwreq iwr;
    struct ieee80211req_athdbg req;
    int s = 0, len;
    char filter_ssid[IEEE80211_NWID_LEN] = {0};

    if (argc != 3) {
        fprintf(stderr, "usage: wifitool athX g_filter_ssid \n");
        return;
    }
    ...
    req.cmd = IEEE80211_DBGREQ_TR181;
    req.data.tr181_req.cmdid = TR181_GET_FILTER_SSID;
    req.data.tr181_req.data_addr = filter_ssid;
    req.data.tr181_req.data_size = IEEE80211_NWID_LEN;
    iwr.u.data.pointer = (void *) &req;
    iwr.u.data.length = (sizeof(struct ieee80211req_athdbg));
    if (ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
        perror("tr181 get filter ssid  failed \n");
    }

    printf(" %s:     FILTER_SSID :   \"%s\"  \n", argv[1], filter_ssid);

    close(s);
    return;
}

static void wifi_csaDeauthMode(const char *ifname, int argc, char *argv[])
{
    struct iwreq iwr = { 0 };
    struct ieee80211req_athdbg req = { 0 };
    int s = 0;
    ...
    if (argc != 4) {
        fprintf(stderr, "usage: wifitool athX csa_deauth_mode < 0 | 1 | 2 > \n");
        return;
    }
    ...
    args[0] = atoi(argv[3]);  // mode

    req.cmd = IEEE80211_DBGREQ_TR181;
    req.data.tr181_req.cmdid = TR181_SET_CSA_DEAUTH_MODE;
    req.data.tr181_req.data_addr = args;
    req.data.tr181_req.data_size = sizeof(int) * 3;
    iwr.u.data.pointer = (void *) &req;
    iwr.u.data.length = (sizeof(struct ieee80211req_athdbg));
    if (ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
        perror("tr181 set deauth mode csa failed \n");
        free(args);
        close(s);
        return;
    }
...
static void wifi_setFilterSsid(const char *ifname, int argc, char *argv[])
{
    struct iwreq iwr = { 0 };
    struct ieee80211req_athdbg req = { 0 };
    int s = 0, len;
    char filter_ssid[IEEE80211_NWID_LEN];

    if (argc != 4) {
        fprintf(stderr, "usage: wifitool athX filter_ssid <ssid_string> \n");
        return;
    }
    ...
    if (strlcpy(filter_ssid, argv[3], sizeof(filter_ssid)) >= sizeof(filter_ssid)) {
        fprintf(stderr, "filter_ssid too long: %s\n", argv[3]);
        close(s);
        return;
    }

    req.cmd = IEEE80211_DBGREQ_TR181;
    req.data.tr181_req.cmdid = TR181_SET_FILTER_SSID;
    req.data.tr181_req.data_addr = filter_ssid;
    req.data.tr181_req.data_size = sizeof(int) * 3;
    iwr.u.data.pointer = (void *) &req;
    iwr.u.data.length = (sizeof(struct ieee80211req_athdbg));
    if (ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
        perror("tr181 set filter ssid  failed \n");
    }
...
/*
 * @Set cfr capture params per sta given the input sta mac address.
 *  Sets the feature, bw, period and method for the node with the given mac address.
 * @param ifname : interface name
 * @param argc   : argument count
 * @param argv   : argument value
 */
static void
cfr_capture(const char *ifname, int argc, char *argv[])
{
    struct iwreq iwr;
    int s, len;
    struct ieee80211req_athdbg req;
    ieee80211_cfr_param_info *cfrinfo = &req.data.cfrinfo;

    if ((argc < 5) || (atoi(argv[4]) && argc != 8)) {
        usage_cfr_capture();
    }
    else {
        memset(&req, 0, sizeof(struct ieee80211req_athdbg));
        ...
        if (strlcpy(iwr.ifr_name, ifname, sizeof(iwr.ifr_name)) >= sizeof(iwr.ifr_name)) {
            fprintf(stderr, "%s ifname too long:%s\n",__func__,ifname);
            close(s);
            return;
        }
        req.cmd = IEEE80211_DBGREQ_CFR_SET_PARAMS;
        ...
        cfrinfo->cfr_request = atoi(argv[4]);
        if (atoi(argv[4])) {
            cfrinfo->cfr_bw      = atoi(argv[5]);
            cfrinfo->cfr_period  = atoi(argv[6]);
            cfrinfo->cfr_method  = atoi(argv[7]);
        }
...
#define MAX_POSSIBLE_RATES 256
    ...
    ratelist = (int *) malloc (sizeof(int) * MAX_POSSIBLE_RATES);
    ...
    memset(ratelist, 0, sizeof(int) * MAX_POSSIBLE_RATES);
    req.data.tr069_req.data_addr = ratelist;
    req.data.tr069_req.cmdid = TR069_GET_POSIBLRATE;
    req.data.tr069_req.data_size = sizeof(int) * MAX_POSSIBLE_RATES;
...
static void
cfr_list(const char *ifname, int argc, char *argv[])
{
    struct iwreq iwr;
    struct ieee80211req_athdbg req;
    int s = 0;
    ...
    req.cmd = IEEE80211_DBGREQ_CFR_GET_PARAMS;
    if (!wifitool_mac_aton(argv[3], &req.dstmac[0], 6)) {
        errx(1, "Invalid mac address");
        close(s);
        return;
    ...
    if (ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
        errx(1, "IEEE80211_IOCTL_DBGREQ:"
                "IEEE80211_DBGREQ_CFR_GET_CLI_PARAMS");
        close(s);
        return;
    }

    printf("For STA %s -> Request:%d Bandwidth:%d Period:%d Method:%d \n",argv[3],req.data.cfrinfo.cfr_request,
                req.data.cfrinfo.cfr_bw,req.data.cfrinfo.cfr_period,req.data.cfrinfo.cfr_method);
    close(s);
}
...
#if ATH_ACL_SOFTBLOCKING
static void
set_acl_softblocking(const char *ifname, int argc, char *argv[])
{
    struct iwreq iwr = { 0 };
    struct ieee80211req_athdbg req = { 0 };
    int s = 0;
    ...
    if (ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
        errx(1, "IEEE80211_IOCTL_DBGREQ:"
                "IEEE80211_DBGREQ_SET_SOFTBLOCKING failed. "
                "No entry with the given mac in the ACL list");
        close(s);
        return;
    }
    close(s);
...
static void
get_acl_softblocking(const char *ifname, int argc, char *argv[])
{
    struct iwreq iwr = { 0 };
    struct ieee80211req_athdbg req = { 0 };
    int s = 0;
    ...
    if (ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
        errx(1, "IEEE80211_IOCTL_DBGREQ:"
                "IEEE80211_DBGREQ_GET_SOFTBLOCKING failed");
        close(s);
        return;
    }
...
main(int argc, char *argv[])
{
    ...
    } else if (streq(cmd, "cfr_capture")) {
        cfr_capture(ifname, argc, argv);
    } else if (streq(cmd, "cfr_list")) {
        cfr_list(ifname, argc, argv);
...
#endif
    } else if (streq(cmd, "get_acl_mode")) {
        getapMacAddressControlMode(ifname, argc, argv);
    } else if (streq(cmd, "push_radio_channel2")) {
        wifi_pushRadioChannel2(ifname, argc, argv);
    } else if (streq(cmd, "csa_deauth_mode")) {
        wifi_csaDeauthMode(ifname, argc, argv);
    } else if (streq(cmd, "filter_ssid")) {
        wifi_setFilterSsid(ifname, argc, argv);
    } else if (streq(cmd, "g_csa_deauth_mode")) {
        wifi_getCsaDeauthMode(ifname, argc, argv);
    } else if (streq(cmd, "g_filter_ssid")) {
        wifi_getFilterSsid(ifname, argc, argv);
#if ATH_SUPPORT_PRIMARY_ALLOWED_CHAN
