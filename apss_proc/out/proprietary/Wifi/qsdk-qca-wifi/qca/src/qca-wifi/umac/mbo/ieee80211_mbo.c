...
int ieee80211_check_non_oce_ap(void *arg, wlan_scan_entry_t se)
{
	ieee80211_oce_scan_cbinfo_t *cb_info = (ieee80211_oce_scan_cbinfo_t *) arg;
	struct ieee80211_ath_channel *chan = ieee80211_scan_entry_channel(se);
	bool oce_cap = false;
	u_int8_t *mboie;
	...
	if (ieee80211_scan_entry_age(se) > (cb_info->age_timeout * 1000))
		return EOK;

	mboie = ieee80211_scan_entry_mboie(se);
	if (mboie) {
 ...
 */
int ieee80211_update_non_oce_ap_presence (struct ieee80211vap *vap)
{
	...
	status = ieee80211_scan_table_iterate(ic->ic_scan_table, ieee80211_check_non_oce_ap, &cb_info);
...
int ieee80211_check_11b_ap(void *arg, wlan_scan_entry_t se)
{
	ieee80211_oce_scan_cbinfo_t *cb_info = (ieee80211_oce_scan_cbinfo_t *) arg;
	struct ieee80211_ath_channel *chan = ieee80211_scan_entry_channel(se);
	u_int32_t phymode;
 ...
 */
int ieee80211_update_11b_ap_presence (struct ieee80211vap *vap)
{
	struct ieee80211com *ic = vap->iv_ic;
	ieee80211_oce_t oce = vap->oce;
	struct ieee80211vap *nbr_vap, *tvap;
	...
	/* Go through scan list */
	status = ieee80211_scan_table_iterate(ic->ic_scan_table, ieee80211_check_11b_ap, &cb_info);
...
int ieee80211_fill_rnr_entry(void *arg, wlan_scan_entry_t se)
{
	ieee80211_rnr_cbinfo_t *cb_info = (ieee80211_rnr_cbinfo_t *) arg;
	...
	for (i = 0; i < MAX_RADIO_CNT; i++) {
		GLOBAL_IC_LOCK(vap->iv_ic->ic_global_list);
		nbr_ic = ic->ic_global_list->global_ic[i];
		GLOBAL_IC_UNLOCK(vap->iv_ic->ic_global_list);

		if (!nbr_ic || TAILQ_EMPTY(&nbr_ic->ic_vaps))
			continue;

		TAILQ_FOREACH_SAFE(nbr_vap, &nbr_ic->ic_vaps, iv_next, tvap) {
			if (!OS_MEMCMP(nbr_vap->iv_myaddr, ieee80211_scan_entry_bssid(se), IEEE80211_ADDR_LEN))
				return EOK;
		}
	}

	scan_ssid = ieee80211_scan_entry_ssid(se, &scan_ssid_len);
	country_ie = (struct ieee80211_country_ie *) ieee80211_scan_entry_country(se);
	chan = ieee80211_scan_entry_channel(se);
	if (country_ie) {
		ap_info->op_class = regdmn_get_opclass (country_ie->cc, chan);
	} else {
		ap_info->op_class = regdmn_get_opclass (vap->iv_ic->ic_country_iso, chan);
	}
	...
	IEEE80211_ADDR_COPY (ap_info->tbtt_info[0].bssid, ieee80211_scan_entry_bssid(se));
...
int ieee80211_update_rnr (struct ieee80211vap *vap)
{
	struct ieee80211com *ic = vap->iv_ic;
	...
	for (i = 0; i < MAX_RADIO_CNT; i++) {
		...
		if (tbtt_info_cnt) {
			...
			ap_info->op_class = regdmn_get_opclass (nbr_vap->iv_ic->ic_country_iso, nbr_vap->iv_ic->ic_curchan);
	...
	for (i = 0; i < MAX_RADIO_CNT; i++) {
		...
		status = ieee80211_scan_table_iterate(nbr_ic->ic_scan_table, ieee80211_fill_rnr_entry, &cb_info);
		frm = cb_info.ap_info;
	...
	if (ieee80211_bg_scan_enabled(vap) && (status == EOK))
...
int ieee80211_add_ap_chan_rpt_entry(struct ieee80211vap *vap, int op_class, int channel)
{
	int status = EOK;
	int i;
	int j;
...
int ieee80211_fill_ap_chan_rpt_entry(void *arg, wlan_scan_entry_t se)
{
	ieee80211_acr_cbinfo_t *cb_info = (ieee80211_acr_cbinfo_t *) arg;
	struct ieee80211vap *vap = cb_info->vap;
	...
	if (country_ie) {
		...
		op_class = regdmn_get_opclass (vap->iv_ic->ic_country_iso, chan);
	}
...
int ieee80211_update_ap_chan_rpt (struct ieee80211vap *vap)
{
	struct ieee80211com *ic = vap->iv_ic;
	struct ieee80211com *nbr_ic;
	struct ieee80211vap *nbr_vap, *tvap;
	...
	for (i = 0; i < MAX_RADIO_CNT; i++) {
		...
		TAILQ_FOREACH_SAFE(nbr_vap, &nbr_ic->ic_vaps, iv_next, tvap) {
			...
			ap_chan_info[ap_chan_info_cnt].op_class = regdmn_get_opclass (nbr_vap->iv_ic->ic_country_iso, nbr_vap->iv_ic->ic_curchan);
	...
	for (i = 0; i < MAX_RADIO_CNT; i++) {
		...
		status = ieee80211_scan_table_iterate(nbr_ic->ic_scan_table, ieee80211_fill_ap_chan_rpt_entry, &cb_info);
...
u_int8_t *ieee80211_add_ap_chan_rpt_ie (u_int8_t *frm, struct ieee80211vap *vap)
{
	...
	if (vap->ap_chan_rpt_cnt) {
		...
		*frm++ = IEEE80211_ELEMID_AP_CHAN_RPT;
		*frm++ = 1;
		*frm++ = regdmn_get_opclass (vap->iv_ic->ic_country_iso, vap->iv_ic->ic_curchan);
	}
