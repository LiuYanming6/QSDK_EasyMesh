...
ath_set_config(ath_dev_t dev, ath_param_ID_t ID, void *buff)
{
    ...
    switch(ID)
    {
        ...
        case ATH_PARAM_AGGR_BURST_DURATION:
            if (sc->sc_aggr_burst) {
                ...
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Error: enable bursting before setting duration\n");
            }
        break;
#endif
        ...
        case ATH_PARAM_ATH_3WIRE_BT_COEX:
        {
            ...
            if (val == 1) {
                ...
                if (!ath_hal_enable_basic_3wire_btcoex(sc->sc_ah)) {
                    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "BT_COEX 3WIRE MODE Enabled \n");
                    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "WL Weight: 0x%lx BT Weight:0x%lx\n",
                            ah->ah_bt_coex_wl_weight,ah->ah_bt_coex_bt_weight);
                } else {
                    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " BT_COEX not supported for this Radio \n");
                }
            } else if (val == 0) {
                sc->sc_3wire_bt_coex_enable = val;
                ah->ah_3wire_bt_coex_enable = val;
                if (!ath_hal_disable_basic_3wire_btcoex(sc->sc_ah)) {
                    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " BT_COEX 3WIRE MODE Disasbled \n");
                } else {
                    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " BT_COEX not supported for this Radio \n");
                }
            } else {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Enter: 1 to Enable BT_COEX , 0 to Disable the BT_COEX \n");
            }
        }
        break;
        ...
        case ATH_PARAM_ATH_3WIRE_BT_WEIGHT:
            ...
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "bt_weight = 0x%lx\n", ah->ah_bt_coex_bt_weight);
        break;

        case ATH_PARAM_ATH_3WIRE_WL_WEIGHT:
            bt_weight = *(unsigned long *)buff;
            ah->ah_bt_coex_wl_weight = bt_weight;
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "wl_weight = 0x%lx\n", ah->ah_bt_coex_wl_weight);
            break;
#endif
        ...
        case ATH_PARAM_VOWEXT:
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Setting new VOWEXT parameters buffers with %d\n",ATH_TXDESC);
            sc->sc_vowext_flags = (*(u_int32_t*)buff) & ATH_CAP_VOWEXT_MASK;
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "VOW enabled features: \n");
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "\tfair_queuing: %d\n\t"\
                   "retry_delay_red %d,\n\tbuffer reordering %d\n\t"\
        ...
        case ATH_PARAM_VSP_ENABLE:
            ...
            if(*(int *)buff)
            {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ATH_TX_BUF_FLOW_CNTL is not compiled in so can't enable VSP\n");
            }
            #endif
        break;
        ...
        case ATH_PARAM_DCS_ENABLE:
            ...
            if ( (sc->sc_dcs_enabled & ATH_CAP_DCS_WLANIM) && (!((sc->sc_curmode == WIRELESS_MODE_11NA_HT20)
                ...
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Disabling DCS for 11G mode");
                sc->sc_dcs_enabled &= (~ATH_CAP_DCS_WLANIM);
            }
             QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "\tDCS for CW interference mitigation:   %d\n\t"\
        ...
        case ATH_PARAM_CNTRX_NUM:
            ...
            if((sc->sc_process_rx_num % 2)
                ...
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Adjusting to Nearest Even Value %d \n",sc->sc_process_rx_num);
            }
            break;
#endif
        ...
        case ATH_PARAM_ANTENNA_GAIN_5G:
            ...
            sc->sc_reset_type = ATH_RESET_DEFAULT;
            break;
        case ATH_PARAM_TWICE_ANTENNA_GAIN:
            if ((*(int *)buff >=0) && (*(int *)buff <= (MAX_ANTENNA_GAIN * 2))) {
                if (IEEE80211_IS_CHAN_2GHZ(ic->ic_curchan)) {
                    ath_hal_set_twice_antenna_gain(sc->sc_ah, *((int *)buff), 1);
                } else {
                    ath_hal_set_twice_antenna_gain(sc->sc_ah, *((int *)buff), 0);
                }
            }
            /* internal reset for power table re-calculation */
            sc->sc_reset_type = ATH_RESET_NOLOSS;
        ...
        case ATH_PARAM_RTS_CTS_RATE:
            ...
            {
                sc->sc_cus_cts_set=0;
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ATH_PARAM_RTS_CTS_RATE not support\n");
            }
            break;
        case ATH_PARAM_NODEBUG:
            sc->sc_nodebug = *(int *)buff;
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "sc nodebug %d \n",sc->sc_nodebug);
        ...
        case ATH_PARAM_ALDSTATS:
            ...
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "sc aldstatsenabled %d \n",sc->sc_aldstatsenabled);
        ...
        case ATH_PARAM_EN_SELECTIVE_RTS:
            ...
            {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Enable Rate limit first \r\n");
                retval = (-1);
            }
            break;
        ...
        case ATH_PARAM_ATF_STRICT_SCHED:
            ...
            if(((*(u_int32_t*)buff) == 1) && (!(val & IEEE80211_ATF_GROUP_SCHED_POLICY)) && (ic->ic_atf_ssidgroup))
            {
        ...
        case ATH_MIN_RSSI_ENABLE:
            {
                ...
                if (val == 0 || val == 1) {
                    ...
                    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Enter 0 or 1\n");
                    return (-1);
                }
            }
            break;
        case ATH_MIN_RSSI:
            {
                ...
                if (val <= 0) {
                    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "snr should be a positive value\n");
                    return (-1);
                } else
        ...
        case ATH_PARAM_CLIENT_MCAST:
            ...
            if (*(int *)buff) {
                ...
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Enabling MCAST client traffic to go on corresponding STA VAP\n");
            } else {
                ic->ic_global_list->force_client_mcast_traffic = 0;
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Disabling MCAST client traffic to go on corresponding STA VAP\n");
            }
            GLOBAL_IC_UNLOCK(ic->ic_global_list);
            break;
#endif
        ...
        case ATH_PARAM_TID_OVERRIDE_QUEUE_MAPPING:
             ic->tid_override_queue_mapping = *(int *)buff;
            break;
	...
	case ATH_PARAM_FAST_LANE:
        ...
        case ATH_PARAM_PREFERRED_UPLINK:
            ...
            break;
#endif
        case ATH_PARAM_BEST_UL_HYST:
            val = *(int *)buff;
            if (val > 0 && val <= 100)
                ic->ic_bestUL_hyst = val;
            break;
        ...
        case ATH_PARAM_TT_TEMP:
            sc->sc_tt_temp = *(int *)buff;
            break;
        case ATH_PARAM_TT_LEVEL:
            sc->sc_tt_level = *(int *)buff;
            break;
        ...
        case ATH_PARAM_NF_THRESH:
            if ((*(int *)buff < -65) && (*(int *)buff > -85))
            {
                ic->acs_noisefloor_threshold = *(int *)buff;
            }
            else {
...
ath_get_config(ath_dev_t dev, ath_param_ID_t ID, void *buff)
{
    ...
    switch(ID)
    {
        ...
        case ATH_PARAM_ATH_RXQ_INFO:
        {
            ...
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "LP details ---\n");
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "rxfifo =%pK, rxfifodepth = %d, rxfifohwsize =%d, rxfifoheadindex=%d, rxfifotailindex=%d\n",
                    rxedma_lp->rxfifo, rxedma_lp->rxfifodepth, rxedma_lp->rxfifohwsize, rxedma_lp->rxfifoheadindex, rxedma_lp->rxfifotailindex );

            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "HP details ---\n");
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "rxfifo =%pK, rxfifodepth = %d, rxfifohwsize =%d, rxfifoheadindex=%d, rxfifotailindex=%d\n",
                    rxedma_hp->rxfifo, rxedma_hp->rxfifodepth, rxedma_hp->rxfifohwsize, rxedma_hp->rxfifoheadindex, rxedma_hp->rxfifotailindex );

            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "sc_imask = 0x%x\n", sc->sc_imask);
            ...
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "sc_rxbuf num of elements = %d, bf=%pK, bf_mpdu=%pK\n", i, bf, bf?bf->bf_mpdu:0 );
        }
        break;
	...
	case ATH_PARAM_GPIO_LED_CUSTOM:
        ...
        case ATH_PARAM_VSP_STATS:
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "VSP Statistics:\n\t"\
                    "vi contention: %d\n\tbe(penalized) drop : %d\n\t"\
                    "bk(penalized) drop : %d\n\tvi(penalized) drop : %d\n",\
...
//            *(int *)buff = sc->sc_eap_lowest_rate;
        ...
        case ATH_PARAM_TWICE_ANTENNA_GAIN:
            if (IEEE80211_IS_CHAN_2GHZ(ic->ic_curchan)) {
                *(int *)buff = ath_hal_get_twice_antenna_gain(sc->sc_ah, 1);
            } else {
                *(int *)buff = ath_hal_get_twice_antenna_gain(sc->sc_ah, 0);
            }
            break;
...
#define ANI_CCK_ERR_OFFSET 11
       ...
       case ATH_PARAM_DIAG_ENABLE:
        ...
        case ATH_PARAM_ACS_RANK:
            ...
            break;
        case ATH_PARAM_TT_TEMP:
            *(int *)buff = sc->sc_tt_temp;
            break;
        case ATH_PARAM_TT_LEVEL:
            *(int *)buff = sc->sc_tt_level;
            break;
#if ATH_PARAMETER_API
       case ATH_PARAM_PAPI_ENABLE:
