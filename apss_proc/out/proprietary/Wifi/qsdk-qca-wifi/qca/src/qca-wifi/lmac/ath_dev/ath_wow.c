...
ath_set_wow_enable(ath_dev_t dev, int clearbssid)
{
    ...
    if(ath_hal_wowEnable(ah, wakeUpEvents, sc->sc_wowInfo->timeoutinsecs, clearbssid, wowOffload) == -1) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d]: Error entering wow mode\n", __func__, __LINE__);
        return (-1);
    }
    else
    {
...
ath_wow_add_wakeup_pattern(ath_dev_t dev,u_int32_t patternId, u_int8_t *patternBytes, u_int8_t *maskBytes, u_int32_t patternLen)
{
    ...
    for (i = 0; i < MAX_NUM_USER_PATTERN; i++) {
        ...
        if (pattern->valid) {
            ...
            {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Pattern added already \n");
                return 0;
            }
        }
    }
    ...
    if (i == MAX_NUM_USER_PATTERN) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Error : All the %d pattern are in use. Cannot add a new pattern \n", MAX_NUM_PATTERN);
        return (-1);
    }
...
ath_wow_remove_wakeup_pattern(ath_dev_t dev, u_int8_t *patternBytes, u_int8_t *maskBytes)
{
    ...
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "mask = %x pat = %x \n",(u_int32_t)maskBytes, (u_int32_t)patternBytes);
