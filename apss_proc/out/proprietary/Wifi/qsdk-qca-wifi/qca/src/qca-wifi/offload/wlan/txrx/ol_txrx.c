...
#ifdef ATH_SUPPORT_RDKB
void
ol_wlanstats_event_sub(void *pdev,
                ...
                enum htt_rx_status status)
{
    ...
    /*
     * msg offset is calculated using the below offset values
     * 2 + 2 + 2 + 2 + 4 + 4
     */
#define MSG_OFFSET 16
    ...
    case 1: /* TX_CTRL, ppdu start */
        if (scn->wlanstats_txstat_discard)
            break;
        ...
        wlan_tx_stats_collect_ar98xx(ic,
                ni,
                msg,
                scn->wlanstats_txstat_ppdu_end);
...
unref_peer:
    ...
    case 2: /* TX_STAT, ppdu_end */
        scn->wlanstats_txstat_discard = 0;
        memcpy(scn->wlanstats_txstat_ppdu_end, msg,
               sizeof(scn->wlanstats_txstat_ppdu_end));
        break;
    case 4: /* TX_FRM_HDR, means local fw frame */
    case 8: /* TX_VIRT_ADDR, means local fw frame */
        scn->wlanstats_txstat_discard = 1;
        break;
    }
    ...
    qdf_device_t osdev)
{
...
#if QCA_PARTNER_DIRECTLINK
    ath_init_HTT_TX_CE_partner(pdev);
    ...
    pdev->carrier_vow_config = scn->carrier_vow_config;
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "pdev attach %pK %d \n",scn->nss_wifiol_ctx,scn->nss_wifiol_id);
    ...
    qdf_print(KERN_INFO"%s: %d tx desc's allocated ; range starts from %pK\n",
...
#ifdef ATH_SUPPORT_RDKB
    memset(&scn->wlanstats_event_sub, 0, sizeof(scn->wlanstats_event_sub));
    scn->wlanstats_event_sub.callback = ol_wlanstats_event_sub;
    ret = wdi_event_sub(pdev, &scn->wlanstats_event_sub, WDI_EVENT_OFFLOAD_ALL);
    if (ret)
        qdf_print("wlanstats: failed to subscribe to WDI_EVENT_OFFLOAD_ALL: %d\n", ret);
#endif
...
ol_txrx_pdev_detach(ol_txrx_pdev_handle pdev, int force)
{
    ...
    wdi_event_unsub(pdev, &scn->wlanstats_event_sub, WDI_EVENT_OFFLOAD_ALL);
#endif
    ...
    enum wlan_op_mode op_mode)
{
  ...
  /* Header cache update for each vdev */
    ...
    if (pdev->carrier_vow_config) {
        vdev->carrier_vow_config = pdev->carrier_vow_config;
    }
    return vdev;
}
    ...
    struct ol_txrx_ops *txrx_ops)
{
    ...
    if (ol_cfg_is_high_latency(vdev->pdev->ctrl_pdev)) {
        ...
        txrx_ops->txrxclassify = vdev->txrx_classify = ol_txrx_classify;
    }
}

void ol_txrx_cfr_rx_cb_set(ol_txrx_pdev_handle pdev,
                                 ol_txrx_cfr_rx_ind_cb cfr_rx_ind_cb,void *ctxt)
{
     pdev->cfr_callback.cfr_rx_htt_ind   = cfr_rx_ind_cb;
     pdev->cfr_callback.ctxt             = ctxt;
}
...
ol_txrx_peer_unref_delete(ol_txrx_peer_handle peer)
{
    ...
    if (qdf_atomic_dec_and_test(&peer->ref_cnt)) {
        ...
        if (found) {
             ...
             QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "WARN peer %pK not found in vdev (%pK)->peer_list:%pK\n", peer, vdev,
                        &peer->vdev->peer_list);
        }
                ...
                status == HTT_DBG_STATS_STATUS_PARTIAL)
        {
            ...
            if ((status != HTT_DBG_STATS_STATUS_PARTIAL) &&
...
#if OL_STATS_WORK_QUEUE
                {
                    ...
                    qdf_create_work(pdev->osdev, &(stats_buffer->stats_wq),
                            stats_deferred_work, stats_buffer);
                    qdf_mem_copy(stats_buffer->stats_info, stats_info_list, length);
                    qdf_sched_work(pdev->osdev, &(stats_buffer->stats_wq));
                }
#else
                htt_t2h_stats_print(stats_info_list, req->base.print.concise, scn->target_type, vdev_id);
#endif
