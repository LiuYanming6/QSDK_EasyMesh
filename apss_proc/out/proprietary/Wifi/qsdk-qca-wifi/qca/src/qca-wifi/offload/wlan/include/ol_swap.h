/*
* Copyright (c) 2011-2014, 2018 Qualcomm Innovation Center, Inc.
* All Rights Reserved.
...
struct swap_seg_info {
    ...
    u_int64_t seg_cpuaddr[SWAP_MAX_SEGS];
};
...
int
ol_swap_seg_alloc (struct ol_ath_softc_net80211 *scn, struct swap_seg_info **ret_seg_info, 
                                             u_int64_t **scn_cpuaddr, const char* filename, int type);
int 
ol_swap_wlan_memory_expansion(struct ol_ath_softc_net80211 *scn, struct swap_seg_info *seg_info,
                                                  const char*  filename, u_int32_t *target_addr);


int ol_transfer_swap_struct(struct ol_ath_softc_net80211 *scn, ATH_SWAP_INFO swap_info,
                                                                   char *bin_filename);


int ol_ath_code_data_swap(struct ol_ath_softc_net80211 *scn,const char * bin_filename, 
