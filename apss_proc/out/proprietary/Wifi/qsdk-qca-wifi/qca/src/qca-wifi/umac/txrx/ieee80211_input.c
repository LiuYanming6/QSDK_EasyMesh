...
ieee80211_input_data_ap(struct ieee80211_node *ni, wbuf_t wbuf, struct ieee80211_frame *wh, int dir)
{
    ...
    if (ni == vap->iv_bss) {
        ...
        */
        ...
        if (ieee80211_vap_ready_is_set(vap) && (ic->ic_isoffchan == 0) && !IEEE80211_IS_MULTICAST(wh->i_addr2))
#else
        if (ieee80211_vap_ready_is_set(vap) && !IEEE80211_IS_MULTICAST(wh->i_addr2))
#endif
        {
...
ieee80211_input_data(struct ieee80211_node *ni, wbuf_t wbuf, struct ieee80211_rx_status *rs, int subtype, int dir)
{
    ...
    int realhdrsize;
    int padsize;
    int is_mcast, is_ipv6_mcast, is_amsdu = 0, is_bcast, is_mdns = 0;
#if not_yet
    ...
    is_ipv6_mcast = (dir == IEEE80211_FC1_DIR_DSTODS ||
                dir == IEEE80211_FC1_DIR_TODS ) ?
        IEEE80211_IS_IPV6_MULTICAST(IEEE80211_WH4(wh)->i_addr3) :
        IEEE80211_IS_IPV6_MULTICAST(wh->i_addr1);
        ...
        if (is_mcast || is_ipv6_mcast) {
            ...
            ( dir == IEEE80211_FC1_DIR_DSTODS ||
              dir == IEEE80211_FC1_DIR_TODS ) ?
                ...
                IEEE80211_ADDR_COPY(mcast_addr, wh->i_addr1);
...
static INLINE bool
ieee80211_vap_reject_mgmt_frame(struct ieee80211vap *vap, uint8_t *macaddr)
{
    uint8_t i;
    for (i = 0; i < IEEE80211_REJECT_MGMT_MAX; i++) {
        if (IEEE80211_ADDR_EQ(macaddr, vap->iv_conf_rej_mgmt[i].addr)) {
            return TRUE;
        }
    }
    return FALSE;
...
ieee80211_iter_input_all(void *arg, struct ieee80211vap *vap, bool is_last_vap)
{
    ...
    wh = (struct ieee80211_frame *)wbuf_header(params->wbuf);
