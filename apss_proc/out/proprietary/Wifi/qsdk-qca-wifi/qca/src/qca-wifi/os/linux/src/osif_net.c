...
__qdf_net_start_tx(struct sk_buff  *skb, struct net_device  *netdev)
{
    ...
    if(unlikely((skb->len <= ETH_HLEN))) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "QDF_NET:Bad skb len %d\n", skb->len);
        qdf_nbuf_free(skb);
        return NETDEV_TX_OK;
    }
...
{
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " QDF_NET_IOCTL_SUPPORT is not On \n");
                        ...
                        qdf_net_dev_info_t     *info, void          *vap_cfg)
{
    ...
    if(!is_valid_ether_addr(info->dev_addr)){
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "QDF_NET:invalid MAC address\n");
        error = EINVAL;
    }
...
#if 0
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "addrs : sa : %x : da:%x\n", skb->nh.iph->saddr, skb->nh.iph->daddr);
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "head : %pK tail : %pK iph %pK %pK\n", skb->head, skb->tail,
...
__qdf_net_dev_tx(qdf_net_handle_t hdl, struct sk_buff  *skb)
{
    ...
    if(unlikely(!netdev)){
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "QDF_NET:netdev not found\n");
        qdf_assert(0);
    }
...
__qdf_net_register_wpscb(qdf_net_handle_t radiodev, void *ctx, qdf_net_wps_cb_t cb)
{
    ...
    if(ret == 0)
    {
        ...
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Registerd wps callback for %s \n",__FUNCTION__,netdev->name);
    }
    else
    {
        status = QDF_STATUS_E_FAILURE ;
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Failed to register wps callback for %s \n",__FUNCTION__,netdev->name);
    }
...
__qdf_net_unregister_wpscb(qdf_net_handle_t radiodev)
{
    ...
    for(i = 0 ; i < QDF_MAX_WPS_CALLBACKS ; i++)
    {
        if(scCB[i].dev == netdev)
        {
            ...
            scCB[i].dev = NULL ;
            scCB[i].push_dur = 0 ;
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Deregister wps callback for %s \n",__FUNCTION__,netdev->name);
            if(ret == 0)
                status = QDF_STATUS_SUCCESS ;
