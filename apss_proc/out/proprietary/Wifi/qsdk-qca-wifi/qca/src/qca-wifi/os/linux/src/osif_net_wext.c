                ...
                union iwreq_data *iwreq, char *extra)
{
    ...
    if (scan == NULL || data == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : Mem Allocation Fail\n",__FUNCTION__);
        if (scan)   vfree(scan);
        if (data)   vfree(data);
                    ...
                    union iwreq_data *iwreq, char *extra)
{
    ...
    if (status == QDF_STATUS_SUCCESS) {
        ...
        if(len <= (QDF_CHANINFO_MAX * sizeof(unsigned int)))
            ...
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : Not enough memory\n",__FUNCTION__);
    }
