...
void ath_lowi_if_netlink_send(void *buf, int len)
{
    ...
    if (!skb) {
        qdf_print("%s: No memory for skb\n", __func__);
        return;
    }

    nlh = nlmsg_put(skb, pid, 0, 0, len, 0);
    if (!nlh) {
        qdf_print("%s: nlmsg_put() failed\n", __func__);
        kfree_skb(skb);
        return;
    }
    ...
    netlink_unicast(lowi_if_nl->lowi_if_sock, skb, pid, MSG_DONTWAIT,0);
#else
...
int ath_lowi_if_netlink_init(void)
{
    ...
    if (!lowi_if_nl) {
        lowi_if_nl = (struct lowi_if_netlink *)
            kzalloc(sizeof(struct lowi_if_netlink), GFP_KERNEL);
        if (!lowi_if_nl) {
            return -ENODEV;
        }
        ...
        qdf_print("%s LOWI Netlink successfully created\n", __func__);
    } else {
        atomic_inc(&lowi_if_nl->lowi_if_refcnt);
        qdf_print("%s Incremented LOWI netlink ref count: %d\n", __func__, atomic_read(&lowi_if_nl->lowi_if_refcnt));
    }
    return 0;
}
/**
...
{
    if (!lowi_if_nl) {
        qdf_print("%s lowi_if_nl is NULL\n", __func__);
        return -ENODEV;
    }
    qdf_print("%s Going to decrement current LOWI netlink ref count: %d\n", __func__, atomic_read(&lowi_if_nl->lowi_if_refcnt));
...
void ath_lowi_if_processmsg(void *data)
{
    ...
    if(!ic){
        qdf_print("%s: ERROR: No offload-capable radio with active VAP was found. Dropping LOWI message\n", __func__);
        return;
    }
    ...
    switch(ani_hdr->msg_type)
    {
        ...
        case ANI_MSG_OEM_DATA_REQ:
            ...
            switch(msgsubType) {
                ...
                case TARGET_OEM_MEASUREMENT_REQ:
                    ...
                    qdf_print("%s: Received RTT measurement request from LOWI \n", __func__);
                    /* Find radio that supports this channel */
                    ic = ath_lowi_if_get_ic(NULL, &wmi_ch.mhz, NULL);
                    if (!ic) {
                            qdf_print("%s: ERROR: No Radio Found supporting channel (%d mhz). Dropping ranging request.\n", __func__, wmi_ch.mhz);
                        return;
                    }
                    /* Send message to FW */
                    if(ic->ic_lowi_frame_send)
                ...
                case TARGET_OEM_CONFIGURE_LCI:
                    qdf_print("%s: Received LCI/LCR request from LOWI \n", __func__);
                ...
                case TARGET_OEM_CONFIGURE_WRU:
                    qdf_print("%s: Received Where Are You frame request from LOWI \n", __func__);
                ...
                case TARGET_OEM_CONFIGURE_FTMRR:
                    qdf_print("%s: Received FTMRR frame request from LOWI \n", __func__);
                ...
                default:
                    qdf_print("%s: ERROR: UNKNOWN message sub-type received for ANI_MSG_OEM_DATA_REQ lowi message\n", __func__);
                    return;
            }
        ...
        default:
            qdf_print("%s: ERROR: UNKNOWN LOWI message received\n", __func__);
    }
}
        ...
        u_int8_t *data, u_int16_t datalen, u_int8_t msgtype, u_int32_t msg_subtype, u_int8_t error_code)
{
    ...
    {
        qdf_print("%s: Error report recevied from FW: Error Code: %d\n", __func__, error_code);
    }
    /* Allocate buffer for entire message including header */
                ...
                for(j=0; j<ic->ic_nchans; j++){
                    ...
                    if(c->ic_freq == (u_int16_t) *channel_mhz){
                        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_NORMAL,
                            IEEE80211_MSG_WIFIPOS, "%s: Found matching freq (%d mhz) on SCN[%d]\n", __func__, *channel_mhz, i);
                        return ic;
                    }
                }
            }
            if(sta_mac){
                /* Find if sta_mac is associated with any vap on this ic */
                struct ieee80211_node *ni = NULL;
                /* Search sta_mac on this radio */
                ni = ieee80211_find_node(&ic->ic_sta, sta_mac);
                if(ni != NULL) {
                    /* Found matching node in associated node table */
