...
extern void
ol_txrx_classify(struct ol_txrx_vdev_t *vdev, qdf_nbuf_t nbuf, enum ol_txrx_pkt_direction dir, struct ol_txrx_nbuf_classify *nbuf_class);
...
osif_deliver_data_ol(os_if_t osif, struct sk_buff *skb_list)
{
    ...
    while (skb_list) {
        ...
        if (vap->iv_drop_mdns) {
            is_mcast = (IEEE80211_IS_MULTICAST(eh->ether_dhost) || IEEE80211_IS_IPV6_MULTICAST(eh->ether_dhost)) ? 1 : 0;
            if ((is_mcast) && ((IEEE80211_ADDR_EQ(eh->ether_dhost, (uint8_t *)mdns_mac)) ||
...
extern void ol_tx_partner(struct sk_buff *skb, struct net_device *dev, uint32_t peer_id);
#endif
...
void interframe_delay_stats(struct ol_txrx_vdev_t * vdev, struct sk_buff *skb, uint32_t dir)
{
    ...
    /* Interframe Section */
    /* check first pkt */
    tstamp = qdf_ktime_get_real();
    if (interfrm->start_tick.tv64 == 0) {
        interfrm->start_tick = qdf_ktime_get_real();
        interfrm->last_frame_tick = qdf_ktime_get_real();
        interfrm->max_delay = qdf_ktime_get_real();
        interfrm->max_interframe_delay.tv64 = 0;
    } else {
        cur_ifdtstamp.tv64 = qdf_ktime_to_us(qdf_ktime_sub(qdf_ktime_get_real(), interfrm->last_frame_tick));
     ...
     */
    if (skb->tstamp.tv64) {
        msdu_tstamp.tv64 = qdf_ktime_to_us(net_timedelta(skb->tstamp));
...
osif_ol_ll_vap_hardstart(struct sk_buff *skb, struct net_device *dev)
{
    ...
    /* Reset the CB of the skb */
    qdf_nbuf_reset_ctxt(skb);

    /*
     * Carrier Vow stats
     */
    if ((osdev->iv_txrx_handle) && osdev->iv_txrx_handle->carrier_vow_config) {
        ol_txrx_classify(osdev->iv_txrx_handle, skb, tx_direction, &nbuf_class);
        if (osdev->iv_txrx_handle->pdev->delay_counters_enabled && nbuf_class.pkt_tid == QDF_TID_VI) {
            interframe_delay_stats(osdev->iv_txrx_handle, skb, TX_DIR);
            __net_timestamp(skb);
        } else
            skb->tstamp.tv64 = 0;
        if(nbuf_class.pkt_tid == QDF_TID_VI) {
            PFLOW_CTRL_PDEV_VIDEO_STATS_ADD(osdev->iv_txrx_handle->pdev, TX_VIDEO_TID_MSDU_TOTAL_LINUX_SUBSYSTEM, 1);
        }
        peer = ol_txrx_peer_find_by_id(osdev->iv_txrx_handle->pdev, nbuf_class.peer_id);
        if (peer)
            PFLOW_TXRX_TIDQ_STATS_ADD(peer, nbuf_class.pkt_tid, TX_MSDU_TOTAL_LINUX_SUBSYSTEM, 1);
    }
    ...
    if (ic->atf_mode && (pscn->target_type == TARGET_TYPE_AR9888 &&
        ...
        if (vap->ac_blk_cnt){
            ...
            if (ni->ni_block_bitmap == ATF_TRAFFIC_CTRL_AC_ALL) {
                ieee80211_free_node(ni);
                goto bad1;
            } else {
                tid = wbuf_classify(skb, ic->tid_override_queue_mapping);
                ac = TID_TO_WME_AC(tid);
                if (ni->ni_block_bitmap & (1 << ac)) {
                    ieee80211_free_node(ni);
                    goto bad1;
                }
                ieee80211_free_node(ni);
...

{
    ...
    if (osdev->iv_vap_send && osdev->iv_txrx_handle)
...
void osif_vap_setup_ol (struct ieee80211vap *vap, osif_dev *osifp) {
        ...
        osifp->iv_vap_classify = ops.txrxclassify;
...
#if QCA_PARTNER_DIRECTLINK
EXPORT_SYMBOL(osif_ol_ll_vap_hardstart);
#endif
