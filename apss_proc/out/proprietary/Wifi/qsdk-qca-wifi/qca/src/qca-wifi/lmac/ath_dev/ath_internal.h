...
#include "ath_mci.h"
#endif
#include "../../offload/wlan/include/ol_if_thermal.h"
...
#define ATH_RX_REFILL_TIMER_MS (20)
#endif
...
typedef struct ath_spectral_ops {
   ...
   u_int32_t (*is_spectral_supported)(void* arg);
...
#define ATH_IS_FOUR_CHAIN(x)        ((x) == 15)
#define IS_SINGLE_STREAM(rate) (rate < 8)  ? 1 : 0
#define IS_THREE_STREAM(rate)  ((rate > 15) && (rate < 24)) ? 1 : 0
#define IS_FOUR_STREAM(rate)   (rate > 23)  ? 1 : 0
...
#define ATH_NODE_LEAVE           0x20    /* indicates the node leave */
    ...
    u_int8_t     an_txrateflags;  /* last packet  tx rate flag*/
    u_int8_t     an_rxrateflags;  /* last packet rx rate flag*/
    ...
    u_int32_t   an_atf_nodepaused;     /* Bitmap indicating the TID paused by ATF */
...
struct ath_vap {
    ...
    u_int8_t                        av_ctsrts_ratecode;     /* To modify cts / rts rate via iwpriv */
};
...
struct ath_softc {
    ...
    int                     sc_print_idx;      /* Print control index for LMAC module */
#if ATH_DEBUG
	...
	sc_cddLimitingEnabled  : 1, /* Disable Cyclic Delay Diversity on the channel for legacy rates */
        ...
        sc_extapEnabled    : 1; /* extap enabled */
...
#if !ATH_RXBUF_RECYCLE
    ath_bufhead             sc_rxrecyclebuf; /* receive buffers to be allocated on failure of recycle */
    os_timer_t              sc_rxbuf_refill_timer;
    unsigned int            sc_rxbuf_refill_trigger:1;
    unsigned int            sc_rxbuf_reuse_enable:1;
#endif
...
#define ATH_GPIO_MAX_NUM 32
    ...
    u_int8_t                rng_thread_run_state;
#if ATH_SUPPORT_WRAP
    bool                    sc_qwrap_rx_war;  /*QWRAP RX WAR flag */
#endif
    struct thermal_param    thermal_param;
    os_timer_t              sc_tt_timer;     /* Thermal Throttling timer */
    u_int8_t    sc_dc_per_event;
    u_int8_t    sc_tx_on;
    u_int16_t   sc_tt_temp;
    u_int16_t   sc_tt_level;
    u_int8_t    sc_quadstream_support    :1;
    u_int8_t    sc_tt_chain;
    int16_t     sc_tt_temp1;
    int16_t     sc_tt_temp2;
    int16_t     sc_tt_temp3;
    int16_t     sc_tt_temp4;
...
void ath_set_extap_state(ath_dev_t dev, int val);
...
void _ath_tx_complete_buf(struct ath_softc *sc, struct ath_buf *bf, ath_bufhead *bf_q,
                         struct ath_tx_status *ts, int txok, u_int8_t txbf_status, u_int32_t tstamp);
#else
void _ath_tx_complete_buf(struct ath_softc *sc, struct ath_buf *bf, ath_bufhead *bf_q,
...
#define ath_tx_complete_buf(_sc, _bf, _bf_q, _ts, _txok, _txbf_status, _tstamp) \
    _ath_tx_complete_buf(_sc, _bf, _bf_q, _ts, _txok, _txbf_status, _tstamp)

#else

#define ath_tx_complete_buf(_sc, _bf, _bf_q, _ts, _txok) \
      _ath_tx_complete_buf(_sc, _bf, _bf_q, _ts, _txok)
...
void
ath_edma_free_complete_buf( struct ath_softc *sc , struct ath_txq *txq, ath_bufhead * bf_headfree, struct ath_tx_status *ts, int txok );
...
void ath_wbuf_recycle(struct ath_softc *sc, struct ath_buf *bf, wbuf_t wbuf);
bool ath_can_wbuf_recycle(wbuf_t wbuf);
#endif
void ath_rx_refill_try(void *arg);
void ath_rx_refill_queue(struct ath_softc *sc, struct ath_buf *bf);
#endif
...
#define ath_hal_is_spectral_supported(_ah) \
            ((*(_ah)->ah_ar_is_spectral_supported)((_ah)))
...
#define ath_hal_set_temp_deg_c_chain(_ah,_preferred_therm_chain) \
            ((*(_ah)->ah_set_temp_deg_c_chain)((_ah),(_preferred_therm_chain)))
#define ath_hal_get_temp_deg_c(_ah,_temp) \
            ((*(_ah)->ah_get_temp_deg_c)((_ah),(_temp)))
...
#if defined(QCA_PARTNER_PLATFORM) && !(QCA_PARTNER_DIRECTLINK || QCA_PARTNER_CBM_DIRECTPATH)
static u_int32_t
#else
static INLINE u_int32_t
#endif
