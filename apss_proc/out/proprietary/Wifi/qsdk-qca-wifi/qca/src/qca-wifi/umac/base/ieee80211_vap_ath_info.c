    ...
    u_int32_t                   *param2)
{
    ...
    if (vap->iv_vap_ath_info_handle == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Error: not attached.\n", __func__);
        return EINVAL;
    }
...
ieee80211_vap_ath_info_detach(ieee80211_vap_ath_info_t h_mgr)
{
    ...
    if (!h_mgr) {
        ...
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : Not even attached.\n", __func__);
        return;
    }
    vap = h_mgr->vap;
