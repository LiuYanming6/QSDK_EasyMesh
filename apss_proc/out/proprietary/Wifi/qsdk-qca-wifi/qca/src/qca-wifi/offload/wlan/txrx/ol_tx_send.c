...
static void tx_info_dump(struct ieee80211com *ic, struct ieee80211_tx_status *ts, uint8_t *dest_addr)
{
    ...
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "==ts->ts_rateKbps=%dkbps\n",ts->ts_rateKbps);
        ...
        qdf_nbuf_t netbuf)
{
    ...
    wlanstats_sojourn_collect(&scn->sc_ic,
            ni,
            tidno,
            netbuf);
    ...
    int *num_htt_compl)
{
    ...
    if (qdf_likely(peer)) {
...
#endif
            /*
             * Hardstart to FW tx completion delay counter
             */
	    ...
	    if (qdf_unlikely(IS_WLANSTATS_ENABLED(ic)))
		    ol_tx_sojourn_collect(pdev, scn, netbuf);
#endif
