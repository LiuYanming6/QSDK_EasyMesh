...
void wlan_debug_dump_nodes_tgt(wlan_if_t vaphandle);
#endif
...
#endif
extern void wlan_offchan_mgmt_tx_stop(wlan_if_t vap_handle);
extern int wlan_offchan_mgmt_tx_start(wlan_if_t vap_handle, u_int32_t freq, u_int32_t scan_time,
...
#if 0
		QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s() spydata %pK, num %d\n", __func__, spydata, wrqu->data.length);
		for (i = 0; i < wrqu->data.length; i++)
...
#define TU_TO_MS(_tu)        (((_tu) * 1024) / 1000)
    ...
    if(ic->ic_use_custom_chan_list !=0 && vap->iv_opmode == IEEE80211_M_STA)
    {
        ...
        /*If STAVAP is down, then supplicant scan will be going on,
         *so scan table should not be flushed */
	if (osifp->is_up) {
    ...
    struct iw_point *data, char *extra)
{
    ...
    if (!(((osifp->os_opmode == IEEE80211_M_STA) && !osifp->is_up )||
        osifp->os_opmode == IEEE80211_M_P2P_DEVICE ||
        osifp->os_opmode == IEEE80211_M_P2P_CLIENT)) {
    ...
    void *w, char *extra)
{
    ...
    if (osif_restart_for_config(ic, NULL))
        return -EFAULT;
    ...
    void *w, char *extra)
{
    ...
    case IEEE80211_MLME_ASSOC:
	    ...
	    osifp->os_opmode == IEEE80211_M_IBSS) {
            ...
            ieee80211_ucfg_getparam(vap, IEEE80211_PARAM_AUTHMODE, &auth_value);
            if (auth_value == IEEE80211_AUTH_SAE) {
    ...
    case IEEE80211_MLME_REASSOC:
        if (osifp->os_opmode == IEEE80211_M_STA ||
            ...
            /* NB: the broadcast address means do 'em all */
	    if (!IEEE80211_ADDR_EQ(mlme->im_macaddr, ieee80211broadcastaddr)) {
                if (!(ni = ieee80211_vap_find_node(vap, mlme->im_macaddr)))
                    return -EINVAL;
    ...
    case IEEE80211_MLME_DEAUTH:
        ...
        if(ieee80211_vap_pmf_mgmt_xtry_is_set(vap)) {
            ieee80211_vap_pmf_mgmt_xtry_clear(vap);
        }
    ...
    case IEEE80211_MLME_UNAUTHORIZE:
        ...
        if (osifp->os_opmode == IEEE80211_M_HOSTAP || \
            osifp->os_opmode == IEEE80211_M_BTAMP || \
            osifp->os_opmode == IEEE80211_M_IBSS) {
...
#if defined (ATH_SUPPORT_HYFI_ENHANCEMENTS) || defined (ATH_PARAMETER_API)
...
//       print_hex_dump(KERN_ERR, " ", DUMP_PREFIX_NONE, 16, 1, (u_int8_t *)sa, iwd->data.length, false);
        if (vap) {
            if (!sa->cancel_current_wait) {
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_IOCTL,
                          "%s: IEEE80211_IOC_P2P_SEND_ACTION size=%d \n", __func__,
                          iwr->u.data.length);
            if(wlan_offchan_mgmt_tx_start(vap, sa->freq, sa->scan_time,
    #if UMAC_SUPPORT_WNM
...
#endif
        if ((vap == ic->ic_sta_vap) && vap->iv_supplicant_state) {
            vap->iv_supplicant_state = 0;
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,
                    "%s: supplicant_state_timer: Clearing the flag\n",__func__);
        }
    ...
    */
        ...
        if (iwd->data.length) {
            ...
            if (scan_extra->ie_len) {
                ...
                ie = (u_int8_t *)scan_params.ie_data;
                while (((ie+1) < (scan_params.ie_data + scan_params.ie_len)) && (ie + ie[1] + 1 < (scan_params.ie_data + scan_params.ie_len))) {
                    if (iswpsoui(ie))
                    {
                        vap->iv_wps_session = 1;
                        break;
                    }
                    else {
                        ie += ie[1] + 2;
...
ieee80211_ioctl_hmwds(struct net_device *dev, struct iwreq *iwr)
{
    ...
    switch (config->cmdtype) {
        ...
        case IEEE80211_WLANCONFIG_HMWDS_READ_ADDR:
            ...
            if (copy_to_user(iwr->u.data.pointer, config, iwr->u.data.length)) {
                ret = -EFAULT;
            }
            break;
...
ieee80211_ioctl_addie(struct net_device *dev, struct iwreq *iwr)
{
    ...
    switch(ie_buffer->ftype) {
        ...
        case IEEE80211_APPIE_FRAME_ASSOC_RESP:
            ...
            break;
        default:
            QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " Frame type not supported\n");
            error = -ENXIO;
    }
...
ieee80211_ioctl_nac_rssi(struct net_device *dev, struct iwreq *iwr)
{
    ...
    IEEE80211_DPRINTF(vap, IEEE80211_MSG_IOCTL,"%s:Cmd_type=%d BSSID:%2x:%2x:%2x:%2x:%2x:%2x,client:%2x:%2x:%2x:%2x:%2x:%2x channel: %d\n",
                                        __func__,config.cmdtype,nac_rssi->mac_bssid[0],nac_rssi->mac_bssid[1],nac_rssi->mac_bssid[2],
                                        nac_rssi->mac_bssid[3],nac_rssi->mac_bssid[4],nac_rssi->mac_bssid[5],
                                        nac_rssi->mac_client[0],nac_rssi->mac_client[1],nac_rssi->mac_client[2],
                                        nac_rssi->mac_client[3],nac_rssi->mac_client[4],nac_rssi->mac_client[5], nac_rssi->chan_num);
    ...
    switch (config.cmdtype) {
        ...
        case IEEE80211_WLANCONFIG_NAC_RSSI_ADDR_LIST:
             wlan_list_nac_rssi(vap, IEEE80211_NAC_RSSI_PARAM_LIST, nac_rssi);
...
ieee80211acs_ioctl_getchanlist(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    ieee80211_user_chanlist_t list_p = {0};
    ieee80211_user_chanlist_t list_r = {0};
    u_int32_t chan_count;
    int ret_val = EOK;

    if ((void *)(uintptr_t) req->data.param[0] == NULL) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY,
                 QDF_TRACE_LEVEL_INFO, "%s: User memory not allocated or corrupted \n", __func__);
        return -EFAULT;
    }

    if (copy_from_user(&list_p, (void *)(uintptr_t) req->data.param[0], sizeof(ieee80211_user_chanlist_t))) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY,
                        QDF_TRACE_LEVEL_INFO, "%s: Copy from user failed \n", __func__);
        return -EFAULT;
    }
    ...
    chan_count = acs->acs_uchan_list.uchan_cnt;

    list_r.chan = qdf_mem_malloc(sizeof(u_int8_t) * chan_count);
    if (list_r.chan == NULL) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY,
                QDF_TRACE_LEVEL_INFO, "%s: Unable to allocate memory\n", __func__);
        return -ENOMEM;
    }

    list_r.n_chan = wlan_acs_get_user_chanlist(vap, list_r.chan);

    if (copy_to_user((void*)(uintptr_t) req->data.param[0], (int *) &list_r.n_chan, sizeof(u_int8_t))) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY,
                        QDF_TRACE_LEVEL_INFO, "%s: Copy to user failed \n", __func__);
        ret_val = -EFAULT;
        goto end;
    }

    if (copy_to_user(list_p.chan, list_r.chan, sizeof(u_int8_t) * chan_count)) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY,
                        QDF_TRACE_LEVEL_INFO, "%s: Copy to user failed \n", __func__);
        ret_val = -EFAULT;
        goto end;
    ...
    for(i = 0; i < ic_primary_chanlist->n_chan; i++) {
        if (isclr(ic->ic_chan_avail, ic_primary_chanlist->chan[i])) {
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY,
                    QDF_TRACE_LEVEL_INFO, "%s: Channel number %d is invalid \n",
...
u_int8_t remove_nol_chan(struct ieee80211com *ic)
{
        ...
        for(i = 0; i < ic_chanlist->n_chan; i++)
        {
                ...
                if(ic->ic_dfs_chan_in_nol(ic, freq)) {
                        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY,
                                        QDF_TRACE_LEVEL_INFO, "%s: Cannot add NOL channel %d into primary allowed channel list \n",
                ...
                struct ieee80211req_athdbg *req)
{
        ...
        if(stavap) {
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY,
                                QDF_TRACE_LEVEL_INFO, "%s: Primary allowed channel selection feature is currently not implemented for repeater mode \n",
                                __func__);
        ...
        if(chanlist->n_chan == 0) {
                if(ic->ic_primary_allowed_enable) {
                        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY,
                                        ...
                                        "Primary allowed channel list is cleared \n");
                        ...
                        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY,
                                        QDF_TRACE_LEVEL_INFO,
                                        "Primary allowed channel list is empty, nothing to clear \n");
                }
                qdf_mem_free(chanlist);
                return EOK;
        }
        ...
        if(!ieee80211_check_allowed_prim_chanlist(ic, ic->ic_curchan->ic_ieee)) {
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY,
                                QDF_TRACE_LEVEL_INFO,
                                "%s: Current channel %d is not a primary allowed channel \n",
                                __func__, ic->ic_curchan->ic_ieee);
                if (ic->no_chans_available == 1) {
                        target_channel = ic->ic_random_channel(ic,
                                        IEEE80211_SELECT_NONDFS_AND_DFS,
                                        IEEE80211_SELECT_NXT_CH_POST_RADAR_DETECT, FALSE);
                        if ( target_channel != -1) {
                ...
                struct ieee80211req_athdbg *req)
{
    ...
    if(ic->ic_primary_allowed_enable) {
        ...
        if((void *)(uintptr_t) req->data.param[0] == NULL) {
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY,
                    QDF_TRACE_LEVEL_INFO,
                    "%s: User memory not allocated or corrupted \n", __func__);
            return - EFAULT;
        }
...
ieee80211acs_ioctl_setchanlist(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    ieee80211_user_chanlist_t list;
    u_int8_t *chan_list=NULL;

    if (copy_from_user(&list, (void *)(uintptr_t) req->data.param[0], sizeof(ieee80211_user_chanlist_t))) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY,
                        QDF_TRACE_LEVEL_INFO, "%s: Copy from user failed \n", __func__);
        return -EFAULT;
    }

    chan_list = qdf_mem_malloc(sizeof(u_int8_t) * list.n_chan);

    if(chan_list == NULL) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY,
                        QDF_TRACE_LEVEL_INFO, "%s: Malloc failed \n", __func__);
        return -EFAULT;
    }

    if (copy_from_user(chan_list, list.chan,
		sizeof(u_int8_t) * list.n_chan)){
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY,
                        QDF_TRACE_LEVEL_INFO, "%s: Copy from user failed \n", __func__);
        return -EFAULT;
    }
    ...
    wlan_acs_set_user_chanlist(vap,chan_list);
...
ieee80211dbg_ioctl_acs_block_channel(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    ret_val = wlan_acs_block_channel_list(vap,chan_list,list.n_chan);
...
ieee80211dbg_ioctl_ctrl_table(wlan_if_t vap, struct ieee80211req_athdbg *req)
{
    ...
    if (copy_from_user(&ctrl_tbl_user, (ieee80211_user_ctrl_tbl_t *)(uintptr_t)req->data.param[0],
    ...
    if (ctrl_tbl_user.ctrl_len > MAX_USER_CTRL_TABLE_LEN) {
         QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY,
                        QDF_TRACE_LEVEL_INFO, "%s: Maximum arguments cannot exceed %d \n",
                        __func__, MAX_USER_CTRL_TABLE_LEN);
        return -EFAULT;
    }
    ...
    if (copy_from_user(ctrl_array, ctrl_tbl_user.ctrl_array,
...
ieee80211tr69_ioctl_set_oper_rate(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    int oprate, mcs, gintval, retv = -EINVAL;
    int leg_rtmask, ht_rtmask, vht_rtmask;
    enum ieee80211_phymode phymode;
    int htflag;
    oprate = mcs = leg_rtmask = ht_rtmask = vht_rtmask = 0;
    retv = -EINVAL;
...
ieee80211tr69_ioctl_get_posiblrate(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    struct ieee80211com *ic = vap->iv_ic;
    ieee80211req_tr069_t * reqptr = &req->data.tr069_req;
    int *ratelist = (int *) reqptr->data_addr;
    int gintval, htflag;
    enum ieee80211_phymode phymode;
#if !ALL_POSSIBLE_RATES_SUPPORTED
    int nss, ch_width;
...
static bool ieee80211_tr181_push_radio_channel2(struct net_device *dev,
                                                    struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    ieee80211req_tr181_t * reqptr = &req->data.tr181_req;
    int *param = reqptr->data_addr;
    return ieee80211_ucfg_set_chanswitch(vap, param[0], param[1], param[2]);
}

static bool ieee80211_tr181_get_csa_deauth_mode(struct net_device *dev,
                                            struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    ieee80211req_tr181_t * reqptr = &req->data.tr181_req;

    return copy_to_user(reqptr->data_addr, &vap->iv_csa_deauth_mode, sizeof(int)) ? -EFAULT : 0;
}

static bool ieee80211_tr181_set_csa_deauth_mode(struct net_device *dev,
                                            struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    ieee80211req_tr181_t * reqptr = &req->data.tr181_req;
    int *param = reqptr->data_addr;
    vap->iv_csa_deauth_mode = param[0];
    return 0;
}

static bool ieee80211_tr181_get_last_beacon_rssi(struct net_device *dev,
                                            struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    struct ieee80211com *ic = vap->iv_ic;
    ieee80211req_tr181_t * reqptr = &req->data.tr181_req;
    struct ieee80211_node *ni = NULL;
    struct ieee80211req_sta_stats *stats = (struct ieee80211req_sta_stats *) reqptr->data_addr;
    struct ieee80211_nodestats *sta_stats = &stats->is_stats;

    ni = ieee80211_find_node(&ic->ic_sta, stats->is_u.macaddr);
    if (!ni) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                       "\nCould not find node with macaddr %s\n", ether_sprintf(stats->is_u.macaddr));
        return -1;
    }

    sta_stats->rssi_bcn.rssi[0] = ni->ni_mgmt_stats.ni_last_bcn_rssi & 0x000000FF;
    sta_stats->rssi_bcn.rssi[1] = ni->ni_mgmt_stats.ni_last_bcn_rssi & 0x0000FF00;
    sta_stats->rssi_bcn.rssi[2] = ni->ni_mgmt_stats.ni_last_bcn_rssi & 0x00FF0000;
    sta_stats->rssi_bcn.rssi[3] = ni->ni_mgmt_stats.ni_last_bcn_rssi & 0xFF000000;

    sta_stats->rssi_bcn.time_s[0] = ni->ni_mgmt_stats.ni_last_bcn_age & 0x000000FF;
    sta_stats->rssi_bcn.time_s[1] = ni->ni_mgmt_stats.ni_last_bcn_age & 0x0000FF00;
    sta_stats->rssi_bcn.time_s[2] = ni->ni_mgmt_stats.ni_last_bcn_age & 0x00FF0000;
    sta_stats->rssi_bcn.time_s[3] = ni->ni_mgmt_stats.ni_last_bcn_age & 0xFF000000;

    sta_stats->rssi_bcn.count = ni->ni_mgmt_stats.ni_last_bcn_cnt;
...
static bool ieee80211_tr181_get_last_ack_rssi(struct net_device *dev,
                                            struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    struct ieee80211com *ic = vap->iv_ic;
    ...
    sta_stats->rssi_ack.rssi[0] = ni->ni_ctrl_stats.ni_last_ack_rssi & 0x000000FF;
    sta_stats->rssi_ack.rssi[1] = ni->ni_ctrl_stats.ni_last_ack_rssi & 0x0000FF00;
    sta_stats->rssi_ack.rssi[2] = ni->ni_ctrl_stats.ni_last_ack_rssi & 0x00FF0000;
    sta_stats->rssi_ack.rssi[3] = ni->ni_ctrl_stats.ni_last_ack_rssi & 0xFF000000;

    sta_stats->rssi_ack.time_s[0] = ni->ni_ctrl_stats.ni_last_ack_age & 0x000000FF;
    sta_stats->rssi_ack.time_s[1] = ni->ni_ctrl_stats.ni_last_ack_age & 0x0000FF00;
    sta_stats->rssi_ack.time_s[2] = ni->ni_ctrl_stats.ni_last_ack_age & 0x00FF0000;
    sta_stats->rssi_ack.time_s[3] = ni->ni_ctrl_stats.ni_last_ack_age & 0xFF000000;

    sta_stats->rssi_ack.count = ni->ni_ctrl_stats.ni_last_ack_cnt;
...
static bool ieee80211_tr181_get_filter_ssid(struct net_device *dev,
                                            struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    ieee80211req_tr181_t * reqptr = &req->data.tr181_req;

    return copy_to_user((char *)reqptr->data_addr, vap->iv_filter_ssid, (IEEE80211_NWID_LEN + 1)) ? -EFAULT : 0;
}

static bool ieee80211_tr181_set_filter_ssid(struct net_device *dev,
                                            struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    ieee80211req_tr181_t * reqptr = &req->data.tr181_req;
    char *param = reqptr->data_addr;
    int len;

    memset(vap->iv_filter_ssid, 0, IEEE80211_NWID_LEN);

    len = strlen(param);
    if (len > IEEE80211_NWID_LEN)
        strlcpy(vap->iv_filter_ssid, param, IEEE80211_NWID_LEN + 1);
    else
        strlcpy(vap->iv_filter_ssid, param, len + 1);

    return 0;
}

static int
ieee80211dbg_ioctl_tr181(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    struct ieee80211com *ic = vap->iv_ic;
    ieee80211req_tr181_t * reqptr = &req->data.tr181_req;
    ieee80211_tr181_cmd cmdid = reqptr->cmdid;
    bool retv = 0;

    switch(cmdid){
        case TR181_GET_ACL_MODE:
            retv = ieee80211_tr181_get_acl_mode(vap, req);
            break;
        case TR181_PUSH_RADIO_CHANNEL2:
            retv = ieee80211_tr181_push_radio_channel2(dev, req);
            break;
        case TR181_SET_CSA_DEAUTH_MODE:
            retv = ieee80211_tr181_set_csa_deauth_mode(dev, req);
            break;
        case TR181_SET_FILTER_SSID:
            retv = ieee80211_tr181_set_filter_ssid(dev, req);
            break;
        case TR181_GET_LAST_BEACON_RSSI:
            retv = ieee80211_tr181_get_last_beacon_rssi(dev, req);
            break;
        case TR181_GET_LAST_ACK_RSSI:
            retv = ieee80211_tr181_get_last_ack_rssi(dev, req);
            break;
        case TR181_GET_CSA_DEAUTH_MODE:
            retv = ieee80211_tr181_get_csa_deauth_mode(dev, req);
            break;
        case TR181_GET_FILTER_SSID:
            retv = ieee80211_tr181_get_filter_ssid(dev, req);
            break;

        default:
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY,
                           QDF_TRACE_LEVEL_INFO, "Invalid IOCTL CMD: %d\n", cmdid);
            break;
    }
...
ieee80211dbg_ioctl_tr069(struct net_device *dev, struct ieee80211req_athdbg *req)
{
	...
	ieee80211_rrmutil_t *rrm_util=reqptr->data_addr;
	int retv = 0;
		...
		case TR069_CHANHIST:
			...
			break;
                case TR069_GET_RRM_UTIL:
                        ...
                        rrm_util->chann_util = ic->ic_ch_util.ap_rx +
                               ic->ic_ch_util.ap_tx;
                        rrm_util->obss_util = ic->ic_ch_util.obss_rx;
                        rrm_util->noise_floor = ic->ic_get_cur_chan_nf(ic);
			rrm_util->radar_detect = ic->ic_curchan->ic_flagext & IEEE80211_CHAN_CSA_RECEIVED;
                        break;
		case TR069_TXPOWER:
...
static int
ieee80211dbg_set_cfr_capture(struct net_device *dev,
                                      const struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    ieee80211_cfr_param_info *cfrinfo =
            (ieee80211_cfr_param_info *)(&req->data.cfrinfo);
    return wlan_set_cfr_capture(vap, req->dstmac, cfrinfo);
}
...
static int
ieee80211dbg_acl_set_cli_params(struct net_device *dev,
                                      const struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    return wlan_acl_set_cli_params(vap, req);
}

static int
ieee80211dbg_acl_get_cli_params(const struct net_device *dev,
                                      struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    return wlan_acl_get_cli_params(vap, req);
}
...
static int
ieee80211dbg_get_cfr_capture(const struct net_device *dev,
                                         struct ieee80211req_athdbg *req)
{
    const struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    ieee80211_cfr_param_info *cfrinfo =
            (ieee80211_cfr_param_info *)(&req->data.cfrinfo);
    return wlan_get_cfr_capture((struct ieee80211vap *)vap, req->dstmac, cfrinfo);
}
...
static int ieee80211dbg_son_map_get_assoc_frame(const struct net_device *dev,
                                                struct ieee80211req_athdbg *req) {
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);

    int ret = wlan_bsteering_get_assoc_frame(vap, (const u_int8_t *)req->dstmac,
...
static int
ieee80211dbg_son_map_set_rssi_policy(struct net_device *dev,
                                    struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    return wlan_bsteering_set_map_rssi_policy(vap, req);
}

static int
ieee80211dbg_son_add_mac_timer_policy(struct net_device *dev,
                                    struct ieee80211req_athdbg *req)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    return ieee80211_acl_add_with_validity(vap, req->data.client_assoc_req_acl.stamac,
...
ieee80211dbg_get_assoc_watermark_time(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    struct timespec timenow;
    uint32_t jiffies_now = 0 , jiffies_delta = 0, jiffies_then;
    struct timespec *data =(struct timespec *)(uintptr_t)req->data.param[0];
...
ieee80211dbg_ioctl_mu_scan(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    for (i = 0; i < sizeof(modes) / sizeof(modes[0]); i++) {
        ...
        TAILQ_FOREACH(tmpvap, &ic->ic_vaps, iv_next) {
            ...
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_NL, "Set freq vap stop send + %pK\n", tmpvap);
            wlan_mlme_stop_bss(tmpvap, 0);
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_NL, "Set freq vap stop send - %pK\n", tmpvap);

            waitcnt = 0;
            while (tmp_osnetdev->is_stop_event_pending && !ic->recovery_in_progress &&
                   (waitcnt < OSIF_MAX_STOP_VAP_TIMEOUT_CNT)) {
            ...
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_NL, "Set wait done -- %pK\n", tmpvap);
...
ieee80211dbg_ioctl_atf_debug_size(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    ret = ieee80211_atf_set_debug_size(ic, req->data.param[0]);
#endif
...
ieee80211dbg_ioctl_atf_dump_nodestate(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    struct ieee80211com *ic = vap->iv_ic;
    struct ieee80211_node *ni = NULL;
    struct atf_nodestate atfnodestate;

    ni = ieee80211_find_node(&ic->ic_sta, req->dstmac);
    if (ni) {
        ret = ieee80211_atf_get_debug_nodestate(ic, ni, &atfnodestate);
        if (ret) {
            ret = -EINVAL;
        } else if (copy_to_user(req->data.atf_dbg_req.ptr, &atfnodestate, sizeof(struct atf_nodestate))) {
            ret = -EFAULT;
        }
        ieee80211_free_node(ni);
    } else {
...
static int
ieee80211dbg_ioctl_atf_get_lists(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    int error = 0;
#if QCA_AIRTIME_FAIRNESS
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    ...
    buf = (struct atfgrouplist_table *)OS_MALLOC(ic->ic_osdev, sizeof(struct atfgrouplist_table), GFP_KERNEL);
    if (!buf) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: memory alloc failed\n", __func__);
        return -ENOMEM;
    }

    if (copy_from_user(buf, req->data.atf_dbg_req.ptr, sizeof(struct atfgrouplist_table))) {
    ...
    TAILQ_FOREACH(group, &ic->ic_atfgroups, group_next) {
        k = buf->info_cnt;
        OS_MEMCPY((char *)(buf->atf_list[k].grpname), (char *)group->group_name, IEEE80211_NWID_LEN);
        TAILQ_FOREACH(subgroup, &group->atf_subgroups, sg_next) {
            i = buf->atf_list[buf->info_cnt].num_subgroup;
            OS_MEMCPY(buf->atf_list[k].sg_table[i].subgrpname, (char*)subgroup->sg_name, IEEE80211_NWID_LEN);
            buf->atf_list[buf->info_cnt].num_subgroup++;
        }
        buf->info_cnt++;
    }

    if (copy_to_user(req->data.atf_dbg_req.ptr, buf, sizeof(struct atfgrouplist_table))) {
...
ieee80211dbg_ioctl_atf_dump_debug(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    u_int32_t buf[2];
    struct atf_stats *stats = req->data.atf_dbg_req.ptr;
    struct group_list *group = NULL;
    struct subgroup_list *subgroup = NULL;

    ret = 0;
    TAILQ_FOREACH(group, &ic->ic_atfgroups, group_next) {
        if ((strncmp(group->group_name, stats->group_name, strlen(stats->group_name))) ||
            (strlen(group->group_name) != strlen(stats->group_name)) ) {
            continue;
        }
        TAILQ_FOREACH(subgroup, &group->atf_subgroups, sg_next) {
            if ((strncmp(subgroup->sg_name, stats->subgroup_name, strlen(stats->subgroup_name))) ||
                (strlen(subgroup->sg_name) != strlen(stats->subgroup_name)) ) {
                continue;
            }
            ret = ieee80211_atf_get_debug_dump(ic, subgroup, &ptr, &size, &id);
            if (ptr && req->data.atf_dbg_req.ptr &&
                ...
                if (copy_to_user(req->data.atf_dbg_req.ptr, buf, sizeof(buf))) {
                    ...
                    if (copy_to_user((u_int8_t *)(req->data.atf_dbg_req.ptr) + sizeof(buf), ptr, size)) {
                    ret = -EFAULT;
                    goto end;
                    }
                }
...
static int
ieee80211dbg_acl_set_softblocking(const struct net_device *dev,
                                         struct ieee80211req_athdbg *req)
{
    const struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    return wlan_acl_set_softblocking((struct ieee80211vap *)vap, req->dstmac, req->data.acl_softblocking);
}
static int
ieee80211dbg_acl_get_softblocking(const struct net_device *dev,
                                         struct ieee80211req_athdbg *req)
{
    const struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    req->data.acl_softblocking = wlan_acl_get_softblocking((struct ieee80211vap *)vap, req->dstmac);
    return EOK;
}
#endif

static int ieee80211dbg_son_map_get_apcap(const struct net_device *dev,
                                          struct ieee80211req_athdbg *req) {
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    int error = EOK;

    error = wlan_bsteering_get_apcap(vap, &req->data.mapapcap);
    return error;
}
static int ieee80211dbg_son_map_get_operable_channels(const struct net_device *dev,
                                                      struct ieee80211req_athdbg *req) {
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    int error = EOK;

    error = wlan_bsteering_get_operable_channels(vap, &req->data.map_op_chan);
    return error;
}
    ...
    void *w, char *extra)
{
    ...
    switch (req->cmd) {
        ...
        case IEEE80211_DBGREQ_GET_RRM_STA_LIST:
            {
                ieee80211_sta_info_t *rrm_info  = (ieee80211_sta_info_t *) (uintptr_t)req->data.param[0];
                unsigned char *addr_list = (unsigned char *)(uintptr_t)req->data.param[1];
                retv = wlan_get_rrm_sta_list(vap, req->dstmac, req->data.param);
                rrm_info->count = req->data.param[0];
                addr_list = (unsigned char *)(uintptr_t)req->data.param[1];
            }
            break;
#endif
        case IEEE80211_DBGREQ_GET_BTM_STA_LIST:
            {
                ieee80211_sta_info_t *btm_info  = (ieee80211_sta_info_t *) (uintptr_t)req->data.param[0];
                unsigned char *addr_list = (unsigned char *)(uintptr_t)req->data.param[1];
                retv = wlan_get_btm_sta_list(vap, req->dstmac, req->data.param);
                btm_info->count = req->data.param[0];
                addr_list = (unsigned char *)(uintptr_t)req->data.param[1];
            }
            break;
        ...
        case IEEE80211_DBGREQ_DISPLAY_TRAFFIC_STATISTICS:
            {
                ieee80211_node_info_t *node_info = (ieee80211_node_info_t *)(uintptr_t)req->data.param[0];
                unsigned char *addr_list = (unsigned char *)(uintptr_t)req->data.param[1];
                ieee80211_noise_stats_t *noise_info = (ieee80211_noise_stats_t *)(uintptr_t)req->data.param[3];
                ...
                addr_list = (unsigned char *)(uintptr_t)req->data.param[1];
                noise_info = (ieee80211_noise_stats_t *)(uintptr_t)req->data.param[3];
            }
            break;
#if UMAC_SUPPORT_WNM
        ...
        case IEEE80211_DBGREQ_TR069:
            ...
            break;
        case IEEE80211_DBGREQ_TR181:
            retv = ieee80211dbg_ioctl_tr181(dev, req);
            break;
#if ATH_SUPPORT_FIPS
...
#define ATH_FIPS_TIMEOUT ((CONVERT_SEC_TO_SYSTEM_TIME(1)/10) + 1) /* 100 ms*/
                ...
                if(ic->ic_output_fips) {
                    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s received fips event %d %pK length %d\n", __func__,
    ...
    case IEEE80211_DBGREQ_CFR_SET_PARAMS:
        retv = ieee80211dbg_set_cfr_capture(dev,req);
        break;

    case IEEE80211_DBGREQ_CFR_GET_PARAMS:
        retv = ieee80211dbg_get_cfr_capture(dev,req);
        if (EOK == retv) {
    ...
    case IEEE80211_DBGREQ_OFFCHAN_TX:
        ...
        case IEEE80211_DBGREQ_BSTEERING_GET_PROBE_RESP_WH:
            ...
            break;
        case IEEE80211_DBGREQ_ACL_SET_CLI_PARAMS:
            retv = ieee80211dbg_acl_set_cli_params(dev, req);
            break;

        case IEEE80211_DBGREQ_ACL_GET_CLI_PARAMS:
            retv = ieee80211dbg_acl_get_cli_params(dev, req);
            if (EOK == retv) {
                retv = (copy_to_user(wri->pointer, req, sizeof(*req))) ?
                    -EFAULT : 0;
             }
    ...
    case IEEE80211_DBGREQ_MAP_CLIENT_CAP:
        retv = ieee80211dbg_son_map_get_assoc_frame(dev, req);
        if (EOK == retv) {
            retv = (copy_to_user(wri->pointer, req, sizeof(*req))) ? -EFAULT : 0;
        }
        break;
#if ATH_ACL_SOFTBLOCKING
    ...
    case IEEE80211_DBGREQ_MAP_RADIO_HWCAP:
        retv = ieee80211dbg_son_map_get_apcap(dev, req);
        if (EOK == retv) {
            retv = (copy_to_user(wri->pointer, req, sizeof(*req))) ? -EFAULT : 0;
        }
        break;
    case IEEE80211_DBGREQ_BSTEERING_MAP_SET_RSSI:
        retv = ieee80211dbg_son_map_set_rssi_policy(dev, req);
        break;
    case IEEE80211_DBGREQ_ADD_MAC_VALIDITY_TIMER_ACL:
        retv = ieee80211dbg_son_add_mac_timer_policy(dev, req);
    break;
    case IEEE80211_DBGREQ_MAP_GET_OP_CHANNELS:
        retv = ieee80211dbg_son_map_get_operable_channels(dev, req);
        if (EOK == retv) {
            retv = (copy_to_user(wri->pointer, req, sizeof(*req))) ?
                -EFAULT : 0;
        }
 ...
 */
int wlan_get_traffic_stats(wlan_if_t vap, u_int8_t *macaddr, u_long *param)
{
    struct ieee80211com *ic = vap->iv_ic;
    ...
    if(IEEE80211_ADDR_EQ(macaddr, IEEE80211_GET_BCAST_ADDR(vap->iv_ic)))
    {
        ...
        node_info.dest_addr = (u_int8_t *)(uintptr_t)param[1];
        node_info.noise_stats = (ieee80211_noise_stats_t *)(uintptr_t)param[3];
        node_info.count = 0;
    ...
    void *w, char *extra)
{
    ...
    switch(u->data.flags)
    {
        ...
        case IEEE80211_PARAM_GET_CONFIG_REJECT_MGMT_FRAME:
            for (i = 0; i < IEEE80211_REJECT_MGMT_MAX; i++) {
                if (IEEE80211_IS_BROADCAST(vap->iv_conf_rej_mgmt[i].addr))
                    continue;
                qdf_mem_copy(&(sa[num_mac]).sa_data, vap->iv_conf_rej_mgmt[i].addr, IEEE80211_ADDR_LEN);
                sa[num_mac].sa_family = ARPHRD_ETHER;
                num_mac++;
            }
            u->data.length = num_mac;
            break;

        case IEEE80211_PARAM_ADD_MAC_REJECT_MGMT_FRAME:
            sa = (struct sockaddr *)u->data.pointer;
            rc = wlan_update_mgmt_reject(vap, sa->sa_data, 1);
            if (rc)
                return EFAULT;
            break;

        case IEEE80211_PARAM_DEL_MAC_REJECT_MGMT_FRAME:
            sa = (struct sockaddr *)u->data.pointer;
            rc = wlan_update_mgmt_reject(vap, sa->sa_data, 0);
            if (rc)
                return EFAULT;
            break;
...
ieee80211_ioctl_getstastats(struct net_device *dev, struct iwreq *iwr)
{
    ...
    /* passing recv rssi to user */
    ni->ni_stats.ns_rssi = ni->ni_rssi;

    /* update time delta in ms . We assume current time is always greater than
     * the RSSI measured time
     */
    ni->ni_stats.ns_rssi_time_delta =
        CONVERT_SYSTEM_TIME_TO_MS(OS_GET_TIMESTAMP() - ni->ni_rssi_measured_time);
...
#if DBDC_REPEATER_SUPPORT
/* To find the scan entry of RootAP*/
static void
get_preferred_bssid(struct scanreq *req, wlan_scan_entry_t se)
{
    u_int8_t *ssid;
    u_int8_t ssid_len;
    ...
    if (vap->iv_psta && !vap->iv_mpsta){
        /*For PSTAs no need find preferred bssid*/
        return;
    }
#endif
    ...
    if (des_ssid_found) {
	extender_ie = wlan_scan_entry_extenderie(se);
	if (!extender_ie) {
	    /*When RootAP is present, give preferrence to RootAP bssid*/
	    req->preferred_bssid = wlan_scan_entry_bssid(se);
	     QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Preferred RootAP bssid:%s ssid:%s \n",ether_sprintf(req->preferred_bssid), ssid);
	}
    }
    return;
}
...
/* Process the scan entries whose ssid matches with desired ssid
   return 1- scan entry will be given to wpa_supplicant
   return 0- scan entry will be dropped*/

static int
process_desired_ssid (struct scanreq *req, wlan_scan_entry_t se, u_int8_t *bssid)
{
    struct ieee80211vap *vap = req->vap;
    struct ieee80211com *ic = vap->iv_ic;
...
#if ATH_SUPPORT_WRAP
    if (vap->iv_psta && !vap->iv_mpsta){
        /*For PSTAs provide only the scan entry whose bssid matched
          with MPSTA bss node mac*/
         mpsta_vap = ic->ic_mpsta_vap;
         ni = mpsta_vap->iv_bss;
         if (OS_MEMCMP(ni->ni_macaddr, bssid, IEEE80211_ADDR_LEN) ==0) {
             return 1;
         } else {
             return 0;
         }
...
#endif
    if (req->preferred_bssid) {
        if (OS_MEMCMP(req->preferred_bssid, bssid, IEEE80211_ADDR_LEN)!=0) {
            return 0;
        } else {
	    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Bssid matched with preferred RootAP bssid:%s\n",ether_sprintf(bssid));
            return 1;
        }
    }
    extender_ie = wlan_scan_entry_extenderie(se);
	...
	if (extender_ie) {
	    ...
	    if (ic_list->num_rptr_clients && ((*extender_ie & STAVAP_CONNECTION_MASK) == STAVAP_CONNECTION_MASK) && ((*extender_ie & ROOTAP_ACCESS_MASK) != ROOTAP_ACCESS_MASK)) {
	        return 0;
	    }
	}
    } else {
	...
	if (ic_list->ap_preferrence == 1) {
	    ...
	    for(i=0;i<MAX_RADIO_CNT;i++) {
		if(OS_MEMCMP(bssid, &ic_list->preferred_mac_list[i][0], IEEE80211_ADDR_LEN) == 0) {
	            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Bssid matched with preferred mac list:%s\n",ether_sprintf(bssid));
		    rptr_bssid_found = 1;
                    break;
		}
	    }
...
#if DBDC_REPEATER_SUPPORT
    if (ic_list->same_ssid_support && vap->iv_wps_session) {
        se_wps_ie = wlan_scan_entry_wps(se);
        if (se_wps_ie) {
            goto add_entry;
        }
    }
...
#if DBDC_REPEATER_SUPPORT
                if (ic_list->same_ssid_support) {
                    ret = process_desired_ssid(req, se, bssid);
                    if (ret == 0) {
                        return 0;
                    }
                }
...
ieee80211_ioctl_getscanresults(struct net_device *dev, struct iwreq *iwr)
{
    ...
    if (!(((osifp->os_opmode == IEEE80211_M_STA) && !osifp->is_up ) ||
        ...
        {
            osifp->os_giwscan_count++;
            QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "scan_in_progress opmode:%d vap-%d(%s) \n",
                    vap->iv_opmode,vap->iv_unit,vap->iv_netdev_name);
            return -EAGAIN;
        }
    }
...
/**
 * ieee80211_ioctl_setatfssid() - Configure SSID based atf
 * @dev: netdev structure
 * @iwr: user config
 *
 * Return: 0 on success
 *         -EIVAL on error
...
/**
 * ieee80211_ioctl_delatfssid() - Delete ssid based ATF config
 * @dev: netdev structure
 * @iwr: user config
 *
 * Return: 0 on success
...
/**
 * ieee80211_ioctl_setatfsta() - Configure Peer based ATF
 * @dev: netdev structure
 * @iwr: user config
 *
 * Return: 0 on success
...
/**
 * ieee80211_ioctl_delatfssid() - Delete Peer based ATF config
 * @dev: netdev structure
 * @iwr: user config
 *
 * Return: 0 on success
...
/**
 * ieee80211_atf_grouptable() - shows the configured groups & associated SSID
 * @dev: netdev structure
 * @iwr: user config
 *
 * Return: 0 on success
            ...
            for (j = 0, init_flag = 0; j < ATF_ACTIVED_MAX_CLIENTS; j++)
            {
                            ...
                            (ni->ni_vap->iv_opmode == IEEE80211_M_STA)))
                {
                    ...
                    if(init_flag == 0)
                    {
                        ...
                        buf->atf_info[buf->info_cnt].atf_ac_cfg = ic->atfcfg_set.atfgroup[i].atf_cfg_ac.ac_bitmap;
                        buf->info_cnt++;
                        init_flag++;
                        grp_index[i] = i+1;
                        k += 1;
                    ...
                    OS_MEMCPY((char *)(buf->atf_info[buf->info_cnt].ssid),(char *)(ni->ni_vap->iv_bss->ni_essid),strlen(ni->ni_vap->iv_bss->ni_essid));
                     buf->atf_info[buf->info_cnt].cfg_value = ic->atfcfg_set.peer_id[j].sta_cfg_value[ic->atfcfg_set.peer_id[j].index_group];
                    /* Show global configuration if we don't have group specific Peer percentage */
            ...
            for (j = 0, init_flag = 0; j < ATF_ACTIVED_MAX_CLIENTS; j++)
            {
                            ...
                            (ni->ni_vap->iv_opmode == IEEE80211_M_STA)))
                {
                    ...
                    if(init_flag == 0)
                    {
                        ...
                        buf->atf_info[buf->info_cnt].cfg_value = ic->atfcfg_set.vap[i].vap_cfg_value;
                        buf->atf_info[buf->info_cnt].info_mark = 0;
                        buf->atf_info[buf->info_cnt].atf_ac_cfg = ic->atfcfg_set.vap[i].atf_cfg_ac.ac_bitmap;
                        buf->info_cnt++;
                        init_flag++;
                        vap_index[i] = i+1;
                        k += 1;
...
/**
 * ieee80211_atf_is_default_subgroup - Check if it's default subgroup
 * @subgrpname: Subgroup name
 *
 * Return: 1 if subgroup name matches with default subgroup
 *         0 if subgroup name doesn't match with default subgroup
 *         -EIVAL on error
 */
int8_t ieee80211_atf_is_default_subgroup(u_int8_t *subgrpname)
{
    ...
    if (!(strncmp(subgrpname, DEFAULT_SUBGROUPNAME, strlen(DEFAULT_SUBGROUPNAME))) &&
        (strlen(subgrpname) == strlen(DEFAULT_SUBGROUPNAME))) {
        ret = 1;
    }
...
/**
 * ieee80211_atf_is_default_group - Check if it's default group
 * @grpname: Group name
 *
 * Return: 1 if group name matches with default group
 *         0 if group name doesn't match with default group
 *         -EIVAL on error
 */
int8_t ieee80211_atf_is_default_group(u_int8_t *grpname)
{
    ...
    if (!(strncmp(grpname, DEFAULT_GROUPNAME, strlen(DEFAULT_GROUPNAME))) &&
        (strlen(grpname) == strlen(DEFAULT_GROUPNAME))) {
        ret = 1;
    }
...
static int ieee80211_ioctl_atf_flushtable(struct net_device *dev, struct iwreq *iwr)
{
    int ret = 0;
    osif_dev  *osifp = ath_netdev_priv(dev);
    wlan_if_t vap = osifp->os_if;

    ret = ieee80211_ucfg_atf_flushtable(vap);

    return ret;
}

/**
 * ieee80211_ioctl_addatfac() - add an ATF AC configuration
 ...
 *         -EINVAL on error
 */
int ieee80211_ioctl_addatfac(struct net_device *dev, struct iwreq *iwr)
{
    osif_dev *osifp = ath_netdev_priv(dev);
    wlan_if_t vap = osifp->os_if;
    struct ieee80211com *ic = vap->iv_ic;
    ...
    buf = (struct atf_ac *)OS_MALLOC(osifp->os_handle, sizeof(struct atf_ac), GFP_KERNEL);
    if (!buf) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: memory alloc failed\n", __func__);
        return -ENOMEM;
    }
    ...
    error = ieee80211_ucfg_addatfac(vap, buf);
    kfree(buf);
...
/**
 * ieee80211_ioctl_delatfac() - Delete an ATF AC configuration
 * @dev: netdevice structure
 * @iwr: User config
 *
 * Return: 0 on success
 *         -EINVAL on error
 */
int ieee80211_ioctl_delatfac(struct net_device *dev, struct iwreq *iwr)
{
    osif_dev *osifp = ath_netdev_priv(dev);
    wlan_if_t vap = osifp->os_if;
    struct ieee80211com *ic = vap->iv_ic;
    ...
    error = ieee80211_ucfg_delatfac(vap, buf);
    kfree(buf);
...
/**
 * ieee80211_ioctl_addatfgroup() - Add a new ATF group
 * create a new group & add ssid to the group, if group doesn't exist
 * add ssid to the group if group exists
 * @dev: netdevice structure
 * @iwr: User config
 *
 * Return: 0 on success
...
ieee80211_ioctl_addatfgroup(struct net_device *dev, struct iwreq *iwr)
{
    ...
    error = ieee80211_ucfg_addatfgroup(vap, buf);
    kfree(buf);
...
/**
 * ieee80211_ioctl_configatfgroup() - Config Airtime for ATF group
 * @dev: netdev structure
 * @iwr: user config
 *
 * Return: 0 on success
...
ieee80211_ioctl_configatfgroup(struct net_device *dev, struct iwreq *iwr)
{
    ...
    if (!ic->ic_atf_ssidgroup) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                        "SSID grouping need to be enabled before group configuration \n\r");
        return -EINVAL;
    }

   /* Find the Total Airtime configured (for all groups) */
    for (i = 0; i < ic->atfcfg_set.grp_num_cfg; i++) {
        if (!ieee80211_atf_is_default_group(ic->atfcfg_set.atfgroup[i].grpname))
    ...
    error = ieee80211_ucfg_configatfgroup(vap, buf, airtime_group);
    kfree(buf);
...
/**
 * ieee80211_ioctl_atfgroupsched() - Scheduling policy for group
 * @dev: netdev structure
 * @iwr: user config
 *
 * Return: 0 on success
...
ieee80211_ioctl_atfgroupsched(struct net_device *dev, struct iwreq *iwr)
{
    ...
    if (!ic->ic_atf_ssidgroup) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                        "SSID grouping need to be enabled before configuring group schedule policy \n\r");
        return -EINVAL;
    }
    ...
    error = ieee80211_ucfg_atfgroupsched(vap, buf);
    kfree(buf);
...
/**
 * ieee80211_ioctl_delatfgroup() - Delete ATF group
 * @dev: netdev structure
 * @iwr: user config
 *
 * Return: 0 on success
...
ieee80211_ioctl_delatfgroup(struct net_device *dev, struct iwreq *iwr)
{
    ...
    error = ieee80211_ucfg_delatfgroup(vap, buf);
    kfree(buf);
...
/**
 * ieee80211_ioctl_showatfsubgroup() - Shows ATF Subgroups configured
 * Configured subgroups & corresponding airtime will be listed
 * @dev: netdev structure
 * @iwr: user config
 *
 * Return: 0 on success
...
ieee80211_ioctl_showatfsubgroup(struct net_device *dev, struct iwreq *iwr)
{
    ...
    buf = (struct atfgrouplist_table *)OS_MALLOC(osifp->os_handle, sizeof(struct atfgrouplist_table), GFP_KERNEL);
    if (!buf) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: memory alloc failed\n", __func__);
        return -ENOMEM;
    }

    if (copy_from_user(buf, iwr->u.data.pointer, sizeof(struct atfgrouplist_table))) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: copy_from_user error\n", __func__);
        kfree(buf);
        return -EFAULT;
    }
    ...
    TAILQ_FOREACH(group, &ic->ic_atfgroups, group_next) {
        ...
        TAILQ_FOREACH(subgroup, &group->atf_subgroups, sg_next) {
            i = buf->atf_list[buf->info_cnt].num_subgroup;
            OS_MEMCPY(buf->atf_list[k].sg_table[i].subgrpname, (char*)subgroup->sg_name, IEEE80211_NWID_LEN);
            buf->atf_list[k].sg_table[i].subgrp_type = subgroup->sg_type;
            buf->atf_list[k].sg_table[i].subgrp_value = subgroup->sg_atf_units;
            if (subgroup->sg_type == IEEE80211_ATF_SUBGROUP_TYPE_PEER) {
                OS_MEMCPY(buf->atf_list[k].sg_table[i].peermac, subgroup->sg_peermac, IEEE80211_ADDR_LEN);
            } else if(subgroup->sg_type == IEEE80211_ATF_SUBGROUP_TYPE_AC) {
                buf->atf_list[k].sg_table[i].subgrp_ac_id = subgroup->ac_id;
            }
            buf->atf_list[k].sg_table[i].subgrp_cfg_value = subgroup->usr_cfg_val;
            buf->atf_list[buf->info_cnt].num_subgroup++;
        }
        buf->info_cnt++;
    }

    if (copy_to_user(iwr->u.data.pointer, buf, sizeof(struct atfgrouplist_table))) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: copy_to_user failed\n",__func__);
        error =  -EFAULT;
    }
    kfree(buf);
...
ieee80211_ioctl_showatfgroup(struct net_device *dev, struct iwreq *iwr)
{
    ...
    for (i = 0; i < ic->atfcfg_set.grp_num_cfg; i++)
    {
        ...
        buf->atf_groups[buf->info_cnt].grp_sched = ic->atfcfg_set.atfgroup[i].grp_sched_policy;
...
static const struct iw_priv_args ieee80211_priv_args[] = {
   ...
   /* Video debug parameters */
    ...
    { IEEE80211_PARAM_STEALTH_DOWN,
        IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "stealthdown" },
    { IEEE80211_PARAM_STEALTH_DOWN,
        0,IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_stealthdown" },
   ...
   { IEEE80211_PARAM_GET_ACS,
    ...
    { IEEE80211_PARAM_GET_CONFIG_REJECT_MGMT_FRAME,
    0, IW_PRIV_TYPE_ACLMACLIST, "g_mgmt_rjct" },
    ...
    { IEEE80211_PARAM_ADD_MAC_REJECT_MGMT_FRAME,
      IW_PRIV_TYPE_ADDR | IW_PRIV_SIZE_FIXED | 1, 0, "add_mgmt_rjct" },
    { IEEE80211_PARAM_DEL_MAC_REJECT_MGMT_FRAME,
      IW_PRIV_TYPE_ADDR | IW_PRIV_SIZE_FIXED | 1, 0, "del_mgmt_rjct" },
    ...
    { IEEE80211_PARAM_PROBEREQ_EVENT,
        IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "probereq_ev" },
    { IEEE80211_PARAM_PROBEREQ_EVENT,
        0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_probereq_ev" },
  ...
  { IEEE80211_PARAM_RXINFO_PERPKT,
    ...
    { IEEE80211_PARAM_DFS_DISABLE_CHAN_CHANGE,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "dis_chanchange" },
    { IEEE80211_PARAM_DFS_SELECT_RANDOM_CHAN_AFTER_SPOOF_TEST_FAIL,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "random_chan" },
    { IEEE80211_PARAM_MAP,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "map" },
    { IEEE80211_PARAM_MAP, 0,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_map" },
    { IEEE80211_PARAM_MAP_BSS_TYPE,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "MapBSSType" },
    { IEEE80211_PARAM_MAP_BSS_TYPE, 0,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_MapBSSType" },
    { IEEE80211_PARAM_DROP_MDNS,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "dropmdns" },
    { IEEE80211_PARAM_DROP_MDNS, 0,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_dropmdns" },
    ...
    { IEEE80211_PARAM_MAX_SCAN_TIME_ACS_REPORT, 0,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_acsmaxscantime" },
#ifdef ATH_SUPPORT_RDKB
    { IEEE80211_PARAM_Q_UTIL_INTERVAL_MS,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "qutil_intval" },
    { IEEE80211_PARAM_Q_UTIL_INTERVAL_MS, 0,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_qutil_intval" },
...
#endif
    { IEEE80211_PARAM_MAP_VAP_BEACONING,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "mapvapup" },
    { IEEE80211_PARAM_MAP_VAP_BEACONING, 0,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_mapvapup" },
};
        ...
        if (i == ATF_TPUT_MAX_STA) {
            for (i = 0; i < ATF_TPUT_MAX_STA; i++) {
                if (!OS_MEMCMP(ic->ic_atf_tput_tbl[i].mac_addr, mac, IEEE80211_ADDR_LEN)) {
                    ...
                    if (group) {
                        subgroup = ieee80211_atf_new_subgroup_list(ic, group, buf.sta_mac, 0, IEEE80211_ATF_SUBGROUP_TYPE_PEER);
                        if (subgroup == NULL) {
                            qdf_print("%s ERROR!! Failed to create new subgroup\n",__func__);
                        }
                    } else {
                        qdf_print("%s Default group could not be found. Failed to create new subgroup\n",__func__);
                    }
                    break;
                }
            }
...
ieee80211_ioctl_delsta_tput(struct net_device *dev, struct iwreq *iwr)
{
    ...
    if (!OS_MEMCMP(buf.sta_mac, mac, IEEE80211_ADDR_LEN)) {
        /* Clear whole table */
        for (i = 0; i < ATF_TPUT_MAX_STA; i++) {
            ieee80211_atf_mark_del_subgroup_list(ic, ic->ic_atf_tput_tbl[i].mac_addr);
    ...
    for (t = 0; t < WME_NUM_TID; t++) {
        ...
        if (tid->seq_start != 0 ||
            ...
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,  "\n   %pK: no=%02d ac=0x%pK hwq=%d sch=%d p=%d bp=%d cl=%d f=%d ",
                    ...
                    tid->sched, qdf_atomic_read(&tid->paused), tid->bar_paused, tid->cleanup_inprogress, tid->filtered);
...
void
wlan_debug_dump_nodes_tgt(struct ieee80211vap *vaphandle)
{
    struct ieee80211com *ic;
    struct ieee80211vap *vap;
    struct ieee80211_node_table *nt;
    ...
    for_each_netdev(&init_net, dev) {
        ...
        TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {
            ...
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "      ps_pending=%d, ps_sta=%d, sta_assoc=%d, aid_count=%d, iv_bss=%pK, is_deleted=%d\n",
        ...
        TAILQ_FOREACH_SAFE(ni, &nt->nt_node, ni_list, next) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "0x%pK: %pM flags=0x%08x refcnt=%03d (%s)\n",
        ...
        TAILQ_FOREACH(ni, &ic->ic_nodes, ni_alloc_list) {
            ...
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "0x%pK: %pM flags=0x%08x refcnt=%03d (%s) bss=%d temp=%d in_tbl=%d dqlen=%u, mqlen=%u, bss_node=%pK, is_obss=%d, assoc_msec=%d\n",
                ni, ni->ni_macaddr, ni->ni_flags, ieee80211_node_refcnt(ni),
                (((osif_dev *)ni->ni_vap->iv_ifp)->netdev)->name,
