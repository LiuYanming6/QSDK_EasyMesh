...
void 
ah_tx99_tgt_channel_pwr_update(struct ath_hal *ah, HAL_CHANNEL *c, u_int32_t txpower)
{
...
void
ah_tx99_tgt_chainmsk_setup(struct ath_hal *ah, int tx_chainmask)
{
    if (tx_chainmask == 0x5) {
...
void
ah_tx99_set_single_carrier(struct ath_hal *ah, int tx_chain_mask, int chtype)
{
    OS_REG_WRITE(ah, 0x98a4, OS_REG_READ(ah, 0x98a4) | (0x7ff<<11) | 0x7ff);
    OS_REG_WRITE(ah, 0xa364, OS_REG_READ(ah, 0xa364) | (1<<7) | (1<<1));
    OS_REG_WRITE(ah, 0xa350, (OS_REG_READ(ah, 0xa350) | (1<<31) | (1<<15)) & ~(1<<13));
...
void 
ah_tx99_tgt_start(struct ath_hal *ah, u_int8_t *data)
{
    uint32_t val;
...
void 
ah_tx99_tgt_stop(struct ath_hal *ah)
{
    OS_REG_WRITE(ah, AR_PHY_TEST, OS_REG_READ(ah, AR_PHY_TEST) &~ PHY_AGC_CLR);
    OS_REG_WRITE(ah, AR_DIAG_SW, OS_REG_READ(ah, AR_DIAG_SW) &~ (AR_DIAG_FORCE_RX_CLEAR | AR_DIAG_IGNORE_VIRT_CS));
}
