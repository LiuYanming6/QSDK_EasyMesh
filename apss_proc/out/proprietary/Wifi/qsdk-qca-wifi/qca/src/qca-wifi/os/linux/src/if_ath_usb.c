...
static void ath_usb_disconnect(struct usb_interface *interface)
  ...
  {    
    ...
    if(i == g_magpie_usb_ifc_max) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s: interface %pK not found\n", __func__, interface);
        return;
    }
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s, terminate htc thread %pK\n", __func__, os_hdl->athHTC_task);
