...
extern int ieee80211_get_precac_chan_state(struct ieee80211com *ic, uint8_t chan);
        ...
        TAILQ_FOREACH(tmpvap, &ic->ic_vaps, iv_next) {
            ...
            if(ic->ic_is_mode_offload(ic)) {
                if (ic->ic_vap_set_param) {
                    retv = ic->ic_vap_set_param(tmpvap,IEEE80211_FIXED_NSS,nss);
                    if (retv == EOK) {
                        tmpvap->iv_nss = nss;
                    } else {
                        qdf_print("%s: vap %d :%p Failed to configure NSS from %d to %d \n",
                                __func__,tmpvap->iv_unit,tmpvap,tmpvap->iv_nss,nss);
                    }
                }
            }
        }
...
#else
            if ((value > 0) && (value < (IEEE80211_512_AID + 1))) {
                if (scn->target_type == TARGET_TYPE_AR9888) {
                    if (value < (IEEE80211_128_AID + 1))
                        ic->ic_num_clients = value;
                    else {
                        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "AR9888 PLATFORM: Range 1-128 clients");
                        retval = -EINVAL;
                        break;
                   }
                } else
                ...
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Max clients Range 1-512");
                retval = -EINVAL;
                break;
            }
#endif
        ...
        case OL_ATH_PARAM_PERIODIC_CFR_CAPTURE:
        {
            ...
            if (!value) {
             /* disable periodic cfr capture */
                ic->ic_cfr_timer_enable = value;
                retval = ol_ath_pdev_set_param(scn,
                                wmi_pdev_param_per_peer_prd_cfr_enable, WMI_HOST_PEER_CFR_TIMER_DISABLE, 0);
            } else {
             /* enable periodic cfr capture (default base timer is 10ms ) */
                ic->ic_cfr_timer_enable = 1;
                retval = ol_ath_pdev_set_param(scn,
                                wmi_pdev_param_per_peer_prd_cfr_enable, WMI_HOST_PEER_CFR_TIMER_ENABLE, 0);
            }
        }
        break;
...
#undef ARP_RESET
                ...
                if (scn->btcoex_support) {
                    if (ol_ath_pdev_set_param(scn, wmi_pdev_param_enable_btcoex, val, 0) == EOK) {
                        ...
                        if (scn->btcoex_enable) {
                            if (scn->btcoex_wl_priority == 0) {
                                scn->btcoex_wl_priority  = WMI_HOST_PDEV_VI_PRIORITY_BIT |
                                    WMI_HOST_PDEV_BEACON_PRIORITY_BIT |
                                    WMI_HOST_PDEV_MGMT_PRIORITY_BIT;

                                if (ol_ath_btcoex_wlan_priority(scn, scn->btcoex_wl_priority) != EOK) {
                                    qdf_print("%s Assigning btcoex_wlan_priority:%d failed \n",
                                            __func__,scn->btcoex_wl_priority);
                                    return -ENOMEM;
                                } else {
                                    qdf_print("%s Set btcoex_wlan_priority:%d \n",
                                            __func__,scn->btcoex_wl_priority);
                                }
                            }
                            if (scn->btcoex_duty_cycle && (scn->btcoex_period <= 0)) {
                                if ( ol_ath_btcoex_duty_cycle(scn,DEFAULT_PERIOD,DEFAULT_WLAN_DURATION) != EOK ) {
                                    qdf_print("%s Assigning btcoex_period:%d btcoex_duration:%d failed \n",
                                            __func__,DEFAULT_PERIOD,DEFAULT_WLAN_DURATION);
                                    return -ENOMEM;
                                } else {
                                    scn->btcoex_period = DEFAULT_PERIOD;
                                    scn-> btcoex_duration = DEFAULT_WLAN_DURATION;
                                    qdf_print("%s Set default val btcoex_period:%d btcoex_duration:%d \n",
                                            __func__,scn->btcoex_period,scn-> btcoex_duration);
                                }
                            }
                        }
                    } else {
                        qdf_print("%s Failed to send enable btcoex cmd:%d \n",__func__,val);
                        return -ENOMEM;
                    }
                } else {
                    retval = -EPERM;
        ...
        case OL_ATH_PARAM_PRECAC_ENABLE:
            if ((scn->target_type == TARGET_TYPE_QCA9984) &&
                ...
                ic->ic_precac_enable = 0;
            if (ic->ic_precac_timer_running) {
                /* Cancel the precac timer */
                printk("%s :Precac flag changed. Cancel the precac timer\n",__func__);
        ...
        case OL_ATH_PARAM_PRECAC_INTER_CHANNEL:
            {
                ...
                freq = ieee80211_ieee2mhz(ic, value, 0);
                c = ieee80211_find_channel(ic, freq, 0, IEEE80211_CHAN_11AC_VHT80);

                if (c == NULL) {
                    printk("%s:Channel not found\n", __func__);
                    retval = -EINVAL;
                } else if (IEEE80211_IS_CHAN_DFS(c)) {
                    printk("%s:Intermediate channel should not be DFS channel!!\n", __func__);
                    retval = -EINVAL;
                } else
                     ic->ic_precac_inter_chan = value;
            }
            break;
        ...
        case OL_ATH_PARAM_DYN_AUTO_BURST:
        {
            value = !!value;
            if (scn->dyn_auto_burst != value) {
                if (ol_ath_pdev_set_param(scn, wmi_pdev_param_dyn_auto_burst, value, 0) != EOK) {
                    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
                                   "ERROR - dynamic auto burst could not be %s\n", value ? "enabled" : "disabled");
                } else {
                    scn->dyn_auto_burst = value;
                }
            }
        }
        break;
        ...
        case OL_ATH_PARAM_ESP_PERIODICITY:
            if (value < 0 || value > 5000){
                qdf_print("Invalid value! Periodicity value should be between 0 and 5000 \n");
                retval = -EINVAL;
            }
            else{
                if (ol_ath_pdev_set_param(scn, wmi_pdev_param_esp_indication_period, value, 0) != EOK)
                    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ERROR - Param setting failed. Periodicity value 0x%x.\n", value);
                else{
        ...
        case OL_ATH_PARAM_ESP_AIRTIME:
            if (value < 0 || value > 255) {
                qdf_print("Invalid value! Airtime value should be between 0 and 255 \n");
                retval = -EINVAL;
            }
            else {
        ...
        case OL_ATH_PARAM_ESP_PPDU_DURATION:
            if (value < 0 || value > 255) {
                qdf_print("Invalid value! PPDU duration target should be between 0 and 255 \n");
                retval = -EINVAL;
            }
            else {
        ...
        case OL_ATH_PARAM_ESP_BA_WINDOW:
            if (value <= 0 || value > 7){
                qdf_print("Invalid value! BA window size should be between 1 and 7 \n");
                retval = -EINVAL;
            }
            else {
         ...
         case OL_ATH_PARAM_DFS_HOST_WAIT_TIMEOUT:
             ieee80211_dfs_override_host_wait_timeout(ic, value);
             break;
        ...
        case OL_ATH_PARAM_SMARTLOG_P1PINGFAIL:
            if ((value != 0) && (value != 1)) {
                qdf_print("Invalid value. Value for P1 ping failure smart "
                ...
                if (!ic->smart_logging_p1pingfail_started) {
                    if (!ic->smart_logging) {
                        qdf_print("Smart logging feature not enabled. Cannot "
                                  "start P1 ping failure smart logging.\n");
                        retval = -EINVAL;
                    } else {
                    ...
                    qdf_print("P1 ping failure smart logging already started. "
                              "Ignoring.\n");
                }
            } else {
                 if (ic->smart_logging_p1pingfail_started) {
                    if ((send_fatal_cmd(scn,
                          ...
                          QDF_STATUS_SUCCESS) {
                        qdf_print("Error: Could not successfully send command "
                                  "to stop P1 ping failure logging. Smart "
                                  "logging or rest of system may no longer "
                                  "function as expected. Investigate!!.\n"
                                  "Marking P1 ping failure as stopped within "
                    ...
                    qdf_print("P1 ping failure smart logging already stopped. "
                              "Ignoring.\n");
                }
            }
            break;
        ...
        case OL_ATH_PARAM_TXACKTIMEOUT:
            {
                   ...
                   (scn->target_type == TARGET_TYPE_QCA9888) || (scn->target_type == TARGET_TYPE_IPQ4019))
                {
                    ...
                    {
                        (void)ol_ath_pdev_set_param(scn,
                                wmi_pdev_param_tx_ack_timeout, value, 0);
                        scn->tx_ack_timeout = value;
                    }
                    else
                    {
                        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "TX ACK time out value need between 0x40 and 0xFF\n");
                        retval = -1;
                    }
                }
                else
                {
                    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Command is not support for this FW target=0%x\n",scn->target_revision);
                    retval = -1;
                }
            }
            break;

        case OL_ATH_PARAM_BEST_UL_HYST:
            if (value > 0 && value <= 100)
                ic->ic_bestUL_hyst = value;
            break;

        case OL_ATH_PARAM_RX_MITIGATION:
            retval = ol_ath_pdev_set_param(scn,
                                   wmi_pdev_param_rx_mitigation, value, 0);
            if (!retval) {
                scn->rx_mitigation_param = value;
            }
            break;
        ...
        case OL_ATH_PARAM_NF_THRESH:
            ...
            {
                ic->acs_noisefloor_threshold = value;
            }
            else {
        ...
        case OL_ATH_PARAM_SET_WLANSTATS:
            if (value >= 0 && value < 2)
                ic->ic_wlanstats_enabled = value;
            else
                qdf_print("Invalid values: Valid range 0 to 1\n");
            break;
#endif
        default:
            return (-1);
        ...
        *(int *)buff =ic->atf_obss_scale;
        ...
        case OL_ATH_PARAM_PRECAC_ENABLE:
            *(int *)buff = ic->ic_precac_enable;
            break;
        case OL_ATH_PARAM_PRECAC_INTER_CHANNEL:
            *(int *)buff = ic->ic_precac_inter_chan;
            break;
        case OL_ATH_PARAM_PRECAC_CHAN_STATE:
            retval = ieee80211_get_precac_chan_state(ic,  *((int *)buff + 1));
            if (retval < 0) {
                retval = -EINVAL;
            } else {
                *(int *)buff = retval;
        ...
        case OL_ATH_PARAM_TXCHAINSOFT:
            *(int *)buff = scn->soft_chain;
            break;
        case OL_ATH_PARAM_DYN_AUTO_BURST:
            *(int *)buff = scn->dyn_auto_burst;
            break;
        case OL_ATH_PARAM_BEST_UL_HYST:
            *(int *)buff = ic->ic_bestUL_hyst;
            break;
        case OL_ATH_PARAM_ACS_RANK:
            if (ic->ic_acs){
        ...
        case OL_ATH_PARAM_TXACKTIMEOUT:
            ...
            {
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Command is not support for this FW target=0x%x\n",scn->target_type);
                retval = -1;
            }
            break;

        case OL_ATH_PARAM_RX_MITIGATION:
            *(int *)buff = scn->rx_mitigation_param;
            break;
        case OL_ATH_PARAM_CBS:
            if (ic->ic_cbs) {
        ...
        case OL_ATH_PARAM_CHAN_AP_RX_UTIL:
            *(int *)buff = ic->ic_ch_util.ap_rx;
            break;

        case OL_ATH_PARAM_CHAN_FREE:
            *(int *)buff = ic->ic_ch_util.free_medium;
            break;

        case OL_ATH_PARAM_CHAN_AP_TX_UTIL:
            *(int *)buff = ic->ic_ch_util.ap_tx;
            break;

        case OL_ATH_PARAM_CHAN_OBSS_RX_UTIL:
            *(int *)buff = ic->ic_ch_util.obss_rx;
            break;

        case OL_ATH_PARAM_CHAN_NON_WIFI:
            *(int *)buff = ic->ic_ch_util.non_wifi;
            break;
        ...
        case OL_ATH_PARAM_PERIODIC_CFR_CAPTURE:
            if (scn->cfr_support)
                *(int *)buff = ic->ic_cfr_timer_enable;
            else
                retval = -EINVAL;
            break;
...
#endif
        case OL_ATH_PARAM_NF_THRESH:
            *(int *)buff = ic->acs_noisefloor_threshold;
            break;
#if ATH_SUPPORT_ZERO_CAC_DFS
        case OL_ATH_PARAM_DFS_HOST_WAIT_TIMEOUT:
            {
                int timeout;
                ieee80211_dfs_get_override_host_wait_timeout(ic,&timeout);
                *(int *)buff = timeout;
            }
            break;
#endif
#ifdef ATH_SUPPORT_RDKB
        case OL_ATH_PARAM_GET_WLANSTATS:
            *(int *)buff = ic->ic_wlanstats_enabled;
            break;
