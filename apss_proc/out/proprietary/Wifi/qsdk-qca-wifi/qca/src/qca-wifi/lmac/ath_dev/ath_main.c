...
unsigned int skb_reuse_enable = 0; /*skb_reuse is disabled by default*/
module_param(skb_reuse_enable, uint, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
MODULE_PARM_DESC(skb_reuse_enable, "Enable skb reuse part");
#endif
#endif
#endif /* __linux__ */
...
#if ATH_GEN_RANDOMNESS
#define ATH_RNG_ENTROPY(__x) (((__x) * 8 * 10) >> 5) /* quality: 10/32 */

#define ATH_RANDOM_INPUT_INTERVAL       100 /* 100 ms */
extern void add_hwgenerator_randomness(const char *buffer, size_t count, size_t entropy);

static void ath_random_input_words(u_int32_t *rng_buf, int bytes_read)
{
#if LINUX_VERSION_CODE <= KERNEL_VERSION(3,3,8)
   add_hwgenerator_randomness((const char *)rng_buf, bytes_read, ATH_RNG_ENTROPY(bytes_read));
#endif
}

/* random number generator thread, to add random bits to
 * kernel random number generator through ADC. start to
 * start the thread and stop to stop.
 */
...
void ath_rng_start(struct ath_softc *sc)
{
   ...
   printk("Starting random number generator thread\n");
...
void ath_rng_stop(struct ath_softc *sc)
{
    if (sc->rng_task) {
        ...
        while (sc->rng_thread_run_state) {
            OS_SLEEP_INTERRUPTIBLE(__qdf_system_msecs_to_ticks(100));
        }
    }
}
OS_EXPORT_SYMBOL(ath_rng_stop);
#else
#define ath_rng_start(ath_softc_p)
#define ath_rng_stop(ath_softc_p)
#endif
...
static inline void ath_dfs_clear_cac(struct ath_softc *sc)
{
    ...
    if (sc->sc_dfs_wait ) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Exit DFS_WAIT state %d\n", __func__, sc->sc_dfs_wait);
        sc->sc_dfs_wait = 0;
        /*
 ...
 */
static    u_int8_t ath_numchains[16] = {
    0 /* 0000 */,
    1 /* 0001 */,
    1 /* 0010 */,
    2 /* 0011 */,
...
#define WAIT_SC_INTERVAL 100
    while (atomic_read(&sc->sc_inuse_cnt)){
        ...
        if (elapsed_time > timeout) {
           QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: sc_inuse count(%d) stuck after waiting for %d msec. Investigate!!!\n",
...
ath_newassoc(ath_dev_t dev, ath_node_t node, int isnew, int isuapsd)
{
    ...
    if ((sc->sc_opmode == HAL_M_HOSTAP)
        ...
        if(sc->sc_ieee_ops->get_node_numstreams(an->an_node) > 1)
            ath_green_ap_state_mc(sc->sc_ieee, ATH_PS_EVENT_INC_MULTISTREAM_STA);
    }
#if UNIFIED_SMARTANTENNA
                ...
                HAL_BOOL outDoor, HAL_BOOL xchanMode)
{
    ...
    if (chans == NULL) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: unable to allocate channel table\n", __func__);
        return -ENOMEM;
    }
    ...
    if (!ath_hal_init_channels(ah, chans, IEEE80211_CHAN_MAX, (u_int *)&nchan,
        ...
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: unable to collect channel list from hal; "
...
ath_halchan2mode(HAL_CHANNEL *chan)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: cannot map channel to mode; freq %u flags 0x%x\n",
...
ath_lean_chan_set(struct ath_softc *sc, HAL_CHANNEL *hchan, int tx_timeout, int rx_timeout, bool retry)
{
        ...
        sc->sc_full_reset)
    {
        ...
        {
            HAL_STATUS status;
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: unable to change channel %u (%uMhz) "
                   "flags 0x%x \n",
                   __func__,
                   ath_hal_mhz2ieee(ah, hchan->channel, hchan->channel_flags),
                   hchan->channel, hchan->channel_flags);
            ...
            if (!ath_hal_reset(ah, sc->sc_opmode, &sc->sc_curchan, ht_macmode,
               ...
               "(freq %u flags 0x%x)\n", __func__, status,
               sc->sc_curchan.channel, sc->sc_curchan.channel_flags);
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: unable to reset hardware; hal status %u "
               "(freq %u flags 0x%x)\n", __func__, status,
               sc->sc_curchan.channel, sc->sc_curchan.channel_flags);
#if !ATH_RESET_SERIAL
               ...
               atomic_dec(&sc->sc_in_reset);
                ...
                if (ATH_STARTRECV(sc) != 0) {
                    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: unable to restart recv logic\n",
                           __func__);
                }
            ...
            if (ATH_STARTRECV(sc) != 0) {
                ...
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_DEBUG"STARTED RECV \n");
            }
            return -EIO;
        }
#if !ATH_RESET_SERIAL
...
ath_chan_set(struct ath_softc *sc, HAL_CHANNEL *hchan, int tx_timeout, int rx_timeout, bool retry, bool ignore_dfs)
{
        ...
        sc->sc_full_reset)
    {
        ...
        */
        ...
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s %d %d\n", __func__,
__LINE__, sc->sc_dpc_stop);
        ...
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s %d %d\n", __func__,
__LINE__, sc->sc_dpc_stop);
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Begin chan_set sequence......\n");
#endif
        ...
        if (!ath_hal_reset(ah, sc->sc_opmode, hchan,
                           ...
                           sc->sc_scanning)) {
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: unable to reset channel %u (%uMhz) "
        ...
        if (sc->sc_hasbtcoex && (!fastcc)) {
            ...
            if (!sc->sc_enhanceddmasupport) {
                ...
                if (atomic_read(&sc->sc_hold_reset))
                    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Wow, still multiple reset in process,--investigate---\n");
            ...
            (sc->sc_dfs))
        {
            ...
            if (sc->sc_curchan.priv_flags & CHANNEL_DFS) {
                ...
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: start DFS WAIT period on channel %d\n",
                ...
                if (sc->sc_dfs->sc_dfswait) {
                   /* sc->sc_dfs->sc_dfswait=0;*/   /* We should not clear sc_dfswait here,
                    ...
                    OS_CANCEL_TIMER(&sc->sc_dfs->sc_dfswaittimer);
                    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
...
OS_TIMER_FUNC(ath_txq_stuck_checker)
{
    ...
    if (reset_required) {
        ...
        sc->sc_reset_type = ATH_RESET_NOLOSS;
        /* Take a reference to all VAPs, so that the VAP is not freed within
         * the sc_internal_reset_lock , which will case a double lock.
         * Free the reference after releasing the lock below
         */
        ...
        TAILQ_FOREACH(tmp_vap, &ic->ic_vaps, iv_next) {
            ...
            bss_node[vaps_count++] = ieee80211_ref_bss_node(tmp_vap);
        }
        IEEE80211_COMM_UNLOCK(ic);

        ath_internal_reset(sc);
        /* Free the reference outside the sc_internal_reset_lock taken above */
...
ath_sc_chk_intr(struct ath_softc *sc, HAL_ANISTATS *ani_stats)
{
    ...
    if( !dcs_params->intr_cnt ){
        ...
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Interference Detected, Changing Channel\n");
#if ATH_SUPPORT_SPECTRAL
        if (sc->sc_icm_active) {
...
ath_calibrate(void *arg)
{
    ...
    //if (ath_hal_get_device_info(ah, HAL_MAC_VERSION) == AR_SREV_VERSION_JET)
    //    sc->sc_dcs_enabled = 0;
        ...
        if (longcal || shortcal || multical) {
            ...
            if (sc->sc_curchan.channel_flags & CHANNEL_CW_INT) {
                ...
                if( sc->sc_dcs_enabled & ATH_CAP_DCS_CWIM) {
                    ...
                    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Too much interference, Trying to change channels\n");
...
#ifdef PRINT_CHAN_CHANGE
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "====================================================\n");
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " Chan %u(%08x) - %u(%08x) \n", sc->sc_curchan.channel,
                                            sc->sc_curchan.channel_flags,
                                            hchan->channel,
                                            hchan->channel_flags);
    ts1 = read_c0_count();
...
#ifdef PRINT_CHAN_CHANGE
    ts2 = read_c0_count();
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "dur: %lu\n", ts2 - ts1);
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "====================================================\n");
#endif
}
#if ATH_SUPPORT_WIFIPOS
    ...
    if (avp->av_opmode == HAL_M_HOSTAP || avp->av_opmode == HAL_M_IBSS) {
        ...
        if (!sc->sc_removed)
        {
            ...
            } else
            {
                ...
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:%d  Unable to stop CABQ. Now Stopping all Queues \n", __func__, __LINE__);
    ...
    if (!sc->sc_invalid) {
        if (ah != NULL) {
            if (ah->ah_set_interrupts != NULL)
                ...
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d] : ah_set_interrupts was NULL\n", __func__, __LINE__);

        }
        else {
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d] : ah was NULL\n", __func__, __LINE__);
        }
    }
...
ath_open(ath_dev_t dev, HAL_CHANNEL *initial_chan)
{
    ...
    if (ATH_STARTRECV(sc) != 0) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: unable to start recv logic\n", __func__);
        error = -EIO;
        goto done;
    }
...
__LINE__, sc->sc_dpc_stop);
    ...
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s %d %d\n", __func__,
__LINE__, sc->sc_dpc_stop);
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Begin reset sequence......\n");
#endif
...
#define MAX_CONSECUTIVE_GTT_COUNT 5
        ...
        if (!(status & HAL_INT_RXHP)) {
            ...
            ath_gen_randomness(sc, current_rssi);
        }
#endif
...
ath_get_caps(ath_dev_t dev, ATH_CAPABILITY_TYPE type)
{
    ...
    case ATH_CAP_TS:
        ...
        break;
    case ATH_CAP_QS:
        supported = ath_hal_getcapability(ah, HAL_CAP_QS, 0, NULL) == HAL_OK ? TRUE : FALSE;
        break;
    case ATH_CAP_NUMTXCHAINS:
        {
            ...
            {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d] tx chainmask mismatch actual %d sc_chainmak %d\n", __func__, __LINE__,
    ...
    case ATH_CAP_NUMRXCHAINS:
        {
            ...
            {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d] rx chainmask mismatch actual %d sc_chainmak %d\n", __func__, __LINE__,
...
ath_getiqueconfig(ath_dev_t dev)
{
    ...
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "AC\tPER\tProbe Interval\n\n");
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "BE&BK\t%d\t\t%d \n",
           sc->sc_rc_params[0].per_threshold,
           sc->sc_rc_params[0].probe_interval);
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "VI&VO\t%d\t\t%d \n",
           sc->sc_rc_params[1].per_threshold,
           sc->sc_rc_params[1].probe_interval);

    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "\n========= Access Category Config===========\n");
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "AC\tRTS \tAggr Scaling\tMin Rate(Kbps)\tHBR \tPER LO \tRXTIMEOUT\n\n");
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "BE\t%s\t\t%d\t%6d\t\t%s\t%d\t%d\n",
           ...
           sc->sc_rxtimeout[0]);
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "BK\t%s\t\t%d\t%6d\t\t%s\t%d\t%d\n",
           ...
           sc->sc_rxtimeout[1]);
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "VI\t%s\t\t%d\t%6d\t\t%s\t%d\t%d\n",
           ...
           sc->sc_rxtimeout[2]);
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "VO\t%s\t\t%d\t%6d\t\t%s\t%d\t%d\n",
                                ...
                                u_int32_t actual_duration, u_int32_t est_duration)
{
    ...
    return sc->sc_ieee_ops->atf_adjust_tokens((ieee80211_node_t)((an)->an_node), ac,
...
u_int8_t ath_atf_account_tokens( ath_atx_tid_t *tid, u_int32_t pkt_duration)
{
    ...
    return sc->sc_ieee_ops->atf_account_tokens((ieee80211_node_t)((an)->an_node), ac, pkt_duration);
}

static void ath_atf_node_unblock( ath_node_t node, struct atf_stats *stats)
{
    struct ath_node *an = ATH_NODE(node);
    ath_atf_sc_node_unblock(an, stats);
}
...
static int32_t ath_atf_debug_nodestate(ath_node_t node, struct atf_nodestate *atfnodestate)
{
    struct ath_node *an = ATH_NODE(node);
    return ath_atf_debug_node_state(an, atfnodestate);
}
    ...
    if (val) {
        for(i = 0; i < sc->sc_nvaps; i++) {
            ...
            if(sc->sc_vaps[i]->av_opmode != HAL_M_HOSTAP) {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s Cannot enable Green power save when not in HOSTAP mode\n", __func__);
                return;
            }
        }
    }
...
#define IEEE80211_GET_COUNTER_VALUE 3
    switch(cmd)
    {
        ...
        case IEEE80211_GET_COUNTER_VALUE:
          ...
          break;
        default:
              QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "UNKNOWN param %s %d \n",__func__,__LINE__);
          break;
    }
 ...
 */
static bool
ath_update_bcn_rate(ath_dev_t dev, u_int8_t if_id, int new_rate)
{
    struct ath_softc *sc = ATH_DEV_TO_SC(dev);
    struct ath_vap *avp;
...
/*
 * This will set the av_ctsrts_ratecode to the user specified ratecode
 * which will be passed while setting the rts / cts rate during
 * disabling the selected legacy rate.
 */
static void
ath_update_cts_rate(ath_dev_t dev, u_int8_t if_id, int ctsrate, u_int8_t dis_legacy_flag)
{
    struct ath_softc *sc = ATH_DEV_TO_SC(dev);
    struct ath_vap *avp;
    ...
    if (dis_legacy_flag) {
        ...
        for (count = 0; count < rt->rateCount; count++) {
            if (rt->info[count].rateKbps == ctsrate) {
                avp->av_ctsrts_ratecode = rt->info[count].rate_code;
                break;
            }
        }
    } else {
        avp->av_ctsrts_ratecode = 0;
    }
...
static const struct ath_ops ath_ar_ops = {
    ...
    ath_update_bcn_rate,        /* modify_bcn_rate */
    ath_update_cts_rate,        /* modify_cts_rate */
...
int init_sc_params(struct ath_softc *sc,struct ath_hal *ah)
{
    ...
    if (error != 0) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,  "%s[%d]: Failed to get channel information! error[%d]\n", __func__, __LINE__, error );
        return -1;
    }
...
#define    N(a)    (sizeof(a)/sizeof(a[0]))
    ...
    if (sc == NULL) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Unable to allocate device object.\n", __func__);
        error = -ENOMEM;
        goto bad;
    }
    ...
    *ops = &sc->sc_ath_ops;
    ...
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,
            "%s: Init of internal reset lock done \n",__func__);
#endif
    ...
    if (ah == NULL) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: unable to attach hardware; HAL status %u\n",
               __func__, status);
        error = -ENXIO;
        goto bad;
    }
    ...
    if (sc->sc_keymax > ATH_KEYMAX) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Warning, using only %u entries in %u key cache\n",
    ...
    if (sc->sc_bhalq == (u_int) -1) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: unable to setup a beacon xmit queue!\n", __func__);
        error = -EIO;
        goto bad2;
    }
    ...
    if (sc->sc_cabq == NULL) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: unable to setup CAB xmit queue!\n", __func__);
        error = -EIO;
        goto bad2;
    }
   ...
   if (error) goto bad2;
    ...
    if (!ATH_SET_TX(sc)) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,  "%s[%d]: Failed to set Tx h/w queue\n", __func__, __LINE__ );
        error = -EIO;
        goto bad2;
    }
    ...
    if (sc->sc_hasWow)
    {
        ...
        if (sc->sc_wowInfo != NULL) {
            ...
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,  "%s[%d]: Allocation of WoW information failed!\n", __func__, __LINE__ );
            error = -ENOMEM;
            goto bad2;
        }
    }
    ...
    if (sc->sc_rc == NULL) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,  "%s[%d]: Rate attach failed!\n", __func__, __LINE__ );
        error = EIO;
        goto bad2;
    }
    ...
    if (tx99_attach(sc)) {
        error = EIO;
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s TX99 init failed\n", __func__);
    }
#endif
#if ATH_SUPPORT_SPECTRAL
    ...
    if(ath_hal_singleAntPwrSavePossible(ah)) {
        if ((sc->sc_green_ap = ath_green_ap_attach((struct ieee80211com*)sc->sc_ieee)) == NULL) {
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s [%d]: Green AP attach failed!\n", __func__, __LINE__ );
            error = -EIO;
            goto bad2;
        } else {
    ...
    if (sc->sc_hasgentimer) {
        sc->sc_rx_looplimit_timer = ath_gen_timer_alloc(sc,
                                                  ...
                                                  NULL,
                                                  sc);
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Register the looplimit timer\n");
    } else {
   ...
   /* scn = ath_netdev_priv(osdev->netdev);*/
    ...
    /* Thermal throttling support is enabled */
    if (ath_hal_get_device_info(ah, HAL_MAC_VERSION) == AR_SREV_VERSION_JET) {
        sc->thermal_param.tt_support = TRUE;
    }
    /*
    ...
    ath_rng_start(sc);
    ath_initialize_timer(sc->sc_osdev, &sc->sc_chan_busy, ATH_CHAN_BUSY_INTVAL,
                         ath_sc_chan_busy, sc);
    ath_set_timer_period(&sc->sc_chan_busy, ATH_CHAN_BUSY_INTVAL);
    ath_start_timer(&sc->sc_chan_busy);

    if (ath_hal_get_device_info(sc->sc_ah, HAL_MAC_VERSION) == AR_SREV_VERSION_JET) {
        /*WAR to disable higher MCS rates on 3rd and 4th stream
          since, JET on QWRAP mode has issue in RX*/
#if ATH_SUPPORT_WRAP
        sc->sc_qwrap_rx_war = 1;
#endif
        sc->sc_quadstream_support = 1;
    }
...
#endif
{
    ...
    {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Invalid interface id = %u\n", __func__, if_id);
        return -EINVAL;
    }
    ...
    if (sc->sc_vaps[if_id] != NULL) {
        ...
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: WAR_DELETE_VAP sc_nvaps=%d\n", __func__, sc->sc_nvaps);
    }
#else
...
ath_node_set_fixedrate(ath_node_t node, u_int8_t fixed_ratecode)
{
    ...
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: an_fixedrate_enable = %d, an_fixedrix = %d, an_fixedratecode = 0x%x\n",
...
void
ath_set_extap_state(ath_dev_t dev, int val)
{
    struct ath_softc *sc = ATH_DEV_TO_SC(dev);
    if (val)
        sc->sc_extapEnabled = 1;
    else
        sc->sc_extapEnabled = 0;
    return;
}
...
ath_node_detach(ath_dev_t dev, ath_node_t node)
{
    ...
    if((sc->sc_opmode == HAL_M_HOSTAP) && (an->is_sta_node)) {
        ...
        if (sc->sc_ieee_ops->get_node_numstreams(an->an_node) > 1)
            ath_green_ap_state_mc(sc->sc_ieee, ATH_PS_EVENT_DEC_MULTISTREAM_STA);
    }
...
#ifdef  ATH_SUPPORT_TxBF
        ath_tx_complete_buf(sc, bf, &bf_head, NULL, 0, 0, 0);
#else
        ath_tx_complete_buf(sc, bf, &bf_head, NULL, 0);
#endif
...
ath_node_cleanup(ath_dev_t dev, ath_node_t node)
{
    ...
    if (atomic_read(&an->an_active_tx_cnt)) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "skipping lmac node %pK cleanup due to tx activity\n", an);
        return 1;
    }
...
ath_set_regdomain(ath_dev_t dev, int regdomain, HAL_BOOL wrtEeprom)
{
    ...
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " %s: ret %d, regdomain %d \n", __func__, ret, regdomain);
...
static int ath_get_rx_signal_dbm(ath_dev_t dev, int8_t *signal_dbm)
{
    ...
    HAL_CHANNEL hchan;
    HAL_CHANNEL *ext_ch;
    int8_t nf_pwr[8], nf_cal[8], rssi[8];
    int16_t nf_run[8];
    int i;
    ...
    rssi[3] = sc->sc_stats.ast_rx_rssi_ctl3;
    rssi[4] = sc->sc_stats.ast_rx_rssi_ext0;
    rssi[5] = sc->sc_stats.ast_rx_rssi_ext1;
    rssi[6] = sc->sc_stats.ast_rx_rssi_ext2;
    rssi[7] = sc->sc_stats.ast_rx_rssi_ext3;
    ...
    if (ext_ch) {
        ...
        ath_get_rx_nf_offset(dev, hchan.channel, hchan.channel_flags, nf_pwr, nf_cal);
        for (i=4; i<8; i++) {
            if (ah->ah_max_chainmask & (1 << (i % 4))){
                if (rssi[i] != ATH_RSSI_BAD) {
                    signal_dbm[i] = rssi[i] + nf_run[i] + nf_pwr[i-4] - nf_cal[i-4];
                } else {
                    signal_dbm[i] = ATH_RSSI_BAD;
                }
            }
...
OS_TIMER_FUNC(ath_radar_task)
{
        ...
        if (dfs_process_radarevent(sc,&hchan)) {
                ...
                if (ext_ch) {
                    struct ieee80211_ath_channel iextchan;
        ...
        if ((sc->sc_dfs->dfs_rinfo.rn_use_nol != 1) &&
                ...
                /* The actual channel change will be done in ieee80211_mark_dfs,
                   ...
                   here */
                       QDF_PRINT_INFO(AH_PRIVATE(ah)->ah_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Radar found on channel %d (%d MHz)\n",
...
OS_TIMER_FUNC(ath_check_dfs_clear)
{
        ...
        if ((hchan.priv_flags & CHANNEL_DFS_CLEAR) ||
                ...
                sc->sc_dfs->sc_dfswait=0;
                ath_hal_dfs_cac_war (sc->sc_ah, 0);
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "End of DFS wait period\n");
                for (j = i = 0; j < sc->sc_nvaps; j++, i++) {
                        ...
                        */
                        ...
                        if(avp->av_dfswait_run) {
                            ...
                            if(error) {
                                       QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s error allocating beacon\n", __func__);
                                       return;
                            }
...
void ath_set_rx_chainmask(ath_dev_t dev, u_int8_t mask)
{
    ...
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d] sc->sc_config.rxchainmask %d mask %d\n", __func__, __LINE__,
...
void ath_set_tx_chainmask(ath_dev_t dev, u_int8_t mask)
{
    ...
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d] sc->sc_config.txchainmask %d mask %d\n", __func__, __LINE__,
...
void ath_set_tx_chainmaskopt(ath_dev_t dev, u_int8_t mask)
{
    ...
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Primary txchainmask 0x%02x, optional txchainmask 0x%02x\n",
...
/*
 * Function     : ath_spectral_is_spectral_supported
 * Description  : Check if SPECTRAL is supported
 * Input        : Pointer to spectral
 * Ouput        : Enabled/Disabled
 *
 */
static u_int32_t ath_spectral_is_spectral_supported(void* arg)
{
       u_int32_t is_enabled = AH_FALSE;
       struct ath_softc* sc = GET_SPECTRAL_ATHSOFTC(arg);
       struct ath_hal* ah = sc->sc_ah;
       is_enabled = ath_hal_is_spectral_supported(ah);
       return is_enabled;
}
...
int ath_attach_spectral(struct ath_softc* sc, struct ath_reg_parm* ath_conf_parm)
{
    ...
    if ((sc->sc_spectral = (struct ath_spectral*)spectral_attach((struct ieee80211com*)sc->sc_ieee)) == NULL) {
        err = EIO;
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s SPECTRAL attach failed, HW may not be spectral capable\n", __func__);
    }else {
        /*
        ...
        if (spectral_check_hw_capability((struct ieee80211com*)sc->sc_ieee) == AH_FALSE) {
            ...
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "SPECTRAL :----- module attached\n");
        }
...
void ath_dev_stop_spectral_scan(ath_dev_t dev)
{
    ...
    if (state != ATH_PWRSAVE_AWAKE) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Warning - Not in Awake state. Explicitly waking up\n",
                __func__);
        ATH_PS_WAKEUP(sc);
    }
...
u_int32_t ath_init_spectral_ops(struct ath_spectral_ops* p_sops)
{
    ...
    p_sops->is_spectral_supported   = ath_spectral_is_spectral_supported;
...
u_int32_t ath_green_ap_ps_on_off(void* arg, u_int32_t val)
{
    ...
    if ((((green_ap->power_save_mode == GREEN_AP_MODE_NO_STA) && !green_ap->num_nodes)
            || ((green_ap->power_save_mode == GREEN_AP_MODE_NUM_STREAM) && !green_ap->num_nodes_multistream))
            && (val)) {
...
u_int32_t ath_get_noderate(ath_node_t node, u_int8_t type)
{
    ...
    } else if (type == IEEE80211_RATEFLAGS_RX) {
        return an->an_rxrateflags;
    } else {
        return 0;
    }
}
...
#if ATH_GEN_RANDOMNESS
void ath_gen_randomness(struct ath_softc *sc, u_int32_t random_value)
{
    sc->sc_random_value |= (random_value & 0x1) <<
                   sc->sc_random_pos;
...
#define   HAL_CAP_STRONG_DIV 2
    ...
    if (ath_hal_getcapability(sc->sc_ah, HAL_CAP_DIVERSITY, HAL_CAP_STRONG_DIV,
           ...
           QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO,  "%s[%d] SHOULD ABORT \n", __func__, __LINE__);
    }
    } else {
    ...
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s[%d] dfsdomain %d\n", __func__, __LINE__, sc->sc_dfsdomain);
...
static int ath_smart_ant_enable(ath_dev_t dev, int enable, int mode)
{
    ...
    if (retval == -1)
    {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Smart antenna is not supported\n");
        return 0;
    }
...
/*
 * decide number of msec to delay based on the number of times
 * adc read fails
 */

static inline u_int32_t
ath_rng_delay_get(u_int32_t fails)
{
    return (fails > 105) ? 10000 : ((fails > 100) ? 1000 : 10);
}

/*
 * kthread to read and update random value
 * Mode 1 : Keep obtaining the RSSI, and check if that can be
 *          added to random pool, and sleep for 50ms.
 * Mode 2 : Random is updated by rssi every 50 ms. and if we
 *          read same rssi for 5 times we will read from ADC
 *          regs and sleep at least 100ms.
 * Mode 3 : Always use ADC and always make sure, that kernel calls this,
...
static int ath_rng_kthread(void *data)
{
    ...
    struct ath_hal *ah = sc->sc_ah;
    u_int32_t *rng_buf = NULL;
    u_int32_t last_rssi = 0;
    u_int32_t current_rssi = 0;
    u_int32_t rssi_const_count  = 0;
    u_int32_t mode = 0;
    u_int32_t last_mode = 0;
    u_int32_t adc_rd_fails = 0;
    unsigned long thread_start_timestamp = OS_GET_TIMESTAMP();
    struct ieee80211com *ic = (struct ieee80211com *)sc->sc_ieee;

    sc->sc_stats.ast_rssi_sync_count_m1 = 0;
    sc->sc_stats.ast_rssi_sync_count_m2 = 0;
    sc->sc_stats.ast_adc_sync_count_m2 = 0;
    sc->sc_stats.ast_adc_sync_count_m3 = 0;

    sc->sc_stats.last_rssi_sync_m1_m2 = 0;
    sc->sc_stats.last_adc_sync_m2 = 0;
    sc->sc_stats.last_adc_sync_m3 = 0;

    /*
     * if this IC is for offload radio, do not do anything.
     */
    ...
    rng_buf= ( u_int32_t *) OS_MALLOC(sc->sc_osdev,
                                      ATH_RNG_BUF_SIZE*(sizeof(uint32_t)),
                                      GFP_KERNEL);
    if (!rng_buf) {
        goto out;
    }
    OS_MEMSET(rng_buf, 0, ATH_RNG_BUF_SIZE * sizeof(rng_buf[0]));
    while (!kthread_should_stop()) {
        sc->rng_thread_run_state = 1;
        mode = ic->ic_random_gen_mode;
        /* ideally there is no requirement to track the mode changes,
         * every mode should work independently
         */
        if (last_mode != mode) {
            last_mode = mode;
        }
        /* do sanity on current state of the device, and the driver
         * before starting the random.
         */
        if (!sc->sc_nvaps || sc->sc_invalid || sc->sc_scanning ||
            atomic_read(&sc->sc_in_reset)) {
                OS_SLEEP_INTERRUPTIBLE(__qdf_system_msecs_to_ticks(100));
            continue;
        } /* mode */
        /*
         * Mode 1 : Update random seed every 100 ms. Sleep extra 20 ms
         *          for every cycle we read constant rssi. Force adding
         *          the RSSI to random pool even if RSSI is constant for
         *          ATH_RSSI_CONST_COUNT_LIMIT to avoid empty entropy.
         * Mode 2 : Random is updated by rssi every 100 ms. and if we
         *          read same rssi for 5 times we will read from ADC
         *          regs and sleep at least 100ms.
         */
        ...
        if ((1 == mode) || (2 == mode)) {
            current_rssi = sc->sc_ath_ops.ath_get_rssi_chain0(sc);
            sc->sc_stats.last_rssi_sync_m1_m2 =
                CONVERT_SYSTEM_TIME_TO_MS(OS_GET_TIMESTAMP() - thread_start_timestamp);
            if (current_rssi == last_rssi) {
                 rssi_const_count ++;
                 if ((1 == mode) && (rssi_const_count <= ATH_RSSI_CONST_COUNT_LIMIT)) {
                     OS_SLEEP_INTERRUPTIBLE(__qdf_system_msecs_to_ticks(20));
                     continue;
                 }
            } else {
                rssi_const_count = 0;
            ...
            /* check below for other case of mode 2 */
            if (1 == mode) {
               int i = 0;
               ath_gen_randomness(sc, current_rssi);
               for (i = 0; i < 5 && !kthread_should_stop(); i++)
                   OS_SLEEP_INTERRUPTIBLE(__qdf_system_msecs_to_ticks(20));
               sc->sc_stats.ast_rssi_sync_count_m1++;
               rssi_const_count = 0;
               continue;
            }
        } /* mode 1 */
        ...
        /* Mode 2
         * It is assumed, in some cases, RSSI could go constant, based
         * on operating environment and stations fixed orientatin and the
         * data rate. If that goes constant, random seeding might be
         * wrong. Look if we can use the adc for this.
         */

        if (2 == mode) {
            if (rssi_const_count > ATH_RSSI_CONST_COUNT_LIMIT) {
                /* do not wake and sleep, instead sleep and awake */
                 ATH_PS_WAKEUP(sc);

                 bytes_read = ath_hal_adc_data_read(ah, rng_buf,
                                          ATH_RNG_BUF_SIZE,&sc->rng_last);
                 sc->sc_stats.last_adc_sync_m2 =
                    CONVERT_SYSTEM_TIME_TO_MS(OS_GET_TIMESTAMP() - thread_start_timestamp);
                 sc->sc_stats.ast_last_rng_m2 = sc->rng_last;

                 ATH_PS_SLEEP(sc);
                if (unlikely(!bytes_read)) {
                    u_int32_t delay;
                    delay = ath_rng_delay_get(++adc_rd_fails);
                    OS_SLEEP_INTERRUPTIBLE(delay);
                    /* See mode3 for rationalle. It makes sense to blurp if ADC is
                     * failing too much in mode1 and mode2 too.
                     */
                    if (adc_rd_fails > 512){
                        ic->ic_random_gen_mode = 1;
                        adc_rd_fails = 0;
                        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_WARN, "%s:%d *** BAD ADC READ *** mode=1\n", __func__, __LINE__);
                    }
                    continue;
                } else adc_rd_fails = 0; /* reset fails */

                if (bytes_read > ATH_RNG_BUF_SIZE * 4)
                    bytes_read = ATH_RNG_BUF_SIZE * 4;

                /* let kernel choose when random seed is required two ways,
                 *
                 * 1. Fill the random input words, and wait for next call
                 * 2. have the data, but feed immediate after the call
                 * Both approaches looks same.
                 */

                ath_random_input_words(rng_buf, bytes_read);
                {
                    int i = 0;
                    for (i = 0; i < 5 && !kthread_should_stop(); i++)
                           OS_SLEEP_INTERRUPTIBLE(__qdf_system_msecs_to_ticks(20));
                ...
                OS_MEMSET(rng_buf, 0, ATH_RNG_BUF_SIZE * sizeof(rng_buf[0]));
                sc->sc_stats.ast_adc_sync_count_m2++;
                rssi_const_count  = 0;
                continue;
            } else {
                int i = 0;
                ...
                for (i = 0; i < 5 && !kthread_should_stop(); i++)
                    OS_SLEEP_INTERRUPTIBLE(__qdf_system_msecs_to_ticks(20));
                sc->sc_stats.ast_rssi_sync_count_m2++;
                rssi_const_count = 0;
                continue;
            }
        } /* mode 2 */
        ...
        /* Mode 3:
         * Make sure random seed update happens only through ADC
         * and only through kernel request mode. As commented
         * above, there are two ways, either get the data have
         * it ready, update when kernel calls
         *
         * OR
         *
         * Wait for kernel and then read the data. Note that there is
         * no interruptible sleep at end of this.
         */
        if (3 == mode) {
           /* make sure, device is awake and matain the same sleep
            * state after the read of ADC.
            */
           ATH_PS_WAKEUP(sc);
           bytes_read = ath_hal_adc_data_read(ah,
                                              rng_buf, ATH_RNG_BUF_SIZE,
                                              &sc->rng_last);
           sc->sc_stats.last_adc_sync_m3 =
               CONVERT_SYSTEM_TIME_TO_MS(OS_GET_TIMESTAMP() - thread_start_timestamp);
           sc->sc_stats.ast_last_rng_m3 = sc->rng_last;
           ...
           if (unlikely(!bytes_read)) {
               ...
               delay = ath_rng_delay_get(++adc_rd_fails);
               OS_SLEEP_INTERRUPTIBLE(delay);
               /* Seems some boards are exhibiting adc read  failures. if we are not
                * able to read the data for 512 cycles ( 2048 / 4 = 512, assuming
                * one such adc read gives us 4 bytes of data, better disable this
                * feature on this board, as this might severly impact the basic
                * functionality of AP. If we see a failed board, switch to rssi based
                * method.
                */
               if (adc_rd_fails > 512){
                   ic->ic_random_gen_mode = 1;
            ...
            ath_random_input_words(rng_buf, bytes_read);
            OS_MEMSET(rng_buf, 0, ATH_RNG_BUF_SIZE * sizeof(rng_buf[0]));
            sc->sc_stats.ast_adc_sync_count_m3++;
        } /* mode 3*/
    } /* while kthread */
    sc->rng_thread_run_state = 0;
    OS_FREE(rng_buf);
out:
    sc->rng_task = NULL;
    return 0;
