...
#if ATH_SUPPORT_NR_SYNC
    if (vap->iv_ic->ic_nr_share_radio_flag != 0xff) {
        ...
        if (cb_info.ssid[0] == 0) {
            /* Nbr request with SSID=0 is valid entry (wild card), add all nodes
               (irrespective of ssid) by default design. Change the design to use
               current ess value as filter here as customer requested for FR36672 */
            /* Added share_nr_flag to retain the previous functionality */
        ...
        for (i = 0; i < MAX_RADIO_CNT; i++) {
            ...
            if (tmp_ic) {
                ...
                if (tmp_ic != vap->iv_ic) {
                    if (!(share_nr_flag && (tmp_ic->ic_nr_share_radio_flag & (1 << i))))
                        continue;
                }
                IEEE80211_DPRINTF(vap, IEEE80211_MSG_RRM, "Go through the scan table for IC %d: channel=%d\n",
                        i, tmp_ic->ic_curchan->ic_ieee);
#if ATH_SUPPORT_MULTIPLE_SCANS
                /* ATH_SUPPORT_MULTIPLE_SCANS is disabled by default. Compiling failed if enable it.
                   So this path is not tested yet. */
                TAILQ_FOREACH(tmpvap, &tmp_ic->ic_vaps, iv_next) {
                    wlan_scan_table_iterate(tmpvap, ieee80211_fill_nrinfo, &cb_info);
                }
#else
                ieee80211_scan_table_iterate(tmp_ic->ic_scan_table, ieee80211_fill_nrinfo, &cb_info);
#endif /* ATH_SUPPORT_MULTIPLE_SCANS */
            }
        }
    } else { /* ic_nr_share_radio_flag == 0xff */
        wlan_scan_table_iterate(vap, ieee80211_fill_nrinfo, &cb_info);
    }
#else /* ATH_SUPPORT_NR_SYNC */
    wlan_scan_table_iterate(vap, ieee80211_fill_nrinfo, &cb_info);
#endif /* ATH_SUPPORT_NR_SYNC */
        ...
        u_int8_t *sfrm,u_int32_t frm_len)
{
    ...
    if (bcnrpt_count) {
        ieee80211_bsteering_send_rrm_bcnrpt_event(
            vap, IEEE80211_ACTION_RM_TOKEN, vap->rrm->rrm_macaddr,
            reports, bcnrpt_count);
    }
    ieee80211_bsteering_dealloc_rrm_bcnrpt(&reports);

    if (total_bcnrpt_count) {
       ieee80211_bsteering_send_rrm_frame_bcnrpt_event(
            vap, IEEE80211_ACTION_RM_TOKEN, vap->rrm->rrm_macaddr,
