/*
* Copyright (c) 2013, 2016, 2019 Qualcomm Innovation Center, Inc.
* All Rights Reserved.
...
#if ACFG_NETLINK_TX
        acfg_offchan_cancel(&scn->sc_ic);
#endif  /* end of UMAC_SUPPORT_ACFG */
#endif
...
#endif
extern void alloc_htc_credit_history_buf(void);
extern void dealloc_htc_credit_history_buf(void);
static int __init
init_ath_pci(void)
{
    ...
    /* allocate the credit history buffer */
    alloc_htc_credit_history_buf();
#ifdef ATH_AHB
    ahbret = init_ath_ahb();
    if(ahbret < 0 ) {
...
exit_ath_pci(void)
{
    ...
    /* Deallocate the credit history buffer */
    dealloc_htc_credit_history_buf();
