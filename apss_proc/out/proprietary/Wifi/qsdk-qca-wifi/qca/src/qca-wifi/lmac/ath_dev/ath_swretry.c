...
dumpTxQueue(struct ath_softc *sc, ath_bufhead *bfhead) 
{
    ...
    if (!bf)
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "EMPTY QUEUE\n");
    ...
    while (bf) {
       ...
       QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Buffer #%pK --> Next#%pK Prev#%pK Last#%pK\n",bf, TAILQ_NEXT(bf,bf_list), 
                TAILQ_PREV(bf, ath_bufhead_s, bf_list), bf->bf_lastfrm);
       QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "                     Stas#%08X flag#%08X Node#%pK\n", bf->bf_status, bf->bf_flags, bf->bf_node);

       QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Descr  #%pK --> Next#%08X Data#%08X Ctl0#%08X Ctl1#%08X\n", (uintptr_t) bf->bf_daddr, ds->ds_link, ds->ds_data, ds->ds_ctl0, ds->ds_ctl1);
       QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "                     Ctl2#%08X Ctl3#%08X Sta0#%08X Sta1#%08X\n",ds->ds_hw[0], ds->ds_hw[1], lastds->ds_hw[2], lastds->ds_hw[3]);
       }       
       bf = TAILQ_NEXT(bf,bf_list);
    }                           
...
#ifdef ATH_SUPPORT_TxBF
    if (sc->sc_enhanceddmasupport)
        ath_tx_complete_buf(sc, bf, bf_head, &ts, 0, ts.ts_txbfstatus, 0);
    else
        ath_tx_complete_buf(sc, bf, bf_head, &ts, 0, 0, 0);
#else
    ath_tx_complete_buf(sc, bf, bf_head, &ts, 0);
#endif
