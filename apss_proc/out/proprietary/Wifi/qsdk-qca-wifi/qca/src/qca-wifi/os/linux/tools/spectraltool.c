...
static int
spectralIsSupported(struct spectralhandler *spectral)
{
    struct ifreq ifr;
    u_int32_t status =  0;

    spectral->atd.ad_id = SPECTRAL_IS_SUPPORTED | ATH_DIAG_DYN;
    spectral->atd.ad_out_data = (void *)&status;
    spectral->atd.ad_out_size = sizeof(u_int32_t);
    strlcpy(ifr.ifr_name, spectral->atd.ad_name, sizeof(ifr.ifr_name));
    ifr.ifr_data = (caddr_t)&spectral->atd;
...
#define	streq(a,b)	(strcasecmp(a,b) == 0)
    ...
    if (spectralIsSupported(&spectral) != true) {
        printf("Spectral is not supported\n");
        exit(-1);
    }
