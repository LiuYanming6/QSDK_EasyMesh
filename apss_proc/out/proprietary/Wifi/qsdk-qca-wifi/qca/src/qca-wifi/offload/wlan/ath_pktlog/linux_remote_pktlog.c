/*
* Copyright (c) 2014, 2015, 2018 Qualcomm Innovation Center, Inc.
* All Rights Reserved.
...
int stop_service(ol_scn_t scn)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Missed Count: %d Fends: %d \n", service->missed_records, service->fend_counts);
    return 0;
}
            ...
            if((pl_info->rlog_write_index - pl_info->rlog_read_index) >= MAX_SEND_SIZE) { 
               ...
               pl_info->rlog_read_index += MAX_SEND_SIZE;
                ...
                if (service->running) {
                    ...
                    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Service Stopped: sending last %d bytes \n", (pl_info->rlog_write_index - pl_info->rlog_read_index));
                }
            }
        } else {
...
void pktlog_start_accept_service(void *context)
{
    ...
    if(error < 0) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s: send socket create error \n", __func__);
        return;
    }
    ...
    if (error < 0) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Could not set sockopt: %s \n", __func__);
    }

    error = service->send_socket->ops->setsockopt(service->send_socket, IPPROTO_IP, IP_TOS, &val, sizeof(val));
    if (error < 0) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Could not set sockopt 2: %s \n", __func__);
    }
#endif
    ...
    while (service->running) {
        ...
        if(error < 0) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "accept error,release the socket\n");
            sock_release(service->send_socket);
            return;
        }
        service->connect_done = TRUE;
        qdf_sched_work(scn->qdf_dev, &service->send_service);
        if (service->connect_done) {
            break;
        }
    }
...
void pktlog_start_remote_service(void *context)
{
    ...
    if(error < 0){ 
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s: listen socket create error \n", __func__);
        return;
    }
    ...
    if(error < 0) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s: Error in bind address \n", __func__);
        return;
    }
    
    error = socket->ops->listen(socket, MAX_LISTEN_CONECTIONS);
    if(error < 0) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "%s: Error in listen \n", __func__);
        return;
    }
    
    qdf_sched_work(scn->qdf_dev, &service->accept_service);
    return;
}
...
int pktlog_remote_enable(struct ol_ath_softc_net80211 *scn, int enable)
{
    ...
    if (enable && !service->running) {
        ...
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s , port %d \n", __func__, service->port);
        /* init buffer related counters/index */
        pl_info->rlog_max_size = pl_info->buf_size;
        pl_info->rlog_write_index = 0;
        pl_info->rlog_read_index = 0;
        pl_info->is_wrap = 0;
        qdf_sched_work(scn->qdf_dev, &service->connection_service);
    } else {
        service->running = 0;
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Stopping Service: write index: %d read index: %d \n", pl_info->rlog_write_index, pl_info->rlog_read_index);
    }
