    ...
    qdf_nbuf_t tail_msdu)
{
    ...
    if(peer->tids_rx_reorder[tid].array == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:tids_rx_reorder.array for tid %d is NULL for peer %pK,"
    ...
    unsigned seq_num_end)
{
    ...
    if(peer->tids_rx_reorder[tid].array == NULL) {
            if ((scn->dbg.err_types.seq_num_errors % scn->dbg.print_rate_limit) == 0) {
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:tids_rx_reorder.array for tid %d is NULL for peer %pK,"
                       " (%02x:%02x:%02x:%02x:%02x:%02x)\n"
                     "so skipping flush, This could be due to peer corruption \n",
                                                              __func__,tid, peer,
    ...
    do {
        ...
        if (qdf_likely(rx_reorder_array_elem->head)) {
            ...
            (scn->dbg.err_types.seq_num_errors)++;

            if ((scn->dbg.err_types.seq_num_errors % scn->dbg.print_rate_limit) == 0) {
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "=== Reducing pace for error prints ==== \n");
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Start seq_num %d End seq_num %d tid %d failed seq %d - error occured %d times\n",
                        orig_seq_num_start, seq_num_end, tid, seq_num_start, scn->dbg.err_types.seq_num_errors);
            }
        }
    ...
    u_int8_t win_sz)
{
    ...
    if (rx_reorder->array == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:Memory alloc failed,"
    ...
    u_int8_t tid)
{
    ...
    if (rx_reorder->array != &rx_reorder->base){
        ...
        qdf_print("Check BA session state.Peer:%pK,id:%d,tid:%d\n",peer,peer_id,tid);
