...
const struct pci_device_id ath_pci_id_table[] = {
#else
...
ath_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    if (pci_set_dma_mask(pdev, 0xffffffff)) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "ath_pci: 32-bit DMA not available\n");
        goto bad;
    }
    ...
    if (!request_mem_region(pci_resource_start(pdev, 0), pci_resource_len(pdev, 0), "ath")) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "ath_pci: cannot reserve PCI memory region\n");
        goto bad;
    }

    mem = (unsigned long) ioremap(pci_resource_start(pdev, 0), pci_resource_len(pdev, 0));
    if (!mem) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "ath_pci: cannot remap PCI memory region\n") ;
        goto bad1;
    }
    ...
    if (dev == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_ERR "ath_pci: no memory for device state\n");
        goto bad2;
    }
...

    {
        ...
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO "%s: %s: mem=0x%llx, irq=%d hw_base=0x%pK\n",
