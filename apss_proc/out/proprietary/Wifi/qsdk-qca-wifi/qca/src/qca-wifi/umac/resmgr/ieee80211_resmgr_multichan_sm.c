...
static void ieee80211_resmgr_multichan_sm_debug_print (void *ctx, const char *fmt,...) 
{
    ...
    va_end(ap);
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s", tmp_buf);
}
...
static bool ieee80211_resmgr_state_multichan_idle_event(void *ctx, u_int16_t event_type, u_int16_t event_data_len, void *event_data) 
{
    ...
    case IEEE80211_RESMGR_EVENT_BSSCHAN_REQUEST:
    {
        ...
        if (ieee80211_resmgr_oc_sched_get_state(resmgr) != OC_SCHEDULER_ON) {       
            ...
            if (resmgr->scandata.chan){
                ...
                ic->ic_curchan_ieee = resmgr->scandata.chan->ic_ieee;
                ic->ic_curchan_cfreq2 = resmgr->scandata.chan->ic_vhtop_ch_freq_seg2;
            }
    ...
    case IEEE80211_RESMGR_EVENT_OFFCHAN_REQUEST:
    {
        ...
        if (ieee80211_resmgr_oc_sched_get_state(resmgr) != OC_SCHEDULER_ON) {       
            ...
            ic->ic_curchan_ieee = event->chan->ic_ieee;
            ic->ic_curchan_cfreq2 = event->chan->ic_vhtop_ch_freq_seg2;
            ic->ic_scan_start(ic);
            ic->ic_set_channel(ic);
    ...
    if (ieee80211_resmgr_oc_sched_get_state(resmgr) == OC_SCHEDULER_ON) {
        ...
        if (check_scheduled_operation == true) {
            ...
            if (transition_scan == true) {
                ...
                ic->ic_curchan_ieee = resmgr->scandata.scan_chan->ic_ieee;
                ic->ic_curchan_cfreq2 = resmgr->scandata.scan_chan->ic_vhtop_ch_freq_seg2;
                ic->ic_scan_start(ic);
                ic->ic_set_channel(ic);
                    ...
                    case VAP_STARTING:
                        {
                            ...
                            if (resmgr_vap && (!params.vaps_active || !IS_CHAN_EQUAL(ic->ic_curchan, resmgr_vap->chan))) {
                                ...
                                ic->ic_curchan_ieee = resmgr_vap->chan->ic_ieee;
                                ic->ic_curchan_cfreq2 = resmgr_vap->chan->ic_vhtop_ch_freq_seg2;
...
int ieee80211_resmgr_multichan_sm_create(ieee80211_resmgr_t resmgr)
{
    ...
    if (!resmgr->hsm_handle) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : ieee80211_sm_create failed \n", __func__);
        return ENOMEM;
    }
