...
#ifdef AR6K_ALLOC_DEBUG 
struct sk_buff* __qdf_nbuf_alloc_debug(size_t size, int reserve, int align, char *func, int line)
{
    struct sk_buff *skb;
    ...
    if (!skb) {
        if (printk_ratelimit()) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ERROR:NBUF alloc failed\n");
        }
        return NULL;
    }
