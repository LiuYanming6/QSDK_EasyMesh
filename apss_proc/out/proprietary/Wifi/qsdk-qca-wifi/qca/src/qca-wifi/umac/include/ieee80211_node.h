...
/*
 * IEEE80211_IS_MYNODE includes every node in the BSS, while IEEE80211_IS_MYSTA
 * doesn't include the BSS node itself.
 */
#define IEEE80211_IS_MYNODE(_v, _n) (_n && (_n)->ni_vap == _v && (_n)->ni_associd)
#define IEEE80211_IS_MYSTA(_v, _n) (IEEE80211_IS_MYNODE(_v, _n) && _n != (_v)->iv_bss)
...
#define IEEE80211_INVAL_DEFKEY  0x7FFF
    ...
    u_int8_t                ni_disassoc_tx_xretry; /*Counter for disassoc_retry */
    u_int8_t                ni_deauth_tx_xretry; /*Counter for deauth_retry */
...
#define	IEEE80211_NODE_RATECHG                  8
    ...
    u_int32_t               ni_rssi_seq; /* last RSSI sequence number */
    ...
    u_int8_t                ni_bs_rssi;    /* recv rssi used in bsteering */
    ...
    bool                    ni_bs_steering_flag : 1; /* if the node is under steering */
    bool                    ni_bs_inact_flag : 1; /* inactivity flag */

    u_int8_t                ni_last_rssi;
    wbuf_t                  assoc_frame; // Max length
    u_int8_t                ni_bs_rssi_prev_xing_map;    /* store the prev xing used for MAP */
#endif
    u_int8_t                ni_inact_rssi_xing;
    u_int8_t                ni_low_rssi_xing;
    u_int8_t                ni_low_rate_rssi_xing;
    u_int8_t                ni_high_rate_rssi_xing;
...
#define IEEE80211_NODE_C_UAPSD  0x0004  /* U-APSD capable */
    ...
    ni_cfr_param_t          ni_cfr;         /* cfr capture params */
...
#define IEEE80211_NODE_IPV6_MAX 7
...
#endif
    u_int8_t    ni_fixed_rate;    /* STA's fixed data rate */
#if ATH_SUPPORT_HS20
    ...
    u_int8_t  ni_block_bitmap;          /* Indicates traffic blocked based on AC */
    u_int8_t  ni_atf_airtime_cap;       /* Configured max airtime for the node */
    u_int8_t  ni_atf_airtime_configured:1; /* Dynamic airtime configured for this node */
    u_int32_t ni_atf_tput;              /* Configured throughput for the node */
    u_int16_t ni_atf_airtime;           /* Airtime that will be alloted to a node */
    u_int16_t ni_atf_airtime_new;       /* Temp value to hold the airtime estimated */
    u_int16_t ni_atf_airtime_more;      /* Temp value to hold the difference of the above two */
    u_int16_t ni_atf_airtime_subseq;    /* Sum of airtimes of all nodes which have lower priority */
    u_int32_t ni_throughput;            /* Possible UDP DL BE throughput for this node, from ratectrl */
    struct subgroup_list *ni_ac_list_ptr[WME_NUM_AC];
#endif
#if ATH_SUPPORT_SPLITMAC
#define IEEE80211_SPLITMAC_NODE_INIT         0x0
#define IEEE80211_SPLITMAC_IN_ASSOC_REQ      0x1
#define IEEE80211_SPLITMAC_ASSOC_RESP_START  0x2
#define IEEE80211_SPLITMAC_ASSOC_RESP_END    0x3

    u_int8_t    splitmac_state;
#endif
    ...
    systime_t   ni_last_mdns_rx; /* Last MDNS pkt Rx time */
    systime_t   ni_rssi_measured_time;
#ifdef ATH_SUPPORT_RDKB
    u64         wlanstats_peer_cookie;
    struct wlan_rx_stats *rx_stats;
    struct wlan_tx_stats *tx_stats;
    int32_t ni_rssi_ave;
#endif
...
int atf_assign_subgroup(struct ieee80211_node *ni,
                        struct group_list *group, u_int8_t acbitmap, u_int8_t cfg_type);
#endif
...
#define	ieee80211_find_txnode_wds(ni, macaddr)                    \
    ieee80211_find_txnode_wds_debug(ni, macaddr, __func__, __LINE__, __FILE__)
...
static INLINE void
ieee80211node_set_whc_apinfo_flag(struct ieee80211_node *ni, u_int8_t flag)
{
    ni->ni_whc_apinfo_flags |= flag;
}

static INLINE void
ieee80211node_clear_whc_apinfo_flag(struct ieee80211_node *ni, u_int8_t flag)
{
    ni->ni_whc_apinfo_flags &= ~flag;
}

static INLINE int
ieee80211node_has_whc_apinfo_flag(struct ieee80211_node *ni, u_int8_t flag)
{
    return ((ni->ni_whc_apinfo_flags & flag) != 0);
}

static INLINE void
ieee80211node_set_whc_rept_info(struct ieee80211_node *ni)
{
    ni->ni_whc_rept_info = 1;
}

static INLINE void
ieee80211node_clear_whc_rept_info(struct ieee80211_node *ni)
{
    ni->ni_whc_rept_info = 0;
}

static INLINE u_int8_t
ieee80211node_get_whc_rept_info(struct ieee80211_node *ni)
{
    return ni->ni_whc_rept_info;
}
