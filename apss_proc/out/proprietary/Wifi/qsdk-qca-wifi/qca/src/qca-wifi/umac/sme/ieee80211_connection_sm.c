...
extern struct ieee80211_ath_channel *ieee80211_find_channel(struct ieee80211com *ic, int freq, u_int8_t des_cfreq2, u_int32_t flags);
...
static bool ieee80211_connection_state_connecting_event(void *ctx, u_int16_t event, u_int16_t event_data_len, void *event_data) 
{
    ...
    case IEEE80211_CONNECTION_EVENT_ASSOC_SUCCESS: 
...
#if OL_ATH_SMART_LOGGING
        {
            ...
            if (ic->ic_smart_log_connection_fail_stop) {
                /* Currently, we will not alter the rest of the state machine
                 * flow based on whether we succeeded or failed in stopping
                 ...
                 */
                ic->ic_smart_log_connection_fail_stop(ic);
            }
        }
...
void  wlan_connection_sm_delete(wlan_connection_sm_t smhandle)
{
    ...
    if (wlan_scan_unregister_event_handler(smhandle->vap_handle, ieee80211_connection_sm_scan_evhandler, smhandle) != EOK) {
        IEEE80211_DPRINTF(smhandle->vap_handle,IEEE80211_MSG_STATE,"%s : cannot unregister scan event handler (%pK,%pK)\n", 
