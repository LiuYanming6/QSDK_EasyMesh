...
HAL_STATUS
ar9300_proc_qs_rx_desc_fast(struct ath_hal *ah, struct ath_desc *ds,
    u_int32_t pa, struct ath_desc *nds, struct ath_rx_status *rxs,
    void *buf_addr)
{
    struct qcn5500_rxs *rxsp = QCN5500RXS(buf_addr);

    /*
       ath_hal_printf(ah,"CHH=RX: ds_info 0x%x  status1: 0x%x  Jet_status13: 0x%x\n",
       rxsp->ds_info,rxsp->status1,rxsp->status13);
     */

    if ((rxsp->status13 & QCN_rx_done) == 0) {
    ...
    rxs->rs_rssi_ctl3 = MS(rxsp->status12, QCN_rx_rssi_ant03);
    ...
    rxs->rs_rssi_ext3 = MS(rxsp->status12, QCN_rx_rssi_ant13);
    if (rxsp->status13 & QCN_rx_key_idx_valid) {
        rxs->rs_keyix = MS(rxsp->status13, QCN_key_idx);
    } else {
    ...
    rxs->rs_isaggr = (rxsp->status13 & QCN_rx_aggr) ? 1 : 0;
    rxs->rs_moreaggr = (rxsp->status13 & QCN_rx_more_aggr) ? 1 : 0;
#if UNIFIED_SMARTANTENNA
    rxs->rs_antenna = MS(rxsp->status4, AR_rx_antenna);
#else
    rxs->rs_antenna = (MS(rxsp->status4, AR_rx_antenna) & 0xf);
#endif
    rxs->rs_isapsd = (rxsp->status13 & QCN_apsd_trig) ? 1 : 0;
    rxs->rs_flags  = (rxsp->status4 & AR_gi) ? HAL_RX_GI : 0;
    if (!((rxsp->status13 & QCN_rx_aggr) && ((rxsp->status13 & QCN_rx_first_aggr) || rxsp->status13 & QCN_rx_more_aggr)))
    {
     ...
     */
    rxs->rx_hw_upload_data_type = MS(rxsp->status13, QCN_hw_upload_data_type);
#if 0
    if (((rxsp->status13 & QCN_rx_frame_ok) == 0) &&
            ((rxsp->status4 & AR_rx_not_sounding) == 0))
    {
        HDPRINTF(AH_NULL, HAL_DBG_UNMASKABLE,
                "=>%s:sounding, Ness= %d,Rx_2:%x,Rx4:%x, Jet_Rx_13:%x,rate %d\n",
                __func__, MS(rxsp->status4, AR_rx_ness),
                rxsp->status2, rxsp->status4, rxsp->status13, rxs->rs_rate);
        HDPRINTF(AH_NULL, HAL_DBG_UNMASKABLE,
    ...
    rxs->evm4 = rxsp->status10;
    rxs->evm5 = rxsp->status11;

    if (rxsp->status13 & QCN_pre_delim_crc_err) {
        rxs->rs_flags |= HAL_RX_DELIM_CRC_PRE;
    }
    if (rxsp->status13 & QCN_post_delim_crc_err) {
        rxs->rs_flags |= HAL_RX_DELIM_CRC_POST;
    }
    if (rxsp->status13 & QCN_decrypt_busy_err) {
        rxs->rs_flags |= HAL_RX_DECRYPT_BUSY;
    }
    if (rxsp->status13 & QCN_hi_rx_chain) {
        rxs->rs_flags |= HAL_RX_HI_RX_CHAIN;
    }
    if (rxsp->status13 & QCN_key_miss) {
    ...
    if ((rxsp->status13 & QCN_rx_frame_ok) == 0) {
        ...
        if (rxsp->status13 & QCN_crc_err) {
            rxs->rs_status |= HAL_RXERR_CRC;
            /*
             * ignore CRC flag for phy reports
             */
            if (rxsp->status13 & QCN_phyerr) {
                u_int phyerr = MS(rxsp->status13, QCN_phy_err_code);
                rxs->rs_status |= HAL_RXERR_PHY;
                rxs->rs_phyerr = phyerr;
            }
        } else if (rxsp->status13 & QCN_phyerr) {
            u_int phyerr;
             ...
             * (EV 70071)
             */
            phyerr = MS(rxsp->status13,QCN_phy_err_code);
            if ((phyerr == HAL_PHYERR_OFDM_RESTART) &&
                    (rxsp->status13 & QCN_post_delim_crc_err)) {
                rxs->rs_phyerr = 0;
            } else {
                rxs->rs_status |= HAL_RXERR_PHY;
                rxs->rs_phyerr = phyerr;
            }
        } else if (rxsp->status13 & QCN_decrypt_crc_err) {
            rxs->rs_status |= HAL_RXERR_DECRYPT;
        } else if (rxsp->status13 & QCN_michael_err) {
            rxs->rs_status |= HAL_RXERR_MIC;
        }
    }
#if ATH_SUPPORT_WIFIPOS
    else {
        rxs->rx_position_bit = (rxsp->status13 & QCN_position_bit) ? 1 : 0;
#ifdef ATH_SUPPORT_TxBF
        if (rxs->rx_position_bit && rxs->rx_hw_upload_data &&
                rxs->rx_hw_upload_data_valid && rxs->rx_hw_upload_data_type == 1) {
            rxs->hdump = (u_int8_t *)((char *)rxsp + sizeof(struct qcn5500_rxs));
        }
#endif
    }
#endif
    ...
    void *buf_addr)
{
     ...
     */
    if (AR_SREV_JET(ah)) {
        retval = ar9300_proc_qs_rx_desc_fast(ah, ds, pa, nds, rxs, buf_addr);
        return retval;
    }
    ...
    rxs->rs_rssi_ctl1 = MS(rxsp->status1, AR_rx_rssi_ant01);
    rxs->rs_rssi_ctl2 = MS(rxsp->status1, AR_rx_rssi_ant02);
    rxs->rs_rssi_ctl3 = HAL_RSSI_BAD;
    rxs->rs_rssi_ext0 = MS(rxsp->status5, AR_rx_rssi_ant10);
    rxs->rs_rssi_ext1 = MS(rxsp->status5, AR_rx_rssi_ant11);
    rxs->rs_rssi_ext2 = MS(rxsp->status5, AR_rx_rssi_ant12);
    rxs->rs_rssi_ext3 = HAL_RSSI_BAD;
...
void ar9300_fill_qs_radiotap_hdr(struct ath_hal *ah,
        ...
        void *buf_addr)
{
    ...
    if (rh != NULL) {
        ...
        if (rxsp->status13 & QCN_rx_aggr) {
            rh->wr_rx_flags |= AH_RADIOTAP_11NF_RX_AGGR;
        }
        if (rxsp->status13 & QCN_rx_first_aggr) {
            rh->wr_rx_flags |= AH_RADIOTAP_11NF_RX_FIRSTAGGR;
        }
        if (rxsp->status13 & QCN_rx_more_aggr) {
        ...
        if (rxsp->status13 & QCN_decrypt_crc_err) {
        ...
        if (rxsp->status13 & QCN_phyerr) {
            ...
            rh->ath_ext.phyerr_code = MS(rxsp->status13, QCN_phy_err_code);
        }
        ...
        if (rxsp->status13 & QCN_post_delim_crc_err) {
            ...
            rh->ath_ext.ch_info.rssi_ctl[3] =
                MS(rxsp->status12, QCN_rx_rssi_ant03);
    ...
    if (ppi != NULL) {
        ...
        if (pmacphy != NULL) {
            ...
            pmacphy->macphy_rssi_ant3_ctl = MS(rxsp->status12, QCN_rx_rssi_ant03);
            ...
            pmacphy->macphy_rssi_ant3_ext = MS(rxsp->status12, QCN_rx_rssi_ant13);
                           ...
                           void *buf_addr)
{
    ...
    if (AR_SREV_JET(ah)) {
        ar9300_fill_qs_radiotap_hdr(ah, rh, ppi, ds, buf_addr);
        return;
    }
    if (rh != NULL) {
        /* Fill out Radiotap header */
        ...
        if (rxsp->status11 & AR_post_delim_crc_err) {
            ...
            rh->ath_ext.ch_info.rssi_ctl[2] =
                MS(rxsp->status1, AR_rx_rssi_ant02);
            rh->ath_ext.ch_info.rssi_ctl[3] = HAL_RSSI_BAD;
            ...
            rh->ath_ext.ch_info.rssi_ext[2] =
                MS(rxsp->status5, AR_rx_rssi_ant12);
            rh->ath_ext.ch_info.rssi_ext[2] = HAL_RSSI_BAD;

            rh->ath_ext.ch_info.evm[0] = rxsp->AR_rx_evm0;
            rh->ath_ext.ch_info.evm[1] = rxsp->AR_rx_evm1;
            rh->ath_ext.ch_info.evm[2] = rxsp->AR_rx_evm2;
            rh->ath_ext.ch_info.evm[2] = HAL_EVM_BAD;
        }
    ...
    if (ppi != NULL) {        
        ...
        if (pmacphy != NULL) {
            ...
            pmacphy->macphy_rssi_ant1_ctl = MS(rxsp->status1, AR_rx_rssi_ant01);
            pmacphy->macphy_rssi_ant2_ctl = MS(rxsp->status1, AR_rx_rssi_ant02);
            pmacphy->macphy_rssi_ant3_ctl = HAL_RSSI_BAD;
            pmacphy->macphy_rssi_ant0_ext = MS(rxsp->status5, AR_rx_rssi_ant10);
            pmacphy->macphy_rssi_ant1_ext = MS(rxsp->status5, AR_rx_rssi_ant11);
            pmacphy->macphy_rssi_ant2_ext = MS(rxsp->status5, AR_rx_rssi_ant12);
            pmacphy->macphy_rssi_ant3_ext = HAL_RSSI_BAD;
            ...
            pmacphy->macphy_ant2_signal =
                MS(rxsp->status1, AR_rx_rssi_ant02) - 96;
            pmacphy->macphy_ant3_signal = HAL_RSSI_BAD;
            
            pmacphy->macphy_ant0_noise = -96;
            pmacphy->macphy_ant1_noise = -96;
            pmacphy->macphy_ant2_noise = -96;
            pmacphy->macphy_ant3_noise = HAL_RSSI_BAD;
            
            pmacphy->macphy_evm0 = rxsp->AR_rx_evm0;
            pmacphy->macphy_evm1 = rxsp->AR_rx_evm1;
            pmacphy->macphy_evm2 = rxsp->AR_rx_evm2;
            pmacphy->macphy_evm3 = HAL_EVM_BAD;
        }
    }
}
