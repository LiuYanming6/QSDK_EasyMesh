...
#define STAT_FMT(x,fmt) \
    ...
    STAT(tx_pr_xretries, "tx failed 'cuz too many retries for probe response");
    STAT(tx_hw_success, "tx successes indicated by hw");
    STAT(tx_hw_pr_success, "tx successes indicated by hw, for probe response");
...
/* PHY statistics */
    ...
    if (IS_RSSI_VALID(stats->ast_tx_rssi_ctl3))
        fprintf(fd, "rssi of last ack[ctl, ch3]: %u\n",
                stats->ast_tx_rssi_ctl3);
    ...
    if (IS_RSSI_VALID(stats->ast_tx_rssi_ext3))
        fprintf(fd, "rssi of last ack[ext, ch3: %u\n",
                stats->ast_tx_rssi_ext3);
    ...
    if (IS_RSSI_VALID(stats->ast_rx_rssi_ctl3))
        fprintf(fd, "rssi of last rcv[ctl, ch3]: %u\n",
                stats->ast_rx_rssi_ctl3);
    ...
    if (IS_RSSI_VALID(stats->ast_rx_rssi_ext3))
        fprintf(fd, "rssi of last rcv[ext, ch3]: %u\n",
                stats->ast_rx_rssi_ext3);
    ...
    STAT(tx_hw_pr_retries, "number of transmit retries for probe response in hardware");
    STAT(rx_data_bytes, "number of data bytes received");
    ...
    fprintf(fd, "VOW STATS: ul_tx_calls: %d %d %d %d ath_txq_calls: %d %d %d %d drops(be/bk): %d %d\n",
        ...
        fprintf(fd, "Timestamp  RSSIC0 RSSIC1 RSSIC2 RSSIC3 RSSIE0 RSSIE1 RSSIE2 RSSIE3 RSSI  EVM0 EVM1 EVM2 EVM3 RXRATE TXFRAMECNT RXFRAMECNT RXCLRCOUNT RXEXCLRCNT CYCLECOUNT\n");
    for (i = 0; i < ATH_STATS_VI_LOG_LEN; i++) {
                fprintf(fd, "0x%08x %06d %06d %06d %06d %06d %06d %06d %06d %06d %4hhd %4hhd %4hhd %4hhd 0x%04x %010d %010u %010u %010u %010u\n",
                             stats->vi_timestamp[i], stats->vi_rssi_ctl0[i], stats->vi_rssi_ctl1[i], stats->vi_rssi_ctl2[i],stats->vi_rssi_ctl3[i],
                             stats->vi_rssi_ext0[i], stats->vi_rssi_ext1[i], stats->vi_rssi_ext2[i],stats->vi_rssi_ext3[i], stats->vi_rssi[i],
                             stats->vi_evm0[i], stats->vi_evm1[i], stats->vi_evm2[i],  stats->vi_evm3[i], stats->vi_rs_rate[i],
    ...
    fprintf(fd,"\n mcs24- mcs31 STATS:");
    for (i=0 ;i<8 ;i++)
        fprintf(fd,"%6d,",stats->ast_mcs_count[i+24]);
    fprintf(fd, "\n");
    ...
    fprintf(fd," mcs 0- mcs 7 STATS:");
    for (i = 0; i< 8; i++)
        fprintf(fd,"%6u,",stats->ast_rx_rate_packets[i]);
    fprintf(fd,"\n mcs 8- mcs15 STATS:");
    for (i = 0;i < 8; i++)
        fprintf(fd,"%6u,",stats->ast_rx_rate_packets[i+8]);
    fprintf(fd,"\n mcs16- mcs23 STATS:");
    for (i = 0; i < 8; i++)
        fprintf(fd,"%6u,",stats->ast_rx_rate_packets[i+16]);
    fprintf(fd,"\n mcs24- mcs31 STATS:");
    for (i = 0; i < 8; i++)
        fprintf(fd,"%6u,",stats->ast_rx_rate_packets[i+24]);
    fprintf(fd,"\n");
    ...
    fprintf(fd,"\nRandom Number Generation Stats:\n");

    fprintf(fd,"random : RSSI sync counts (mode 1): %d\n", stats->ast_rssi_sync_count_m1);
    fprintf(fd,"random : last RSSI sync timestamp (mode 1/2): %d\n\n", stats->last_rssi_sync_m1_m2);

    fprintf(fd,"random : RSSI sync counts (mode 2): %d\n", stats->ast_rssi_sync_count_m2);
    fprintf(fd,"random : ADC sync counts (mode 2): %d\n", stats->ast_adc_sync_count_m2);
    fprintf(fd,"random : last ADC sync timestamp (mode 2): %d\n\n", stats->last_adc_sync_m2);

    fprintf(fd,"random : ADC sync counts (mode 3): %d\n", stats->ast_adc_sync_count_m3);
    fprintf(fd,"random : last ADC sync timestamp (mode 3): %d\n\n",stats->last_adc_sync_m3);
#endif
...
ol_radiostats_print(const struct ol_ath_radiostats *radio_stats, const struct wlan_dbg_stats *wlan_pdev_stats, FILE *fd)
{
    ...
    fprintf(fd,"ast_rx_data_bytes   :\t%u\n",radio_stats->rx_data_bytes);
