...
extern unsigned int skb_reuse_enable;
#endif
...
#if !ATH_RXBUF_RECYCLE
        TAILQ_INIT(&sc->sc_rxrecyclebuf);
        OS_INIT_TIMER(sc->sc_osdev, &sc->sc_rxbuf_refill_timer,
                  ath_rx_refill_try, sc, QDF_TIMER_TYPE_WAKE_APPS);
        sc->sc_rxbuf_refill_trigger = 1;
        sc->sc_rxbuf_reuse_enable = skb_reuse_enable;
#endif
...
#if !ATH_RXBUF_RECYCLE
    sc->sc_rxbuf_refill_trigger = 0;
    OS_FREE_TIMER(&sc->sc_rxbuf_refill_timer);
#endif
    TAILQ_FOREACH(bf, &sc->sc_rxbuf, bf_list) {
        wbuf = bf->bf_mpdu;
        if (wbuf) {
        ...
        ieee80211_rx_status_t *rx_status, u_int8_t *chainreset)
{
     ...
     */
    {
        ...
        if (ath_green_ap_sc_get_enable_print(sc->sc_ieee)) {
            if (!(print_now & 0xff)) {
                  qdf_print("Rx rssi0 %d rssi1 %d rssi2 %d rssi3 %d\n",
    ...
    if ((rxs->rs_rate >= 0x80) && ((rxs->rs_rate <= 0x9F))) {
    ...
    if (type == IEEE80211_FC0_TYPE_MGT) {
        ...
        sc->sc_stats.ast_rx_data_bytes += wbuf_get_pktlen(wbuf);
    }
...
#else
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "RX_LOOPLIMIT enabled in non-supported device\n");
#endif
    }
#endif
    return 0;
...
#define ATH_RX_CHAINMASK_CLR(_chainmask, _chain) ((_chainmask) &= ~(1 << (_chain)))
    ...
    case 0:
        ...
        if (rssiRef - rxstat->rs_rssi_ctl3 > detectDelta) {
            ATH_RX_CHAINMASK_CLR(rx_chainmask, 3);
        }
        break;
...
#define MICROSEC_PER_SEC       1000000
    ...
    if (sc->sc_vi_dbg_start == 0 || (sc->sc_vi_dbg_start == 1 && timestamp_diff >= MICROSEC_PER_SEC)) {
        ...
        stats->vi_rssi_ctl3[index]    = rxs->rs_rssi_ctl3;
        ...
        stats->vi_rssi_ext3[index]    = rxs->rs_rssi_ext3;
...
{
    if (sc->sc_enhanceddmasupport) {
        ath_rx_edma_refill_queue(sc, bf);
    } else {
        ATH_RXBUF_LOCK(sc);
        TAILQ_INSERT_TAIL(&sc->sc_rxrecyclebuf, bf, bf_list);
        ATH_RXBUF_UNLOCK(sc);
    }
    ATH_RXBUF_LOCK(sc);
    if (sc->sc_rxbuf_refill_trigger == 1) {
        OS_SET_TIMER(&sc->sc_rxbuf_refill_timer, ATH_RX_REFILL_TIMER_MS);
        sc->sc_rxbuf_refill_trigger = 0;
    }
    ATH_RXBUF_UNLOCK(sc);
}
...
void ath_rx_refill_try(void *arg)
{
    ...
    wbuf_t nwbuf;
    ATH_RXBUF_LOCK(sc);
    if (!(TAILQ_EMPTY(&sc->sc_rxrecyclebuf))) {
        TAILQ_FOREACH_SAFE(bf, &sc->sc_rxrecyclebuf, bf_list, tbf){

            nwbuf = ath_rxbuf_alloc(sc, sc->sc_rxbufsize);
            if (nwbuf != NULL) {
                TAILQ_REMOVE(&sc->sc_rxrecyclebuf, bf, bf_list);
                bf->bf_mpdu = nwbuf;
...
EXPORT_SYMBOL(ath_rx_refill_try);
EXPORT_SYMBOL(ath_rx_refill_queue);
#endif
