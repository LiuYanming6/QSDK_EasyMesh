    ...
    qdf_nbuf_t msdu_list)
{
...
#endif
            (scn->dbg.err_types.pn_errors)++;
            if ((scn->dbg.err_types.pn_errors % scn->dbg.print_rate_limit) == 0) {
                ...
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "PN check failed - TID %d, peer %pK "
                        ...
                        htt_rx_mpdu_desc_seq_num(pdev->htt_pdev, rx_desc), scn->dbg.err_types.pn_errors);
    ...
    if(peer->pn_replay_counter > MAX_ALLOWED_CONSECUTIVE_PN_FAILURE) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Kicking out node %x:%x:%x:%x:%x:%x due to excessive PN replays\n", peer->mac_addr.raw[0], peer->mac_addr.raw[1],
