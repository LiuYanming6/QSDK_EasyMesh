...
extern void ol_ath_wlanstats_peer_init(struct ieee80211com *ic,
        struct ieee80211_node *ni);
extern void ol_ath_wlanstats_peer_deinit(struct ieee80211com *ic,
        struct ieee80211_node *ni);
#endif
...
set_node_wep_keys(struct ieee80211vap *vap, const u_int8_t *macaddr)
{
    ...
    wkey = (struct ieee80211_key *)OS_MALLOC(scn->sc_osdev,
						sizeof(struct ieee80211_key),
                                                0);
    if (!wkey) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_SYS, QDF_TRACE_LEVEL_INFO, "Can't create an wkey due to no Memory\n");
        return;
    }
    ...
    if (!tmpnode && !is_node_self_peer(vap, macaddr) && (vap->iv_opmode != IEEE80211_M_MONITOR)) {
        ...
        if (qdf_atomic_dec_and_test(&scn->peer_count)) {
            AR_DEBUG_PRINTF(ATH_DEBUG_INFO,
                    ("%s: vap (%pK) scn (%pK) the peer count exceeds the supported number %d \n",
...
attach_failed:
    ...
    if (wmi_unified_peer_delete_send(scn->wmi_handle, (u_int8_t *)macaddr, avn->av_if_id)) {
        qdf_print("%s : Unable to Delete peer in Target \n", __func__);
        goto wmi_cmd_failed;
    }
err_node_alloc:
...
static int
ol_ath_node_cfr_capture(struct ieee80211_node *ni)
{
    struct ieee80211com *ic = ni->ni_ic;
    struct ol_ath_softc_net80211 *scn = OL_ATH_SOFTC_NET80211(ic);
    ...
    qdf_mem_set(&param, sizeof(param), 0);
    param.request = ni->ni_cfr.request;
    param.macaddr = ni->ni_macaddr;
    param.vdev_id = avn->av_if_id;
    param.periodicity = ni->ni_cfr.period;
    param.bandwidth = ni->ni_cfr.bw;
    param.capture_method = ni->ni_cfr.method;

    retv = wmi_unified_peer_cfr_capture_send(scn->wmi_handle, &param);
    if (retv)
        qdf_print("%s:Unable to send wmi cfr params\n", __func__);
...
#if ATH_SUPPORT_GREEN_AP
        if ((ic->ic_opmode == IEEE80211_M_HOSTAP) && (ni->ni_associd > 0)
                && (ni->is_sta_node) && (ni != ni->ni_bss_node)) {
            ath_green_ap_state_mc(ic, ATH_PS_EVENT_DEC_STA);

            if (wlan_node_get_max_nss(ni) > 1)
                ath_green_ap_state_mc(ic, ATH_PS_EVENT_DEC_MULTISTREAM_STA);
        }
#endif  /* ATH_SUPPORT_GREEN_AP */
...
ol_ath_node_getrate(const struct ieee80211_node *ni, u_int8_t type)
{
   if (type == IEEE80211_RATE_TX) {
        ...
        return (OL_ATH_NODE_NET80211(ni))->an_ni_rx_ratecode;
    } else if (type == IEEE80211_RATEFLAGS_RX) {
        return (OL_ATH_NODE_NET80211(ni))->an_ni_rx_flags;
    } else {
        return 0;
    }
   return 0;
...
ol_ath_node_set_bridge_mac_addr(struct ieee80211com *ic, u_int8_t *bridge_mac)
{
    ...
    if (wmi_unified_set_bridge_mac_addr_cmd_send(scn->wmi_handle, &param)){
        printk("%s:Unable to set bridge MAC address\n", __func__);
    }
}
...
ol_ath_node_authorize(struct ieee80211_node *ni, u_int32_t authorize)
{
    ...
    */
    ...
    if(authorize && (RSN_AUTH_IS_WPA(&ni->ni_rsn) ||
                  ...
                  && (ni->ni_vap->iv_rsn.rsn_keymgmtset  != RSN_AUTH_KEY_MGMT_OWE)
      ) {
       return;
    }
#if MESH_MODE_SUPPORT
...
ol_ath_node_attach(struct ol_ath_softc_net80211 *scn, struct ieee80211com *ic)
{
    ...
    ic->ic_node_cfr_capture = ol_ath_node_cfr_capture;
...
#endif
#ifdef ATH_SUPPORT_RDKB
    ic->ic_wlanstats_peer_init = ol_ath_wlanstats_peer_init;
    ic->ic_wlanstats_peer_deinit = ol_ath_wlanstats_peer_deinit;
#endif
