...
static struct profileElement estimatorElementDefaultTable[] = {
    ...
    { ESTIMATOR_ACT_DETECT_MIN_INTERVAL_KEY,          "30"},
    { ESTIMATOR_ACT_DETECT_MIN_PKT_PER_SEC_KEY,        "2"},
    { NULL, NULL }
};
 ...
 */
static wlanif_chwidth_e estimatorCmnResolveChannelBandwidth(stadbEntry_handle_t entry) {
    if(!entry) {
        // Assume 20 MHz by default
        return wlanif_chwidth_20;
    }
    // TODO: better estimate of the channel bandwidth. Assuming 20MHz for now,
    // as we don't really know over which bandwidth the RSSI was measured.
    return wlanif_chwidth_20;
}
...
static LBD_STATUS estimatorCmnPerform11kMeasurement(stadbEntry_handle_t entry,
    ...
    if (stadbEntry_getActStatus(entry, &state->activeBefore11k, NULL) == LBD_OK) {
        ...
        if (dot11kChannel != LBD_CHANNEL_INVALID) {
            ...
            if (estimatorRequestDownlinkRSSI(entry,
                                             bssStats,
                                             servingBSS,
                                             1 /* numChannels */,
                                             &dot11kChannel) == LBD_OK) {
...
static LBD_STATUS estimatorCmnEstimateSTADataMetricsImpl(
    ...
    // There are two cases where we need to enable the stats:
...
static const char *estimatorMenuAirtimeHelp[] = {
    ...
    "Usage:",
    "\tairtime <channel> [<ap id>]: estimate for the specified channel",
    NULL
};
...
void estimatorMenuAirtimeHandler(struct cmdContext *context,
    ...
    if (estimator_estimatePerSTAAirtimeOnChannel(apId, channel) != LBD_OK) {
