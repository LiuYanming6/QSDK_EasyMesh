...
typedef struct steeralgPriv_t {
    ...
    struct {
        ...
        /// The min ratio between a path capacity and the max path capacity. Only
        /// path capacities above the threshold are considered for steering
        u_int8_t apSteerTopCapThreshold[wlanif_band_invalid];

        /// The max number of mobility steering attempts allowed for a STA
        ...
        // If set to be 1, the client's estimated air time will be added
        // to the channel's measured air time when making decision for steering.
        LBD_BOOL applyEstimatedAirTimeOnSteering;

        // If true, select target BSS based on end-to-end path capacity.
        LBD_BOOL usePathCapToSelectBSS;
    } config;
...
#ifdef LBD_DBG_MENU
/**
 * @brief Print the path capacity from a target BSS to CAP for a STA
 *
 * This is only supported in MBSA.
 *
 * @param [in] context  The operating context
 * @param [in] cmd  The debug cli command
 */
void steeralgMenuPathCapacityHandler(struct cmdContext *context, const char *cmd);
#endif /*LBD_DBG_MENU*/
