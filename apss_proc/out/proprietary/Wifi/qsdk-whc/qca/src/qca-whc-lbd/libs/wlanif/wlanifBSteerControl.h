...
LBD_STATUS wlanifBSteerControlSendBTMRequestMapParams(
    ...
    const struct ether_addr *staAddr, u_int8_t dialogToken, u_int16_t disassocTimer,
    LBD_BOOL disassocImminent, LBD_BOOL abridgedBit, const wlanif_bssTargetInfo_t *candidate);
 ...
 *
 * @return LBD_OK on successfully set overload or no radios to set;
 *         otherwise LBD_NOK
 */
 ...
 *                    the candidate list
 * @param [in] servingBSSInfo Current info of the serving BSS
 * @param [in] validitySecs seconds to block the STA. If 0
 *                          then its infinitely valid
 *
 * @return LBD_STATUS LBD_OK if the state could be set, LBD_NOK
 *                    otherwise
 */
 ...
 *        the excluded interfaces .
 *
 * @param [in] handle  the handle returned from
 *                     wlanifBSteerControlCreate()
 * @param [in] staAddr the MAC address of the STA
 * @param [in] enable if LBD_TRUE, will enable for all
 *                     the excluded interfaces
 *
 * @return LBD_STATUS LBD_OK if the state could be set, LBD_NOK
 *                    otherwise
...
/**
 * @brief Run an ioctl operation that takes a single MAC address on all
 *        the vaps on the other ESS .
 *
 * @param [in] handle  the handle returned from
 *                     wlanifBSteerControlCreate()
 * @param [in] lastServing ESS  Last ESS the STA
 *                              was associated on
 * @param [in] staAddr the MAC address of the STA
 * @param [in] enable if LBD_TRUE, will enable for all
 *                     the VAPS on the other ESS
 ...
 * @param [out] bss structure to be filled in with BSS information
 *
 * @return LBD_STATUS LBD_OK if the BSS was found, LBD_NOK
 *                    otherwise
 */
...
/**
 * @brief Get Radio MAC address from BSS
 *
 * @param [in] handle  the handle returned from wlanifBSteerControlCreate()
 * @param [in] bss  the BSS
 * @param [out] radioAddr  MAC address of the Radio
 *
 * @return LBD_OK if the success; otherwise return LBD_NOK
 */
...
#define WLANIFBSTEERCONTROL_WLAN_INTERFACES_EXCLUDED "WlanInterfacesExcluded"
#define WLANIFBSTEERCONTROL_WLAN_RADIO_VENDORMAP "RadioToVendorMap"
#define WLANIFBSTEERCONTROL_ALLOW_ZERO_AP_INTERFACES "AllowZeroAPInterfaces"
...
#define WLANIFBSTEERCONTROL_BLACKLIST_OTHER_ESS "BlacklistOtherESS"
#define WLANIFBSTEERCONTROL_INACT_DETECTION_FROM_TX "InactDetectionFromTx"
