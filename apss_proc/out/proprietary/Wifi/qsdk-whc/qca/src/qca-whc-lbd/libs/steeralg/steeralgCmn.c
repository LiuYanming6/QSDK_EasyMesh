...
static void steeralgCmnUpdatePHYBasedPrioritization(void);
 ...
 *       prioritization when remote-only channels are added.
 */
static void steeralgCmnUpdatePHYBasedPrioritization(void) {
    if (steeralgState.numActiveChannels >= WLANIF_MAX_RADIOS &&
        steeralgState.config.phyBasedPrioritization) {
        dbgf(steeralgState.dbgModule, DBGERR,
             "%s: PHY based prioritization should be disabled for three radio "
             "platform",
             __func__);
...
static void steeralgCmnOffloadOverloadedChannel(size_t numOverloadedChannels,
    ...
    if (numChannels != steeralgState.numActiveChannels) {
        dbgf(steeralgState.dbgModule, DBGINFO,
             "%s: Number of active channels does not match: Expect %u, actual %zu. Updating...",
             __func__, steeralgState.numActiveChannels, numChannels);
        steeralgState.numActiveChannels = numChannels;

        // Since number of active channels has changed, PHY based
        // prioritization may need to be disabled
        steeralgCmnUpdatePHYBasedPrioritization();
    }
    ...
    overloadedChannelCount = bandmon_getOverloadedChannelList(overloadedChannels, WLANIF_MAX_RADIOS);
    ...
    if (LBD_NOK == estimator_estimatePerSTAAirtimeOnChannel(apId, overloadedSrcChannel)) {
...
static void steeralgCmnHandlePerSTAAirtimeCompleteEvent(struct mdEventNode *event) {
    ...
    do {
        steeralgCmnSelectOffloadingCandidatesParams_t params = {
            airtimeCompleteEvent->apId, airtimeCompleteEvent->channelId,
