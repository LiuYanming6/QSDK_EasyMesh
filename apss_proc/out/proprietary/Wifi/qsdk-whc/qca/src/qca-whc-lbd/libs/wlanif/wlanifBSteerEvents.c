...
// Must be defined in the daemon-specific code.
void wlanifBSteerEventsInit(wlanifBSteerEventsHandle_t state);
...
#else
    state->probeDbgModule = dbgModuleFind("probe");
    //Do not fail lbd init when NL socket is not created
    //When SON Lib is enabled, NL socket is created during
    //Son Lib init where ifname-vendor mapping is created

    //Send EventsHandler to SonLib to be used while recieving events
    soncmnSetLbdEventHandler(state);
#endif

    // Allow the "derived" logic to run.
    wlanifBSteerEventsInit(state);
    return state;
}
...
LBD_STATUS wlanifBSteerEventsEnable(wlanifBSteerEventsHandle_t state, u_int32_t sysIndex) {
    ...
    if(if_indextoname (sysIndex, ifname) == NULL) {
        dbgf(state->dbgModule, DBGERR, "%s: Failed to obtain ifname for sysIndex: %d",
             __func__, sysIndex);
        return LBD_NOK;
    }
    ...
    if (drvHandle->ops->enableBSteerEvents(sysIndex) < 0)
    {
...
LBD_STATUS wlanifBSteerEventsHandleRemoteAssocUpdate(
        wlanifBSteerEventsHandle_t state, const struct ether_addr *staAddr,
        const struct ether_addr *bssid, LBD_BOOL isAssoc) {
    if (!state || !staAddr || !bssid) {
        return LBD_NOK;
    }

    lbd_bssInfo_t assocBSS;
    if (wlanifBSteerControlGetBSSInfoFromBSSID(state->bsteerControlHandle,
                                               bssid->ether_addr_octet,
                                               &assocBSS) != LBD_OK) {
        dbgf(state->dbgModule, DBGERR,
             "%s: Received remote association notification for STA "
             lbMACAddFmt(":") " on BSSID "  lbMACAddFmt(":")
             " but can't resolve BSS, ignoring",
             __func__, lbMACAddData(staAddr->ether_addr_octet),
             lbMACAddData(bssid->ether_addr_octet));
        return LBD_NOK;
    }

    wlanif_event_e eventType = isAssoc ? wlanif_event_assoc :
                                         wlanif_event_disassoc;

    // Most of the fields are not available for a remote association, so
    // just fill in what is possible.
    wlanif_assocEvent_t assocEvent;
    lbCopyBSSInfo(&assocBSS, &assocEvent.bss);
    ...
    assocEvent.bandCap = 0;
    assocEvent.isMUMIMOSupported = wlanif_cap_unchanged;

    assocEvent.isStaticSMPS = wlanif_cap_unchanged;
    assocEvent.phyCapInfo.valid = LBD_FALSE;

    mdCreateEvent(mdModuleID_WlanIF, mdEventPriority_Low,
                  eventType, &assocEvent, sizeof(assocEvent));
...
//When SonLib is enabled, NL socket is created in Son Lib init
#if !SONLIB_SUPPORT_ENABLED
...
static void wlanifBSteerEventsHandleNodeAssociatedInd(
    ...
    dbgf(state->dbgModule, DBGINFO,
         ...
         "Max bandwidth: %u, Num of spatial streams: %u, PHY mode: %u, Max MCS: %u. Max TX power: %u, bandcap: %u",
         ...
         event->data.bs_node_associated.band_cap);
    ...
    assocEvent.rrmStatus =
        event->data.bs_node_associated.isRRMSupported ? wlanif_cap_enabled :
                                                        wlanif_cap_disabled;
    assocEvent.bandCap = event->data.bs_node_associated.band_cap;
    assocEvent.isMUMIMOSupported =
        event->data.bs_node_associated.datarate_info.is_mu_mimo_supported ?
        wlanif_cap_enabled : wlanif_cap_disabled;

    assocEvent.isStaticSMPS =
        event->data.bs_node_associated.datarate_info.is_static_smps ?
        wlanif_cap_enabled : wlanif_cap_disabled;
...
/**
 * @brief Handle the 802.11k beacon frame report received from driver
 *
 * @param [in] state  the "this" pointer
 * @param [in] receivingBss  the BSS from which the report is
 *                           received
 * @param [in] report  the beacon report received
 */
static void wlanifBSteerEventsHandleBeaconFrameReport(
        wlanifBSteerEventsHandle_t state,
        const lbd_bssInfo_t *receivingBss,
        const struct bs_rrm_frame_report_ind *report) {
    wlanif_beaconFrameReportEvent_t *bcnrptEvent = NULL;

    size_t eventLen = sizeof(wlanif_beaconFrameReportEvent_t) + report->data_len;
    bcnrptEvent = (wlanif_beaconFrameReportEvent_t *)malloc(eventLen);

    if (!bcnrptEvent) {
        dbgf(state->dbgModule, DBGERR, "%s: Failed to create event buffer", __func__);
        return;
    }
    ...
    if (report->measrpt_mode != IEEE80211_RRM_MEASRPT_MODE_SUCCESS) {
        bcnrptEvent->valid = LBD_FALSE;
        bcnrptEvent->numBcnrpt = 0;
        bcnrptEvent->dataLen = 0;
    } else {
        bcnrptEvent->valid = LBD_TRUE;
        bcnrptEvent->numBcnrpt = report->num_meas_rpts;
        bcnrptEvent->dataLen = report->data_len;
        memcpy(bcnrptEvent->data, report->meas_rpt_data, report->data_len);
    }

    mdCreateEvent(mdModuleID_WlanIF, mdEventPriority_Low,
                      wlanif_event_beacon_frame_report, bcnrptEvent,
                      eventLen);
...
/**
 * @brief React to an indication from the driver that an 802.11k radio
 *        resource measurement frame report has been received.
 *
 * @param [in] state  the "this" pointer
 * @param [in] event  the data sent from the kernel
 * @param [in] bss BSS the event was received from
 */
static void wlanifBSteerEventsHandleRRMFrameReportInd(
        wlanifBSteerEventsHandle_t state,
        const ath_netlink_bsteering_event_t *event,
        const lbd_bssInfo_t *bss) {
    const struct bs_rrm_frame_report_ind *report = &event->data.rrm_frame_report;
    switch (report->rrm_type) {
        case BSTEERING_RRM_TYPE_BCNRPT:
            wlanifBSteerEventsHandleBeaconFrameReport(state, bss, report);
            break;
        default:
            dbgf(state->dbgModule, DBGINFO, "%s: Unhandled RRM msg: type %u",
                 __func__, report->rrm_type);
    ...
    switch (event->data.wnm_event.wnm_type) {
        case BSTEERING_WNM_TYPE_BSTM_RESPONSE:
            dbgf(state->dbgModule, DBGDEBUG,
                 ...
                 ": Dialog Token: %u, Status: %u, Reject: %u, BSS termination "
                 "delay: %u, Target BSSID " lbMACAddFmt(":"),
                 ...
                 event->data.wnm_event.data.bstm_resp.reject_code,
            ...
            resp.reject_code = event->data.wnm_event.data.bstm_resp.reject_code;
        ...
        case BSTEERING_WNM_TYPE_BSTM_QUERY:
            dbgf(state->dbgModule, DBGDEBUG,
                 "%s: Received BTM Query from " lbMACAddFmt(":")
                 ": Dialog Token: %u, Reason: %u ",
                 __func__, lbMACAddData(event->data.wnm_event.macaddr),
                 event->data.wnm_event.dialog_token,
                 event->data.wnm_event.data.bstm_query.reason);

            wlanif_btmQueryEvent_t query;
            query.dialog_token = event->data.wnm_event.dialog_token;
            lbCopyMACAddr(event->data.wnm_event.macaddr,
                          query.sta_addr.ether_addr_octet);
            query.reason = event->data.wnm_event.data.bstm_query.reason;

            mdCreateEvent(mdModuleID_WlanIF, mdEventPriority_High,
                          wlanif_event_btm_query, &query, sizeof(query));
            break;
#endif
        default:
...
static void wlanifBSteerEventsHandleSTAStatsInd(
    ...
    for (i = 0; i < stats->peer_count; i++) {
        ...
        dbgf(state->dbgModule, DBGDUMP, "%s: Updated stats for STA " lbMACAddFmt(":")
             " on " lbBSSInfoAddFmt() " Tx bytes: %"PRIu64", Rx bytes: %"PRIu64", Tx packets: %u, "
 ...
 * @param [in] bss BSS the event was received from
 */
static void wlanifBSteerEventsHandleRSSIXingIndMap(
        wlanifBSteerEventsHandle_t state,
        const ath_netlink_bsteering_event_t *event,
        const lbd_bssInfo_t *bss) {
    dbgf(state->dbgModule, DBGDEBUG,
         "%s: " lbMACAddFmt(":") " RSSI measurement %u on " lbBSSInfoAddFmt() "; ", __func__,
         lbMACAddData(event->data.bs_rssi_xing_map.client_addr), event->data.bs_rssi_xing_map.rssi,
         lbBSSInfoAddData(bss));

    wlanif_rssiXingEventMap_t rssiXingEventMap;
    lbCopyMACAddr(event->data.bs_rssi_xing_map.client_addr,
                  rssiXingEventMap.sta_addr.ether_addr_octet);

    lbCopyBSSInfo(bss, &rssiXingEventMap.bss);

    rssiXingEventMap.rssi = event->data.bs_rssi_xing.rssi;
    rssiXingEventMap.mapRssiXing = wlanifMapToXingDirection(state->dbgModule, event->data.bs_rssi_xing_map.map_rssi_xing);

    mdCreateEvent(mdModuleID_WlanIF, mdEventPriority_Low,
                  wlanif_event_rssi_xing_map, &rssiXingEventMap, sizeof(rssiXingEventMap));
}
...
void wlanifBSteerEventsMsgRx(wlanifBSteerEventsHandle_t state,
    ...
    switch (event->type) {
        ...
        case ATH_EVENT_BSTEERING_RRM_FRAME_REPORT:
            wlanifBSteerEventsHandleRRMFrameReportInd(state, event, &bss);
            break;
        ...
        case ATH_EVENT_BSTEERING_MAP_CLIENT_RSSI_CROSSING:
            wlanifBSteerEventsHandleRSSIXingIndMap(state, event, &bss);
            break;
...
//For SON Lib, NL socket registration happens in SON Lib init
#if !SONLIB_SUPPORT_ENABLED
/**
