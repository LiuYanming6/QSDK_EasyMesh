...
/**
 * @brief The type to be used to store transaction Id
 */
typedef u_int16_t steerexec_transactionId_t;
...
/**
 * @brief Enables sending steer events to son event service module.
 *
 * @param [in] enabled  LBD_TRUE or LBD_FALSE
 *
 * @return LBD_OK on success; otherwise LBD_NOK
 */
LBD_STATUS steerexecSonEventServiceStart(int enabled);
#endif
...
/**
 * @brief Update the BTM state if needed in STA steering state
 *
 ...
 */
LBD_STATUS steerexec_updateBTMCapability(stadbEntry_handle_t entry,
...
/**
* @brief Whether the steering is subject to steering attempt limit
*
* @param [in] reason  The reason for the steer
*
* Return LBD_TRUE if the steering is subject to steering attempt limit
*/
LBD_BOOL steerexec_isSubjectToSteeringLimit(steerexec_reason_e reason);
...
#define STEEREXEC_LEGACY_UPGRADE_UNFRIENDLY_TIME_KEY "LegacyUpgradeUnfriendlyTime"

#define STEEREXEC_DISABLE_LEGACY_STEERING "DisableLegacySteering"
