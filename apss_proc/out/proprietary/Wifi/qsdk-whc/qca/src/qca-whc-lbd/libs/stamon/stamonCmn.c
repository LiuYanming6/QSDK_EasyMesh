...
static void stamonCmnHandleLegacySteeringReq(struct mdEventNode *event);
...
LBD_STATUS stamon_init(void) {
    ...
    mdListenTableRegister(mdModuleID_Estimator, estimator_event_requestLegacySteering11kSta,
                          stamonCmnHandleLegacySteeringReq);
...
static void stamonCmnTriggerActivityOrSteering(stadbEntry_handle_t entry,
    ...
    stamonMakeLegacySteerDecision(entry);
...
static void stamonCmnHandleUtilizationChange(struct mdEventNode *event) {
    ...
    if (util->numOverloadedChannels >= bandmon_getNumActiveChannels()) {
...
static void stamonCmnActiveSteerRateXing(const struct ether_addr *staAddr,
    ...
    if (xing == wlanif_xing_down) {
        ...
        trigger = steerexec_reason_activeDowngradeRate;
               ...
               stamonCmnIsEligibleForActiveUpgrade(entry, staAddr, band, txRate, rssi)) {
...
static LBD_BOOL stamonCmnIsSteerCandidate(stadbEntry_handle_t entry,
    ...
    if (servingBSS) {
        ...
        lbDbgAssertExit(stamonState.dbgModule, servingBSSInfo);

        return stamonIsBSSAllowedToInitSteering(servingBSSInfo) &&
               stadbEntry_isDualBand(entry) &&
               LBD_OK == stadbEntry_getActStatus(entry, isActive, NULL) &&
               LBD_INVALID_AIRTIME ==
                   stadbEntry_getReservedAirtime(entry, servingBSS);
    }
...
/**
 * @brief React to an event requesting for Legacy Steering for a STA
 */
static void stamonCmnHandleLegacySteeringReq(struct mdEventNode *event) {
    const estimator_staDataMetricsCompleteEvent_t *info =
        (const estimator_staDataMetricsCompleteEvent_t *)event->Data;
    stadbEntry_handle_t entry = stadb_find(&info->addr);
    if (!entry) {
        dbgf(stamonState.dbgModule, DBGERR,
             "%s: Received legacy steering request event from unknown MAC address: "
             lbMACAddFmt(":"),
             __func__, lbMACAddData(info->addr.ether_addr_octet));
        return;
    }
...
static void stamonCmnMakeSteerDecisionActive(stadbEntry_handle_t entry) {
    ...
    do {
        ...
        if (!lbIsBSSLocal(bss)) {
            stadbEntry_getFullCapacities(entry, servingBSS, NULL, &staStats.lastTxRate,
                                         NULL, &elapsedSecs);
            if (staStats.lastTxRate == LBD_INVALID_LINK_CAP ||
                elapsedSecs > stamonState.config.freshnessLimit) {
                dbgf(stamonState.dbgModule, DBGERR,
                     "%s: Failed to get DL rate information for " lbMACAddFmt(
                         ":") ". Age: %lu sec.",
                     __func__, lbMACAddData(staAddr->ether_addr_octet), elapsedSecs);
                break;
            }
        } else {
...
LBD_BOOL stamonCmnIsEligibleForActiveUpgrade(stadbEntry_handle_t entry,
    ...
    if (!servingBSSInfo) {
        dbgf(stamonState.dbgModule, DBGDEBUG,
             "%s: " lbMACAddFmt(
                 ":") " is not eligible for upgrade due to not associated to any BSS",
             __func__, lbMACAddData(staAddr->ether_addr_octet));
        return LBD_FALSE;
    }

    if (bandmon_isInSteeringBlackout(servingBSSInfo->apId)) {
