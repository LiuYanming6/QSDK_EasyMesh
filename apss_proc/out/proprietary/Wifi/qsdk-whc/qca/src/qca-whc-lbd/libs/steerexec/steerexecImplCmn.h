...
typedef struct steerexecImplCmnLegacyConfig_t {
    ...
    /// Disable the legacy steering
    LBD_BOOL disableLegacySteering;

    /// The amount of time a device should be considered as legacy upgrade steering
    /// unfriendly before being attempted again.
    u_int32_t legacyUpgradeUnfriendlyTime;
...
typedef enum steerexecImplCmnSteeringType_e {
    ...
    // Best-effort and BTM Force Steering if MBO reason
    // LoadBalancing got rejected by STA with any valid
    // Reject Code
    steerexecImplCmnSteeringType_btm_be_active_force_steer,
...
/**
 * @brief Enable/Disable SON event service.
 *
 * @param [in] exec  the executor instance to use
 * @param [in] enabled LBD_TRUE or LBD_FALSE
 *
 * @return LBD_OK on success; otherwise LBD_NOK
 */
...
/**
 * @brief Start a steer for a STA associated on a remote BSS.
 *
 * @param [in] staHandle  the handle of the STA which is being steered
 * @param [in] staAddr  the MAC address of the STA which is being
 *                      steered
 * @param [in] assocBSS  the BSS that the STA is associated to
 * @param [in] forceSteer  whether this is a forced steer
 * @param [in] candidateCount  the number of target BSSes in the list
 * @param [in] candidateList  the list of target BSSes
 * @param [out] msgTransaction  the messaging transaction ID for the
 *                              remote steer command
 *
 * @return LBD_OK if success; otherwise LBD_NOK
 */
LBD_STATUS steerexecImplStartRemoteSteer(const struct ether_addr *staAddr,
                                         const lbd_bssInfo_t *assocBSS,
                                         LBD_BOOL forceSteer, u_int8_t candidateCount,
                                         const lbd_bssInfo_t *candidateList,
                                         steerexec_transactionId_t *msgTransaction);
 ...
 * @param [in] steerType  the type of the steer being aborted
 *
 * @return LBD_OK if the abort handling was successful, LBD_NOK
 *         otherwise
 */
LBD_STATUS steerexecImplAbort(steerexec_transactionId_t transId,
...
/**
 * @brief Wrapper for calling UpdateBTMCapability from other modules
 *
 * @param [in] exec  the executor instance to use
 * @param [in] entry  the entry for which BTM capability should
 *                    be updated
 * @param [in] staAddr  MAC address of STA
 *
 * @return LBD_OK if successful, LBD_NOK otherwise
 */
