        ...
        if [ "$IS_GW_REACHABLE" -eq 0 ]; then
            if [ "$is_front_haul_VAPs_brought_down" -eq 0 ]; then
                if  __repacd_wifimon_is_timeout "$GW_NOT_REACHABLE_TIMESTAMP" \
                    ...
                    is_front_haul_VAPs_brought_down=1
                    #TWB EAP 
                    # If we reach here, that means ping-to-gateway has failed over 300 sec.
                    # But bSTA is still associated with AP.
                    # We try to resend dhcp in an attempt to recover the ping 
                    __repacd_fronthaulmgrmon_debug "Release current DHCP..."
                    kill -SIGUSR2 "$(cat "/var/run/udhcpc-br-lan.pid")"
                    __repacd_fronthaulmgrmon_debug "Renew DHCP..."
