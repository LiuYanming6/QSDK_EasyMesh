...
enable_qcawifi() {
	...
	config_get papi "$device" papi
	[ -n "$papi" ] && iwpriv "$phy" set_papi "$papi"
	...
	for vif in $vifs; do
		...
		[ -n "${DEBUG}" ] && echo wlanconfig "$ifname" create wlandev "$phy"
		if [ "$map" -gt 0 ] && [ "$mode" = "sta" ] && [ "$enable_macreq" -gt 0 ]; then
			wlanmode "$wlanmode" ${wlanaddr:+wlanaddr "$wlanaddr"} ${nosbeacon:+nosbeacon} ${vapid:+vapid "$vapid"}
		else
			wlanmode "$wlanmode" ${wlanaddr:+wlanaddr "$wlanaddr"} ${nosbeacon:+nosbeacon}
		fi
		if [ -z "$recover" ] || [ "$recover" -eq "0" ]; then
			if [ "$map" -gt 0 ] && [ "$mode" = "sta" ] && [ "$enable_macreq" -gt 0 ]; then
	...
	if [ -f "/lib/update_smp_affinity.sh" ]; then
        ...
        if [ "$device == 'wifi1' "]; then 
            ...
            echo "Dynamic Mesh Formation: enable $device, set sensitivity to -75dB" > /dev/console
        fi 
        ###
